Trigger.prototype.tutorialGoals = [
  ...
  },
  {
    instructions: [
      markForTranslation(
        "To start off, select your building, the Civic Center, by clicking on it. A selection ring in the color of your civilization will be displayed after clicking."
      ),
    ],
  },
  {
  ...
  },
  {
    instructions: [
      markForTranslation(
        "You have two main types of starting units: Female Citizens and Citizen Soldiers. Female Citizens are purely economic units; they have low health and little to no attack. Citizen Soldiers are workers by default, but in times of need, can utilize a weapon to fight. You have two categories of Citizen Soldiers: Infantry and Cavalry. Female Citizens and Infantry Citizen Soldiers can gather any land resources while Cavalry Citizen Soldiers can only gather meat from animals.\n"
      ),
    ],
  },
  {
  ...
  },
  {
    instructions: [
      markForTranslation(
        "Now, let's gather some wood with your Infantry Citizen Soldiers. Select your Infantry Citizen Soldiers and order them to gather wood by right-clicking on the nearest tree."
      ),
    ],
    OnPlayerCommand: function (msg) {
      if (
  ...
  },
  {
    instructions: [
      markForTranslation(
        "Cavalry Citizen Soldiers are good for hunting. Select your Cavalry and order him to hunt the chickens around your Civic Center in similar fashion."
      ),
    ],
    OnPlayerCommand: function (msg) {
      if (
  ...
  },
  {
    ...
    OnPlayerCommand: function (msg) {
      if (
        ...
        this.WarningMessage(
          markForTranslation(
            "Select the Civic Center, then hover the cursor over a tree and right-click when you see the cursor change into a wood icon."
          )
        );
        return;
      }
  ...
  },
  {
    instructions: [
      ...
      markForTranslation(
        "Citizen Soldiers gather wood faster than Female Citizens. Select the Civic Center and, while holding Shift, click on the second unit icon, the Hoplites (holding Shift trains a batch of five units). You can also train units individually by simply clicking, but training 5 units together takes less time than training 5 units individually."
      ),
    ],
    OnTrainingQueued: function (msg) {
      if (
        msg.unitTemplate != "units/athen/infantry_spearman_b" ||
        +msg.count == 1
      ) {
        ...
        let txt =
          +msg.count == 1
            ? markForTranslation(
                "Do not forget to hold Shift while clicking to train several units."
              )
  ...
  },
  {
    instructions: [
      ...
      markForTranslation(
        "This is a very important concept to keep in mind: gathered resources have to be brought back to a dropsite to be accounted, and you should always try to minimize the distance between resource and nearest dropsite to improve your gathering efficiency."
      ),
    ],
  ...
  },
  {
    instructions: [
      markForTranslation(
        "The newly trained units automatically go to the trees and start gathering wood.\n"
      ),
      markForTranslation(
    ...
    OnPlayerCommand: function (msg) {
      if (
        msg.cmd.type == "construct" &&
        msg.cmd.template == "structures/athen/storehouse"
      )
        this.NextGoal();
    },
  },
  ...
  },
  {
    instructions: [
      ...
      markForTranslation(
        "Let's train some Female Citizens to gather more food. Select the Civic Center, hold Shift and click on the Female Citizen icon to train five Female Citizens."
      ),
    ],
    Init: function () {
    ...
    OnTrainingQueued: function (msg) {
      if (
        msg.unitTemplate != "units/athen/support_female_citizen" ||
        +msg.count == 1
      ) {
        let entity = msg.trainerEntity;
        let cmpProductionQueue = Engine.QueryInterface(
        ...
        let txt =
          +msg.count == 1
            ? markForTranslation(
                "Do not forget to hold Shift and click to train several units."
              )
            : markForTranslation(
                "Hold shift and click on the Female Citizen icon."
              );
        this.WarningMessage(txt);
        return;
      }
  ...
  },
  {
    ...
    OnPlayerCommand: function (msg) {
      if (msg.cmd.type == "unset-rallypoint") this.NextGoal();
    },
    OnTrainingFinished: function (msg) {
  ...
  },
  {
    instructions: [
      ...
      markForTranslation(
        "Reminder: to select only two Female Citizens, click on the first one and then hold Shift and click on the second one."
      ),
    ],
    Init: function () {
    ...
    OnOwnershipChanged: function (msg) {
      if (msg.from != INVALID_PLAYER && this.houseGoal.has(+msg.entity)) {
        ...
        if (cmpFoundation && cmpFoundation.GetBuildProgress() < 1)
          // Destroyed before built
          --this.houseCount;
      } else if (
        msg.from == INVALID_PLAYER &&
        msg.to == this.playerID &&
  ...
  },
  {
    ...
    delay: -1,
    OnOwnershipChanged: function (msg) {
      if (this.houseGoal.has(+msg.entity)) this.houseGoal.delete(+msg.entity);
    },
  },
  {
    instructions: [
    ...
    OnPlayerCommand: function (msg) {
      if (
        msg.cmd.type == "construct" &&
        msg.cmd.template == "structures/athen/farmstead"
      )
        this.NextGoal();
    },
    OnOwnershipChanged: function (msg) {
  ...
  },
  {
    instructions: [
      markForTranslation(
        "When both Houses are built, select your two Female Citizens and order them to build a Field as close as possible to the Farmstead, which is a dropsite for all types of food."
      ),
    ],
    OnPlayerCommand: function (msg) {
      if (
        msg.cmd.type == "construct" &&
        msg.cmd.template == "structures/athen/field"
      )
        this.NextGoal();
    },
  },
  {
    instructions: [
      markForTranslation(
        "When the Field is ready, the builders will automatically start gathering it.\n"
      ),
      markForTranslation(
        "The cavalry unit should have slaughtered all chickens by now. Select it and explore the south-west area: there is a lake with some camels around. Move your cavalry by right-clicking on the point you want to go, and when you see a herd of camels, right-click on one of them to start hunting for food."
      ),
    ],
    OnPlayerCommand: function (msg) {
      if (
  ...
  },
  {
    instructions: [
      markForTranslation(
        "Up to five Workers can gather from a Field. To add additional Workers, select the Civic Center and set a rally point on a Field by right-clicking on it. If the Field is not yet finished, new Workers sent by a rally point will help building it, and when built, they will gather food."
      ),
    ],
    OnPlayerCommand: function (msg) {
      if (
        ...
        this.WarningMessage(
          markForTranslation(
            "Select the Civic Center and right-click on the Field."
          )
        );
        return;
      }
  ...
  },
  {
    instructions: [
      markForTranslation(
        "Now click three times on the Female Citizen icon in the bottom right panel to train three additional farmers."
      ),
    ],
    Init: function (msg) {
      this.femaleCount = 0;
    ...
    OnTrainingQueued: function (msg) {
      if (
        ...
        let txt =
          +msg.count != 1
            ? markForTranslation(
                "Click without holding Shift to train a single unit."
              )
            : markForTranslation("Click on the Female Citizen icon.");
        this.WarningMessage(txt);
        return;
      }
      if (++this.femaleCount == 3) this.NextGoal();
    },
  },
  {
    instructions: [
    ...
    IsDone: function () {
      return (
        TriggerHelper.HasDealtWithTech(
          this.playerID,
    ...
    OnResearchQueued: function (msg) {
      if (
        ...
        TriggerHelper.EntityMatchesClassList(msg.researcherEntity, "Farmstead")
      )
        this.NextGoal();
    },
  },
  {
    ...
    OnPlayerCommand: function (msg) {
      if (
        msg.cmd.type == "construct" &&
        msg.cmd.template == "structures/athen/barracks"
      )
        this.NextGoal();
    },
  },
  ...
  },
  {
    instructions: [
      markForTranslation(
        "Thus, we should order them to deposit their wood in the Civic Center along the way. To do so, we will hold Shift while clicking to queue orders: select your soldiers, hold Shift and right-click on the Civic Center to deposit their wood and then hold Shift and right-click on the stone quarry to gather it.\n"
      ),
      markForTranslation(
    ...
    IsDone: function () {
      if (!this.stone || !this.metal) return false;
      return TriggerHelper.HasDealtWithTech(this.playerID, "phase_town_athen");
    },
    OnPlayerCommand: function (msg) {
  ...
  },
  {
    instructions: [
      markForTranslation(
        "This is the end of the walkthrough. This should give you a good idea of the basics of setting up your economy."
      ),
    ],
  },
];

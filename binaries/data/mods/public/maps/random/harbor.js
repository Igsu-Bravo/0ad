...
const [playerIDs, playerPosition] = createBasesByPattern(
  "radial",
...
function addCenterLake() {
  ...
  let fDist = 50;
  if (mapSize <= 192) fDist = 20;
}

function addHarbors() {
  for (let position of playerPosition) {
    let harborPosition = Vector2D.add(
      position,
      Vector2D.sub(mapCenter, position).div(2.5).round()
    );
    createArea(
      ...
      avoidClasses(g_TileClasses.player, 15, g_TileClasses.hill, 1)
    );
  }
}
...
function addSpines() {
  ...
  if (currentBiome() == "generic/arctic") spineTile = g_Terrains.tier1Terrain;

  if (currentBiome() == "generic/alpine" || currentBiome() == "generic/savanna")
    spineTile = g_Terrains.tier2Terrain;

  if (currentBiome() == "generic/autumn") spineTile = g_Terrains.tier4Terrain;
  ...
  for (let i = 0; i < numPlayers * split; ++i) {
    ...
    let start = Vector2D.add(
      mapCenter,
      new Vector2D(fractionToTiles(0.12), 0).rotate(-tang)
    );
    let end = Vector2D.add(
      mapCenter,
      new Vector2D(fractionToTiles(0.4), 0).rotate(-tang)
    );

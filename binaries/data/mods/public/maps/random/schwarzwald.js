...
var baseRadius = 15;
var minPlayerRadius = Math.min(
  mapRadius - 1.5 * baseRadius,
  (5 / 8) * mapRadius
);
...
var maxTreeDensity = Math.min(
  (256 * (192 + 8 * numPlayers)) / Math.square(mapSize),
  1
...
var heightRange = {
  ...
  avg:
    (MIN_HEIGHT * (g_Map.size + 512) + MAX_HEIGHT * (g_Map.size + 512)) / 16384,
};
...
createArea(new MapBoundsPlacer(), new SmoothingPainter(1, 0.8, 5));
...
for (let i = 0; i < numPlayers; ++i) {
  playerPosition[i] = Vector2D.add(
    ...
    new Vector2D(randFloat(minPlayerRadius, maxPlayerRadius), 0).rotate(
      -(
        (playerAngleStart +
          i * playerAngleAddAvrg +
          randFloat(0, playerAngleMaxOff)) %
        (2 * Math.PI)
      )
    )
  ).round();
  ...
  createArea(
    ...
    new SmoothElevationPainter(
      ELEVATION_SET,
      g_Map.getHeight(playerPosition[i]),
      20
    )
  );
}
...
for (let [minHeight, maxHeight] of [
  [heighLimits[3], (heighLimits[4] + heighLimits[3]) / 2],
  [(heighLimits[5] + heighLimits[6]) / 2, heighLimits[7]],
])
  for (let [template, tileClass] of [
    [oStoneLarge, clRock],
    [oMetalLarge, clMetal],
  ])
    createObjectGroups(
      new SimpleGroup(
        [new SimpleObject(template, 1, 1, 0, 4)],
        true,
        tileClass
      ),
      0,
...
for (var x = 0; x < mapSize; x++)
  for (var z = 0; z < mapSize; z++) {
    ...
    let radius = Vector2D.add(position, new Vector2D(0.5, 0.5)).distanceTo(
      mapCenter
    );
    var minDistToSL = mapSize;
    for (let i = 0; i < numPlayers; ++i)
      minDistToSL = Math.min(
        minDistToSL,
        position.distanceTo(playerPosition[i])
      );

    // Woods tile based
    var tDensFactSL = Math.max(
      Math.min((minDistToSL - baseRadius) / baseRadius, 1),
      0
    );
    ...
    if (!randBool(tDensActual)) continue;
...
placePlayersNomad(
  clPlayer,
  avoidClasses(clWater, 4, clForest, 1, clFood, 2, clMetal, 4, clRock, 4)
);

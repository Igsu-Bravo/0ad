...
var jebelBarkal_templates = deepfreeze(
  Object.keys(jebelBarkal_templateClasses).reduce((templates, name) => {
    templates[name] = TriggerHelper.GetTemplateNamesByClasses(
      jebelBarkal_templateClasses[name],
      "kush",
      undefined,
      jebelBarkal_rank,
      true
 ...
 */
var scaleByTime = (minCurrent, min0, min60) =>
  min0 + (min60 - min0) * Math.min(1, minCurrent / 60);

/**
 *  @returns min0 value at the beginning of the game, min60 after an hour of gametime or longer and
 *  a proportionate number between these two values before the first hour is reached.
 */
var scaleByMapSize = (min, max) =>
  min + ((max - min) * (TriggerHelper.GetMapSizeTiles() - 128)) / (512 - 128);

/**
 * Defensive Infantry units patrol along the paths of the city.
 */
var jebelBarkal_cityPatrolGroup_count = (time) =>
  TriggerHelper.GetMapSizeTiles() > 192
    ? scaleByTime(time, 3, scaleByMapSize(3, 10))
    : 0;
 ...
 */
var jebelBarkal_attackInterval = (time, difficulty) =>
  randFloat(5, 7) + time / difficulty / 10;
 ...
 */
var jebelBarkal_attackerGroup_sizeFactor = (
  numPlayers,
  numInitialSpawnPoints,
  difficulty
...
var jebelBarkal_buildingGarrison = (difficulty) => [
  ...
  },
  {
    ...
    unitTemplates: [
      ...jebelBarkal_templates.citizenSoldiers,
      ...jebelBarkal_templates.champions,
    ],
    capacityRatio: 1,
  },
  {
  ...
  },
  {
    ...
    unitTemplates: [
      ...jebelBarkal_templates.females,
      ...jebelBarkal_templates.healers,
    ],
    capacityRatio: 0.5,
  },
  {
  ...
  },
  {
    ...
    unitTemplates:
      difficulty > 3
        ? jebelBarkal_templates.champion_infantry_ranged
        : jebelBarkal_templates.citizenSoldier_infantry_ranged,
...
var jebelBarkal_attackerGroup_balancing = [
  ...
  },
  {
    ...
    formations: [],
    targetClasses: () =>
      pickRandom(["Defensive SiegeEngine Monument Wonder", "Structure"]),
  },
];
...
Trigger.prototype.JebelBarkal_Init = function () {
  ...
  this.RegisterTrigger("OnOwnershipChanged", "JebelBarkal_OwnershipChange", {
    enabled: true,
  });
  ...
  this.DoAfterDelay(
    jebelBarkal_firstCityPatrolTime(this.GetDifficulty(), isNomad) * 60 * 1000,
    "JebelBarkal_SpawnCityPatrolGroups",
    {}
  );
...
Trigger.prototype.JebelBarkal_TrackUnits = function () {
  ...
  this.jebelBarkal_ritualHealers = TriggerHelper.GetPlayerEntitiesByClass(
  ...
  this.jebelBarkal_patrolGroupSpawnPoints =
    TriggerHelper.GetPlayerEntitiesByClass(
      jebelBarkal_playerID,
      jebelBarkal_cityPatrolGroup_balancing.buildingClasses
    );

  this.debugLog(
    "Patrol spawn points: " + uneval(this.jebelBarkal_patrolGroupSpawnPoints)
  );
  ...
  this.jebelBarkal_attackerGroupSpawnPoints =
    TriggerHelper.GetPlayerEntitiesByClass(
      jebelBarkal_playerID,
      jebelBarkal_attackerGroup_balancing.reduce(
        (classes, attackerSpawning) =>
          classes.concat(attackerSpawning.buildingClasses),
        []
      )
    );
  ...
  this.debugLog(
    "Attacker spawn points: " +
      uneval(this.jebelBarkal_attackerGroupSpawnPoints)
  );
};

Trigger.prototype.JebelBarkal_SetDefenderStance = function () {
  for (let ent of TriggerHelper.GetPlayerEntitiesByClass(
    jebelBarkal_playerID,
    "Human"
  ))
...
Trigger.prototype.JebelBarkal_UpdateRitualAnimations = function () {
  for (let ent of this.jebelBarkal_ritualHealers) {
    ...
    if (
      cmpVisual &&
      jebelBarkal_ritualAnimations.indexOf(cmpVisual.GetAnimationName()) == -1
    )
      cmpVisual.SelectAnimation(
        pickRandom(jebelBarkal_ritualAnimations),
        false,
        1,
        ""
      );
...
Trigger.prototype.JebelBarkal_GarrisonBuildings = function () {
  for (let buildingGarrison of jebelBarkal_buildingGarrison(
    ...
    TriggerHelper.SpawnAndGarrisonAtClasses(
      jebelBarkal_playerID,
      buildingGarrison.buildingClasses,
      buildingGarrison.unitTemplates,
      buildingGarrison.capacityRatio
    );
};
...
Trigger.prototype.JebelBarkal_SpawnCityPatrolGroups = function () {
  if (!this.jebelBarkal_patrolGroupSpawnPoints.length) return;

  let time = TriggerHelper.GetMinutes();
  let groupCount = Math.floor(
    Math.max(0, jebelBarkal_cityPatrolGroup_count(time)) -
      this.jebelBarkal_patrolingUnits.length
  );
  ...
  for (let i = 0; i < groupCount; ++i) {
    ...
    let templateCounts = TriggerHelper.BalancedTemplateComposition(
      jebelBarkal_cityPatrolGroup_balancing.unitComposition(
        time,
        this.jebelBarkal_heroes
      ),
    ...
    let groupEntities = this.JebelBarkal_SpawnTemplates(
      spawnEnt,
      templateCounts
    );
    ...
    TriggerHelper.SetUnitFormation(
      jebelBarkal_playerID,
      groupEntities,
      pickRandom(jebelBarkal_formations)
    );

    for (let patrolTarget of shuffleArray(
      this.GetTriggerPoints(jebelBarkal_cityPatrolGroup_triggerPointPath)
    )) {
  ...
  this.DoAfterDelay(
    jebelBarkal_cityPatrolGroup_interval(time) * 60 * 1000,
    "JebelBarkal_SpawnCityPatrolGroups",
    {}
  );
};

Trigger.prototype.JebelBarkal_SpawnTemplates = function (
  spawnEnt,
  templateCounts
) {
  let groupEntities = [];
...
Trigger.prototype.JebelBarkal_SpawnAttackerGroups = function () {
  if (!this.jebelBarkal_attackerGroupSpawnPoints) return;

  let time = TriggerHelper.GetMinutes();
  this.JebelBarkal_StartAttackTimer(
    jebelBarkal_attackInterval(time, this.GetDifficulty())
  );

  this.debugLog(
    "Attacker wave (at most " +
      (jebelBarkal_maxPopulation - this.jebelBarkal_attackerUnits.length) +
      " attackers)"
  );

  let activePlayers = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_PlayerManager
  ).GetActivePlayers();
  ...
  let patrolPoints = this.GetTriggerPoints(
  ...
  for (let spawnPointBalancing of jebelBarkal_attackerGroup_balancing) {
    let targets = playerEntities.reduce(
      (allTargets, playerEnts) =>
        allTargets.concat(
          shuffleArray(
            TriggerHelper.MatchEntitiesByClass(
              playerEnts,
              spawnPointBalancing.targetClasses()
            )
          ).slice(0, 10)
        ),
      []
    ...
    for (let spawnEnt of TriggerHelper.MatchEntitiesByClass(
      this.jebelBarkal_attackerGroupSpawnPoints,
      spawnPointBalancing.buildingClasses
    )) {
      ...
      this.debugLog(
        "Spawning " +
          unitCount +
          " attackers at " +
          uneval(spawnPointBalancing.buildingClasses) +
          " " +
          spawnEnt +
          ":\n" +
      ...
      let spawnedEntities = this.JebelBarkal_SpawnTemplates(
        spawnEnt,
        templateCounts
      );

      this.jebelBarkal_attackerUnits =
        this.jebelBarkal_attackerUnits.concat(spawnedEntities);
      ...
      let entityGroups = formation
        ? [spawnedEntities]
        : spawnedEntities.reduce(
            (entityGroup, ent) => entityGroup.concat([[ent]]),
            []
          );
      for (let i = 0; i < jebelBarkal_patrolPointCount; ++i)
        for (let entities of entityGroups) {
          let pos = TriggerHelper.GetEntityPosition2D(
            pickRandom(i == 0 ? targets : patrolPoints)
          );
          ProcessCommand(jebelBarkal_playerID, {
            type: "patrol",
            entities: entities,
...
Trigger.prototype.JebelBarkal_OwnershipChange = function (data) {
  ...
  this.jebelBarkal_patrolingUnits = this.jebelBarkal_patrolingUnits.filter(

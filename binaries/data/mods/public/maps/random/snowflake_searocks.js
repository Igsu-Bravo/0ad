...
function initIsConnected() {
  for (let m = 0; m < numIslands; ++m) {
    isConnected[m] = [];
    for (let n = 0; n < numIslands; ++n) isConnected[m][n] = 0;
  }
}
...
function createIsland(islandID, size, tileClass) {
  createArea(
    new ClumpPlacer(
      size * diskArea(playerIslandRadius),
      0.95,
      0.6,
      Infinity,
      islandPos[islandID]
...
function createIslandAtRadialLocation(
  playerID,
  islandID,
  playerIDOffset,
  distFromCenter,
  ...
  let angle =
    startAngle + ((playerID * 2 + playerIDOffset) * Math.PI) / numPlayers;
  islandPos[islandID] = Vector2D.add(
    mapCenter,
    new Vector2D(fractionToTiles(distFromCenter), 0).rotate(-angle)
  ).round();
...
function createSnowflakeSearockWithCenter(sizeID) {
  let [
    tertiaryIslandDist,
    tertiaryIslandRadius,
    islandBetweenPlayersDist,
  ...
  for (let playerID = 0; playerID < numPlayers; ++playerID) {
    ...
    let islandID_betweenPlayerAndCenterNeighbor =
      playerID_neighbor + 2 * numPlayers;
    ...
    isConnected[islandID_betweenPlayerAndCenter][
      islandID_betweenPlayerAndCenterNeighbor
    ] = 1;
    createIslandAtRadialLocation(
      playerID,
      islandID_betweenPlayerAndCenter,
      0,
      islandBetweenPlayerAndCenterDist,
...
if (mapSize <= 128) {
  ...
  if (numPlayers < 6) createSnowflakeSearockWithCenter("medium");
  else createSnowflakeSearockWithoutCenter();
} else if (mapSize <= 320) {
  if (numPlayers < 8) createSnowflakeSearockWithCenter("medium");
  else createSnowflakeSearockWithoutCenter();
} else createSnowflakeSearockWithCenter(numPlayers < 6 ? "large1" : "large2");
...
var size =
  (forestTrees / (scaleByMapSize(2, 8) * numPlayers)) *
...
createObjectGroupsDeprecated(
  ...
  0,
  [
    avoidClasses(clForest, 1, clPlayer, 10, clMetal, 10, clRock, 5),
...
createStragglerTrees(
  ...
  [
    avoidClasses(clForest, 1, clPlayer, 9, clMetal, 6, clRock, 6),

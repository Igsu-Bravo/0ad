...
createArea(new MapBoundsPlacer(), new SmoothingPainter(1, 0.1, 1));
...
for (let i = 0; i < scaleByMapSize(5, 12); ++i) {
  ...
  createPassage({
    start: new Vector2D(x, mapBounds.bottom).rotateAround(
      riverAngle + (Math.PI / 2) * randFloat(0.8, 1.2),
      mapCenter
    ),
    end: new Vector2D(x, mapBounds.top).rotateAround(
      riverAngle + (Math.PI / 2) * randFloat(0.8, 1.2),
      mapCenter
    ),
...
placePlayerBases({
  ...
  Mines: {
    types: [{ template: g_Gaia.metalLarge }, { template: g_Gaia.stoneLarge }],
  },
  Trees: {
...
setWaterTint(0.224, 0.271, 0.27);
setWaterColor(0.224, 0.271, 0.27);
setWaterWaviness(8);

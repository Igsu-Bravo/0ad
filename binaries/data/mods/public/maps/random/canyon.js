...
g_Map.log(
  "Reserving space for the players, their initial forests and some less space therein without trees"
);
...
for (let g = 0; g < scaleByMapSize(5, 30); ++g) {
  let position = new Vector2D(
    randIntInclusive(1, mapSize - 1),
    randIntInclusive(1, mapSize - 1)
  );

  let newarea = createArea(
    new ClumpPlacer(
      diskArea(fractionToTiles(0.06)),
      0.7,
      0.1,
      Infinity,
      position
...
createForests(
  ...
  [
    avoidClasses(clPlayer, 1, clForest, 15, clHill, 1, clHill2, 0),
...
createLayeredPatches(
  ...
  [1, 1],
  [
    avoidClasses(clForest, 0, clHill, 0, clDirt, 5, clPlayer, 4, clHill2, 0),
...
createMines(
  ...
  [
    avoidClasses(clForest, 1, clPlayer, 3, clRock, 10, clHill, 1, clHill2, 1),
...
var group = new SimpleGroup([new SimpleObject(aTree, 1, 1, 0, 1)], true);
createObjectGroupsDeprecated(
  group,
  0,
  avoidClasses(clLand, 5),
  scaleByMapSize(200, 800),
  50
);
...
createFood(
  ...
  [
    avoidClasses(clForest, 0, clPlayer, 4, clHill, 1, clFood, 20, clHill2, 1),
    stayClasses(clLand, 3),
  ],
  clFood
);
...
createFood(
  ...
  [3 * numPlayers],
  [
    avoidClasses(clForest, 0, clPlayer, 4, clHill, 1, clFood, 10, clHill2, 1),
    stayClasses(clLand, 3),
  ],
  clFood
);
...
for (let i = 0; i < randIntInclusive(3, 8); ++i)
  for (let template of [oFoodTreasure, oWoodTreasure])
    g_Map.placeEntityPassable(
      ...
      Vector2D.add(
        mapCenter,
        new Vector2D(randFloat(0, 7), 0).rotate(randomAngle())
      ),
      randomAngle()
    );

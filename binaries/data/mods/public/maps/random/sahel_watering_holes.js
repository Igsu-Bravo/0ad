...
var riverStart = distributePointsOnCircle(
  numPlayers,
  startAngle + Math.PI / numPlayers,
  fractionToTiles(0.15),
  mapCenter
)[0];
var riverEnd = distributePointsOnCircle(
  numPlayers,
  startAngle + Math.PI / numPlayers,
...
for (let i = 0; i < numPlayers; ++i) {
  ...
  // Animals in shallows
  let shallowPosition = Vector2D.average([
    playerPosition[i],
    playerPosition[neighborID],
  ]).round();
  let objects = [
  ...
  for (let object of objects)
    createObjectGroup(
      new SimpleGroup([object], true, clFood, shallowPosition),
      0
    );
}
...
group = new SimpleGroup([new SimpleObject(oGazelle, 2, 3, 0, 2)], true, clFood);
createObjectGroupsDeprecated(
  group,
  0,
  avoidClasses(clWater, 3, clForest, 0, clPlayer, 20, clHill, 1, clFood, 5),
...
group = new SimpleGroup([new SimpleObject(oGiraffe, 2, 3, 0, 2)], true, clFood);
createObjectGroupsDeprecated(
  group,
  0,
  avoidClasses(clWater, 3, clForest, 0, clPlayer, 20, clHill, 1, clFood, 5),
...
group = new SimpleGroup([new SimpleObject(oZebra, 2, 3, 0, 2)], true, clFood);
createObjectGroupsDeprecated(
  group,
  0,
  avoidClasses(clWater, 3, clForest, 0, clPlayer, 20, clHill, 1, clFood, 5),

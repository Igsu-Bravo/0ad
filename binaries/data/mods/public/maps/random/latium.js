...
const pPoplarForest = [
  tForestFloor + TERRAIN_SEPARATOR + oLombardyPoplar,
  tGrass,
];
const pMainForest = [
...
var playerPosition = playerPlacementLine(
...
for (var ix = 0; ix <= mapSize; ix++)
  for (var iz = 0; iz <= mapSize; iz++) {
    ...
    let distToWater = clWater.has(position)
      ? 0
      : 0.5 - WATER_WIDTH - Math.abs(c - 0.5);
    let h = distToWater
      ? heightHill * (1 - Math.abs(c - 0.5) / (0.5 - WATER_WIDTH))
    ...
    if (oldH > 0)
      h +=
        (0.4 * noise2a.get(x, z) + 0.2 * noise2b.get(x, z)) *
        Math.min(oldH / 10, 1);
    ...
    if (h > -10) {
      ...
      cliffNoise -= 0.59;
      cliffNoise *= pn;
      if (cliffNoise > 0) h += (19 * Math.min(cliffNoise, 0.045)) / 0.045;
    }
...
for (var ix = 0; ix < mapSize; ix++)
  for (var iz = 0; iz < mapSize; iz++) {
    ...
    if (maxH > 15) {
      ...
      for (let nx = Math.max(ix - 1, 0); nx <= maxNx; ++nx)
        for (let nz = Math.max(iz - 1, 0); nz <= maxNz; ++nz)
          minAdjHeight = Math.min(
            minAdjHeight,
            g_Map.getHeight(new Vector2D(nx, nz))
          );
    }
    ...
    // water
    if (maxH < -12) t = tOceanDepths;
    else if (maxH < -8.8) t = tOceanRockDeep;
    else if (maxH < -4.7) t = tOceanCoral;
    else if (maxH < -2.8) t = tOceanRockShallow;
    else if (maxH < 0.9 && minH < 0.35) t = tBeachWet;
    else if (maxH < 1.5 && minH < 0.9) t = tBeachDry;
    else if (maxH < 2.3 && minH < 1.3) t = tBeachGrass;

    if (minH < 0) clWater.add(position);
    ...
    if (diffH > 2.9 && minH > -7) {
      ...
      (diffH > 2.5 && minH > -5) ||
      (maxH - minAdjHeight > 2.9 && minH > 0)
    ) {
      if (minH < -1) t = tCliff;
      else if (minH < 0.5) t = tBeachCliff;
    ...
    if (minH >= 20) clCliff.add(position);

    // forests
    if (g_Map.getHeight(position) < 11 && diffH < 2 && minH > 1) {
      var forestNoise =
        ((noise6.get(x, z) + 0.5 * noise7.get(x, z)) / 1.5) * pn - 0.59;
      ...
      if (forestNoise > 0 && randBool()) {
        if (minH < 11 && minH >= 4) {
          ...
          if (typeNoise < 0.43 && forestNoise < 0.05) t = pPoplarForest;
          else if (typeNoise < 0.63) t = pMainForest;
          else t = pPineForest;
    ...
    if (t == tGrass) {
      ...
      if (grassNoise < 0.3) t = diffH > 1.2 ? tDirtCliff : tDirt;
...
placePlayerBases({
  ...
  CityPatch: {
    ...
    innerTerrain: tCity,
    width: 4,
    painters: [new SmoothElevationPainter(ELEVATION_SET, heightPlayer, 2)],
  },
  Chicken: {},
  Berries: {
    template: oBerryBush,
...
group = new SimpleGroup(
  [new SimpleObject(oMetalLarge, 1, 1, 0, 2)],
  true,
  clMetal
);
createObjectGroups(
...
group = new SimpleGroup(
  [new SimpleObject(oBerryBush, 5, 7, 0, 3)],
  true,
  clFood
);
createObjectGroupsDeprecated(

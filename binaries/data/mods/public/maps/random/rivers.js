...
createArea(
  new ClumpPlacer(
    diskArea(fractionToTiles(0.075)),
    0.7,
    0.1,
    Infinity,
    mapCenter
...
let rivers = distributePointsOnCircle(
  numRivers,
  startAngle + Math.PI / numRivers,
  fractionToTiles(0.5),
  mapCenter
)[0];
for (let i = 0; i < numRivers; ++i) {
  if (
    isNomad()
      ? randBool()
      : areAllies(playerIDs[i], playerIDs[(i + 1) % numPlayers])
  )
    continue;

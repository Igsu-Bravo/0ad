...
createAreas(
  ...
  [new TerrainPainter(tDunes), new TileClassPainter(clPatch)],
...
createAreas(
  ...
  [new TerrainPainter([tSand, tFineSand]), new TileClassPainter(clPatch)],
  avoidClasses(clPatch, 2, clPlayer, 0),
...
createAreas(
  new ClumpPlacer(scaleByMapSize(25, 100), 0.2, 0.1, 0),
  [new TerrainPainter([tDirt]), new TileClassPainter(clPatch)],
  avoidClasses(clPatch, 2, clPlayer, 0),
  scaleByMapSize(15, 50)
);
...
createArea(new ClumpPlacer(diskArea(oasisRadius), 0.6, 0.15, 0, mapCenter), [
  new LayeredPainter(
    [
      [tSand, pForest],
...
var constraint = new AndConstraint([
  borderClasses(clOasis, 0, 3),
  avoidClasses(clOasis, 0),
]);
for (var i = 0; i < num; ++i) {
  let animalPosition;
  let r = 0;
  ...
  do {
    ...
    animalPosition = Vector2D.add(
      mapCenter,
      new Vector2D(r, 0).rotate(-angle)
    ).round();
    ++r;
...
constraint = new AndConstraint([
  borderClasses(clOasis, 15, 0),
  avoidClasses(clFood, 5),
]);
...
createAreas(
  ...
  [new TerrainPainter([tSand, pForest]), new TileClassPainter(clForest)],

...
var [playerIDs, playerPosition, playerAngle] = playerPlacementCircle(
...
if (!isNomad()) {
  ...
  for (let i = 0; i < numPlayers; i++) {
    ...
    let dockLocation = findLocationInDirectionBasedOnHeight(
      playerPosition[i],
      mapCenter,
      -3,
      heightLand - 0.5,
  ...
  for (let type of types)
    createAreas(
      new ChainPlacer(
        ...
        forestTrees / (num * Math.floor(scaleByMapSize(2, 5))),
        0.5
      ),
      [new LayeredPainter(type, [2]), new TileClassPainter(clForest)],
      [
        avoidClasses(clPlayer, 0, clForest, 10, clHill, 0),
...
var numberOfPatches =
  scaleByMapSize(15, 45) * (currentBiome() == "generic/savanna" ? 3 : 1);
for (let size of [
  scaleByMapSize(3, 6),
  scaleByMapSize(5, 10),
  scaleByMapSize(8, 21),
...
createObjectGroupsDeprecated(
  ...
  0,
  [
    avoidClasses(clForest, 1, clPlayer, 0, clRock, 10, clHill, 1),
...
createObjectGroupsDeprecated(
  ...
  0,
  [
    avoidClasses(clForest, 0, clPlayer, 0, clHill, 1, clFood, 20),
    stayClasses(clLand, 5),
  ],
  3 * numPlayers,
  50
...
createObjectGroupsDeprecated(
  ...
  0,
  [
    avoidClasses(clForest, 0, clPlayer, 8, clHill, 1, clFood, 20),
    stayClasses(clLand, 5),
  ],

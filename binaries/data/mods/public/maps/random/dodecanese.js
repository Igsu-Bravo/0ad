...
createAreas(
  new ChainPlacer(
    6,
    Math.floor(scaleByMapSize(8, 10)),
    Math.floor(scaleByMapSize(10, 35)),
    0.2
  ),
  [
    new TerrainPainter(tPrimary),
...
var areaShoreline = createArea(
  ...
  [new TerrainPainter(tShoreLower), new TileClassPainter(clShore)],
...
createMines(
  [
    [
      new SimpleObject(oMetalSmall, 0, 1, 0, 4),
...
createDecoration(
  ...
  [scaleByMapSize(16, 262), scaleByMapSize(40, 360)],
  avoidClasses(
    clWater,
    4,
    clPlayer,
...
for (let bridgeStart of shuffleArray(areaShoreline.getPoints())) {
  if (
    new NearTileClassConstraint(clBridge, bridgeLength * 8).allows(bridgeStart)
  )
    continue;
  ...
  for (let direction = 0; direction < 4; ++direction) {
    ...
    let bridgeOffset = new Vector2D(
      direction % 2 ? 2 : 0,
      direction % 2 ? 0 : 2
    );
    let bridgeCenter1 = Vector2D.add(
      bridgeStart,
      Vector2D.mult(bridgeDirection, bridgeLength / 2)
    );
    let bridgeCenter2 = Vector2D.add(bridgeCenter1, bridgeOffset);
    if (
      avoidClasses(clWater, 0).allows(bridgeCenter1) &&
      avoidClasses(clWater, 0).allows(bridgeCenter2)
    )
      continue;

    let bridgeEnd1 = Vector2D.add(
      bridgeStart,
      Vector2D.mult(bridgeDirection, bridgeLength)
    );
    let bridgeEnd2 = Vector2D.add(bridgeEnd1, bridgeOffset);
    if (
      avoidClasses(clShore, 0).allows(bridgeEnd1) &&
    ...
    if (
      avoidClasses(clWater, 0).allows(Vector2D.add(bridgeCenter1, bridgeP)) ||
    ...
    g_Map.placeEntityAnywhere(
      ...
      0,
      bridgeCenter2,
      bridgeOrientation + Math.PI
    );

    createArea(
      new RectPlacer(
        Vector2D.sub(bridgeStart, areaOffset),
        Vector2D.add(bridgeEnd1, areaOffset)
      ),
      [new ElevationPainter(heightBridge), new TileClassPainter(clBridge)]
    );

    for (let center of [bridgeStart, bridgeEnd2])
      createArea(new DiskPlacer(2, center), new SmoothingPainter(1, 1, 1));

    break;
  }

  if (bridges >= maxBridges) break;
}

...
var teamPosition = distributePointsOnCircle(
  numTeams,
  startAngle,
  fractionToTiles(0.3),
  mapCenter
...
for (let i = 0; i < teams.length; ++i) {
  if (!teams[i] || isNomad()) continue;

  ++teamNo;

  let [playerPosition, playerAngle] = distributePointsOnCircle(
    teams[i].length,
    startAngle + (2 * Math.PI) / teams[i].length,
    teamRadius,
    teamPosition[i]
  );
  ...
  for (let p = 0; p < teams[i].length; ++p) {
    ...
    createArea(
      new ChainPlacer(
        2,
        Math.floor(scaleByMapSize(5, 11)),
        Math.floor(scaleByMapSize(60, 250)),
        Infinity,
        playerPosition[p],
        Infinity,
        [(defaultPlayerBaseRadius() * 3) / 4]
      ),
      [
  ...
  for (let p = 0; p < teams[i].length; ++p)
    for (let mine of mines) {
      let position = Vector2D.add(
        playerPosition[p],
        new Vector2D(g_InitialMineDistance, 0).rotate(
          -playerAngle[p] - mine.angle
        )
      );
      createObjectGroup(
        new SimpleGroup(
          [new SimpleObject(mine.template, 1, 1, 0, 4)],
          true,
          clBaseResource,
          position
        ),
  ...
  for (let p = 0; p < teams[i].length; ++p) {
    ...
    for (let x = 0; x < tries; ++x) {
      let tAngle =
        playerAngle[p] + (randFloat(-1, 1) * 2 * Math.PI) / teams[i].length;
      let treePosition = Vector2D.add(
        playerPosition[p],
        new Vector2D(16, 0).rotate(-tAngle)
      ).round();
      if (
        createObjectGroup(
          new SimpleGroup(
...
createMines(
  ...
  [
    avoidClasses(clForest, 1, clPlayer, 40, clMetal, 20),
...
createAreas(
  ...
  [
    avoidClasses(clBaseResource, 20, clHill, 15, clRock, 6, clMetal, 6),
...
g_Map.log("Smoothing heightmap");
createArea(new MapBoundsPlacer(), new SmoothingPainter(1, 0.8, 3));
...
createFood(
  ...
  [
    avoidClasses(clForest, 0, clPlayer, 20, clHill, 1, clRock, 6, clMetal, 6),
...
if (currentBiome() == "generic/sahara") {
  ...
  let group = new SimpleGroup([new SimpleObject(oObelisk, 1, 1, 0, 1)], true);
...
for (let size of [
  ...
  createAreas(
    ...
    [
      avoidClasses(clForest, 0, clHill, 0, clDirt, 5, clPlayer, 0),
      stayClasses(clLand, 4),
    ],
...
let group = new SimpleGroup([new SimpleObject(aRockMedium, 1, 3, 0, 1)], true);
createObjectGroupsDeprecated(
  group,
  0,

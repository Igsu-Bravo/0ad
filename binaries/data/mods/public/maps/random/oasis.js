...
const pForestMain = [
  tForestFloor + TERRAIN_SEPARATOR + ePalmShort,
  tForestFloor + TERRAIN_SEPARATOR + ePalmTall,
  tForestFloor,
];
const pOasisForestLight = [
  tForestFloor + TERRAIN_SEPARATOR + ePalmShort,
  tForestFloor + TERRAIN_SEPARATOR + ePalmTall,
  tForestFloor,
  tForestFloor,
...
for (let i = 0; i < numPlayers; ++i) {
  ...
  do {
    ...
    forestPosition = Vector2D.add(
      playerPosition[i],
      new Vector2D(forestDist, 0).rotate(-forestAngle)
    );
  } while (
    !createArea(
  ...
  do {
    ...
    waterPosition = Vector2D.add(
      forestPosition,
      new Vector2D(6, 0).rotate(-waterAngle)
    ).round();
    flowerPosition = Vector2D.add(
      forestPosition,
      new Vector2D(3, 0).rotate(-waterAngle)
    ).round();
    reedsPosition = Vector2D.add(
      forestPosition,
      new Vector2D(5, 0).rotate(-waterAngle)
    ).round();
  } while (
    !createArea(
...
placePlayerBases({
  ...
  Mines: {
    ...
    maxAngle: Math.PI / 2,
    groupElements: shuffleArray([aBushA, aBushB, ePalmShort, ePalmTall]).map(
      (t) => new SimpleObject(t, 1, 1, 3, 4)
    ),
  },
...
createArea(
  new ClumpPlacer(
    diskArea(forestDistance + shoreDistance + waterRadius),
    0.8,
    0.2,
    Infinity,
    mapCenter
...
if (mapSize > 150 && randBool()) {
  ...
  let points = distributePointsOnCircle(
    2,
    randomAngle(),
    waterRadius + shoreDistance + forestDistance + pathWidth,
    mapCenter
  )[0];
  createArea(new PathPlacer(points[0], points[1], pathWidth, 0.4, 1, 0.2, 0), [
...
var group = new SimpleGroup(
  [
    new SimpleObject(ePalmTall, 1, 1, 0, 0),
    new SimpleObject(ePalmShort, 1, 2, 1, 2),
...
createObjectGroupsDeprecated(
  group,
  0,
  stayClasses(clPassage, 3),
  scaleByMapSize(60, 250),
  100
);
...
group = new SimpleGroup(
  [
    new SimpleObject(eStoneMine, 1, 1, 0, 0),
    new SimpleObject(ePalmShort, 1, 2, 3, 3),
    new SimpleObject(ePalmTall, 0, 1, 3, 3),
...
group = new SimpleGroup(
  [
    new SimpleObject(eMetalMine, 1, 1, 0, 0),
    new SimpleObject(ePalmShort, 1, 2, 2, 3),
    new SimpleObject(ePalmTall, 0, 1, 2, 2),
    new SimpleObject(aBushB, 1, 1, 2, 2),
    new SimpleObject(aBushA, 0, 2, 1, 3),
  ],
  true,
...
group = new SimpleGroup([new SimpleObject(eCamel, 1, 2, 0, 4)], true, clFood);
createObjectGroupsDeprecated(
  group,
  0,
  avoidClasses(clOasis, 3, clForest, 0, clPlayer, 10, clHill, 1, clFood, 20),
...
group = new SimpleGroup([new SimpleObject(eGazelle, 2, 4, 0, 2)], true, clFood);
createObjectGroupsDeprecated(
  group,
  0,
  avoidClasses(clOasis, 3, clForest, 0, clPlayer, 10, clHill, 1, clFood, 20),
...
for (let i = 0; i < scaleByMapSize(5, 30); ++i) {
  let animalPos = Vector2D.add(
    mapCenter,
    new Vector2D(forestDistance + shoreDistance + waterRadius, 0).rotate(
      randomAngle()
    )
  );
...
var group = new SimpleGroup([
  new SimpleObject(aBushB, 1, 2, 0, 2),
  new SimpleObject(aBushA, 2, 4, 0, 2),
]);
createObjectGroupsDeprecated(
  group,
  0,
  ...
  for (var sandz = 0; sandz < mapSize; sandz += 4) {
    ...
    if (height > heightSand) {
      if (randBool((height - heightSand) / 1.4))
        createObjectGroup(
          new SimpleGroup(
            [new SimpleObject(aSand, 0, 1, 0, 2)],
            true,
            undefined,
            position
          ),
      ...
      if (randBool(0.4))
        createObjectGroup(
          new SimpleGroup(
            [new SimpleObject(aWaterFlower, 1, 4, 1, 2)],
            true,
            undefined,
            position
          ),
        ...
        createObjectGroup(
          new SimpleGroup(objectsWaterFlora, true, undefined, position),
          0
        );

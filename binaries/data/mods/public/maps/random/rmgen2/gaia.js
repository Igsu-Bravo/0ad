...
function createBluffsPassages(playerPosition) {
  ...
  for (let position of playerPosition) {
    ...
    for (let tryCount = 0; tryCount < 80; ++tryCount) {
      let angle =
        position.angleTo(g_Map.getCenter()) + (randFloat(-1, 1) * Math.PI) / 2;
      ...
      for (let point of area.getPoints()) g_Map.deleteTerrainEntity(point);

      createArea(new MapBoundsPlacer(), new TerrainPainter(g_Terrains.cliff), [
...
function addPlateaus(constraint, size, deviation, fill) {
  ...
  if (currentBiome() == "generic/arctic") plateauTile = g_Terrains.tier1Terrain;

  if (currentBiome() == "generic/alpine" || currentBiome() == "generic/savanna")
    plateauTile = g_Terrains.tier2Terrain;

  if (currentBiome() == "generic/autumn") plateauTile = g_Terrains.tier4Terrain;
...
function addValleys(constraint, size, deviation, fill, baseHeight) {
  ...
  if (currentBiome() == "generic/autumn") valleyFloor = g_Terrains.tier3Terrain;
...
function addMetal(constraint, size, deviation, fill) {
  ...
  createObjectGroupsDeprecated(
    new SimpleGroup(
      [new SimpleObject(g_Gaia.metalLarge, offset, offset, 0, 4 * offset)],
      true,
      g_TileClasses.metal
    ),
    0,
...
function addStragglerTrees(constraint, size, deviation, fill) {
  ...
  for (var i = 0; i < trees.length; ++i) {
    ...
    if (
      i == 2 &&
      (currentBiome() == "generic/sahara" || currentBiome() == "generic/aegean")
    )
      treesMax = 1;

...
function areAllies(playerID1, playerID2) {
  return (
    ...
    g_MapSettings.PlayerData[playerID1].Team ===
      g_MapSettings.PlayerData[playerID2].Team
  );
}

function getPlayerTeam(playerID) {
  if (g_MapSettings.PlayerData[playerID].Team === undefined) return -1;
 ...
 */
function placeStartingEntities(
  location,
  playerID,
  civEntities,
  dist = 6,
 ...
 */
function placeCivDefaultStartingEntities(
  position,
  playerID,
  wallType,
  dist = 6,
...
function placeStartingWalls(
  ...
  if (civ != "iber" || g_Map.getSize() <= 128) return;
  ...
  else if (wallType) placeGenericFortress(position, 20, playerID);
}
...
function placePlayerBase(playerBaseArgs) {
  ...
  if (playerBaseArgs.PlayerTileClass)
    addCivicCenterAreaToClass(
      playerBaseArgs.playerPosition,
      playerBaseArgs.PlayerTileClass
    );

  for (let functionID of g_PlayerBaseFunctions) {
    let funcName = "placePlayerBase" + functionID;
    let func = global[funcName];
    if (!func) throw new Error("Could not find " + funcName);

    if (!playerBaseArgs[functionID]) continue;
...
function addCivicCenterAreaToClass(position, tileClass) {
  createArea(new DiskPlacer(5, position), new TileClassPainter(tileClass));
}
...
function getPlayerBaseArgs(playerBaseArgs) {
  let baseResourceConstraint =
    playerBaseArgs.BaseResourceClass &&
    avoidClasses(playerBaseArgs.BaseResourceClass, 4);

  if (playerBaseArgs.baseResourceConstraint)
    baseResourceConstraint = new AndConstraint([
      baseResourceConstraint,
  ...
  return [
    (property, defaultVal) =>
      playerBaseArgs[property] === undefined
        ? defaultVal
        : playerBaseArgs[property],
    playerBaseArgs.playerPosition,
    baseResourceConstraint,
  ];
}
...
function placePlayerBaseCityPatch(args) {
  ...
  if (args.outerTerrain && args.innerTerrain)
    painters.push(
      new LayeredPainter(
        [args.outerTerrain, args.innerTerrain],
        [get("width", 1)]
      )
    );

  if (args.painters) painters = painters.concat(args.painters);
...
function placePlayerBaseChicken(args) {
  ...
  for (let i = 0; i < get("groupCount", 2); ++i) {
    ...
    for (let tries = 0; tries < get("maxTries", 30); ++tries) {
      let position = new Vector2D(0, get("distance", 9))
        .rotate(randomAngle())
        .add(basePosition);
      if (
        createObjectGroup(
...
function placePlayerBaseBerries(args) {
  let [get, basePosition, baseResourceConstraint] = getPlayerBaseArgs(args);
  for (let tries = 0; tries < get("maxTries", 30); ++tries) {
    let position = new Vector2D(0, get("distance", 12))
      .rotate(randomAngle())
      .add(basePosition);
    if (
      createObjectGroup(
...
function placePlayerBaseMines(args) {
  ...
  let angleBetweenMines = randFloat(
    get("minAngle", Math.PI / 6),
    get("maxAngle", Math.PI / 3)
  );
  ...
  for (let tries = 0; tries < get("maxTries", 75); ++tries) {
    ...
    for (let i = 0; i < mineCount; ++i) {
      ...
      pos[i] = new Vector2D(0, get("distance", 12))
        .rotate(angle)
        .add(basePosition)
        .round();
      if (
        !g_Map.validTilePassable(pos[i]) ||
        !baseResourceConstraint.allows(pos[i])
      ) {
        pos = undefined;
        break;
      }
    ...
    for (let i = 0; i < mineCount; ++i) {
      if (args.types[i].type && args.types[i].type == "stone_formation") {
        createStoneMineFormation(
          pos[i],
          args.types[i].template,
          args.types[i].terrain
        );
      ...
      createObjectGroup(
        new SimpleGroup(
          [new SimpleObject(args.types[i].template, 1, 1, 0, 0)].concat(
            groupElements
          ),
          true,
          args.BaseResourceClass,
...
function placePlayerBaseTrees(args) {
  ...
  for (let x = 0; x < get("maxTries", 30); ++x) {
    let position = new Vector2D(
      0,
      randFloat(get("minDist", 11), get("maxDist", 13))
    )
      .rotate(randomAngle())
      .add(basePosition)
      .round();
...
function placePlayerBaseTreasures(args) {
  ...
  for (let resourceTypeArgs of args.types) {
    get = (property, defaultVal) =>
      resourceTypeArgs[property] === undefined
        ? defaultVal
        : resourceTypeArgs[property];
...
function placePlayerBaseDecoratives(args) {
  ...
  for (let i = 0; i < get("count", scaleByMapSize(2, 5)); ++i) {
    ...
    for (let x = 0; x < get("maxTries", 30); ++x) {
      let position = new Vector2D(
        0,
        randIntInclusive(get("minDist", 8), get("maxDist", 11))
      )
        .rotate(randomAngle())
        .add(basePosition)
        .round();
...
function placePlayersNomad(playerClass, constraints) {
  ...
  for (let i = 0; i < numPlayers; ++i) {
    let objects = getStartingEntities(playerIDs[i])
      .filter((ents) => ents.Template.startsWith("units/"))
      .map(
        (ents) =>
          new SimpleObject(
            ents.Template,
    ...
    let ccCost = Engine.GetTemplate(
      "structures/" + getCivCode(playerIDs[i]) + "/civil_centre"
    ).Cost.Resources;
      ...
      let count = Math.max(
        ...
        Math.ceil(
          (ccCost[resourceType] - (g_MapSettings.StartingResources || 0)) /
            Engine.GetTemplate(treasureTemplate).Treasure.Resources[
              resourceType
            ]
        )
      );
    ...
    for (let distanceFactor of [1, 1 / 2, 1 / 4, 0])
      if (
        createObjectGroups(
          ...
          new AndConstraint([
            constraint,
            avoidClasses(playerClass, distance * distanceFactor),
          ]),
          1,
          200,
          false
    ...
    if (!success)
      throw new Error(
        "Could not place starting units for player " + playerIDs[i] + "!"
      );
  }
...
function sortPlayers(playerIDs) {
  return shuffleArray(playerIDs).sort(
    (playerID1, playerID2) =>
      getPlayerTeam(playerID1) - getPlayerTeam(playerID2)
  );
}
...
function sortAllPlayers() {
  let playerIDs = [];
  for (let i = 0; i < getNumPlayers(); ++i) playerIDs.push(i + 1);
...
function partitionPlayers(playerIDs) {
  ...
  let teams = teamIDs.map((teamID) =>
    playerIDs.filter((playerID) => getPlayerTeam(playerID) == teamID)
  );
  if (teamIDs.indexOf(-1) != -1)
    teams = teams.concat(
      teams.splice(teamIDs.indexOf(-1), 1)[0].map((playerID) => [playerID])
    );
...
function playerPlacementCustomAngle(radius, center, playerAngleFunc) {
  ...
  for (let i = 0; i < numPlayers; ++i) {
    ...
    playerPosition[i] = Vector2D.add(
      center,
      new Vector2D(radius, 0).rotate(-playerAngle[i])
    ).round();
  }
...
function playerPlacementArcs(
  ...
  let eastPosition = playerPlacementArc(
    east,
    center,
    radius,
    mapAngle + startAngle,
  ...
  let westPosition = playerPlacementArc(
    west,
    center,
    radius,
    mapAngle - startAngle,
...
function playerPlacementRiver(angle, width, center = undefined) {
  ...
  for (let i = 0; i < numPlayers; ++i) {
    ...
    playerPosition[i] = new Vector2D(
      ...
      fractionToTiles(
        ((i - 1 + offsetDivident) / 2 + 1) /
          ((numPlayers + offsetDivisor) / 2 + 1)
      )
    )
      .rotateAround(angle, centerPosition)
      .round();
  }
...
function groupPlayersByArea(playerIDs, locations) {
  ...
  heapsPermute(
    shuffleArray(locations).slice(0, playerIDs.length),
    (v) => v.clone(),
    (permutation) => {
      let dist = 0;
      let teamDist = 0;
      ...
      if (teamSize) dist += teamDist / teamSize;
...
function groupPlayersCycle(startLocations) {
  ...
  for (let i = 0; i < g_MapSettings.PlayerData.length - 1; ++i) {
    ...
    if (teams.indexOf(t) == -1 && t !== undefined) teams.push(t);
  }

  playerIDs = sortPlayers(playerIDs);

  if (!teams.length) return [playerIDs, startLocations];
  ...
  for (let s = 0; s < playerIDs.length; ++s) {
    ...
    for (let pi = 0; pi < playerIDs.length - 1; ++pi) {
      ...
      if (teams.indexOf(t1) === -1) continue;

...
function getTreeCounts(minTrees, maxTrees, forestRatio) {
  return [forestRatio, 1 - forestRatio].map(
    (p) => p * scaleByMapSize(minTrees, maxTrees)
  );
}
...
function createForests(
  ...
  if (typeof treeCount === "number") {
    ...
    numberOfForests = Math.floor(
      numberOfTrees /
        (scaleByMapSize(3, 6) * getNumPlayers() * forestVariants.length)
    );
  } else {
  ...
  for (const forestVariant of forestVariants)
    createAreas(
      new ChainPlacer(
        ...
        numberOfTrees / numberOfForests,
        0.5
      ),
      [
        new LayeredPainter(
          [forestVariant.borderTerrains, forestVariant.interiorTerrains],
          [2]
        ),
 ...
 */
function createDefaultForests(
  terrainSet,
  constraints,
  tileClass,
  totalNumberOfTrees
) {
  return createForests(terrainSet, constraints, tileClass, {
...
function createStragglerTrees(
  ...
  for (let templateName of templateNames)
    createObjectGroupsDeprecated(
      new SimpleGroup(
        [new SimpleObject(templateName, 1, 1, 0, 3)],
        true,
        tileClass
      ),
      0,
...
function createBalancedMines(
  ...
  if (randomness > 0 && randomness < 1) {
    largeCount = Math.round(
      largeCount * randFloat(1 - randomness, 1 + randomness)
    );
    smallCount = Math.round(
      smallCount * randFloat(1 - randomness, 1 + randomness)
    );
    randomSmallCount = Math.round(
      randomSmallCount * randFloat(1 - randomness, 1 + randomness)
    );
  }

  const arrayConstraints = Array.isArray(constraints)
    ? constraints
    : [constraints];
  ...
  createObjectGroups(
    ...
    0,
    new AndConstraint([
      avoidClasses(clMine, scaleByMapSize(25, 50)),
      ...arrayConstraints,
    ]),
    largeCount,
    100
  ...
  createObjectGroups(
    ...
    0,
    new AndConstraint([
      avoidClasses(clMine, scaleByMapSize(18, 35)),
      ...arrayConstraints,
    ]),
    smallCount,
    50
 ...
 */
function createBalancedMetalMines(
  oSmall,
  oLarge,
  clMine,
  constraints,
  ...
  return createBalancedMines(
    ...
    {
      largeCount:
        Math.max(scaleByMapSize(1, 9), getNumPlayers() * 1.8 - 0.8) * counts,
 ...
 */
function createBalancedStoneMines(
  oSmall,
  oLarge,
  clMine,
  constraints,
  ...
  return createBalancedMines(
    ...
    {
      largeCount:
        Math.max(scaleByMapSize(1, 9), getNumPlayers() * 1.25) * counts,
 ...
 */
function createStoneMineFormation(
  position,
  templateName,
  terrain,
  radius = 2.5,
  ...
  for (let i = 0; i < count; ++i) {
    let pos = Vector2D.add(
      position,
      new Vector2D(radius + randFloat(0, maxOffset), 0).rotate(-angle)
    ).round();
...
function placeDocks(
  ...
  if (!areaSearchEnd || !areaSearchEnd.getPoints().length) return;
  ...
  for (let i = 0; i < count; ++i)
    for (let tries = 0; tries < retryFactor; ++tries) {
      ...
      let positionDock = findLocationInDirectionBasedOnHeight(
        positionWaterLarge,
        positionLand,
        heightMin,
        heightMax,
      ...
      let angle = positionDock.angleTo(
        Vector2D.average(
          new DiskPlacer(8, positionDock).place(stayClasses(tileClassWater, 0))
        )
      );

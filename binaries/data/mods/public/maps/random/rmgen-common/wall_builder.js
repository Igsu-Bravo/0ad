...
function loadWallsetsFromCivData() {
  ...
  for (let civ in g_CivData) {
    ...
    for (let path of civInfo.WallSets) {
      ...
      if (path.split("/").indexOf(civ) != -1) style = civ + "/" + style;
...
function loadWallset(wallsetPath, civ) {
  ...
  for (let element in wallsetData.templates)
    if (element == "curves")
      ...
      newWallset[element] = readyWallElement(
        wallsetData.templates[element],
        civ
      );
...
function createDefaultFortressTypes() {
   ...
   */
  let addFortress = (type, walls) =>
    (defaultFortresses[type] = { wall: walls.concat(walls, walls, walls) });
...
function getWallElement(element, style) {
  ...
  if (g_WallStyles[style][element]) return g_WallStyles[style][element];
  ...
  let ret = wallset.tower
    ? clone(wallset.tower)
    : { angle: 0, bend: 0, length: 0, indent: 0 };

  switch (element) {
    case "cornerIn":
      if (wallset.curves)
        for (let curve of wallset.curves)
          if (curve.bend == quarterBend) ret = curve;
    ...
    case "entryTower":
      ret.templateName = g_CivData[civ]
        ? "structures/" + civ + "/defense_tower"
        : "structures/palisades_watchtower";
    ...
    case "end":
      if (wallset.end) ret = wallset.end;
      break;
    ...
    default:
      if (element.startsWith("gap_")) {
        ...
        if (!g_CivData[civ]) civ = Object.keys(g_CivData)[0];

        let templateName = "structures/" + civ + "/" + element;
        if (Engine.TemplateExists(templateName)) {
          ret.indent =
            ret.length *
            (element == "outpost" || element.endsWith("_tower") ? -3 : 3.5);
          ret.templateName = templateName;
          ret.length = 0;
        } else
          warn(
            "Unrecognised wall element: '" +
              element +
              "' (" +
              style +
              "). Defaulting to " +
              (wallset.tower ? "'tower'." : "a blank element.")
          );
      }
  }
...
function readyWallElement(path, civCode) {
  ...
  let length = template.wallPiece
    ? template.wallPiece.length
    : template.obstruction.shape.width;

  return deepfreeze({
    templateName: path,
    angle: template.wallPiece ? template.wallPiece.angle : Math.PI,
    length: length / TERRAIN_TILE_SIZE,
    indent: template.wallPiece
      ? template.wallPiece.indent / TERRAIN_TILE_SIZE
      : 0,
...
function getWallAlignment(
  ...
  for (let i = 0; i < wall.length; ++i) {
    ...
    if (!element && i == 0) {
      warn(
        "Not a valid wall element: style = " +
          style +
          ", wall[" +
          i +
          "] = " +
    ...
    alignment.push({
      position: Vector2D.sub(
        wallPosition,
        new Vector2D(element.indent, 0).rotate(-orientation)
      ),
    ...
    if (i + 1 < wall.length) {
      ...
      let distance =
        (element.length + nextElement.length) / 2 - g_WallStyles[style].overlap;
      ...
      wallPosition.add(
        new Vector2D(distance, 0).rotate(-orientation).perpendicular()
      );
    }
  }
  return alignment;
...
function getCenterToFirstElement(alignment) {
  return alignment.reduce(
    (result, align) =>
      result.sub(Vector2D.div(align.position, alignment.length)),
    new Vector2D(0, 0)
  );
}
...
function validateStyle(style, playerId = 0) {
  if (!style || !g_WallStyles[style]) {
    if (playerId == 0) return Object.keys(g_WallStyles)[0];
...
function placeWall(
  ...
  for (let align of getWallAlignment(position, wall, style, orientation))
    if (
      align.templateName &&
      g_Map.inMapBounds(align.position) &&
      constraint.allows(align.position.clone().floor())
    )
      entities.push(
...
function placeCustomFortress(
  ...
  if (centerToFirstElement === undefined)
    centerToFirstElement = getCenterToFirstElement(
      getWallAlignment(new Vector2D(0, 0), fortress.wall, style)
    );
  ...
  let position = Vector2D.sum([
    ...
    new Vector2D(centerToFirstElement.y, 0)
      .perpendicular()
      .rotate(-orientation),
  ]);
...
function placeFortress(
  ...
  return placeCustomFortress(
    centerPosition,
    g_FortressTypes[type],
    style,
    playerId,
    orientation,
    constraints
...
function placeLinearWall(
  ...
  for (let element of wallPart)
    if (getWallElement(element, style).bend != 0)
      warn(
        "placeLinearWall : Bending is not supported by this function, but the following bending wall element was used: " +
          element
      );
  ...
  if (endWithFirst)
    numParts = Math.ceil(
      (totalLength - getWallElement(wallPart[0], style).length) / wallPartLength
    );
  ...
  if (endWithFirst)
    scaleFactor =
      totalLength /
      (numParts * wallPartLength + getWallElement(wallPart[0], style).length);

  // Setup angle
  let wallAngle = getAngle(
    startPosition.x,
    startPosition.y,
    targetPosition.x,
    targetPosition.y
  ...
  for (let partIndex = 0; partIndex < numParts; ++partIndex)
    for (let elementIndex = 0; elementIndex < wallPart.length; ++elementIndex) {
      ...
      // Indent correction
      let place = Vector2D.add(
        position,
        new Vector2D(0, wallEle.indent).rotate(-wallAngle)
      );

      if (
        wallEle.templateName &&
        g_Map.inMapBounds(place) &&
        constraint.allows(place.clone().floor())
      )
        entities.push(
          g_Map.placeEntityPassable(
            wallEle.templateName,
...
function placeCircularWall(
  ...
  if (endWithFirst === undefined) endWithFirst = maxAngle < Math.PI * 2 - 0.001; // Can this be done better?

  // Check arguments
  if (maxBendOff > Math.PI / 2 || maxBendOff < 0)
    warn(
      "placeCircularWall : maxBendOff should satisfy 0 < maxBendOff < PI/2 (~1.5rad) but it is: " +
        maxBendOff
    );
  ...
  for (let element of wallPart)
    if (getWallElement(element, style).bend != 0)
      warn(
        "placeCircularWall : Bending is not supported by this function, but the following bending wall element was used: " +
          element
      );
  ...
  let position = Vector2D.add(
    center,
    new Vector2D(radius, 0).rotate(-actualAngle)
  );
  let overlap = g_WallStyles[style].overlap;
  for (let partIndex = 0; partIndex < numParts; ++partIndex)
    for (let wallEle of wallPart) {
      ...
      let target = Vector2D.add(
        center,
        new Vector2D(radius, 0).rotate(-actualAngle - addAngle)
      );
      ...
      if (
        ...
        let entity = g_Map.placeEntityPassable(
          wallEle.templateName,
          playerId,
          place,
          placeAngle + wallEle.angle
...
function placePolygonalWall(
  ...
  let corners = new Array(numCorners)
    .fill(0)
    .map((zero, i) =>
      Vector2D.add(
        centerPosition,
        new Vector2D(radius, 0).rotate(-angleStart - i * angleAdd)
      )
    );

  for (let i = 0; i < numCorners; ++i) {
    let angleToCorner = getAngle(
      corners[i].x,
      corners[i].y,
      centerPosition.x,
      centerPosition.y
    ...
    if (
      g_Map.inMapBounds(corners[i]) &&
      constraint.allows(corners[i].clone().floor())
    ) {
      let entity = g_Map.placeEntityPassable(
        getWallElement(cornerWallElement, style).templateName,
        playerId,
        corners[i],
        angleToCorner
      );
    ...
    if (!skipFirstWall || i != 0) {
      ...
      let cornerPosition = new Vector2D(cornerLength, 0)
        .rotate(-cornerAngle)
        .perpendicular();
...
function placeIrregularPolygonalWall(
  ...
  let defaultWallPartsAssortment = [
    ["short"],
    ["medium"],
    ["long"],
    ["gate", "tower", "short"],
  ...
  let extendingWallPartAssortment = [
    ["tower", "long"],
    ["tower", "medium"],
  ];
  ...
  for (let assortment of extendingWallPartAssortment) {
    ...
    for (let j = 0; j < radius; ++j) {
      if (j % 2 == 0) wallPart = wallPart.concat(assortment);
      else {
        assortment.reverse();
  ...
  for (let i = 0; i < numCorners; ++i) {
    ...
    angleAddList.push(
      (angleToCover / (numCorners - i)) *
        (1 + randFloat(-irregularity, irregularity))
    );
  ...
  for (let i = 0; i < numCorners; ++i) {
    corners.push(
      Vector2D.add(centerPosition, new Vector2D(radius, 0).rotate(-angleActual))
    );

    if (i < numCorners - 1) angleActual += angleAddList[i + 1];
  }
  ...
  for (let wallPart of wallPartsAssortment) {
    ...
    if (length > maxWallPartLength) maxWallPartLength = length;
  }
  ...
  for (let i = 0; i < numCorners; ++i) {
    ...
    for (
      let partIndex = 0;
      partIndex < wallPartsAssortment.length;
      ++partIndex
    ) {
    ...
    if (!skipFirstWall || i != 0) {
      ...
      entities = entities.concat(
        placeLinearWall(
          ...
          Vector2D.sub(
            corners[i],
            new Vector2D(cornerLength, 0).perpendicular().rotate(-startAngle)
          ),
          Vector2D.add(
            corners[targetCorner],
            new Vector2D(cornerLength, 0).rotate(-targetAngle - Math.PI / 2)
          ),
          wallPartList[i],
          style,
          playerId,
...
function placeGenericFortress(
  ...
  while (tries < maxTries && minOverlap > g_WallStyles[style].overlap) {
    ...
    while (true) {
      let indent = randFloat(
        -irregularity * pointDistance,
        irregularity * pointDistance
      );
      let tmp = new Vector2D(radius + indent, 0).rotate(
      ...
      if (numPoints > 3 && distanceToTarget < pointDistance) {
        ...
        overlap =
          pointDistance -
          pointDerivation[numPoints - 1].distanceTo(pointDerivation[0]);
  ...
  log(
    "placeGenericFortress: Reduced overlap to " +
      minOverlap +
      " after " +
      tries +
      " tries"
  ...
  for (
    let pointIndex = 0;
    pointIndex < bestPointDerivation.length;
    ++pointIndex
  ) {
    let start = Vector2D.add(center, bestPointDerivation[pointIndex]);
    let target = Vector2D.add(
      center,
      bestPointDerivation[(pointIndex + 1) % bestPointDerivation.length]
    );
    ...
    if (element.templateName) {
      let pos = Vector2D.add(
        start,
        new Vector2D(start.distanceTo(target) / 2, 0).rotate(-angle)
      );
    ...
    start = Vector2D.add(
      center,
      bestPointDerivation[
        (pointIndex + bestPointDerivation.length - 1) %
          bestPointDerivation.length
      ]
    );

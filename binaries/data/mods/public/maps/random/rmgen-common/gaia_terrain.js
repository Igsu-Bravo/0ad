...
function createMountain(
  ...
  if (!g_Map.inMapBounds(position) || !constraint.allows(position)) return;
  ...
  for (let i = 0; i < mapSize; ++i) {
    gotRet[i] = [];
    for (let j = 0; j < mapSize; ++j) gotRet[i][j] = -1;
  }
  ...
  for (let i = 0; i < numCircles; ++i) {
    ...
    let radius;
    if (queueEmpty) radius = randIntInclusive(minRadius, maxRadius);
    else {
      radius = q.pop();
      queueEmpty = !q.length;
    }
    ...
    for (let ix = sx; ix <= lx; ++ix) {
      for (let iz = sz; iz <= lz; ++iz) {
        ...
        if (
          Math.euclidDistance2D(ix, iz, cx, cz) > radius2 ||
          !g_Map.inMapBounds(pos)
        )
          continue;
    ...
    for (let ix = sx; ix <= lx; ++ix)
      for (let iz = sz; iz <= lz; ++iz) {
        if (
          gotRet[ix][iz] != -2 ||
          (fcc &&
            (x - ix > fcc || ix - x > fcc || z - iz > fcc || iz - z > fcc)) ||
      ...
      for (let iz = sz; iz <= lz; ++iz) {
        ...
        let newHeight =
          ...
          Math.round(
            (2 / 3) *
              clumpHeight *
              (Math.sin(((Math.PI * 2) / 3) * (3 / 4 - distance / radius)) +
                0.5)
          );
        ...
        if (g_Map.getHeight(position) < newHeight)
          ...
          g_Map.getHeight(position) >= newHeight &&
          g_Map.getHeight(position) < newHeight + 4
        )
          g_Map.setHeight(position, newHeight + 4);

        if (terrain) createTerrain(terrain).place(position);

        if (tileClass) tileClass.add(position);
      }
  }
}
...
function createVolcano(
  ...
  let layers = [
    ...
    },
    {
      ...
      painter:
        lavaTextures &&
        new LayeredPainter([terrainTexture, ...lavaTextures], [1, 1, 1]),
      steepness: 1,
    },
  ];
...
function createPatches(
  ...
  for (let size of sizes)
    createAreas(
      ...
      [new TerrainPainter(terrain), new TileClassPainter(tileClass)],
      constraints,
      count
    );
}
...
function paintRiver(args) {
  ...
  let meanderShort = fractionToTiles(
    args.meanderShort / scaleByMapSize(35, 160)
  );
  ...
  let riverCurve = (riverFraction, startAngle, seed) =>
    meanderShort *
      rndRiver(startAngle + fractionToTiles(riverFraction) / 128, seed) +
    meanderLong *
      rndRiver(startAngle + fractionToTiles(riverFraction) / 256, seed);
  ...
  for (let ix = 0; ix < mapSize; ++ix)
    for (let iz = 0; iz < mapSize; ++iz) {
      ...
      if (args.constraint && !args.constraint.allows(vecPoint)) continue;

      // Compute the shortest distance to the river.
      let distanceToRiver = distanceOfPointFromLine(
        args.start,
        args.end,
        vecPoint
      );
      ...
      let river = Vector2D.sub(
        vecPoint,
        unitVecRiver.perpendicular().mult(distanceToRiver)
      );
      ...
      let shoreDist1 =
        riverCurve1 + distanceToRiver - deviation - args.width / 2;
      let shoreDist2 =
        riverCurve2 + distanceToRiver - deviation + args.width / 2;
      ...
      if (shoreDist1 < 0 && shoreDist2 > 0) {
        ...
        if (shoreDist1 > -args.fadeDist)
          height +=
            (args.heightLand - args.heightRiverbed) *
            (1 + shoreDist1 / args.fadeDist);
        else if (shoreDist2 < args.fadeDist)
          height +=
            (args.heightLand - args.heightRiverbed) *
            (1 - shoreDist2 / args.fadeDist);

        if (args.minHeight === undefined || height < args.minHeight)
          g_Map.setHeight(vecPoint, height);

        if (args.waterFunc) args.waterFunc(vecPoint, height, riverFraction);
      } else if (args.landFunc) args.landFunc(vecPoint, shoreDist1, shoreDist2);
    }
}
...
function rndRiver(f, seed) {
  ...
  for (let i = 0; i <= f; ++i) rndRw = 10 * (rndRw % 1);

  let rndRr = f % 1;
  let retVal = (Math.floor(f) % 2 ? -1 : 1) * rndRr * (rndRr - 1);

  let rndRe = Math.floor(rndRw) % 5;
  if (rndRe == 0) retVal *= 2.3 * (rndRr - 0.5) * (rndRr - 0.5);
  else if (rndRe == 1) retVal *= 2.6 * (rndRr - 0.3) * (rndRr - 0.7);
  else if (rndRe == 2)
    retVal *=
      22 * (rndRr - 0.2) * (rndRr - 0.3) * (rndRr - 0.3) * (rndRr - 0.8);
  else if (rndRe == 3)
    retVal *=
      180 *
      (rndRr - 0.2) *
      ...
      (rndRr - 0.6) *
      (rndRr - 0.8);
  else if (rndRe == 4) retVal *= 2.6 * (rndRr - 0.5) * (rndRr - 0.7);
 ...
 */
function createTributaryRivers(
  riverAngle,
  riverCount,
  riverWidth,
  heightRiverbed,
  ...
  if (shallowTileClass)
    riverConstraint = new AndConstraint([
      riverConstraint,
      avoidClasses(shallowTileClass, 2),
    ]);

  for (let i = 0; i < riverCount; ++i) {
    // Determining tributary river location
    let searchCenter = new Vector2D(
      fractionToTiles(randFloat(tapering, 1 - tapering)),
      mapCenter.y
    );
    ...
    let searchEnd = Vector2D.sub(searchCenter, distanceVec).rotateAround(
      riverAngle,
      mapCenter
    );

    let start = findLocationInDirectionBasedOnHeight(
      searchStart,
      searchEnd,
      heightRange[0],
      heightRange[1],
    ...
    start.round();
    let end = Vector2D.add(
      mapCenter,
      new Vector2D(mapSize, 0).rotate(
        riverAngle - sign * randFloat(maxAngle, 2 * Math.PI - maxAngle)
      )
    ).round();
  ...
  if (shallowTileClass) {
    ...
    for (let z of [0.25, 0.75])
      createPassage({
        start: new Vector2D(mapBounds.left, fractionToTiles(z)).rotateAround(
          riverAngle,
          mapCenter
        ),
        end: new Vector2D(mapBounds.right, fractionToTiles(z)).rotateAround(
          riverAngle,
          mapCenter
        ),
...
function createPassage(args) {
  let bound = (x) =>
    Math.max(0, Math.min(Math.round(x), g_Map.height.length - 1));

  let startHeight =
    args.startHeight !== undefined
      ? args.startHeight
      : g_Map.getHeight(new Vector2D(bound(args.start.x), bound(args.start.y)));
  let endHeight =
    args.endHeight !== undefined
      ? args.endHeight
      : g_Map.getHeight(new Vector2D(bound(args.end.x), bound(args.end.y)));
  ...
  for (
    ...
    lengthFraction <= 1;
    lengthFraction += lengthStep
  ) {
    let locationLength = Vector2D.add(
      args.start,
      Vector2D.mult(passageVec, lengthFraction)
    );
    let halfPassageWidth =
      (args.startWidth + (args.endWidth - args.startWidth) * lengthFraction) /
      2;
    let passageHeight =
      startHeight + (endHeight - startHeight) * lengthFraction;

    for (
      let stepWidth = -halfPassageWidth;
      stepWidth <= halfPassageWidth;
      stepWidth += 0.5
    ) {
      let location = Vector2D.add(
        locationLength,
        Vector2D.mult(widthDirection, stepWidth)
      ).round();

      if (
        !g_Map.inMapBounds(location) ||
      ...
      let smoothDistance =
        args.smoothWidth + Math.abs(stepWidth) - halfPassageWidth;

      g_Map.setHeight(
        location,
        smoothDistance > 0
          ? (g_Map.getHeight(location) * smoothDistance +
              passageHeight / smoothDistance) /
              (smoothDistance + 1 / smoothDistance)
          : passageHeight
      );

      if (args.tileClass) args.tileClass.add(location);

      if (args.edgeTerrain && smoothDistance > 0)
        createTerrain(args.edgeTerrain).place(location);
      else if (args.terrain) createTerrain(args.terrain).place(location);
    }
  }
 ...
 */
function findLocationInDirectionBasedOnHeight(
  startPoint,
  endPoint,
  minHeight,
  maxHeight,

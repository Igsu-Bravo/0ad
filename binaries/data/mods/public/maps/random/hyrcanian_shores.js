...
placePlayerBases({
  PlayerPlacement: [
    sortAllPlayers(),
    playerPlacementLine(startAngle, mapCenter, fractionToTiles(0.2)),
  ],
...
paintRiver({
  ...
  end: new Vector2D(mapBounds.right, mapBounds.top).rotateAround(
    startAngle,
    mapCenter
  ),
  ...
  waterFunc: (position, height, riverFraction) => {
    if (height < heightShore2) clWater.add(position);
...
const highlandShare = 0.4;
{
  var types = [
    [
      [tGrassDForest, tGrass, pForestD],
      [tGrassDForest, pForestD],
    ],
  ];
  ...
  for (const type of types)
    createAreas(
      new ClumpPlacer(
        (forestTrees * (1.0 - highlandShare)) / numberOfForests,
        0.1,
        0.1,
        Infinity
      ),
...
g_Map.log("Creating highland forests");
{
  var types = [
    [
      [tGrassDForest, tGrass, pForestP],
      [tGrassDForest, pForestP],
    ],
  ];
  ...
  for (const type of types)
    createAreas(
      new ClumpPlacer(
        (forestTrees * highlandShare) / numberOfForests,
        0.1,
        0.1,
        Infinity
      ),
...
group = new SimpleGroup([new SimpleObject(oGoat, 2, 3, 0, 2)], true, clFood);
createObjectGroupsDeprecated(
  group,
  0,
...
group = new SimpleGroup([new SimpleObject(oBoar, 2, 3, 0, 2)], true, clFood);
createObjectGroupsDeprecated(
  group,
  0,
  avoidClasses(clWater, 0, clForest, 0, clPlayer, 20, clHill, 0, clFood, 20),

...
var islandLocations = [new Vector2D(0.05, 0.05), new Vector2D(0.95, 0.95)].map(
  (v) => v.mult(mapSize).rotateAround(-swapAngle, mapCenter)
);
...
for (let island = 0; island < 2; ++island) {
  ...
  for (let i = 0; i < nbSubIsland + 1; ++i) {
    ...
    let location = Vector2D.add(
      islandLocations[island],
      new Vector2D(radiusIsland, 0).rotate(-angle)
    );
    createArea(
      new ClumpPlacer(
        diskArea(fractionToTiles(0.09)),
        0.6,
        0.03,
        Infinity,
        location
  ...
  for (let i = 0; i < nbCreeks + 1; ++i) {
    ...
    let location = Vector2D.add(
      islandLocations[island],
      new Vector2D(radiusCreeks, 0).rotate(-angle)
    );
    createArea(new ClumpPlacer(creeksArea(), 0.4, 0.01, Infinity, location), [
  ...
  for (let i = 0; i < nbBeaches + 1; ++i) {
    let angle =
      ...
      swapAngle;
    let start = Vector2D.add(
      islandLocations[island],
      new Vector2D(radiusIsland, 0).rotate(-angle)
    );
    let end = Vector2D.add(
      islandLocations[island],
      new Vector2D(radiusBeach, 0).rotate(-angle)
    );
...
var p = 0;
for (let island = 0; island < 2; ++island) {
  let playersPerIsland =
    island == 0 ? Math.ceil(numPlayers / 2) : Math.floor(numPlayers / 2);

  for (let i = 0; i < playersPerIsland; ++i) {
    playerAngle[p] =
      Math.PI * ((i + 0.5) / (2 * playersPerIsland) + island) + swapAngle;
    playerPosition[p] = Vector2D.add(
      islandLocations[island],
...
placePlayerBases({
  ...
  CityPatch: {
    ...
    coherence: 0.8,
    radius: 6,
    painters: [new TileClassPainter(clSettlement)],
  },
  Chicken: {},
  Berries: {
    template: eBush,
  ...
  Mines: {
    types: [{ template: eMetalMine }, { template: eStoneMine }],
  },
...
createAreas(
  ...
  [stayClasses(clIsland, 2), avoidClasses(clPlayer, 6, clPassage, 2)],
...
for (let mapX = 0; mapX < mapSize; ++mapX)
  for (let mapZ = 0; mapZ < mapSize; ++mapZ) {
    ...
    if (terrain == tCliffs || terrain == tSteepCliffs) clCliffs.add(position);
  }
...
function getCosricaSardiniaTerrain(position) {
  ...
  if (height >= 0.5 && height < 1.5 && isShore) return tSandTransition;

  // Paint land cliffs and grass
  if (height >= 1 && !isWater) {
    if (isPassage) return tGrass;

    if (slope >= 1.25) return height > 25 ? tSteepCliffs : tCliffs;

    if (height < 17) return tGrass;

    if (slope < 0.625) return tHill;

    return tMountain;
  }

  if (slope >= 1.125) return tCliffs;

  if (height >= 1.5) return undefined;

  if (height >= -0.75) return tShore;

  if (height >= -3) return tShoreBlend;

  if (height >= -6) return tCreekWater;

  if (height > -10 && slope < 0.75) return tDeepWater;
...
createAreas(
  ...
  [new TerrainPainter(tLushGrass), new TileClassPainter(clForest)],
  avoidClasses(clWater, 1, clPlayer, 6, clBaseResource, 3, clCliffs, 1),
...
for (let group of plantGroups)
  createObjectGroupsDeprecated(
    group,
    0,
    avoidClasses(clWater, 0, clBaseResource, 4, clShore, 3),
...
createObjectGroupsDeprecated(
  ...
  0,
  avoidClasses(clWater, 3, clBaseResource, 4, clPlayer, 6),
  scaleByMapSize(20, 100),
  50
);
...
createObjectGroupsDeprecated(
  ...
  0,
  [stayClasses(clWater, 3), avoidClasses(clCreek, 3, clShore, 3)],
...
placePlayersNomad(
  clPlayer,
  avoidClasses(clWater, 4, clForest, 1, clBaseResource, 4, clCliffs, 4)
);

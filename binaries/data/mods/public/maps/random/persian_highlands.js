...
else setBiome("persian_highlands/summer");
...
placePlayerBases({
  ...
  Mines: {
    ...
    groupElements: shuffleArray(aBushes).map(
...
createAreas(
  ...
  [new TerrainPainter(tRocky), new TileClassPainter(clPatch)],
...
createAreas(
  ...
  [new TerrainPainter([tRocky, tRocks]), new TileClassPainter(clPatch)],
...
createAreas(
  ...
  [new TerrainPainter([tGrass]), new TileClassPainter(clPatch)],
  avoidClasses(clPatch, 2, clPlayer, 4),
  scaleByMapSize(15, 50)
);
...
for (let i = 0; i < scaleByMapSize(20, 80); ++i)
  createMountain(
    ...
    avoidClasses(
      ...
      5,
      clHill,
      Math.floor(scaleByMapSize(18, 25))
    ),
...
var types = [
  [
    [tDirtMain, tForestFloor, pForestO],
    [tForestFloor, pForestO],
  ],
  [
    [tDirtMain, tForestFloor, pForestO],
    [tForestFloor, pForestO],
...
group = new SimpleGroup([new SimpleObject(oCamel, 2, 3, 0, 2)], true, clFood);
createObjectGroupsDeprecated(
  group,
  0,
  stayClasses(clCP, 2),

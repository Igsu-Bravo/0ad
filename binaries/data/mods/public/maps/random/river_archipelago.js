...
const oPalm1 = "gaia/tree/palm_tropic";
const oPalm2 = "gaia/tree/palm_tropical";
...
stripWidthsRight = stripWidthsRight.map((strip) => [
  1 - strip[1],
  1 - strip[0],
]);
...
for (let i = 0; i < stripWidths.length; ++i) {
  ...
  for (let j = 0; j < scaleByMapSize(20, 100); ++j) {
    let position = new Vector2D(
      ...
      fractionToTiles(randFloat(...stripWidths[i]))
    )
...
for (let i = 0; i < numPlayers; ++i)
  createArea(
    new ChainPlacer(1, 6, 40, 1, playerPosition[i], 0, [
...
var areaWater = createArea(
  new HeightPlacer(
    Elevation_IncludeMin_ExcludeMax,
    -Infinity,
    heightWaterLevel
  ),
  [new TerrainPainter(tWater), new TileClassPainter(clWater)]
);
...
createAreas(
  ...
  [avoidClasses(clPlayerTerritory, 0, clHill, 5, clGaia, 1, clWater, 2)],
  scaleByMapSize(1, 5)
);

createBumps(
  avoidClasses(clPlayer, 8, clWater, 2),
...
var types = [
  [
    [tGrass, tGrass, tGrass, tGrass, pForestD],
    [tGrass, tGrass, tGrass, pForestD],
  ],
  [
    [tGrass, tGrass, tGrass, tGrass, pForestP1],
    [tGrass, tGrass, tGrass, pForestP1],
  ],
  [
    [tGrass, tGrass, tGrass, tGrass, pForestP2],
    [tGrass, tGrass, tGrass, pForestP2],
  ],
];
...
for (let type of types)
  createAreas(
    ...
    avoidClasses(clPlayer, 12, clForest, 6, clHill, 0, clGaia, 1, clWater, 2),
    num
  );
...
for (let size of [
  ...
  createAreas(
    new ChainPlacer(1, Math.floor(scaleByMapSize(3, 5)), size, 0.5),
    [new TerrainPainter(tPlants), new TileClassPainter(clDirt)],
    avoidClasses(
      clWater,
      8,
      clForest,
...
createObjectGroups(
  ...
  scaleByMapSize(25, 400),
  50
);

g_Map.log("Creating small grass tufts");
createObjectGroups(
  new SimpleGroup([new SimpleObject(aBush1, 1, 2, 0, 1)]),
  0,
  avoidClasses(
    clWater,
    4,

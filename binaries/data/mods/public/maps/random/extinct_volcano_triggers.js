...
var waterWarningTexts = [
  ...
  markForTranslation(
    "The rivers are standing high, we need to find a safe place!"
  ),
...
Trigger.prototype.GarrisonWoodenTowers = function () {
  for (let gaiaEnt of Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_RangeManager
  ).GetEntitiesByPlayer(0)) {
    let cmpIdentity = Engine.QueryInterface(gaiaEnt, IID_Identity);
    if (!cmpIdentity || !cmpIdentity.HasClass("Tower")) continue;
...
Trigger.prototype.RaiseWaterLevelStep = function () {
  ...
  this.DebugLog(
    "Raising water level to " +
      Math.round(newLevel) +
      " took " +
      (cmpTimer.GetTime() - time)
  );

  if (newLevel < maxWaterLevel)
    this.DoAfterDelay(
      (debugWaterRise ? 10 : randFloat(...waterIncreaseTime) * 60) * 1000,
      "RaiseWaterLevelStep",
      {}
    );
  else this.DebugLog("Water reached final level");
  ...
  for (let ent of cmpRangeManager.GetGaiaAndNonGaiaEntities()) {
    ...
    if (pos.y + drownHeight >= newLevel) continue;
    ...
    if (cmpHealth && cmpIdentity.HasClass(drownClass)) {
      ...
      if (debugLog)
        killedTemplates[templateName] =
          (killedTemplates[templateName] || 0) + 1;
    ...
    if (!cmpVisualActor) continue;

    let height = cmpPosition.GetHeightOffset();
    let rot = cmpPosition.GetRotation();

    let actorTemplate =
      cmpTemplateManager.GetTemplate(templateName).VisualActor.Actor;
...
{
  let waterRiseTime = debugWaterRise
    ? 0
    : InitAttributes.settings.SeaLevelRiseTime || 0;
  let cmpTrigger = Engine.QueryInterface(SYSTEM_ENTITY, IID_Trigger);
  cmpTrigger.GarrisonWoodenTowers();
  cmpTrigger.DoAfterDelay(
    (waterRiseTime - waterRiseNotificationDuration) * 60 * 1000,

...
for (let i = 0; i < numPlayers; ++i)
  createArea(
    new ClumpPlacer(
      diskArea(1.4 * defaultPlayerBaseRadius()),
      0.8,
      0.1,
      Infinity,
      playerPosition[i]
...
for (let m = 0; m < randIntInclusive(8, 17); ++m) {
  ...
  createArea(
    ...
    [
      new LayeredPainter(
        [tCliff, tForestFloor2],
        [Math.floor(elevRand / 3), 40]
      ),
      new SmoothElevationPainter(
        ELEVATION_MODIFY,
        elevRand,
...
createObjectGroup(
  new SimpleGroup(
    [
      new SimpleObject(
        ...
        6,
        25,
        Math.floor(fractionToTiles(0.25))
      ),
    ],
    true,
    clBaseResource,
  ...
  0,
  [
    avoidClasses(clBaseResource, 20, clPlayer, 40, clMountain, 4),
    stayClasses(clHill, 10),
  ]
);
...
createDecoration(
  [
    ...
    [
      new SimpleObject(aGrass, 2, 10, 0, 1.8),
      new SimpleObject(aGrassShort, 3, 10, 1.2, 2.5),
    ],
    [
      new SimpleObject(aBushMedium, 1, 5, 0, 2),

...
var landElevationPainter = new SmoothElevationPainter(
  ELEVATION_SET,
  heightLand,
  4
);
...
var unknownMapFunctions = {
  ...
  naval: ["CentralSea", "CentralRiverNaval", "Archipelago", "RiversAndLake"],
};
...
function createUnknownMap() {
  const landscape =
    g_MapSettings.Landscape ||
    pickRandom([...unknownMapFunctions.land, ...unknownMapFunctions.naval]);
...
function unknownArchipelago() {
  ...
  if (type == 1) {
    ...
    createAreas(
      ...
      [landElevationPainter, new TileClassPainter(clLand)],
      null,
...
function unknownContinent() {
  ...
  if (randBool(1 / 3)) {
    g_Map.log(
      "Creating peninsula (i.e. half the map not being surrounded by water)"
    );
    let angle = randomAngle();
    let peninsulaPosition1 = Vector2D.add(
      mapCenter,
    ...
    let peninsulaPosition2 = Vector2D.add(
      mapCenter,
      new Vector2D(fractionToTiles(0.35), 0).rotate(-angle)
    );
    createArea(
      new ClumpPlacer(
        diskArea(fractionToTiles(0.33)),
        0.9,
        0.01,
        Infinity,
        peninsulaPosition2
...
function unknownCentralSeaOrIsthmus(isthmus) {
  ...
  paintRiver({
    ...
    waterFunc: (position, height, riverFraction) => {
      if (height < 0) clWater.add(position);
    },
  ...
  if (!isNomad()) {
    [playerIDs, playerPosition] = playerPlacementRiver(
  ...
  if (isthmus) {
    ...
    let [isthmusStart, isthmusEnd] = centralRiverCoordinates(
      startAngle + Math.PI / 2
    );
    createArea(
      new PathPlacer(
...
function unknownCentralRiver(shallows) {
  ...
  g_Map.log(
    "Creating small water spots at the map border to ensure separation of players"
  );
  for (let coord of [coord1, coord2])
    createArea(
      new ClumpPlacer(
        diskArea(scaleByMapSize(5, 10)),
        0.95,
        0.6,
        Infinity,
        coord
  ...
  if (shallows) {
    ...
    for (let i = 0; i <= randIntInclusive(1, scaleByMapSize(4, 8)); ++i) {
      ...
      createPassage({
        start: new Vector2D(location, mapBounds.top).rotateAround(
          startAngle,
          mapCenter
        ),
        end: new Vector2D(location, mapBounds.bottom).rotateAround(
          startAngle,
          mapCenter
        ),
...
function unknownRiversAndLake() {
...

  {
    ...
    for (let river of distributePointsOnCircle(
      numPlayers,
  ...
  if (!isNomad && lake && randBool(2 / 3)) {
    ...
    createArea(
      new ClumpPlacer(
        diskArea(fractionToTiles(0.05)),
        0.7,
        0.1,
        Infinity,
        mapCenter
      ),
      [landElevationPainter, new TileClassPainter(clWater)]
    );
  }
}
...
function unknownEdgeSeas() {
  ...
  for (let side of pickRandom([[0], [Math.PI], [0, Math.PI]]))
    paintRiver({
      parallel: true,
      start: new Vector2D(mapBounds.left, mapBounds.top).rotateAround(
        side + startAngle,
        mapCenter
      ),
      end: new Vector2D(mapBounds.left, mapBounds.bottom).rotateAround(
        side + startAngle,
        mapCenter
      ),
...
function unknownGulf() {
  ...
  if (!isNomad()) {
    ...
    playerPosition = playerPlacementCustomAngle(
      ...
      (i) =>
        ...
        (2 / 3) *
          Math.PI *
          (-1 + (numPlayers == 1 ? 1 : (2 * i) / (numPlayers - 1)))
    )[0];
  ...
  for (let gulfPart of gulfParts) {
    let position = Vector2D.sub(
      mapCenter,
      new Vector2D(gulfPart.distance, 0).rotate(-startAngle)
    ).round();
    createArea(
...
function unknownLakes() {
  ...
  createAreas(
    ...
    [
      ...
      randBool() ? avoidClasses(clWater, 8) : new NullConstraint(),
    ],
    scaleByMapSize(5, 16)
  );
}
...
function unknownPasses() {
  ...
  if (!isNomad()) {
    [playerIDs, playerPosition, playerAngle, startAngle] =
      playerPlacementCircle(fractionToTiles(0.35));
    markPlayerArea("small");
  } else startAngle = randomAngle();

  g_Map.log("Creating a mountain range between neighboring players");
  for (let mountain of distributePointsOnCircle(
    numPlayers,
    startAngle + Math.PI / numPlayers,
    fractionToTiles(0.5),
    mapCenter
  ...
  let passes = distributePointsOnCircle(
    numPlayers * 2,
    startAngle,
    fractionToTiles(0.35),
    mapCenter
...
function unknownLowlands() {
  ...
  createArea(new MapBoundsPlacer(), new ElevationPainter(heightMountain));
  ...
  for (let valley of distributePointsOnCircle(
    valleys,
    startAngle,
    fractionToTiles(0.35),
    mapCenter
...
function unknownMainland() {
  createArea(new MapBoundsPlacer(), new ElevationPainter(3));
...
function createShoreJaggedness(
  waterHeight,
  borderClass,
  shoreDist,
  inwards = true
...
function createExtensionsOrIslands() {
  ...
  if (rnd == 1) {
    ...
    createAreas(
      new ClumpPlacer(
        Math.square(
          randIntInclusive(scaleByMapSize(8, 15), scaleByMapSize(15, 23))
        ),
        0.8,
        0.1,
        randFloat(0, 0.2)
...
function markPlayerArea(size) {
  for (let i = 0; i < numPlayers; ++i) {
    ...
    if (size == "large")
      createArea(
        new ClumpPlacer(
          diskArea(scaleByMapSize(17, 29) / 3),
          0.6,
          0.3,
          Infinity,
          playerPosition[i]
...
function createUnknownObjects() {
  ...
  createAreas(
    ...
    [
      avoidClasses(clPlayer, 15, clHill, randIntInclusive(6, 18)),
      stayClasses(clLand, 0),
    ],
    randIntInclusive(0, scaleByMapSize(4, 8)) *
      randIntInclusive(1, scaleByMapSize(4, 9))
  );
  ...
  let types = [
    [
      [tForestFloor2, tMainTerrain, pForest1],
      [tForestFloor2, pForest1],
    ],
    [
      [tForestFloor1, tMainTerrain, pForest2],
      [tForestFloor1, pForest2],
    ],
  ];
  ...
  for (let type of types)
    createAreas(
      ...
      [new LayeredPainter(type, [2]), new TileClassPainter(clForest)],
      [
        avoidClasses(
          clPlayer,
          20,
          clForest,
          randIntInclusive(5, 15),
          clHill,
          2
        ),
  ...
  let patchCount =
    (currentBiome() == "generic/savanna" ? 3 : 1) * scaleByMapSize(15, 45);
  for (let patchSize of [
    scaleByMapSize(3, 48),
    scaleByMapSize(5, 84),
    scaleByMapSize(8, 128),
  ])
    createAreas(
      ...
      [
        avoidClasses(clForest, 0, clHill, 2, clDirt, 5, clPlayer, 7),
        stayClasses(clLand, 4),
      ],
      patchCount
    );
  ...
  createObjectGroupsDeprecated(
    ...
    0,
    [
      avoidClasses(clForest, 1, clPlayer, 10, clRock, 10, clHill, 2),
  ...
  createObjectGroupsDeprecated(
    ...
    0,
    [
      avoidClasses(clWater, 0, clForest, 0, clPlayer, 0, clHill, 2),
      stayClasses(clLand, 3),
    ],
  ...
  createObjectGroupsDeprecated(
    new SimpleGroup([
      new SimpleObject(aLillies, 1, 2, 0, 2),
      new SimpleObject(aReeds, 2, 4, 0, 2),
    ]),
    0,
  ...
  createObjectGroupsDeprecated(
    ...
    0,
    [
      avoidClasses(clWater, 1, clHill, 2, clPlayer, 1, clDirt, 1),
      stayClasses(clLand, 3),
    ],
    planetm * scaleByMapSize(13, 200),
    50

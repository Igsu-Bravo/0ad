...
const oTower =
  g_MapSettings.Size >= 256 && getDifficulty() >= 3
    ? "structures/kush/defense_tower"
...
const oPtolSiege = [
  "units/ptol/siege_lithobolos_unpacked",
  "units/ptol/siege_polybolos_unpacked",
];
...
const positionPyramids = new Vector2D(
  fractionToTiles(0.15),
  fractionToTiles(0.75)
);
...
const placeNapataWall =
  mapSize < 192 || getDifficulty() < 2
    ? false
    : getDifficulty() < 3
    ? "napata_palisade"
    : "napata_wall";
...
const layoutKushCity = [
  ...
  },
  {
    ...
    difficulty: "Medium",
    constraints: [
      avoidClasses(clFortress, 25),
      new NearTileClassConstraint(clPath, 8),
    ],
  ...
  (building) =>
    getDifficulty() >=
    getDifficulties().find(
      (difficulty) => difficulty.Name == building.difficulty
    ).Difficulty
);
...
createArea(
  ...
  new HeightmapPainter(
    translateHeightmap(
      ...
      undefined,
      convertHeightmap1Dto2D(
        Engine.LoadMapTerrain("maps/random/jebel_barkal.pmp").height
      )
    ),
    minHeightSource,
    maxHeightSource
...
paintRiver({
  parallel: true,
  start: new Vector2D(mapBounds.left, mapBounds.bottom).rotateAround(
    -riverAngle,
    mapCenter
  ),
  end: new Vector2D(mapBounds.right, mapBounds.bottom).rotateAround(
    -riverAngle,
    mapCenter
  ),
  ...
  landFunc: (position, shoreDist1, shoreDist2) => {
    for (let riv of layoutFertileLandTextures)
      if (
        (riv.left < +shoreDist1 && +shoreDist1 < riv.right) ||
...
createArea(
  new MapBoundsPlacer(),
  [new TileClassPainter(clWater), new TileClassUnPainter(clFertileLand)],
  new HeightConstraint(-Infinity, heightWaterLevel)
);

g_Map.log("Marking desert");
const avoidWater = new StaticConstraint(avoidClasses(clWater, 0));
createArea(new MapBoundsPlacer(), new TileClassPainter(clDesert), [
...
for (let area of irrigationCanalAreas) {
  ...
  irrigationCanalLocations.push(
    pickRandom(area.getPoints()).clone().rotateAround(riverAngle, mapCenter).x
  );
  createArea(
    new MapBoundsPlacer(),
    [
    ...
    [
      ...
      new HeightConstraint(heightIrrigationCanal, heightDesert),
    ]
  );
}
...
for (let i = 0; i < irrigationCanalLocations.length; ++i) {
  ...
  let next =
    i == irrigationCanalLocations.length - 1
      ? mapBounds.right
  ...
  for (let tries = 0; tries < 100; ++tries) {
    y =
      (previousPassageY +
        randIntInclusive(0.2 * widthFertileLand, 0.8 * widthFertileLand)) %
      widthFertileLand;

    let pos = new Vector2D((x1 + x2) / 2, y)
      .rotateAround(-riverAngle, mapCenter)
      .round();

    if (
      g_Map.validTilePassable(new Vector2D(pos.x, pos.y)) &&
  ...
  let area = createArea(
    ...
    [new ElevationPainter(heightPassage), new TileClassPainter(clPassage)],
    [
  ...
  if (!area || !area.getPoints().length) continue;
...
var gridCenter = new Vector2D(0, fractionToTiles(0.3))
  .rotate(-riverAngle)
  .add(mapCenter)
  .round();
...
for (let y in cityGridPosition)
  for (let x in cityGridPosition[y]) {
    ...
    createArea(new DiskPlacer(pathWidth, cityGridPosition[y][x]), [
...
for (let i = 0; i < layoutKushTemples.length; ++i) {
  ...
  templePosition[i] = Vector2D.add(
    cityGridPosition[0][x],
    layoutKushTemples[i].pathOffset.rotate(-Math.PI / 2 - cityGridAngle[0][x])
  );
  entitiesTemples[i] = g_Map.placeEntityPassable(
    layoutKushTemples[i].template,
    0,
    templePosition[i],
    cityGridAngle[0][x]
  );
...
createArea(
  new MapBoundsPlacer(),
  [new TerrainPainter(tHillCliff), new TileClassPainter(clCliff)],
  [stayClasses(clHill, 0), new SlopeConstraint(2, Infinity)]
);

g_Map.log("Painting temple ground");
createArea(new MapBoundsPlacer(), new TerrainPainter(tPathWild), [
...
var centralPathLength = centralPathStart.distanceTo(
...
for (let i = 0; i < 2; ++i)
  for (let stat = 0; stat < statueCount; ++stat) {
    let start = new Vector2D(0, ((pathWidthCenter * 3) / 4) * (i - 0.5))
      .rotate(centralPathAngle)
      .add(centralPathStart);
    let position = new Vector2D(centralPathLength, 0)
      .mult(stat / statueCount)
      .rotate(-centralPathAngle)
      .add(start)
      .add(new Vector2D(0.5, 0.5));

    if (!avoidClasses(clPathCrossing, 2).allows(position)) continue;

    g_Map.placeEntityPassable(
      pickRandom(aStatues),
      0,
      position,
      centralPathAngle - Math.PI * (i + 0.5)
    );
...
for (let i = 0; i < 2; ++i)
  for (let champ = 0; champ < centralChampionsCount; ++champ) {
    let start = new Vector2D(0, ((pathWidthCenter * 1) / 2) * (i - 0.5))
      .rotate(-centralPathAngle)
      .add(centralPathStart);
    let position = new Vector2D(centralPathLength, 0)
      .mult(champ / centralChampionsCount)
      .rotate(-centralPathAngle)
      .add(start)
      .add(new Vector2D(0.5, 0.5));
...
for (let x of [gridPointXCenter - 1, gridPointXCenter + 1]) {
  g_Map.placeEntityAnywhere(
    aStatueKush,
    0,
    cityGridPosition[gridPointYCenter][x],
...
var ritualAngle =
  (cityGridAngle[0][gridPointXCenter] +
    cityGridAngle[0][gridPointXCenter - 1]) /
    2 +
  Math.PI / 2;
...
createObjectGroupsByAreas(
  new SimpleGroup(
    [new SimpleObject(oTriggerPointCityPath, 1, 1, 0, 0)],
    true,
    clTriggerPointCityPath
  ),
  0,
  ...
  for (let x = 1; x < gridPointsX; ++x)
    createArea(
      ...
      [
        ...
        new CityPainter(
          layoutKushCity,
          (-cityGridAngle[y][x - 1] - cityGridAngle[y][x]) / 2,
          0
        ),
...
if (placeNapataWall) {
  ...
  let wallGridPositionFront = distributePointsOnCircularSegment(
    gridPointsX,
    wallGridMaxAngleBack,
    wallGridStartAngle,
    wallGridRadiusFront,
    gridCenter
  )[0];
  let wallGridPositionBack = distributePointsOnCircularSegment(
    gridPointsX,
    wallGridMaxAngleBack,
    wallGridStartAngle,
    wallGridRadiusBack,
  ...
  let wallGridPosition = [
    wallGridPositionFront[0],
    ...wallGridPositionBack,
    wallGridPositionFront[wallGridPositionFront.length - 1],
  ];
  ...
  entitiesGates = entitiesWalls.filter((entity) =>
    entity.templateName.endsWith(oWallGate)
  );
  createArea(
  ...
  createArea(
    ...
    [new NearTileClassConstraint(clWall, 1), avoidClasses(clCliff, 0)]
  );
...
var areaCityPaths = new Area(
  areasCityPaths.reduce((points, area) => points.concat(area.getPoints()), [])
);
var areaRoads = [];
for (let roadStart of roadStartLocations) {
  if (areaRoads.length >= scaleByMapSize(2, 5)) break;

  let closestPoint = areaCityPaths.getClosestPointTo(roadStart);
  roadConstraint = new StaticConstraint([
    roadConstraint,
    avoidClasses(clRoad, 20),
  ]);
  for (let tries = 0; tries < 30; ++tries) {
    let area = createArea(
      new PathPlacer(
        Vector2D.add(
          closestPoint,
          new Vector2D(0, (3 / 4) * mapSize).rotate(
            closestPoint.angleTo(roadStart)
          )
        ),
        roadStart,
        scaleByMapSize(5, 3),
...
var areaRoadPalms = createArea(new MapBoundsPlacer(), undefined, [
...
if (areaRoadPalms && areaRoadPalms.getPoints().length) {
  ...
  createObjectGroupsByAreas(
    new SimpleGroup(
      [new RandomObject(aBushesCity, 1, 1, 0, 0)],
      true,
      clForest
    ),
    0,
...
var areaCityBushes = createArea(new MapBoundsPlacer(), undefined, [
...
var areaCityPalms = createArea(new MapBoundsPlacer(), undefined, [
...
const areaFertileLand = createArea(
  new MapBoundsPlacer(),
  undefined,
  stayFertileLand
);
...
const mineObjects = (templateSmall, templateLarge) => ({
  ...
  small: [new SimpleObject(templateSmall, 3, 4, 1, 3, 0, 2 * Math.PI, 1)],
});
...
for (let i = 0; i < scaleByMapSize(6, 22); ++i) {
  ...
  for (let i in mineObjectsBiome.desert)
    createObjectGroupsByAreas(
      new SimpleGroup(
        mineObjectsBiome.desert[i],
        true,
        mineObjectsBiome.tileClass
      ),
      0,
      avoidCollisionsMines.concat([
        avoidClasses(clFertileLand, 12, mineObjectsBiome.tileClass, 15),
      ]),
      1,
      60,
      [areaDesert]
...
for (
  let i = 0;
  i < (isNomad() ? scaleByMapSize(6, 16) : scaleByMapSize(0, 8));
  ++i
) {
  let mineObjectsBiome = pickRandom(mineObjectsPerBiome);
  createObjectGroupsByAreas(
    new SimpleGroup(
      mineObjectsBiome.fertileLand.small,
      true,
      mineObjectsBiome.tileClass
    ),
    0,
...
createObjectGroups(
  new SimpleGroup(
    [new SimpleObject(oTriggerPointAttackerPatrol, 1, 1, 0, 0)],
    true,
    clTriggerPointMap
  ),
  0,
...
createObjectGroupsByAreas(
  new SimpleGroup(
    [new SimpleObject(oBerryBushGrapes, 4, 6, 1, 2)],
    true,
    clFood
  ),
  0,
...
createObjectGroups(
  new SimpleGroup(
    [
      new SimpleObject(oGiraffe, 2, 3, 2, 4),
      new SimpleObject(oGiraffeInfant, 2, 3, 2, 4),
    ],
    true,
    clFood
  ),
...
if (!isNomad()) {
  ...
  createObjectGroupsByAreas(
    new SimpleGroup(
      [
        new SimpleObject(oLion, 1, 2, 2, 4),
        new SimpleObject(oLioness, 2, 3, 2, 4),
      ],
      true,
      clFood
    ),
...
createObjectGroupsByAreas(
  new SimpleGroup(
    [
      new SimpleObject(oElephant, 2, 3, 2, 4),
      new SimpleObject(oElephantInfant, 2, 3, 2, 4),
    ],
    true,
    clFood
  ),
...
var areaIrrigationCanalTrees = createArea(new MapBoundsPlacer(), undefined, [
  nearWater,
...
createObjectGroupsByAreas(
  new SimpleGroup(
    [
      new RandomObject(
        [oKushCitizenArcher, oKushChampionArcher],
        scaleByMapSize(4, 10),
        scaleByMapSize(6, 20),
        1,
        4
...
createObjectGroupsByAreas(
  new SimpleGroup(
    [new RandomObject([oKushCitizenArcher, oKushChampionArcher], 1, 1, 1, 3)],
    true,
    clSoldier
  ),
  0,
...
const avoidCollisionsPyramids = new StaticConstraint([
  avoidCollisions,
...
if (!isNomad()) {
  ...
  createObjectGroupsByAreas(
    new SimpleGroup(
      [new SimpleObject(oKushCitizenArcher, 1, 1, 1, 1)],
      true,
      clSoldier
    ),
    0,
  ...
  createObjectGroupsByAreas(
    new SimpleGroup(
      [new RandomObject(oTreasuresHill, 1, 1, 2, 2)],
      true,
      clTreasure
    ),
    0,
...
createObjectGroupsByAreas(
  new SimpleGroup(
    [new RandomObject(oTreasuresCity, 1, 1, 0, 2)],
    true,
    clTreasure
  ),
  0,
...
createObjectGroupsByAreas(
  new SimpleGroup(
    [new SimpleObject(aHandcart, 1, 1, 1, 1)],
    true,
    clDecorative
  ),
  0,
...
createObjectGroupsByAreas(
  new SimpleGroup(
    [new SimpleObject(aPlotFence, 1, 1, 1, 1)],
    true,
    clDecorative
  ),
  0,
  new StaticConstraint([
    avoidCollisions,
    avoidClasses(clWater, 6, clDecorative, 10),
  ]),
  scaleByMapSize(1, 10),
  250,
  [areaFertileLand]
...
createDecoration(
  ...
  aBushesFertileLand.map(
    (bush) => scaleByMapSize(20, 120) * randIntInclusive(1, 3)
  ),
  [stayFertileLand, avoidCollisions]
);
...
placePlayersNomad(clPlayer, [
  avoidClasses(clHill, 15, clSoldier, 20, clCity, 15, clWall, 20),
  avoidCollisionsNomad,
]);

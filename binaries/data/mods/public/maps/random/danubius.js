...
const day =
  g_MapSettings.Daytime !== undefined
    ? g_MapSettings.Daytime == "day"
    : randBool(2 / 3);
...
var palisadeSideShort = [
  ...palisadeGate,
  ...palisadeWallShort,
  ...palisadeCorner,
  ...palisadeWallShort,
];
var palisadeSideLong = [
  ...palisadeGate,
  ...palisadeWallShort,
  ...palisadeCorner,
  ...palisadeWallLong,
...
if (gallicCC) {
  ...
  for (let i = 0; i < 2; ++i) {
    let civicCenterPosition = new Vector2D(
      i == 0
        ? mapBounds.left + gaulCityBorderDistance
    ...
    if (addCelticRitual) {
      ...
      let meetingPlacePosition = new Vector2D(
        ...
        mapCenter.y +
          fractionToTiles(randFloat(0.1, 0.4)) * (randBool() ? 1 : -1)
      ).rotateAround(startAngle, mapCenter);
      ...
      for (let participants of ritualParticipants) {
        let [positions, angles] = distributePointsOnCircle(
          participants.count,
          startAngle,
          participants.radius * mRadius,
          meetingPlacePosition
        );
        for (let j = 0; j < positions.length; ++j)
          g_Map.placeEntityPassable(
            pickRandom(participants.templates),
            0,
            positions[j],
            angles[j] + participants.angle
          );
      }
    }
    ...
    createArea(
      new ClumpPlacer(
        diskArea(gaulCityRadius),
        0.6,
        0.3,
        Infinity,
        civicCenterPosition
      ),
      [new TerrainPainter(tShore), new TileClassPainter(clGauls)]
    );

    // Place walls and buildings
    placeCustomFortress(
      civicCenterPosition,
      fortressDanubiusVillage,
      "danubius_village",
      0,
      startAngle + Math.PI
    );
    ...
    for (let j = 0; j < gallicCCTreasureCount; ++j)
      g_Map.placeEntityPassable(
        ...
        Vector2D.add(
          civicCenterPosition,
          new Vector2D(randFloat(-0.8, 0.8) * gaulCityRadius, 0).rotate(
            randomAngle()
          )
        ),
        randomAngle()
...
paintRiver({
  parallel: true,
  start: new Vector2D(mapCenter.x, mapBounds.top).rotateAround(
    startAngle,
    mapCenter
  ),
  end: new Vector2D(mapCenter.x, mapBounds.bottom).rotateAround(
    startAngle,
    mapCenter
  ),
  width: waterWidth,
  ...
  waterFunc: (position, height, riverFraction) => {
    ...
    if (
      0 < height &&
      height < 1 &&
      origPos.y > ShorelineDistance &&
      origPos.y < mapSize - ShorelineDistance
    )
      clShore[origPos.x < mapCenter.x ? 0 : 1].add(position);
  },
  landFunc: (position, shoreDist1, shoreDist2) => {
    if (shoreDist1 > 0) clLand[0].add(position);

    if (shoreDist2 < 0) clLand[1].add(position);
  },
});
...
var areasLand = [0, 1].map((i) =>
  createArea(new MapBoundsPlacer(), undefined, stayClasses(clLand[i], 0))
);
...
createBumps(
  avoidClasses(clPlayer, 6, clWater, 2, clPath, 1, clGauls, 1),
...
createObjectGroupsByAreas(
  ...
  0,
  [
    avoidClasses(clMetal, 4, clRock, 4, clTower, 5, clOutpost, 20),
    stayClasses(clIsland, 7),
  ],
  scaleByMapSize(3, 10),
  20,
  areaIslands
  ...
  createDecoration(
    ...
    i == 0
      ...
      : [
          ...
          avoidClasses(clForest, 1, clRock, 4, clMetal, 4),
        ]
  );
...
if (!isNomad())
  createFood(
    ...
    [scaleByMapSize(5, 20), scaleByMapSize(5, 20), scaleByMapSize(5, 20)],
    avoidClasses(
      clIsland,
      2,
      clFood,
...
createStragglerTrees(
  ...
  [
    stayClasses(clIsland, 4),
    avoidClasses(clMetal, 4, clRock, 4, clTower, 4, clOutpost, 4),
  ],
  clForest,
  stragglerTrees * 7
);
...
createObjectGroupsByAreas(
  new SimpleGroup(
    [new SimpleObject(triggerPointShipSpawn, 1, 1, 0, 0)],
    true,
    clShip
  ),
  0,
...
createObjectGroupsByAreas(
  new SimpleGroup(
    [new SimpleObject(triggerPointShipPatrol, 1, 1, 0, 0)],
    true,
    clShipPatrol
  ),
  0,
...
for (let i = 0; i < 2; ++i) {
  let areaShore = [
    createArea(new MapBoundsPlacer(), undefined, stayClasses(clShore[i], 0)),
  ];
...
g_Map.placeEntityAnywhere(
  ...
  0,
  Vector2D.add(mapCenter, new Vector2D(0, 1).rotate(startAngle)),
  randomAngle()
);
...
createObjectGroupsByAreas(
  new SimpleGroup(
    [new SimpleObject(aWaterLog, 1, 1, 0, 0, startAngle, startAngle)],
    true,
    clWaterLog
  ),
  0,
...
if (day) {
  ...
  setWaterColor(0.074, 0.101, 0.09);
  setWaterTint(0.129, 0.16, 0.137);
}

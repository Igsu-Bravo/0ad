...
placePlayerBases({
  PlayerPlacement: [
    ...
    playerPlacementLine(
      0,
      new Vector2D(mapCenter.x, playerPosition),
      fractionToTiles(0.2)
...
for (let i = 0; i < scaleByMapSize(20, 120); ++i) {
  let position = new Vector2D(
    fractionToTiles(randFloat(0.28, 0.34)),
    fractionToTiles(randFloat(0.1, 0.9))
  )
    .rotateAround(startAngle - Math.PI / 2, mapCenter)
    .round();
  createArea(
    new ChainPlacer(
      1,
...
var types = [
  ...
  [
    [tGrass, tGrass, tGrass, tGrass, pForestP],
    [tGrass, tGrass, tGrass, pForestP],
  ],
];
...
group = new SimpleGroup([new SimpleObject(oBush, 5, 7, 0, 4)], true, clFood);
createObjectGroupsDeprecated(
  group,
  0,

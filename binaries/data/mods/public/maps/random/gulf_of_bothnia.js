...
else setBiome("gulf_of_bothnia/winter");
  ...
  PlayerPlacement: [
    ...
    ...playerPlacementCustomAngle(
      ...
      (i) =>
        ...
        (1 / 3) *
          Math.PI *
          (1 + 2 * (numPlayers == 1 ? 1 : (2 * i) / (numPlayers - 1)))
    ),
  ],
...
for (let gulfLake of gulfLakePositions) {
  let position = Vector2D.add(
    mapCenter,
    new Vector2D(gulfLake.x, 0).rotate(-startAngle)
  ).round();
...
if (currentBiome() === "gulf_of_bothnia/frozen_lake") {
  createAreas(
    new ChainPlacer(1, 4, scaleByMapSize(16, 40), 0.3),

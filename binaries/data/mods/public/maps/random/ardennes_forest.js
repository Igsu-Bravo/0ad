...
var pForest = [
  tPineForestFloor + TERRAIN_SEPARATOR + oOak,
  tForestFloor,
  tPineForestFloor + TERRAIN_SEPARATOR + oPine,
  tForestFloor,
  tPineForestFloor + TERRAIN_SEPARATOR + oAleppoPine,
  tForestFloor,
  tForestFloor,
];
...
var explorableArea = createArea(new MapBoundsPlacer(), undefined, [
...
var [forestTreesJoin, forestTrees] = getTreeCounts(
  forestTrees,
  forestTrees,
  0.25
);
...
createAreasInAreas(
  ...
  [new TerrainPainter(pForest), new TileClassPainter(clForest)],
...
createBalancedMetalMines(oMetalSmall, oMetalLarge, clMetal, [
  stayClasses(clExplorable, 1),
  avoidClasses(clForest, 0, clPlayer, scaleByMapSize(15, 25), clHill, 1),
]);

g_Map.log("Creating stone mines");
createBalancedStoneMines(oStoneSmall, oStoneLarge, clRock, [
  stayClasses(clExplorable, 1),
  avoidClasses(
    clForest,
    0,
...
group = new SimpleGroup([new SimpleObject(oBoar, 2, 3, 0, 5)], true, clFood);
createObjectGroupsByAreasDeprecated(
  group,
  0,
...
group = new SimpleGroup([new SimpleObject(oBear, 1, 1, 0, 4)], false, clFood);
createObjectGroupsByAreasDeprecated(
  group,
  0,

...
Trigger.prototype.LoadAttackerTemplates = function () {
  for (let civ of ["gaia", ...Object.keys(loadCivFiles(false))])
    attackerUnitTemplates[civ] = {
      heroes: TriggerHelper.GetTemplateNamesByClasses(
        "Hero",
        civ,
        undefined,
        true
...
Trigger.prototype.InitStartingUnits = function () {
  for (
    ...
    this.playerCivicCenter[playerID] = TriggerHelper.GetPlayerEntitiesByClass(
      playerID,
      "CivilCentre"
    )[0];
    this.treasureFemale[playerID] = TriggerHelper.GetPlayerEntitiesByClass(
      playerID,
      "FemaleCitizen"
    )[0];
    Engine.QueryInterface(
      this.treasureFemale[playerID],
      IID_Resistance
...
Trigger.prototype.StartAnEnemyWave = function () {
  ...
  let totalAttackers = Math.ceil(
    Math.min(
      totalAttackerLimit,
      (initialAttackers *
        Math.pow(percentPerMinute, currentMin) *
        nextWaveTime) /
        maxWaveTime
    )
  );
  ...
  this.debugLog(
    "Spawning " +
      totalAttackers +
      " attackers, siege ratio " +
      siegeRatio.toFixed(2)
  );
    ...
    [
      {
        ...
        count:
          currentMin > heroTime() && attackerUnitTemplates[civ].heroes.length
            ? 1
            : 0,
      },
      {
  ...
  for (let point of this.GetTriggerPoints("A")) {
    ...
    for (let templateName in attackerCount) {
      let isHero =
        attackerUnitTemplates[civ].heroes.indexOf(templateName) != -1;
      ...
      if (this.gaiaHeroes[playerID] && isHero) {
        let cmpHealth = Engine.QueryInterface(
          this.gaiaHeroes[playerID],
          IID_Health
        );
        if (cmpHealth && cmpHealth.GetHitpoints() != 0) {
          this.debugLog(
            "Not spawning hero for player " +
              playerID +
      ...
      let entities = TriggerHelper.SpawnUnits(
        point,
        templateName,
        attackerCount[templateName],
        0
      );
      ...
      if (isHero) this.gaiaHeroes[playerID] = entities[0];
    }
    spawned = true;
  }
...

{
  ...
  cmpTrigger.RegisterTrigger("OnOwnershipChanged", "OnOwnershipChanged", {
    enabled: true,
  });
}

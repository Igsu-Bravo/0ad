...
function placeStartLocationResources(
  ...
  for (let i = 0; i < quantity; ++i) {
    ...
    let woodPosition = Vector2D.add(
      point,
      new Vector2D(randFloat(10, 15), 0).rotate(-angle)
    );
    g_Map.placeEntityPassable(
      pickRandom(objectList),
      0,
    ...
    createArea(new ClumpPlacer(5, 1, 1, Infinity, woodPosition), [
  ...
  for (let i = 0; i < quantity; ++i) {
    angle = currentAngle + randFloat(0, dAngle);
    let berriesPosition = Vector2D.add(
      point,
      new Vector2D(randFloat(10, 15), 0).rotate(-angle)
    );
    g_Map.placeEntityPassable(
...
let heighLimits = [
  ...
  heightSeaGroundAdjusted +
    (1 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 3 Shore
  heightSeaGroundAdjusted +
    (2 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 4 Low ground
  heightSeaGroundAdjusted +
    (3 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 5 Player and path height
  heightSeaGroundAdjusted +
    (4 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 6 High ground
  heightSeaGroundAdjusted +
    (5 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 7 Lower forest border
  heightSeaGroundAdjusted +
    (6 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 8 Forest
  heightSeaGroundAdjusted +
    (7 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 9 Upper forest border
...
myBiome.push({
  ...
  actor: [
    ["gaia/fish/generic", "actor|geology/stone_granite_boulder.xml"],
    0.02,
  ],
  textureHS: ["alpine_mountainside"],
  actorHS: [["gaia/fish/generic"], 0.1],
});
myBiome.push({
  // 1 Medium Water
...
myBiome.push({
  ...
  textureHS: ["alpine_mountainside"],
  actorHS: [["actor|props/flora/grass_soft_tuft_a.xml"], 0.1],
});
myBiome.push({
  // 4 Low ground
...
myBiome.push({
  ...
  textureHS: ["alpine_cliff_c"],
  actorHS: [
    ["actor|props/flora/grass_tufts_a.xml", "actor|geology/highland2_moss.xml"],
    0.1,
  ],
});
myBiome.push({
...
myBiome.push({
  ...
  textureHS: ["alpine_cliff_b"],
  actorHS: [
    ["actor|geology/stone_granite_med.xml", "actor|geology/snow1.xml"],
    0.1,
  ],
});
myBiome.push({
  ...
  textureHS: ["alpine_cliff_c"],
  actorHS: [["actor|geology/highland1.xml"], 0.0],
});

let [playerIDs, playerPosition] = groupPlayersCycle(
  getStartLocationsByHeightmap(
    { min: heighLimits[4], max: heighLimits[5] },
    1000,
    30
  )
);
...
for (let i = 0; i < playerPosition.length; ++i)
  createArea(
    new RandomPathPlacer(
      playerPosition[i],
      playerPosition[(i + 1) % playerPosition.length],
      4,
      2,
      false
    ),
...
let resourceSpots = getPointsByHeight(
  {
...
for (let x = 0; x < tchm.length; ++x)
  for (let y = 0; y < tchm[0].length; ++y) {
    ...
    if (!avoidClasses(clPath, 0).allows(position)) continue;
...
for (let i = 0; i < resourceSpots.length; ++i) {
  ...
  if (choice == 0) placeMine(pos, "gaia/rock/temperate_large_02");
  if (choice == 1) placeMine(pos, "gaia/ore/temperate_large");
  if (choice == 2)
    placeCustomFortress(pos, pickRandom(fences), "other", 0, randomAngle());
  if (choice == 3) placeGrove(pos);
  if (choice == 4) placeCamp(pos);
}

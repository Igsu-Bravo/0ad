...
function placeGrove(
  ...
  g_Map.placeEntityPassable(
    pickRandom(["structures/gaul/outpost", "gaia/tree/oak_new"]),
    0,
    position,
    randomAngle()
  );
  ...
  for (let i = 0; i < quantity; ++i) {
    ...
    if (i % 3 == 0) objectList = groveActors;
    ...
    if (groveTileClass) painters.push(new TileClassPainter(groveTileClass));

    createArea(new ClumpPlacer(5, 1, 1, Infinity, pos), painters);
  }
}

var farmEntities = {
  "generic/temperate": {
    building: "structures/mace/farmstead",
    animal: "gaia/fauna_pig",
  },
  "generic/arctic": {
    building: "structures/brit/farmstead",
    animal: "gaia/fauna_sheep",
  },
  "generic/sahara": {
    building: "structures/pers/farmstead",
    animal: "gaia/fauna_camel",
  },
  "generic/alpine": {
    building: "structures/rome/farmstead",
    animal: "gaia/fauna_sheep",
  },
  "generic/aegean": {
    building: "structures/iber/farmstead",
    animal: "gaia/fauna_pig",
  },
  "generic/savanna": {
    building: "structures/sele/farmstead",
    animal: "gaia/fauna_horse",
  },
  "generic/india": {
    building: "structures/ptol/farmstead",
    animal: "gaia/fauna_camel",
  },
  "generic/autumn": {
    building: "structures/gaul/farmstead",
    animal: "gaia/fauna_horse",
  },
};
...
g_WallStyles.other = {
  ...
  animal: {
    ...
    indent: 0.75,
    bend: 0,
    templateName: farmEntities[currentBiome()].animal,
  },
  farmstead: {
    angle: Math.PI,
    length: 0,
    indent: -3,
    bend: 0,
    templateName: farmEntities[currentBiome()].building,
  },
};
...
function placeCamp(
  ...
  for (let i = 0; i < quantity; ++i) {
    ...
    g_Map.placeEntityPassable(
      pickRandom(otherEntities),
      0,
      Vector2D.add(position, new Vector2D(dist, 0).rotate(-angle)),
      randomAngle()
    );
  }
...
function placeStartLocationResources(
  ...
  placeMine(
    Vector2D.add(point, new Vector2D(averageDistToCC, 0).rotate(-angle)),
    g_Gaia.stoneLarge
  );
  ...
  for (let i = 0; i < quantity; ++i) {
    ...
    let objectList = groveEntities;
    if (i % 2 == 0) objectList = groveActors;

    let position = Vector2D.add(point, new Vector2D(dist, 0).rotate(-angle));
    g_Map.placeEntityPassable(
      pickRandom(objectList),
      0,
      position,
      randomAngle()
    );

    createArea(new ClumpPlacer(5, 1, 1, Infinity, position), [
  ...
  for (let i = 0; i < quantity; ++i) {
    ...
    let dist = getRandDist();
    g_Map.placeEntityPassable(
      pickRandom(foodEntities),
      0,
      Vector2D.add(point, new Vector2D(dist, 0).rotate(-angle)),
      randomAngle()
    );
    currentAngle += dAngle;
  }
...
let heightRange = {
  min: MIN_HEIGHT * heightScale,
  max: MAX_HEIGHT * heightScale,
};
...
let heightSeaGround =
  -MIN_HEIGHT +
  heightRange.min +
  averageWaterCoverage * (heightRange.max - heightRange.min); // Water height in environment and the engine
...
for (let i = 0; i < 5; ++i) splashErodeMap(0.1);
...
let heighLimits = [
  ...
  heightSeaGroundAdjusted +
    (2 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 2 Shore
  heightSeaGroundAdjusted +
    (3 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 3 Low ground
  heightSeaGroundAdjusted +
    (4 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 4 Player and path height
  heightSeaGroundAdjusted +
    (6 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 5 High ground
  heightSeaGroundAdjusted +
    (7 / 8) * (heightRange.max - heightSeaGroundAdjusted), // 6 Lower forest border
  heightRange.max, // 7 Forest
];
let playerHeightRange = { min: heighLimits[3], max: heighLimits[4] };
let resourceSpotHeightRange = {
...
let [playerIDs, playerPosition] = groupPlayersCycle(
  getStartLocationsByHeightmap(playerHeightRange, 1000, 30)
);
...
for (let i = 0; i < avoidPoints.length; ++i) avoidPoints[i].dist = 30;
let resourceSpots = getPointsByHeight(resourceSpotHeightRange, avoidPoints).map(
  (point) => new Vector2D(point.x, point.y)
);
...
for (let i = 0; i < resourceSpots.length; ++i) {
  ...
  if (choice == 0) placeMine(resourceSpots[i], g_Gaia.stoneLarge);
  if (choice == 1) placeMine(resourceSpots[i], g_Gaia.metalLarge);
  if (choice == 2) placeGrove(resourceSpots[i]);
  if (choice == 3) {
  ...
  if (choice == 4) {
    if (mercenaryCamps) {
      placeStartingEntities(
        resourceSpots[i],
        0,
        mercenaryCampGuards[currentBiome()]
      );
      radius = 15;
      --mercenaryCamps;
    } else {
  ...
  if (radius)
    createArea(
      ...
      new SmoothElevationPainter(
        ELEVATION_SET,
        g_Map.getHeight(resourceSpots[i]),
        radius / 3
      )
    );
}

...
var riverStart = new Vector2D(mapCenter.x, 0).rotateAround(
  riverAngle,
  mapCenter
);
var riverEnd = new Vector2D(mapCenter.x, mapSize).rotateAround(
  riverAngle,
  mapCenter
);
...
const middlePoint = riverStart
  .clone()
  .mult(fraction)
  .add(riverEnd.clone().mult(1 - fraction));
let passageStart = new Vector2D(
  middlePoint.x,
  middlePoint.y - riverWidth * 1.1
).rotateAround(riverAngle, middlePoint);
let passageEnd = new Vector2D(
  middlePoint.x,
  middlePoint.y + riverWidth * 1.1
).rotateAround(riverAngle, middlePoint);
...
createArea(
  ...
  new OrConstraint([stayClasses(clWater, 0), borderClasses(clWater, 0, 3)])
);

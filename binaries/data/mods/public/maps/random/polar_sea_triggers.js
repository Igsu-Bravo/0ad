...
Trigger.prototype.SpawnWolvesAndAttack = function () {
  let waveSize = Math.round(
    Math.random() * (maxWaveSize - minWaveSize) + minWaveSize
  );
  let attackers = TriggerHelper.SpawnUnitsFromTriggerPoints(
    "A",
    attackerTemplate,
    waveSize,
    0
  ...
  if (debugLog)
    print(
      "Spawned " +
        ...
        attackerTemplate +
        " at " +
        Object.keys(attackers).length +
        " points\n"
    );

  let allTargets;

  let players = new Array(TriggerHelper.GetNumberOfPlayers())
    .fill(0)
    .map((v, i) => i + 1);
  ...
  for (let spawnPoint in attackers) {
    ...
    let targets = PositionHelper.EntitiesNearPoint(
      attackerPos,
      200,
      players,
      IID_Health
      ...
      return (
        cmpIdentity &&
        MatchesClassList(cmpIdentity.GetClassesList(), targetClasses)
      );
    });
  ...
  this.DoAfterDelay(
    (Math.random() * (maxWaveTime - minWaveTime) + minWaveTime) * 60 * 1000,
    "SpawnWolvesAndAttack",
    {}
  );
};

{
  ...
  cmpTrigger.RegisterTrigger("OnInitGame", "DisableTechnologies", {
    enabled: true,
  });
  cmpTrigger.DoAfterDelay(
    firstWaveTime * 60 * 1000,
    "SpawnWolvesAndAttack",
    {}
  );
}

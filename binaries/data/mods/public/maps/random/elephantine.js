...
const oTowers = new Array(2)
  .fill("uncapturable|structures/kush/sentry_tower")
  .concat(["uncapturable|structures/kush/defense_tower"]);

const oHeroes = Engine.FindTemplates("units/kush/", true).filter(
  (templateName) => templateName.startsWith("units/kush/hero_")
);
const oUnits = Engine.FindTemplates("units/kush/", false).filter(
  (templateName) =>
    ...
    Engine.GetTemplate(templateName)
      .Identity.VisibleClasses._string.split(" ")
      .some((type) => ["Soldier", "Healer", "Female"].indexOf(type) != -1)
);
...
const aBushesIslands = aBushesShoreline.concat(
  new Array(3).fill(actorTemplate("props/flora/foliagebush"))
);
...
var groupTemple = createObjectGroupsByAreas(
  new SimpleGroup(
    [
      new RandomObject(
        g_Map.getSize() >= 320 ? [oWonder] : oTemples,
        1,
        1,
        0,
        1,
...
var groupPyramid = createObjectGroupsByAreas(
  new SimpleGroup(
    [new SimpleObject(oPyramid, 1, 1, 0, 1, riverAngle, riverAngle)],
    true,
    clTemple
  ),
  0,
  [
    stayClasses(clIsland, scaleByMapSize(10, 20)),
    avoidClasses(clTemple, scaleByMapSize(20, 50)),
    avoidCollisions,
  ],
  1,
  200,
...
var cityCenters = [
  groupTemple[0] &&
    groupTemple[0][0] && {
      pos: Vector2D.from3D(groupTemple[0][0].position),
      radius: 10,
    },
  groupPyramid[0] &&
    groupPyramid[0][0] && {
      pos: Vector2D.from3D(groupPyramid[0][0].position),
      radius: 6,
    },
].filter((pos) => !!pos);
...
createBumps(
  avoidClasses(clPlayer, 10, clWater, 2, clTemple, 10, clPath, 1),
  scaleByMapSize(10, 500),
  1,
  8,
  4,
  0.2,
...
createArea(new MapBoundsPlacer(), new TileClassPainter(clCliff), [
...
createForests(
  ...
  [
    avoidCollisions,
    avoidClasses(clIsland, 0, clPlayer, 20, clForest, 18, clWater, 2),
  ],
  clForest,
  forestTrees / 2
);
...
createForests(
  ...
  [
    stayClasses(clIsland, 0),
    avoidClasses(clForest, 15, clWater, 2),
    avoidCollisions,
  ],
  clForest,
  forestTrees / 2
...
createObjectGroups(
  ...
  0,
  [new NearTileClassConstraint(clWater, 3), avoidCollisions],
  scaleByMapSize(2, 10),
  50
);
...
createStragglerTrees([oAcacia], avoidCollisions, clForest, stragglerTrees);
...
createDecoration(
  ...
  aBushesIslands.map(
    (bush) => scaleByMapSize(20, 150) * randIntInclusive(1, 3)
  ),

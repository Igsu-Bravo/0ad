...
addElements([
  ...
  {
    func: addDecoration,
    avoid: [g_TileClasses.baseResource, 5, g_TileClasses.forest, 2],
    stay: [g_TileClasses.player, 1],
    sizes: ["normal"],
    mixes: ["normal"],
    amounts: ["normal"],
...
placePlayersNomad(g_TileClasses.player, [
...
function createSunkenTerrain() {
  ...
  if (currentBiome() == "generic/autumn") middle = g_Terrains.shore;
  ...
  createArea(new DiskPlacer(fractionToTiles(0.29), mapCenter), [
  ...
  createArea(new DiskPlacer(fractionToTiles(0.21), mapCenter), [
  ...
  let getCoords = (distance, playerID, playerIDOffset) => {
    let angle =
      startAngle + ((playerID + playerIDOffset) * 2 * Math.PI) / numPlayers;
    return Vector2D.add(
      mapCenter,
      new Vector2D(fractionToTiles(distance), 0).rotate(-angle)
    ).round();
  };
  ...
  for (let i = 0; i < numPlayers; ++i) {
    ...
    createArea(
      new ClumpPlacer(
        diskArea(fractionToTiles(0.1)) / (isNomad() ? 2 : 1),
        0.9,
        0.3,
        Infinity,
        playerPosition
  ...
  for (let i = 0; i < numPlayers; ++i) {
    ...
    createArea(new ClumpPlacer(expSize, 0.9, 0.3, Infinity, position), [
      new LayeredPainter([g_Terrains.cliff, lower], [3]),
      new SmoothElevationPainter(ELEVATION_SET, heightValley, 3),
      new TileClassPainter(g_TileClasses.settlement),
    ]);

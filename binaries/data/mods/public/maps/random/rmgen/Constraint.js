...
function AndConstraint(constraints) {
  if (constraints instanceof Array) this.constraints = constraints;
  else if (!constraints) this.constraints = [];
...
AdjacentToAreaConstraint.prototype.allows = function (position) {
  return this.areas.some(
    (area) =>
      ...
      g_Map
        .getAdjacentPoints(position)
        .some((adjacentPosition) => area.contains(adjacentPosition))
  );
};
...
BorderTileClassConstraint.prototype.allows = function (position) {
  return (
    this.tileClass.countMembersInRadius(position, this.distanceOutside) > 0 &&
...
HeightConstraint.prototype.allows = function (position) {
  return (
    this.minHeight <= g_Map.getHeight(position) &&
    g_Map.getHeight(position) <= this.maxHeight
  );
};
...
SlopeConstraint.prototype.allows = function (position) {
  return (
    this.minSlope <= g_Map.getSlope(position) &&
    g_Map.getSlope(position) <= this.maxSlope
  );
};
...
StaticConstraint.prototype.allows = function (position) {
  if (!this.cache[position.x][position.y])
    this.cache[position.x][position.y] = this.constraint.allows(position)
      ? 2
      : 1;

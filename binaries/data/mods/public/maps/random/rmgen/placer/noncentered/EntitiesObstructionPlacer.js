 ...
 */
function EntitiesObstructionPlacer(
  entities,
  margin = 0,
...
EntitiesObstructionPlacer.prototype.place = function (constraint) {
  ...
  for (let entity of this.entities) {
    let halfObstructionSize = getObstructionSize(
      entity.templateName,
      this.margin
    ).div(2);
    ...
    let obstructionCorners = [
      ...
      Vector2D.add(entity.GetPosition2D(), corner.rotate(-entity.rotation.y))
    );

    points = points.concat(
      new ConvexPolygonPlacer(obstructionCorners, this.failFraction).place(
        constraint
      )
    );
  }

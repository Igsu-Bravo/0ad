...
DiskPlacer.prototype.place = function (constraint) {
  ...
  const xMax = Math.ceil(
    Math.min(g_Map.getSize() - 1, this.centerPosition.x + this.radius)
  );
  const yMax = Math.ceil(
    Math.min(g_Map.getSize() - 1, this.centerPosition.y + this.radius)
  );
  ...
  for (it.x = xMin; it.x <= xMax; ++it.x)
    for (it.y = yMin; it.y <= yMax; ++it.y) {
      if (
        this.centerPosition.distanceToSquared(it) <= this.radiusSquared &&

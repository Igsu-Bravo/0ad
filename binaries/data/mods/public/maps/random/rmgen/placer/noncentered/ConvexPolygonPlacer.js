...
function ConvexPolygonPlacer(points, failFraction = 0) {
  this.polygonVertices = this.getConvexHull(
    points.map((point) => point.clone().round())
  );
...
ConvexPolygonPlacer.prototype.place = function (constraint) {
  ...
  for (let point of getPointsInBoundingBox(
    getBoundingBox(this.polygonVertices)
  )) {
    if (
      this.polygonVertices.some(
        (vertex, i) =>
          distanceOfPointFromLine(
            this.polygonVertices[i],
            this.polygonVertices[(i + 1) % this.polygonVertices.length],
            point
          ) > 0
      )
    )
...
ConvexPolygonPlacer.prototype.getConvexHull = function (points) {
  ...
  while (result.length < uniquePoints.length) {
    ...
    for (let point of uniquePoints) {
      if (point == result[result.length - 1]) continue;
    ...
    if (result.indexOf(nextLeftmostPoint) != -1) break;

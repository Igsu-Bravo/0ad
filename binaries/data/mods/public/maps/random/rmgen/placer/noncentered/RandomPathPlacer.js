...
function RandomPathPlacer(pathStart, pathEnd, pathWidth, offset, blended) {
  this.pathStart = Vector2D.add(
    pathStart,
    Vector2D.sub(pathEnd, pathStart).normalize().mult(offset)
  ).round();
...
RandomPathPlacer.prototype.place = function (constraint) {
  ...
  while (
    position.distanceToSquared(this.pathEnd) >= this.offsetSquared &&
    pathLength++ < this.maxPathLength
  ) {
    position
      .add(
        new Vector2D(1, 0).rotate(
          -getAngle(
            ...
            (-Math.PI / 2) * (randFloat(-1, 1) + (this.blended ? 0.5 : 0))
        )
      )
      .round();

...
PathPlacer.prototype.place = function (constraint) {
  ...
  let pathPerpendicular = Vector2D.sub(this.end, this.start)
    .normalize()
    .perpendicular();
  let segments1 = [];
  let segments2 = [];
  ...
  for (let j = 0; j < totalSteps; ++j) {
    ...
    let stepStart = Vector2D.add(
      Vector2D.mult(this.start, 1 - step1),
      Vector2D.mult(this.end, step1)
    );
    let stepEnd = Vector2D.add(
      Vector2D.mult(this.start, 1 - step2),
      Vector2D.mult(this.end, step2)
    );

    // Find noise offset points
    let noiseStart = Vector2D.add(
      stepStart,
      Vector2D.mult(pathPerpendicular, noise[j])
    );
    let noiseEnd = Vector2D.add(
      stepEnd,
      Vector2D.mult(pathPerpendicular, noise[j + 1])
    );
    ...
    segments1.push(
      Vector2D.sub(
        noiseStart,
        Vector2D.mult(noisePerpendicular, taperedWidth)
      ).round()
    );
    segments2.push(
      Vector2D.add(
  ...
  for (let j = 0; j < segments1.length - 1; ++j) {
    let points = new ConvexPolygonPlacer(
      [segments1[j], segments1[j + 1], segments2[j], segments2[j + 1]],
      Infinity
    ...
    for (let point of points) {
      if (!constraint.allows(point)) {
        if (!this.failFraction) return undefined;
  ...
  return failed > this.failFraction * this.width * pathLength
    ? undefined
    : retVec;
};

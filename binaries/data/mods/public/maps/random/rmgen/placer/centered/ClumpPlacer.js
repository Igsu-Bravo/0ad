...
ClumpPlacer.prototype.place = function (constraint) {
  ...
  for (let i = 0; i < intPerim; ++i) {
    ...
    noise[i] = cubicInterpolation(
      1,
      (i - ctrlCoords[c]) /
        ((looped ? perim : ctrlCoords[(c + 1) % ctrlPts]) - ctrlCoords[c]),
  ...
  for (let stepAngle = 0; stepAngle < intPerim; ++stepAngle) {
    ...
    let radiusUnitVector = new Vector2D(0, 1).rotate(
      (-2 * Math.PI * stepAngle) / perim
    );
    let maxRadiusSteps = Math.ceil(
      radius * (1 + (1 - this.coherence) * noise[stepAngle])
    );

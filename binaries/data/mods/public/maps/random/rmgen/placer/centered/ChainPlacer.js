...
ChainPlacer.prototype.place = function (constraint) {
  ...
  if (
    !g_Map.inMapBounds(this.centerPosition) ||
    !constraint.allows(this.centerPosition)
  )
    return undefined;
  ...
  for (let i = 0; i < this.numCircles; ++i) {
    ...
    let radius = this.queue.length
      ? this.queue.pop()
      : randIntInclusive(this.minRadius, this.maxRadius);
    let radius2 = Math.square(radius);

    let bbox = getPointsInBoundingBox(
      getBoundingBox([
        new Vector2D(
          Math.max(0, chainPos.x - radius),
          Math.max(0, chainPos.y - radius)
        ),
        new Vector2D(
          Math.min(chainPos.x + radius, size),
          Math.min(chainPos.y + radius, size)
        ),
      ])
    );

    for (let position of bbox) {
      if (position.distanceToSquared(chainPos) >= radius2) continue;
    ...
    for (let pos of bbox) {
      ...
      if (gotRet[pos.x][pos.y] != -2) continue;

      if (
        (pos.x > 0 && gotRet[pos.x - 1][pos.y] == -1) ||

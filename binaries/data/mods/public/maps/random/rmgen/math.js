...
function distributePointsOnCircle(pointCount, startAngle, radius, center) {
  return distributePointsOnCircularSegment(
    pointCount,
    (2 * Math.PI * (pointCount - 1)) / pointCount,
    startAngle,
    radius,
    center
  );
 ...
 */
function distributePointsOnCircularSegment(
  pointCount,
  maxAngle,
  startAngle,
  radius,
...
function distanceOfPointFromLine(lineStart, lineEnd, point) {
  ...
  return Vector2D.sub(lineStart, lineEnd)
    .normalize()
    .cross(Vector2D.sub(point, lineEnd));
}
...
function testLineIntersection(start1, end1, start2, end2, width) {
  ...
  return (
    ...
    (start1end1.cross(start1start2) *
      start1end1.cross(Vector2D.sub(start1, end2)) <=
      0 &&
      start2end2.cross(start1start2) *
        start2end2.cross(Vector2D.sub(start2, end1)) >=
        0)
  );
}
...
function sortPointsShortestCycle(points) {
  ...
  for (let i = 0; i < 3; ++i) {
    order.push(i);
    pointsToAdd.shift();
    if (i) distances.push(points[order[i]].distanceTo(points[order[i - 1]]));
  }
  ...
  for (let i = 0; i < numPointsToAdd; ++i) {
    ...
    for (let k = 0; k < order.length; ++k) {
      ...
      let dist2 = pointsToAdd[0].distanceTo(

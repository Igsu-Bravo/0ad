...
function RandomMap(baseHeight, baseTerrain) {
  ...
  for (let i = 0; i < this.size; ++i) {
    ...
    for (let j = 0; j < this.size; ++j) this.terrainEntities[i][j] = undefined;
  }

  // Create 2D array for heightmap
  let mapSize = this.size;
  if (!TILE_CENTERED_HEIGHT_MAP) ++mapSize;

  this.height = [];
  for (let i = 0; i < mapSize; ++i) {
    this.height[i] = new Float32Array(mapSize);

    for (let j = 0; j < mapSize; ++j) this.height[i][j] = baseHeight;
  }
...
RandomMap.prototype.LoadMapTerrain = function (filename) {
  ...
  let heightmapPainter = new HeightmapPainter(
    convertHeightmap1Dto2D(mapTerrain.height)
  );

  createArea(new MapBoundsPlacer(), [
    heightmapPainter,
    new TerrainTextureArrayPainter(
      mapTerrain.textureIDs,
      mapTerrain.textureNames
    ),
  ]);
 ...
 */
RandomMap.prototype.LoadHeightmapImage = function (
  filename,
  normalMinHeight,
  normalMaxHeight
) {
  ...
  let heightmapPainter = new HeightmapPainter(
    convertHeightmap1Dto2D(
      Engine.LoadHeightmapImage("maps/random/" + filename)
    ),
    normalMinHeight,
    normalMaxHeight
  );
...
RandomMap.prototype.getTextureID = function (texture) {
  if (texture in this.nameToID) return this.nameToID[texture];
...
RandomMap.prototype.validTile = function (position, distance = 0) {
  if (this.isCircularMap())
    return (
      Math.round(position.distanceTo(this.getCenter())) <
      this.size / 2 - distance - 1
    );

  return (
    position.x >= distance &&
    position.y >= distance &&
    position.x < this.size - distance &&
    position.y < this.size - distance
  );
};
...
RandomMap.prototype.randomCoordinate = function (passableOnly) {
  ...
  if (this.isCircularMap())
    ...
    return Vector2D.add(
      this.getCenter(),
      new Vector2D((this.size / 2 - border) * Math.sqrt(randFloat(0, 1)), 0)
        .rotate(randomAngle())
        .floor()
    );
 ...
 */
RandomMap.prototype.placeEntityAnywhere = function (
  templateName,
  playerID,
  position,
  orientation
 ...
 */
RandomMap.prototype.placeEntityPassable = function (
  templateName,
  playerID,
  position,
  orientation
) {
  if (!this.validTilePassable(position)) return undefined;

  return this.placeEntityAnywhere(
    templateName,
    playerID,
    position,
    orientation
...
RandomMap.prototype.getTerrainEntity = function (position) {
  if (!this.validTilePassable(position))
    throw new Error(
      "getTerrainEntity: invalid tile position " + uneval(position)
    );
 ...
 */
RandomMap.prototype.setTerrainEntity = function (
  templateName,
  playerID,
  position,
  orientation
  ...
  if (!this.validTilePassable(tilePosition))
    throw new Error(
      "setTerrainEntity: invalid tile position " + uneval(position)
    );

  this.terrainEntities[tilePosition.x][tilePosition.y] = new Entity(
    this.getEntityID(),
    templateName,
    playerID,
    position,
...
RandomMap.prototype.getExactHeight = function (position) {
  ...
  return (
    (1 - zf) * ((1 - xf) * h00 + xf * h10) + zf * ((1 - xf) * h01 + xf * h11)
  );
};
...
RandomMap.prototype.getAdjacentPoints = function (position) {
  ...
  for (let adjacentCoordinate of g_AdjacentCoordinates) {
    ...
    if (this.inMapBounds(adjacentPos)) adjacentPositions.push(adjacentPos);
  }
...
RandomMap.prototype.getAverageHeight = function (position) {
  ...
  if (!adjacentPositions.length) return 0;

  return (
    adjacentPositions.reduce(
      (totalHeight, pos) => totalHeight + this.getHeight(pos),
      0
...
RandomMap.prototype.getSlope = function (position) {
  ...
  return (
    adjacentPositions.reduce(
      (totalSlope, adjacentPos) =>
        totalSlope +
        Math.abs(this.getHeight(adjacentPos) - this.getHeight(position)),
      0
    ) / adjacentPositions.length
  );
};
...
RandomMap.prototype.exportHeightData = function () {
  ...
  for (let x = 0; x < heightmapSize; ++x)
    for (let z = 0; z < heightmapSize; ++z) {
      ...
      let currentHeight = TILE_CENTERED_HEIGHT_MAP
        ? this.cornerHeight(position)
        : this.getHeight(position);

      // Correct height by SEA_LEVEL and prevent under/overflow in terrain data
      heightmap[z * heightmapSize + x] = Math.max(
        0,
        Math.min(
          0xffff,
          Math.floor((currentHeight + SEA_LEVEL) * HEIGHT_UNITS_PER_METRE)
        )
      );
    }

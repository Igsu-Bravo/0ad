 ...
 */
function HeightmapPainter(
  heightmap,
  normalMinHeight = undefined,
  normalMaxHeight = undefined
) {
...
HeightmapPainter.prototype.paint = function (area) {
  ...
  let seen = new Array(g_Map.height.length)
    .fill(0)
    .map((zero) => new Uint8Array(g_Map.height.length).fill(0));
      ...
      g_Map.setHeight(
        ...
        bicubicInterpolation(
          ...
          ...getPointsInBoundingBox(
            getBoundingBox([
              brushPosition,
              Vector2D.add(brushPosition, brushSize),
            ])
          ).map((pos) => this.scaleHeight(this.heightmap[pos.x][pos.y]))
        )
      );
    }
};

...
function SmoothingPainter(size, strength, iterations) {
  if (size < 1) throw new Error("Invalid size: " + size);

  if (strength <= 0 || strength > 1)
    throw new Error("Invalid strength: " + strength);

  if (iterations <= 0) throw new Error("Invalid iterations: " + iterations);
...
SmoothingPainter.prototype.paint = function (area) {
  let brushPoints = getPointsInBoundingBox(
    getBoundingBox(
      new Array(2)
        .fill(0)
        .map((zero, i) => new Vector2D(1, 1).mult(this.size).mult(i ? 1 : -1))
    )
  );
  ...
  for (let i = 0; i < this.iterations; ++i) {
    ...
    let seen = new Array(heightmap.length)
      .fill(0)
      .map((zero) => new Uint8Array(heightmap.length).fill(0));

    for (let point of area.getPoints())
      for (let tileVertex of g_TileVertices) {
        let vertex = Vector2D.add(point, tileVertex);
        if (!g_Map.validHeight(vertex) || seen[vertex.x][vertex.y]) continue;
        ...
        for (let brushPoint of brushPoints) {
          ...
          if (!distance || !g_Map.validHeight(position)) continue;

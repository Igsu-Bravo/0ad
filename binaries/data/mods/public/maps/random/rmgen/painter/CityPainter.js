...
function CityPainter(templates, angle, playerID) {
  ...
  this.templates = templates.map((template) => {
    let obstructionSize = getObstructionSize(
    ...
    return {
      ...
      constraint:
        template.constraints && new AndConstraint(template.constraints),
  ...
  for (let x = 0; x < mapSize; x += 0.5)
    for (let y = 0; y < mapSize; y += 0.5) {
      let point = new Vector2D(x, y)
        .rotateAround(this.angle, mapCenter)
        .round();
      if (
        !area.contains(point) ||
        processed[point.x][point.y] ||
        !g_Map.validTilePassable(point)
      )
        continue;
      ...
      for (let template of shuffleArray(templates)) {
        if (template.constraint && !template.constraint.allows(point)) continue;
        ...
        let obstructionCorners = template.obstructionCorners.map(
          (obstructionCorner) =>
            Vector2D.add(point, obstructionCorner.clone().rotate(buildingAngle))
        );

        let obstructionPoints = new ConvexPolygonPlacer(
          obstructionCorners,
          0
        ).place(
          new AndConstraint([
            new StayAreasConstraint([area]),
            avoidClasses(tileClass, 0),
            new PassableMapAreaConstraint(),
          ])
        );

        if (!obstructionPoints) continue;

        g_Map.placeEntityPassable(
          template.templateName,
          this.playerID,
          Vector2D.average(obstructionCorners),
          -buildingAngle
        );
        ...
        templates = templates.filter(
          (template) =>
            templateCounts[template.templateName] < template.maxCount
        );
        break;
      }
    }

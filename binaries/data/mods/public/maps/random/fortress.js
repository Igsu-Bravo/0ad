  ...
  createArea(
    new ConvexPolygonPlacer(
      new Array(4)
        .fill(0)
        .map((zero, j) =>
          new Vector2D(tilesSize, 0)
            .rotate((j * Math.PI) / 2 - playerAngle - Math.PI / 4)
            .add(playerPosition[i])
        ),
      Infinity
    ),
    [new TerrainPainter(tRoad), new TileClassPainter(clPlayer)]
  );

  // Fortress
  // To take into account houses that offer a lower population bonus
  if (
    Engine.GetTemplate("structures/" + civ + "/house").Population.Bonus <= 5
  ) {
    var wall = [
      "gate",
      "tower",
...
createObjectGroupsByAreasDeprecated(
  new SimpleGroup(
    [
      ...
      new SimpleObject(aLillies, 0, 1, 0, 4),
    ],
    true
  ),
  0,
...
var types = [
  [
    [tForestFloor, tGrass, pForestD],
    [tForestFloor, pForestD],
  ],
  [
    [tForestFloor, tGrass, pForestO],
    [tForestFloor, pForestO],
  ],
  [
    [tForestFloor, tGrass, pForestP],
    [tForestFloor, pForestP],
  ],
];

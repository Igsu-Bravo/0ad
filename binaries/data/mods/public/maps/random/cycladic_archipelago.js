...
var islandPosition = distributePointsOnCircle(
  islandCount,
  startAngle,
  fractionToTiles(0.39),
  mapCenter
)[0].map((position) => position.round());

var centralIslandRadius = scaleByMapSize(15, 30);
var centralIslandCount = Math.floor(scaleByMapSize(1, 4));
var centralIslandPosition = new Array(numPlayers)
  .fill(0)
  .map((v, i) =>
    Vector2D.add(
      mapCenter,
      new Vector2D(fractionToTiles(randFloat(0.1, 0.16)), 0)
        .rotate(
          -startAngle -
            Math.PI * ((i * 2) / centralIslandCount + randFloat(-1, 1) / 8)
        )
        .round()
    )
  );
...
function createCycladicArchipelagoIsland(
  position,
  tileClass,
  radius,
  coralRadius
...
for (let i = 0; i < islandCount; ++i) {
  let isPlayerIsland =
    numPlayers >= 6 || i == startingPlaces[numPlayers - 1][nPlayer];
  if (isPlayerIsland) {
...
placePlayerBases({
  ...
  CityPatch: {
    ...
    outerTerrain: tGrass,
    innerTerrain: tCity,
    painters: [new TileClassPainter(clCity)],
  },
  Chicken: {},
  Berries: {
    template: oBerryBush,
...
var forestTypes = [
  [
    [tForestFloor, tGrass, pPalmForest],
    [tForestFloor, pPalmForest],
  ],
  [
    [tForestFloor, tGrass, pPineForest],
    [tForestFloor, pPineForest],
  ],
  [
    [tForestFloor, tGrass, pPoplarForest],
    [tForestFloor, pPoplarForest],
  ],
  [
    [tForestFloor, tGrass, pMainForest],
    [tForestFloor, pMainForest],
  ],
];
...
for (let size of [
  ...
  createAreasInAreas(
    new ClumpPlacer(size, 0.3, 0.06, 0.5),
    [new LayeredPainter([tGrassDry], []), new TileClassPainter(clDirt)],
    avoidClasses(
      clWater,
      3,
      clHill,

...
for (var i = 0; i < numPlayers; i++) {
  ...
  playerPosition[i] = Vector2D.add(
    mapCenter,
    new Vector2D(randFloat(minPlayerRadius, maxPlayerRadius), 0)
      .rotate(-playerAngle[i])
      .round()
  );
}
...
placePlayerBases({
  ...
  CityPatch: {
    ...
    painters: [
      new LayeredPainter(
        [terrainBaseBorder, terrainBase, terrainBaseCenter],
  ...
  Mines: {
    types: [{ template: templateMetalMine }, { template: templateStoneMine }],
    minAngle: Math.PI / 2,
    maxAngle: Math.PI,
  },
  Trees: {
...
for (let i = 0; i < numPlayers; ++i)
  for (let rIndex = 0; rIndex < resourcePerPlayer.length; ++rIndex) {
    let angleDist =
      numPlayers > 1
        ? (playerAngle[(i + 1) % numPlayers] - playerAngle[i] + 2 * Math.PI) %
          (2 * Math.PI)
        : 2 * Math.PI;
    ...
    let angle =
      playerAngle[i] +
      (angleDist * (rIndex + 1)) / (resourcePerPlayer.length + 1);
    let position = Vector2D.add(
      mapCenter,
      new Vector2D(resourceRadius, 0).rotate(-angle)
    ).round();

    g_Map.placeEntityPassable(
      resourcePerPlayer[rIndex],
      0,
      position,
      randomAngle()
    );

    createArea(new ClumpPlacer(40, 1 / 2, 1 / 8, Infinity, position), [
...
for (var x = 0; x < mapSize; x++)
  for (var z = 0; z < mapSize; z++) {
    ...
    var radius = mapCenter.distanceTo(
      Vector2D.add(position, new Vector2D(0.5, 0.5))
    );
    ...
    // General height map
    let hVarMiddleHill =
      fractionToTiles(1 / 64) *
      (1 + Math.cos(((3 / 2) * Math.PI * radius) / mapRadius));
    var hVarHills = 5 * (1 + Math.sin(x / 10) * Math.sin(z / 10));
    g_Map.setHeight(
      position,
      g_Map.getHeight(position) + hVarMiddleHill + hVarHills + 1
    );
  }
...
placePlayersNomad(
  clPlayer,
  avoidClasses(clForest, 1, clBaseResource, 4, clHill, 4)
);

...
else setBiome("fields_of_meroe/dry");
...
paintRiver({
  parallel: false,
  start: new Vector2D(fractionToTiles(0.25), mapBounds.top).rotateAround(
    riverAngle,
    mapCenter
  ),
  end: new Vector2D(fractionToTiles(0.25), mapBounds.bottom).rotateAround(
    riverAngle,
    mapCenter
  ),
  ...
  landFunc: (position, shoreDist1, shoreDist2) => {
    for (let riv of riverTextures)
      if (
        ...
        if (riv.terrain) createTerrain(riv.terrain).place(position);
      }
  },
});
...
for (let x of [
  fractionToTiles(randFloat(0.15, 0.25)),
  fractionToTiles(randFloat(0.75, 0.85)),
]) {
...
var [playerIDs, playerPosition] = playerPlacementRandom(
  sortAllPlayers(),
  avoidClasses(clRiver, 15, clPlayer, 30)
);
placePlayerBases({
...
for (let retryCount = 0; retryCount < scaleByMapSize(3, 10); ++retryCount) {
  ...
  if (
    ...
    createArea(new ClumpPlacer(40, 0.6, 0.3, Infinity, coordinate), [
...
for (let coordinate of kushiteTownPositions) {
  for (let building in kushVillageBuildings)
    g_Map.placeEntityPassable(
      kushVillageBuildings[building].template,
      0,
      Vector2D.add(coordinate, kushVillageBuildings[building].offset),
      Math.PI
    );

  createObjectGroup(
    new SimpleGroup(
      [new SimpleObject(oKushUnits, 5, 7, 1, 2)],
      true,
      clKushiteVillages,
      coordinate
    ),
...
createObjectGroups(
  new SimpleGroup(
    [new SimpleObject(oPyramidLarge, 1, 1, 0, 1)],
    true,
    clKushiteVillages
  ),
  0,
...
createAreas(
  ...
  new StaticConstraint(
    avoidClasses(clPlayer, 5, clKushiteVillages, 10, clRiver, 20)
  ),
...
createObjectGroups(
  new SimpleGroup(
    [
      new SimpleObject(aRockB, 1, 2, 0, 1),
      new SimpleObject(aRockC, 1, 3, 0, 1),
    ],
    true
  ),
  0,
...
createObjectGroups(
  new SimpleGroup(
    [
      new SimpleObject(aBushB, 1, 2, 0, 1),
      new SimpleObject(aBushA, 1, 3, 0, 2),
    ],
    true
  ),
  0,

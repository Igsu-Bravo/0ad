...
if (!isNomad())
  for (let i = 0; i < numPlayers; ++i) {
    let marketPos = Vector2D.add(
      playerPosition[i],
      new Vector2D(12, 0).rotate(randomAngle())
    ).round();
...
createDecoration(
  ...
  [scaleByMapSize(16, 262), scaleByMapSize(8, 131)],
...
createFood(
  ...
  [3 * numPlayers, 5 * numPlayers, 5 * numPlayers, 12 * numPlayers],
  avoidClasses(
    clPlayer,
    35,
    clFood,
...
createFood(
  ...
  [scaleByMapSize(1, 6) * 3, scaleByMapSize(1, 6) * 3],
...
createObjectGroupsDeprecated(
  new SimpleGroup(
    [new SimpleObject("trigger/trigger_point_A", 1, 1, 0, 0)],
    true,
    clArcticWolf
  ),
  0,

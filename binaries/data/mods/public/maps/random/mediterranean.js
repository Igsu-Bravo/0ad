...
for (let zone of climateZones) {
  ...
  createArea(
    ...
    [new HeightConstraint(heightWaterLevel, Infinity), zone.constraint]
  );
}
...
if (!isNomad()) {
  ...
  let [playerIDs, playerPosition] = playerPlacementRandom(sortAllPlayers(), [
  ...
  for (let i = 0; i < getNumPlayers(); ++i) {
    ...
    setBiome(
      climateZones.find((zone) => zone.tileClass.has(playerPosition[i])).biome
    );
...
createArea(new MapBoundsPlacer(), new TerrainPainter(tSnowedRocks), [
  new HeightConstraint(heightSnow, Infinity),
  avoidClasses(
    g_TileClasses.africa,
    0,
    g_TileClasses.southern_europe,
    0,
    g_TileClasses.player,
    6
  ),
...
addElements([
  {
    ...
    avoid: [
      g_TileClasses.fish,
      2,
      g_TileClasses.desert,
      50,
      g_TileClasses.steppe,
      50,
    ],

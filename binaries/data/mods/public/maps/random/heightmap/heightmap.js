...
function rescaleHeightmap(
  ...
  for (let x = 0; x < heightmap.length; ++x)
    for (let y = 0; y < heightmap[x].length; ++y)
      heightmap[x][y] =
        minHeight +
        ((heightmap[x][y] - oldHeightRange.min) /
          (oldHeightRange.max - oldHeightRange.min)) *
          (maxHeight - minHeight);
  return heightmap;
}
 ...
 * @return {Vector2D[]}
 */
function getStartLocationsByHeightmap(
  heightRange,
  maxTries = 1000,
  minDistToBorder = 20,
  numberOfPlayers = g_MapSettings.PlayerData.length - 1,
  heightmap = g_Map.height,
  isCircular = g_MapSettings.CircularMap
) {
  let validStartLoc = [];
  ...
  for (let x = minDistToBorder; x < mapSize - minDistToBorder; ++x)
    for (let y = minDistToBorder; y < mapSize - minDistToBorder; ++y) {
      ...
      if (
        heightConstraint.allows(position) &&
        (!isCircular || position.distanceTo(mapCenter)) <
          mapSize / 2 - minDistToBorder
      )
 ...
 */
function setBaseTerrainDiamondSquare(
  minHeight = MIN_HEIGHT,
  maxHeight = MAX_HEIGHT,
  initialHeightmap = undefined,
  smoothness = 0.5,
  heightmap = g_Map.height
) {
  ...
  initialHeightmap = initialHeightmap || [
    [
      randFloat(minHeight / 2, maxHeight / 2),
      randFloat(minHeight / 2, maxHeight / 2),
    ],
    [
      randFloat(minHeight / 2, maxHeight / 2),
  ...
  if (heightRange <= 0) warn("setBaseTerrainDiamondSquare: heightRange <= 0");
    ...
    for (let x = 0; x < 2 * oldWidth - 1; ++x) {
      ...
      for (let y = 0; y < 2 * oldWidth - 1; ++y) {
        if (x % 2 == 0 && y % 2 == 0)
          ...
          // New tile with diagonal old tile neighbors
          newHeightmap[x].push(
            (initialHeightmap[(x - 1) / 2][(y - 1) / 2] +
              initialHeightmap[(x + 1) / 2][(y - 1) / 2] +
              initialHeightmap[(x - 1) / 2][(y + 1) / 2] +
              initialHeightmap[(x + 1) / 2][(y + 1) / 2]) /
              4
          ...
          newHeightmap[x][y] +=
            ((newHeightmap[x][y] - minHeight) / heightRange) *
            randFloat(-offset, offset);
        } // New tile with straight old tile neighbors
        else newHeightmap[x].push(undefined); // Define later
      }
    }
    // Diamond
    for (let x = 0; x < 2 * oldWidth - 1; ++x) {
      for (let y = 0; y < 2 * oldWidth - 1; ++y) {
        if (newHeightmap[x][y] !== undefined) continue;

        if (
          x > 0 &&
          x + 1 < newHeightmap.length - 1 &&
          y > 0 &&
          y + 1 < newHeightmap.length - 1
        ) {
          // Not a border tile
          newHeightmap[x][y] =
            (newHeightmap[x + 1][y] +
          ...
          x < newHeightmap.length - 1 &&
          y > 0 &&
          y < newHeightmap.length - 1
        ) {
          // Left border
          newHeightmap[x][y] =
            (newHeightmap[x + 1][y] +
          ...
          newHeightmap[x][y] +=
            ((newHeightmap[x][y] - minHeight) / heightRange) *
            randFloat(-offset, offset);
        } else if (x > 0 && y > 0 && y < newHeightmap.length - 1) {
          // Right border
          newHeightmap[x][y] =
            (newHeightmap[x][y + 1] +
              newHeightmap[x - 1][y] +
          ...
          newHeightmap[x][y] +=
            ((newHeightmap[x][y] - minHeight) / heightRange) *
            randFloat(-offset, offset);
        } else if (x > 0 && x < newHeightmap.length - 1 && y > 0) {
          // Top border
          newHeightmap[x][y] =
            (newHeightmap[x + 1][y] +
              newHeightmap[x - 1][y] +
  ...
  let shift = [
    Math.floor((newHeightmap.length - heightmap.length) / 2),
    Math.floor((newHeightmap[0].length - heightmap[0].length) / 2),
  ];
  for (let x = 0; x < heightmap.length; ++x)
 ...
 * @param {boolean} [isCircular=g_MapSettings.CircularMap] - If the map is circular or rectangular
 */
function getPointsByHeight(
  heightRange,
  avoidPoints = [],
  avoidClass = undefined,
  minDistance = 20,
  maxTries = 2 * g_Map.size,
  heightmap = g_Map.height,
  isCircular = g_MapSettings.CircularMap
) {
  const points = [];
  ...
  for (let x = minDistance; x < heightmap.length - minDistance; ++x)
    for (let y = minDistance; y < heightmap[x].length - minDistance; ++y) {
      ...
      if (
        heightmap[x][y] > heightRange.min &&
        heightmap[x][y] < heightRange.max && // Has correct height
        (!isCircular || r - Math.euclidDistance2D(x, y, r, r) >= minDistance)
      )
        // Enough distance to the map border
  ...
  for (let tries = 0; tries < maxTries; ++tries) {
    ...
    if (
      placements.every(
        (p) =>
          Math.euclidDistance2D(p.x, p.y, point.x, point.y) >
...
function getSlopeMap(inclineMap = getInclineMap(g_Map.height)) {
  ...
  for (let x = 0; x < max_x; ++x) {
    ...
    for (let y = 0; y < max_y; ++y)
      slopeMap[x][y] = Math.euclidDistance2D(
        0,
        0,
        inclineMap[x][y].x,
        inclineMap[x][y].y
  ...
  for (let x = 0; x < max_x; ++x) {
    ...
    for (let y = 0; y < max_y; ++y) {
      ...
      for (let i = 0; i < slopes.length; ++i)
        if (slopes[i] > 0) sumSlopes += slopes[i];
      ...
      let sumDrain = 0;
      for (let i = 0; i < drain.length; ++i) sumDrain += drain[i];

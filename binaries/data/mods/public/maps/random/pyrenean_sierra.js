  ...
  for (var iz = 0; iz < mapSize; iz++) {
    ...
    if (g_Map.inMapBounds(position)) {
      let height =
        ...
        randFloat(-1, 1) +
        scaleByMapSize(1, 3) *
          (Math.cos(ix / scaleByMapSize(5, 30)) +
            Math.sin(iz / scaleByMapSize(5, 30)));
      g_Map.setHeight(position, height);
      baseHeights[ix].push(height);
    } else baseHeights[ix].push(heightInit);
  }
}
...
function sigmoid(x, peakPosition) {
  return (
    ...
    (0.2 - Math.max(0, Math.abs(0.5 - peakPosition) - 0.3)) *
    5
  );
}
    ...
    for (let distance = 0; distance < mountainWidth; distance += 1 / 3) {
      ...
      for (let direction of [-1, 1]) {
        let pos = Vector2D.sum([
          ...
          new Vector2D(distance, 0).rotate(
...
for (let passLoc of [passageLocation, 1 - passageLocation])
  for (let direction of [1, -1]) {
    let passageStart = Vector2D.add(
      mountainStart,
      Vector2D.mult(mountainVec, passLoc)
    );
    let passageEnd = Vector2D.add(
      passageStart,
      Vector2D.mult(passageVec, direction)
    );
...
for (let ocean of distributePointsOnCircle(
  2,
  oceanAngle,
  fractionToTiles(0.48),
  mapCenter
)[0])
  createArea(
    new ClumpPlacer(
    ...
    [new ElevationPainter(heightOcean), new TileClassPainter(clWater)]
  );
...
var types = [
  [tForestTransition, pForestLandVeryLight, pForestLandLight, pForestLand],
];
...
var group = new SimpleGroup(
  [new SimpleObject(oPine, 1, 2, 1, 3), new SimpleObject(oBeech, 1, 2, 1, 3)],
  true,
  clForest
);
createObjectGroupsDeprecated(
  group,
  0,
  avoidClasses(clWater, 3, clForest, 1, clPlayer, 8, clPyrenneans, 1),
  num,
  20
);
...
for (let x = 0; x < mapSize; ++x)
  for (let z = 0; z < mapSize; ++z) {
    ...
    if (terrainShore) createTerrain(terrainShore).place(position);
  }

function getShoreTerrain(position, height, heightDiff) {
  if (height <= heightWaterTerrain) return tWater;

  if (
    height <= heightSand &&
    new NearTileClassConstraint(clWater, 2).allows(position)
  )
    return heightDiff < 2.5 ? tSand : tMidRangeCliffs;

  // Notice the sand transition is also be painted below height -2
  if (
    height <= heightSandTransition &&
    new NearTileClassConstraint(clWater, 3).allows(position)
  )
...
for (let size of [
  ...
  createAreas(
    ...
    [new TerrainPainter(tDirtyGrass), new TileClassPainter(clDirt)],
    avoidClasses(
      clWater,
      3,
      clForest,
...
createObjectGroupsDeprecated(
  group,
  0,
  avoidClasses(clWater, 2, clPlayer, 1, clPyrenneans, 1),
  scaleByMapSize(13, 200),
  50
);
...
createObjectGroupsDeprecated(
  group,
  0,
  [avoidClasses(clFood, 15), stayClasses(clWater, 6)],
  20 * numPlayers,
  60
);

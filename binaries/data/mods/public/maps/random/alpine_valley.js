...
MountainRangeBuilder.prototype.InitPossibleEdges = function () {
  for (let i = 0; i < this.vertices.length; ++i)
    for (let j = numPlayers; j < this.vertices.length; ++j)
      if (j > i) this.possibleEdges.push([i, j]);
};
...
MountainRangeBuilder.prototype.SetConnectable = function (isConnectable) {
  this.verticesConnectable[this.currentEdge[0]][this.currentEdge[1]] =
    isConnectable;
  this.verticesConnectable[this.currentEdge[1]][this.currentEdge[0]] =
    isConnectable;
};
...
MountainRangeBuilder.prototype.CreateMountainRanges = function () {
  g_Map.log(
    "Creating mountainrange with " +
      this.possibleEdges.length +
      " possible edges"
  );
  ...
  while (this.possibleEdges.length) {
    ...
    if (
      this.vertexDegree[this.currentEdge[0]] < this.maxDegree &&
...
new MountainRangeBuilder({
  ...
  points: [
    ...
    ...distributePointsOnCircle(
      ...
      fractionToTiles(0.49),
      mapCenter
    )[0],
    ...distributePointsOnCircle(
      numPlayers,
      startAngle + (Math.PI / numPlayers) * 1.4,
      fractionToTiles(0.34),
      mapCenter
    )[0],
    ...distributePointsOnCircle(
      numPlayers,
      startAngle + (Math.PI / numPlayers) * 0.6,
      fractionToTiles(0.34),
      mapCenter
    )[0],
    ...distributePointsOnCircle(
...
var types = [
  [
    [tForestFloor, tPrimary, pForest],
    [tForestFloor, pForest],
  ],
];

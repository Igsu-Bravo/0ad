...
placePlayerBases({
  PlayerPlacement: [
    ...
    playerPlacementLine(
      0,
      new Vector2D(mapCenter.x, fractionToTiles(0.45)),
      fractionToTiles(0.2)
...
for (let i = 0; i < scaleByMapSize(20, 120); ++i) {
  let position = new Vector2D(
    ...
    fractionToTiles(randFloat(0.67, 0.74))
  )
...
var types = [
  [
    [tSnowA, tSnowA, tSnowA, tSnowA, pForestD],
    [tSnowA, tSnowA, tSnowA, pForestD],
  ],
  [
    [tSnowA, tSnowA, tSnowA, tSnowA, pForestS],
    [tSnowA, tSnowA, tSnowA, pForestS],
  ],
];
...
for (let size of [
  ...
  createAreas(
    new ChainPlacer(1, Math.floor(scaleByMapSize(3, 5)), size, 0.5),
    [new TerrainPainter(tSnowE), new TileClassPainter(clDirt)],
    avoidClasses(clWater, 8, clForest, 0, clHill, 0, clPlayer, 20, clDirt, 16),
    scaleByMapSize(20, 80)
  );
...
group = new SimpleGroup([new SimpleObject(oWalrus, 5, 7, 0, 4)], true, clFood);
createObjectGroupsDeprecated(
  group,
  0,

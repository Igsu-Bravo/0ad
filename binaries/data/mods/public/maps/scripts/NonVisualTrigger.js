...
Trigger.prototype.EndGameAction = function () {
  if (
    !this.once ||
    Engine.QueryInterface(SYSTEM_ENTITY, IID_PlayerManager).GetActivePlayers()
      .length
  )
    return;
  ...
  for (let player of Engine.GetEntitiesWithInterface(IID_StatisticsTracker)) {
    let cmpStatisticsTracker = Engine.QueryInterface(
      player,
...

{
  ...
  cmpTrigger.RegisterTrigger("OnPlayerDefeated", "EndGameAction", {
    enabled: true,
  });

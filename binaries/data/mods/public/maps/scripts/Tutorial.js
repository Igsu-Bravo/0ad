Trigger.prototype.InitTutorial = function (data) {
  ...
  this.RegisterTrigger("OnDeserialized", "OnDeserializedTrigger", {
    enabled: true,
  });
  this.RegisterTrigger("OnPlayerCommand", "OnPlayerCommandTrigger", {
    enabled: false,
  });
  ...
  for (let goal of this.tutorialGoals) {
    for (let key in goal) {
      if (
        typeof goal[key] != "function" ||
        this.tutorialEvents.indexOf(key) != -1
      )
        continue;
      if (key == "Init") continue;
      if (key == "IsDone") continue;
...
Trigger.prototype.NextGoal = function (deserializing = false) {
  ...
  for (let event of this.tutorialEvents) {
    ...
    if (goal[event]) {
      ...
      if (!goalAlreadyDone) needDelay = false;
    } else this.DisableTrigger(event, action);
  }
  ...
  if (goal.delay || needDelay) {
    if (goal.delay && goal.delay > 0)
      ...
      Trigger.prototype.OnPlayerCommandTrigger = function (msg) {
        if (
          msg.cmd.type == "dialog-answer" &&
          msg.cmd.tutorial &&
          msg.cmd.tutorial == "ready"
        )
          this.NextGoal();
      };
      readyButton = true;
  ...
  this.GoalMessage(
    goal.instructions,
    readyButton,
    ++this.index == this.tutorialGoals.length
  );
};

Trigger.prototype.GoalMessage = function (
  instructions,
  readyButton = false,
  leave = false
) {

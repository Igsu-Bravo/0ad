Trigger.prototype.WonderVictoryEntityRenamed = function (data) {
  if (
    this.wonderVictoryMessages[data.entity] &&
    Engine.QueryInterface(data.newentity, IID_Wonder)
  ) {
    ...
    this.wonderVictoryMessages[data.newentity] =
      this.wonderVictoryMessages[data.entity];
...
Trigger.prototype.WonderVictoryOwnershipChanged = function (data) {
  if (!Engine.QueryInterface(data.entity, IID_Wonder)) return;

  this.WonderVictoryDeleteTimer(data.entity);

  if (data.to > 0) this.WonderVictoryStartTimer(data.entity, data.to);
};

Trigger.prototype.WonderVictoryDiplomacyChanged = function (data) {
  if (
    !Engine.QueryInterface(SYSTEM_ENTITY, IID_EndGameManager).GetAlliedVictory()
  )
    return;

  for (let ent in this.wonderVictoryMessages) {
    if (
      this.wonderVictoryMessages[ent].playerID != data.player &&
      this.wonderVictoryMessages[ent].playerID != data.otherPlayer
    )
      continue;

    let owner = this.wonderVictoryMessages[ent].playerID;
    let otherPlayer = owner == data.player ? data.otherPlayer : data.player;
    let newAllies = new Set(
      QueryPlayerIDInterface(owner).GetPlayersByDiplomacy(
        "IsExclusiveMutualAlly"
      )
    );
    if (
      (newAllies.has(otherPlayer) &&
        !this.wonderVictoryMessages[ent].allies.has(otherPlayer)) ||
      (!newAllies.has(otherPlayer) &&
        this.wonderVictoryMessages[ent].allies.has(otherPlayer))
    ) {
...
Trigger.prototype.WonderVictoryStartTimer = function (ent, player) {
  ...
  let allies = cmpEndGameManager.GetAlliedVictory()
    ? QueryPlayerIDInterface(player).GetPlayersByDiplomacy(
        "IsExclusiveMutualAlly"
      )
    : [];
      ...
      cmpGuiInterface.AddTimeNotification(
        {
          message: allies.length
            ? markForTranslation(
                "%(_player_)s owns a Wonder and %(_player_)s and their allies will win in %(time)s."
              )
            : markForTranslation(
                "%(_player_)s owns a Wonder and will win in %(time)s."
              ),
          players: others,
          parameters: {
            _player_: player,
      ...
      cmpGuiInterface.AddTimeNotification(
        {
          message: markForTranslation(
            "%(_player_)s owns a Wonder and you will win in %(time)s."
          ),
          players: allies,
          parameters: {
            _player_: player,
      ...
      cmpGuiInterface.AddTimeNotification(
        {
          message: allies.length
            ? markForTranslation(
                "You own a Wonder and you and your allies will win in %(time)s."
              )
...
Trigger.prototype.WonderVictoryDeleteTimer = function (ent) {
  if (!this.wonderVictoryMessages[ent]) return;
...

{
  ...
  cmpTrigger.RegisterTrigger("OnEntityRenamed", "WonderVictoryEntityRenamed", {
    enabled: true,
  });
  cmpTrigger.RegisterTrigger(
    "OnOwnershipChanged",
    "WonderVictoryOwnershipChanged",
    { enabled: true }
  );
  cmpTrigger.RegisterTrigger(
    "OnDiplomacyChanged",
    "WonderVictoryDiplomacyChanged",
    { enabled: true }
  );
  cmpTrigger.RegisterTrigger("OnPlayerWon", "WonderVictoryPlayerWon", {
    enabled: true,
  });
  cmpTrigger.RegisterTrigger(
    "OnPlayerDefeated",
    "WonderVictoryPlayerDefeated",
    { enabled: true }
  );

Trigger.prototype.InitCaptureTheRelic = function () {
  ...
  let catafalqueTemplates = shuffleArray(
    cmpTemplateManager
      .FindAllTemplates(false)
      .filter(
        (name) =>
          GetIdentityClasses(
            cmpTemplateManager.GetTemplate(name).Identity || {}
          ).indexOf("Relic") != -1
      )
  );
  ...
  if (!potentialSpawnPoints.length) {
    error(
      "No gaia entities found on this map that could be used as spawn points!"
    );
    return;
  }
  ...
  this.playerRelicsCount = new Array(TriggerHelper.GetNumberOfPlayers()).fill(
    0,
    1
  );
  ...
  for (let i = 0; i < numSpawnedRelics; ++i) {
    this.relics[i] = TriggerHelper.SpawnUnits(
      pickRandom(potentialSpawnPoints),
      catafalqueTemplates[i],
      1,
      0
    )[0];
...
Trigger.prototype.CheckCaptureTheRelicVictory = function (data) {
  ...
  if (
    !cmpIdentity ||
    !cmpIdentity.HasClass("Relic") ||
    data.from == INVALID_PLAYER
  )
    return;
  ...
  } else ++this.playerRelicsCount[data.to];
...
Trigger.prototype.CheckCaptureTheRelicCountdown = function () {
  ...
  let relicOwners = activePlayers.filter(
    (playerID) => this.playerRelicsCount[playerID]
  );
  ...
  let winningPlayers = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_EndGameManager
  ).GetAlliedVictory()
    ? activePlayers.filter((playerID) =>
        relicOwners.every((owner) =>
          QueryPlayerIDInterface(playerID).IsMutualAlly(owner)
        )
      )
    : [relicOwners[0]];
  ...
  if (
    winningPlayers.length != this.relicsVictoryCountdownPlayers.length ||
    winningPlayers.some(
      (player) => this.relicsVictoryCountdownPlayers.indexOf(player) == -1
    )
  ) {
...
Trigger.prototype.DeleteCaptureTheRelicVictoryMessages = function () {
  if (!this.relicsVictoryTimer) return;

  Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer).CancelTimer(
    this.relicsVictoryTimer
  );
...
Trigger.prototype.StartCaptureTheRelicCountdown = function (winningPlayers) {
  ...
  for (
    let playerID = 1;
    playerID < TriggerHelper.GetNumberOfPlayers();
    ++playerID
  ) {
    let cmpPlayer = QueryPlayerIDInterface(playerID);
    if (cmpPlayer.GetState() == "won") return;

    if (winningPlayers.indexOf(playerID) == -1) others.push(playerID);
  }
  ...
  let captureTheRelicDuration =
    cmpEndGameManager.GetGameSettings().relicDuration;
  ...
  this.ownRelicsVictoryMessage = cmpGuiInterface.AddTimeNotification(
    {
      message: isTeam
        ? markForTranslation(
            "%(_player_)s and their allies have captured all relics and will win in %(time)s."
          )
        : markForTranslation(
            "%(_player_)s has captured all relics and will win in %(time)s."
          ),
      players: others,
      parameters: {
  ...
  this.othersRelicsVictoryMessage = cmpGuiInterface.AddTimeNotification(
    {
      message: isTeam
        ? markForTranslation(
            "You and your allies have captured all relics and will win in %(time)s."
          )
        : markForTranslation(
            "You have captured all relics and will win in %(time)s."
          ),
  ...
  this.relicsVictoryTimer = cmpTimer.SetTimeout(
    SYSTEM_ENTITY,
    IID_Trigger,
    "CaptureTheRelicVictorySetWinner",
    captureTheRelicDuration,
    winningPlayers
  );
};
...

{
  ...
  cmpTrigger.RegisterTrigger(
    "OnDiplomacyChanged",
    "CheckCaptureTheRelicCountdown",
    { enabled: true }
  );
  cmpTrigger.RegisterTrigger(
    "OnOwnershipChanged",
    "CheckCaptureTheRelicVictory",
    { enabled: true }
  );
  cmpTrigger.RegisterTrigger(
    "OnPlayerWon",
    "DeleteCaptureTheRelicVictoryMessages",
    { enabled: true }
  );
  cmpTrigger.RegisterTrigger(
    "OnPlayerDefeated",

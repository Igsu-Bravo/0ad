Trigger.prototype.ConquestOwnershipChanged = function (msg) {
  if (!this.conquestDataInit) return;

  for (let query of this.conquestQueries) {
    if (!TriggerHelper.EntityMatchesClassList(msg.entity, query.classFilter))
      continue;

    if (msg.to > 0) query.entitiesByPlayer[msg.to].push(msg.entity);

    if (msg.from <= 0) continue;
    ...
    if (!entities.length) {
      ...
      if (cmpPlayer) cmpPlayer.SetState("defeated", query.defeatReason);
    }
  }
};
...
Trigger.prototype.ConquestStartGameCount = function () {
  ...
  let entitiesByPlayer = Engine.QueryInterface(SYSTEM_ENTITY, IID_PlayerManager)
    .GetAllPlayers()
    .map((playerID) => cmpRangeManager.GetEntitiesByPlayer(playerID));

  for (let query of this.conquestQueries)
    query.entitiesByPlayer = entitiesByPlayer.map((ents) =>
      ents.filter((ent) =>
        TriggerHelper.EntityMatchesClassList(ent, query.classFilter)
      )
    );
...

{
  ...
  cmpTrigger.RegisterTrigger("OnOwnershipChanged", "ConquestOwnershipChanged", {
    enabled: true,
  });

Trigger.prototype.InitRegicideGame = function (msg) {
  ...
  for (
    ...
    let spawnPoints = cmpRangeManager
    ...
    if (
      !regicideGarrison &&
      TriggerHelper.EntityMatchesClassList(spawnPoints[0], "Ship")
    ) {
      let shipPosition = Engine.QueryInterface(
        spawnPoints[0],
        IID_Position
      ).GetPosition2D();

      let distanceToShip = (entity) =>
        Engine.QueryInterface(entity, IID_Position)
          .GetPosition2D()
          .distanceToSquared(shipPosition);

      spawnPoints = TriggerHelper.GetLandSpawnPoints().sort(
        (entity1, entity2) => distanceToShip(entity1) - distanceToShip(entity2)
      );
    }

    this.regicideHeroes[playerID] = this.SpawnRegicideHero(
      playerID,
      heroTemplates[playersCivs[playerID]],
      spawnPoints
    );
  }
};
 ...
 */
Trigger.prototype.SpawnRegicideHero = function (
  playerID,
  heroTemplates,
  spawnPoints
) {
  for (let heroTemplate of shuffleArray(heroTemplates))
    for (let spawnPoint of spawnPoints) {
      ...
      if (isShip) {
        ...
        if (
          cmpGarrisonHolder.IsFull() ||
          !MatchesClassList(
            heroTemplate.classes,
      ...
      let hero = TriggerHelper.SpawnUnits(
        spawnPoint,
        heroTemplate.templateName,
        1,
        playerID
      );
...
Trigger.prototype.RenameRegicideHero = function (data) {
  ...
  if (index != -1) this.regicideHeroes[index] = data.newentity;
};
...

{
  ...
  cmpTrigger.RegisterTrigger("OnOwnershipChanged", "CheckRegicideDefeat", {
    enabled: true,
  });
  cmpTrigger.RegisterTrigger("OnEntityRenamed", "RenameRegicideHero", {
    enabled: true,
  });
}

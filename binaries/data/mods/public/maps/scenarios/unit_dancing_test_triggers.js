      ...
      attackers.push(
        Attack(
          ...
          WalkTo(
            ...
            gy + i * 2,
            true,
            QuickSpawn(gx + att_distance + i, gy, attacker, ATTACKER),
            ATTACKER
          )
        )
      );
...
var manual_square_dance = function (
  attacker,
  target,
  dance_distance,
  att_distance = 50,
...
var manual_zigzag_dance = function (
  attacker,
  target,
  dance_distance,
  att_distance = 50,
...
var manual_formation_dance = function (
  attacker,
  target,
  dance_distance,
  att_distance = 50,
...
var avoidance = function (attacker, target, att_distance = 10) {
  return () => {
    ...
    let attackers = [];
    attackers.push(
      Attack(dancer, QuickSpawn(200, 290, attacker, ATTACKER), ATTACKER)
    );
    return [[dancer], attackers];
  };
};
...
Trigger.prototype.SetupUnits = function () {
  ...
  for (let key in experiments) {
    ...
    let ReportResults = (killed) => {
      warn(
        `Exp ${key} finished in ${
          Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer).GetTime() - start
...
var cmpModifiersManager = Engine.QueryInterface(
  SYSTEM_ENTITY,
  IID_ModifiersManager
);

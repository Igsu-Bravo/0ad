Trigger.prototype.IntroductionMessage = function (data) {
  ...
  cmpGUIInterface.PushNotification({
    ...
    message: markForTranslation(
      "Collect the treasures before your enemy does! May the better win!"
    ),
...
Trigger.prototype.TreasureCollected = function (data) {
  ...
  if (
    count == this.treasureCount.maximum / 2 &&
  ...
  } else if (count >= goalCount) {
    // Check for victory
    cmpGUIInterface.PushNotification({
      players: [otherPlayer],
      message: markForTranslation(
        "Your enemy's treasury is filled to the brim, you lose!"
      ),
      translateMessage: true,
    });
    cmpGUIInterface.PushNotification({
      players: [data.player],
      message: markForTranslation(
        "Your treasury is filled to the brim, you are victorious!"
      ),
      translateMessage: true,
    });
    ...
    if (count + 3 == goalCount) {
      cmpGUIInterface.PushNotification({
        players: [otherPlayer],
        message: markForTranslation(
          "Hurry up! Your enemy is close to victory!"
        ),
        translateMessage: true,
      });
    }
    ...
    if (count + 3 >= goalCount) {
      ...
      cmpGUIInterface.PushNotification({
        ...
        message: markForTranslation(
          "Treasures remaining to collect for victory:  %(remainingTreasures)s!"
        ),
        translateMessage: true,
      });
    } else {
...
cmpTrigger.RegisterTrigger("OnTreasureCollected", "TreasureCollected", {
  enabled: true,
});

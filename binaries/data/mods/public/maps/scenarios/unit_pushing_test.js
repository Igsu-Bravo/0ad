...
var Do = function (name, data, ent, owner = 1) {
  ...
  for (let k in data) comm[k] = data[k];
...
experiments.building = {
  spawn: (gx, gy) => {
    let target = QuickSpawn(
      gx + 20,
      gy + 20,
      "foundation|structures/athen/storehouse"
    );
    for (let i = 0; i < 8; ++i)
      Do(
        "repair",
...
experiments.collecting_tree = {
  spawn: (gx, gy) => {
    ...
    let storehouse = QuickSpawn(
      gx - 10,
      gy - 10,
      "structures/athen/storehouse"
    );
    for (let i = 0; i < 8; ++i)
      Do(
        "gather",
...
experiments.overlapping = {
  spawn: (gx, gy) => {
    for (let i = 0; i < 20; ++i) QuickSpawn(gx, gy, REG_UNIT_TEMPLATE);
  },
};
    ...
    for (let i = 0; i < 20; i += 2)
      for (let j = 0; j < 20; j += 2)
        WalkTo(
          ...
          gy,
          false,
          QuickSpawn(gx + i, gy + j + 70, REG_UNIT_TEMPLATE)
        );
  },
};
    ...
    for (let x = 0; x < 20 * 16 * 4 - 20; x += spacing)
      for (let z = 0; z < 20 * 16 * 4 - 20; z += spacing) {
        for (let i = 0; i < 10; ++i) {
          ...
          let ent = QuickSpawn(
            x + i + randFloat(-0.5, 0.5),
            z + 20 * (i % 2) + randFloat(-0.5, 0.5),
            REG_UNIT_TEMPLATE
          );
...
var woodcutting = (gx, gy) => {
  ...
  for (let i = 10; i <= 90; i += 5)
    Do(
      "gather-near-position",
      {
        x: gx + i,
        z: gy + 10,
        resourceType: { generic: "wood", specific: "tree" },
        resourceTemplate: "gaia/tree/acacia",
      },
      QuickSpawn(gx + i, gy, REG_UNIT_TEMPLATE)
    );
};
...
Trigger.prototype.Cleanup = function () {
  ...
  let ents = cmpRangeManager
    .GetEntitiesByPlayer(1)
    .concat(cmpRangeManager.GetEntitiesByPlayer(2));
  for (let ent of ents) Engine.DestroyEntity(ent);
};

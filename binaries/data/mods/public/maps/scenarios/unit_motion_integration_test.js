...
experiments.enclosed = {
  spawn: () => {
    ...
    experiments.enclosed.remove = QuickSpawn(
      gx,
      gy + 36,
      "structures/palisades_long"
    );
    Rotate(
      Math.PI / 2,
      QuickSpawn(gx - 8, gy + 28, "structures/palisades_long")
    );
    Rotate(
      Math.PI / 2,
      QuickSpawn(gx + 8, gy + 28, "structures/palisades_long")
    ...
    for (let i = -16; i <= 16; i += 8)
      for (let j = -16; j <= 16; j += 8)
        Do(
          "stance",
          { name: "standground" },
          QuickSpawn(gx + i, gy + 50 + j, UNIT_TEMPLATE)
        );
    WalkTo(gx, gy + 100, QuickSpawn(gx, gy, UNIT_TEMPLATE));
...
experiments.u_shape_tight_exit = {
  spawn: () => {
    ...
    Rotate(
      Math.PI / 2,
      QuickSpawn(gx + 8, gy + 12, "structures/palisades_long")
    );
    ...
    Rotate(
      Math.PI / 2,
      QuickSpawn(gx - 16, gy + 8, "structures/palisades_long")
    );
    QuickSpawn(gx - 4, gy, "structures/palisades_medium");
    QuickSpawn(gx - 12, gy, "structures/palisades_medium");
    Rotate(
      Math.PI / 4,
      QuickSpawn(gx - 10, gy + 8, "structures/palisades_short")
    );
    QuickSpawn(gx, gy - 10, "structures/palisades_long");
    Rotate(
      Math.PI / 2,
      QuickSpawn(gx + 8, gy - 5, "structures/palisades_long")
    );
    Rotate(
      Math.PI / 2,
      QuickSpawn(gx - 8, gy - 7, "structures/palisades_long")
    );
    ...
    for (let i = -3; i <= 3; i += 1) {
      ...
      Do(
        "gather",
        { target: tree },
        QuickSpawn(gx + i, gy - 3 - 5, UNIT_TEMPLATE)
      );
    }
  },
};
...
experiments.need_to_backtrack = {
  spawn: () => {
    ...
    QuickSpawn(gx - 30, gy + 40, SMALL_STRUCTURE_TEMPLATE);
    for (let i = -4; i <= 4; ++i) QuickSpawn(gx + i, gy + 65, UNIT_TEMPLATE);

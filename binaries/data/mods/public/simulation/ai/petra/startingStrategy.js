...
PETRA.HQ.prototype.gameAnalysis = function (gameState) {
  ...
  if (!this.regionAnalysis(gameState)) return;
  ...
  for (let cc of ccEnts.values())
    if (cc.foundationProgress() === undefined) this.createBase(gameState, cc);
    else this.createBase(gameState, cc, "unconstructed");
  ...
  if (
    ...
    if (
      !gameState.isTemplateAvailable(template) ||
      !gameState.getTemplate(template).available(gameState)
    ) {
      if (this.Config.debug > 1)
        API3.warn(" this AI is unable to produce any units");
      this.canBuildUnits = false;
      this.dispatchUnits(gameState);
    } else this.buildFirstBase(gameState);
  }

  // configure our first base strategy
  if (this.baseManagers.length > 1) this.configFirstBase(gameState);
};
...
PETRA.HQ.prototype.assignStartingEntities = function (gameState) {
  for (let ent of gameState.getOwnEntities().values()) {
    ...
    if (ent.hasClasses(["Trader+!Ship"])) this.tradeManager.assignTrader(ent);
    ...
    if (!pos) {
      ...
      if (ent.isGarrisonHolder() && ent.garrisoned().length)
        API3.warn(
          "Petra warning: support for garrisoned units inside garrisoned holders not yet implemented"
        );
      continue;
    }
    ...
    if (land > 1 && !this.landRegions[land]) this.landRegions[land] = true;
    let sea = gameState.ai.accessibility.navalPassMap[index];
    if (sea > 1 && !this.navalRegions[sea]) this.navalRegions[sea] = true;
    ...
    if (
      ...
      ent.garrisoned().length &&
      !ent.hasClass("Ship")
    )
      for (let id of ent.garrisoned()) ent.unload(id);

    let bestbase;
    let territorypos = this.territoryMap.gamePosToMapPos(pos);
    let territoryIndex =
      territorypos[0] + territorypos[1] * this.territoryMap.width;
    for (let i = 1; i < this.baseManagers.length; ++i) {
      let base = this.baseManagers[i];
      if (
        (!ent.getMetadata(PlayerID, "base") ||
          ent.getMetadata(PlayerID, "base") != base.ID) &&
    ...
    if (!bestbase) {
      ...
      if (
        ent.hasClass("Structure") &&
        !ent.decaying() &&
        ent.resourceDropsiteTypes()
      )
        bestbase = this.createBase(gameState, ent, "anchorless");
      else bestbase = PETRA.getBestBase(gameState, ent) || this.baseManagers[0];
...
PETRA.HQ.prototype.regionAnalysis = function (gameState) {
  ...
  if (!landIndex) {
    ...
    for (let ent of gameState.getOwnEntities().values()) {
      if (
        !ent.position() ||
        (!ent.hasClass("Unit") && !ent.trainableEntities(civ))
      )
        continue;
      ...
      if (!seaIndex && sea > 1) seaIndex = sea;
    }
  }
  ...
  for (let i = 0; i < accessibility.regionSize.length; ++i) {
    if (landIndex && i == landIndex) this.landRegions[i] = true;
    else if (
      accessibility.regionType[i] === "land" &&
      cellArea * accessibility.regionSize[i] > 320
    ) {
      if (landIndex) {
        let sea = this.getSeaBetweenIndices(gameState, landIndex, i);
        if (
          sea &&
          (accessibility.regionSize[i] > minLandSize ||
      ...
      accessibility.regionType[i] === "water" &&
      accessibility.regionSize[i] > minWaterSize
    ) {
      this.navalMap = true;
      this.navalRegions[i] = true;
    } else if (
      accessibility.regionType[i] === "water" &&
      cellArea * accessibility.regionSize[i] > 3600
    )
      this.navalRegions[i] = true;
  }

  if (this.Config.debug < 3) return true;
...
PETRA.HQ.prototype.buildFirstBase = function (gameState) {
  if (gameState.ai.queues.civilCentre.hasQueuedUnits()) return;
  let templateName = gameState.applyCiv("structures/{civ}/civil_centre");
  if (gameState.isTemplateDisabled(templateName)) return;
  ...
  if (!total.canAfford(new API3.Resources(template.cost()))) {
    ...
    for (let ent of gameState.getOwnUnits().values()) {
      ...
      if (ent.isIdle()) PETRA.gatherTreasure(gameState, ent);
      ...
      for (let type in types)
        if (type in totalExpected) totalExpected[type] += types[type];
      // If we can collect enough resources from these treasures, wait for them.
      if (totalExpected.canAfford(new API3.Resources(template.cost()))) return;
    }

    // not enough resource to build a cc, try with a dock to accumulate resources if none yet
    if (
      !this.navalManager.docks
        .filter(API3.Filters.byClass("Dock"))
        .hasEntities()
    ) {
      if (gameState.ai.queues.dock.hasQueuedUnits()) return;
  ...
  if (!this.canBuild(gameState, templateName)) return;

  // We first choose as startingPoint the point where we have the more units
  let startingPoint = [];
  for (let ent of gameState.getOwnUnits().values()) {
    if (!ent.hasClass("Worker")) continue;
    if (PETRA.isFastMoving(ent)) continue;
    let pos = ent.position();
    if (!pos) {
      let holder = PETRA.getHolder(gameState, ent);
      if (!holder || !holder.position()) continue;
      pos = holder.position();
    }
    let gamepos = gameState.ai.accessibility.gamePosToMapPos(pos);
    let index = gamepos[0] + gamepos[1] * gameState.ai.accessibility.width;
    ...
    for (let point of startingPoint) {
      if (land !== point.land || sea !== point.sea) continue;
      if (API3.SquareVectorDistance(point.pos, pos) > 2500) continue;
      point.weight += 1;
      found = true;
      break;
    }
  ...
  if (!startingPoint.length) return;

  let imax = 0;
  for (let i = 1; i < startingPoint.length; ++i)
    if (startingPoint[i].weight > startingPoint[imax].weight) imax = i;

  if (goal == "dock") {
    let sea = startingPoint[imax].sea > 1 ? startingPoint[imax].sea : undefined;
    gameState.ai.queues.dock.addPlan(
    ...
    gameState.ai.queues.civilCentre.addPlan(
      new PETRA.ConstructionPlan(gameState, "structures/{civ}/civil_centre", {
        base: -1,
        resource: "wood",
        proximity: startingPoint[imax].pos,
      })
...
PETRA.HQ.prototype.dispatchUnits = function (gameState) {
  ...
  if (allycc.length) {
    ...
    let num = Math.max(
      Math.min(
        Math.round(
          0.08 * (1 + this.Config.personality.cooperative) * units.length
        ),
        20
      ),
      5
    ...
    units.filter(API3.Filters.byClasses(["Infantry+Ranged"])).forEach((ent) => {
      if (!num || !num1) return;
      if (ent.getMetadata(PlayerID, "allied")) return;
    ...
    units.forEach((ent) => {
      if (!num && !ent.hasClass("Support")) return;
      if (ent.getMetadata(PlayerID, "allied")) return;
      let access = PETRA.getLandAccess(gameState, ent);
      for (let cc of allycc) {
        if (!cc.position() || PETRA.getLandAccess(gameState, cc) != access)
          continue;
        if (!ent.hasClass("Support")) --num;
        ent.setMetadata(PlayerID, "allied", true);
        let range = 1.5 * cc.footprintRadius();
        ent.moveToRange(cc.position()[0], cc.position()[1], range, range + 5);
        break;
...
PETRA.HQ.prototype.configFirstBase = function (gameState) {
  if (this.baseManagers.length < 2) return;
  ...
  for (let region in this.landRegions) {
    for (let base of this.baseManagers) {
      if (!base.anchor || base.accessIndex != +region) continue;
  ...
  if (this.Config.debug > 1)
    API3.warn(
      "starting size " + startingSize + "(cut at 24000 for fish pushing)"
    );
  if (startingSize < 25000) {
    this.saveSpace = true;
    this.Config.Economy.popForDock = Math.min(
  ...
  } else if (startingSize < 60000) this.maxFields = 2;
  else this.maxFields = false;
  ...
  if (startingWood < 6000) {
    ...
    this.Config.Economy.popPhase2 = Math.floor(
      0.75 * this.Config.Economy.popPhase2
    ); // Switch to town phase sooner to be able to expand
  ...
  if (startingWood > 8500 && this.canBuildUnits) {
    ...
    if (gameState.isCeasefireActive()) {
      if (gameState.ceasefireTimeRemaining > 900) allowed = 0;
  ...
  if (!gameState.getOwnEntitiesByClass("DropsiteWood", true).hasEntities()) {
    const newDP = this.baseManagers[1].findBestDropsiteAndLocation(
      gameState,
      "wood"
    );
    if (newDP.quality > 40 && this.canBuild(gameState, newDP.templateName)) {
      ...
      let numWorkers = gameState
        .getOwnUnits()
        .filter(API3.Filters.byClass("Worker")).length;
      if (
        (numWorkers > 12 && newDP.quality > 60) ||
        gameState.getPopulation() > gameState.getPopulationLimit() + 20
      ) {
        const cost = new API3.Resources(
          gameState.getTemplate(newDP.templateName).cost()
        );
        gameState.ai.queueManager.setAccounts(gameState, cost, "dropsites");
      }
      gameState.ai.queues.dropsites.addPlan(
        new PETRA.ConstructionPlan(
          gameState,
          newDP.templateName,
          { base: this.baseManagers[1].ID },
          newDP.pos
        )
      );
    }

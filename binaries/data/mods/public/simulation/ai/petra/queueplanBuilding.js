...
PETRA.ConstructionPlan = function (gameState, type, metadata, position) {
  if (!PETRA.QueuePlan.call(this, gameState, type, metadata)) return false;
...
PETRA.ConstructionPlan.prototype.canStart = function (gameState) {
  if (gameState.ai.HQ.turnCache.buildingBuilt)
    // do not start another building if already one this turn
    return false;

  if (!this.isGo(gameState)) return false;

  if (
    this.template.requiredTech() &&
...
PETRA.ConstructionPlan.prototype.start = function (gameState) {
  ...
  if (!pos) {
    gameState.ai.HQ.buildManager.setUnbuildable(
      gameState,
      this.type,
      90,
      "room"
  ...
  if (
    this.metadata &&
    this.metadata.expectedGain &&
    (!this.template.hasClass("Market") ||
  ...
  if (this.metadata === undefined) this.metadata = { base: pos.base };
  else if (this.metadata.base === undefined) this.metadata.base = pos.base;

  if (pos.access) this.metadata.access = pos.access;
  // needed for Docks whose position is on water
  else
    this.metadata.access = gameState.ai.accessibility.getAccessValue([
      pos.x,
      pos.z,
    ]);
  ...
  // try with the lowest, move towards us unless we're same
    ...
    for (let step = 0; step <= 1; step += 0.2)
      builder.construct(
        this.type,
        step * pos.x + (1 - step) * pos.xx,
        step * pos.z + (1 - step) * pos.zz,
        pos.angle,
        this.metadata
      );
  }
  ...
  if (this.metadata && this.metadata.proximity)
    gameState.ai.HQ.navalManager.createTransportIfNeeded(
      gameState,
      this.metadata.proximity,
      [pos.x, pos.z],
...
PETRA.ConstructionPlan.prototype.findGoodPosition = function (gameState) {
  ...
  if (template.hasClass("Storehouse") && this.metadata && this.metadata.base) {
    ...
    const newpos = base.findBestDropsiteLocation(
      gameState,
      type,
  ...
  if (!this.position) {
    if (template.hasClass("CivCentre")) {
      ...
      if (this.metadata && this.metadata.resource) {
        let proximity = this.metadata.proximity
          ? this.metadata.proximity
          : undefined;
        pos = HQ.findEconomicCCLocation(
          gameState,
          template,
          this.metadata.resource,
          proximity
        );
      } else pos = HQ.findStrategicCCLocation(gameState, template);
      ...
      if (
        gameState.ai.HQ.canBuild(gameState, templateName) &&
        !gameState.isTemplateDisabled(templateName)
      ) {
        ...
        if (
          ...
          gameState
            .getResources()
            .canAfford(new API3.Resources(template.cost()))
        )
      ...
      if (
        !template.hasClass("Fortress") ||
        gameState.getOwnEntitiesByClass("Fortress", true).hasEntities()
      )
        return false;
    } else if (template.hasClass("Market")) {
      // Docks are done before.
  ...
  if (this.position) {
    ...
    if (this.metadata && this.metadata.base !== undefined) {
      ...
      for (let j = 0; j < placement.map.length; ++j)
        if (HQ.basesMap.map[j] == base) placement.set(j, 45);
    } else {
      for (let j = 0; j < placement.map.length; ++j)
        if (HQ.basesMap.map[j] != 0) placement.set(j, 45);
    }
    ...
    if (!HQ.requireHouses || !template.hasClass("House")) {
      gameState.getOwnStructures().forEach(function (ent) {
        ...
        if (
          struct.resourceDropsiteTypes() &&
          struct.resourceDropsiteTypes().indexOf("food") != -1
        ) {
          if (template.hasClasses(["Field", "Corral"]))
            placement.addInfluence(x, z, 80 / cellSize, 50);
          // If this is not a field add a negative influence because we want to leave this area for fields
          else placement.addInfluence(x, z, 80 / cellSize, -20);
          ...
          template.hasClass("Farmstead") &&
          !ent.hasClasses(["Field", "Corral"]) &&
          ent.hasClasses(["Gate", "!Wall"])
        )
          placement.addInfluence(x, z, 100 / cellSize, -25);
        // move farmsteads away to make room (Wall test needed for iber)
          ...
          template.genericName() == "Rotary Mill" &&
          ent.hasClass("Field")
        )
    ...
    if (template.hasClass("Farmstead")) {
      for (let j = 0; j < placement.map.length; ++j) {
        let value =
          placement.map[j] -
          gameState.sharedScript.resourceMaps.wood.map[j] / 3;
        if (HQ.borderMap.map[j] & PETRA.fullBorder_Mask) value /= 2; // we need space around farmstead, so disfavor map border
  ...
  let disfavorBorder =
    gameState.currentPhase() > 1 && !template.hasDefensiveFire();
  let favoredBase =
    this.metadata &&
    (this.metadata.favoredBase ||
      (this.metadata.militaryBase
        ? HQ.findBestBaseForMilitary(gameState)
        : undefined));
  if (this.metadata && this.metadata.base !== undefined) {
    let base = this.metadata.base;
    for (let j = 0; j < placement.map.length; ++j) {
      if (HQ.basesMap.map[j] != base) placement.map[j] = 0;
      else if (placement.map[j] > 0) {
        if (favorBorder && HQ.borderMap.map[j] & PETRA.border_Mask)
          placement.set(j, placement.map[j] + 50);
        else if (
          disfavorBorder &&
          !(HQ.borderMap.map[j] & PETRA.fullBorder_Mask)
        )
          placement.set(j, placement.map[j] + 10);

        let x = ((j % placement.width) + 0.5) * cellSize;
        let z = (Math.floor(j / placement.width) + 0.5) * cellSize;
        if (HQ.isNearInvadingArmy([x, z])) placement.map[j] = 0;
      }
    }
  } else {
    for (let j = 0; j < placement.map.length; ++j) {
      if (HQ.basesMap.map[j] == 0) placement.map[j] = 0;
      else if (placement.map[j] > 0) {
        if (favorBorder && HQ.borderMap.map[j] & PETRA.border_Mask)
          placement.set(j, placement.map[j] + 50);
        else if (
  ...
  let radius = 0;
  if (
    template.hasClasses(["Fortress", "Arsenal"]) ||
    ...
    !template.hasClasses(["House", "Field", "Market"])
  )
    radius = Math.ceil(
      (template.obstructionRadius().max + 4) / obstructions.cellSize
    );
  else
    radius = Math.ceil(
  ...
  if (template.hasClass("House") && !alreadyHasHouses) {
    ...
    if (!bestTile.val) bestTile = undefined;
  }

  if (!bestTile) bestTile = placement.findBestTile(radius, obstructions);

  if (!bestTile.val) return false;
  ...
  return {
    ...
    z: z,
    angle: (3 * Math.PI) / 4,
    base: HQ.basesMap.map[territoryIndex],
  };
};
...
PETRA.ConstructionPlan.prototype.findDockPosition = function (gameState) {
  ...
  let wantedLand =
    this.metadata && this.metadata.land ? this.metadata.land : null;
  let wantedSea = this.metadata && this.metadata.sea ? this.metadata.sea : null;
  let proxyAccess =
    this.metadata && this.metadata.proximity
      ? gameState.ai.accessibility.getAccessValue(this.metadata.proximity)
      : null;
  let oversea =
    this.metadata && this.metadata.oversea ? this.metadata.oversea : null;
  ...
  const maxWater = 16;
  let ccEnts = oversea
    ? gameState.updatingGlobalCollection(
        "allCCs",
        API3.Filters.byClass("CivCentre")
      )
    : null;
  let docks = oversea
    ? gameState.getOwnStructures().filter(API3.Filters.byClass("Dock"))
    : null;
  ...
  for (let j = 0; j < territoryMap.length; ++j) {
    ...
    if (i < 0) continue;
    if (wantedSea && navalPassMap[i] != wantedSea) continue;

    let res = dropsiteTypes
      ? Math.min(
          maxRes,
          this.getResourcesAround(gameState, dropsiteTypes, j, 80)
        )
      : maxRes;
    let pos = [
    ...
    if (proxyAccess) score = API3.VectorDistance(this.metadata.proximity, pos);
    ...
    if (oversea) {
      ...
      for (let cc of ccEnts.values()) {
        let owner = cc.owner();
        if (owner != PlayerID && !gameState.isPlayerEnemy(owner)) continue;
        let dist = API3.SquareVectorDistance(pos, cc.position());
        if (owner == PlayerID && (!ownDist || dist < ownDist)) ownDist = dist;
        if (gameState.isPlayerEnemy(owner) && (!enemyDist || dist < enemyDist))
          enemyDist = dist;
      }
      if (ownDist && enemyDist && enemyDist < 0.5 * ownDist) continue;

      // And maximize distance for trade.
      let dockDist = 0;
      for (let dock of docks.values()) {
        if (PETRA.getSeaAccess(gameState, dock) != navalPassMap[i]) continue;
        let dist = API3.SquareVectorDistance(pos, dock.position());
        if (dist > dockDist) dockDist = dist;
      }
      if (dockDist > 0) {
        dockDist = Math.sqrt(dockDist);
        if (dockDist > width * cellSize)
          // Could happen only on square maps, but anyway we don't want to be too far away
          continue;
    ...
    if (gameState.ai.HQ.borderMap.map[j] & PETRA.fullBorder_Mask) score += 20;

    // Do a pre-selection, supposing we will have the best possible water
    if (bestIdx !== undefined && score > bestVal + 5 * maxWater) continue;
    ...
    if (angle == false) continue;
    let ret = this.checkDockPlacement(
      gameState,
      x,
      z,
      halfDepth,
    ...
    if (
      !ret ||
      !gameState.ai.HQ.landRegions[ret.land] ||
      (wantedLand && !wantedLand[ret.land])
    )
      continue;
    ...
    if (
      this.metadata.proximity &&
      gameState.ai.accessibility.regionSize[ret.land] < 4000
    )
      continue;
  ...
  if (!baseIndex) baseIndex = -2; // We'll do an anchorless base around it
 ...
 */
PETRA.ConstructionPlan.prototype.buildOverseaDock = function (
  gameState,
  template
) {
  let docks = gameState.getOwnStructures().filter(API3.Filters.byClass("Dock"));
  if (!docks.hasEntities()) return;
  ...
  for (let i = 0; i < gameState.ai.accessibility.regionSize.length; ++i) {
    if (
      gameState.ai.accessibility.regionType[i] != "land" ||
      cellArea * gameState.ai.accessibility.regionSize[i] < 3600
    )
      continue;
    let keep = true;
    for (let dock of docks.values()) {
      if (PETRA.getLandAccess(gameState, dock) != i) continue;
      keep = false;
      break;
    }
    if (!keep) continue;
  ...
  if (!gameState.ai.HQ.navalMap)
    API3.warn(
      "petra.findOverseaLand on a non-naval map??? we should never go there "
    );
...
PETRA.ConstructionPlan.prototype.getDockAngle = function (
  gameState,
  x,
  z,
  size
  ...
  for (let dist = 0; dist < 4; ++dist) {
    ...
    for (let i = 0; i < numPoints; ++i) {
      ...
      pos = [
        x - (1 + dist) * size * Math.sin(angle),
        z + (1 + dist) * size * Math.cos(angle),
      ];
      pos = gameState.ai.accessibility.gamePosToMapPos(pos);
      if (
        pos[0] < 0 ||
        pos[0] >= gameState.ai.accessibility.width ||
        pos[1] < 0 ||
        pos[1] >= gameState.ai.accessibility.height
      )
        continue;
 ...
 */
PETRA.ConstructionPlan.prototype.checkDockPlacement = function (
  gameState,
  x,
  z,
  halfDepth,
  ...
  if (
    gameState.ai.accessibility.landPassMap[j] > 1 ||
    gameState.ai.accessibility.navalPassMap[j] < 2
  )
    return null;
  ...
  j = pos[0] + pos[1] * gameState.ai.accessibility.width;
  if (gameState.ai.accessibility.landPassMap[j] != land) return null;
  pos = gameState.ai.accessibility.gamePosToMapPos([x - sz - sw, z - cz + cw]);
  j = pos[0] + pos[1] * gameState.ai.accessibility.width;
  if (gameState.ai.accessibility.landPassMap[j] != land) return null;
  let water = 0;
  ...
  for (let i = 1; i < 5; ++i) {
    pos = gameState.ai.accessibility.gamePosToMapPos([
      x + sz + i * (sp + sw),
      z + cz + i * (cp - cw),
    ]);
    if (
...
PETRA.ConstructionPlan.prototype.isDockLocation = function (
  gameState,
  j,
  dimension,
  wantedLand,
  ...
  if (
    (landPass > 1 && wantedLand && !wantedLand[landPass]) ||
    (landPass < 2 && accessibility.navalPassMap[k] < 2)
  )
    return false;

  for (let a of around) {
    pos = accessibility.gamePosToMapPos([
      x + dimLand * a[0],
      z + dimLand * a[1],
    ]);
    if (pos[0] < 0 || pos[0] >= accessibility.width) continue;
    if (pos[1] < 0 || pos[1] >= accessibility.height) continue;
    k = pos[0] + pos[1] * accessibility.width;
    landPass = accessibility.landPassMap[k];
    if (landPass < 2 || (wantedLand && !wantedLand[landPass])) continue;
    pos = accessibility.gamePosToMapPos([x - dimSea * a[0], z - dimSea * a[1]]);
    if (pos[0] < 0 || pos[0] >= accessibility.width) continue;
    if (pos[1] < 0 || pos[1] >= accessibility.height) continue;
    k = pos[0] + pos[1] * accessibility.width;
    if (
      (wantedSea && accessibility.navalPassMap[k] != wantedSea) ||
 ...
 */
PETRA.ConstructionPlan.prototype.getFrontierProximity = function (
  gameState,
  j
) {
  ...
  for (let a of around) {
    for (let i = 1; i < 5; ++i) {
      ...
      if (borderMap.map[jx + width * jz] & PETRA.outside_Mask) continue;
 ...
 */
PETRA.ConstructionPlan.prototype.getResourcesAround = function (
  gameState,
  types,
  i,
  radius
  ...
  let nbcell = 0;
  for (let k of types) {
    if (k == "food" || !resourceMaps[k]) continue;
...
PETRA.ConstructionPlan.prototype.isGo = function (gameState) {
  if (this.goRequirement && this.goRequirement == "houseNeeded") {
    if (
      !gameState.ai.HQ.canBuild(gameState, "structures/{civ}/house") &&
    ...
    for (let ent of gameState.getOwnFoundations().values()) {
      ...
      if (template) freeSlots += template.getPopulationBonus();
    }

    if (gameState.ai.HQ.saveResources) return freeSlots <= 10;
    if (gameState.getPopulation() > 55) return freeSlots <= 21;
    if (gameState.getPopulation() > 30) return freeSlots <= 15;
    return freeSlots <= 10;
  }
  return true;
};
...
PETRA.ConstructionPlan.prototype.onStart = function (gameState) {
  if (this.queueToReset)
    gameState.ai.queueManager.changePriority(
      this.queueToReset,
      gameState.ai.Config.priorities[this.queueToReset]
    );
};

...
PETRA.TradeManager.prototype.init = function (gameState) {
  this.traders = gameState
    .getOwnUnits()
    .filter(API3.Filters.byMetadata(PlayerID, "role", "trader"));
...
PETRA.TradeManager.prototype.trainMoreTraders = function (gameState, queues) {
  if (!this.hasTradeRoute() || queues.trader.hasQueuedUnits()) return;
  ...
  gameState.getOwnTrainingFacilities().forEach(function (ent) {
    for (let item of ent.trainingQueue()) {
      if (
        !item.metadata ||
        !item.metadata.role ||
        item.metadata.role != "trader"
      )
        continue;
      numTraders += item.count;
      if (item.metadata.sea !== undefined) numSeaTraders += item.count;
      else numLandTraders += item.count;
    }
  });
  if (
    numTraders >= this.targetNumTraders &&
    ((!this.tradeRoute.sea &&
      numLandTraders >= Math.floor(this.targetNumTraders / 2)) ||
      (this.tradeRoute.sea &&
        numSeaTraders >= Math.floor(this.targetNumTraders / 2)))
  )
    return;
  ...
  if (this.tradeRoute.sea) {
    ...
    let already = false;
    let shipToSwitch;
    gameState.ai.HQ.navalManager.seaTransportShips[this.tradeRoute.sea].forEach(
      function (ship) {
        if (already || !ship.hasClass("Trader")) return;
    ...
    if (shipToSwitch) {
      ...
      else shipToSwitch.setMetadata(PlayerID, "role", "switchToTrader");
      return;
    }
    ...
    else metadata.base = this.tradeRoute.target.getMetadata(PlayerID, "base");
  }

  if (!gameState.getTemplate(template)) {
    if (this.Config.debug > 0)
      API3.warn(
        "Petra error: trying to train " +
          template +
          " for civ " +
  ...
  queues.trader.addPlan(
    new PETRA.TrainingPlan(gameState, template, metadata, 1, 1)
  );
};
...
PETRA.TradeManager.prototype.updateTrader = function (gameState, ent) {
  ...
  if (!this.hasTradeRoute() || !ent.isIdle() || !ent.position()) return;
  ...
  if (!route) {
    ...
    if (this.Config.debug > 0)
      API3.warn(
        " no available route for " + ent.genericName() + " " + ent.id()
      );
    Engine.ProfileStop();
    return;
  }
  ...
  if (
    API3.SquareVectorDistance(route.target.position(), ent.position()) <
    API3.SquareVectorDistance(route.source.position(), ent.position())
  )
    nearerSource = false;
  ...
  if (nearerSource) ent.tradeRoute(route.target, route.source);
  else ent.tradeRoute(route.source, route.target);
...
PETRA.TradeManager.prototype.setTradingGoods = function (gameState) {
  ...
  if (!resTradeCodes.length) return;
  let tradingGoods = {};
  for (let res of resTradeCodes) tradingGoods[res] = 0;
  // first, try to anticipate future needs
  let stocks = gameState.ai.HQ.getTotalResourceLevel(gameState);
  let mostNeeded = gameState.ai.HQ.pickMostNeededResources(
    gameState,
    resTradeCodes
  );
  ...
  else tradingGoods[mostNeeded[0].type] += nextNeed;
...
PETRA.TradeManager.prototype.performBarter = function (gameState) {
  let barterers = gameState
    .getOwnEntitiesByClass("Barter", true)
    .filter(API3.Filters.isBuilt())
    .toEntityArray();
  if (barterers.length == 0) return false;
  let resBarterCodes = Resources.GetBarterableCodes();
  if (!resBarterCodes.length) return false;
  ...
  let getBarterRate = (prices, buy, sell) =>
    Math.round((100 * prices.sell[sell]) / prices.buy[buy]);
  ...
  for (let buy of resBarterCodes) {
    ...
    if (needs[buy] == 0 || needs[buy] < rates[buy] * 30) continue;
    ...
    for (let sell of resBarterCodes) {
      ...
      if (needs[sell] > 0 || available[sell] < 500) continue;

      let barterRateMin;
      if (sell == "food") {
        barterRateMin = 30;
        if (available[sell] > 40000) barterRateMin = 0;
        else if (available[sell] > 15000) barterRateMin = 5;
        else if (available[sell] > 1000) barterRateMin = 10;
      } else {
        barterRateMin = 70;
        if (available[sell] > 5000) barterRateMin = 30;
        else if (available[sell] > 1000) barterRateMin = 50;
        if (buy == "food") barterRateMin += 20;
      }
  ...
  if (
    available.food < 1000 ||
    needs.food > 0 ||
    resBarterCodes.indexOf("food") == -1
  )
    return false;
  let bestToBuy;
  let bestChoice = 0;
  for (let buy of resBarterCodes) {
    if (buy == "food") continue;
    ...
    if (barterRate < barterRateMin) continue;
  ...
  if (bestToBuy !== undefined) {
    ...
    if (this.Config.debug > 2)
      API3.warn(
        "Contingency bartering: sold food for " +
          bestToBuy +
          " available sell " +
          available.food +
          " available buy " +
          available[bestToBuy] +
          " barterRate " +
          getBarterRate(barterPrices, bestToBuy, "food") +
          " amount " +
          amount
      );
    return true;
...
PETRA.TradeManager.prototype.checkEvents = function (gameState, events) {
  ...
  for (let evt of events.EntityRenamed) {
    ...
    if (!ent || !ent.hasClass("Trade")) continue;
    for (let trader of this.traders.values()) {
      let route = trader.getMetadata(PlayerID, "route");
      if (!route) continue;
      if (route.source == evt.entity) route.source = evt.newentity;
      else if (route.target == evt.entity) route.target = evt.newentity;
      else continue;
...
PETRA.TradeManager.prototype.activateProspection = function (gameState) {
  ...
  gameState.ai.HQ.buildManager.setBuildable(
    gameState.applyCiv("structures/{civ}/market")
  );
  gameState.ai.HQ.buildManager.setBuildable(
    gameState.applyCiv("structures/{civ}/dock")
  );
};
...
PETRA.TradeManager.prototype.checkRoutes = function (gameState, accessIndex) {
  ...
  let market1 = gameState.updatingCollection(
    "OwnMarkets",
    API3.Filters.byClass("Trade"),
    gameState.getOwnStructures()
  );
  let market2 = gameState.updatingCollection(
    "diplo-ExclusiveAllyMarkets",
    API3.Filters.byClass("Trade"),
    gameState.getExclusiveAllyEntities()
  );
  if (market1.length + market2.length < 2) {
    // We have to wait  ... markets will be built soon
    this.tradeRoute = undefined;
    this.potentialTradeRoute = undefined;
    return false;
  }
  ...
  for (let m1 of market1.values()) {
    if (!m1.position()) continue;
    let access1 = PETRA.getLandAccess(gameState, m1);
    let sea1 = m1.hasClass("Naval")
      ? PETRA.getSeaAccess(gameState, m1)
      : undefined;
    for (let m2 of market2.values()) {
      if (onlyOurs && m1.id() >= m2.id()) continue;
      if (!m2.position()) continue;
      let access2 = PETRA.getLandAccess(gameState, m2);
      let sea2 = m2.hasClass("Naval")
        ? PETRA.getSeaAccess(gameState, m2)
        : undefined;
      ...
      if (
        land &&
        PETRA.isLineInsideEnemyTerritory(
          gameState,
          m1.position(),
          m2.position()
        )
      ...
      let gain = Math.round(
        gainMultiplier *
          TradeGain(
            API3.SquareVectorDistance(m1.position(), m2.position()),
            mapSize
          )
      );
      if (gain < this.minimalGain) continue;
      if (
        m1.foundationProgress() === undefined &&
        m2.foundationProgress() === undefined
      ) {
        if (accessIndex) {
          if (
            gameState.ai.accessibility.regionType[accessIndex] == "water" &&
            sea == accessIndex
          ) {
            if (gain < bestIndex.gain) continue;
            bestIndex = {
              source: m1,
              target: m2,
              gain: gain,
            ...
            gameState.ai.accessibility.regionType[accessIndex] == "land" &&
            land == accessIndex
          ) {
            if (gain < bestIndex.gain) continue;
            bestIndex = {
            ...
            if (gain < bestLand.gain) continue;
            bestLand = {
              source: m1,
              target: m2,
              gain: gain,
        ...
        if (gain < candidate.gain) continue;
        candidate = {
          source: m1,
          target: m2,
          gain: gain,
      ...
      if (gain < potential.gain) continue;
      potential = { source: m1, target: m2, gain: gain, land: land, sea: sea };
    }
  }

  if (potential.gain < 1) this.potentialTradeRoute = undefined;
  else this.potentialTradeRoute = potential;

  if (candidate.gain < 1) {
    if (this.Config.debug > 2) API3.warn("no better trade route possible");
    this.tradeRoute = undefined;
    return false;
  }
  ...
  if (this.Config.debug > 1 && this.tradeRoute) {
    if (candidate.gain > this.tradeRoute.gain)
      API3.warn(
        "one better trade route set with gain " +
          candidate.gain +
          " instead of " +
  ...
  if (this.Config.chat) {
    ...
    if (owner == PlayerID) owner = this.tradeRoute.target.owner();
  ...
  if (accessIndex) {
    if (bestIndex.gain > 0) return bestIndex;
    else if (
      gameState.ai.accessibility.regionType[accessIndex] == "land" &&
      bestLand.gain > 0
    )
...
PETRA.TradeManager.prototype.checkTrader = function (gameState, ent) {
  ...
  if (
    !possibleRoute ||
    (possibleRoute.source.id() != presentRoute.source &&
      possibleRoute.source.id() != presentRoute.target) ||
    (possibleRoute.target.id() != presentRoute.source &&
      possibleRoute.target.id() != presentRoute.target)
  ) {
...
PETRA.TradeManager.prototype.prospectForNewMarket = function (
  gameState,
  queues
) {
  if (
    queues.economicBuilding.hasQueuedUnitsWithClass("Trade") ||
    queues.dock.hasQueuedUnitsWithClass("Trade")
  )
    return;
  if (!gameState.ai.HQ.canBuild(gameState, "structures/{civ}/market")) return;
  if (
    !gameState
      .updatingCollection(
        "OwnMarkets",
  ...
  if (!marketPos || marketPos[3] == 0) {
    // marketPos[3] is the expected gain
    // no position found
    ...
    else this.routeProspection = false;
    return;
  }
  this.routeProspection = false;
  if (!this.isNewMarketWorth(marketPos[3])) return; // position found, but not enough gain compared to our present route
    ...
    if (this.potentialTradeRoute)
      API3.warn(
        "turn " +
          ...
          "we could have a new route with gain " +
          marketPos[3] +
          " instead of the present " +
          this.potentialTradeRoute.gain
      );
    else
      API3.warn(
        "turn " +
          gameState.ai.playedTurn +
          "we could have a first route with gain " +
          marketPos[3]
      );
  }
  ...
  if (!this.tradeRoute)
    gameState.ai.queueManager.changePriority(
      "economicBuilding",
      2 * this.Config.priorities.economicBuilding
    );
  let plan = new PETRA.ConstructionPlan(gameState, "structures/{civ}/market");
  if (!this.tradeRoute) plan.queueToReset = "economicBuilding";
...
PETRA.TradeManager.prototype.isNewMarketWorth = function (expectedGain) {
  ...
  if (expectedGain < this.minimalGain) return false;
  if (
    this.potentialTradeRoute &&
    expectedGain < 2 * this.potentialTradeRoute.gain &&
...
PETRA.TradeManager.prototype.update = function (gameState, events, queues) {
  ...
  if (this.Config.difficulty <= 1) return;

  if (this.checkEvents(gameState, events)) {
    // true if one market was built or destroyed
  ...
  if (this.tradeRoute) {
    ...
    if (gameState.ai.playedTurn % 60 == 0) this.setTradingGoods(gameState);
  }

  if (this.routeProspection) this.prospectForNewMarket(gameState, queues);
};

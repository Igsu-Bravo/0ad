...
PETRA.TransportPlan = function (
  ...
  if (startIndex == 1) {
    ...
    ship.setStance("none");
    for (let ent of units) ent.setMetadata(PlayerID, "onBoard", "onBoard");
  } else {
    this.sea = gameState.ai.HQ.getSeaBetweenIndices(
      gameState,
      startIndex,
      endIndex
    );
    if (!this.sea) {
      ...
      if (this.debug > 1)
        API3.warn(
          "transport plan with bad path: startIndex " +
            startIndex +
            " endIndex " +
            endIndex
        );
  ...
  if (this.debug > 1)
    API3.warn(
      "Starting a new transport plan with ID " +
        this.ID +
        " to index " +
        endIndex +
        " with units length " +
...
PETRA.TransportPlan.prototype.init = function (gameState) {
  this.units = gameState
    .getOwnUnits()
    .filter(API3.Filters.byMetadata(PlayerID, "transport", this.ID));
  this.ships = gameState.ai.HQ.navalManager.ships.filter(
    API3.Filters.byMetadata(PlayerID, "transporter", this.ID)
  );
  this.transportShips = gameState.ai.HQ.navalManager.transportShips.filter(
    API3.Filters.byMetadata(PlayerID, "transporter", this.ID)
  );
...
PETRA.TransportPlan.prototype.countFreeSlotsOnShip = function (ship) {
  if (ship.hitpoints() < ship.garrisonEjectHealth() * ship.maxHitpoints())
    return 0;
  let occupied =
    ship.garrisoned().length +
    this.units.filter(API3.Filters.byMetadata(PlayerID, "onBoard", ship.id()))
      .length;
  return Math.max(ship.garrisonMax() - occupied, 0);
};

PETRA.TransportPlan.prototype.assignUnitToShip = function (gameState, ent) {
  if (this.needTransportShips) return;

  for (let ship of this.transportShips.values()) {
    if (this.countFreeSlotsOnShip(ship) == 0) continue;
  ...
  if (!this.needSplit) this.needSplit = [ent];
  else this.needSplit.push(ent);
};
...
PETRA.TransportPlan.prototype.assignShip = function (gameState) {
  ...
  gameState.ai.HQ.navalManager.seaTransportShips[this.sea].forEach((ship) => {
    if (ship.getMetadata(PlayerID, "transporter")) return;
    if (pos) {
      let dist = API3.SquareVectorDistance(pos, ship.position());
      if (dist > distmin) return;
      distmin = dist;
      nearest = ship;
    } else if (!nearest) nearest = ship;
  });
...
PETRA.TransportPlan.prototype.removeUnit = function (gameState, unit) {
  ...
  if (shipId == "onBoard") return;
  // too late, already onBoard
  else if (shipId !== undefined) unit.stopMoving(); // cancel the garrison order
  ...
  if (shipId) {
    ...
    if (
      ship &&
      !ship.garrisoned().length &&
      !this.units.filter(API3.Filters.byMetadata(PlayerID, "onBoard", shipId))
        .length
    ) {
...
PETRA.TransportPlan.prototype.releaseShip = function (ship) {
  if (ship.getMetadata(PlayerID, "transporter") != this.ID) {
    API3.warn(
      " Petra: try removing a transporter ship with " +
        ship.getMetadata(PlayerID, "transporter") +
        " from " +
        this.ID +
        " and stance " +
        ship.getStance()
  ...
  if (defaultStance) ship.setStance(defaultStance);
...
PETRA.TransportPlan.prototype.releaseAll = function () {
  for (let ship of this.ships.values()) this.releaseShip(ship);
...
PETRA.TransportPlan.prototype.cancelTransport = function (gameState) {
  ...
  if (!base.anchor || !base.anchor.position()) {
    for (let newbase of gameState.ai.HQ.baseManagers) {
      if (!newbase.anchor || !newbase.anchor.position()) continue;
      ent.setMetadata(PlayerID, "base", newbase.ID);
      base = newbase;
      break;
    }
    if (!base.anchor || !base.anchor.position()) return false;
...
PETRA.TransportPlan.prototype.update = function (gameState) {
  if (this.state == "boarding") this.onBoarding(gameState);
  else if (this.state == "sailing") this.onSailing(gameState);
...
PETRA.TransportPlan.prototype.onBoarding = function (gameState) {
    ...
    if (!ent.getMetadata(PlayerID, "onBoard")) {
      ...
      if (ent.getMetadata(PlayerID, "onBoard")) {
        ...
        if (!this.boardingPos[shipId]) {
          this.boardingPos[shipId] = this.getBoardingPos(
            gameState,
            ship,
            this.startIndex,
            this.sea,
      ...
      let distShip = API3.SquareVectorDistance(
        this.boardingPos[shipId],
        ship.position()
      );
      if (!shipTested[shipId] && distShip > this.boardingRange) {
        ...
        if (
          unitAIState == "INDIVIDUAL.WALKING" ||
          ...
          if (time - ship.getMetadata(PlayerID, "timeGarrison") > 2) {
            ...
            if (oldPos[0] == newPos[0] && oldPos[1] == newPos[1]) retry = true;
            ent.setMetadata(PlayerID, "posGarrison", newPos);
            ent.setMetadata(PlayerID, "timeGarrison", time);
          }
        } else if (
          (unitAIState != "INDIVIDUAL.PICKUP.LOADING" &&
        ...
        if (retry) {
          if (!this.nTry[shipId]) this.nTry[shipId] = 1;
          else ++this.nTry[shipId];
          if (this.nTry[shipId] > 1) {
            // we must have been blocked by something ... try with another boarding point
            this.nTry[shipId] = 0;
            if (this.debug > 1)
      ...
      if (time - ent.getMetadata(PlayerID, "timeGarrison") > 2) {
        ...
        if (oldPos[0] == newPos[0] && oldPos[1] == newPos[1]) {
          if (distShip < this.boardingRange) {
            // looks like we are blocked ... try to go out of this trap
            if (!this.nTry[ent.id()]) this.nTry[ent.id()] = 1;
            else ++this.nTry[ent.id()];
            if (this.nTry[ent.id()] > 5) {
              if (this.debug > 1)
                API3.warn(
                  "unit blocked, but no ways out of the trap ... destroy it"
                );
            ...
            API3.SquareVectorDistance(this.boardingPos[shipId], newPos) > 225
          )
            ent.moveToRange(
              this.boardingPos[shipId][0],
              this.boardingPos[shipId][1],
              0,
              15
            );
        } else this.nTry[ent.id()] = 0;
        ent.setMetadata(PlayerID, "timeGarrison", time);
        ent.setMetadata(PlayerID, "posGarrison", ent.position());
      }
    }
  ...
  for (let ship of this.ships.values()) {
    ...
    this.boardingPos[ship.id()] = this.getBoardingPos(
      gameState,
      ship,
      this.endIndex,
      this.sea,
...
PETRA.TransportPlan.prototype.isOnBoard = function (ent) {
  for (let ship of this.transportShips.values()) {
    if (ship.garrisoned().indexOf(ent.id()) == -1) continue;
...
PETRA.TransportPlan.prototype.getBoardingPos = function (
  gameState,
  ship,
  landIndex,
  seaIndex,
  ...
  if (!gameState.ai.HQ.navalManager.landingZones[landIndex]) {
    ...
    API3.warn(
      " >>> no landing zone for land " + landIndex + " and sea " + seaIndex
    );
    return destination;
  }
  ...
  let alliedDocks = gameState
    .getAllyStructures()
    .filter(
      API3.Filters.and(
        API3.Filters.byClass("Dock"),
        API3.Filters.byMetadata(PlayerID, "sea", seaIndex)
      )
    )
    .toEntityArray();
  for (let i of gameState.ai.HQ.navalManager.landingZones[landIndex][
    seaIndex
  ]) {
    let pos = [(i % width) + 0.5, Math.floor(i / width) + 0.5];
    pos = [cell * pos[0], cell * pos[1]];
    let dist = API3.VectorDistance(startPos, pos);
    if (destination) dist += API3.VectorDistance(pos, destination);
    if (avoidEnnemy) {
    ...
    for (let shipId in this.boardingPos)
      if (
        this.boardingPos[shipId] !== undefined &&
        API3.SquareVectorDistance(this.boardingPos[shipId], pos) <
          this.boardingRange
      )
        dist += 1000000;
    ...
    for (let dock of alliedDocks) {
      if (dock.foundationProgress() !== undefined) distSquare = 900;
      else distSquare = 4900;
...
PETRA.TransportPlan.prototype.onSailing = function (gameState) {
  ...
  for (let recov of this.recovered) {
    ...
    if (!ent)
      ...
      continue;
    if (!ent.position()) {
      // reloading succeeded ... move a bit the ship before trying again
      let ship = gameState.getEntityById(recov.shipId);
      if (ship) ship.moveApart(recov.entPos, 15);
      continue;
    }
    if (this.debug > 1)
    ...
    if (gameState.ai.HQ.landRegions[index]) {
      if (this.debug > 1) API3.warn(" recovered entity kept " + ent.id());
      this.resetUnit(gameState, ent);
      // TODO we should not destroy it, but now the unit could still be reloaded on the next turn
      // and mess everything
      ent.destroy();
    } else {
      if (this.debug > 1) API3.warn("recovered entity destroyed " + ent.id());
      this.resetUnit(gameState, ent);
      ent.destroy();
    }
  }
    ...
    if (!ent)
      ...
      if (ship) {
        if (ship.garrisoned().indexOf(entId) != -1)
          ...
          API3.warn(
            "Petra transportPlan problem: unit not on ship without position ???"
          );
          this.resetUnit(gameState, ent);
          ent.destroy();
        }
      ...
      if (this.debug > 1)
        API3.warn(
          ">>> unit unloaded on a wrong region ! try to garrison it again <<<"
        );
      let ship = gameState.getEntityById(ent.getMetadata(PlayerID, "onBoard"));
      if (ship && !this.canceled) {
        ...
        if (this.debug > 1) API3.warn("no way ... we destroy it");
        this.resetUnit(gameState, ent);
        ent.destroy();
      }
    } else {
      ...
      if (dist > 30) ent.moveToRange(goal[0], goal[1], dist - 25, dist - 20);
      else ent.moveToRange(pos[0], pos[1], 20, 25);
      ent.setMetadata(PlayerID, "transport", undefined);
      ent.setMetadata(PlayerID, "onBoard", undefined);
      ent.setMetadata(PlayerID, "endPos", undefined);
    }
  ...
  for (let ship of this.transportShips.values()) {
    if (ship.unitAIState() == "INDIVIDUAL.WALKING") continue;
    let shipId = ship.id();
    ...
    for (let entId of ship.garrisoned()) {
      let ent = gameState.getEntityById(entId);
      if (!ent.getMetadata(PlayerID, "transport")) continue;
      remaining++;
      if (dist < 625) {
        ship.unload(entId);
    ...
    let recovering = 0;
    for (let recov of this.recovered) if (recov.shipId == shipId) recovering++;

    if (!remaining && !recovering) {
      // when empty, release the ship and move apart to leave room for other ships. TODO fight

...
PETRA.GarrisonManager.prototype.update = function (gameState, events) {
  ...
  for (let evt of events.EntityRenamed) {
    ...
    for (let id of this.decayingStructures.keys()) {
      ...
      if (this.decayingStructures.has(evt.newentity)) continue;
      let ent = gameState.getEntityById(evt.newentity);
      if (!ent || !ent.territoryDecayRate() || !ent.garrisonRegenRate())
        continue;
      let gmin = Math.ceil(
        (ent.territoryDecayRate() - ent.defaultRegenRate()) /
          ent.garrisonRegenRate()
      );
  ...
  for (let [id, data] of this.holders.entries()) {
    ...
    if (!holder || !gameState.isPlayerAlly(holder.owner())) {
      ...
      for (let entId of list) {
        let ent = gameState.getEntityById(entId);
        if (!ent || ent.getMetadata(PlayerID, "garrisonHolder") != id) continue;
        this.leaveGarrison(ent);
        ent.stopMoving();
      }
      this.holders.delete(id);
    ...
    for (let j = 0; j < list.length; ++j) {
      for (let evt of events.EntityRenamed)
        if (evt.entity === list[j]) list[j] = evt.newentity;

      let ent = gameState.getEntityById(list[j]);
      if (!ent)
        // unit must have been killed while garrisoning
        list.splice(j--, 1);
      else if (holder.garrisoned().indexOf(list[j]) !== -1) {
        // unit is garrisoned
        this.leaveGarrison(ent);
        list.splice(j--, 1);
      } else {
        if (
          ent
            .unitAIOrderData()
            .some((order) => order.target && order.target == id)
        )
          continue;
        if (ent.getMetadata(PlayerID, "garrisonHolder") == id) {
          ...
          if (gameState.ai.Config.debug > 0) {
            API3.warn(
              "Petra garrison error: unit " +
                ent.id() +
                " (" +
                ent.genericName() +
                ") is expected to garrison in " +
                id +
                " (" +
                holder.genericName() +
                "), but has no such garrison order " +
    ...
    if (!holder.position())
      // could happen with siege unit inside a ship
      continue;

    if (
      gameState.ai.elapsedTime -
        holder.getMetadata(PlayerID, "holderTimeUpdate") >
      3
    ) {
      let range = holder.attackRange("Ranged")
        ? holder.attackRange("Ranged").max
        : 80;
      let around = {
        defenseStructure: false,
        meleeSiege: false,
        rangeSiege: false,
        unit: false,
      };
      for (let ent of gameState.getEnemyEntities().values()) {
        if (ent.hasClass("Structure")) {
          if (!ent.attackRange("Ranged")) continue;
        } else if (ent.hasClass("Unit")) {
          if (
            ent.owner() == 0 &&
            (!ent.unitAIState() || ent.unitAIState().split(".")[1] != "COMBAT")
          )
            continue;
        } else continue;
      ...
      for (let entId of holder.garrisoned()) {
        ...
        if (
          ent.owner() === PlayerID &&
          !this.keepGarrisoned(ent, holder, around)
        )
          holder.unload(entId);
      }
      for (let j = 0; j < list.length; ++j) {
        let ent = gameState.getEntityById(list[j]);
        if (this.keepGarrisoned(ent, holder, around)) continue;
        if (ent.getMetadata(PlayerID, "garrisonHolder") == id) {
          this.leaveGarrison(ent);
          ent.stopMoving();
        }
      ...
      if (this.numberOfGarrisonedSlots(holder) === 0) this.holders.delete(id);
      else
        holder.setMetadata(
          PlayerID,
          "holderTimeUpdate",
  ...
  for (let [id, gmin] of this.decayingStructures.entries()) {
    ...
    if (!ent || ent.owner() !== PlayerID) this.decayingStructures.delete(id);
    else if (this.numberOfGarrisonedSlots(ent) < gmin)
      gameState.ai.HQ.defenseManager.garrisonUnitsInside(gameState, ent, {
        min: gmin,
        type: "decay",
      });
  }
...
PETRA.GarrisonManager.prototype.numberOfGarrisonedUnits = function (holder) {
  if (!this.holders.has(holder.id())) return holder.garrisoned().length;
...
PETRA.GarrisonManager.prototype.numberOfGarrisonedSlots = function (holder) {
  if (!this.holders.has(holder.id())) return holder.garrisonedSlots();
...
PETRA.GarrisonManager.prototype.allowMelee = function (holder) {
  if (!this.holders.has(holder.id())) return undefined;
...
PETRA.GarrisonManager.prototype.garrison = function (
  gameState,
  ent,
  holder,
  type
  ...
  if (
    this.numberOfGarrisonedSlots(holder) >= holder.garrisonMax() ||
    !ent.canGarrison()
  )
    return;
  ...
  if (gameState.ai.Config.debug > 2) {
    ...
    warn(
      " we try to garrison a unit with plan " +
...
PETRA.GarrisonManager.prototype.cancelGarrison = function (ent) {
  ...
  if (!holderId || !this.holders.has(holderId)) return;
  let list = this.holders.get(holderId).list;
  let index = list.indexOf(ent.id());
  if (index !== -1) list.splice(index, 1);
};

PETRA.GarrisonManager.prototype.keepGarrisoned = function (
  ent,
  holder,
  around
) {
  switch (ent.getMetadata(PlayerID, "garrisonType")) {
    ...
    case "protection": // hurt unit for healing or infantry for defense
      ...
      if (ent.hasClasses(holder.getGarrisonArrowClasses())) {
        if (around.unit || around.defenseStructure) return true;
      ...
      if (ent.attackTypes() && ent.attackTypes().indexOf("Melee") !== -1)
        return false;
      if (around.unit) return ent.hasClass("Support") || PETRA.isSiegeUnit(ent); // only ranged siege here and below as melee siege already released above
    ...
    case "emergency": // f.e. hero in regicide mode
      ...
      return holder.buffHeal() && ent.needsHeal();
    default:
      if (ent.getMetadata(PlayerID, "onBoard") === "onBoard")
        // transport is not (yet ?) managed by garrisonManager
        return true;
      API3.warn(
        "unknown type in garrisonManager " +
          ent.getMetadata(PlayerID, "garrisonType") +
          " for " +
          ent.genericName() +
          " id " +
          ent.id() +
 ...
 */
PETRA.GarrisonManager.prototype.addDecayingStructure = function (
  gameState,
  entId,
  justCaptured
) {
  if (this.decayingStructures.has(entId)) return true;
  let ent = gameState.getEntityById(entId);
  if (
    !ent ||
    (!(ent.hasClass("Barracks") && justCaptured) && !ent.hasDefensiveFire())
  )
    return false;
  if (!ent.territoryDecayRate() || !ent.garrisonRegenRate()) return false;
  let gmin = Math.ceil(
    (ent.territoryDecayRate() - ent.defaultRegenRate()) /
      ent.garrisonRegenRate()
  );
...
PETRA.GarrisonManager.prototype.removeDecayingStructure = function (entId) {
  if (!this.decayingStructures.has(entId)) return;

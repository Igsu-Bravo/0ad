...
PETRA.DefenseArmy = function (gameState, foeEntities, type) {
  ...
  // actually add units
  for (let id of foeEntities) this.addFoe(gameState, id, true);
...
PETRA.DefenseArmy.prototype.addFoe = function (gameState, enemyId, force) {
  if (this.foeEntities.indexOf(enemyId) !== -1) return false;
  ...
  if (
    !force &&
    API3.SquareVectorDistance(ent.position(), this.foePosition) >
      this.compactSize
  )
    return false;
 ...
 */
PETRA.DefenseArmy.prototype.removeFoe = function (
  gameState,
  enemyId,
  enemyEntity
) {
  ...
  for (let to in this.assignedTo)
    if (this.assignedTo[to] == enemyId) this.assignedTo[to] = undefined;

  let ent = enemyEntity ? enemyEntity : gameState.getEntityById(enemyId);
  if (ent) {
    // TODO recompute strength when no entities (could happen if capture+destroy)
...
PETRA.DefenseArmy.prototype.addOwn = function (gameState, id, force) {
  if (this.ownEntities.indexOf(id) !== -1) return false;
  let ent = gameState.getEntityById(id);
  if (!ent || (!ent.position() && !force)) return false;
  ...
  if (plan !== undefined) ent.setMetadata(PlayerID, "plan", -2);
  else ent.setMetadata(PlayerID, "plan", -3);
...
PETRA.DefenseArmy.prototype.removeOwn = function (gameState, id, Entity) {
  ...
  if (this.assignedTo[id] !== 0) {
    ...
    if (temp) temp.splice(temp.indexOf(id), 1);
  }
  ...
  else ent.setMetadata(PlayerID, "plan", undefined);
  ...
  else ent.setMetadata(PlayerID, "subrole", undefined);
...
PETRA.DefenseArmy.prototype.clear = function (gameState) {
  ...
  let distmin;
  let radius = 0;
  if (nOwn > 0) destination = [posOwn[0] / nOwn, posOwn[1] / nOwn];
  else if (nAlly > 0) destination = [posAlly[0] / nAlly, posAlly[1] / nAlly];
  else {
    posOther[0] /= nOther;
    posOther[1] /= nOther;
    ...
    for (let struct of gameState.getAllyStructures().values()) {
      ...
      if (
        !pos ||
        !gameState.isPlayerMutualAlly(
          gameState.ai.HQ.territoryMap.getOwner(pos)
        )
      )
        continue;
      if (PETRA.getLandAccess(gameState, struct) !== armyAccess) continue;
      let defensiveStruct = struct.hasDefensiveFire();
      if (defensiveFound && !defensiveStruct) continue;
      let dist = API3.SquareVectorDistance(posOther, pos);
      if (distmin && dist > distmin && (defensiveFound || !defensiveStruct))
        continue;
      if (defensiveStruct) defensiveFound = true;
      distmin = dist;
      destination = pos;
  ...
  while (this.ownEntities.length > 0) {
    ...
    if (ent) {
      ...
      if (
        destination &&
        !gameState.isPlayerMutualAlly(
          gameState.ai.HQ.territoryMap.getOwner(ent.position())
        )
      )
...
PETRA.DefenseArmy.prototype.assignUnit = function (gameState, entID) {
  ...
  for (let id of this.foeEntities) {
    ...
    if (
      eEnt.hasClass("Unit") &&
      eEnt.unitAIOrderData() &&
      eEnt.unitAIOrderData().length &&
      eEnt.unitAIOrderData()[0].target &&
    ...
    if (
      this.assignedAgainst[id].length > 8 ||
      (this.assignedAgainst[id].length > 5 &&
        !eEnt.hasClass("Hero") &&
        !PETRA.isSiegeUnit(eEnt))
    )
      continue;
    ...
    if (this.assignedAgainst[id].length > 2) continue;
  ...
  let idFoe;
  if (idMin !== undefined) idFoe = idMin;
  else if (idMinAll !== undefined) idFoe = idMinAll;
  else return false;
...
PETRA.DefenseArmy.prototype.getState = function () {
  if (!this.foeEntities.length) return 0;
  return 1;
};
...
PETRA.DefenseArmy.prototype.merge = function (gameState, otherArmy) {
  ...
  for (let i in otherArmy.assignedAgainst) {
    if (this.assignedAgainst[i] === undefined)
      ...
      this.assignedAgainst[i] = this.assignedAgainst[i].concat(
        otherArmy.assignedAgainst[i]
      );
  }
  for (let i in otherArmy.assignedTo)
    this.assignedTo[i] = otherArmy.assignedTo[i];

  for (let id of otherArmy.foeEntities) this.addFoe(gameState, id, true);
  // TODO: reassign those ?
  for (let id of otherArmy.ownEntities) this.addOwn(gameState, id, true);
...
PETRA.DefenseArmy.prototype.needsDefenders = function (gameState) {
  ...
  if (territoryOwner == PlayerID)
    ...
    for (let p = 1; p < gameState.sharedScript.playersData.length; ++p)
      if (
        p != territoryOwner &&
        gameState.sharedScript.playersData[p].isAlly[territoryOwner]
      )
        ++numExclusiveAllies;
    defenseRatio /= 1 + 0.5 * Math.max(0, numExclusiveAllies - 1);
  } else defenseRatio = this.Config.Defense.defenseRatio.neutral;
  ...
  if (this.foeStrength * defenseRatio <= this.ownStrength) return false;
...
PETRA.DefenseArmy.prototype.recalculatePosition = function (gameState, force) {
  if (!force && this.positionLastUpdate === gameState.ai.elapsedTime) return;
...
PETRA.DefenseArmy.prototype.evaluateStrength = function (ent, isOwn, remove) {
  ...
  if (ent.hasClass("Structure")) {
    ...
    // small strength used only when we try to recover capture points
    else entStrength = 2;
  } else
    entStrength = PETRA.getMaxStrength(
      ent,
      this.Config.debug,
  ...
  if (ent.hasClasses(["Animal+Elephant"])) entStrength *= 3;

  if (remove) entStrength *= -1;

  if (isOwn) this.ownStrength += entStrength;
  else this.foeStrength += entStrength;
};
  ...
  for (let evt of events.EntityRenamed) {
    ...
    if (this.foeEntities.indexOf(evt.entity) !== -1) {
      ...
      for (let against in this.assignedAgainst) {
        if (!this.assignedAgainst[against]) continue;
        if (this.assignedAgainst[against].indexOf(evt.entity) !== -1)
          this.assignedAgainst[against][
            this.assignedAgainst[against].indexOf(evt.entity)
          ] = evt.newentity;
      }
    }
  }
...
PETRA.DefenseArmy.prototype.update = function (gameState) {
  for (let entId of this.ownEntities) {
    ...
    if (!orderData.length && !ent.getMetadata(PlayerID, "transport"))
      ...
      orderData.length &&
      orderData[0].target &&
      orderData[0].attackType &&
      orderData[0].attackType === "Capture"
    ) {
  ...
  if (this.type == "capturing") {
    if (
      this.foeEntities.length &&
      gameState.getEntityById(this.foeEntities[0])
    ) {
      ...
      let capture = gameState
        .getEntityById(this.foeEntities[0])
        .capturePoints();
      if (capture)
        for (let j = 0; j < capture.length; ++j)
          if (gameState.isPlayerEnemy(j) && capture[j] > 0) return [];
      this.removeFoe(gameState, this.foeEntities[0]);
    }
    return [];
  }
  ...
  if (gameState.ai.elapsedTime - this.positionLastUpdate > 5) {
    ...
    for (let i = 0; i < this.foeEntities.length; ++i) {
      ...
      if (
        ...
        breakaways.push(id);
        if (this.removeFoe(gameState, id)) i--;
      }
    }

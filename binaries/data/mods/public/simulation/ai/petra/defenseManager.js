...
PETRA.DefenseManager.prototype.update = function (gameState, events) {
  ...
  for (let enemy in this.attackingUnits) {
    if (!this.attackingUnits[enemy]) continue;
    for (let ally in this.attackingUnits[enemy]) {
      if (this.attackingUnits[enemy][ally] < 8) continue;
      if (attackingArmies[enemy] === undefined) attackingArmies[enemy] = {};
...
PETRA.DefenseManager.prototype.makeIntoArmy = function (
  gameState,
  entityID,
  type = "default"
) {
  if (type == "default") {
    ...
    for (let army of this.armies)
      if (army.getType() == type && army.addFoe(gameState, entityID)) return; // over
  }
...
PETRA.DefenseManager.prototype.getArmy = function (partOfArmy) {
  ...
  for (let army of this.armies) if (army.ID == partOfArmy) return army;
...
PETRA.DefenseManager.prototype.isDangerous = function (gameState, entity) {
  ...
  if (
    entity.unitAIState() &&
    entity.unitAIState() == "INDIVIDUAL.REPAIR.REPAIRING"
  ) {
    let targetId = entity.unitAIOrderData()[0].target;
    if (this.targetList.indexOf(targetId) != -1) return true;
    let target = gameState.getEntityById(targetId);
    if (target) {
      let isTargetEnemy = gameState.isPlayerEnemy(target.owner());
      if (isTargetEnemy && territoryOwner == PlayerID) {
        if (target.hasClass("Structure")) this.targetList.push(targetId);
        return true;
        ...
        for (let building of myBuildings.values()) {
          if (building.foundationProgress() == 0) continue;
          if (
            API3.SquareVectorDistance(building.position(), entity.position()) >
            30000
          )
            continue;
  ...
  if (entity.attackTypes().indexOf("Ranged") != -1)
    dist2Min =
      (entity.attackRange("Ranged").max + 30) *
      (entity.attackRange("Ranged").max + 30);
  ...
  for (let targetId of this.targetList) {
    ...
    if (
      API3.SquareVectorDistance(target.position(), entity.position()) < dist2Min
    )
      return true;
  }
  ...
  for (let building of gameState.getOwnStructures().values()) {
    if (
      ...
      continue;
    if (
      !this.territoryMap.isBlinking(building.position()) ||
      gameState.ai.HQ.isDefendable(building)
    )
      return true;
  }
  ...
  if (gameState.isPlayerMutualAlly(territoryOwner)) {
    ...
    if (
      ...
      this.attackedAllies[territoryOwner] &&
      this.attackedAllies[territoryOwner] > 1 &&
      this.GetCooperationLevel(territoryOwner) > 0.7
    ) {
      for (let building of gameState
        .getAllyStructures(territoryOwner)
        .values()) {
        if (
          building.foundationProgress() == 0 ||
          API3.SquareVectorDistance(building.position(), entity.position()) >
          ...
          continue;
        if (!this.territoryMap.isBlinking(building.position())) return true;
      }
    }
...
PETRA.DefenseManager.prototype.checkEnemyUnits = function (gameState) {
  ...
  if (i == PlayerID) {
    if (!this.armies.length) {
      ...
      for (let ent of gameState.getOwnStructures().values()) {
        if (ent.decaying()) continue;
        let capture = ent.capturePoints();
        if (capture === undefined) continue;
        let lost = 0;
        for (let j = 0; j < capture.length; ++j)
          if (gameState.isPlayerEnemy(j)) lost += capture[j];
        if (lost < Math.ceil(0.25 * capture[i])) continue;
        this.makeIntoArmy(gameState, ent.id(), "capturing");
        break;
      }
    }
    return;
  } else if (!gameState.isPlayerEnemy(i)) return;

  for (let ent of gameState.getEnemyUnits(i).values()) {
    if (ent.getMetadata(PlayerID, "PartOfArmy") !== undefined) continue;
    ...
    if (ent.hasClass("Animal")) {
      ...
      if (!orders || !orders.length || !orders[0].target) continue;
      let target = gameState.getEntityById(orders[0].target);
      if (!target || !gameState.isPlayerAlly(target.owner())) continue;
    }

    // TODO what to do for ships ?
    if (ent.hasClasses(["Ship", "Trader"])) continue;
  ...
  for (let ent of gameState.getEnemyStructures(i).values()) {
    if (
      !ent.position() ||
      ent.getMetadata(PlayerID, "PartOfArmy") !== undefined
    )
      continue;
    if (!ent.capturePoints() && !ent.hasDefensiveFire()) continue;
    let owner = this.territoryMap.getOwner(ent.position());
    if (owner == PlayerID) this.makeIntoArmy(gameState, ent.id(), "capturing");
  }
};
...
PETRA.DefenseManager.prototype.checkEnemyArmies = function (gameState) {
  for (let i = 0; i < this.armies.length; ++i) {
    ...
    // Assume dangerosity.
    for (let breaker of breakaways) this.makeIntoArmy(gameState, breaker);

    if (army.getState() == 0) {
      if (army.getType() == "default") this.switchToAttack(gameState, army);
  ...
  for (let i = 0; i < this.armies.length - 1; ++i) {
    let army = this.armies[i];
    if (army.getType() != "default") continue;
    for (let j = i + 1; j < this.armies.length; ++j) {
      let otherArmy = this.armies[j];
      if (
        otherArmy.getType() != "default" ||
        API3.SquareVectorDistance(army.foePosition, otherArmy.foePosition) >
          this.armyMergeSize
      )
        continue;
  ...
  if (gameState.ai.playedTurn % 5 != 0) return;
  ...
  for (let i = 0; i < this.armies.length; ++i) {
    ...
    let bases = gameState.updatingGlobalCollection(
      "allCCs",
      API3.Filters.byClass("CivCentre")
    );
    for (let base of bases.values()) {
      if (!gameState.isEntityAlly(base)) continue;
      let cooperation = this.GetCooperationLevel(base.owner());
      if (cooperation < 0.3 && !gameState.isEntityOwn(base)) continue;
    ...
    for (let dock of gameState
      .getOwnStructures()
      .filter(API3.Filters.byClass("Dock"))
      .values()) {
...
PETRA.DefenseManager.prototype.assignDefenders = function (gameState) {
  if (!this.armies.length) return;

  let armiesNeeding = [];
  // Let's add defenders.
  for (let army of this.armies) {
    let needsDef = army.needsDefenders(gameState);
    if (needsDef === false) continue;
  ...
  if (!armiesNeeding.length) return;
  ...
  gameState.getOwnUnits().forEach(function (ent) {
    ...
    if (
      ...
      return;
    if (ent.hasClass("Support") || ent.attackTypes() === undefined) return;
    if (ent.hasClasses(["StoneThrower", "Support", "FishingBoat"])) return;
    if (
      ent.getMetadata(PlayerID, "transport") !== undefined ||
      ent.getMetadata(PlayerID, "transporter") !== undefined
    )
      return;
    if (gameState.ai.HQ.victoryManager.criticalEnts.has(ent.id())) return;
    if (
  ...
  for (let ipass = 0; ipass < 2; ++ipass) {
    ...
    for (let i = 0; i < potentialDefenders.length; ++i) {
      ...
      for (let a = 0; a < armiesNeeding.length; ++a) {
        if (access && armiesNeeding[a].access != access) continue;

        // Do not assign defender if it cannot attack at least part of the attacking army.
        if (
          !armiesNeeding[a].army.foeEntities.some((eEnt) => {
            let eEntID = gameState.getEntityById(eEnt);
            return ent.canAttackTarget(
              eEntID,
              PETRA.allowCapture(gameState, ent, eEntID)
            );
          })
        )
          continue;
        ...
        if (aMin !== undefined && dist > distMin) continue;
        aMin = a;
        distMin = dist;
      }
      ...
      if (
        ...
        (aMin == undefined ||
          (distMin > 40000 &&
            this.territoryMap.getOwner(armiesNeeding[aMin].army.foePosition) !=
              PlayerID))
      ) {
        ++backup;
        potentialDefenders[i] = undefined;
        continue;
      } else if (aMin === undefined) continue;

      armiesNeeding[aMin].need -= PETRA.getMaxStrength(
        ent,
        this.Config.debug,
        this.Config.DamageTypeImportance
      );
      ...
      if (armiesNeeding[aMin].need <= 0) armiesNeeding.splice(aMin, 1);
      if (!armiesNeeding.length) return;
    }
  }
...
PETRA.DefenseManager.prototype.abortArmy = function (gameState, army) {
  army.clear(gameState);
  for (let i = 0; i < this.armies.length; ++i) {
    if (this.armies[i].ID != army.ID) continue;
    this.armies.splice(i, 1);
    break;
  }
};
...
PETRA.DefenseManager.prototype.checkEvents = function (gameState, events) {
  ...
  for (let army of this.armies) army.checkEvents(gameState, events);
  ...
  for (let evt of events.OwnershipChanged) {
    if (gameState.isPlayerMutualAlly(evt.from) && evt.to > 0) {
      ...
      if (ent && ent.hasClass("CivCentre"))
        gameState.ai.HQ.attackManager.switchDefenseToAttack(gameState, ent, {
          range: 150,
        });
    }
  }
  ...
  let allAttacked = {};
  for (let evt of events.Attacked) allAttacked[evt.target] = evt.attacker;
  ...
  for (let evt of events.Attacked) {
    ...
    if (
      attacker &&
      gameState.isEntityOwn(attacker) &&
      gameState.isEntityEnemy(target) &&
      !attacker.hasClass("Ship") &&
      ...
      if (
        attacker.position() &&
        attacker.isGarrisonHolder() &&
        attacker.getArrowMultiplier() &&
        (target.owner() != 0 ||
    ...
    if (!gameState.isEntityOwn(target)) continue;

    // If attacked by one of our allies (he must trying to recover capture points), do not react.
    if (attacker && gameState.isEntityAlly(attacker)) continue;
    ...
    if (attacker && attacker.position() && target.hasClass("FishingBoat")) {
      ...
      if (target.isIdle() || unitAIStateOrder == "GATHER") {
        ...
        let range = attacker.attackRange("Ranged")
          ? attacker.attackRange("Ranged").max + 15
          : 25;
    ...
    if (target.hasClass("Ship")) continue;
    ...
    if (target.hasClass("Structure")) {
      let base = gameState.ai.HQ.getBaseByID(
        target.getMetadata(PlayerID, "base")
      );
      if (
        (this.territoryMap.isBlinking(target.position()) &&
          !gameState.ai.HQ.isDefendable(target)) ||
        !base ||
        gameState.ai.HQ.baseManagers.every(
          (b) => !b.anchor || b.accessIndex != base.accessIndex
        )
      ) {
        let capture = target.capturePoints();
        if (!capture) continue;
        let captureRatio = capture[PlayerID] / capture.reduce((a, b) => a + b);
        if (captureRatio > 0.5 && captureRatio < 0.7) target.destroy();
        continue;
      }
    }
    ...
    if (plan !== undefined && plan >= 0) {
      ...
      if (attack && attack.state != "unexecuted") continue;
    }
    ...
    if (
      attacker &&
      attacker.position() &&
      attacker.getMetadata(PlayerID, "PartOfArmy") === undefined &&
    ...
    if (target.getMetadata(PlayerID, "PartOfArmy") !== undefined) {
      ...
      if (army.getType() == "capturing") {
        ...
        if (army.ownEntities.indexOf(target.id()) != -1) abort = true;
        else if (
          army.foeEntities[0] == target.id() &&
          ...
          let capture = target.capturePoints();
          for (let j = 0; j < capture.length; ++j) {
            if (!gameState.isPlayerEnemy(j) || capture[j] == 0) continue;
            abort = false;
            break;
          }
        }
        if (abort) this.abortArmy(gameState, army);
      }
      continue;
    }
    ...
    if (
      target.hasClass("Support") &&
      target.healthLevel() < this.Config.garrisonHealthLevel.medium &&
      !target.getMetadata(PlayerID, "transport") &&
      plan != -2 &&
      plan != -3
    ) {
    ...
    if (!attacker || !attacker.position()) continue;
    ...
    if (target.hasClass("Unit") && attacker.hasClass("Unit")) {
      ...
      let currentTarget =
        orderData && orderData.length && orderData[0].target
          ? gameState.getEntityById(orderData[0].target)
          : undefined;
      if (currentTarget) {
        let unitAIState = target.unitAIState();
        let unitAIStateOrder = unitAIState ? unitAIState.split(".")[1] : "";
        if (
          unitAIStateOrder == "COMBAT" &&
          (currentTarget == attacker.id() ||
        ...
        if (
          ...
          if (minEnt) {
            ...
            minEnt.attack(
              attacker.id(),
              PETRA.allowCapture(gameState, minEnt, attacker)
            );
          }
        }
      }
...
PETRA.DefenseManager.prototype.garrisonUnitsInside = function (
  gameState,
  target,
  data
) {
  ...
  if (
    gameState.ai.HQ.garrisonManager.numberOfGarrisonedSlots(target) >=
    minGarrison
  )
    return false;
  if (data.attacker) {
    let attackTypes = target.attackTypes();
    if (!attackTypes || attackTypes.indexOf("Ranged") == -1) return false;
    let dist = API3.SquareVectorDistance(
  ...
  if (allowMelee === undefined) {
    ...
    if (data.attacker)
      allowMelee = data.attacker.hasClass("Structure")
        ? data.attacker.attackRange("Ranged")
        : !PETRA.isSiegeUnit(data.attacker);
  ...
  let units = gameState
    ...
    .filter((ent) => {
      ...
      if (!ent.hasClasses(garrisonArrowClasses)) return false;
      if (
        typeGarrison != "decay" &&
        !allowMelee &&
        ent.attackTypes().indexOf("Melee") != -1
      )
        return false;
      if (ent.getMetadata(PlayerID, "transport") !== undefined) return false;
      let army = ent.getMetadata(PlayerID, "PartOfArmy")
        ? this.getArmy(ent.getMetadata(PlayerID, "PartOfArmy"))
        : undefined;
      if (
        !army &&
        (ent.getMetadata(PlayerID, "plan") == -2 ||
      ...
      if (
        ent.getMetadata(PlayerID, "plan") !== undefined &&
        ent.getMetadata(PlayerID, "plan") >= 0
      ) {
        let subrole = ent.getMetadata(PlayerID, "subrole");
      ...
      if (PETRA.getLandAccess(gameState, ent) != access) return false;
      return true;
    })
  ...
  for (let ent of units.values()) {
    if (garrisonManager.numberOfGarrisonedSlots(target) >= minGarrison) break;
    if (
      ent.getMetadata(PlayerID, "plan") !== undefined &&
      ent.getMetadata(PlayerID, "plan") >= 0
    ) {
      let attackPlan = gameState.ai.HQ.attackManager.getPlan(
        ent.getMetadata(PlayerID, "plan")
      );
      if (attackPlan) attackPlan.removeUnit(ent, true);
    }
    let army = ent.getMetadata(PlayerID, "PartOfArmy")
      ? this.getArmy(ent.getMetadata(PlayerID, "PartOfArmy"))
      : undefined;
    if (army) army.removeOwn(gameState, ent.id());
...
PETRA.DefenseManager.prototype.garrisonSiegeUnit = function (gameState, unit) {
  ...
  for (let ent of gameState.getAllyStructures().values()) {
    if (!ent.isGarrisonHolder()) continue;
    if (!unit.hasClasses(ent.garrisonableClasses())) continue;
    if (garrisonManager.numberOfGarrisonedSlots(ent) >= ent.garrisonMax())
      continue;
    if (ent.hitpoints() < ent.garrisonEjectHealth() * ent.maxHitpoints())
      continue;
    if (PETRA.getLandAccess(gameState, ent) != unitAccess) continue;
    let dist = API3.SquareVectorDistance(ent.position(), unit.position());
    if (dist > distmin) continue;
    distmin = dist;
    nearest = ent;
  }
  if (nearest) garrisonManager.garrison(gameState, unit, nearest, "protection");
 ...
 */
PETRA.DefenseManager.prototype.garrisonAttackedUnit = function (
  gameState,
  unit,
  emergency = false
) {
  ...
  for (let ent of gameState.getAllyStructures().values()) {
    if (!ent.isGarrisonHolder()) continue;
    if (!emergency && !ent.buffHeal()) continue;
    if (!unit.hasClasses(ent.garrisonableClasses())) continue;
    if (
      garrisonManager.numberOfGarrisonedSlots(ent) >= ent.garrisonMax() &&
  ...
  if (garrisonManager.numberOfGarrisonedSlots(nearest) >= nearest.garrisonMax())
    // make room for this ent
    nearest.unload(nearest.garrisoned()[0]);

  garrisonManager.garrison(
    gameState,
    unit,
    nearest,
    nearest.buffHeal() ? "protection" : "emergency"
  );
  return true;
};
  ...
  for (let targetId of this.targetList) {
    ...
    for (let entId of army.ownEntities) {
      ...
      if (
        !ent ||
        !ent.position() ||
        PETRA.getLandAccess(gameState, ent) != targetAccess
      )
        continue;
      if (API3.SquareVectorDistance(targetPos, ent.position()) > 14400)
        continue;
      gameState.ai.HQ.attackManager.switchDefenseToAttack(gameState, target, {
        armyID: army.ID,
        uniqueTarget: true,
      });
      return;
...
PETRA.DefenseManager.prototype.Serialize = function () {
  ...
  let armies = [];
  for (let army of this.armies) armies.push(army.Serialize());

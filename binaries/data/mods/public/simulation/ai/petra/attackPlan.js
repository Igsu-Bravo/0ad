...
PETRA.AttackPlan = function (gameState, Config, uniqueID, type, data) {
  ...
  for (let base of gameState.ai.HQ.baseManagers) {
    ...
    if (!allAccesses[access]) allAccesses[access] = base.anchor.position();
  }
  if (!rallyPoint) {
  ...
  if (gameState.ai.HQ.navalMap) {
    for (let structure of gameState.getEnemyStructures().values()) {
      if (this.target && structure.id() != this.target.id()) continue;
      ...
      if (access in allAccesses) {
        ...
        let sea = gameState.ai.HQ.getSeaBetweenIndices(
          gameState,
          rallyAccess,
          access
        );
        ...
        this.overseas = sea;
        gameState.ai.HQ.navalManager.setMinimalTransportShips(
          gameState,
          sea,
          1
        );
  ...
  if (type == "Rush") {
    ...
    this.unitStat.Infantry = {
      ...
      interests: [
        ...
        ["costsResource", 0.6, "metal"],
      ],
    };
    this.unitStat.FastMoving = {
      priority: 1,
      minSize: 2,
      targetSize: 4,
      batchSize: 2,
      classes: ["FastMoving+CitizenSoldier"],
    ...
    this.unitStat.RangedInfantry = {
      priority: 0.7,
      minSize: 5,
      targetSize: 20,
      batchSize: 5,
      classes: ["Infantry+Ranged+CitizenSoldier"],
      interests: [["strength", 3]],
    };
    this.unitStat.MeleeInfantry = {
      priority: 0.7,
      minSize: 5,
      targetSize: 20,
      batchSize: 5,
      classes: ["Infantry+Melee+CitizenSoldier"],
      interests: [["strength", 3]],
    };
    this.unitStat.ChampRangedInfantry = {
      priority: 1,
      minSize: 3,
      targetSize: 18,
      batchSize: 3,
      classes: ["Infantry+Ranged+Champion"],
      interests: [["strength", 3]],
    };
    this.unitStat.ChampMeleeInfantry = {
      priority: 1,
      minSize: 3,
      targetSize: 18,
      batchSize: 3,
      classes: ["Infantry+Melee+Champion"],
      interests: [["strength", 3]],
    };
    this.unitStat.RangedFastMoving = {
      priority: 0.7,
      minSize: 4,
      targetSize: 20,
      batchSize: 4,
      classes: ["FastMoving+Ranged+CitizenSoldier"],
      interests: [["strength", 2]],
    };
    this.unitStat.MeleeFastMoving = {
      priority: 0.7,
      minSize: 4,
      targetSize: 20,
      batchSize: 4,
      classes: ["FastMoving+Melee+CitizenSoldier"],
      interests: [["strength", 2]],
    };
    this.unitStat.ChampRangedFastMoving = {
      priority: 1,
      minSize: 3,
      targetSize: 15,
      batchSize: 3,
      classes: ["FastMoving+Ranged+Champion"],
      interests: [["strength", 3]],
    };
    this.unitStat.ChampMeleeFastMoving = {
      priority: 1,
      minSize: 3,
      targetSize: 15,
      batchSize: 3,
      classes: ["FastMoving+Melee+Champion"],
      interests: [["strength", 2]],
    };
    this.unitStat.Hero = {
      priority: 1,
    ...
    this.unitStat.RangedInfantry = {
      ...
      targetSize: 16,
      batchSize: 3,
      classes: ["Infantry+Ranged"],
      interests: [
        ["canGather", 1],
        ["strength", 1.6],
        ["costsResource", 0.3, "stone"],
        ["costsResource", 0.3, "metal"],
      ],
    };
    this.unitStat.MeleeInfantry = {
      priority: 1,
  ...
  if (this.Config.difficulty < 2) {
    for (const cat in this.unitStat) {
      this.unitStat[cat].targetSize = Math.ceil(
        variation * this.unitStat[cat].targetSize
      );
      this.unitStat[cat].minSize = Math.min(
        this.unitStat[cat].targetSize,
        Math.min(this.unitStat[cat].minSize, 2)
      );
  ...
  for (let cat in this.unitStat) {
    this.unitStat[cat].targetSize = Math.ceil(
      this.Config.popScaling * this.unitStat[cat].targetSize
    );
    this.unitStat[cat].minSize = Math.ceil(
      this.Config.popScaling * this.unitStat[cat].minSize
    );
  }
  ...
  gameState.ai.queueManager.addQueue(
    "plan_" + this.name + "_champ",
    priority + 1
  );
...
PETRA.AttackPlan.prototype.init = function (gameState) {
  ...
  this.unitCollection = gameState
    .getOwnUnits()
    .filter(API3.Filters.byMetadata(PlayerID, "plan", this.name));
  ...
  for (let cat in this.unitStat) {
    ...
    this.unit[cat] = this.unitCollection.filter(
      API3.Filters.byClasses(Unit.classes)
    );
...
PETRA.AttackPlan.prototype.canStart = function () {
  if (!this.canBuildUnits) return true;
...
PETRA.AttackPlan.prototype.mustStart = function () {
  ...
  if (!this.canBuildUnits) return this.unitCollection.hasEntities();
  ...
  if (MaxReachedEverywhere) return true;
...
PETRA.AttackPlan.prototype.addBuildOrder = function (
  gameState,
  name,
  unitStats,
  resetQueue
...
PETRA.AttackPlan.prototype.addSiegeUnits = function (gameState) {
  ...
  for (let ent of gameState.getOwnTrainingFacilities().values()) {
    ...
    for (let trainable of trainables) {
      ...
      if (!template || !template.available(gameState)) continue;
      for (let i = 0; i < classes.length; ++i)
        if (template.hasClasses(classes[i])) hasTrainer[i] = true;
    }
  }
  if (hasTrainer.every((e) => !e)) return false;
  ...
  if (this.Config.difficulty < 3)
    targetSize =
      this.type == "HugeAttack"
        ? Math.max(this.Config.difficulty, 1)
        : Math.max(this.Config.difficulty - 1, 0);
  else
    targetSize =
      this.type == "HugeAttack"
        ...
        : this.Config.difficulty - 1;
  targetSize = Math.max(
    Math.round(this.Config.popScaling * targetSize),
    this.type == "HugeAttack" ? 1 : 0
  );
  ...
  let stat = {
    ...
    batchSize: Math.min(targetSize, 2),
    classes: classes[i],
...
PETRA.AttackPlan.prototype.updatePreparation = function (gameState) {
  ...
  if (this.state == "completing") {
    ...
    if (
      ...
      if (this.waitingForTransport()) return 1;
      // bloqued units which cannot finish their order should not stop the attack
      if (
        gameState.ai.elapsedTime < this.maxCompletingTime &&
        this.hasForceOrder()
      )
        return 1;
      return 2;
  ...
  if (
    this.overseas &&
    !gameState.ai.HQ.navalManager.seaTransportShips[this.overseas].length
  )
    return 1;

  if (this.type != "Raid" || !this.forced)
    // Forced Raids have special purposes (as relic capture)
    this.assignUnits(gameState);
  if (
    this.type != "Raid" &&
    gameState.ai.HQ.attackManager.getAttackInPreparation("Raid") !== undefined
  )
  ...
  if (gameState.ai.playedTurn % 5 == 0 && this.hasSiegeUnits()) {
    ...
    for (let i = 1; i < gameState.sharedScript.playersData.length; ++i) {
      if (
        !gameState.isPlayerEnemy(i) ||
        gameState.ai.HQ.attackManager.defeated[i]
      )
        continue;
      hasEnemies = true;
  ...
  if (gameState.getPopulationMax() - gameState.getPopulation() < 5) {
    ...
    if (this.canStart() || this.unitCollection.length > lengthMin) {
      this.emptyQueues();
    } // Abort the plan so that its units will be reassigned to other plans.
    else {
      if (this.Config.debug > 1) {
        let am = gameState.ai.HQ.attackManager;
        API3.warn(
          " attacks upcoming: raid " +
            am.upcomingAttacks.Raid.length +
            " rush " +
            am.upcomingAttacks.Rush.length +
            " attack " +
            am.upcomingAttacks.Attack.length +
            " huge " +
            am.upcomingAttacks.HugeAttack.length
        );
        API3.warn(
          " attacks started: raid " +
            am.startedAttacks.Raid.length +
            " rush " +
            am.startedAttacks.Rush.length +
            " attack " +
            am.startedAttacks.Attack.length +
            " huge " +
            am.startedAttacks.HugeAttack.length
        );
      }
      return 0;
    }
    ...
    if (this.canBuildUnits) {
      ...
      if (
        this.siegeState == 0 ||
        (this.siegeState == 1 && gameState.ai.playedTurn % 5 == 0)
      )
      ...
      if (!this.buildOrders.length) return 0; // will abort the plan
    }
    return 1;
  }
  ...
  if (!this.target && !this.chooseTarget(gameState)) return 0;
  if (!this.overseas) this.getPathToTarget(gameState);
  ...
  if (this.type == "Raid")
    ...
    else this.maxCompletingTime = gameState.ai.elapsedTime + 60;
  ...
  for (let unitIds of gameState.ai.HQ.attackManager.bombingAttacks.values()) {
    for (let entId of unitIds.values()) {
      ...
      if (!ent || ent.getMetadata(PlayerID, "plan") != this.name) continue;
  ...
  for (let ent of this.unitCollection.values()) {
    ...
    if (index == rallyIndex)
      ...
      gameState.ai.HQ.navalManager.requireTransport(
        gameState,
        ent,
        index,
        rallyIndex,
...
PETRA.AttackPlan.prototype.trainMoreUnits = function (gameState) {
  ...
  for (let order of this.buildOrders) {
    ...
    let aQueued = gameState.countOwnQueuedEntitiesWithMetadata(
      "special",
      special
    );
  ...
  this.buildOrders.sort((a, b) => {
    ...
    if (a[0] >= a[3].targetSize) va += 1000;
    let vb = b[0] / b[3].targetSize - b[3].priority;
    if (b[0] >= b[3].targetSize) vb += 1000;
    return va - vb;
  });
  ...
  if (this.Config.debug > 1 && gameState.ai.playedTurn % 50 === 0) {
    ...
    for (let order of this.buildOrders) {
      ...
      let inTraining = gameState.countOwnQueuedEntitiesWithMetadata(
        "special",
        specialData
      );
      let queue1 = this.queue.countQueuedUnitsWithMetadata(
        "special",
        specialData
      );
      let queue2 = this.queueChamp.countQueuedUnitsWithMetadata(
        "special",
        specialData
      );
      let queue3 = this.queueSiege.countQueuedUnitsWithMetadata(
        "special",
        specialData
      );
      API3.warn(
  ...
  if (firstOrder[0] < firstOrder[3].targetSize) {
    ...
    if (
      MatchesClassList(firstOrder[3].classes, [
        "Siege",
        "Elephant+Melee+Champion",
      ])
    )
    ...
    if (queue.length() <= 5) {
      let template = gameState.ai.HQ.findBestTrainableUnit(
        gameState,
        firstOrder[1],
        firstOrder[3].interests
      );
      ...
      if (template === undefined) {
        ...
        if (this.Config.debug > 2)
          API3.warn(
            "attack template " + template + " added for plan " + this.name
          );
        ...
        data.role = gameState.getTemplate(template).hasClass("CitizenSoldier")
          ? "worker"
          : "attack";
        let trainingPlan = new PETRA.TrainingPlan(
          gameState,
          template,
          data,
          max,
        ...
        if (trainingPlan.template) queue.addPlan(trainingPlan);
        else if (this.Config.debug > 1)
          API3.warn(
            "training plan canceled because no template for " +
              template +
              "   build1 " +
              uneval(firstOrder[1]) +
              " build3 " +
              uneval(firstOrder[3].interests)
          );
      }
    }
  }
...
PETRA.AttackPlan.prototype.assignUnits = function (gameState) {
  ...
  if (!this.canBuildUnits) {
    for (let ent of gameState.getOwnUnits().values()) {
      if (
        ent.getMetadata(PlayerID, "allied") ||
  ...
  for (let ent of gameState.getOwnEntitiesByRole(undefined, true).values()) {
    if (
      ent.hasClasses(["!Unit", "Ship", "Support"]) ||
      !this.isAvailableUnit(gameState, ent) ||
  ...
  for (let ent of gameState.ai.HQ.attackManager.outOfPlan.values()) {
    if (!this.isAvailableUnit(gameState, ent)) continue;
    ent.setMetadata(PlayerID, "plan", plan);
    this.unitCollection.updateEnt(ent);
    added = true;
  }
  ...
  if (this.Config.difficulty <= 2) return added;
...
PETRA.AttackPlan.prototype.isAvailableUnit = function (gameState, ent) {
  ...
  if (
    (ent.getMetadata(PlayerID, "plan") !== undefined &&
      ent.getMetadata(PlayerID, "plan") !== -1) ||
    ent.getMetadata(PlayerID, "transport") !== undefined ||
    ent.getMetadata(PlayerID, "transporter") !== undefined
  )
    return false;
  if (
    gameState.ai.HQ.victoryManager.criticalEnts.has(ent.id()) &&
    (this.overseas || ent.healthLevel() < 0.8)
  )
    return false;
  return true;
};
...
PETRA.AttackPlan.prototype.reassignFastUnit = function (gameState) {
  for (let ent of this.unitCollection.values()) {
    if (!ent.position() || ent.getMetadata(PlayerID, "transport") !== undefined)
      continue;
    if (!ent.hasClasses(["FastMoving", "CitizenSoldier"])) continue;
...
PETRA.AttackPlan.prototype.chooseTarget = function (gameState) {
  if (this.targetPlayer === undefined) {
    this.targetPlayer = gameState.ai.HQ.attackManager.getEnemyPlayer(
      gameState,
      this
    );
    if (this.targetPlayer === undefined) return false;
  }

  this.target = this.getNearestTarget(gameState, this.rallyPoint);
  if (!this.target) {
    if (this.uniqueTargetId) return false;
  ...
  if (targetIndex != rallyIndex) {
    ...
    for (let base of gameState.ai.HQ.baseManagers) {
      ...
      if (base.accessIndex == targetIndex) {
        if (dist >= distminSame) continue;
        distminSame = dist;
        rallySame = anchor.position();
      } else {
        dist /= Math.sqrt(
          gameState.ai.accessibility.regionSize[base.accessIndex]
        );
        if (dist >= distminDiff) continue;
        distminDiff = dist;
  ...
  } else if (this.overseas) this.overseas = 0;
 ...
 */
PETRA.AttackPlan.prototype.getNearestTarget = function (
  gameState,
  position,
  sameLand
) {
  ...
  if (this.uniqueTargetId) {
    ...
    if (ent) targets.addEnt(ent);
  } else {
    if (this.type == "Raid") targets = this.raidTargetFinder(gameState);
    ...
    } else targets = this.defaultTargetFinder(gameState, this.targetPlayer);
  }
  if (!targets.hasEntities()) return undefined;
  ...
  for (let ent of targets.values()) {
    if (
      ...
      (!ent.hasClass("Relic") ||
        gameState.ai.HQ.victoryManager.targetedGaiaRelics.has(ent.id()))
    )
      continue;
  ...
  if (
    this.targetPlayer == 0 &&
    gameState.getVictoryConditions().has("capture_the_relic") &&
    target.hasClass("Relic")
  )
    gameState.ai.HQ.victoryManager.targetedGaiaRelics.set(target.id(), [
      this.name,
    ]);
 ...
 */
PETRA.AttackPlan.prototype.defaultTargetFinder = function (
  gameState,
  playerEnemy
) {
  ...
  if (gameState.getVictoryConditions().has("wonder"))
    for (let ent of gameState
      .getEnemyStructures(playerEnemy)
      .filter(API3.Filters.byClass("Wonder"))
      .values())
      targets.addEnt(ent);
  if (gameState.getVictoryConditions().has("regicide"))
    for (let ent of gameState
      .getEnemyUnits(playerEnemy)
      .filter(API3.Filters.byClass("Hero"))
      .values())
      targets.addEnt(ent);
  if (gameState.getVictoryConditions().has("capture_the_relic"))
    for (let ent of gameState
      .updatingGlobalCollection("allRelics", API3.Filters.byClass("Relic"))
      .filter((relic) => relic.owner() == playerEnemy)
      .values())
      targets.addEnt(ent);
  targets = targets.filter(this.isValidTarget, this);
  if (targets.hasEntities()) return targets;

  let validTargets = gameState
    .getEnemyStructures(playerEnemy)
    .filter(this.isValidTarget, this);
  ...
  if (!targets.hasEntities())
    targets = gameState
      .getEntities(playerEnemy)
      .filter(API3.Filters.byClass("ConquestCritical"))
...
PETRA.AttackPlan.prototype.isValidTarget = function (ent) {
  ...
  if (
    this.sameLand &&
    PETRA.getLandAccess(this.gameState, ent) != this.sameLand
  )
    return false;
  return (
    !ent.decaying() ||
    ent.getDefaultArrow() ||
    (ent.isGarrisonHolder() && ent.garrisoned().length)
  );
};

/** Rush target finder aims at isolated non-defended buildings */
PETRA.AttackPlan.prototype.rushTargetFinder = function (
  gameState,
  playerEnemy
) {
  let targets = new API3.EntityCollection(gameState.sharedScript);
  ...
  else buildings = gameState.getEnemyStructures().toEntityArray();
  if (!buildings.length) return targets;

  this.position = this.unitCollection.getCentrePosition();
  if (!this.position) this.position = this.rallyPoint;

  let target;
  let minDist = Math.min();
  for (let building of buildings) {
    if (building.owner() == 0) continue;
    if (building.hasDefensiveFire()) continue;
    if (!this.isValidTarget(building)) continue;
    let pos = building.position();
    let defended = false;
    for (let defense of buildings) {
      if (!defense.hasDefensiveFire()) continue;
      let dist = API3.SquareVectorDistance(pos, defense.position());
      if (dist < 6400) {
        // TODO check on defense range rather than this fixed 80*80
        defended = true;
        break;
      }
    }
  ...
  if (target) targets.addEnt(target);
...
PETRA.AttackPlan.prototype.raidTargetFinder = function (gameState) {
  ...
  for (let targetId of gameState.ai.HQ.defenseManager.targetList) {
    ...
    if (target && target.position()) targets.addEnt(target);
  }
  return targets;
};
 ...
 */
PETRA.AttackPlan.prototype.checkTargetObstruction = function (
  gameState,
  target,
  position
) {
  if (
    PETRA.getLandAccess(gameState, target) !=
    gameState.ai.accessibility.getAccessValue(position)
  )
    return target;
  ...
  for (let struct of gameState.getEnemyStructures().values()) {
    ...
    let x =
      pathPos[0] - structPos[0] + (1.8 * (targetPos[0] - pathPos[0])) / dist;
    let y =
      pathPos[1] - structPos[1] + (1.8 * (targetPos[1] - pathPos[1])) / dist;

    if (struct.get("Obstruction/Static")) {
      if (!struct.angle()) continue;
      ...
      depth = Math.max(
        depth,
        +struct.get("Obstruction/Obstructions/Left/@depth")
      );
      width += +struct.get("Obstruction/Obstructions/Right/@width");
      depth = Math.max(
        depth,
        +struct.get("Obstruction/Obstructions/Right/@depth")
      );
      let cosa = Math.cos(angle);
      let sina = Math.sin(angle);
      let u = x * cosa - y * sina;
      ...
      for (let i = 1; i < path.length; ++i) {
        let u1 =
          (path[i - 1].x - structPos[0]) * cosa -
          (path[i - 1].y - structPos[1]) * sina;
        let v1 =
          (path[i - 1].x - structPos[0]) * sina +
          (path[i - 1].y - structPos[1]) * cosa;
        let u2 =
          (path[i].x - structPos[0]) * cosa - (path[i].y - structPos[1]) * sina;
        let v2 =
          (path[i].x - structPos[0]) * sina + (path[i].y - structPos[1]) * cosa;
        if (v1 * v2 < 0) {
          let u0 = (u1 * v2 - u2 * v1) / (v2 - v1);
          if (Math.abs(u0) > doorHalfWidth) continue;
          blocker = struct;
          break;
        }
      }
...
PETRA.AttackPlan.prototype.getPathToTarget = function (
  gameState,
  fixedRallyPoint = false
) {
  ...
  if (startAccess != endAccess) return false;
  ...
  let path = Engine.ComputePath(
    startPos,
    endPos,
    gameState.getPassabilityClassMask("large")
  );
  this.path = [];
  this.path.push(this.targetPos);
  for (let p in path) this.path.push([path[p].x, path[p].y]);
  this.path.push(this.rallyPoint);
  this.path.reverse();
  // Change the rally point to something useful
  if (!fixedRallyPoint) this.setRallyPoint(gameState);
  Engine.ProfileStop();
...
PETRA.AttackPlan.prototype.setRallyPoint = function (gameState) {
  for (let i = 0; i < this.path.length; ++i) {
    ...
    if (i === 0) this.rallyPoint = this.path[0];
    else if (
      i > 1 &&
      gameState.ai.HQ.isDangerousLocation(gameState, this.path[i - 1], 20)
    ) {
...
PETRA.AttackPlan.prototype.StartAttack = function (gameState) {
  ...
  for (let ent of this.unitCollection.values()) {
    ...
    if (ent.getStance() != stance) ent.setStance(stance);
  }
  ...
  if (rallyAccess == targetAccess) {
    if (!this.path) this.getPathToTarget(gameState, true);
    if (!this.path || !this.path[0][0] || !this.path[0][1]) return false;
    this.overseas = 0;
    this.state = "walking";
    this.unitCollection.moveToRange(this.path[0][0], this.path[0][1], 0, 15);
  } else {
    this.overseas = gameState.ai.HQ.getSeaBetweenIndices(
      gameState,
      rallyAccess,
      targetAccess
    );
    if (!this.overseas) return false;
...
PETRA.AttackPlan.prototype.update = function (gameState, events) {
  if (!this.unitCollection.hasEntities()) return 0;
  ...
  if (this.state == "transporting") this.UpdateTransporting(gameState, events);
  ...
  if (this.state == "arrived") {
    ...
    if (this.type == "Rush") {
      // try to find a better target for rush
  ...
  if (this.state == "") {
    ...
    for (let evt of events.Attacked) {
      if (!this.unitCollection.hasEntId(evt.target)) continue;
      ...
      if (PETRA.isSiegeUnit(ourUnit)) {
        ...
        let collec = this.unitCollection
          .filter(API3.Filters.not(API3.Filters.byClass("Siege")))
          .filterNearest(ourUnit.position(), 5);
        for (let ent of collec.values()) {
          if (PETRA.isSiegeUnit(ent))
            // needed as mauryan elephants are not filtered out
            continue;

          let allowCapture = PETRA.allowCapture(gameState, ent, attacker);
          if (!ent.canAttackTarget(attacker, allowCapture)) continue;
        ...
        if (
          PETRA.isSiegeUnit(attacker) &&
          attacker.hasClass("Melee") &&
          ourUnit.hasClass("Melee") &&
          ourUnit.canAttackTarget(
            attacker,
            PETRA.allowCapture(gameState, ourUnit, attacker)
          )
        ) {
          ourUnit.attack(
            attacker.id(),
        ...
        if (
          this.isBlocked &&
          !ourUnit.hasClass("Ranged") &&
          attacker.hasClass("Ranged")
        ) {
          ...
          let collec = this.unitCollection
            .filter(API3.Filters.byClass("Melee"))
            .filterNearest(ourUnit.position(), 5);
          for (let ent of collec.values()) {
            let allowCapture = PETRA.allowCapture(gameState, ent, attacker);
            if (!ent.canAttackTarget(attacker, allowCapture)) continue;
          ...
          for (let ent of collec.values()) {
            let allowCapture = PETRA.allowCapture(gameState, ent, attacker);
            if (
              PETRA.isSiegeUnit(ent) ||
              !ent.canAttackTarget(attacker, allowCapture)
            )
              continue;
            let orderData = ent.unitAIOrderData();
            if (orderData && orderData.length && orderData[0].target) {
              if (orderData[0].target === attacker.id()) continue;
    ...
    for (let ent of this.unitCollection.values()) {
      ...
      if (!(targetId in unitTargets)) {
        ...
        else unitTargets[targetId] = -3;
      }
      ++unitTargets[targetId];
    }
    let veto = {};
    for (let target in unitTargets)
      if (unitTargets[target] > 0) veto[target] = true;
    ...
    if (this.type == "Rush")
      targetClassesUnit = {
        ...
        avoid: ["Palisade", "Wall", "Tower", "Fortress"],
        vetoEntities: veto,
      };
    else {
    ...
    if (this.target.hasDefensiveFire()) {
      targetClassesUnit.avoid = targetClassesUnit.avoid.concat(
        "House",
        "Storehouse",
        "Farmstead",
        "Field",
      ...
      targetClassesSiege.avoid = targetClassesSiege.avoid.concat(
        "House",
        "Storehouse",
        "Farmstead",
        "Field",
    ...
    if (
      this.unitCollUpdateArray === undefined ||
      !this.unitCollUpdateArray.length
    )
    ...
    let lgth =
      this.unitCollUpdateArray.length < 15 || this.startingAttack
        ? this.unitCollUpdateArray.length
        : 10;
    for (let check = 0; check < lgth; check++) {
      ...
      if (ent.isIdle()) needsUpdate = true;
        ...
        if (!target || gameState.isPlayerAlly(target.owner()))
          ...
          needsUpdate = true;
          --unitTargets[targetId];
        } else if (!target.hasClass("Structure")) maybeUpdate = true;
      } else if (targetId) {
        let target = gameState.getEntityById(targetId);
        if (!target || gameState.isPlayerAlly(target.owner()))
          needsUpdate = true;
          ...
          !ent.hasClass("FastMoving") &&
          !ent.hasClass("Ranged") &&
          target.hasClass("FemaleCitizen") &&
          target.unitAIState().split(".")[1] == "FLEEING"
        )
          maybeUpdate = true;
      }
      ...
      if (!needsUpdate) {
        ...
        let deltat =
          ent.unitAIState() === "INDIVIDUAL.COMBAT.APPROACHING" ? 10 : 5;
        let lastAttackPlanUpdateTime = ent.getMetadata(
          PlayerID,
          "lastAttackPlanUpdateTime"
        );
        if (
          lastAttackPlanUpdateTime &&
          time - lastAttackPlanUpdateTime < deltat
        )
          continue;
      }
      ent.setMetadata(PlayerID, "lastAttackPlanUpdateTime", time);
      ...
      else if (ent.hasClass("FastMoving")) range += 30;
      range *= range;
      let entAccess = PETRA.getLandAccess(gameState, ent);
      // Checking for gates if we're a siege unit.
      if (siegeUnit) {
        let mStruct = enemyStructures
          .filter((enemy) => {
            if (
              !enemy.position() ||
              !ent.canAttackTarget(
                enemy,
                PETRA.allowCapture(gameState, ent, enemy)
              )
            )
              return false;
            if (
              API3.SquareVectorDistance(enemy.position(), ent.position()) >
              range
            )
              return false;
            if (enemy.foundationProgress() == 0) return false;
        ...
        if (mStruct.length) {
          mStruct.sort((structa, structb) => {
            ...
            if (structa.hasClass("Gate") && ent.canAttackClass("Wall"))
              vala += 10000;
            else if (structa.hasDefensiveFire()) vala += 1000;
            else if (structa.hasClass("ConquestCritical")) vala += 200;
            let valb = structb.costSum();
            if (structb.hasClass("Gate") && ent.canAttackClass("Wall"))
              valb += 10000;
            else if (structb.hasDefensiveFire()) valb += 1000;
            else if (structb.hasClass("ConquestCritical")) valb += 200;
            return valb - vala;
          });
          if (mStruct[0].hasClass("Gate"))
            ent.attack(
              mStruct[0].id(),
              PETRA.allowCapture(gameState, ent, mStruct[0])
            );
          else {
            let rand = randIntExclusive(0, mStruct.length * 0.2);
            ent.attack(
              mStruct[rand].id(),
              PETRA.allowCapture(gameState, ent, mStruct[rand])
            );
          }
        } else {
        ...
        let mUnit = enemyUnits
          .filter((enemy) => {
            if (
              ...
              return false;
            if (enemy.hasClass("Animal")) return false;
            if (
              nearby &&
              enemy.hasClass("FemaleCitizen") &&
              enemy.unitAIState().split(".")[1] == "FLEEING"
            )
              return false;
            let dist = API3.SquareVectorDistance(
              enemy.position(),
              ent.position()
            );
            ...
            if (
              veto[enemy.id()] &&
              API3.SquareVectorDistance(this.targetPos, ent.position()) > 2500
            )
              return false;
        ...
        if (mUnit.length) {
          mUnit.sort((unitA, unitB) => {
            ...
            if (ent.counters(unitA)) vala += 100;
            let valb = unitB.hasClass("Support") ? 50 : 0;
            if (ent.counters(unitB)) valb += 100;
            ...
            if (veto[unitA.id()]) vala -= 20000;
            if (veto[unitB.id()]) valb -= 20000;
            return valb - vala;
          });
          let rand = randIntExclusive(0, mUnit.length * 0.1);
          ent.attack(
            mUnit[rand].id(),
            PETRA.allowCapture(gameState, ent, mUnit[rand])
          );
        }
        ...
        // cannot attack. See similar behaviour at #5741.
          ...
          if (
            maybeUpdate &&
            ent.unitAIState() === "INDIVIDUAL.COMBAT.APPROACHING"
          ) {
            // we may be blocked by walls, attack everything
          ...
          if (mStruct.length) {
            mStruct.sort((structa, structb) => {
              ...
              if (structa.hasClass("Gate") && ent.canAttackClass("Wall"))
                vala += 10000;
              else if (structa.hasClass("ConquestCritical")) vala += 100;
              let valb = structb.costSum();
              if (structb.hasClass("Gate") && ent.canAttackClass("Wall"))
                valb += 10000;
            ...
            if (mStruct[0].hasClass("Gate")) ent.attack(mStruct[0].id(), false);
            else {
              let rand = randIntExclusive(0, mStruct.length * 0.2);
              ent.attack(
                mStruct[rand].id(),
            ...
            // really nothing   let's try to help our nearest unit
            let distmin = Math.min();
            let attacker;
            this.unitCollection.forEach((unit) => {
              if (!unit.position()) return;
              if (
                unit.unitAIState().split(".")[1] != "COMBAT" ||
                !unit.unitAIOrderData().length ||
                !unit.unitAIOrderData()[0].target
              )
                return;
              let target = gameState.getEntityById(
                unit.unitAIOrderData()[0].target
              );
              if (!target) return;
              let dist = API3.SquareVectorDistance(
              ...
              if (
                !ent.canAttackTarget(
                  target,
                  PETRA.allowCapture(gameState, ent, target)
                )
              )
                return;
              attacker = target;
            ...
            if (attacker)
              ent.attack(
                attacker.id(),
                PETRA.allowCapture(gameState, ent, attacker)
              );
          }
        }
      }
...
PETRA.AttackPlan.prototype.UpdateTransporting = function (gameState, events) {
  ...
  for (let ent of this.unitCollection.values()) {
    ...
    if (!done) continue;
    if (ent.getMetadata(PlayerID, "transport") !== undefined) done = false;
  }
  ...
  for (let evt of events.Attacked) {
    if (!this.unitCollection.hasEntId(evt.target)) continue;
    let attacker = gameState.getEntityById(evt.attacker);
    if (!attacker || !gameState.getEntityById(evt.target)) continue;
    for (let ent of this.unitCollection.values()) {
      if (ent.getMetadata(PlayerID, "transport") !== undefined) continue;
...
PETRA.AttackPlan.prototype.UpdateWalking = function (gameState, events) {
  ...
  for (let evt of events.Attacked) {
    ...
    if (attacker && (attacker.owner() !== 0 || this.targetPlayer === 0)) {
      attackedNB++;
      if (attacker.hasClass("Unit")) attackedUnitNB++;
    }
  }
  // Are we arrived at destination ?
  if (attackedNB > 1 && (attackedUnitNB || this.hasSiegeUnits())) {
    if (
      gameState.ai.HQ.territoryMap.getOwner(this.position) ===
        this.targetPlayer ||
      attackedNB > 3
    ) {
      this.state = "arrived";
  ...
  if (
    API3.SquareVectorDistance(this.position, this.position5TurnsAgo) < 10 &&
    this.path.length > 0 &&
    ...
    for (let ent of this.unitCollection
      .filter(API3.Filters.byClass("Siege"))
      .values()) {
    ...
    if (farthestEnt) farthestEnt.destroy();
  }
  if (gameState.ai.playedTurn % 5 === 0) this.position5TurnsAgo = this.position;

  if (
    this.lastPosition &&
    API3.SquareVectorDistance(this.position, this.lastPosition) < 16 &&
    this.path.length > 0
  ) {
    ...
    for (let ent of gameState
      .getEnemyStructures()
      .filter(API3.Filters.byClass(["Palisade", "Wall"]))
      .values()) {
      ...
      if (
        this.unitCollection
          .filter(API3.Filters.byCanAttackClass(enemyClass))
          .hasEntities()
      ) {
        if (this.Config.debug > 1)
          API3.warn(
  ...
  if (API3.SquareVectorDistance(this.position, this.targetPos) < 10000) {
    ...
    this.path.length &&
    API3.SquareVectorDistance(this.position, this.path[0]) < 1600
  ) {
    this.path.shift();
    if (this.path.length)
      this.unitCollection.moveToRange(this.path[0][0], this.path[0][1], 0, 15);
...
PETRA.AttackPlan.prototype.UpdateTarget = function (gameState) {
  ...
  if (
    this.targetPlayer === undefined ||
    !gameState.isPlayerEnemy(this.targetPlayer)
  ) {
    this.targetPlayer = gameState.ai.HQ.attackManager.getEnemyPlayer(
      gameState,
      this
  ...
  if (this.target && this.target.owner() === 0 && this.targetPlayer !== 0)
    // this enemy has resigned
    this.target = undefined;

  if (!this.target || !gameState.getEntityById(this.target.id())) {
    if (this.Config.debug > 1)
      API3.warn(
        "Seems like our target for plan " +
          this.name +
    ...
    if (!this.target) {
      ...
      for (let attackType in attackManager.startedAttacks) {
        for (let attack of attackManager.startedAttacks[attackType]) {
          if (attack.name == this.name) continue;
          if (
            !attack.target ||
            !gameState.getEntityById(attack.target.id()) ||
            !gameState.isPlayerEnemy(attack.target.owner())
          )
            continue;
          if (accessIndex != PETRA.getLandAccess(gameState, attack.target))
            continue;
          if (attack.target.owner() == 0 && attack.targetPlayer != 0)
            // looks like it has resigned
            continue;
          if (!gameState.isPlayerEnemy(attack.targetPlayer)) continue;
      ...
      if (!this.target) {
        ...
        if (!this.target) {
          if (this.Config.debug > 1)
            API3.warn(
              "No new target found. Remaining units " +
...
PETRA.AttackPlan.prototype.Abort = function (gameState) {
  ...
  if (this.unitCollection.hasEntities()) {
    ...
    if (this.isStarted()) {
      ...
      if (
        this.rallyPoint &&
        gameState.ai.accessibility.getAccessValue(this.rallyPoint) == access
      ) {
      ...
      for (let base of gameState.ai.HQ.baseManagers) {
        ...
        let newdist = API3.SquareVectorDistance(
          this.position,
    ...
    for (let ent of this.unitCollection.values()) {
      if (ent.getMetadata(PlayerID, "role") == "attack") ent.stopMoving();
      if (rallyPoint) ent.moveToRange(rallyPoint[0], rallyPoint[1], 0, 15);
      this.removeUnit(ent);
    }
  }

  for (let unitCat in this.unitStat) this.unit[unitCat].unregister();
...
PETRA.AttackPlan.prototype.removeUnit = function (ent, update) {
  if (ent.getMetadata(PlayerID, "role") == "attack") {
    ...
    else ent.setMetadata(PlayerID, "role", undefined);
    ent.setMetadata(PlayerID, "subrole", undefined);
  }
  ent.setMetadata(PlayerID, "plan", -1);
  if (update) this.unitCollection.updateEnt(ent);
};

PETRA.AttackPlan.prototype.checkEvents = function (gameState, events) {
  for (let evt of events.EntityRenamed) {
    if (!this.target || this.target.id() != evt.entity) continue;
    if (this.type == "Raid" && !this.isStarted()) this.target = undefined;
    else this.target = gameState.getEntityById(evt.newentity);
    if (this.target) this.targetPos = this.target.position();
  }

  for (let evt of events.OwnershipChanged) // capture event
    if (
      this.target &&
      this.target.id() == evt.entity &&
      gameState.isPlayerAlly(evt.to)
    )
      this.target = undefined;

  for (let evt of events.PlayerDefeated) {
    if (this.targetPlayer !== evt.playerId) continue;
    this.targetPlayer = gameState.ai.HQ.attackManager.getEnemyPlayer(
      gameState,
      this
    );
  ...
  if (!this.overseas || this.state !== "unexecuted") return;
  // let's check if an enemy has built a structure at our access
  for (let evt of events.Create) {
    let ent = gameState.getEntityById(evt.entity);
    if (!ent || !ent.position() || !ent.hasClass("Structure")) continue;
    if (!gameState.isPlayerEnemy(ent.owner())) continue;
    let access = PETRA.getLandAccess(gameState, ent);
    for (let base of gameState.ai.HQ.baseManagers) {
      if (!base.anchor || !base.anchor.position()) continue;
...
PETRA.AttackPlan.prototype.waitingForTransport = function () {
  for (let ent of this.unitCollection.values())
    if (ent.getMetadata(PlayerID, "transport") !== undefined) return true;
  return false;
};

PETRA.AttackPlan.prototype.hasSiegeUnits = function () {
  for (let ent of this.unitCollection.values())
    if (PETRA.isSiegeUnit(ent)) return true;
  return false;
};

PETRA.AttackPlan.prototype.hasForceOrder = function (data, value) {
  for (let ent of this.unitCollection.values()) {
    if (data && +ent.getMetadata(PlayerID, data) !== value) continue;
    let orders = ent.unitAIOrderData();
    for (let order of orders) if (order.force) return true;
  }
  return false;
};
...
PETRA.AttackPlan.prototype.Deserialize = function (gameState, data) {
  ...
  if (this.target) this.target = gameState.getEntityById(this.target);

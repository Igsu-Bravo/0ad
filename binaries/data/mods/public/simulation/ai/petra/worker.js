...
PETRA.Worker.prototype.update = function (gameState, ent) {
  ...
  if (ent.getMetadata(PlayerID, "transport") !== undefined) {
    ...
    if (
      ...
      let plan = gameState.ai.HQ.navalManager.getPlan(
        ent.getMetadata(PlayerID, "transport")
      );
      let target = gameState.getEntityById(
        ent.getMetadata(PlayerID, "target-foundation")
      );
      ...
      if (plan.state == "boarding" && ent.position()) {
        ...
        for (let structure of gameState.getOwnStructures().values()) {
          if (PETRA.getLandAccess(gameState, structure) != plan.endIndex)
            continue;
          let resourceDropsiteTypes = PETRA.getBuiltEntity(
            gameState,
            structure
          ).resourceDropsiteTypes();
          if (
            !resourceDropsiteTypes ||
            resourceDropsiteTypes.indexOf(gatherType) == -1
          )
            continue;
          hasDropsite = true;
          break;
        ...
        if (!hasDropsite) {
          for (let unit of gameState
            .getOwnUnits()
            .filter(API3.Filters.byClass("Support"))
            .values()) {
            if (
              !unit.position() ||
              PETRA.getLandAccess(gameState, unit) != plan.endIndex
            )
              continue;
        ...
        if (!hasDropsite) plan.removeUnit(gameState, ent);
      }
    }
    if (ent.getMetadata(PlayerID, "transport") !== undefined) return;
  }
  ...
  else this.baseAccess = this.base.accessIndex;

  if (!subrole) {
    // subrole may-be undefined after a transport, garrisoning, army, ...
  ...
  if (
    (subrole == "hunter" || subrole == "gatherer") &&
    ...
    if (this.isInaccessibleSupply(gameState)) {
      if (this.retryWorking(gameState, subrole)) return;
      ent.stopMoving();
    }

    if (
      unitAIState == "INDIVIDUAL.COMBAT.APPROACHING" &&
      ent.unitAIOrderData().length
    ) {
      ...
      if (orderData && orderData.target) {
        ...
        if (
          target &&
          target.resourceSupplyType() &&
          target.resourceSupplyType().generic == "food"
        ) {
          let territoryOwner = gameState.ai.HQ.territoryMap.getOwner(
            target.position()
          );
  ...
  if (unitAIStateOrder == "COMBAT") {
    if (subrole == "fisher") this.startFishing(gameState);
    else if (
      unitAIState == "INDIVIDUAL.COMBAT.APPROACHING" &&
      ent.unitAIOrderData().length &&
      ...
      if (orderData && orderData.target) {
        ...
        if (
          target &&
          (!target.position() ||
            PETRA.getLandAccess(gameState, target) != this.entAccess)
        ) {
          if (this.retryWorking(gameState, subrole)) return;
          ent.stopMoving();
        }
      ...
      unitAIState == "INDIVIDUAL.COMBAT.ATTACKING" &&
      ent.unitAIOrderData().length &&
      !ent.getMetadata(PlayerID, "PartOfArmy")
    ) {
      let orderData = ent.unitAIOrderData()[0];
      if (
        orderData &&
        orderData.target &&
        orderData.attackType &&
        orderData.attackType == "Capture"
      ) {
        ...
        if (
          target &&
          target.owner() > 0 &&
          !gameState.isPlayerAlly(target.owner())
        )
          ent.attack(
            orderData.target,
  ...
  if (subrole == "gatherer") {
    if (ent.isIdle()) {
      ...
      if (
        ...
        !ent.resourceCarrying().length ||
        ent.resourceCarrying()[0].type ==
          ent.getMetadata(PlayerID, "gather-type")
      ) {
        this.startGathering(gameState);
      } else if (!PETRA.returnResources(gameState, ent)) {
      ...
      if (ent.unitAIOrderData().length) {
        ...
        if (
          supply &&
          !supply.hasClasses(["Field", "Animal"]) &&
          supplyId != ent.getMetadata(PlayerID, "supply")
        ) {
          let nbGatherers =
            supply.resourceSupplyNumGatherers() +
            gameState.ai.HQ.GetTCGatherer(supplyId);
          if (
            nbGatherers > 1 &&
            supply.resourceSupplyAmount() / nbGatherers < 30
          ) {
            ...
            if (nearby.some((sup) => sup.id == supplyId))
              ...
              if (medium.length && !medium.some((sup) => sup.id == supplyId)) {
                gameState.ai.HQ.RemoveTCGatherer(supplyId);
                this.startGathering(gameState);
              } else ent.setMetadata(PlayerID, "supply", supplyId);
            }
          }
        }
      }
      if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING") {
        if (gameState.ai.playedTurn % 10 == 0) {
          ...
          let dropsite = gameState.getEntityById(
            ent.unitAIOrderData()[0].target
          );
          if (
            dropsite &&
            dropsite.position() &&
            this.entAccess != PETRA.getLandAccess(gameState, dropsite)
          )
        ...
        let gatherType = ent.getMetadata(PlayerID, "gather-type");
        let influenceGroup =
          Resources.GetResource(gatherType).aiAnalysisInfluenceGroup;
        if (influenceGroup && influenceGroup == "sparse") {
          let supplyId = ent.getMetadata(PlayerID, "supply");
          if (supplyId) {
            let nearby = this.base.dropsiteSupplies[gatherType].nearby;
            if (!nearby.some((sup) => sup.id == supplyId)) {
              if (nearby.length) ent.setMetadata(PlayerID, "supply", undefined);
              else {
                let medium = this.base.dropsiteSupplies[gatherType].medium;
    ...
    if (unitAIStateOrder == "REPAIR") {
      ...
      if (targetId && gameState.getEntityById(targetId)) return;
      ent.stopMoving();
    }
    ...
    if (
      !target ||
      (target.foundationProgress() === undefined &&
        target.needsRepair() === false)
    ) {
      ent.setMetadata(PlayerID, "subrole", "idle");
      ent.setMetadata(PlayerID, "target-foundation", undefined);
      if (this.baseID != gameState.ai.HQ.baseManagers[0].ID) {
      ...
      if (this.entAccess == goalAccess)
        ent.repair(target, target.hasClass("House"), queued);
      // autocontinue=true for houses
      else
    ...
    if (
      ent.isIdle() &&
      (!lastHuntSearch || gameState.ai.elapsedTime - lastHuntSearch > 20)
    ) {
    ...
    if (ent.isIdle()) this.startFishing(gameState);
    // if we have drifted towards ennemy territory during the fishing, go home
    else {
      let territoryOwner = gameState.ai.HQ.territoryMap.getOwner(
        ent.position()
      );
...
PETRA.Worker.prototype.startBuilding = function (gameState) {
  let target = gameState.getEntityById(
    this.ent.getMetadata(PlayerID, "target-foundation")
  );
  if (
    !target ||
    (target.foundationProgress() === undefined && target.needsRepair() == false)
  )
    return false;
  if (PETRA.getLandAccess(gameState, target) != this.entAccess) return false;
...
PETRA.Worker.prototype.startGathering = function (gameState) {
  ...
  if (PETRA.gatherTreasure(gameState, this.ent)) return true;

  let resource = this.ent.getMetadata(PlayerID, "gather-type");

  // If we are gathering food, try to hunt first
  if (resource == "food" && this.startHunting(gameState)) return true;
  ...
  let findSupply = function (ent, supplies) {
    ...
    for (let i = 0; i < supplies.length; ++i) {
      ...
      if (PETRA.IsSupplyFull(gameState, supplies[i].ent)) continue;
      let inaccessibleTime = supplies[i].ent.getMetadata(
        PlayerID,
        "inaccessibleTime"
      );
      ...
      if (!gatherRates[supplyType]) continue;
      // check if available resource is worth one additionnal gatherer (except for farms)
      let nbGatherers =
        supplies[i].ent.resourceSupplyNumGatherers() +
        gameState.ai.HQ.GetTCGatherer(supplies[i].id);
      if (
        supplies[i].ent.resourceSupplyType().specific != "grain" &&
        nbGatherers > 0 &&
  ...
  for (let base of gameState.ai.HQ.baseManagers) {
    if (base.ID == this.baseID) continue;
    if (base.accessIndex != this.entAccess) continue;
  ...
  let shouldBuild =
    this.ent.isBuilder() &&
    foundations.some(function (foundation) {
      if (
        !foundation ||
        PETRA.getLandAccess(gameState, foundation) != this.entAccess
      )
        return false;
      let structure = gameState.getBuiltTemplate(foundation.templateName());
      if (
        structure.resourceDropsiteTypes() &&
        structure.resourceDropsiteTypes().indexOf(resource) != -1
      ) {
        if (foundation.getMetadata(PlayerID, "base") != this.baseID)
          this.ent.setMetadata(
            PlayerID,
            "base",
            foundation.getMetadata(PlayerID, "base")
          );
  ...
  if (allowDistant) {
    if (this.baseAccess == this.entAccess) {
      supply = findSupply(
        this.ent,
        this.base.dropsiteSupplies[resource].faraway
      );
      if (supply) {
        this.ent.gather(supply);
        return true;
  ...
  if (gameState.ai.Config.debug > 2)
    API3.warn(
      " >>>>> worker with gather-type " + resource + " with nothing to gather "
    );
...
PETRA.Worker.prototype.startHunting = function (gameState, position) {
  // First look for possible treasure if any
  if (!position && PETRA.gatherTreasure(gameState, this.ent)) return true;

  let resources = gameState.getHuntableSupplies();
  if (!resources.hasEntities()) return false;
  ...
  let hasFoodDropsiteWithinDistance = function (
    supplyPosition,
    supplyAccess,
    distSquare
  ) {
    for (let dropsite of foodDropsites.values()) {
      ...
      if (
        owner != PlayerID &&
        (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner))
      )
        continue;
      if (supplyAccess != PETRA.getLandAccess(gameState, dropsite)) continue;
      if (
        API3.SquareVectorDistance(supplyPosition, dropsite.position()) <
        distSquare
      )
        return true;
    }
  ...
  for (let supply of resources.values()) {
    ...
    if (PETRA.IsSupplyFull(gameState, supply)) continue;
    // Check if available resource is worth one additionnal gatherer (except for farms).
    let nbGatherers =
      supply.resourceSupplyNumGatherers() +
      gameState.ai.HQ.GetTCGatherer(supply.id());
    if (
      nbGatherers > 0 &&
      supply.resourceSupplyAmount() / (1 + nbGatherers) < 30
    )
      continue;

    let canFlee =
      !supply.hasClass("Domestic") &&
      supply.templateName().indexOf("resource|") == -1;
    // Only FastMoving and Ranged units should hunt fleeing animals.
    if (canFlee && !isFastMoving && !isRanged) continue;

    let supplyAccess = PETRA.getLandAccess(gameState, supply);
    if (supplyAccess != this.entAccess) continue;
    ...
    if (!isFastMoving && dist > 25000) continue;
...
PETRA.Worker.prototype.startFishing = function (gameState) {
  if (!this.ent.position()) return false;
  ...
  let fishDropsites = (
    gameState.playerData.hasSharedDropsites
      ? gameState.getAnyDropsites("food")
      : gameState.getOwnDropsites("food")
  )
  ...
  let nearestDropsiteDist = function (supply) {
    ...
    for (let dropsite of fishDropsites) {
      ...
      if (
        ...
        continue;
      if (fisherSea != PETRA.getSeaAccess(gameState, dropsite)) continue;
      distMin = Math.min(
        distMin,
        API3.SquareVectorDistance(pos, dropsite.position())
      );
    }
    return distMin;
  };
  ...
  resources.forEach(function (supply) {
    ...
    let supplyType = supply.get("ResourceSupply/Type");
    if (!gatherRates[supplyType]) return;

    if (PETRA.IsSupplyFull(gameState, supply)) return;
    // check if available resource is worth one additionnal gatherer (except for farms)
    let nbGatherers =
      supply.resourceSupplyNumGatherers() +
      gameState.ai.HQ.GetTCGatherer(supply.id());
    ...
    if (!gameState.ai.HQ.navalManager.canFishSafely(gameState, supply)) return;

    // measure the distance from the resource to the nearest dropsite
    let dist = nearestDropsiteDist(supply);
    if (dist > nearestSupplyDist) return;
...
PETRA.Worker.prototype.gatherNearestField = function (gameState, baseID) {
  let ownFields = gameState
    .getOwnEntitiesByClass("Field", true)
    .filter(API3.Filters.isBuilt())
    .filter(API3.Filters.byMetadata(PlayerID, "base", baseID));
  let bestFarm;

  let gatherRates = this.ent.resourceGatherRates();
  for (let field of ownFields.values()) {
    if (PETRA.IsSupplyFull(gameState, field)) continue;
    ...
    if (diminishing < 1) {
      let num =
        field.resourceSupplyNumGatherers() +
        gameState.ai.HQ.GetTCGatherer(field.id());
      if (num > 0) rate = Math.pow(diminishing, num);
    }
    // Add a penalty distance depending on rate
    let dist =
      API3.SquareVectorDistance(field.position(), this.ent.position()) +
      (1 - rate) * 160000;
  ...
  if (
    ...
    (bestFarm.rate < 0.7 &&
      gameState
        .getOwnFoundations()
        .filter(API3.Filters.byClass("Field"))
        .filter(API3.Filters.byMetadata(PlayerID, "base", baseID))
        .hasEntities())
  )
    return false;
...
PETRA.Worker.prototype.buildAnyField = function (gameState, baseID) {
  if (!this.ent.isBuilder()) return false;
  ...
  for (let found of gameState.getOwnFoundations().values()) {
    if (
      found.getMetadata(PlayerID, "base") != baseID ||
      !found.hasClass("Field")
    )
      continue;
...
PETRA.Worker.prototype.moveToGatherer = function (gameState, ent, forced) {
  ...
  if (
    ...
    gameState.ai.elapsedTime <
      (ent.getMetadata(PlayerID, "nextMoveToGatherer") || 5)
  )
    return;
  ...
  for (let gatherer of gatherers.values()) {
    ...
    if (!gathererType || types.indexOf(gathererType) == -1) continue;
    if (
      !gatherer.position() ||
      gatherer.getMetadata(PlayerID, "transport") !== undefined ||
      PETRA.getLandAccess(gameState, gatherer) != access ||
      gatherer.isIdle()
    )
      continue;
  ...
  ent.setMetadata(
    PlayerID,
    "nextMoveToGatherer",
    gameState.ai.elapsedTime + (destination ? 12 : 5)
  );
  if (destination && dist > 10) ent.move(destination[0], destination[1]);
};
...
PETRA.Worker.prototype.isInaccessibleSupply = function (gameState) {
  ...
  if (!target.resourceSupplyType()) return false;

  let approachingTarget = this.ent.getMetadata(PlayerID, "approachingTarget");
  let carriedAmount = this.ent.resourceCarrying().length
    ? this.ent.resourceCarrying()[0].amount
    : 0;
  ...
  if (!approachingTime || gameState.ai.elapsedTime - approachingTime > 3) {
    ...
    if (
      !approachingPos ||
      approachingPos[0] != presentPos[0] ||
      approachingPos[1] != presentPos[1]
    ) {
      this.ent.setMetadata(
        PlayerID,
        "approachingTime",

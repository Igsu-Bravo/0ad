...
PETRA.AttackManager.prototype.init = function (gameState) {
  this.outOfPlan = gameState
    .getOwnUnits()
    .filter(API3.Filters.byMetadata(PlayerID, "plan", -1));
  this.outOfPlan.registerUpdates();
};

PETRA.AttackManager.prototype.setRushes = function (allowed) {
  if (
    this.Config.personality.aggressive > this.Config.personalityCut.strong &&
    allowed > 2
  ) {
    this.maxRushes = 3;
    this.rushSize = [16, 20, 24];
  } else if (
    this.Config.personality.aggressive > this.Config.personalityCut.medium &&
    allowed > 1
  ) {
    this.maxRushes = 2;
    this.rushSize = [18, 22];
  } else if (
    this.Config.personality.aggressive > this.Config.personalityCut.weak &&
    allowed > 0
  ) {
    this.maxRushes = 1;
    this.rushSize = [20];
...
PETRA.AttackManager.prototype.checkEvents = function (gameState, events) {
  for (let evt of events.PlayerDefeated) this.defeated[evt.playerId] = true;
  ...
  for (let evt of events.AttackRequest) {
    if (
      evt.source === PlayerID ||
      !gameState.isPlayerAlly(evt.source) ||
      !gameState.isPlayerEnemy(evt.player)
    )
      continue;
    ...
    if (available > 12) {
      ...
      answer = "join";
    } else if (other !== undefined) answer = "other";
...
PETRA.AttackManager.prototype.assignBombers = function (gameState) {
    ...
    if (!target || !gameState.isPlayerEnemy(target.owner()))
      ...
      for (let entId of unitIds.values()) {
        ...
        if (ent && ent.owner() == PlayerID) {
          ...
          let lastOrder =
            orders && orders.length ? orders[orders.length - 1] : null;
          if (
            lastOrder &&
            lastOrder.target &&
            lastOrder.target == targetId &&
            plan != -2 &&
            plan != -3
          )
            continue;
      ...
      if (!unitIds.size) this.bombingAttacks.delete(targetId);
    }
  }

  const bombers = gameState.updatingCollection(
    "bombers",
    API3.Filters.byClasses(["BoltShooter", "StoneThrower"]),
  ...
  for (let ent of bombers.values()) {
    ...
    for (let unitIds of this.bombingAttacks.values()) {
      if (!unitIds.has(ent.id())) continue;
      alreadyBombing = true;
      break;
    }
    if (alreadyBombing) break;
    ...
    for (let struct of gameState.getEnemyStructures().values()) {
      if (
        !ent.canAttackTarget(struct, PETRA.allowCapture(gameState, ent, struct))
      )
        continue;
      ...
      if (struct.hasClass("Field")) {
        if (
          !struct.resourceSupplyNumGatherers() ||
          !gameState.isPlayerEnemy(
            gameState.ai.HQ.territoryMap.getOwner(structPos)
          )
        )
          continue;
      }
      ...
      if (dist > range) {
        ...
        if (owner != 0 && gameState.isPlayerEnemy(owner)) continue;
        x = structPos[0] + ((entPos[0] - structPos[0]) * range) / dist;
        z = structPos[1] + ((entPos[1] - structPos[1]) * range) / dist;
        if (
          gameState.ai.HQ.territoryMap.getOwner([x, z]) != PlayerID ||
      ...
      let attackingUnits;
      for (let [targetId, unitIds] of this.bombingAttacks) {
        if (targetId != struct.id()) continue;
        attackingUnits = unitIds;
        break;
      }
      if (attackingUnits && attackingUnits.size > 4) continue; // already enough units against that target
      if (!attackingUnits) {
        attackingUnits = new Set();
        this.bombingAttacks.set(struct.id(), attackingUnits);
      }
      attackingUnits.add(ent.id());
      if (dist > range) ent.move(x, z);
...
PETRA.AttackManager.prototype.update = function (gameState, queues, events) {
  ...
  for (let attackType in this.upcomingAttacks) {
    for (let i = 0; i < this.upcomingAttacks[attackType].length; ++i) {
      ...
      if (attack.isStarted())
        API3.warn(
          "Petra problem in attackManager: attack in preparation has already started ???"
        );
      ...
      if (updateStep == 1 || attack.isPaused()) {
        // just chillin'
        if (attack.state == "unexecuted") ++unexecutedAttacks[attackType];
        ...
        if (attack.StartAttack(gameState)) {
          if (this.Config.debug > 1)
            API3.warn(
              "Attack Manager: Starting " +
                attack.getType() +
                " plan " +
                attack.getName()
            );
          if (this.Config.chat)
            PETRA.chatLaunchAttack(
              gameState,
              attack.targetPlayer,
              attack.getType()
            );
          this.startedAttacks[attackType].push(attack);
        } else attack.Abort(gameState);
        this.upcomingAttacks[attackType].splice(i--, 1);
      }
    }
  }
  ...
  let barracksNb = gameState
    .getOwnEntitiesByClass("Barracks", true)
    .filter(API3.Filters.isBuilt()).length;
  if (this.rushNumber < this.maxRushes && barracksNb >= 1) {
    if (unexecutedAttacks.Rush === 0) {
      ...
      let attackPlan = new PETRA.AttackPlan(
        gameState,
        this.Config,
        this.totalNumber,
        "Rush",
    ...
    unexecutedAttacks.Attack == 0 &&
    unexecutedAttacks.HugeAttack == 0 &&
    this.startedAttacks.Attack.length + this.startedAttacks.HugeAttack.length <
      Math.min(2, 1 + Math.round(gameState.getPopulationMax() / 100)) &&
    (this.startedAttacks.Attack.length +
      this.startedAttacks.HugeAttack.length ==
      0 ||
    ...
    if (
      ...
      !gameState.ai.HQ.baseManagers[1]
    ) {
      // if we have no base ... nothing else to do than attack
      let type =
        this.attackNumber < 2 || this.startedAttacks.HugeAttack.length > 0
          ? "Attack"
          : "HugeAttack";
      let attackPlan = new PETRA.AttackPlan(
        gameState,
      ...
      if (attackPlan.failed) this.attackPlansEncounteredWater = true;
      // hack
      else {
        if (this.Config.debug > 1)
          API3.warn(
            "Military Manager: Creating the plan " +
              type +
              "  " +
              this.totalNumber
          );
  ...
  if (
    ...
    for (let targetId of gameState.ai.HQ.defenseManager.targetList) {
      ...
      if (!target) continue;
      if (gameState.isPlayerEnemy(target.owner())) break;
      target = undefined;
    }
    if (target)
...
PETRA.AttackManager.prototype.getPlan = function (planName) {
  for (let attackType in this.upcomingAttacks) {
    for (let attack of this.upcomingAttacks[attackType])
      if (attack.getName() == planName) return attack;
  }
  for (let attackType in this.startedAttacks) {
    for (let attack of this.startedAttacks[attackType])
      if (attack.getName() == planName) return attack;
...
PETRA.AttackManager.prototype.pausePlan = function (planName) {
  ...
  if (attack) attack.setPaused(true);
};

PETRA.AttackManager.prototype.unpausePlan = function (planName) {
  let attack = this.getPlan(planName);
  if (attack) attack.setPaused(false);
};

PETRA.AttackManager.prototype.pauseAllPlans = function () {
  for (let attackType in this.upcomingAttacks)
    for (let attack of this.upcomingAttacks[attackType]) attack.setPaused(true);

  for (let attackType in this.startedAttacks)
    for (let attack of this.startedAttacks[attackType]) attack.setPaused(true);
};
...
PETRA.AttackManager.prototype.unpauseAllPlans = function () {
  ...
  for (let attackType in this.startedAttacks)
    for (let attack of this.startedAttacks[attackType]) attack.setPaused(false);
};

PETRA.AttackManager.prototype.getAttackInPreparation = function (type) {
  return this.upcomingAttacks[type].length
    ? this.upcomingAttacks[type][0]
    : undefined;
};
...
PETRA.AttackManager.prototype.getEnemyPlayer = function (gameState, attack) {
  ...
  let veto = {};
  for (let i in this.defeated) veto[i] = true;
  // No rush if enemy too well defended (i.e. iberians)
  if (attack.type == "Rush") {
    for (let i = 1; i < gameState.sharedScript.playersData.length; ++i) {
      if (!gameState.isPlayerEnemy(i) || veto[i]) continue;
      if (this.defeated[i]) continue;
      let enemyDefense = 0;
      for (let ent of gameState.getEnemyStructures(i).values())
        if (ent.hasClasses(["Tower", "WallTower", "Fortress"])) enemyDefense++;
      if (enemyDefense > 6) veto[i] = true;
    }
  }
  ...
  if (attack.type != "HugeAttack") {
    if (
      attack.targetPlayer === undefined &&
      this.currentEnemyPlayer !== undefined &&
  ...
  for (let i = 1; i < gameState.sharedScript.playersData.length; ++i) {
    if (veto[i]) continue;
    if (!gameState.isPlayerEnemy(i)) continue;
    let enemyCount = 0;
    let enemyCivCentre = false;
    for (let ent of gameState.getEntities(i).values()) {
      enemyCount++;
      if (ent.hasClass("CivCentre")) enemyCivCentre = true;
    }
    if (enemyCivCentre) enemyCount += 500;
    if (!enemyCount || enemyCount < max) continue;
    max = enemyCount;
    enemyPlayer = i;
  }
  if (attack.targetPlayer === undefined) this.currentEnemyPlayer = enemyPlayer;
  return enemyPlayer;
};
 ...
 */
PETRA.AttackManager.prototype.getWonderEnemyPlayer = function (
  gameState,
  attack
) {
  let enemyPlayer;
  let enemyWonder;
  let moreAdvanced;
  for (let wonder of gameState
    .getEnemyStructures()
    .filter(API3.Filters.byClass("Wonder"))
    .values()) {
    if (wonder.owner() == 0) continue;
    let progress = wonder.foundationProgress();
    if (progress === undefined) {
      enemyWonder = wonder;
      break;
    }
    if (enemyWonder && moreAdvanced > progress) continue;
    enemyWonder = wonder;
 ...
 */
PETRA.AttackManager.prototype.getRelicEnemyPlayer = function (
  gameState,
  attack
) {
  let enemyPlayer;
  let allRelics = gameState.updatingGlobalCollection(
    "allRelics",
    API3.Filters.byClass("Relic")
  );
  ...
  for (let i = 0; i < gameState.sharedScript.playersData.length; ++i) {
    ...
    if (relicsCount <= maxRelicsOwned) continue;
...
PETRA.AttackManager.prototype.cancelAttacksAgainstPlayer = function (
  gameState,
  player
) {
  for (let attackType in this.upcomingAttacks)
    for (let attack of this.upcomingAttacks[attackType])
      if (attack.targetPlayer === player) attack.targetPlayer = undefined;
...
PETRA.AttackManager.prototype.raidTargetEntity = function (gameState, ent) {
  ...
  if (attackPlan.failed) return null;
  if (this.Config.debug > 1)
...
PETRA.AttackManager.prototype.numAttackingUnitsAround = function (pos, dist) {
  ...
  for (let attackType in this.startedAttacks)
    for (let attack of this.startedAttacks[attackType]) {
      if (!attack.position)
        // this attack may be inside a transport
        continue;
 ...
 */
PETRA.AttackManager.prototype.switchDefenseToAttack = function (
  gameState,
  target,
  data
) {
  ...
  if (!data.range && !data.armyID) {
    API3.warn(
      " attackManager.switchDefenseToAttack inconsistent data " + uneval(data)
    );
    return false;
  }
  let attackData = data.uniqueTarget
  ...
  if (attackPlan.failed) return false;
  this.totalNumber++;
  attackPlan.init(gameState);
  ...
  for (let army of gameState.ai.HQ.defenseManager.armies) {
    if (data.range) {
      ...
      if (
        API3.SquareVectorDistance(pos, army.foePosition) >
        data.range * data.range
      )
        continue;
    } else if (army.ID != +data.armyID) continue;
    ...
    while (army.ownEntities.length > 0) {
      ...
      let accessOk =
        unit.getMetadata(PlayerID, "transport") !== undefined ||
        (unit.position() &&
          PETRA.getLandAccess(gameState, unit) == targetAccess);
...
PETRA.AttackManager.prototype.Serialize = function () {
  ...
  return {
    ...
    upcomingAttacks: upcomingAttacks,
    startedAttacks: startedAttacks,
  };
};
...
PETRA.AttackManager.prototype.Deserialize = function (gameState, data) {
  ...
  for (let key in data.upcomingAttacks) {
    ...
    for (let dataAttack of data.upcomingAttacks[key]) {
      let attack = new PETRA.AttackPlan(
        gameState,
        this.Config,
        dataAttack.properties.name
      );

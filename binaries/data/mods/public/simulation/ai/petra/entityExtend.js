...
PETRA.getMaxStrength = function (
  ...
  if (!attackTypes) return strength;
  ...
  for (let type of attackTypes) {
    ...
    for (let str in attackStrength) {
      ...
      if (againstClass) val *= ent.getMultiplierAgainst(type, againstClass);
      if (DamageTypeImportance[str])
        ...
        API3.warn(
          "Petra: " +
            str +
            " unknown attackStrength in getMaxStrength (please add " +
            str +
            "  to config.js)."
        );
    }
    ...
    if (attackRange) strength += attackRange.max * 0.0125;
    ...
    for (let str in resistanceStrength.Damage) {
      ...
      if (DamageTypeImportance[str])
        ...
        API3.warn(
          "Petra: " +
            str +
            " unknown resistanceStrength in getMaxStrength (please add " +
            str +
            "  to config.js)."
        );
    }
...
PETRA.getLandAccess = function (gameState, ent) {
  if (ent.hasClass("Unit")) {
    ...
    if (!pos) {
      ...
      if (holder) return PETRA.getLandAccess(gameState, holder);
...
PETRA.allowCapture = function (gameState, ent, target) {
  if (!target.isCapturable() || !ent.canCapture(target)) return false;
  if (target.isInvulnerable()) return true;
  // always try to recapture capture points from an allied, except if it's decaying
  if (gameState.isPlayerAlly(target.owner())) return !target.decaying();
  ...
  if (target.decaying()) antiCapture -= target.territoryDecayRate();
  ...
  if (!capturableTargets.has(target.id())) {
    ...
    capturableTargets.set(target.id(), {
      strength: capture,
      ents: new Set([ent.id()]),
    });
  } else {
  ...
  if (
    target.hasDefensiveFire() &&
    target.isGarrisonHolder() &&
    target.garrisoned()
  )
...
PETRA.getAttackBonus = function (ent, target, type) {
  ...
  for (let key in bonuses) {
    ...
    if (bonus.Civ && bonus.Civ !== target.civ()) continue;
...
PETRA.returnResources = function (gameState, ent) {
  ...
  let dropsiteCollection = gameState.playerData.hasSharedDropsites
    ? gameState.getAnyDropsites(resource)
    : gameState.getOwnDropsites(resource);
  for (let dropsite of dropsiteCollection.values()) {
    ...
    if (
      ...
      continue;
    if (PETRA.getLandAccess(gameState, dropsite) != access) continue;
  ...
  if (!closestDropsite) return false;
...
PETRA.IsSupplyFull = function (gameState, ent) {
  return (
    ...
    ent.resourceSupplyNumGatherers() +
      gameState.ai.HQ.GetTCGatherer(ent.id()) >=
      ent.maxGatherers()
  );
};
 ...
 */
PETRA.getBestBase = function (
  gameState,
  ent,
  onlyConstructedBase = false,
  exclude = false
) {
  let pos = ent.position();
  let accessIndex;
  ...
  } else accessIndex = PETRA.getLandAccess(gameState, ent);
  ...
  for (let base of gameState.ai.HQ.baseManagers) {
    if (
      base.ID == gameState.ai.HQ.baseManagers[0].ID ||
      (exclude && base.ID == exclude)
    )
      continue;
    if (
      onlyConstructedBase &&
      (!base.anchor || base.anchor.foundationProgress() !== undefined)
    )
      continue;
    if (ent.hasClass("Structure") && base.accessIndex != accessIndex) continue;
    ...
    if (base.accessIndex != accessIndex) dist += 50000000;
    if (!base.anchor) dist += 50000000;
    if (dist > distmin) continue;
    distmin = dist;
    bestbase = base;
  }
...
PETRA.getHolder = function (gameState, ent) {
  for (let holder of gameState.getEntities().values()) {
    if (
      holder.isGarrisonHolder() &&
      holder.garrisoned().indexOf(ent.id()) !== -1
    )
      return holder;
  }
  return undefined;
...
PETRA.isLineInsideEnemyTerritory = function (gameState, pos1, pos2, step = 70) {
  let n =
    Math.floor(Math.sqrt(API3.SquareVectorDistance(pos1, pos2)) / step) + 1;
  ...
  for (let i = 1; i < n; ++i) {
    ...
    if (owner && gameState.isPlayerEnemy(owner)) return true;
  }
  return false;
};

PETRA.gatherTreasure = function (gameState, ent, water = false) {
  if (!gameState.ai.HQ.treasures.hasEntities()) return false;
  if (!ent || !ent.position()) return false;
  if (!ent.isTreasureCollector()) return false;
  let treasureFound;
  let distmin = Math.min();
  let access = water
    ? PETRA.getSeaAccess(gameState, ent)
  ...
  for (let treasure of gameState.ai.HQ.treasures.values()) {
    ...
    if (lastGathered && gameState.ai.elapsedTime - lastGathered < 20) continue;
    if (!water && access != PETRA.getLandAccess(gameState, treasure)) continue;
    if (water && access != PETRA.getSeaAccess(gameState, treasure)) continue;
    ...
    if (dist > 120000 || (territoryOwner != PlayerID && dist > 14000))
      // AI has no LOS, so restrict it a bit
      continue;
    if (dist > distmin) continue;
    distmin = dist;
    treasureFound = treasure;
  }
  if (!treasureFound) return false;
...
PETRA.dumpEntity = function (ent) {
  ...
  API3.warn(
    " base " +
      ...
      " >>> role " +
      ent.getMetadata(PlayerID, "role") +
      " subrole " +
      ent.getMetadata(PlayerID, "subrole")
  );
  API3.warn(
    "owner " +
      ent.owner() +
      ...
      ent.maxHitpoints() +
      " foundationProgress " +
      ent.foundationProgress()
  );
  API3.warn(
    " garrisoning " +
      ent.getMetadata(PlayerID, "garrisoning") +
      " garrisonHolder " +
      ent.getMetadata(PlayerID, "garrisonHolder") +
      " plan " +
  ...
  API3.warn(
    " stance " +
      ent.getStance() +
      " transporter " +
      ent.getMetadata(PlayerID, "transporter") +
      " gather-type " +

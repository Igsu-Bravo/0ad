PETRA.ResearchPlan = function (gameState, type, rush = false) {
  if (!PETRA.QueuePlan.call(this, gameState, type, {})) return false;

  if (this.template.researchTime === undefined) return false;
...
PETRA.ResearchPlan.prototype.canStart = function (gameState) {
  ...
  if (!this.researchers) return false;
  this.cost = new API3.Resources(this.template.cost(this.researchers[0]));
  return true;
};

PETRA.ResearchPlan.prototype.getBestResearchers = function (
  gameState,
  noRequirementCheck = false
) {
  let allResearchers = gameState.findResearchers(this.type, noRequirementCheck);
  if (!allResearchers || !allResearchers.hasEntities()) return undefined;
  ...
  for (let ent of allResearchers.values()) {
    ...
    if (cost === costMin) researchers.push(ent);
...
PETRA.ResearchPlan.prototype.isInvalid = function (gameState) {
  return (
    gameState.isResearched(this.type) || gameState.isResearching(this.type)
  );
};
...
PETRA.ResearchPlan.prototype.start = function (gameState) {
  ...
  this.researchers.sort(
    (a, b) => a.trainingQueueTime() - b.trainingQueueTime()
  );
  // Drop anything in the queue if we rush it.
  if (this.rush) this.researchers[0].stopAllProduction(0.45);
...
PETRA.ResearchPlan.prototype.onStart = function (gameState) {
  ...
  for (let i = gameState.getNumberOfPhases(); i > 0; --i) {
    if (this.type != gameState.getPhaseName(i)) continue;

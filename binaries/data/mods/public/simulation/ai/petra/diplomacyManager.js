...
PETRA.DiplomacyManager.prototype.init = function (gameState) {
  ...
  for (let i = 1; i < gameState.sharedScript.playersData.length; ++i) {
    ...
    if (gameState.isPlayerMutualAlly(i))
      this.receivedDiplomacyRequests.set(i, {
        requestType: "ally",
        status: "accepted",
      });
      ...
      gameState.sharedScript.playersData[i].isNeutral[PlayerID] &&
...
PETRA.DiplomacyManager.prototype.tributes = function (gameState) {
  ...
  for (let i = 1; i < gameState.sharedScript.playersData.length; ++i) {
    if (
      i === PlayerID ||
      !gameState.isPlayerAlly(i) ||
      gameState.ai.HQ.attackManager.defeated[i]
    )
      continue;
    let donor =
      gameState.getAlliedVictory() ||
      gameState.getEntities(i).length < gameState.getOwnEntities().length;
    ...
    for (let res of resTribCodes) {
      if (
        donor &&
        availableResources[res] > 200 &&
        allyResources[res] < 0.2 * availableResources[res]
      ) {
        tribute[res] = Math.floor(
          0.3 * availableResources[res] - allyResources[res]
        );
        toSend = true;
      } else if (
        donor &&
        allyPop < Math.min(30, 0.5 * gameState.getPopulation()) &&
        totalResources[res] > 500 &&
        allyResources[res] < 100
      ) {
        tribute[res] = 100;
        toSend = true;
      } else if (
        ...
        allyResources[res] > totalResources[res] + 600
      ) {
        if (gameState.ai.elapsedTime < this.nextTributeRequest.get("all"))
          continue;
        if (
          this.nextTributeRequest.has(res) &&
          gameState.ai.elapsedTime < this.nextTributeRequest.get(res)
        )
          continue;
        if (!mostNeeded)
          mostNeeded = gameState.ai.HQ.pickMostNeededResources(
            gameState,
            resTribCodes
          );
    ...
    if (this.Config.chat) PETRA.chatSentTribute(gameState, i);
...
PETRA.DiplomacyManager.prototype.checkEvents = function (gameState, events) {
  ...
  for (let evt of events.TributeExchanged) {
    if (
      evt.to === PlayerID &&
      !gameState.isPlayerAlly(evt.from) &&
      this.receivedDiplomacyRequests.has(evt.from)
    ) {
      let request = this.receivedDiplomacyRequests.get(evt.from);
      if (
        request.status === "waitingForTribute" &&
        request.type in evt.amounts
      ) {
    ...
    if (evt.to !== PlayerID || !gameState.isPlayerAlly(evt.from)) continue;
    let tributes = 0;
    for (let key in evt.amounts) {
      if (key === "food") tributes += evt.amounts[key];
      else tributes += 2 * evt.amounts[key];
    }
    this.Config.personality.cooperative = Math.min(
      1,
      this.Config.personality.cooperative + 0.0001 * tributes
    );
  }
  ...
  for (let evt of events.Attacked) {
    ...
    if (
      ...
      !gameState.isPlayerAlly(attacker.owner())
    )
      continue;
    this.Config.personality.cooperative = Math.min(
      1,
      this.Config.personality.cooperative + 0.003
    );
  }

  if (
    events.DiplomacyChanged.length ||
    events.PlayerDefeated.length ||
    events.CeasefireEnded.length
  )
    this.lastManStandingCheck(gameState);

  for (let evt of events.DiplomacyChanged) {
    if (evt.otherPlayer !== PlayerID) continue;

    if (this.sentDiplomacyRequests.has(evt.player)) {
      // If another player has accepted a diplomacy request we sent
      let sentRequest = this.sentDiplomacyRequests.get(evt.player);
      if (
        (gameState.sharedScript.playersData[evt.player].isAlly[PlayerID] &&
          sentRequest.requestType === "ally") ||
        (gameState.sharedScript.playersData[evt.player].isNeutral[PlayerID] &&
          sentRequest.requestType === "neutral")
      )
        this.changePlayerDiplomacy(
          gameState,
          evt.player,
          sentRequest.requestType
        );
    ...
    if (
      request !== undefined &&
      ((!gameState.sharedScript.playersData[evt.player].isAlly[PlayerID] &&
        request.requestType === "ally") ||
        (gameState.sharedScript.playersData[evt.player].isEnemy[PlayerID] &&
          request.requestType === "neutral"))
    ) {
      // a player that had requested to be allies changed their stance with us
      if (request.status === "accepted") request.status = "allianceBroken";
      ...
      gameState.sharedScript.playersData[evt.player].isAlly[PlayerID] &&
      gameState.isPlayerNeutral(evt.player)
    )
  ...
  for (let evt of events.TributeRequest) {
    ...
    if (evt.resourceWanted < availableResources[evt.resourceType]) {
      ...
      if (this.Config.debug > 0)
        API3.warn(
          "Responding to tribute request from AI player " +
            evt.source +
            " with " +
...
PETRA.DiplomacyManager.prototype.lastManStandingCheck = function (gameState) {
  ...
  if (!gameState.hasAllies() && !gameState.hasNeutrals()) return;
  ...
  for (let i = 1; i < gameState.sharedScript.playersData.length; ++i) {
    if (i === PlayerID || gameState.ai.HQ.attackManager.defeated[i]) continue;

    turnFactor = gameState.getEntities(i).length;

    if (gameState.isPlayerNeutral(i))
      // be more inclined to turn against neutral players
      turnFactor += this.betrayWeighting;

    if (gameState.getVictoryConditions().has("wonder")) {
      let wonder = gameState
        .getEnemyStructures(i)
        .filter(API3.Filters.byClass("Wonder"))[0];
      if (wonder) {
  ...
  if (playerToTurnAgainst) {
    ...
    if (request && request.status !== "allianceBroken") {
      if (request.status === "waitingForTribute")
        PETRA.chatAnswerRequestDiplomacy(
          gameState,
          player,
          request.requestType,
          "decline"
      ...
      request.status =
        request.status === "accepted" ? "allianceBroken" : "declinedRequest";
    }
 ...
 */
PETRA.DiplomacyManager.prototype.handleDiplomacyRequest = function (
  gameState,
  player,
  requestType
) {
  if (gameState.sharedScript.playersData[PlayerID].teamsLocked) return;
  let response;
  let requiredTribute;
  let request = this.receivedDiplomacyRequests.get(player);
  let moreEnemiesThanAllies =
  ...
  if (
    ...
    if (request.status === "declinedRequest") response = "decline";
    else if (request.status === "allianceBroken")
      // Previous alliance was broken, so decline
    ...
    (requestType === "ally" &&
      gameState.getEntities(player).length <
        gameState.getOwnEntities().length &&
      randBool(0.4)) ||
    ...
    if (resTribCodes.length) {
      requiredTribute = gameState.ai.HQ.pickMostNeededResources(
        gameState,
        resTribCodes
      )[0];
      response = "acceptWithTribute";
      requiredTribute.wanted = Math.max(
        1000,
        gameState.getOwnUnits().length * (requestType === "ally" ? 10 : 5)
      );
      this.receivedDiplomacyRequests.set(player, {
...
PETRA.DiplomacyManager.prototype.changePlayerDiplomacy = function (
  gameState,
  player,
  newDiplomaticStance
) {
  if (
    gameState.isPlayerEnemy(player) &&
    (newDiplomaticStance === "ally" || newDiplomaticStance === "neutral")
  )
    gameState.ai.HQ.attackManager.cancelAttacksAgainstPlayer(gameState, player);
  Engine.PostCommand(PlayerID, {
    type: "diplomacy",
    player: player,
    to: newDiplomaticStance,
  });
  if (this.Config.debug > 1)
    API3.warn(
      "diplomacy stance with player " +
        player +
        " is now " +
        newDiplomaticStance
    );
...
PETRA.DiplomacyManager.prototype.checkRequestedTributes = function (gameState) {
  for (let [player, data] of this.receivedDiplomacyRequests)
    if (
      data.status === "waitingForTribute" &&
      gameState.ai.elapsedTime > data.warnTime
    ) {
      if (data.sentWarning) {
...
PETRA.DiplomacyManager.prototype.sendDiplomacyRequest = function (gameState) {
  ...
  for (let i = 1; i < gameState.sharedScript.playersData.length; ++i) {
    ...
    if (
      i === PlayerID ||
      gameState.isPlayerMutualAlly(i) ||
      gameState.ai.HQ.attackManager.defeated[i] ||
      gameState.ai.HQ.attackManager.currentEnemyPlayer === i ||
      this.sentDiplomacyRequests.get(i) !== undefined ||
      (request && request.status === "declinedRequest")
    )
      continue;

    for (let j = 1; j < gameState.sharedScript.playersData.length; ++j) {
      if (
        gameState.sharedScript.playersData[i].isEnemy[j] &&
        gameState.isPlayerEnemy(j) &&
        !gameState.ai.HQ.attackManager.defeated[j]
      )
        ++mutualEnemies;

      if (mutualEnemies < max) continue;
  ...
  if (this.Config.debug > 0)
    API3.warn(
      "Sending diplomacy request to player " + player + " with " + requestType
    );
  Engine.PostCommand(PlayerID, {
...
PETRA.DiplomacyManager.prototype.checkSentDiplomacyRequests = function (
  gameState
) {
  for (let [player, data] of this.sentDiplomacyRequests)
    if (
      gameState.ai.elapsedTime > data.timeSent + 60 &&
      !gameState.ai.HQ.saveResources &&
      gameState.getPopulation() > 70
    ) {
      PETRA.chatNewRequestDiplomacy(
        gameState,
        player,
        data.requestType,
        "requestExpired"
...
PETRA.DiplomacyManager.prototype.update = function (gameState, events) {
  ...
  if (
    Resources.GetTributableCodes().length &&
    !gameState.ai.HQ.saveResources &&
    gameState.ai.elapsedTime > this.nextTributeUpdate
  )
  ...
  if (gameState.ai.elapsedTime > this.sentDiplomacyRequestLapseTime) {
    this.sentDiplomacyRequestLapseTime =
      gameState.ai.elapsedTime + 300 + randFloat(10, 100);
    let numEnemies = gameState.getEnemies().length;
    // Don't consider gaia
    if (
      numEnemies > 2 &&
      gameState.getMutualAllies().length < numEnemies - 1 &&
      randBool(0.1)
    )

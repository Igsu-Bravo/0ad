...
PETRA.QueueManager = function (Config, queues) {
  ...
  this.queues = queues;
  this.priorities = {};
  for (let i in Config.priorities) this.priorities[i] = Config.priorities[i];
  this.accounts = {};
...
PETRA.QueueManager.prototype.getAvailableResources = function (gameState) {
  ...
  for (let key in this.queues) resources.subtract(this.accounts[key]);
  return resources;
};

PETRA.QueueManager.prototype.getTotalAccountedResources = function () {
  let resources = new API3.Resources();
  for (let key in this.queues) resources.add(this.accounts[key]);
  return resources;
};
...
PETRA.QueueManager.prototype.currentNeeds = function (gameState) {
  ...
  for (let q of this.queueArrays) {
    let queue = q[1];
    if (!queue.hasQueuedUnits() || !queue.plans[0].isGo(gameState)) continue;
...
PETRA.QueueManager.prototype.wantedGatherRates = function (gameState) {
  ...
  if (gameState.ai.playedTurn === 0) {
    ...
    for (let res of Resources.GetCodes())
      ret[res] =
        this.Config.queues.firstTurn[res] ||
        this.Config.queues.firstTurn.default;
    return ret;
  }
  ...
  for (let res of Resources.GetCodes()) {
    totalShort[res] =
      this.Config.queues.short[res] || this.Config.queues.short.default;
    totalMedium[res] =
      this.Config.queues.medium[res] || this.Config.queues.medium.default;
    totalLong[res] =
      this.Config.queues.long[res] || this.Config.queues.long.default;
  }
  let total;
  // queueArrays because it's faster.
  for (let q of this.queueArrays) {
    ...
    for (let j = 0; j < queue.length(); ++j) {
      ...
      if (queue.plans[j].isGo(gameState)) {
        if (j === 0) total = totalShort;
        else total = totalMedium;
      } else total = totalLong;
      for (let type in total) total[type] += cost[type];
      if (!queue.plans[j].isGo(gameState)) break;
    }
  }
  // global rates
  let rates = {};
  ...
  for (let res of Resources.GetCodes()) {
    ...
    rates[res] =
      totalShort[res] / 30 + totalMedium[res] / 60 + totalLong[res] / 300;
  }
...
PETRA.QueueManager.prototype.printQueues = function (gameState) {
  ...
  for (let i in this.queues) {
    ...
    if (q.hasQueuedUnits()) {
      ...
      API3.warn(
        " while maxAccountWanted(0.6) is " +
          uneval(q.maxAccountWanted(gameState, 0.6))
      );
    }
    for (let plan of q.plans) {
      let qStr = "     " + plan.type + " ";
      if (plan.number) qStr += "x" + plan.number;
      qStr += "   isGo " + plan.isGo(gameState);
      API3.warn(qStr);
    }
  }
  API3.warn("Accounts");
  for (let p in this.accounts) API3.warn(p + ": " + uneval(this.accounts[p]));
  ...
  API3.warn(
    "Wanted Gather Rates: " +
      uneval(gameState.ai.HQ.GetWantedGatherRates(gameState))
  );
  API3.warn(
    "Current Gather Rates: " +
      uneval(gameState.ai.HQ.GetCurrentGatherRates(gameState))
  );
  API3.warn(
    "Most needed resources: " +
      uneval(gameState.ai.HQ.pickMostNeededResources(gameState))
  );
  API3.warn("------------------------------------");
};

PETRA.QueueManager.prototype.clear = function () {
  for (let i in this.queues) this.queues[i].empty();
};
...
PETRA.QueueManager.prototype.setAccounts = function (gameState, cost, i) {
  ...
  for (let res of Resources.GetCodes()) {
    if (this.accounts[i][res] >= cost[res]) continue;
    this.accounts[i][res] += Math.min(
      available[res],
      cost[res] - this.accounts[i][res]
    );
  }
};
...
PETRA.QueueManager.prototype.transferAccounts = function (cost, i, j) {
  for (let res of Resources.GetCodes()) {
    if (this.accounts[j][res] >= cost[res]) continue;
    let diff = Math.min(
      this.accounts[i][res],
      cost[res] - this.accounts[j][res]
    );
...
PETRA.QueueManager.prototype.distributeResources = function (gameState) {
  ...
  for (let res of Resources.GetCodes()) {
    if (availableRes[res] < 0) {
      // rescale the accounts if we've spent resources already accounted (e.g. by bartering)
    ...
    for (let j in this.queues) {
      ...
      if (
        this.queues[j].hasQueuedUnits() &&
        this.accounts[j][res] < queueCost[res] &&
        !this.queues[j].paused
      ) {
        ...
        if (tempPrio[j]) totalPriority += tempPrio[j];
    ...
    if (missing && available > 0) {
      // distribute the rest (due to floor) in any queue
      for (let j in tempPrio) {
    ...
    if (available < 0)
      API3.warn(
        "Petra: problem with remaining " + res + " in queueManager " + available
      );
  }
};
...
PETRA.QueueManager.prototype.switchResource = function (gameState, res) {
  ...
  for (let j in this.queues) {
    if (!this.queues[j].hasQueuedUnits() || this.queues[j].paused) continue;

    let queue = this.queues[j];
    let queueCost = queue.maxAccountWanted(gameState, 0);
    if (this.accounts[j][res] >= queueCost[res]) continue;
...
PETRA.QueueManager.prototype.startNextItems = function (gameState) {
  for (let q of this.queueArrays) {
    ...
    if (queue.hasQueuedUnits() && !queue.paused) {
      ...
      if (
        this.accounts[name].canAfford(item.getCost()) &&
        item.canStart(gameState)
      ) {
...
PETRA.QueueManager.prototype.update = function (gameState) {
  ...
  for (let i in this.queues) {
    ...
    API3.warn(
      "QueueManager received bad priorities, please report this error: " +
...
PETRA.QueueManager.prototype.checkPausedQueues = function (gameState) {
  ...
  let workersMin = Math.min(
    Math.max(12, 24 * this.Config.popScaling),
    this.Config.Economy.popPhase2
  );
  for (let q in this.queues) {
    ...
    if (toBePaused) {
      ...
      if (
        q == "ships" &&
        gameState.ai.HQ.needFish &&
        !gameState.ai.HQ.navalManager.ships
          .filter(API3.Filters.byClass("FishingBoat"))
          .hasEntities()
      )
        toBePaused = false;
    }
    ...
    } else if (queue.paused && !toBePaused) queue.paused = false;
    ...
    if (q.indexOf("plan_") != -1 && numWorkers < workersMin && queue.plans[0]) {
      ...
      if (queue.plans[1]) queue.plans[1].number = 1;
    }
  }
};

PETRA.QueueManager.prototype.canAfford = function (queue, cost) {
  if (!this.accounts[queue]) return false;
...
PETRA.QueueManager.prototype.addQueue = function (queueName, priority) {
  ...
  this.queueArrays = [];
  for (let q in this.queues) this.queueArrays.push([q, this.queues[q]]);
  let priorities = this.priorities;
  this.queueArrays.sort((a, b) => priorities[b[0]] - priorities[a[0]]);
};
...
PETRA.QueueManager.prototype.changePriority = function (
  queueName,
  newPriority
) {
...
PETRA.QueueManager.prototype.Serialize = function () {
  ...
  for (let q in this.queues) {
    ...
    if (this.Config.debug == -100)
      API3.warn(
        "queueManager serialization: queue " +
          q +
          " >>> " +
          uneval(queues[q]) +
          " with accounts " +
...
PETRA.QueueManager.prototype.Deserialize = function (gameState, data) {
  ...
  this.queueArrays.sort(
    (a, b) => data.priorities[b[0]] - data.priorities[a[0]]
  );
};

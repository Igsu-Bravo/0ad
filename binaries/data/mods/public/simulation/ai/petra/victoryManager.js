...
PETRA.VictoryManager = function (Config) {
  ...
  this.healersPerCriticalEnt =
    2 + Math.round(this.Config.personality.defensive * 2);
...
PETRA.VictoryManager.prototype.init = function (gameState) {
  if (gameState.getVictoryConditions().has("wonder")) {
    for (let wonder of gameState.getOwnEntitiesByClass("Wonder", true).values())
      this.criticalEnts.set(wonder.id(), {
        guardsAssigned: 0,
        guards: new Map(),
      });
  }
  ...
  if (gameState.getVictoryConditions().has("regicide")) {
    for (let hero of gameState.getOwnEntitiesByClass("Hero", true).values()) {
      ...
      if (hero.getStance() != defaultStance) hero.setStance(defaultStance);
...
PETRA.VictoryManager.prototype.checkEvents = function (gameState, events) {
  if (gameState.getVictoryConditions().has("wonder")) {
    ...
    for (let evt of events.ConstructionFinished) {
      if (!evt || !evt.newentity) continue;

      let ent = gameState.getEntityById(evt.newentity);
      if (ent && ent.isOwn(PlayerID) && ent.hasClass("Wonder"))
        this.criticalEnts.set(ent.id(), {
          guardsAssigned: 0,
          guards: new Map(),
        });
    }
  ...
  if (gameState.getVictoryConditions().has("regicide")) {
    for (let evt of events.Attacked) {
      if (!this.criticalEnts.has(evt.target)) continue;

      let target = gameState.getEntityById(evt.target);
      if (
        !target ||
        !target.position() ||
        target.healthLevel() > this.Config.garrisonHealthLevel.high
      )
        continue;
      ...
      if (plan != -2 && plan != -3) {
        ...
        if (plan >= 0) {
          ...
          if (attackPlan) attackPlan.removeUnit(target, true);
        }

        if (target.getMetadata(PlayerID, "PartOfArmy")) {
          let army = gameState.ai.HQ.defenseManager.getArmy(
            target.getMetadata(PlayerID, "PartOfArmy")
          );
          if (army) army.removeOwn(gameState, target.id());
        }

        hero.garrisonEmergency =
          target.healthLevel() < this.Config.garrisonHealthLevel.low;
        this.pickCriticalEntRetreatLocation(
          gameState,
          target,
          hero.garrisonEmergency
        );
    ...
    for (let evt of events.TrainingFinished)
      for (let entId of evt.entities) {
        ...
        if (
          ent &&
          ent.isOwn(PlayerID) &&
          ent.getMetadata(PlayerID, "role") == "criticalEntHealer"
        )
          this.assignGuardToCriticalEnt(gameState, ent);
      }

    for (let evt of events.Garrison) {
      if (!this.criticalEnts.has(evt.entity)) continue;

      let hero = this.criticalEnts.get(evt.entity);
      if (hero.garrisonEmergency) hero.garrisonEmergency = false;

      let holderEnt = gameState.getEntityById(evt.holder);
      if (!holderEnt) continue;
      ...
      for (let guardId of hero.guards.keys()) {
        ...
        if (pos) guardEnt.moveToRange(pos[0], pos[1], radius, radius + 5);
      }
    }
  }

  for (let evt of events.EntityRenamed) {
    if (!this.guardEnts.has(evt.entity)) continue;
    for (let data of this.criticalEnts.values()) {
      if (!data.guards.has(evt.entity)) continue;
  ...
  for (let evt of events.Destroy) {
    if (!evt.entityObj || evt.entityObj.owner() != PlayerID) continue;
    ...
    if (!this.guardEnts.has(entId)) continue;

    for (let data of this.criticalEnts.values())
      if (data.guards.has(entId)) {
        data.guards.delete(entId);
        if (evt.entityObj.hasClass("Healer")) --data.healersAssigned;
        else --data.guardsAssigned;
        break;
      }
  ...
  for (let evt of events.UnGarrison) {
    ...
    if (
      (ent.getMetadata(PlayerID, "role") == "criticalEntHealer" ||
        ent.getMetadata(PlayerID, "role") == "criticalEntGuard") &&
      !this.guardEnts.get(evt.entity)
    ) {
      this.assignGuardToCriticalEnt(
        gameState,
        ent,
        ent.getMetadata(PlayerID, "guardedEnt")
      );
      continue;
    }
    ...
    for (let [id, isGuarding] of this.guardEnts) {
      if (criticalEnt.guards.size >= this.healersPerCriticalEnt) break;
...
PETRA.VictoryManager.prototype.removeCriticalEnt = function (
  gameState,
  criticalEntId
) {
  for (let [guardId, role] of this.criticalEnts.get(criticalEntId).guards) {
    ...
    if (role == "healer") this.guardEnts.set(guardId, false);
    else {
 ...
 */
PETRA.VictoryManager.prototype.manageCriticalEntHealers = function (
  gameState,
  queues
) {
  if (
    gameState.ai.HQ.saveResources ||
    queues.healer.hasQueuedUnits() ||
    !gameState.getOwnEntitiesByClass("Temple", true).hasEntities() ||
    this.guardEnts.size >
      Math.min(gameState.getPopulationMax() / 10, gameState.getPopulation() / 4)
  )
    return;

  for (let data of this.criticalEnts.values()) {
    if (
      data.healersAssigned === undefined ||
      data.healersAssigned >= this.healersPerCriticalEnt
    )
      continue;
    let template = gameState.applyCiv("units/{civ}/support_healer_b");
    queues.healer.addPlan(
      new PETRA.TrainingPlan(
        gameState,
        template,
        { role: "criticalEntHealer", base: 0 },
        1,
        1
      )
    );
...
PETRA.VictoryManager.prototype.manageCriticalEntGuards = function (gameState) {
  ...
  if (numWorkers < 20) {
    for (let data of this.criticalEnts.values()) {
      for (let guardId of data.guards.keys()) {
        ...
        if (
          !guardEnt ||
          !guardEnt.hasClass("CitizenSoldier") ||
  ...
  for (let [id, data] of this.criticalEnts) {
    ...
    if (!criticalEnt) continue;

    let militaryGuardsPerCriticalEnt =
      (criticalEnt.hasClass("Wonder") ? 10 : 4) +
      Math.round(this.Config.personality.defensive * 5);

    if (data.guardsAssigned >= militaryGuardsPerCriticalEnt) continue;
    ...
    for (let checkForSameAccess of [true, false]) {
      ...
      for (let entity of gameState
        .getOwnEntitiesByClass("Champion", true)
        .values()) {
        if (
          !this.tryAssignMilitaryGuard(
            gameState,
            entity,
            criticalEnt,
            checkForSameAccess
          ...
          continue;
        if (++data.guardsAssigned >= militaryGuardsPerCriticalEnt) break;
      }

      if (
        data.guardsAssigned >= militaryGuardsPerCriticalEnt ||
        numWorkers <= minWorkers + deltaWorkers * data.guardsAssigned
      )
        break;
      ...
      for (let entity of gameState
        .getOwnEntitiesByClass("Soldier", true)
        .values()) {
        if (
          !this.tryAssignMilitaryGuard(
            gameState,
...
PETRA.VictoryManager.prototype.tryAssignMilitaryGuard = function (
  gameState,
  guardEnt,
  criticalEnt,
  checkForSameAccess
  ...
  if (
    guardEnt.getMetadata(PlayerID, "plan") !== undefined ||
    guardEnt.getMetadata(PlayerID, "transport") !== undefined ||
    this.criticalEnts.has(guardEnt.id()) ||
    (checkForSameAccess &&
      (!guardEnt.position() ||
        !criticalEnt.position() ||
        PETRA.getLandAccess(gameState, criticalEnt) !=
          PETRA.getLandAccess(gameState, guardEnt)))
  )
    return false;
...
PETRA.VictoryManager.prototype.pickCriticalEntRetreatLocation = function (
  gameState,
  criticalEnt,
  emergency
) {
  gameState.ai.HQ.defenseManager.garrisonAttackedUnit(
    gameState,
    criticalEnt,
    emergency
  );
 ...
 */
PETRA.VictoryManager.prototype.assignGuardToCriticalEnt = function (
  gameState,
  guardEnt,
  criticalEntId
) {
  ...
  if (!criticalEntId) {
    ...
    for (let [id, data] of this.criticalEnts) {
      if (
        isHealer &&
        (data.healersAssigned === undefined || data.healersAssigned > min)
      )
        continue;
      if (!isHealer && data.guardsAssigned > min) continue;
    ...
    if (criticalEntId) {
      ...
      if (isHealer) ++data.healersAssigned;
      else ++data.guardsAssigned;
    }
  }
  ...
  if (guardEntAccess == criticalEntAccess) {
    ...
    let guardRole =
      guardEnt.getMetadata(PlayerID, "role") == "criticalEntHealer"
        ? "healer"
        : "guard";
    ...
    if (
      criticalEnt.hasClass("Structure") &&
      criticalEnt.getMetadata(PlayerID, "base") !== undefined
    )
      guardEnt.setMetadata(
        PlayerID,
        "base",
        criticalEnt.getMetadata(PlayerID, "base")
      );
  } else
...
PETRA.VictoryManager.prototype.resetCaptureGaiaRelic = function (gameState) {
  ...
  this.tryCaptureGaiaRelicLapseTime =
    gameState.ai.elapsedTime + 240 - 30 * (this.Config.difficulty - 3);
  this.tryCaptureGaiaRelic = false;
};
...
PETRA.VictoryManager.prototype.update = function (gameState, events, queues) {
  ...
  if (gameState.ai.playedTurn == 1) this.init(gameState);
  ...
  if (gameState.getVictoryConditions().has("regicide")) {
    for (let id of this.criticalEnts.keys()) {
      ...
      if (
        ent &&
        ent.healthLevel() > this.Config.garrisonHealthLevel.high &&
        ent.hasClass("Soldier") &&
  ...
  if (gameState.getVictoryConditions().has("capture_the_relic")) {
    if (
      !this.tryCaptureGaiaRelic &&
      gameState.ai.elapsedTime > this.tryCaptureGaiaRelicLapseTime
    )
    ...
    for (let relic of allGaiaRelics.values()) {
      ...
      for (let ent of gameState.getOwnUnits().values()) {
        if (!ent.position() || !ent.visionRange()) continue;
        if (
          API3.SquareVectorDistance(ent.position(), relicPosition) >
          Math.square(ent.visionRange())
        )
          continue;
        this.targetedGaiaRelics.set(relic.id(), []);
        this.captureGaiaRelic(gameState, relic);
...
PETRA.VictoryManager.prototype.captureGaiaRelic = function (gameState, relic) {
  ...
  for (let plan of plans) {
    ...
    for (let ent of attack.unitCollection.values())
      capture +=
        ent.captureStrength() * PETRA.getAttackBonus(ent, relic, "Capture");
  }
  // No need to make a new attack if already enough units
  if (capture > sumCapturePoints / 50) return;
  let relicPosition = relic.position();
  let access = PETRA.getLandAccess(gameState, relic);
  let units = gameState
    .getOwnUnits()
    .filter((ent) => {
      if (!ent.position() || !ent.canCapture(relic)) return false;
      if (ent.getMetadata(PlayerID, "transport") !== undefined) return false;
      if (ent.getMetadata(PlayerID, "PartOfArmy") !== undefined) return false;
      let plan = ent.getMetadata(PlayerID, "plan");
      if (plan == -2 || plan == -3) return false;
  ...
  for (let ent of units.values()) {
    capture +=
      ent.captureStrength() * PETRA.getAttackBonus(ent, relic, "Capture");
    expedition.push(ent);
    if (capture > sumCapturePoints / 25) break;
  }
  ...
  for (let ent of expedition) {
    ...
    if (!attack.rallyPoint) attack.rallyPoint = ent.position();
  }
  attack.forceStart();
  this.targetedGaiaRelics.get(relic.id()).push(plan);
};

PETRA.VictoryManager.prototype.abortCaptureGaiaRelic = function (
  gameState,
  relicId
) {
  for (let plan of this.targetedGaiaRelics.get(relicId)) {
    let attack = gameState.ai.HQ.attackManager.getPlan(plan);
    if (attack) attack.Abort(gameState);
  }

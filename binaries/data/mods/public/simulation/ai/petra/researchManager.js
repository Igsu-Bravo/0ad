...
PETRA.ResearchManager.prototype.checkPhase = function (gameState, queues) {
  if (queues.majorTech.hasQueuedUnits()) return;
  ...
  let petraRequirements =
    ...
    (currentPhaseIndex == 2 &&
      gameState.ai.HQ.getAccountedWorkers(gameState) >
        this.Config.Economy.workPhase3) ||
    (currentPhaseIndex >= 3 &&
      gameState.ai.HQ.getAccountedWorkers(gameState) >
        this.Config.Economy.workPhase4);
  if (petraRequirements && gameState.hasResearchers(nextPhaseName, true)) {
    ...
    gameState.ai.queueManager.changePriority(
      "majorTech",
      gameState.ai.Config.priorities.majorTech
    );
    queues.majorTech.addPlan(
      new PETRA.ResearchPlan(gameState, nextPhaseName, true)
    );
  }
};

PETRA.ResearchManager.prototype.researchPopulationBonus = function (
  gameState,
  queues
) {
  if (queues.minorTech.hasQueuedUnits()) return;

  let techs = gameState.findAvailableTech();
  for (let tech of techs) {
    if (!tech[1]._template.modifications) continue;
...
PETRA.ResearchManager.prototype.researchTradeBonus = function (
  gameState,
  queues
) {
  if (queues.minorTech.hasQueuedUnits()) return;
  ...
  for (let tech of techs) {
    if (!tech[1]._template.modifications || !tech[1]._template.affects)
      continue;
    if (tech[1]._template.affects.indexOf("Trader") === -1) continue;
    // TODO may-be loop on all modifs and check if the effect if positive ?
    if (
...
PETRA.ResearchManager.prototype.researchWantedTechs = function (
  gameState,
  techs
) {
  let phase1 = gameState.currentPhase() === 1;
  let available = phase1
    ? gameState.ai.queueManager.getAvailableResources(gameState)
    : null;
  let numWorkers = phase1
    ? gameState.getOwnEntitiesByRole("worker", true).length
    : 0;
  for (let tech of techs) {
    ...
    if (tech[0] == "traditional_army_sele" || tech[0] == "reformed_army_sele")
      return {
        name: pickRandom(["traditional_army_sele", "reformed_army_sele"]),
    ...
    if (phase1) {
      ...
      if (10 * numWorkers < costMax) continue;
    }
    for (let i in template.modifications) {
      if (
        gameState.ai.HQ.navalMap &&
        template.modifications[i].value === "ResourceGatherer/Rates/food.fish"
      )
        return {
          name: tech[0],
          increasePriority: this.CostSum(template.cost) < 400,
        };
      else if (
        template.modifications[i].value === "ResourceGatherer/Rates/food.fruit"
      )
        return {
          name: tech[0],
          increasePriority: this.CostSum(template.cost) < 400,
        ...
        template.modifications[i].value === "ResourceGatherer/Rates/food.grain"
      )
        return { name: tech[0], increasePriority: false };
      else if (
        template.modifications[i].value === "ResourceGatherer/Rates/wood.tree"
      )
        return {
          name: tech[0],
          increasePriority: this.CostSum(template.cost) < 400,
        ...
        template.modifications[i].value.startsWith(
          "ResourceGatherer/Capacities"
        )
      )
        return { name: tech[0], increasePriority: false };
...
PETRA.ResearchManager.prototype.researchPreferredTechs = function (
  gameState,
  techs
) {
  let phase2 = gameState.currentPhase() === 2;
  let available = phase2
    ? gameState.ai.queueManager.getAvailableResources(gameState)
    : null;
  let numWorkers = phase2
    ? gameState.getOwnEntitiesByRole("worker", true).length
    : 0;
  for (let tech of techs) {
    if (!tech[1]._template.modifications) continue;
    ...
    for (let i in template.modifications) {
      if (
        template.modifications[i].value === "ResourceGatherer/Rates/stone.rock"
      )
        return {
          name: tech[0],
          increasePriority: this.CostSum(template.cost) < 400,
        ...
        template.modifications[i].value === "ResourceGatherer/Rates/metal.ore"
      )
        return {
          name: tech[0],
          increasePriority: this.CostSum(template.cost) < 400,
        ...
        template.modifications[i].value === "BuildingAI/DefaultArrowCount"
      )
        return {
          name: tech[0],
          increasePriority: this.CostSum(template.cost) < 400,
...
PETRA.ResearchManager.prototype.update = function (gameState, queues) {
  ...
  for (let i = 0; i < techs.length; ++i) {
    ...
    if (
      template.affects &&
      template.affects.length === 1 &&
      (template.affects[0] === "Healer" ||
        template.affects[0] === "Outpost" ||
        template.affects[0] === "Wall")
    ) {
      techs.splice(i--, 1);
      continue;
    }
    if (
      ...
      template.modifications.length === 1 &&
  ...
  queues.minorTech.addPlan(
    new PETRA.ResearchPlan(gameState, pickRandom(techs)[0])
  );
};

PETRA.ResearchManager.prototype.CostSum = function (cost) {
  let costSum = 0;
  for (let res in cost) costSum += cost[res];
  return costSum;
};

...
PETRA.Queue.prototype.addPlan = function (newPlan) {
  ...
  for (let plan of this.plans) {
    if (
      newPlan.category === "unit" &&
      plan.type == newPlan.type &&
      plan.number + newPlan.number <= plan.maxMerge
    ) {
...
PETRA.Queue.prototype.check = function (gameState) {
  while (this.plans.length > 0) {
    if (!this.plans[0].isInvalid(gameState)) return;
    let plan = this.plans.shift();
    if (plan.queueToReset)
      gameState.ai.queueManager.changePriority(
        plan.queueToReset,
        gameState.ai.Config.priorities[plan.queueToReset]
      );
  }
};

PETRA.Queue.prototype.getNext = function () {
  if (this.plans.length > 0) return this.plans[0];
  return null;
};
...
PETRA.Queue.prototype.queueCost = function () {
  let cost = new API3.Resources();
  for (let plan of this.plans) cost.add(plan.getCost());
  return cost;
};
...
PETRA.Queue.prototype.countQueuedUnits = function () {
  let count = 0;
  for (let plan of this.plans) count += plan.number;
  return count;
};

PETRA.Queue.prototype.hasQueuedUnitsWithClass = function (classe) {
  return this.plans.some(
    (plan) => plan.template && plan.template.hasClass(classe)
  );
};
...
PETRA.Queue.prototype.countQueuedUnitsWithClass = function (classe) {
  let count = 0;
  for (let plan of this.plans)
    if (plan.template && plan.template.hasClass(classe)) count += plan.number;
  return count;
};
...
PETRA.Queue.prototype.Serialize = function () {
  let plans = [];
  for (let plan of this.plans) plans.push(plan.Serialize());
...
PETRA.Queue.prototype.Deserialize = function (gameState, data) {
  ...
  for (let dataPlan of data.plans) {
    ...
    if (dataPlan.category == "unit")
      ...
      API3.warn(
        "Petra deserialization error: plan unknown " + uneval(dataPlan)
      );
      continue;
    }

...
PETRA.BuildManager.prototype.incrementBuilderCounters = function (
  civ,
  ent,
  increment
) {
  for (let buildable of ent.buildableEntities(civ)) {
    if (this.builderCounters.has(buildable)) {
      ...
      if (count < 0) {
        API3.warn(
          " Petra error in incrementBuilderCounters for " +
            buildable +
            " with count < 0"
        );
        continue;
    ...
    } else if (increment > 0) this.builderCounters.set(buildable, increment);
    else
      API3.warn(
        " Petra error in incrementBuilderCounters for " +
          buildable +
...
PETRA.BuildManager.prototype.checkEvents = function (gameState, events) {
  ...
  for (let evt of events.Create) {
    if (events.Destroy.some((e) => e.entity == evt.entity)) continue;
 ...
 */
PETRA.BuildManager.prototype.findStructuresByFilter = function (
  gameState,
  filter
) {
  const result = [];
  for (let [templateName, count] of this.builderCounters) {
    if (!count || gameState.isTemplateDisabled(templateName)) continue;
    let template = gameState.getTemplate(templateName);
    if (!template || !template.available(gameState)) continue;
    if (filter.func(template)) result.push(templateName);
  }
  return result;
};
 ...
 */
PETRA.BuildManager.prototype.findStructureWithClass = function (
  gameState,
  classes
) {
  return this.findStructuresByFilter(
    gameState,
    API3.Filters.byClasses(classes)
  )[0];
};
...
PETRA.BuildManager.prototype.setUnbuildable = function (
  ...
  if (!this.unbuildables.has(template))
    this.unbuildables.set(template, {
      reason: reason,
      time: gameState.ai.elapsedTime + time,
    });
  else {
...
PETRA.BuildManager.prototype.numberMissingRoom = function (gameState) {
  ...
  for (let unbuildable of this.unbuildables.values())
    if (
      unbuildable.reason == "room" &&
      unbuildable.time > gameState.ai.elapsedTime
    )
      ++num;
  return num;
};
...
PETRA.BuildManager.prototype.resetMissingRoom = function (gameState) {
  for (let [key, unbuildable] of this.unbuildables)
    if (unbuildable.reason == "room") this.unbuildables.delete(key);
};

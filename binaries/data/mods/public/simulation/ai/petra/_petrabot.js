  ...
  if (this.isDeserialized) {
    ...
    for (let key in this.savedEvents) {
      for (let i in this.savedEvents[key]) {
        if (!this.savedEvents[key][i].entityObj) continue;
        let evt = this.savedEvents[key][i];
        let evtmod = {};
        for (let keyevt in evt) {
          evtmod[keyevt] = evt[keyevt];
          evtmod.entityObj = new API3.Entity(
    ...
    this.queues = {};
    for (let i in this.Config.priorities) this.queues[i] = new PETRA.Queue();
...
PETRA.PetraBot.prototype.OnUpdate = function (sharedScript) {
  ...
  if (!this.playedTurn || (this.turn + this.player) % 8 == 5) {
    ...
    for (let i in this.savedEvents) this.savedEvents[i] = [];
...
PETRA.PetraBot.prototype.Serialize = function () {
  ...
  for (let key in this.savedEvents) {
    ...
    for (let i in savedEvents[key]) {
      if (!savedEvents[key][i] || !savedEvents[key][i].entityObj) continue;
      let evt = savedEvents[key][i];
      let evtmod = {};
      for (let keyevt in evt) evtmod[keyevt] = evt[keyevt];

...
PETRA.createObstructionMap = function (gameState, accessIndex, template) {
  ...
  for (let k = 0; k < territoryMap.data.length; ++k) {
    ...
    let isConnected =
      (territoryMap.data[k] & PETRA.TERRITORY_BLINKING_MASK) == 0;
  ...
  let map = new API3.Map(
    gameState.sharedScript,
    "passability",
    obstructionTiles
  );
  map.setMaxVal(255);
  ...
  if (template && template.buildDistance()) {
    ...
    if (minDist) {
      ...
      if (obstructionRadius) minDist -= obstructionRadius.min;
      let fromClass = distance.FromClass;
      let cellSize = passabilityMap.cellSize;
      let cellDist = 1 + minDist / cellSize;
      let structures = gameState
        .getOwnStructures()
        .filter(API3.Filters.byClass(fromClass));
...
PETRA.createTerritoryMap = function (gameState) {
  ...
  ret.getOwner = function (p) {
    return this.point(p) & PETRA.TERRITORY_PLAYER_MASK;
  };
  ret.getOwnerIndex = function (p) {
    return this.map[p] & PETRA.TERRITORY_PLAYER_MASK;
  };
  ret.isBlinking = function (p) {
    return (this.point(p) & PETRA.TERRITORY_BLINKING_MASK) != 0;
  };
  return ret;
};
...
PETRA.createBorderMap = function (gameState) {
  ...
  if (gameState.circularMap) {
    ...
    for (let j = 0; j < map.length; ++j) {
      ...
      for (let k of ind) {
        if (passabilityMap.data[k] & obstructionMask) continue;

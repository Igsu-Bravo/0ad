...
PETRA.HQ = function (Config) {
  ...
  this.extraTowers = Math.round(
    Math.min(this.Config.difficulty, 3) * this.Config.personality.defensive
  );
  this.extraFortresses = Math.round(
    Math.max(Math.min(this.Config.difficulty - 1, 2), 0) *
      this.Config.personality.defensive
  );
...
PETRA.HQ.prototype.postinit = function (gameState) {
  ...
  for (let base of this.baseManagers)
    for (let j of base.territoryIndices) this.basesMap.map[j] = base.ID;

  for (let ent of gameState.getOwnEntities().values()) {
    if (!ent.resourceDropsiteTypes() || !ent.hasClass("Structure")) continue;
...
PETRA.HQ.prototype.createBase = function (gameState, ent, type) {
  ...
  let newbase;
  for (let base of this.baseManagers) {
    if (base.accessIndex != access) continue;
    if (type != "anchorless" && base.anchor) continue;
    if (type != "anchorless") {
      ...
      if (newbase && !base.anchor) continue;
      newbase = base;
      if (newbase.anchor) break;
    }
  }
  ...
  if (this.Config.debug > 0) {
    API3.warn(" ----------------------------------------------------------");
    API3.warn(
      " HQ createBase entrance avec access " + access + " and type " + type
    );
    API3.warn(
      " with access " +
        uneval(this.baseManagers.map((base) => base.accessIndex)) +
        " and base nbr " +
        uneval(this.baseManagers.map((base) => base.ID)) +
        " and anchor " +
        uneval(this.baseManagers.map((base) => !!base.anchor))
    );
  }
  ...
  } else newbase.reset(type);

  if (type != "anchorless") newbase.setAnchor(gameState, ent);
  else newbase.setAnchorlessEntity(gameState, ent);
...
PETRA.HQ.prototype.getSeaBetweenIndices = function (gameState, index1, index2) {
  ...
  if (
    path &&
    path.length == 3 &&
    gameState.ai.accessibility.regionType[path[1]] == "water"
  )
    return path[1];

  if (this.Config.debug > 1) {
    API3.warn(
      "bad path from " + index1 + " to " + index2 + " ??? " + uneval(path)
    );
    API3.warn(
      " regionLinks start " +
        uneval(gameState.ai.accessibility.regionLinks[index1])
    );
    API3.warn(
      " regionLinks end   " +
        uneval(gameState.ai.accessibility.regionLinks[index2])
    );
  }
  return undefined;
};
...
PETRA.HQ.prototype.checkEvents = function (gameState, events) {
  ...
  for (let evt of events.DiplomacyChanged) {
    if (evt.player != PlayerID && evt.otherPlayer != PlayerID) continue;
  ...
  for (let evt of events.Destroy) {
    ...
    if (
      evt &&
      !evt.SuccessfulFoundation &&
      evt.entityObj &&
      evt.metadata &&
      evt.metadata[PlayerID] &&
      evt.metadata[PlayerID].base
    ) {
      let ent = evt.entityObj;
      if (ent.owner() != PlayerID) continue;
      // A new base foundation was created and destroyed on the same (AI) turn
      if (
        evt.metadata[PlayerID].base == -1 ||
        evt.metadata[PlayerID].base == -2
      )
        continue;
      ...
      if (
        evt.metadata[PlayerID].baseAnchor &&
        evt.metadata[PlayerID].baseAnchor === true
      )
  ...
  for (let evt of events.EntityRenamed) {
    ...
    if (
      !ent ||
      ent.owner() != PlayerID ||
      ent.getMetadata(PlayerID, "base") === undefined
    )
      continue;
    let base = this.getBaseByID(ent.getMetadata(PlayerID, "base"));
    if (!base.anchorId || base.anchorId != evt.entity) continue;
  ...
  for (let evt of events.Create) {
    ...
    } else if (ent.getMetadata(PlayerID, "base") == -2) {
      // anchorless base around a dock
  ...
  for (let evt of events.ConstructionFinished) {
    if (evt.newentity == evt.entity)
      // repaired building
      continue;
    let ent = gameState.getEntityById(evt.newentity);
    if (!ent || ent.owner() != PlayerID) continue;
    if (ent.hasClass("Market") && this.maxFields) this.maxFields = false;
    if (ent.getMetadata(PlayerID, "base") === undefined) continue;
    let base = this.getBaseByID(ent.getMetadata(PlayerID, "base"));
    ...
    if (ent.getMetadata(PlayerID, "baseAnchor") === true) {
      if (base.constructing) base.constructing = false;
      addBase = true;
    }
  }
  ...
  for (let evt of events.OwnershipChanged) {
    ...
    if (ent.hasClass("Unit")) {
      ...
      if (ent.hasClass("Ship")) PETRA.setSeaAccess(gameState, ent);
      if (
        !ent.hasClasses(["Support", "Ship"]) &&
    ...
    if (ent.hasClass("CivCentre")) {
      ...
      else base = PETRA.getBestBase(gameState, ent) || this.baseManagers[0];
      base.assignEntity(gameState, ent);
      if (ent.decaying()) {
        if (
          ent.isGarrisonHolder() &&
          this.garrisonManager.addDecayingStructure(gameState, evt.entity, true)
        )
          continue;
  ...
  for (let evt of events.TrainingStarted) {
    let ent = gameState.getEntityById(evt.entity);
    if (!ent || !ent.isOwn(PlayerID)) continue;

    if (!ent._entity.trainingQueue || !ent._entity.trainingQueue.length)
      continue;
    let metadata = ent._entity.trainingQueue[0].metadata;
    if (metadata && metadata.garrisonType) ent.setRallyPoint(ent, "garrison");
    // trained units will autogarrison
  ...
  for (let evt of events.TrainingFinished) {
    for (let entId of evt.entities) {
      ...
      if (!ent.position()) {
        ...
        if (holder) this.garrisonManager.registerHolder(gameState, holder);
      ...
      if (ent.getMetadata(PlayerID, "role") == "worker") {
        ...
        if (ent.getMetadata(PlayerID, "base") === undefined) {
          ...
          base.assignEntity(gameState, ent);
        } else base = this.getBaseByID(ent.getMetadata(PlayerID, "base"));
        ...
        for (let dropsite of dropsites.values()) {
          if (
            !dropsite.position() ||
            PETRA.getLandAccess(gameState, dropsite) != access
          )
            continue;
        ...
        if (goal) ent.moveToRange(goal[0], goal[1]);
      }
    }
  }
  ...
  if (this.Config.difficulty < 2) return;
  for (let entId of this.decayingStructures) {
    ...
    if (ent && ent.decaying() && ent.isOwn(PlayerID)) {
      ...
      if (captureRatio < 0.5) continue;
      let decayToGaia = true;
      for (let i = 1; i < capture.length; ++i) {
        if (gameState.isPlayerAlly(i) || !capture[i]) continue;
        decayToGaia = false;
        break;
      }
      if (decayToGaia) continue;
      let ratioMax = 0.7 + randFloat(0, 0.1);
      for (let evt of events.Attacked) {
        if (ent.id() != evt.target) continue;
        ratioMax = 0.85 + randFloat(0, 0.1);
        break;
      }
      if (captureRatio > ratioMax) continue;
      ent.destroy();
    }
...
PETRA.HQ.prototype.checkPhaseRequirements = function (gameState, queues) {
  if (gameState.getNumberOfPhases() == this.currentPhase) return;

  let requirements = gameState.getPhaseEntityRequirements(
    this.currentPhase + 1
  );
  let plan;
  let queue;
  for (let entityReq of requirements) {
    ...
    if (entityReq.class == "Village" || entityReq.class == "NotField") continue;
    if (
      gameState.getOwnEntitiesByClass(entityReq.class, true).length >=
      entityReq.count
    )
      continue;
    switch (entityReq.class) {
      case "Town":
        if (
          !queues.economicBuilding.hasQueuedUnits() &&
          ...
          if (
            ...
            plan = new PETRA.ConstructionPlan(
              gameState,
      ...
      default:
        ...
        if (!queues[queue].hasQueuedUnits()) {
          let structure = this.buildManager.findStructureWithClass(gameState, [
            entityReq.class,
          ]);
          if (structure && this.canBuild(gameState, structure))
            plan = new PETRA.ConstructionPlan(gameState, structure, {
              phaseUp: true,
            });
        }
    }
...
PETRA.HQ.prototype.trainMoreWorkers = function (gameState, queues) {
  ...
  let templateDef = this.findBestTrainableUnit(
    gameState,
    classesDef,
    requirementsDef
  );
  ...
  gameState.getOwnUnits().forEach((ent) => {
    if (
      ent.getMetadata(PlayerID, "role") == "worker" &&
      ent.getMetadata(PlayerID, "plan") === undefined
    ) {
      ++numberOfWorkers;
      if (ent.hasClass("Support")) ++numberOfSupports;
    }
  });
  ...
  gameState.getOwnTrainingFacilities().forEach(function (ent) {
    for (let item of ent.trainingQueue()) {
      ...
      if (
        ...
        if (item.metadata.support) numberOfSupports += item.count;
      }
    }
  });
  ...
  let size =
    numberOfWorkers < 12 ? 1 : Math.min(5, Math.ceil(numberOfWorkers / 10));
  if (queues.villager.plans[0]) {
    queues.villager.plans[0].number = Math.min(
      queues.villager.plans[0].number,
      size
    );
    if (queues.villager.plans[1])
      queues.villager.plans[1].number = Math.min(
        queues.villager.plans[1].number,
        size
      );
  }
  if (queues.citizenSoldier.plans[0]) {
    queues.citizenSoldier.plans[0].number = Math.min(
      queues.citizenSoldier.plans[0].number,
      size
    );
    if (queues.citizenSoldier.plans[1])
      queues.citizenSoldier.plans[1].number = Math.min(
        queues.citizenSoldier.plans[1].number,
        size
      );
  }
  ...
  if (this.saveResources && numberTotal > this.Config.Economy.popPhase2 + 10)
    return;
  if (
    numberTotal > this.targetNumWorkers ||
    (numberTotal >= this.Config.Economy.popPhase2 &&
      this.currentPhase == 1 &&
  ...
  if (
    numberQueued > 50 ||
    (numberOfQueuedSupports > 20 && numberOfQueuedSoldiers > 20) ||
    numberInTraining > 15
  )
    return;
  ...
  let alpha = 0.85;
  if (
    !gameState.isTemplateAvailable(gameState.applyCiv("structures/{civ}/field"))
  )
    supportRatio = Math.min(this.supportRatio, 0.1);
  if (
    this.attackManager.rushNumber < this.attackManager.maxRushes ||
    this.attackManager.upcomingAttacks.Rush.length
  )
    alpha = 0.7;
  if (gameState.isCeasefireActive())
    alpha +=
      ((1 - alpha) *
        Math.min(Math.max(gameState.ceasefireTimeRemaining - 120, 0), 180)) /
      180;
  let supportMax = supportRatio * this.targetNumWorkers;
  let supportNum =
    supportMax * (1 - Math.exp((-alpha * numberTotal) / supportMax));
  ...
  if (!templateDef || numberOfSupports + numberOfQueuedSupports > supportNum) {
    ...
    if (numberTotal < 45)
      requirements = [
        ["speed", 0.5],
        ["costsResource", 0.5, "stone"],
        ["costsResource", 0.5, "metal"],
      ];
    else requirements = [["strength", 1]];
  ...
  if (!template && templateDef)
    queues.villager.addPlan(
      new PETRA.TrainingPlan(
        gameState,
        templateDef,
        { role: "worker", base: 0, support: true },
        size,
        size
      )
    );
  else if (template)
    queues.citizenSoldier.addPlan(
      new PETRA.TrainingPlan(
        gameState,
        template,
        { role: "worker", base: 0 },
...
PETRA.HQ.prototype.findBestTrainableUnit = function (
  gameState,
  classes,
  requirements
) {
  ...
  if (classes.indexOf("Hero") != -1)
    ...
    // We do not want siege tower as AI does not know how to use it
    units = gameState.findTrainableUnits(classes, ["SiegeTower"]);
  // We do not want hero when not explicitely specified
  else units = gameState.findTrainableUnits(classes, ["Hero"]);
  ...
  let availableResources =
    gameState.ai.queueManager.getAvailableResources(gameState); // available (gathered) resources
  for (let type in remainingResources) {
    if (availableResources[type] > 800) continue;
    if (remainingResources[type] > 800) continue;
    let costsResource = remainingResources[type] > 400 ? 0.6 : 0.2;
    let toAdd = true;
    for (let param of parameters) {
      if (param[0] != "costsResource" || param[2] != type) continue;
      param[1] = Math.min(param[1], costsResource);
      toAdd = false;
      break;
    }
    if (toAdd) parameters.push(["costsResource", costsResource, type]);
  }
  ...
  units.sort((a, b) => {
    ...
    for (let param of parameters) {
      if (param[0] == "strength") {
        aValue +=
          PETRA.getMaxStrength(
            a[1],
            gameState.ai.Config.debug,
            gameState.ai.Config.DamageTypeImportance
          ) * param[1];
        bValue +=
          PETRA.getMaxStrength(
            b[1],
        ...
        if (a[1].cost()[param[2]]) aValue *= param[1];
        if (b[1].cost()[param[2]]) bValue *= param[1];
      } else if (param[0] == "canGather") {
        // checking against wood, could be anything else really.
        if (
          a[1].resourceGatherRates() &&
          a[1].resourceGatherRates()["wood.tree"]
        )
          aValue *= param[1];
        if (
          b[1].resourceGatherRates() &&
          b[1].resourceGatherRates()["wood.tree"]
        )
          bValue *= param[1];
      } else API3.warn(" trainMoreUnits avec non prevu " + uneval(param));
    }
...
PETRA.HQ.prototype.bulkPickWorkers = function (gameState, baseRef, number) {
  ...
  for (let base of baseBest) {
    if (base.ID == baseRef.ID) continue;
    base.pickBuilders(gameState, workers, needed);
    if (workers.length >= number) break;
...
PETRA.HQ.prototype.getTotalResourceLevel = function (gameState) {
  let total = {};
  for (let res of Resources.GetCodes()) total[res] = 0;
  for (let base of this.baseManagers)
    for (let res in total) total[res] += base.getResourceLevel(gameState, res);
...
PETRA.HQ.prototype.GetWantedGatherRates = function (gameState) {
  if (!this.turnCache.wantedRates)
    this.turnCache.wantedRates =
      gameState.ai.queueManager.wantedGatherRates(gameState);
 ...
 */
PETRA.HQ.prototype.pickMostNeededResources = function (
  gameState,
  allowedResources = []
) {
  let wantedRates = this.GetWantedGatherRates(gameState);
  let currentRates = this.GetCurrentGatherRates(gameState);
  if (!allowedResources.length) allowedResources = Resources.GetCodes();

  let needed = [];
  for (let res of allowedResources)
    needed.push({
      type: res,
      wanted: wantedRates[res],
      current: currentRates[res],
    });
  ...
  needed.sort((a, b) => {
    ...
    if (a.current < a.wanted || (a.wanted && !b.wanted)) return -1;
    if (b.current < b.wanted || (b.wanted && !a.wanted)) return 1;
 ...
 */
PETRA.HQ.prototype.findEconomicCCLocation = function (
  gameState,
  template,
  resource,
  proximity,
  ...
  if (template.get("Footprint/Square"))
    halfSize =
      Math.max(
        +template.get("Footprint/Square/@depth"),
        +template.get("Footprint/Square/@width")
      ) / 2;
  else if (template.get("Footprint/Circle"))
    halfSize = +template.get("Footprint/Circle/@radius");

  let ccEnts = gameState.updatingGlobalCollection(
    "allCCs",
    API3.Filters.byClass("CivCentre")
  );
  const dpEnts = gameState
    .getOwnDropsites()
    .filter(API3.Filters.not(API3.Filters.byClasses(["CivCentre", "Unit"])));
  let ccList = [];
  for (let cc of ccEnts.values())
    ccList.push({
      ent: cc,
      pos: cc.position(),
      ally: gameState.isPlayerAlly(cc.owner()),
    });
  let dpList = [];
  for (let dp of dpEnts.values())
    dpList.push({
      ent: dp,
      pos: dp.position(),
      territory: this.territoryMap.getOwner(dp.position()),
    });

  let bestIdx;
  let bestVal;
  let radius = Math.ceil(
    template.obstructionRadius().max / obstructions.cellSize
  );
  let scale = 250 * 250;
  let proxyAccess;
  let nbShips = this.navalManager.transportShips.length;
  if (proximity) {
    // this is our first base
    // if our first base, ensure room around
    radius = Math.ceil(
      (template.obstructionRadius().max + 8) / obstructions.cellSize
    );
    ...
    let cellArea =
      gameState.getPassabilityMap().cellSize *
      gameState.getPassabilityMap().cellSize;
    proxyAccess = gameState.ai.accessibility.getAccessValue(proximity);
    if (
      proxyAccess < 2 ||
      cellArea * gameState.ai.accessibility.regionSize[proxyAccess] < 24000
    )
      scale = 400 * 400;
  }
  ...
  let reduce =
    (template.hasClass("Colony") ? 30 : 0) +
    30 * this.Config.personality.defensive;
  ...
  let cut = 60;
  if (fromStrategic || proximity)
    // be less restrictive
    cut = 30;

  for (let j = 0; j < this.territoryMap.length; ++j) {
    if (this.territoryMap.getOwnerIndex(j) != 0) continue;
    ...
    if (!this.landRegions[index]) continue;
    if (proxyAccess && nbShips == 0 && proxyAccess != index) continue;

    let norm = 0.5; // TODO adjust it, knowing that we will sum 5 maps
    // Checking distance to other cc
    let pos = [
      cellSize * ((j % width) + 0.5),
      cellSize * (Math.floor(j / width) + 0.5),
    ];
    // We will be more tolerant for cc around our oversea docks
    let oversea = false;

    if (proximity)
      // This is our first cc, let's do it near our units
      ...
      for (let cc of ccList) {
        ...
        accessible =
          accessible || index == PETRA.getLandAccess(gameState, cc.ent);
      }
      if (norm == 0) continue;
      ...
      if (minDist > maxAccessDisfavored) {
        // Disfavor if quite far from any allied cc
        if (!accessible) {
          if (minDist > maxNoAccessDisfavored) norm *= 0.5;
          else norm *= 0.8;
        } else norm *= 0.5;
      }
      ...
      oversea =
        !accessible &&
        dpList.some((dp) => PETRA.getLandAccess(gameState, dp.ent) == index);
      if (!oversea) {
    ...
    if (this.borderMap.map[j] & PETRA.fullBorder_Mask)
      // disfavor the borders of the map
      norm *= 0.5;
    ...
    if (oversea) val = Math.max(val, cut + 0.1);

    if (bestVal !== undefined && val < bestVal) continue;
    if (this.isDangerousLocation(gameState, pos, halfSize)) continue;
    bestVal = val;
    bestIdx = i;
  }
  ...
  if (bestVal === undefined) return false;
  if (this.Config.debug > 1)
    API3.warn(
      "we have found a base for " +
        resource +
        " with best (cut=" +
        cut +
        ") = " +
  ...
  // not good enough.
  if (bestVal < cut) return false;

  let x = ((bestIdx % obstructions.width) + 0.5) * obstructions.cellSize;
  let z =
    (Math.floor(bestIdx / obstructions.width) + 0.5) * obstructions.cellSize;
  ...
  for (let base of this.baseManagers) {
    if (!base.anchor || base.accessIndex == indexIdx) continue;
...
PETRA.HQ.prototype.findStrategicCCLocation = function (gameState, template) {
  ...
  if (numAllyCC < 2)
    return this.findEconomicCCLocation(
      gameState,
      template,
      "wood",
      undefined,
  ...
  let favoredDistance =
    (template.hasClass("Colony") ? 220 : 280) -
    40 * this.Config.personality.defensive;
  ...
  for (let j = 0; j < this.territoryMap.length; ++j) {
    ...
    for (let cc of ccList) {
      let dist = API3.SquareVectorDistance(cc.pos, pos);
      if (dist < 14000) {
        // Reject if too near from any cc
        minDist = 0;
        break;
      }
      if (!cc.ally) continue;
      if (dist < 62000) {
        // Reject if quite near from ally cc
        minDist = 0;
        break;
      }
      if (dist < minDist) minDist = dist;
      ...
      if (!distcc0 || dist < distcc0) {
        ...
        distcc2 = distcc1;
        distcc1 = dist;
      } else if (!distcc2 || dist < distcc2) distcc2 = dist;
    }
    if (minDist < 1 || (minDist > 170000 && !this.navalMap)) continue;
    ...
    if (this.borderMap.map[j] & PETRA.fullBorder_Mask) currentVal += 10000;

    if (bestVal !== undefined && currentVal > bestVal) continue;
    if (this.isDangerousLocation(gameState, pos, halfSize)) continue;
    bestVal = currentVal;
    bestIdx = i;
  }
  ...
  if (bestVal === undefined) return undefined;
...
PETRA.HQ.prototype.findMarketLocation = function (gameState, template) {
  let markets = gameState
    .updatingCollection(
      "diplo-ExclusiveAllyMarkets",
      API3.Filters.byClass("Trade"),
  ...
  if (!markets.length)
    // this is the first market. For the time being, place it arbitrarily by the ConstructionPlan
    return [-1, -1, -1, 0];

  // No need for more than one market when we cannot trade.
  if (!Resources.GetTradableCodes().length) return false;
  ...
  for (let j = 0; j < this.territoryMap.length; ++j) {
    ...
    if (this.borderMap.map[j] & PETRA.narrowFrontier_Mask) continue;
    if (this.basesMap.map[j] == 0)
      // only in our territory
      continue;
    ...
    if (bestVal !== undefined && maxVal < bestVal) continue;
    if (this.isDangerousLocation(gameState, pos, halfSize)) continue;
    bestVal = maxVal;
  ...
  if (bestVal === undefined)
    // no constraints. For the time being, place it arbitrarily by the ConstructionPlan
    return [-1, -1, -1, 0];
  let expectedGain = Math.round(
    bestGainMult * TradeGain(bestDistSq, gameState.sharedScript.mapSize)
  );
  if (this.Config.debug > 1)
  ...
  if (expectedGain < this.tradeManager.minimalGain) {
    if (
      template.hasClass("Market") &&
      !gameState.getOwnEntitiesByClass("Market", true).hasEntities()
    )
      idx = -1;
    // Needed by queueplanBuilding manager to keep that Market.
    else return false;
  } else idx = this.basesMap.map[bestJdx];
...
PETRA.HQ.prototype.findDefensiveLocation = function (gameState, template) {
  ...
  const ownStructures = gameState
    .getOwnStructures()
    .filter(API3.Filters.byClasses(["Fortress", "Tower"]))
    .toEntityArray();
  let enemyStructures = gameState
    .getEnemyStructures()
    .filter(API3.Filters.not(API3.Filters.byOwner(0)))
    .filter(API3.Filters.byClasses(["CivCentre", "Fortress", "Tower"]));
  if (!enemyStructures.hasEntities()) {
    // we may be in cease fire mode, build defense against neutrals
    enemyStructures = gameState
    ...
    if (!enemyStructures.hasEntities() && !gameState.getAlliedVictory())
      enemyStructures = gameState
        .getAllyStructures()
        .filter(API3.Filters.not(API3.Filters.byOwner(PlayerID)))
        .filter(API3.Filters.byClasses(["CivCentre", "Fortress", "Tower"]));
    if (!enemyStructures.hasEntities()) return undefined;
  }
  ...
  if (wonderMode) {
    wonders = gameState
      .getOwnStructures()
      .filter(API3.Filters.byClass("Wonder"))
      .toEntityArray();
    wonderMode = wonders.length != 0;
    if (wonderMode)
      wonderDistmin =
        (50 + wonders[0].footprintRadius()) *
        (50 + wonders[0].footprintRadius());
  }
  ...
  for (let j = 0; j < this.territoryMap.length; ++j) {
    if (!wonderMode) {
      ...
      if (!(this.borderMap.map[j] & PETRA.fullFrontier_Mask)) continue;
      if (this.borderMap.map[j] & PETRA.largeFrontier_Mask && isTower) continue;
    }
    if (this.basesMap.map[j] == 0)
      // inaccessible cell
      continue;
    ...
    if (minDist < 0 || minDist == Math.min()) continue;
    if (bestVal !== undefined && minDist > bestVal) continue;
    if (this.isDangerousLocation(gameState, pos, halfSize)) continue;
    bestVal = minDist;
    bestIdx = i;
    bestJdx = j;
...
PETRA.HQ.prototype.buildTemple = function (gameState, queues) {
  ...
  if (
    this.currentPhase < 3 &&
    !gameState.getVictoryConditions().has("regicide")
  )
    return;
  ...
  else if (!this.canBuild(gameState, templateName)) return;
  queues.economicBuilding.addPlan(
    new PETRA.ConstructionPlan(gameState, templateName)
  );
};
...
PETRA.HQ.prototype.buildMarket = function (gameState, queues) {
    ...
    if (!queues.economicBuilding.paused) {
      ...
      if (!queueManager.canAfford("economicBuilding", cost)) {
        for (let q in queueManager.queues) {
          if (q == "economicBuilding") continue;
          queueManager.transferAccounts(cost, q, "economicBuilding");
          if (queueManager.canAfford("economicBuilding", cost)) break;
        }
      }
    }
    return;
  ...
  gameState.ai.queueManager.changePriority(
    "economicBuilding",
    3 * this.Config.priorities.economicBuilding
  );
...
PETRA.HQ.prototype.buildFarmstead = function (gameState, queues) {
  ...
  if (gameState.getOwnEntitiesByClass("Farmstead", true).hasEntities()) return;
  ...
  if (!gameState.getOwnEntitiesByClass("Storehouse", true).hasEntities())
    return;
  if (!gameState.getOwnEntitiesByClass("House", true).hasEntities()) return;
  if (queues.economicBuilding.hasQueuedUnitsWithClass("DropsiteFood")) return;
  if (!this.canBuild(gameState, "structures/{civ}/farmstead")) return;

  queues.economicBuilding.addPlan(
    new PETRA.ConstructionPlan(gameState, "structures/{civ}/farmstead")
  );
};
...
PETRA.HQ.prototype.buildWonder = function (gameState, queues, force = false) {
  if (
    (queues.wonder && queues.wonder.hasQueuedUnits()) ||
  ...
  if (!force) {
    let template = gameState.getTemplate(
      gameState.applyCiv("structures/{civ}/wonder")
    );
    ...
    let lowLevel = 0;
    for (let res in cost) {
      if (resources[res] && resources[res] > 0.7 * cost[res]) ++highLevel;
      else if (!resources[res] || resources[res] < 0.3 * cost[res]) ++lowLevel;
    }
    if (highLevel == 0 || lowLevel > 1) return;
  }

  queues.wonder.addPlan(
    new PETRA.ConstructionPlan(gameState, "structures/{civ}/wonder")
  );
};

/** Build a corral, and train animals there */
PETRA.HQ.prototype.manageCorral = function (gameState, queues) {
  if (queues.corral.hasQueuedUnits()) return;

  let nCorral = gameState.getOwnEntitiesByClass("Corral", true).length;
  if (
    !nCorral ||
    (!gameState.isTemplateAvailable(
      gameState.applyCiv("structures/{civ}/field")
    ) &&
      nCorral < this.currentPhase &&
      gameState.getPopulation() > 30 * nCorral)
  ) {
    if (this.canBuild(gameState, "structures/{civ}/corral")) {
      queues.corral.addPlan(
        new PETRA.ConstructionPlan(gameState, "structures/{civ}/corral")
      );
      return;
    }
    if (!nCorral) return;
  ...
  for (let corral of gameState.getOwnEntitiesByClass("Corral", true).values()) {
    if (corral.foundationProgress() !== undefined) continue;
    let trainables = corral.trainableEntities(civ);
    for (let trainable of trainables) {
      if (gameState.isTemplateDisabled(trainable)) continue;
      let template = gameState.getTemplate(trainable);
      if (!template || !template.isHuntable()) continue;
      let count = gameState.countEntitiesByType(trainable, true);
      for (let item of corral.trainingQueue()) count += item.count;
      if (count > nCorral) continue;
      queues.corral.addPlan(
        new PETRA.TrainingPlan(gameState, trainable, { trainer: corral.id() })
      );
      return;
    }
  }
...
PETRA.HQ.prototype.buildMoreHouses = function (gameState, queues) {
  ...
  if (
    !gameState.isTemplateAvailable(gameState.applyCiv(houseTemplateString)) ||
  ...
  if (gameState.getPopulationMax() <= gameState.getPopulationLimit()) return;
  ...
  if (
    numPlanned > 0 &&
    this.phasing &&
    gameState.getPhaseEntityRequirements(this.phasing).length
  ) {
    ...
    for (let entityReq of gameState.getPhaseEntityRequirements(this.phasing)) {
      if (!houseTemplate.hasClass(entityReq.class)) continue;

      let count = gameState
        .getOwnStructures()
        .filter(API3.Filters.byClass(entityReq.class)).length;
      if (
        count < entityReq.count &&
        this.buildManager.isUnbuildable(gameState, houseTemplateName)
      ) {
        if (this.Config.debug > 1)
    ...
    for (let i = 0; i < numPlanned; ++i)
      if (houseQueue[i].isGo(gameState)) --needed;
      else if (needed > 0) {
  ...
  if (this.requireHouses) {
    let houseTemplate = gameState.getTemplate(
      gameState.applyCiv(houseTemplateString)
    );
    if (
      !this.phasing ||
      gameState
        .getPhaseEntityRequirements(this.phasing)
        .every(
          (req) =>
            !houseTemplate.hasClass(req.class) ||
            gameState.getOwnStructures().filter(API3.Filters.byClass(req.class))
              .length >= req.count
        )
    )
  ...
  let HouseNb = gameState
    .getOwnFoundations()
    .filter(API3.Filters.byClass("House")).length;
  let popBonus = gameState.getTemplate(house).getPopulationBonus();
  let freeSlots =
    gameState.getPopulationLimit() +
    HouseNb * popBonus -
    this.getAccountedPopulation(gameState);
  let priority;
  if (freeSlots < 5) {
    if (this.buildManager.isUnbuildable(gameState, house)) {
      if (this.Config.debug > 1)
        API3.warn(
          "no room to place a house ... try to improve with technology"
        );
      this.researchManager.researchPopulationBonus(gameState, queues);
    } else priority = 2 * this.Config.priorities.house;
  } else priority = this.Config.priorities.house;
...
PETRA.HQ.prototype.checkBaseExpansion = function (gameState, queues) {
  if (queues.civilCentre.hasQueuedUnits()) return;
  ...
  if (
    numUnits >
      activeBases * (65 + numvar + (10 + numvar) * (activeBases - 1)) ||
    (this.saveResources && numUnits > 50)
  ) {
    if (this.Config.debug > 2)
      API3.warn(
        "try to build a new base because of population " +
          numUnits +
          " for " +
          activeBases +
          " CCs"
...
PETRA.HQ.prototype.buildNewBase = function (gameState, queues, resource) {
  ...
  if (
    gameState
      .getOwnFoundations()
      .filter(API3.Filters.byClass("CivCentre"))
      .hasEntities() ||
  ...
  let hasOwnCC = false;
  for (let ent of gameState
    .updatingGlobalCollection("allCCs", API3.Filters.byClass("CivCentre"))
    .values()) {
    if (
      ent.owner() != PlayerID ||
      ent.templateName() != gameState.applyCiv("structures/{civ}/civil_centre")
    )
      continue;
    hasOwnCC = true;
    break;
  ...
  if (hasOwnCC && this.canBuild(gameState, "structures/{civ}/military_colony"))
    ...
    !hasOwnCC &&
    this.canBuild(gameState, "structures/{civ}/military_colony")
  )
    template = "structures/{civ}/military_colony";
  else return false;
  ...
  if (this.Config.debug > 1)
    API3.warn(
      "new base " +
        gameState.applyCiv(template) +
        " planned with resource " +
        resource
    );
  queues.civilCentre.addPlan(
    new PETRA.ConstructionPlan(gameState, template, {
      base: -1,
      resource: resource,
    })
  );
...
PETRA.HQ.prototype.buildDefenses = function (gameState, queues) {
  if (
    (this.saveResources && !this.canBarter) ||
  ...
  if (
    ...
    if (this.canBuild(gameState, "structures/{civ}/fortress")) {
      let numFortresses = gameState.getOwnEntitiesByClass(
        "Fortress",
        true
      ).length;
      if (
        (!numFortresses ||
          gameState.ai.elapsedTime >
            (1 + 0.1 * numFortresses) * this.fortressLapseTime +
              this.fortressStartTime) &&
        ...
        if (!numFortresses)
          gameState.ai.queueManager.changePriority(
            "defenseBuilding",
            2 * this.Config.priorities.defenseBuilding
          );
        let plan = new PETRA.ConstructionPlan(
          gameState,
  ...
  if (
    this.Config.Military.numSentryTowers &&
    this.currentPhase < 2 &&
    this.canBuild(gameState, "structures/{civ}/sentry_tower")
  ) {
    // Count all towers + wall towers.
    let numTowers =
      gameState.getOwnEntitiesByClass("Tower", true).length +
      gameState.getOwnEntitiesByClass("WallTower", true).length;
    let towerLapseTime = this.saveResource
      ? (1 + 0.5 * numTowers) * this.towerLapseTime
      : this.towerLapseTime;
    if (
      numTowers < this.Config.Military.numSentryTowers &&
      gameState.ai.elapsedTime > towerLapseTime + this.fortStartTime
    ) {
      this.fortStartTime = gameState.ai.elapsedTime;
      queues.defenseBuilding.addPlan(
        new PETRA.ConstructionPlan(gameState, "structures/{civ}/sentry_tower")
      );
    }
    return;
  }
  ...
  if (
    this.currentPhase < 2 ||
    !this.canBuild(gameState, "structures/{civ}/defense_tower")
  )
    return;

  let numTowers = gameState.getOwnEntitiesByClass("StoneTower", true).length;
  let towerLapseTime = this.saveResource
    ? (1 + numTowers) * this.towerLapseTime
    : this.towerLapseTime;
  if (
    (!numTowers ||
      gameState.ai.elapsedTime >
        (1 + 0.1 * numTowers) * towerLapseTime + this.towerStartTime) &&
    ...
    if (numTowers > 2 * this.numActiveBases() + 3)
      gameState.ai.queueManager.changePriority(
        "defenseBuilding",
        Math.round(0.7 * this.Config.priorities.defenseBuilding)
      );
    let plan = new PETRA.ConstructionPlan(
      gameState,
...
PETRA.HQ.prototype.buildForge = function (gameState, queues) {
  if (
    this.getAccountedPopulation(gameState) < this.Config.Military.popForForge ||
    queues.militaryBuilding.hasQueuedUnits() ||
    gameState.getOwnEntitiesByClass("Forge", true).length
  )
    return;
  // Build a Market before the Forge.
  if (!gameState.getOwnEntitiesByClass("Market", true).hasEntities()) return;

  if (this.canBuild(gameState, "structures/{civ}/forge"))
    queues.militaryBuilding.addPlan(
      new PETRA.ConstructionPlan(gameState, "structures/{civ}/forge")
    );
};
...
PETRA.HQ.prototype.constructTrainingBuildings = function (gameState, queues) {
  ...
  if (this.saveResources && numBarracks != 0) return;

  let barracksTemplate = this.canBuild(gameState, "structures/{civ}/barracks")
    ? "structures/{civ}/barracks"
    : undefined;

  let rangeTemplate = this.canBuild(gameState, "structures/{civ}/range")
    ? "structures/{civ}/range"
    : undefined;
  let numRanges = gameState.getOwnEntitiesByClass("Range", true).length;

  let stableTemplate = this.canBuild(gameState, "structures/{civ}/stable")
  ...
  if (
    this.getAccountedPopulation(gameState) >
      this.Config.Military.popForBarracks1 ||
    (this.phasing == 2 &&
      gameState.getOwnStructures().filter(API3.Filters.byClass("Village"))
        .length < 5)
  ) {
    ...
    if (numBarracks + numRanges == 0) {
      ...
      if (template) {
        gameState.ai.queueManager.changePriority(
          "militaryBuilding",
          2 * this.Config.priorities.militaryBuilding
        );
        let plan = new PETRA.ConstructionPlan(gameState, template, {
          militaryBase: true,
        });
    ...
    if (numStables == 0 && stableTemplate) {
      queues.militaryBuilding.addPlan(
        new PETRA.ConstructionPlan(gameState, stableTemplate, {
          militaryBase: true,
        })
      );
      return;
    ...
    if (
      numBarracks + numRanges == 1 &&
      this.getAccountedPopulation(gameState) >
        this.Config.Military.popForBarracks2
    ) {
      let template =
        numBarracks == 0
          ? barracksTemplate || rangeTemplate
          : rangeTemplate || barracksTemplate;
      if (template) {
        queues.militaryBuilding.addPlan(
          new PETRA.ConstructionPlan(gameState, template, {
            militaryBase: true,
    ...
    if (
      numBarracks + numRanges + numStables == 2 &&
      this.getAccountedPopulation(gameState) >
        this.Config.Military.popForBarracks2 + 30
    ) {
  ...
  if (this.saveResources) return;

  if (this.currentPhase < 3) return;

  if (
    this.canBuild(gameState, "structures/{civ}/elephant_stable") &&
    !gameState.getOwnEntitiesByClass("ElephantStable", true).hasEntities()
  ) {
    queues.militaryBuilding.addPlan(
      new PETRA.ConstructionPlan(
        gameState,
  ...
  if (
    this.canBuild(gameState, "structures/{civ}/arsenal") &&
    !gameState.getOwnEntitiesByClass("Arsenal", true).hasEntities()
  ) {
    queues.militaryBuilding.addPlan(
      new PETRA.ConstructionPlan(gameState, "structures/{civ}/arsenal", {
        militaryBase: true,
      })
    );
    return;
  ...
  if (
    !nAdvanced ||
    (nAdvanced < this.bAdvanced.length &&
      this.getAccountedPopulation(gameState) > 110)
  ) {
    for (let advanced of this.bAdvanced) {
      if (
        gameState.countEntitiesAndQueuedByType(advanced, true) > 0 ||
        !this.canBuild(gameState, advanced)
      )
        continue;
      ...
      if (template.hasDefensiveFire() || template.trainableEntities(civ))
        queues.militaryBuilding.addPlan(
          new PETRA.ConstructionPlan(gameState, advanced, {
            militaryBase: true,
          })
        );
      // not a military building, but still use this queue
      else
        queues.militaryBuilding.addPlan(
          new PETRA.ConstructionPlan(gameState, advanced)
        );
...
PETRA.HQ.prototype.findBestBaseForMilitary = function (gameState) {
  ...
  for (let cce of ccEnts) {
    if (gameState.isPlayerAlly(cce.owner())) continue;
    if (enemyFound && !gameState.isPlayerEnemy(cce.owner())) continue;
    let access = PETRA.getLandAccess(gameState, cce);
    let isEnemy = gameState.isPlayerEnemy(cce.owner());
    for (let cc of ccEnts) {
      if (cc.owner() != PlayerID) continue;
      if (PETRA.getLandAccess(gameState, cc) != access) continue;
      let dist = API3.SquareVectorDistance(cc.position(), cce.position());
      if (!enemyFound && isEnemy) enemyFound = true;
      else if (dist > distMin) continue;
...
PETRA.HQ.prototype.trainEmergencyUnits = function (gameState, positions) {
  if (gameState.ai.queues.emergency.hasQueuedUnits()) return false;
  ...
  for (let pos of positions) {
    ...
    for (let base of this.baseManagers) {
      if (!base.anchor || !base.anchor.position()) continue;
      if (PETRA.getLandAccess(gameState, base.anchor) != access) continue;
      if (!base.anchor.trainableEntities(civ))
        // base still in construction
        continue;
      ...
      if (queue) {
        ...
        for (let item of queue)
          if (
            item.progress > 0 ||
            (item.metadata && item.metadata.garrisonType)
          )
            time += item.timeRemaining;
        if (time / 1000 > 5) continue;
      }
      let dist = API3.SquareVectorDistance(base.anchor.position(), pos);
      if (nearestAnchor && dist > distmin) continue;
      distmin = dist;
      nearestAnchor = base.anchor;
    }
  }
  if (!nearestAnchor || distmin > distcut) return false;
  ...
  let autogarrison =
    numGarrisoned < nearestAnchor.garrisonMax() &&
    nearestAnchor.hitpoints() >
      nearestAnchor.garrisonEjectHealth() * nearestAnchor.maxHitpoints();
  ...
  for (let trainable of trainables) {
    ...
    if (!template || !template.hasClasses(["Infantry+CitizenSoldier"]))
      continue;
    if (autogarrison && !template.hasClasses(garrisonArrowClasses)) continue;
    if (!total.canAfford(new API3.Resources(template.cost()))) continue;
    templateFound = [trainable, template];
    if (template.hasClass("Ranged") == rangedWanted) break;
  }
  ...
  if (!queueManager.canAfford("emergency", cost)) {
    for (let q in queueManager.queues) {
      if (q == "emergency") continue;
      queueManager.transferAccounts(cost, q, "emergency");
      if (queueManager.canAfford("emergency", cost)) break;
    }
  }
  let metadata = {
    role: "worker",
  ...
  if (autogarrison) metadata.garrisonType = "protection";
  gameState.ai.queues.emergency.addPlan(
    new PETRA.TrainingPlan(gameState, templateFound[0], metadata, 1, 1)
  );
  return true;
};
...
PETRA.HQ.prototype.canBuild = function (gameState, structure) {
  ...
  if (this.buildManager.isUnbuildable(gameState, type)) return false;
  ...
  if (this.numActiveBases() < 1) {
    ...
    if (
      buildTerritories &&
      (!buildTerritories.length ||
        (buildTerritories.length == 1 && buildTerritories[0] == "own"))
    ) {
  ...
  if (
    ...
    gameState.getEntityCounts()[category] >= limits[category]
  ) {
...
PETRA.HQ.prototype.updateTerritories = function (gameState) {
  ...
  for (let j = 0; j < this.territoryMap.length; ++j) {
    if (this.borderMap.map[j] & PETRA.outside_Mask) continue;
    ...
    if (this.territoryMap.getOwnerIndex(j) != PlayerID) {
      ...
      if (this.basesMap.map[j] == 0) continue;
      let base = this.getBaseByID(this.basesMap.map[j]);
      if (base) {
        let index = base.territoryIndices.indexOf(j);
        if (index != -1) base.territoryIndices.splice(index, 1);
        else
          API3.warn(
            " problem in headquarters::updateTerritories for base " +
              this.basesMap.map[j]
          );
      } else
        API3.warn(
          " problem in headquarters::updateTerritories without base " +
            this.basesMap.map[j]
        );
      ...
      for (let a of around) {
        ...
        if (jz < 0 || jz >= width) continue;
        if (this.borderMap.map[jx + width * jz] & PETRA.outside_Mask) continue;
        let territoryOwner = this.territoryMap.getOwnerIndex(jx + width * jz);
        if (
          territoryOwner != PlayerID &&
          !(alliedVictory && gameState.isPlayerAlly(territoryOwner))
        ) {
  ...
  if (expansion * cellArea > 960) this.tradeManager.routeProspection = true;
};
...
PETRA.HQ.prototype.reassignTerritories = function (deletedBase) {
  ...
  let width = this.territoryMap.width;
  for (let j = 0; j < this.territoryMap.length; ++j) {
    if (this.basesMap.map[j] != deletedBase.ID) continue;
    if (this.territoryMap.getOwnerIndex(j) != PlayerID) {
    ...
    for (let base of this.baseManagers) {
      if (!base.anchor || !base.anchor.position()) continue;
      if (base.accessIndex != deletedBase.accessIndex) continue;
      let dist = API3.SquareVectorDistance(base.anchor.position(), pos);
      if (dist >= distmin) continue;
      distmin = dist;
      baseID = base.ID;
...
PETRA.HQ.prototype.getBaseByID = function (baseID) {
  for (let base of this.baseManagers) if (base.ID == baseID) return base;
  ...
  for (let base of this.baseManagers) {
    for (let worker of base.workers.values()) {
      ...
      if (
        orders.length < 2 ||
        !orders[1].target ||
        orders[1].target != worker.getMetadata(PlayerID, "supply")
      )
        continue;
...
PETRA.HQ.prototype.isDangerousLocation = function (gameState, pos, radius) {
  return (
    this.isNearInvadingArmy(pos) ||
    this.isUnderEnemyFire(gameState, pos, radius)
  );
};
...
PETRA.HQ.prototype.isNearInvadingArmy = function (pos) {
  for (let army of this.defenseManager.armies)
    if (
      army.foePosition &&
      API3.SquareVectorDistance(army.foePosition, pos) < 12000
    )
      return true;
  return false;
};
...
PETRA.HQ.prototype.isUnderEnemyFire = function (gameState, pos, radius = 0) {
  if (!this.turnCache.firingStructures)
    this.turnCache.firingStructures = gameState.updatingCollection(
      "diplo-FiringStructures",
      API3.Filters.hasDefensiveFire(),
      gameState.getEnemyStructures()
    );
...
PETRA.HQ.prototype.updateCaptureStrength = function (gameState) {
  ...
  for (let [targetId, capturableTarget] of this.capturableTargets) {
    ...
    for (let entId of capturableTarget.ents) {
      ...
      let orderData = ent.unitAIOrderData();
      if (!orderData || !orderData.length || !orderData[0].attackType) continue;
...
PETRA.HQ.prototype.AddTCGatherer = function (supplyID) {
  if (
    ...
    this.turnCache.resourceGatherer[supplyID] !== undefined
  )
    ++this.turnCache.resourceGatherer[supplyID];
  else {
    if (!this.turnCache.resourceGatherer) this.turnCache.resourceGatherer = {};
...
PETRA.HQ.prototype.AddTCResGatherer = function (resource) {
  ...
  else this.turnCache["resourceGatherer-" + resource] = 1;

  if (this.turnCache.currentRates) this.turnCache.currentRates[resource] += 0.5;
};
...
PETRA.HQ.prototype.isResourceExhausted = function (resource) {
  if (this.turnCache["exhausted-" + resource] == undefined)
    this.turnCache["exhausted-" + resource] = this.baseManagers.every(
      (base) =>
...
PETRA.HQ.prototype.isDefendable = function (ent) {
  if (!this.turnCache.numAround) this.turnCache.numAround = {};
  if (this.turnCache.numAround[ent.id()] === undefined)
    this.turnCache.numAround[ent.id()] =
      this.attackManager.numAttackingUnitsAround(ent.position(), 130);
  ...
  if (this.turnCache.accountedPopulation == undefined) {
    ...
    for (let ent of gameState.getOwnTrainingFacilities().values()) {
      for (let item of ent.trainingQueue()) {
        ...
        let unitPop = gameState
          .getTemplate(item.unitTemplate)
          .get("Cost/Population");
        if (unitPop) pop += item.count * unitPop;
      }
    }
...
PETRA.HQ.prototype.update = function (gameState, queues, events) {
  ...
  this.turnCache = {};
  this.territoryMap = PETRA.createTerritoryMap(gameState);
  this.canBarter = gameState
    .getOwnEntitiesByClass("Market", true)
    .filter(API3.Filters.isBuilt())
    .hasEntities();
  ...
  if (this.currentPhase != gameState.currentPhase()) {
    ...
    if (this.phasing) this.phasing = 0;
  }
  ...
  if (this.phasing) this.checkPhaseRequirements(gameState, queues);
  else this.researchManager.checkPhase(gameState, queues);
  ...
  if (
    this.numPotentialBases() < 1 ||
    (this.canExpand &&
      gameState.ai.playedTurn % 10 == 7 &&
      this.currentPhase > 1)
  )
    this.checkBaseExpansion(gameState, queues);

  if (this.currentPhase > 1 && gameState.ai.playedTurn % 3 == 0) {
    if (!this.canBarter) this.buildMarket(gameState, queues);
  ...
  if (gameState.ai.playedTurn % 3 == 0) {
    ...
    if (this.Config.difficulty > 0) this.buildDefenses(gameState, queues);
  }
  ...
  do {
    ...
    activeBase = this.baseManagers[this.currentBase++].update(
      gameState,
      queues,
      events
    );
  ...
  if (
    ...
    (this.numActiveBases() > 0 || !this.canBuildUnits)
  )
...
PETRA.HQ.prototype.Serialize = function () {
  ...
  let baseManagers = [];
  for (let base of this.baseManagers) baseManagers.push(base.Serialize());
...
PETRA.HQ.prototype.Deserialize = function (gameState, data) {
  for (let key in data.properties) this[key] = data.properties[key];

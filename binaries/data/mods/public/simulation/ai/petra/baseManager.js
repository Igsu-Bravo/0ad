...
PETRA.BaseManager = function (gameState, Config) {
  ...
  this.neededDefenders =
    this.Config.difficulty > 2 ? 3 + 2 * (this.Config.difficulty - 3) : 0;
...
PETRA.BaseManager.prototype.init = function (gameState, state) {
  if (state == "unconstructed") this.constructing = true;
  else if (state != "captured") this.neededDefenders = 0;
  ...
  this.workers = this.units.filter(
    API3.Filters.byMetadata(PlayerID, "role", "worker")
  );
  this.buildings = gameState
    .getOwnStructures()
    .filter(API3.Filters.byMetadata(PlayerID, "base", this.ID));
...
PETRA.BaseManager.prototype.reset = function (gameState, state) {
  if (state == "unconstructed") this.constructing = true;
  else this.constructing = false;

  if (state != "captured" || this.Config.difficulty < 3)
    this.neededDefenders = 0;
  else this.neededDefenders = 3 + 2 * (this.Config.difficulty - 3);
};
  ...
  if (!this.buildings.hasEntities()) {
    ...
    API3.warn(
      " Error: Petra base " +
        ...
        ent.templateName() +
        " with access" +
        PETRA.getLandAccess(gameState, ent)
    );
 ...
 */
PETRA.BaseManager.prototype.assignResourceToDropsite = function (
  gameState,
  dropsite
) {
  if (this.dropsites[dropsite.id()]) {
    if (this.Config.debug > 0)
      warn(
        "assignResourceToDropsite: dropsite already in the list. Should never happen"
      );
    return;
  }
  ...
  for (let type of dropsite.resourceDropsiteTypes()) {
    ...
    resources.forEach(function (supply) {
      if (!supply.position()) return;
      // Moving resources and fields are treated differently.
      if (supply.hasClasses(["Animal", "Field"])) return;
      // quick accessibility check
      if (PETRA.getLandAccess(gameState, supply) != accessIndex) return;

      let dist = API3.SquareVectorDistance(supply.position(), dropsitePos);
      if (dist < maxDistResourceSquare) {
        if (dist < maxDistResourceSquare / 16)
          // distmax/4
          nearby.push({
            dropsite: dropsiteId,
            id: supply.id(),
        ...
        else if (dist < maxDistResourceSquare / 4)
          // distmax/2
          medium.push({
            dropsite: dropsiteId,
            id: supply.id(),
...
PETRA.BaseManager.prototype.removeDropsite = function (gameState, ent) {
  ...
  let removeSupply = function (entId, supply) {
    for (let i = 0; i < supply.length; ++i) {
      ...
      else if (supply[i].dropsite == entId) supply.splice(i--, 1);
    }
  };
 ...
 */
PETRA.BaseManager.prototype.findBestDropsiteAndLocation = function (
  gameState,
  resource
) {
  let bestResult = {
  ...
  for (const templateName of gameState.ai.HQ.buildManager.findStructuresByFilter(
    gameState,
    API3.Filters.isDropsite(resource)
  )) {
    const dp = this.findBestDropsiteLocation(gameState, resource, templateName);
    if (dp.quality < bestResult.quality) continue;
    bestResult = dp;
 ...
 */
PETRA.BaseManager.prototype.findBestDropsiteLocation = function (
  gameState,
  resource,
  templateName
) {
  ...
  let obstructions = PETRA.createObstructionMap(
    gameState,
    this.accessIndex,
    template
  );
  ...
  const dpEnts = gameState
    .getOwnStructures()
    .filter(API3.Filters.isDropsite(resource))
    .toEntityArray();
  ...
  for (const foundation of gameState.getOwnFoundations().toEntityArray())
    if (
      PETRA.getBuiltEntity(gameState, foundation).isResourceDropsite(resource)
    )
  ...
  for (let j of this.territoryIndices) {
    ...
    for (let dp of dpEnts) {
      ...
      if (dist < 3600) {
        total = 0;
        break;
      } else if (dist < 6400) total *= (Math.sqrt(dist) - 60) / 20;
    }
    if (total <= bestVal) continue;

    if (gameState.ai.HQ.isDangerousLocation(gameState, pos, halfSize)) continue;
    bestVal = total;
    bestIdx = i;
  }

  if (this.Config.debug > 2) warn(" for dropsite best is " + bestVal);

  if (bestVal <= 0) return { quality: bestVal, pos: [0, 0] };
...
PETRA.BaseManager.prototype.getResourceLevel = function (
  gameState,
  type,
  nearbyOnly = false
) {
  ...
  for (let supply of this.dropsiteSupplies[type].nearby) {
    if (check[supply.id])
      // avoid double counting as same resource can appear several time
      continue;
    check[supply.id] = true;
    count += supply.ent.resourceSupplyAmount();
  }
  if (nearbyOnly) return count;

  for (let supply of this.dropsiteSupplies[type].medium) {
    if (check[supply.id]) continue;
    check[supply.id] = true;
...
PETRA.BaseManager.prototype.checkResourceLevels = function (gameState, queues) {
  for (let type of Resources.GetCodes()) {
    if (type == "food") {
      if (gameState.ai.HQ.canBuild(gameState, "structures/{civ}/field")) {
        // let's see if we need to add new farms.
        let count = this.getResourceLevel(
          gameState,
          type,
          gameState.currentPhase() > 1
        ); // animals are not accounted
        let numFarms = gameState
          .getOwnStructures()
          .filter(API3.Filters.byClass("Field")).length; // including foundations
        ...
        if (numFarms + numQueue == 0) {
          // starting game, rely on fruits as long as we have enough of them
          if (count < 600) {
            queues.field.addPlan(
              new PETRA.ConstructionPlan(gameState, "structures/{civ}/field", {
                favoredBase: this.ID,
              })
            );
          ...
          !gameState.ai.HQ.maxFields ||
          numFarms + numQueue < gameState.ai.HQ.maxFields
        ) {
          let numFound = gameState
            .getOwnFoundations()
            .filter(API3.Filters.byClass("Field")).length;
          let goal = this.Config.Economy.provisionFields;
          if (
            gameState.ai.HQ.saveResources ||
            gameState.ai.HQ.saveSpace ||
            count > 300 ||
            numFarms > 5
          )
    ...
    if (
      ...
      queues.dropsites.hasQueuedUnits() ||
      gameState
        .getOwnFoundations()
        .filter(API3.Filters.byClass("Storehouse"))
        .hasEntities()
    ) {
    ...
    if (gameState.ai.playedTurn < this.gatherers[type].nextCheck) continue;
    for (let ent of this.gatherersByType(gameState, type).values()) {
      if (ent.unitAIState() == "INDIVIDUAL.GATHER.GATHERING")
        ...
        ent.unitAIState() == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING"
      )
    ...
    if (total > 150 || (total > 60 && type != "wood")) {
      ...
      if (ratio > 0.15) {
        ...
        if (
          ...
          queues.dropsites.addPlan(
            new PETRA.ConstructionPlan(
              gameState,
              newDP.templateName,
              { base: this.ID, type: type },
              newDP.pos
            )
          );
        else if (
...
PETRA.BaseManager.prototype.addGatherRates = function (
  gameState,
  currentRates
) {
  for (let res in currentRates) {
    ...
    this.gatherersByType(gameState, res).forEach((ent) => {
      if (ent.isIdle() || !ent.position()) return;
      let gRate = ent.currentGatherRate();
      if (gRate) currentRates[res] += Math.log(1 + gRate) / 1.1;
    });
    if (res == "food") {
...
PETRA.BaseManager.prototype.assignRolelessUnits = function (
  gameState,
  roleless
) {
  if (!roleless)
    roleless = this.units
      .filter(API3.Filters.not(API3.Filters.byHasMetadata(PlayerID, "role")))
      .values();
...
PETRA.BaseManager.prototype.setWorkersIdleByPriority = function (gameState) {
  ...
  let scale = 1;
  if (sumWanted > 0) scale = sumCurrent / sumWanted;
  ...
  for (let i = mostNeeded.length - 1; i > 0; --i) {
    ...
    for (let j = 0; j < i; ++j) {
      ...
      if (lastFailed && gameState.ai.elapsedTime - lastFailed < 20) continue;
      // Ensure that the most wanted resource is not exhausted
      if (
        moreNeed.type != "food" &&
        gameState.ai.HQ.isResourceExhausted(moreNeed.type)
      ) {
        if (
          lessNeed.type != "food" &&
          gameState.ai.HQ.isResourceExhausted(lessNeed.type)
        )
          continue;
      ...
      if (
        scale * moreNeed.wanted -
          moreNeed.current -
          scale * lessNeed.wanted +
          lessNeed.current >
          1.5 ||
        (lessNeed.type != "food" &&
          gameState.ai.HQ.isResourceExhausted(lessNeed.type))
 ...
 */
PETRA.BaseManager.prototype.switchGatherer = function (
  gameState,
  from,
  to,
  number
  ...
  if (
    from == "food" &&
    gatherers.filter(API3.Filters.byClass("CitizenSoldier")).hasEntities()
  )
    only = "CitizenSoldier";
  else if (
    to == "food" &&
  ...
  for (let ent of gatherers.values()) {
    if (num == 0) return num;
    if (!ent.canGather(to)) continue;
    if (only && !ent.hasClass(only)) continue;
    --num;
    ent.stopMoving();
...
PETRA.BaseManager.prototype.reassignIdleWorkers = function (
  gameState,
  idleWorkers
) {
  ...
  if (!idleWorkers) {
    ...
    idleWorkers = gameState
      .updatingCollection("idle-workers-base-" + this.ID, filter, this.workers)
      .values();
  }
  ...
  for (let ent of idleWorkers) {
    ...
    if (ent.hasClass("Worker")) {
      ...
      if (
        ent.isBuilder() &&
        this.anchor &&
        this.anchor.needsRepair() &&
        gameState.getOwnEntitiesByMetadata(
          "target-foundation",
          this.anchor.id()
        ).length < 2
      )
...
PETRA.BaseManager.prototype.workersBySubrole = function (gameState, subrole) {
  return gameState.updatingCollection(
    "subrole-" + subrole + "-base-" + this.ID,
    API3.Filters.byMetadata(PlayerID, "subrole", subrole),
    this.workers
  );
};
...
PETRA.BaseManager.prototype.gatherersByType = function (gameState, type) {
  return gameState.updatingCollection(
    "workers-gathering-" + type + "-base-" + this.ID,
    API3.Filters.byMetadata(PlayerID, "gather-type", type),
    this.workersBySubrole(gameState, "gatherer")
  );
};
 ...
 */
PETRA.BaseManager.prototype.pickBuilders = function (
  gameState,
  workers,
  number
) {
  let availableWorkers = this.workers
    .filter((ent) => {
      if (!ent.position() || !ent.isBuilder()) return false;
      if (
        ent.getMetadata(PlayerID, "plan") == -2 ||
        ent.getMetadata(PlayerID, "plan") == -3
      )
        return false;
      if (ent.getMetadata(PlayerID, "transport")) return false;
      return true;
    })
    .toEntityArray();
  availableWorkers.sort((a, b) => {
    let vala = 0;
    let valb = 0;
    if (a.getMetadata(PlayerID, "subrole") == "builder") vala = 100;
    if (b.getMetadata(PlayerID, "subrole") == "builder") valb = 100;
    if (a.getMetadata(PlayerID, "subrole") == "idle") vala = -50;
    if (b.getMetadata(PlayerID, "subrole") == "idle") valb = -50;
    if (a.getMetadata(PlayerID, "plan") === undefined) vala = -20;
    if (b.getMetadata(PlayerID, "plan") === undefined) valb = -20;
    return vala - valb;
  });
 ...
 */
PETRA.BaseManager.prototype.assignToFoundations = function (
  gameState,
  noRepair
) {
  let foundations = this.buildings.filter(
    API3.Filters.and(
      API3.Filters.isFoundation(),
      API3.Filters.not(API3.Filters.byClass("Field"))
    )
  );

  let damagedBuildings = this.buildings.filter(
    (ent) => ent.foundationProgress() === undefined && ent.needsRepair()
  );

  // Check if nothing to build
  if (!foundations.length && !damagedBuildings.length) return;
  ...
  if (
    this.constructing &&
    foundations
      .filter(API3.Filters.byMetadata(PlayerID, "baseAnchor", true))
      .hasEntities()
  ) {
    foundations = foundations.filter(
      API3.Filters.byMetadata(PlayerID, "baseAnchor", true)
    );
  ...
  let availableResources =
    gameState.ai.queueManager.getAvailableResources(gameState);
  let builderRatio = 1;
  for (let res of Resources.GetCodes()) {
  ...
  for (let target of foundations.values()) {
    if (target.hasClass("Field")) continue; // we do not build fields

    if (gameState.ai.HQ.isNearInvadingArmy(target.position()))
      if (
        !target.hasClasses(["CivCentre", "Wall"]) &&
        (!target.hasClass("Wonder") ||
          !gameState.getVictoryConditions().has("wonder"))
      )
        continue;

    // if our territory has shrinked since this foundation was positioned, do not build it
    if (PETRA.isNotWorthBuilding(gameState, target)) continue;

    let assigned = gameState.getOwnEntitiesByMetadata(
      "target-foundation",
      target.id()
    ).length;
    let maxTotalBuilders = Math.ceil(workers.length * builderRatio);
    if (maxTotalBuilders < 2 && workers.length > 1) maxTotalBuilders = 2;
    if (
      target.hasClass("House") &&
      gameState.getPopulationLimit() < gameState.getPopulation() + 5 &&
      gameState.getPopulationLimit() < gameState.getPopulationMax()
    )
      maxTotalBuilders += 2;
    let targetNB = 2;
    if (
      target.hasClasses(["Fortress", "Wonder"]) ||
      target.getMetadata(PlayerID, "phaseUp") == true
    )
      targetNB = 7;
    else if (
      target.hasClasses(["Barracks", "Range", "Stable", "Tower", "Market"])
    )
      targetNB = 4;
    else if (target.hasClasses(["House", "DropsiteWood"])) targetNB = 3;
    ...
    if (assigned >= targetNB) continue;
    idleBuilderWorkers.forEach(function (ent) {
      if (ent.getMetadata(PlayerID, "target-foundation") !== undefined) return;
      if (
        assigned >= targetNB ||
        !ent.position() ||
    ...
    if (assigned >= targetNB || builderTot >= maxTotalBuilders) continue;
    let nonBuilderWorkers = workers
      .filter(function (ent) {
        if (ent.getMetadata(PlayerID, "subrole") == "builder") return false;
    ...
    nonBuilderWorkers.sort((workerA, workerB) => {
      let coeffA = API3.SquareVectorDistance(
        target.position(),
        workerA.position()
      );
      if (workerA.getMetadata(PlayerID, "gather-type") == "food") coeffA *= 3;
      let coeffB = API3.SquareVectorDistance(
        target.position(),
        workerB.position()
      );
      if (workerB.getMetadata(PlayerID, "gather-type") == "food") coeffB *= 3;
    ...
    while (
      assigned < targetNB &&
      builderTot < maxTotalBuilders &&
      current < nonBuilderTot
    ) {
      ++assigned;
      ++builderTot;
  ...
  for (let target of damagedBuildings.values()) {
    ...
    let maxTotalBuilders = Math.ceil(workers.length * builderRatio);
    let targetNB = 1;
    if (target.hasClasses(["Fortress", "Wonder"])) targetNB = 3;
    if (
      target.getMetadata(PlayerID, "baseAnchor") == true ||
      (target.hasClass("Wonder") &&
        gameState.getVictoryConditions().has("wonder"))
    ) {
      maxTotalBuilders = Math.ceil(
        workers.length * Math.max(0.3, builderRatio)
      );
      targetNB = 5;
      if (target.healthLevel() < 0.3) {
        maxTotalBuilders = Math.ceil(
          workers.length * Math.max(0.6, builderRatio)
        );
        targetNB = 7;
      }
    }
    ...
    let nearestNonBuilders = nonBuilderWorkers.filterNearest(
      target.position(),
      num
    );
...
PETRA.BaseManager.prototype.update = function (gameState, queues, events) {
  if (this.ID == gameState.ai.HQ.baseManagers[0].ID) {
    ...
    if (gameState.ai.HQ.numActiveBases() > 0) {
      for (let ent of this.units.values()) {
        ...
        if (bestBase.ID != this.ID) bestBase.assignEntity(gameState, ent);
      }
      for (let ent of this.buildings.values()) {
        ...
        if (!bestBase) {
          if (ent.hasClass("Dock"))
            API3.warn(
              "Petra: dock in baseManager[0]. It may be useful to do an anchorless base for " +
                ent.templateName()
            );
          continue;
        }
        if (ent.resourceDropsiteTypes()) this.removeDropsite(gameState, ent);
  ...
  if (!this.anchor) {
    // This anchor has been destroyed, but the base may still be usable
    if (!this.buildings.hasEntities()) {
    ...
    for (let ent of this.buildings.values()) {
      ...
      if (base.anchor) reassignedBase = base;
      break;
    }
  ...
  if (this.constructing) {
    ...
    if (owner != 0 && !gameState.isPlayerAlly(owner)) {
      ...
      for (let cc of ccEnts.values()) {
        if (cc.owner() != owner) continue;
        if (
          API3.SquareVectorDistance(cc.position(), this.anchor.position()) >
          8000
        )
          continue;
        this.anchor.destroy();
    ...
    this.neededDefenders &&
    gameState.ai.HQ.trainEmergencyUnits(gameState, [this.anchor.position()])
  )
    --this.neededDefenders;

  if (
    gameState.ai.elapsedTime > this.timeNextIdleCheck &&
...
PETRA.BaseManager.prototype.Deserialize = function (gameState, data) {
  ...
  this.anchor =
    this.anchorId !== undefined
      ? gameState.getEntityById(this.anchorId)
      : undefined;
};

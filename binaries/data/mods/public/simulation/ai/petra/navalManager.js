...
PETRA.NavalManager.prototype.init = function (gameState, deserializing) {
  // docks
  this.docks = gameState
    .getOwnStructures()
    .filter(API3.Filters.byClasses(["Dock", "Shipyard"]));
  this.docks.registerUpdates();

  this.ships = gameState
    .getOwnUnits()
    .filter(
      API3.Filters.and(
        API3.Filters.byClass("Ship"),
        API3.Filters.not(API3.Filters.byMetadata(PlayerID, "role", "trader"))
      )
    );
  // note: those two can overlap (some transport ships are warships too and vice-versa).
  this.transportShips = this.ships.filter(
    API3.Filters.and(
      API3.Filters.byCanGarrison(),
      API3.Filters.not(API3.Filters.byClass("FishingBoat"))
    )
  );
  ...
  for (let fish of gameState.getFishableSupplies().values()) {
    ...
    else if (sea) availableFishes[sea] = fish.resourceSupplyAmount();
  }
  ...
  for (let i = 0; i < gameState.ai.accessibility.regionSize.length; ++i) {
    if (!gameState.ai.HQ.navalRegions[i]) {
      ...
      let collec = this.ships.filter(
        API3.Filters.byMetadata(PlayerID, "sea", i)
      );
      collec.registerUpdates();
      this.seaShips.push(collec);
      collec = this.transportShips.filter(
        API3.Filters.byMetadata(PlayerID, "sea", i)
      );
      collec.registerUpdates();
      this.seaTransportShips.push(collec);
      collec = this.warShips.filter(
        API3.Filters.byMetadata(PlayerID, "sea", i)
      );
      collec.registerUpdates();
      this.seaWarShips.push(collec);
      collec = this.fishShips.filter(
        API3.Filters.byMetadata(PlayerID, "sea", i)
      );
      collec.registerUpdates();
      ...
      else this.wantedFishShips.push(0);
      this.neededTransportShips.push(0);
      this.neededWarShips.push(0);
    }
  }
  ...
  for (let i = 0; i < length; ++i) {
    ...
    if (naval < 2) continue;
    if (!this.landingZones[land]) this.landingZones[land] = {};
  ...
  for (let land in this.landingZones) {
    for (let sea in this.landingZones[land]) {
      ...
      for (let i of landing) {
        let nb = 0;
        if (landing.has(i - 1)) nb++;
        if (landing.has(i + 1)) nb++;
        if (landing.has(i + width)) nb++;
        if (landing.has(i - width)) nb++;
        nbaround[i] = nb;
      ...
      for (let i of landing) {
        if (nbaround[i] < nbcut) landing.delete(i);
      }
    }
  }

  // Assign our initial docks and ships
  for (let ship of this.ships.values()) PETRA.setSeaAccess(gameState, ship);
  for (let dock of this.docks.values()) PETRA.setSeaAccess(gameState, dock);
};

PETRA.NavalManager.prototype.updateFishingBoats = function (sea, num) {
  if (this.wantedFishShips[sea]) this.wantedFishShips[sea] = num;
};

PETRA.NavalManager.prototype.resetFishingBoats = function (gameState, sea) {
  if (sea !== undefined) this.wantedFishShips[sea] = 0;
  else this.wantedFishShips.fill(0);
};
    ...
    around.every((a) => {
      for (let t = 0; t < ntry; ++t) {
        ...
        if (gameState.ai.accessibility.landPassMap[i + j * width] === 1) {
          let navalPass =
            gameState.ai.accessibility.navalPassMap[i + j * width];
          if (navalPass == sea) return true;
          else if (navalPass == 1)
            // we could be outside the map
            continue;
        }
        return false;
      }
...
PETRA.NavalManager.prototype.canFishSafely = function (gameState, fish) {
  if (fish.getMetadata(PlayerID, "opensea")) return true;
  const ntry = 2;
  const around = [
    [-0.7, 0.7],
    [0, 1],
...
PETRA.NavalManager.prototype.getUnconnectedSeas = function (gameState, region) {
  ...
  this.docks.forEach((dock) => {
    if (
      !dock.hasClass("Dock") ||
      PETRA.getLandAccess(gameState, dock) != region
    )
      return;
    let i = seas.indexOf(PETRA.getSeaAccess(gameState, dock));
    if (i != -1) seas.splice(i--, 1);
  });
  return seas;
};

PETRA.NavalManager.prototype.checkEvents = function (
  gameState,
  queues,
  events
) {
  for (let evt of events.Create) {
    if (!evt.entity) continue;
    let ent = gameState.getEntityById(evt.entity);
    if (
      ent &&
      ent.isOwn(PlayerID) &&
      ent.foundationProgress() !== undefined &&
      ent.hasClasses(["Dock", "Shipyard"])
    )
      PETRA.setSeaAccess(gameState, ent);
  }

  for (let evt of events.TrainingFinished) {
    if (!evt.entities) continue;
    for (let entId of evt.entities) {
      let ent = gameState.getEntityById(entId);
      if (!ent || !ent.hasClass("Ship") || !ent.isOwn(PlayerID)) continue;
      PETRA.setSeaAccess(gameState, ent);
    }
  }
  ...
  for (let evt of events.Destroy) {
    ...
    if (plan.state == "boarding") {
      ...
      plan.units.forEach((ent) => {
        if (
          (ent.getMetadata(PlayerID, "onBoard") == "onBoard" &&
            ent.position()) ||
      ...
      for (let ent of plan.units.values()) {
        if (!ent.position())
          // unit from another ship of this plan ... do nothing
          continue;
...
PETRA.NavalManager.prototype.getPlan = function (ID) {
  for (let plan of this.transportPlans) if (plan.ID === ID) return plan;
  return undefined;
};
 ...
 */
PETRA.NavalManager.prototype.requireTransport = function (
  gameState,
  ent,
  startIndex,
  endIndex,
  ...
  if (!ent.canGarrison()) return false;

  if (ent.getMetadata(PlayerID, "transport") !== undefined) {
    if (this.Config.debug > 0)
      API3.warn(
        "Petra naval manager error: unit " +
          ent.id() +
  ...
  for (let plan of this.transportPlans) {
    ...
    if (
      ...
      plan.units.filter((unit) => PETRA.isSiegeUnit(unit)).length > 3
    )
      continue;
    plans.push(plan);
  }
  ...
  let plan = new PETRA.TransportPlan(
    gameState,
    [ent],
    startIndex,
    endIndex,
  ...
  if (plan.failed) {
    if (this.Config.debug > 1) API3.warn(">>>> transport plan aborted <<<<");
    return false;
  }
  plan.init(gameState);
  this.transportPlans.push(plan);
...
PETRA.NavalManager.prototype.splitTransport = function (gameState, plan) {
  ...
  let newplan = new PETRA.TransportPlan(
    gameState,
    [],
    plan.startIndex,
    plan.endIndex,
  ...
  if (newplan.units.length) this.transportPlans.push(newplan);
 ...
 */
PETRA.NavalManager.prototype.createTransportIfNeeded = function (
  gameState,
  fromPos,
  toPos,
  toAccess
  ...
  if (fromAccess !== 1) return;
  if (toAccess < 2) return;

  for (let ship of this.ships.values()) {
    if (!ship.isGarrisonHolder() || !ship.garrisoned().length) continue;
    if (ship.getMetadata(PlayerID, "transporter") !== undefined) continue;
    let units = [];
...
PETRA.NavalManager.prototype.setMinimalTransportShips = function (
  gameState,
  sea,
  number
) {
...
PETRA.NavalManager.prototype.checkLevels = function (gameState, queues) {
  if (queues.ships.hasQueuedUnits()) return;

  for (let sea = 0; sea < this.neededTransportShips.length; sea++)
    this.neededTransportShips[sea] = 0;

  for (let plan of this.transportPlans) {
    if (!plan.needTransportShips || plan.units.length < 2) continue;
    let sea = plan.sea;
    if (
      gameState.countOwnQueuedEntitiesWithMetadata("sea", sea) > 0 ||
      this.seaTransportShips[sea].length < this.wantedTransportShips[sea]
    )
      continue;
    ++this.neededTransportShips[sea];
    if (
      this.wantedTransportShips[sea] === 0 ||
      this.seaTransportShips[sea].length < plan.transportShips.length + 2
    ) {
  ...
  for (let sea = 0; sea < this.neededTransportShips.length; sea++)
    if (this.neededTransportShips[sea] > 2) ++this.wantedTransportShips[sea];
};

PETRA.NavalManager.prototype.maintainFleet = function (gameState, queues) {
  if (queues.ships.hasQueuedUnits()) return;
  if (!this.docks.filter(API3.Filters.isBuilt()).hasEntities()) return;
  // check if we have enough transport ships per region.
  for (let sea = 0; sea < this.seaShips.length; ++sea) {
    if (this.seaShips[sea] === undefined) continue;
    if (gameState.countOwnQueuedEntitiesWithMetadata("sea", sea) > 0) continue;

    if (this.seaTransportShips[sea].length < this.wantedTransportShips[sea]) {
      let template = this.getBestShip(gameState, sea, "transport");
      if (template) {
        queues.ships.addPlan(
          new PETRA.TrainingPlan(gameState, template, { sea: sea }, 1, 1)
        );
        continue;
      }
    }
...
PETRA.NavalManager.prototype.assignShipsToPlans = function (gameState) {
  for (let plan of this.transportPlans)
    if (plan.needTransportShips) plan.assignShip(gameState);
};
...
PETRA.NavalManager.prototype.isShipBoarding = function (ship) {
  ...
  if (!plan || !plan.boardingPos[ship.id()]) return false;
  return (
    API3.SquareVectorDistance(plan.boardingPos[ship.id()], ship.position()) <
    plan.boardingRange
  );
};
...
PETRA.NavalManager.prototype.moveApart = function (gameState) {
  ...
  for (let ship of this.ships.values()) {
    ...
    if (
      ship.getMetadata(PlayerID, "transporter") !== undefined &&
      this.isShipBoarding(ship)
    )
      continue;
    ...
    if (
      ...
      if (
        gameState.ai.playedTurn <
        ship.getMetadata(PlayerID, "turnPreviousPosition") + 2
      )
        continue;
      ship.moveToRange(
        shipPosition[0] + randFloat(-1, 1),
        shipPosition[1] + randFloat(-1, 1),
        30,
        35
      );
      ...
      let previousIdlePosition = ship.getMetadata(
        PlayerID,
        "previousIdlePosition"
      );
      if (
        !previousIdlePosition ||
        previousIdlePosition[0] != shipPosition[0] ||
      ...
      if (ship.getMetadata(PlayerID, "stationnary")) continue;
      ship.setMetadata(PlayerID, "stationnary", true);
      // Check if there are some treasure around
      if (PETRA.gatherTreasure(gameState, ship, true)) continue;
      ...
      for (let dock of gameState
        ...
        .values()) {
        if (PETRA.getSeaAccess(gameState, dock) != sea) continue;
  ...
  for (let ship of gameState.ai.HQ.tradeManager.traders
    .filter(API3.Filters.byClass("Ship"))
    .values()) {
    let shipPosition = ship.position();
    if (!shipPosition) continue;
    let role = ship.getMetadata(PlayerID, "role");
    if (!role || role != "trader")
      // already accounted before
      continue;
  ...
  for (let ship of blockedShips) {
    ...
    for (let blockingShip of this.seaShips[sea].values()) {
      if (
        blockedIds.indexOf(blockingShip.id()) != -1 ||
        !blockingShip.position()
      )
        continue;
      let distSquare = API3.SquareVectorDistance(
        shipPosition,
        blockingShip.position()
      );
      let unitAIState = blockingShip.unitAIState();
      if (
        blockingShip.getMetadata(PlayerID, "transporter") === undefined &&
        unitAIState != "INDIVIDUAL.GATHER.APPROACHING" &&
        unitAIState != "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING"
      ) {
        if (distSquare < 1600)
    ...
    for (let blockingShip of gameState.ai.HQ.tradeManager.traders
      .filter(API3.Filters.byClass("Ship"))
      .values()) {
      if (blockingShip.getMetadata(PlayerID, "sea") != sea) continue;
      if (
        blockedIds.indexOf(blockingShip.id()) != -1 ||
        !blockingShip.position()
      )
...
PETRA.NavalManager.prototype.buildNavalStructures = function (
  gameState,
  queues
) {
  if (!gameState.ai.HQ.navalMap || !gameState.ai.HQ.baseManagers[1]) return;

  if (
    gameState.ai.HQ.getAccountedPopulation(gameState) >
    this.Config.Economy.popForDock
  ) {
    if (
      queues.dock.countQueuedUnitsWithClass("Dock") === 0 &&
      !gameState
        .getOwnStructures()
        .filter(
          API3.Filters.and(
            API3.Filters.byClass("Dock"),
      ...
      for (let base of gameState.ai.HQ.baseManagers) {
        if (dockStarted) break;
        if (!base.anchor || base.constructing) continue;
        let remaining = this.getUnconnectedSeas(gameState, base.accessIndex);
        for (let sea of remaining) {
          if (!gameState.ai.HQ.navalRegions[sea]) continue;
          let wantedLand = {};
          wantedLand[base.accessIndex] = true;
          queues.dock.addPlan(
            new PETRA.ConstructionPlan(gameState, "structures/{civ}/dock", {
              land: wantedLand,
              sea: sea,
            })
          );
  ...
  if (
    ...
    gameState.ai.HQ.getAccountedPopulation(gameState) <
      this.Config.Economy.popPhase2 + 15 ||
    queues.militaryBuilding.hasQueuedUnits()
  )
    return;
  if (
    !this.docks.filter(API3.Filters.byClass("Dock")).hasEntities() ||
  ...
  let wantedLand = {};
  for (let base of gameState.ai.HQ.baseManagers)
    if (base.anchor) wantedLand[base.accessIndex] = true;
...
PETRA.NavalManager.prototype.getBestShip = function (gameState, sea, goal) {
  ...
  gameState
    ...
    .filter(API3.Filters.byMetadata(PlayerID, "sea", sea))
    .forEach(function (ent) {
      ...
      for (let trainable of trainables) {
        ...
        if (
          template &&
          template.hasClass("Ship") &&
          trainableShips.indexOf(trainable) === -1
        )
  ...
  for (let trainable of trainableShips) {
    let template = gameState.getTemplate(trainable);
    if (!template.available(gameState)) continue;
    ...
    if (goal === "attack") {
      // choose the maximum default arrows
      if (best > arrows) continue;
      best = arrows;
    } else if (goal === "transport") {
      // choose the maximum capacity, with a bonus if arrows or if siege transport
      let capacity = +(template.garrisonMax() || 0);
      if (capacity < 2) continue;
      capacity += 10 * arrows;
      if (MatchesClassList(template.garrisonableClasses(), "Siege"))
        capacity += 50;
      if (best > capacity) continue;
      best = capacity;
    } else if (goal === "fishing")
      if (!template.hasClass("FishingBoat")) continue;
    bestShip = trainable;
  }
  return bestShip;
};

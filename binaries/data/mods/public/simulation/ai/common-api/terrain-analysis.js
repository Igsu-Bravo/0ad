var API3 = (function (m) {
  ...
  m.TerrainAnalysis.prototype.init = function (sharedScript, rawState) {
    ...
    let obstructionMaskLand =
      rawState.passabilityClasses["default-terrain-only"];
	...
	*/
    ...
    for (let i = 0; i < passabilityMap.data.length; ++i) {
      ...
      obstructionTiles[i] =
        passabilityMap.data[i] & obstructionMaskLand ? 0 : 255;

      if (
        !(passabilityMap.data[i] & obstructionMaskWater) &&
        obstructionTiles[i] === 0
      )
        obstructionTiles[i] = 200;
      // if navigable and not walkable (ie basic water), set to 200.
      else if (
        !(passabilityMap.data[i] & obstructionMaskWater) &&
  ...
  m.Accessibility.prototype.getAccessValue = function (position, onWater) {
    ...
    if (onWater) return this.navalPassMap[gamePos[0] + this.width * gamePos[1]];
  ...
  m.Accessibility.prototype.getTrajectTo = function (start, end) {
    ...
    else if (endRegion <= 1) return false;

    let startRegion = onLand
      ? this.landPassMap[istart]
      : this.navalPassMap[istart];
    ...
    while (trajects.size) {
      for (let traj of trajects) {
        ...
        for (let inew of this.regionLinks[ilast]) {
          if (inew === iend) return traj.concat(iend);
          if (explored.has(inew)) continue;
  ...
  m.Accessibility.prototype.getRegionSize = function (position, onWater) {
    ...
    let ID =
      onWater === true ? this.navalPassMap[index] : this.landPassMap[index];
    if (this.regionSize[ID] === undefined) return 0;
    return this.regionSize[ID];
  };

  m.Accessibility.prototype.getRegionSizei = function (index, onWater) {
    if (
      this.regionSize[this.landPassMap[index]] === undefined &&
      (!onWater || this.regionSize[this.navalPassMap[index]] === undefined)
    )
      return 0;
    if (
      onWater &&
      this.regionSize[this.navalPassMap[index]] >
        this.regionSize[this.landPassMap[index]]
    )
  ...
  m.Accessibility.prototype.floodFill = function (startIndex, value, onWater) {
    ...
    if (
      (!onWater && this.landPassMap[startIndex] != 0) ||
      (onWater && this.navalPassMap[startIndex] != 0)
    )
    ...
    while (IndexArray.length) {
      ...
      do {
        ...
        if (
          ...
          floodFor === "water" &&
          this.navalPassMap[index] === 0 &&
          (this.map[index] === 200 || this.map[index] === 201)
        )
          loop = true;
        else break;
      ...
      do {
        ...
        if (index % w > 0) {
          if (
            floodFor === "land" &&
            this.landPassMap[index - 1] === 0 &&
            this.map[index - 1] !== 0 &&
            this.map[index - 1] !== 200
          ) {
            if (!reachLeft) {
            ...
            floodFor === "water" &&
            this.navalPassMap[index - 1] === 0 &&
            (this.map[index - 1] === 200 || this.map[index - 1] === 201)
          ) {
            if (!reachLeft) {
              IndexArray.push(index - 1);
              reachLeft = true;

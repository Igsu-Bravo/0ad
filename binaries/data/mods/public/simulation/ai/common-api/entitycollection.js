var API3 = (function (m) {
  ...
  m.EntityCollection.prototype.Serialize = function () {
    let filters = [];
    for (let f of this._filters) filters.push(uneval(f));
    return {
      ents: this.toIdArray(),
      frozen: this.frozen,
      filters: filters,
  ...
  m.EntityCollection.prototype.Deserialize = function (data, sharedAI) {
    ...
    for (let f of data.filters) this._filters.push(eval(f));

    if (data.frozen) this.freeze();
    else this.defreeze();
  };
  ...
  m.EntityCollection.prototype.filter = function (filter, thisp) {
    ...
    for (let [id, ent] of this._entities)
      if (filter.func.call(thisp, ent, id, this)) ret.set(id, ent);

    return new m.EntityCollection(
      this._ai,
      ret,
  ...
  m.EntityCollection.prototype.filterNearest = function (targetPos, n) {
    ...
    else n = Math.min(n, data.length);

    // Extract the first n
    let ret = new Map();
    for (let i = 0; i < n; ++i) ret.set(data[i][0], data[i][1]);
  ...
  m.EntityCollection.prototype.filter_raw = function (callback, thisp) {
    ...
    for (let [id, ent] of this._entities) {
      let val = ent._entity;
      if (callback.call(thisp, val, id, this)) ret.set(id, ent);
    }
    return new m.EntityCollection(this._ai, ret);
  };

  m.EntityCollection.prototype.forEach = function (callback) {
    for (let ent of this._entities.values()) callback(ent);
    return this;
  };
  ...
  m.EntityCollection.prototype.moveToRange = function (
    x,
    z,
    min,
    max,
  ...
  m.EntityCollection.prototype.attackMove = function (
    x,
    z,
    targetClasses,
    allowCapture = true,
  ...
  m.EntityCollection.prototype.occupyTurret = function (
    target,
    queued = false
  ) {
    Engine.PostCommand(PlayerID, {
  ...
  m.EntityCollection.prototype.removeEnt = function (ent) {
    if (!this._entities.has(ent.id())) return false;
  ...
  m.EntityCollection.prototype.updateEnt = function (ent, force) {
    ...
    if (passesFilters) {
      if (!force && this.frozen) return false;

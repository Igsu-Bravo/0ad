var API3 = (function (m) {
  ...
  m.GameState.prototype.init = function (SharedScript, state, player) {
    ...
    this.phases = [];
    let cctemplate = this.getTemplate(
      this.applyCiv("structures/{civ}/civil_centre")
    );
    if (!cctemplate) return;
    ...
    for (let techName of techs) {
      if (!techName.startsWith("phase")) continue;
      ...
      if (techData._definesPair) {
        ...
        techName =
          pickRandom(
            [techData._template.top, techData._template.bottom].filter(
              (tech) => !this.playerData.disabledTechnologies[tech]
            )
          ) || techData._template.top;

        let supersedes = techData._template.supersedes;
        techData = clone(this.getTemplate(techName));
        if (supersedes) techData._template.supersedes = supersedes;
      }

      phaseData[techName] = GetTechnologyBasicDataHelper(
        techData._template,
        civ
      );
    ...
    this.phases = UnravelPhases(phaseData).map((phaseName) => ({
      ...
      requirements: phaseMap[phaseName]
        ? phaseData[phaseMap[phaseName]].reqs
        : [],
    }));
  };
  ...
  m.GameState.prototype.updatingCollection = function (
    id,
    filter,
    parentCollection
  ) {
  ...
  m.GameState.prototype.updatingGlobalCollection = function (
    gid,
    filter,
    parentCollection
  ) {
    if (this.EntCollecNames.has(gid)) return this.EntCollecNames.get(gid);

    let collection = parentCollection
      ? parentCollection.filter(filter)
  ...
  m.GameState.prototype.destroyGlobalCollection = function (gid) {
    if (!this.EntCollecNames.has(gid)) return;

    this.sharedScript.removeUpdatingEntityCollection(
      this.EntCollecNames.get(gid)
    );
  ...
  m.GameState.prototype.getTemplate = function (type) {
    if (TechnologyTemplates.Has(type)) return new m.Technology(type);

    if (this.templates[type] === undefined) this.sharedScript.GetTemplate(type);

    return this.templates[type]
      ? new m.Template(this.sharedScript, type, this.templates[type])
      : null;
  };
  ...
  m.GameState.prototype.getPlayerCiv = function (player) {
    return player !== undefined
      ? this.sharedScript.playersData[player].civ
      : this.playerData.civ;
  };

  m.GameState.prototype.currentPhase = function () {
    for (let i = this.phases.length; i > 0; --i)
      if (this.isResearched(this.phases[i - 1].name)) return i;
    return 0;
  };
  ...
  m.GameState.prototype.getPhaseEntityRequirements = function (i) {
    ...
    for (let requirement of this.phases[i - 1].requirements) {
      if (!requirement.entities) continue;
  ...
  m.GameState.prototype.isResearching = function (template) {
    return (
      this.playerData.researchStarted.has(template) ||
  ...
  m.GameState.prototype.canResearch = function (
    techTemplateName,
    noRequirementCheck
  ) {
    if (this.playerData.disabledTechnologies[techTemplateName]) return false;
    ...
    return this.checkTechRequirements(
      template.requirements(this.playerData.civ)
    );
  };
  ...
  m.GameState.prototype.checkTechRequirements = function (reqs) {
    ...
    function doesEntitySpecPass(entity) {
      switch (entity.check) {
        case "count":
          if (
            !this.playerData.classCounts[entity.class] ||
        ...
        case "variants":
          if (
            !this.playerData.typeCountsByClass[entity.class] ||
            Object.keys(this.playerData.typeCountsByClass[entity.class])
    ...
    return reqs.some((req) => {
      return Object.keys(req).every((type) => {
        switch (type) {
          case "techs":
            return req[type].every((tech) =>
              this.playerData.researchedTechs.has(tech)
            );
  ...
  m.GameState.prototype.getEnemies = function () {
    let ret = [];
    for (let i in this.playerData.isEnemy)
      if (this.playerData.isEnemy[i]) ret.push(+i);
    return ret;
  };
  ...
  m.GameState.prototype.getNeutrals = function () {
    let ret = [];
    for (let i in this.playerData.isNeutral)
      if (this.playerData.isNeutral[i]) ret.push(+i);
    return ret;
  };
  ...
  m.GameState.prototype.getAllies = function () {
    let ret = [];
    for (let i in this.playerData.isAlly)
      if (this.playerData.isAlly[i]) ret.push(+i);
    return ret;
  };
  ...
  m.GameState.prototype.getExclusiveAllies = function () {
    ...
    let ret = [];
    for (let i in this.playerData.isAlly)
      if (this.playerData.isAlly[i] && +i !== this.player) ret.push(+i);
    return ret;
  };
  ...
  m.GameState.prototype.getMutualAllies = function () {
    ...
    for (let i in this.playerData.isMutualAlly)
      if (
        this.playerData.isMutualAlly[i] &&
  ...
  m.GameState.prototype.getEntities = function (id) {
    if (id === undefined) return this.entities;

    return this.updatingGlobalCollection(
      "player-" + id + "-entities",
      m.Filters.byOwner(id)
    );
  };
  ...
  m.GameState.prototype.getOwnEntities = function () {
    return this.updatingGlobalCollection(
      "player-" + this.player + "-entities",
      m.Filters.byOwner(this.player)
    );
  };

  m.GameState.prototype.getOwnStructures = function () {
    return this.updatingGlobalCollection(
      "player-" + this.player + "-structures",
      m.Filters.byClass("Structure"),
      this.getOwnEntities()
    );
  };
  ...
  m.GameState.prototype.getOwnUnits = function () {
    return this.updatingGlobalCollection(
      "player-" + this.player + "-units",
      m.Filters.byClass("Unit"),
      this.getOwnEntities()
    );
  };
  ...
  m.GameState.prototype.getAllyStructures = function (allyID) {
    if (allyID == undefined)
      return this.updatingCollection(
        "diplo-ally-structures",
        m.Filters.byOwners(this.getAllies()),
        this.getStructures()
      );

    return this.updatingGlobalCollection(
      "player-" + allyID + "-structures",
      m.Filters.byOwner(allyID),
      this.getStructures()
    );
  };
  ...
  m.GameState.prototype.getEnemyStructures = function (enemyID) {
    if (enemyID === undefined)
      return this.updatingCollection(
        "diplo-enemy-structures",
        m.Filters.byOwners(this.getEnemies()),
        this.getStructures()
      );

    return this.updatingGlobalCollection(
      "player-" + enemyID + "-structures",
      m.Filters.byOwner(enemyID),
      this.getStructures()
    );
  };
  ...
  m.GameState.prototype.getOwnEntitiesByMetadata = function (
    key,
    value,
    maintain
  ) {
    if (maintain)
      return this.updatingCollection(
        key + "-" + value,
        m.Filters.byMetadata(this.player, key, value),
        this.getOwnEntities()
      );
    return this.getOwnEntities().filter(
      m.Filters.byMetadata(this.player, key, value)
    );
  };
  ...
  m.GameState.prototype.getOwnFoundationsByClass = function (cls, maintain) {
    ...
    if (maintain)
      return this.updatingCollection(
        "foundations-class-" + cls,
        filter,
        this.getOwnFoundations()
      );
  ...
  m.GameState.prototype.getOwnTrainingFacilities = function () {
    return this.updatingGlobalCollection(
      "player-" + this.player + "-training-facilities",
      m.Filters.byTrainingQueue(),
      this.getOwnEntities()
    );
  };
  ...
  m.GameState.prototype.getOwnResearchFacilities = function () {
    return this.updatingGlobalCollection(
      "player-" + this.player + "-research-facilities",
      m.Filters.byResearchAvailable(this, this.playerData.civ),
      this.getOwnEntities()
    );
  };
  ...
  m.GameState.prototype.countEntitiesAndQueuedByType = function (
    type,
    maintain
  ) {
    ...
    if (template.hasClass("Structure") === true)
    ...
    else if (template.resourceSupplyType() !== undefined)
      // animal resources
      ...
      this.getOwnTrainingFacilities().forEach(function (ent) {
        for (let item of ent.trainingQueue())
          if (item.unitTemplate == type) count += item.count;
      });
    }
  ...
  m.GameState.prototype.countFoundationsByType = function (type, maintain) {
    ...
    if (maintain)
      return this.updatingCollection(
        ...
        m.Filters.byType(foundationType),
        this.getOwnFoundations()
      ).length;

    let count = 0;
    this.getOwnStructures().forEach(function (ent) {
      if (ent.templateName() == foundationType) ++count;
    });
    return count;
  };
  ...
  m.GameState.prototype.countOwnQueuedEntitiesWithMetadata = function (
    data,
    value
  ) {
    // Count entities in building production queues
  ...
  m.GameState.prototype.getOwnFoundations = function () {
    return this.updatingGlobalCollection(
      "player-" + this.player + "-foundations",
      m.Filters.isFoundation(),
      this.getOwnStructures()
    );
  };
  ...
  m.GameState.prototype.getOwnDropsites = function (resource) {
    if (resource)
      return this.updatingCollection(
        "ownDropsite-" + resource,
        m.Filters.isDropsite(resource),
        this.getOwnEntities()
      );
    return this.updatingCollection(
      "ownDropsite-all",
      m.Filters.isDropsite(),
      this.getOwnEntities()
    );
  };
  ...
  m.GameState.prototype.findTrainableUnits = function (classes, anticlasses) {
    ...
    this.getOwnTrainingFacilities().forEach(function (ent) {
      ...
      for (let unit of trainable)
        if (allTrainable.indexOf(unit) === -1) allTrainable.push(unit);
    });
    let ret = [];
    ...
    for (let trainable of allTrainable) {
      if (this.isTemplateDisabled(trainable)) continue;
      let template = this.getTemplate(trainable);
      if (!template || !template.available(this)) continue;
      let limit = template.matchLimit();
      if (matchCounts && limit && matchCounts[trainable] >= limit) continue;
  ...
  m.GameState.prototype.findAvailableTech = function () {
    ...
    for (let ent of this.getOwnEntities().values()) {
      ...
      for (let tech of searchable)
        if (
          !this.playerData.disabledTechnologies[tech] &&
          allResearchable.indexOf(tech) === -1
        )
  ...
  m.GameState.prototype.hasTrainer = function (template) {
    ...
    for (let ent of this.getOwnTrainingFacilities().values()) {
      let trainable = ent.trainableEntities(civ);
      if (trainable && trainable.indexOf(template) !== -1) return true;
    }
    return false;
  };
  ...
  m.GameState.prototype.findBuilder = function (template) {
    ...
    for (let ent of this.getOwnUnits().values()) {
      ...
      if (buildable && buildable.indexOf(template) !== -1) return ent;
    }
    return undefined;
  };

  /** Return true if one of our buildings is capable of researching the given tech */
  m.GameState.prototype.hasResearchers = function (
    templateName,
    noRequirementCheck
  ) {
    // let's check we can research the tech.
    if (!this.canResearch(templateName, noRequirementCheck)) return false;

    let template = this.getTemplate(templateName);
    if (template.autoResearch) return true;
    ...
    for (let ent of this.getOwnResearchFacilities().values()) {
      ...
      for (let tech of techs) {
        ...
        if (temp.pairDef()) {
          ...
          if (
            pairedTechs[0]._templateName == templateName ||
            pairedTechs[1]._templateName == templateName
          )
            return true;
        } else if (tech == templateName) return true;
      }
    }
    return false;
  };
  ...
  m.GameState.prototype.findResearchers = function (
    templateName,
    noRequirementCheck
  ) {
    // let's check we can research the tech.
    if (!this.canResearch(templateName, noRequirementCheck)) return undefined;
  ...
  m.GameState.prototype.isTemplateAvailable = function (templateName) {
    ...
    return (
      this.templates[templateName] && !this.isTemplateDisabled(templateName)
    );
  };

  m.GameState.prototype.isTemplateDisabled = function (templateName) {
    if (!this.playerData.disabledTemplates[templateName]) return false;
  ...
  m.GameState.prototype.isEntityLimitReached = function (category) {
    if (
      this.playerData.entityLimits[category] === undefined ||
      this.playerData.entityCounts[category] === undefined
    )
      return false;
    return (
      this.playerData.entityCounts[category] >=
      this.playerData.entityLimits[category]
    );
  };
  ...
  m.GameState.prototype.getTraderTemplatesGains = function () {
    ...
    let shipMerchantTemplate =
      !this.isTemplateDisabled(shipMechantTemplateName) &&
      this.getTemplate(shipMechantTemplateName);
    let supportTraderTemplate =
      !this.isTemplateDisabled(supportTraderTemplateName) &&

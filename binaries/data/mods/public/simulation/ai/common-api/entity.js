var API3 = (function (m) {
  ...
  m.Template = m.Class({
    ...
    phase: function () {
      let techRequired = this.requiredTech();
      if (!techRequired) return 0;
      if (techRequired == "phase_village") return 1;
      if (techRequired == "phase_town") return 2;
      if (techRequired == "phase_city") return 3;
      if (techRequired.startsWith("phase_")) return 4;
      return 0;
    },

    cost: function (productionQueue) {
      if (!this.get("Cost")) return {};
    ...
    costSum: function (productionQueue) {
      ...
      if (!cost) return 0;
      let ret = 0;
      for (let type in cost) ret += cost[type];
      return ret;
    },
    ...
    obstructionRadius: function () {
      if (!this.get("Obstruction")) return undefined;
    ...
    footprintRadius: function () {
      if (!this.get("Footprint")) return undefined;
    ...
    resistanceStrengths: function () {
      ...
      if (!resistanceTypes || !resistanceTypes.Entity) return undefined;
      ...
      if (resistanceTypes.Entity.Damage) {
        ...
        for (let damageType in resistanceTypes.Entity.Damage)
          resistance.Damage[damageType] = +this.get(
            "Resistance/Entity/Damage/" + damageType
          );
      }
    ...
    attackTypes: function () {
      ...
      let ret = [];
      for (let type in attack) ret.push(type);
      return ret;
    },

    attackRange: function (type) {
      if (!this.get("Attack/" + type)) return undefined;
    ...
    attackStrengths: function (type) {
      ...
      if (!attackDamageTypes) return undefined;
    ...
    captureStrength: function () {
      if (!this.get("Attack/Capture")) return undefined;
    ...
    getCounteredClasses: function () {
      ...
      for (let type in attack) {
        ...
        for (let b in bonuses) {
          let bonusClasses = this.get(
            "Attack/" + type + "/Bonuses/" + b + "/Classes"
          );
          if (bonusClasses)
            Classes.push([
              bonusClasses.split(" "),
              +this.get("Attack/" + type + "/Bonuses/" + b + "/Multiplier"),
            ]);
        }
      }
      return Classes;
    ...
    counters: function (target) {
      ...
      for (let type in attack) {
        ...
        for (let b in bonuses) {
          ...
          if (bonusClasses) mcounter.concat(bonusClasses.split(" "));
        }
      }
    ...
    getMultiplierAgainst: function (type, againstClass) {
      ...
      if (bonuses) {
        for (let b in bonuses) {
          ...
          if (!bonusClasses) continue;
          for (let bcl of bonusClasses.split(" "))
            if (bcl == againstClass)
              return +this.get(
                "Attack/" + type + "/Bonuses/" + b + "/Multiplier"
              );
        }
      }
      return 1;
    ...
    resourceSupplyMax: function () {
      return +this.get("ResourceSupply/Max");
    },
    ...
    resourceGatherRates: function () {
      if (!this.get("ResourceGatherer")) return undefined;
      let ret = {};
    ...
    resourceDropsiteTypes: function () {
      if (!this.get("ResourceDropsite")) return undefined;
    ...
    isTreasure: function () {
      return this.get("Treasure") !== undefined;
    },

    treasureResources: function () {
      if (!this.get("Treasure")) return undefined;
      let ret = {};
    ...
    garrisonSize: function () {
      return this.get("Garrisonable/Size");
    },
    ...
    getGarrisonArrowClasses: function () {
      if (!this.get("BuildingAI")) return undefined;
    ...
    isPackable: function () {
      return this.get("Pack") != undefined;
    },
    ...
    isHuntable: function () {
      ...
      return (
        this.get("ResourceSupply/KillBeforeGather") &&
    ...
    buildTime: function (productionQueue) {
      ...
      if (productionQueue) time *= productionQueue.techCostMultiplier("time");
      return time;
    },
    ...
    buildTerritories: function () {
      if (!this.get("BuildRestrictions")) return undefined;
    ...
    hasDefensiveFire: function () {
      if (!this.get("Attack/Ranged")) return false;
    ...
    isBuilder: function () {
      return this.get("Builder") !== undefined;
    },

    isGatherer: function () {
      return this.get("ResourceGatherer") !== undefined;
    },

    canGather: function (type) {
      let gatherRates = this.get("ResourceGatherer/Rates");
      if (!gatherRates) return false;
      for (let r in gatherRates) if (r.split(".")[0] === type) return true;
      return false;
    },

    isGarrisonHolder: function () {
      return this.get("GarrisonHolder") !== undefined;
    },

    isTurretHolder: function () {
      return this.get("TurretHolder") !== undefined;
    },
    ...
    canCapture: function (target) {
      if (!this.get("Attack/Capture")) return false;
      if (!target) return true;
      if (!target.get("Capturable")) return false;
      let restrictedClasses = this.get(
        "Attack/Capture/RestrictedClasses/_string"
      );
      return !restrictedClasses || !target.hasClasses(restrictedClasses);
    },

    isCapturable: function () {
      return this.get("Capturable") !== undefined;
    },

    canGuard: function () {
      return this.get("UnitAI/CanGuard") === "true";
    },

    canGarrison: function () {
      return "Garrisonable" in this._template;
    },

    canOccupyTurret: function () {
      return "Turretable" in this._template;
    },

    isTreasureCollector: function () {
      return this.get("TreasureCollector") !== undefined;
    },
  });
  ...
  m.Entity = m.Class({
    ...
    _init: function (sharedAI, entity) {
      this._super.call(
        ...
        sharedAI,
        entity.template,
        sharedAI.GetTemplate(entity.template)
      );
    ...
    getStance: function () {
      return this._entity.stance;
    },
    unitAIState: function () {
      return this._entity.unitAIState;
    },
    unitAIOrderData: function () {
    ...
    capturePoints: function () {
      return this._entity.capturePoints;
    },
    isInvulnerable: function () {
      return this._entity.invulnerability || false;
    },

    isSharedDropsite: function () {
      return this._entity.sharedDropsite === true;
    },
    ...
    trainingQueueTime: function () {
      ...
      let time = 0;
      for (let item of queue) time += item.timeRemaining;
      return time / 1000;
    },
    ...
    getBuilders: function () {
      if (this._entity.foundationProgress === undefined) return undefined;
      if (this._entity.foundationBuilders === undefined) return [];
      return this._entity.foundationBuilders;
    },

    getBuildersNb: function () {
      if (this._entity.foundationProgress === undefined) return undefined;
      if (this._entity.foundationBuilders === undefined) return 0;
    ...
    isOwn: function (player) {
      if (typeof this._entity.owner === "undefined") return false;
    ...
    currentGatherRate: function () {
      ...
      if (
        ...
        if (
          this.unitAIState().split(".")[1] == "GATHER" &&
          this.unitAIOrderData()[0].target !== undefined
        )
          res = this._ai._entities.get(this.unitAIOrderData()[0].target);
        else if (
          this.unitAIOrderData()[1] !== undefined &&
          this.unitAIOrderData()[1].target !== undefined
        )
    ...
    canAttackClass: function (aClass) {
      ...
      for (let type in attack) {
        ...
        let restrictedClasses = this.get(
          "Attack/" + type + "/RestrictedClasses/_string"
        );
        if (
          !restrictedClasses ||
          !MatchesClassList([aClass], restrictedClasses)
        )
          return true;
      }
      return false;
    ...
    canAttackTarget: function (target, allowCapture) {
      ...
      if (!attackTypes) return false;

      let canCapture = allowCapture && this.canCapture(target);
      let health = target.get("Health");
      if (!health) return canCapture;

      for (let type in attackTypes) {
        if (type == "Capture" ? !canCapture : target.isInvulnerable()) continue;
    ...
    setStance: function (stance, queued = false, pushFront = false) {
      if (this.getStance() === undefined) return undefined;
      Engine.PostCommand(PlayerID, {
        type: "stance",
        entities: [this.id()],
        name: stance,
    ...
    unload: function (id) {
      if (!this.get("GarrisonHolder")) return undefined;
      Engine.PostCommand(PlayerID, {
        type: "unload",
        garrisonHolder: this.id(),
        entities: [id],
      });
      return this;
    },
    ...
    moveApart: function (point, dist) {
      if (this.position() !== undefined) {
        ...
        Engine.PostCommand(PlayerID, {
          type: "walk",
          entities: [this.id()],
          x: this.position()[0] + direction[0] * dist,
          z: this.position()[1] + direction[1] * dist,
          queued: false,
          pushFront: false,
        });
      }
    ...
    flee: function (unitToFleeFrom) {
      if (
        ...
        unitToFleeFrom.position() !== undefined
      ) {
        let FleeDirection = [
          this.position()[0] - unitToFleeFrom.position()[0],
        ...
        Engine.PostCommand(PlayerID, {
          type: "walk",
          entities: [this.id()],
          x: this.position()[0] + FleeDirection[0],
          z: this.position()[1] + FleeDirection[1],
          queued: false,
          pushFront: false,
        });
      }
    ...
    stopAllProduction: function (percentToStopAt) {
      let queue = this._entity.trainingQueue;
      if (!queue) return true; // no queue, so technically we stopped all production.

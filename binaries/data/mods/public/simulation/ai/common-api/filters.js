var API3 = (function (m) {
  m.Filters = {
    ...
    and: (filter1, filter2) => ({
      ...
      dynamicProperties: filter1.dynamicProperties.concat(
        filter2.dynamicProperties
      ),
    }),
    ...
    isTreasure: () => ({
      func: (ent) => {
        if (!ent.isTreasure()) return false;
        ...
        return (
          template != "gaia/treasure/shipwreck_debris" &&
    ...
    byResource: (resourceType) => ({
      func: (ent) => {
        if (!ent.resourceSupplyMax()) return false;

        let type = ent.resourceSupplyType();
        if (!type) return false;

        // Skip targets that are too hard to hunt
        if (!ent.isHuntable() || ent.hasClass("SeaCreature")) return false;
    ...
    isHuntable: () => ({
      ...
      func: (ent) =>
        ent.hasClass("Animal") &&
        ent.resourceSupplyMax() &&
        ent.isHuntable() &&
        !ent.hasClass("SeaCreature"),
      dynamicProperties: [],
    }),

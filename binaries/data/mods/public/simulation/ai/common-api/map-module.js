var API3 = (function (m) {
  ...
  m.Map = function Map(sharedScript, type, originalMap, actualCopy) {
    ...
    if (originalMap && originalMap.length != this.length)
      warn(
        "AI map size incompatibility with type " +
          type +
          ": original " +
          originalMap.length +
          " new " +
    ...
    if (originalMap && actualCopy) {
      ...
      for (let i = 0; i < this.length; ++i) this.map[i] = originalMap[i];
    } else if (originalMap) this.map = originalMap;
    else this.map = new Uint8Array(this.length);
  };
  ...
  m.Map.prototype.multiplyInfluence = function (
    cx,
    cy,
    maxDist,
    strength,
  ...
  m.Map.prototype.add = function (map) {
    for (let i = 0; i < this.length; ++i) this.set(i, this.map[i] + map.map[i]);
  };
  ...
  m.Map.prototype.findBestTile = function (radius, obstruction) {
    ...
    for (let j = 0; j < this.length; ++j) {
      if (this.map[j] <= bestVal) continue;
  ...
  m.Map.prototype.getNonObstructedTile = function (i, radius, obstruction) {
    ...
    for (let kx = ix; kx < ix + ratio; ++kx) {
      ...
      for (let ky = iy; ky < iy + ratio; ++ky) {
        ...
        if (
          lastPoint &&
          (kx - lastPoint.x) * (kx - lastPoint.x) +
            (ky - lastPoint.y) * (ky - lastPoint.y) <
            r2
        )
          continue;
        lastPoint = obstruction.isObstructedTile(kx, ky, radius);
        if (!lastPoint) return kx + ky * w;
      }
    }
    return -1;
  };
  ...
  m.Map.prototype.isObstructedTile = function (kx, ky, radius) {
    ...
    if (!this.pattern || this.pattern[0] != radius) {
      ...
      for (let i = 1; i <= radius; ++i)
        this.pattern.push(
          Math.floor(Math.sqrt(r2 - (i - 0.5) * (i - 0.5)) + 0.5)
        );
    }
    for (let dy = 0; dy <= radius; ++dy) {
      ...
      for (let dx = 0; dx <= dxmax; ++dx) {
        if (this.map[xp + dx] == 0) return { x: kx + dx, y: ky + dy };
        if (this.map[xm + dx] == 0) return { x: kx + dx, y: ky - dy };
        if (this.map[xp - dx] == 0) return { x: kx - dx, y: ky + dy };
        if (this.map[xm - dx] == 0) return { x: kx - dx, y: ky - dy };
      }
    }
  ...
  m.Map.prototype.dumpIm = function (
    name = "default.png",
    threshold = this.maxVal
  ) {

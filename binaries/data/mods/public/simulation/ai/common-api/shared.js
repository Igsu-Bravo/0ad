var API3 = (function (m) {
  ...
  m.SharedScript = function (settings) {
    ...
    this._players = Object.keys(settings.players).map(
      (key) => settings.players[key]
    ); // TODO SM55 Object.values(settings.players)
    this._templates = settings.templates;

    this._entityMetadata = {};
    for (let player of this._players) this._entityMetadata[player] = {};
  ...
  m.SharedScript.prototype.init = function (state, deserialization) {
    if (!deserialization) this._entitiesModifications = new Map();
    ...
    if (this.mapSize % this.passabilityMap.width !== 0)
      error(
        "AI shared component inconsistent sizes: map=" +
          this.mapSize +
          " while passability=" +
          this.passabilityMap.width
      );
...
*/
  ...
  m.SharedScript.prototype.onUpdate = function (state) {
    ...
    for (let i in this.gameState) this.gameState[i].update(this);
  ...
  m.SharedScript.prototype.ApplyEntitiesDelta = function (state) {
    ...
    for (let evt of state.events.Create) {
      if (!state.entities[evt.entity]) continue; // Sometimes there are things like foundations which get destroyed too fast
    ...
    for (let evt of state.events.EntityRenamed) {
      ...
      for (let player of this._players) {
        this._entityMetadata[player][evt.newentity] =
    ...
    for (let evt of state.events.ConstructionFinished) {
      ...
      if (evt.entity != evt.newentity) foundationFinished[evt.entity] = true;
    }

    for (let evt of state.events.AIMetadata) {
      if (!this._entities.has(evt.id)) continue; // might happen in some rare cases of foundations getting destroyed, perhaps.
      // Apply metadata (here for buildings for example)
      for (let key in evt.metadata)
        this.setMetadata(
          evt.owner,
          this._entities.get(evt.id),
          key,
          evt.metadata[key]
        );
    }
    ...
    for (let evt of state.events.Destroy) {
      if (!this._entities.has(evt.entity)) continue; // probably should remove the event.

      if (foundationFinished[evt.entity]) evt.SuccessfulFoundation = true;
    ...
    for (let id in state.changedEntityTemplateInfo) {
      if (!this._entities.has(+id)) continue; // dead, presumably.
      ...
      for (let change of changes) modif.set(change.variable, change.value);
    }
    Engine.ProfileStop();
  };
  ...
  m.SharedScript.prototype.registerUpdatingEntityCollection = function (
    entCollection
  ) {
    ...
    for (let prop of entCollection.dynamicProperties()) {
      ...
      this._entityCollectionsByDynProp[prop].set(
  ...
  m.SharedScript.prototype.removeUpdatingEntityCollection = function (
    entCollection
  ) {
    let uid = entCollection.getUID();

    if (this._entityCollections.has(uid)) this._entityCollections.delete(uid);
  ...
  m.SharedScript.prototype.updateEntityCollections = function (property, ent) {
    if (this._entityCollectionsByDynProp[property] === undefined) return;
  ...
  m.SharedScript.prototype.deleteMetadata = function (player, ent, key) {
    ...
    if (!metadata || !(key in metadata)) return true;
  ...
  m.SharedScript.prototype.createResourceMaps = function () {
    for (const resource of Resources.GetCodes()) {
      if (
        ...
        !(
          Resources.GetResource(resource).aiAnalysisInfluenceGroup in
    ...
    for (const ent of this._entities.values()) this.addEntityToResourceMap(ent);
  };
  ...
  m.SharedScript.prototype.updateResourceMaps = function (events) {
    for (const e of events.Destroy)
      if (e.entityObj) this.removeEntityFromResourceMap(e.entityObj);
   ...
   */
  m.SharedScript.prototype.changeEntityInResourceMapHelper = function (
    ent,
    multiplication = 1
  ) {
    if (!ent) return;
    ...
    if (!resource || !this.resourceMaps[resource]) return;
    ...
    const strength =
      (multiplication * ent.resourceSupplyMax()) /
      this.normalizationFactor[grp];
    this.resourceMaps[resource].addInfluence(
      x,
      y,
      this.influenceRadius[grp] / cellSize,
      strength / 2,
      "constant"
    );
    this.resourceMaps[resource].addInfluence(
    ...
    this.ccResourceMaps[resource].addInfluence(
      x,
      y,
      this.ccInfluenceRadius[grp] / cellSize,
      strength,
      "constant"
    );
  };

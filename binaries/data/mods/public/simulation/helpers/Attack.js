 ...
 */
AttackHelper.prototype.GetAttackEffectsData = function (
  valueModifRoot,
  template,
  entity
) {
  ...
  if (template.Damage) {
    ...
    let applyMods = (damageType) =>
      ApplyValueModificationsToEntity(
        valueModifRoot + "/Damage/" + damageType,
        +(template.Damage[damageType] || 0),
        entity
      );
  ...
  if (template.Capture)
    ret.Capture = ApplyValueModificationsToEntity(
      valueModifRoot + "/Capture",
      +(template.Capture || 0),
      entity
    );

  if (template.ApplyStatus)
    ret.ApplyStatus = this.GetStatusEffectsData(
      valueModifRoot,
      template.ApplyStatus,
      entity
    );

  if (template.Bonuses) ret.Bonuses = template.Bonuses;

  return ret;
};

AttackHelper.prototype.GetStatusEffectsData = function (
  valueModifRoot,
  template,
  entity
) {
  ...
  for (let effect in template) {
    ...
    result[effect] = {
      Duration: ApplyValueModificationsToEntity(
        valueModifRoot + "/ApplyStatus/" + effect + "/Duration",
        +(statusTemplate.Duration || 0),
        entity
      ),
      Interval: ApplyValueModificationsToEntity(
        valueModifRoot + "/ApplyStatus/" + effect + "/Interval",
        +(statusTemplate.Interval || 0),
        entity
      ),
    ...
    Object.assign(
      ...
      this.GetAttackEffectsData(
        valueModifRoot + "/ApplyStatus" + effect,
        statusTemplate,
        entity
      )
    );
    if (statusTemplate.Modifiers)
      result[effect].Modifiers = this.GetStatusEffectsModifications(
        valueModifRoot,
        statusTemplate.Modifiers,
        entity,
        effect
      );
  }
...
AttackHelper.prototype.GetStatusEffectsModifications = function (
  valueModifRoot,
  template,
  entity,
  effect
  ...
  for (let modifier in template) {
    ...
    if (modifierTemplate.Add !== undefined)
      modifiers[modifier].Add = ApplyValueModificationsToEntity(
        valueModifRoot +
          "/ApplyStatus/" +
          effect +
          "/Modifiers/" +
          modifier +
          "/Add",
        +modifierTemplate.Add,
        entity
      );
    if (modifierTemplate.Multiply !== undefined)
      modifiers[modifier].Multiply = ApplyValueModificationsToEntity(
        valueModifRoot +
          "/ApplyStatus/" +
          effect +
          "/Modifiers/" +
 ...
 */
AttackHelper.prototype.GetTotalAttackEffects = function (
  target,
  effectData,
  effectType,
  bonusMultiplier,
  ...
  let resistanceStrengths = cmpResistance
    ? cmpResistance.GetEffectiveResistanceAgainst(effectType)
    : {};

  if (effectType == "Damage")
    for (let type in effectData.Damage)
      total +=
        effectData.Damage[type] *
        Math.pow(
          0.9,
          resistanceStrengths.Damage ? resistanceStrengths.Damage[type] || 0 : 0
        );
  else if (effectType == "Capture") {
    total =
      effectData.Capture * Math.pow(0.9, resistanceStrengths.Capture || 0);

    // If Health is lower we are more susceptible to capture attacks.
    let cmpHealth = Engine.QueryInterface(target, IID_Health);
    if (cmpHealth)
      total /=
        0.1 + (0.9 * cmpHealth.GetHitpoints()) / cmpHealth.GetMaxHitpoints();
  }
  if (effectType != "ApplyStatus") return total * bonusMultiplier;

  if (!resistanceStrengths.ApplyStatus) return effectData[effectType];
  ...
  for (let statusEffect in effectData[effectType]) {
    ...
    if (effectData[effectType][statusEffect].Duration)
      result[statusEffect].Duration =
        effectData[effectType][statusEffect].Duration *
 ...
 */
AttackHelper.prototype.GetPlayersToDamage = function (
  attackerOwner,
  friendlyFire
) {
  if (!friendlyFire) return QueryPlayerIDInterface(attackerOwner).GetEnemies();
...
AttackHelper.prototype.CauseDamageOverArea = function (data) {
  let nearEnts = PositionHelper.EntitiesNearPoint(
    data.origin,
    data.radius,
  ...
  for (let ent of nearEnts) {
    ...
    let distance = cmpObstructionManager.DistanceToPoint(
      ent,
      data.origin.x,
      data.origin.y
    );
    ...
    if (data.shape == "Circular")
      ...
      damageMultiplier =
        1 - (distance * distance) / (data.radius * data.radius);
      ...
      let entityPosition = Engine.QueryInterface(
        ent,
        IID_Position
      ).GetPosition2D();
      let relativePos = entityPosition
        .sub(data.origin)
        .normalize()
        .mult(distance);
      ...
      if (parallelPos >= 0 && Math.abs(perpPos) < width)
        // If in radius, quadratic falloff in both directions
        damageMultiplier =
          (1 - (parallelPos * parallelPos) / (data.radius * data.radius)) *
 ...
 */
AttackHelper.prototype.HandleAttackEffects = function (
  target,
  data,
  bonusMultiplier = 1
) {
  let cmpResistance = Engine.QueryInterface(target, IID_Resistance);
  if (cmpResistance && cmpResistance.IsInvulnerable()) return false;

  bonusMultiplier *= !data.attackData.Bonuses
    ? 1
    : this.GetAttackBonus(
        data.attacker,
        target,
  ...
  if (!Object.keys(targetState).length) return false;
  ...
  if (!!data.attackData.StatusEffect) return true;

  let cmpPromotion = Engine.QueryInterface(data.attacker, IID_Promotion);
  if (cmpPromotion && targetState.xp) cmpPromotion.IncreaseXp(targetState.xp);
 ...
 */
AttackHelper.prototype.GetAttackBonus = function (
  source,
  target,
  type,
  template
  ...
  for (let key in template) {
    ...
    if (bonus.Civ && bonus.Civ !== targetCiv) continue;
    if (!bonus.Classes || MatchesClassList(targetClasses, bonus.Classes))
      attackBonus *= ApplyValueModificationsToEntity(
        "Attack/" + type + "/Bonuses/" + key + "/Multiplier",
        +bonus.Multiplier,
        source
      );
  }

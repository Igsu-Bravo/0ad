...
function GetWallPlacement(placementData, wallSet, start, end) {
  ...
  let towerWidth =
    placementData[wallSet.templates.tower].templateData.wallPiece.length;
  ...
  if (!placement) {
    error(
      "No placement possible for distance=" +
  ...
  for (let i = 0; i < placedEntities.length; ++i) {
    ...
    result.push({
      ...
      pos: {
        x:
          start.pos.x +
          (progress + spacing + placedEntity.len / 2) * dirNormalized.x,
        z:
          start.pos.z +
          (progress + spacing + placedEntity.len / 2) * dirNormalized.z,
      },
      angle: angle,
    });
    ...
    if (i < placedEntities.length - 1) {
      result.push({
        ...
        pos: {
          x:
            start.pos.x +
            (progress + placedEntity.len + 2 * spacing) * dirNormalized.x,
          z:
            start.pos.z +
            (progress + placedEntity.len + 2 * spacing) * dirNormalized.z,
        },
        angle: angle,
      });
    }
...
function GetWallSegmentsRec(
  ...
  for (let candSegment of candidateSegments) {
    ...
    if (r < rLowerBound) {
      ...
      let recursiveResult = GetWallSegmentsRec(
        d,
        candidateSegments,
        minOverlap,
        maxOverlap,

...
function LoadPlayerSettings(settings, newPlayers) {
  var playerDefaults = Engine.ReadJSONFile(
  ...
  var cmpPlayerManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_PlayerManager
  );
  ...
  if (newPlayers) {
    ...
    if (playerData) settingsNumPlayers = playerData.length;
    // includes gaia (see above)
    else warn("Player.js: Setup has no player data - using defaults");

    while (settingsNumPlayers > numPlayers) {
      // Add player entity to engine
      var entID = Engine.AddEntity(
        GetPlayerTemplateName(
          getSetting(playerData, playerDefaults, numPlayers, "Civ")
        )
      );
      var cmpPlayer = Engine.QueryInterface(entID, IID_Player);
      if (!cmpPlayer)
        throw new Error(
          "Player.js: Error creating player entity " + numPlayers
        );
  ...
  for (var i = 0; i < numPlayers; ++i) {
    var template = GetPlayerTemplateName(
      getSetting(playerData, playerDefaults, i, "Civ")
    );
    var entID = cmpPlayerManager.GetPlayerByID(i);
    if (cmpTemplateManager.GetCurrentTemplateName(entID) === template) continue;
  ...
  for (var i = 0; i < numPlayers; ++i) {
    ...
    if (i == 0) {
      ...
      for (var j = 1; j < numPlayers; ++j) cmpPlayer.SetEnemy(j);
    ...
    if (getSetting(playerData, playerDefaults, i, "Diplomacy") !== undefined)
      cmpPlayer.SetDiplomacy(
        getSetting(playerData, playerDefaults, i, "Diplomacy")
      );
    else {
      // Init diplomacy
      ...
      for (var j = 0; j < numPlayers; ++j) {
        if (i == j) cmpPlayer.SetAlly(j);
        else cmpPlayer.SetEnemy(j);
      }
    ...
    cmpPlayer.SetFormations(
      getSetting(playerData, playerDefaults, i, "Formations") ||
        Engine.ReadJSONFile(
          "simulation/data/civs/" + cmpPlayer.GetCiv() + ".json"
        ).Formations
    );
...
function GetPlayerTemplateName(civ) {
  ...
  if (
    Engine.QueryInterface(SYSTEM_ENTITY, IID_TemplateManager).TemplateExists(
      path + "_" + civ
    )
  )
    return path + "_" + civ;
...
function QueryOwnerEntityID(ent) {
  ...
  if (cmpPlayer) return ent;

  let cmpOwnership = Engine.QueryInterface(ent, IID_Ownership);
  if (!cmpOwnership) return null;

  let owner = cmpOwnership.GetOwner();
  if (owner == INVALID_PLAYER) return null;

  let cmpPlayerManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_PlayerManager
  );
  if (!cmpPlayerManager) return null;
...
function QueryMiragedInterface(ent, iid) {
  ...
  if (cmpMirage && !cmpMirage.Mirages(iid)) return null;
  else if (!cmpMirage) return Engine.QueryInterface(ent, iid);
...
function IsOwnedByEntityHelper(entity, target, check) {
  ...
  if (cmpOwnershipTarget) targetOwner = cmpOwnershipTarget.GetOwner();

 ...
 */
PositionHelper.prototype.DistanceBetweenEntities = function (
  firstEntity,
  secondEntity
) {
  let cmpFirstPosition = Engine.QueryInterface(firstEntity, IID_Position);
  if (!cmpFirstPosition || !cmpFirstPosition.IsInWorld()) return Infinity;

  let cmpSecondPosition = Engine.QueryInterface(secondEntity, IID_Position);
  if (!cmpSecondPosition || !cmpSecondPosition.IsInWorld()) return Infinity;

  return cmpFirstPosition
    .GetPosition2D()
    .distanceTo(cmpSecondPosition.GetPosition2D());
};
 ...
 */
PositionHelper.prototype.EntitiesNearPoint = function (
  origin,
  radius,
  players,
  iid = 0
) {
  if (!origin || !radius || !players || !players.length) return [];
...
PositionHelper.prototype.InterpolatedLocation = function (ent, lateness) {
  ...
  if (!cmpTargetPosition || !cmpTargetPosition.IsInWorld())
    // TODO: handle dead target properly
    return undefined;
...
PositionHelper.prototype.TestCollision = function (ent, point, lateness) {
  ...
  if (targetShape.type == "circle")
    return (
      targetPosition.horizDistanceToSquared(point) <
      targetShape.radius * targetShape.radius
    );

  if (targetShape.type == "square") {
    let angle = Engine.QueryInterface(ent, IID_Position).GetRotation().y;
    let distance = Vector2D.from3D(Vector3D.sub(point, targetPosition)).rotate(
      -angle
    );
    return (
      Math.abs(distance.x) < targetShape.width / 2 &&
      Math.abs(distance.y) < targetShape.depth / 2
    );
  }

  warn(
    "TestCollision called with an invalid footprint shape: " +
      targetShape.type +
      "."
  );
  return false;
 ...
 */
PositionHelper.prototype.PredictTimeToTarget = function (
  firstPosition,
  selfSpeed,
  targetPosition,
  targetVelocity
  ...
  let b =
    ...
    relativePosition.z * targetVelocity.z;
  let c =
  ...
  let disc = b * b - a * c;
  if (a < 0 || (b < 0 && disc >= 0)) return c / (Math.sqrt(disc) - b);
...
PositionHelper.prototype.GetSpawnPosition = function (target, entity, forced) {
  ...
  if (
    ...
    cmpIdentity &&
    cmpIdentity.HasClass("Ship")
  )
    pos = cmpFootprint.PickSpawnPointBothPass(entity);
  else pos = cmpFootprint.PickSpawnPoint(entity);

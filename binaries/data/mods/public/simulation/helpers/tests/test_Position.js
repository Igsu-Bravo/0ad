...
class testDistanceBetweenEntities {
  ...
  test() {
    ...
    this.testDistanceBetweenEntities(
      new Vector3D(1, 0, 0),
      new Vector3D(0, 0, 0),
      1
    );
...
class testInterpolatedLocation {
  ...
  testOutOfWorldEntity() {
    ...
    TS_ASSERT_EQUALS(
      PositionHelper.InterpolatedLocation(this.entity, 0),
      undefined
    );
  ...
  testInterpolatedLocation(
    ...
    TS_ASSERT_UNEVAL_EQUALS(
      PositionHelper.InterpolatedLocation(this.entity, lateness),
      expectedPosition
    );
...
class testTestCollision {
  ...
  testOutOfWorldEntity() {
    ...
    TS_ASSERT(
      !PositionHelper.TestCollision(this.entity, new Vector3D(0, 0, 0), 0)
    );
  ...
  testTestCollision(footprint, collisionPoint, entityPosition, expectedResult) {
    ...
    TS_ASSERT_EQUALS(
      PositionHelper.TestCollision(this.entity, collisionPoint, 0),
      expectedResult
    );
  ...
  testSquareFootprints() {
    ...
    this.testTestCollision(
      square,
      new Vector3D(0, 0, 0),
      new Vector3D(0.999, 0, 0),
      true
    );
    this.testTestCollision(
      square,
      new Vector3D(0, 0, 0),
      new Vector3D(0.999, 0, 0.999),
      true
    );
    this.testTestCollision(
      square,
    ...
    this.testTestCollision(
      square,
      new Vector3D(-1, 0, -1),
      new Vector3D(-0.001, 0, -0.001),
      true
    );
    this.testTestCollision(
      { type: "square", width: 4, depth: 4 },
      new Vector3D(-1, 0, 1),
      new Vector3D(0.999, 0, -0.999),
      true
    );
    ...
    this.testTestCollision(
      square,
      new Vector3D(0, 0, 0),
      new Vector3D(0.5, 10, 0.5),
      true
    );
    this.testTestCollision(
      square,
      new Vector3D(-1, 50, 0),
      new Vector3D(-0.5, 10, 0),
      true
    );
  ...
  testRectangularFootprints() {
    ...
    this.testTestCollision(
      rectangle,
      new Vector3D(0, 0, 0),
      new Vector3D(1.999, 0, 0),
      false
    );
    this.testTestCollision(
      rectangle,
      new Vector3D(0, 0, 0),
      new Vector3D(0, 0, 1.999),
      true
    );
...
class testPredictTimeToTarget {
  ...
  testPredictTimeToTarget(targetPosition, targetVelocity) {
    ...
    if (timeToTarget === false) return;
    // Position of the target after that time.
    let targetPos = Vector3D.mult(targetVelocity, timeToTarget).add(
      targetPosition
    );

function Cheat(input) {
  ...
  if (!cmpPlayerManager || input.player < 0) return;
  var playerEnt = cmpPlayerManager.GetPlayerByID(input.player);
  if (playerEnt == INVALID_ENTITY) return;
  var cmpPlayer = Engine.QueryInterface(playerEnt, IID_Player);
  if (!cmpPlayer) return;

  var cmpGuiInterface = Engine.QueryInterface(SYSTEM_ENTITY, IID_GuiInterface);
  if (!cmpPlayer.GetCheatsEnabled()) return;
  ...
  switch (input.action) {
    ...
    case "convertunit":
      for (let ent of input.selected) {
        ...
        if (cmpOwnership) cmpOwnership.SetOwner(cmpPlayer.GetPlayerID());
      }
      return;
    case "killunits":
      for (let ent of input.selected) {
    ...
    case "createunits":
      var cmpProductionQueue =
        ...
        Engine.QueryInterface(input.selected[0], IID_ProductionQueue);
      if (!cmpProductionQueue) {
        cmpGuiInterface.PushNotification({
          type: "text",
          players: [input.player],
          message: markForTranslation(
            "You need to select a building that trains units."
          ),
      ...
      let cmpOwnership = Engine.QueryInterface(
        input.selected[0],
        IID_Ownership
      );
      ...
      for (
        ...
        Math.min(
          input.parameter,
          cmpPlayer.GetMaxPopulation() - cmpPlayer.GetPopulationCount()
        );
        ++i
      )
    ...
    case "fastactions": {
      ...
      if (cmpModifiersManager.HasAnyModifier("cheat/fastactions", playerEnt))
        ...
        cmpModifiersManager.AddModifiers(
          ...
          {
            "Cost/BuildTime": [
              { affects: [["Structure"], ["Unit"]], multiply: 0.01 },
            ],
            "ResourceGatherer/BaseSpeed": [
              { affects: [["Structure"], ["Unit"]], multiply: 1000 },
            ],
            "Pack/Time": [
              { affects: [["Structure"], ["Unit"]], multiply: 0.01 },
            ],
    ...
    case "changephase":
      var cmpTechnologyManager = Engine.QueryInterface(
        playerEnt,
    ...
    case "researchTechnology":
      if (!input.parameter.length) return;
      ...
      if (input.selected[0]) {
        var cmpProductionQueue = Engine.QueryInterface(
          input.selected[0],
          IID_ProductionQueue
        );
        if (cmpProductionQueue) {
          ...
          var number = +tmp[0];
          var pair =
            tmp.length > 1 && (tmp[1] == "top" || tmp[1] == "bottom")
              ? tmp[1]
              : "top"; // use top as default value
          ...
          if (number || number === 0) {
            ...
            if (number > 0 && number <= techs.length) {
              ...
              // get name of tech
              if (tech.pair) techname = tech[pair];
              else techname = tech;
            } else return;
          }
        }
      ...
      if (
        TechnologyTemplates.Has(techname) &&

...
function ChangeEntityTemplate(oldEnt, newTemplate) {
  ...
  if (newEnt == INVALID_ENTITY) {
    error(
      "Transform.js: Error replacing entity " +
        oldEnt +
        " for a '" +
        newTemplate +
        "'"
  ...
  if (cmpTurretHolder && cmpNewTurretHolder)
    for (let entity of cmpTurretHolder.GetEntities())
      cmpNewTurretHolder.SetReservedTurretPoint(
        cmpTurretHolder.GetOccupiedTurretPointName(entity)
      );
  ...
  if (
    cmpTerritoryDecay &&
    cmpTerritoryDecay.HasTerritoryOwnership() &&
    cmpNewPosition
  ) {
  ...
  if (cmpNewOwnership) cmpNewOwnership.SetOwner(owner);
  ...
  if (cmpCapturable && cmpNewCapturable) {
    let scale =
      ...
      cmpNewCapturable.GetMaxCapturePoints();
    let newCapturePoints = cmpCapturable
      .GetCapturePoints()
      .map((v) => v / scale);
  ...
  if (cmpHealth && cmpNewHealth) {
    var healthLevel = Math.max(
      0,
      Math.min(1, cmpHealth.GetHitpoints() / cmpHealth.GetMaxHitpoints())
    );
  ...
  if (cmpGuard && cmpNewGuard) {
    ...
    if (entities.length) {
      ...
      for (let ent of entities) {
        ...
        if (cmpEntUnitAI) cmpEntUnitAI.SetGuardOf(newEnt);
      }
    }
  }

  let cmpStatusEffectsReceiver = Engine.QueryInterface(
    oldEnt,
    IID_StatusEffectsReceiver
  );
  let cmpNewStatusEffectsReceiver = Engine.QueryInterface(
    newEnt,
    IID_StatusEffectsReceiver
  );
  if (cmpStatusEffectsReceiver && cmpNewStatusEffectsReceiver) {
    ...
    for (let status in activeStatus) {
      ...
      if (newStatus.Duration) newStatus.Duration -= newStatus._timeElapsed;
      cmpNewStatusEffectsReceiver.ApplyStatus(
        { [status]: newStatus },
        newStatus.source.entity,
        newStatus.source.owner
      );
    }
  }
  ...
  Engine.PostMessage(oldEnt, MT_EntityRenamed, {
    entity: oldEnt,
    newentity: newEnt,
  });
  ...
  if (cmpUnitAI && cmpNewUnitAI) {
    ...
    if (pos) cmpNewUnitAI.SetHeldPosition(pos.x, pos.z);
  ...
  if (cmpPosition && cmpPosition.IsInWorld()) cmpPosition.MoveOutOfWorld();
...
function ObstructionsBlockingTemplateChange(ent, templateArg) {
  ...
  if (previewEntity == INVALID_ENTITY) return true;

  CopyControlGroups(ent, previewEntity);
  var cmpBuildRestrictions = Engine.QueryInterface(
    previewEntity,
  ...
  var template = cmpTemplateManager.GetTemplate(
    cmpTemplateManager.GetCurrentTemplateName(ent)
  );
  ...
  if (template.Obstruction && newTemplate.Obstruction) {
    ...
    if (
      newTemplate.Obstruction.Obstructions ||
      (newTemplate.Obstruction.Static &&
        template.Obstruction.Static &&
        (newTemplate.Obstruction.Static["@width"] >
          template.Obstruction.Static["@width"] ||
          newTemplate.Obstruction.Static["@depth"] >
            template.Obstruction.Static["@depth"])) ||
      (newTemplate.Obstruction.Static &&
        template.Obstruction.Unit &&
        (newTemplate.Obstruction.Static["@width"] >
          template.Obstruction.Unit["@radius"] ||
          newTemplate.Obstruction.Static["@depth"] >
            template.Obstruction.Unit["@radius"])) ||
      (newTemplate.Obstruction.Unit &&
        template.Obstruction.Unit &&
        newTemplate.Obstruction.Unit["@radius"] >
          template.Obstruction.Unit["@radius"]) ||
      (newTemplate.Obstruction.Unit &&
        template.Obstruction.Static &&
        (newTemplate.Obstruction.Unit["@radius"] >
      ...
      var cmpNewObstruction = Engine.QueryInterface(
        previewEntity,
        IID_Obstruction
      );
...
function DeleteEntityAndReturn(
  ent,
  cmpPosition,
  position,
  angle,
...
function TransferGarrisonedUnits(oldEnt, newEnt) {
  ...
  if (!cmpOldGarrison || !cmpOldGarrison.GetEntities().length) return;
  ...
  for (let ent of entities) {
    ...
    if (!cmpNewGarrison) continue;
    let cmpGarrisonable = Engine.QueryInterface(ent, IID_Garrisonable);
    if (!cmpGarrisonable) continue;

...
function PreInitGame() {
  ...
  for (
    ...
    if (cmpTechnologyManager) cmpTechnologyManager.UpdateAutoResearch();
  }
...
function InitGame(settings) {
  ...
  let rate = [0.42, 0.56, 0.75, 1.0, 1.25, 1.56];
  let time = [1.4, 1.25, 1.1, 1.0, 1.0, 1.0];
  ...
  for (let i = 0; i < settings.PlayerData.length; ++i) {
    ...
    if (settings.PlayerData[i] && !!settings.PlayerData[i].AI) {
      ...
      cmpAIManager.AddPlayer(
        settings.PlayerData[i].AI,
        i,
        AIDiff,
        settings.PlayerData[i].AIBehavior || "random"
      );
      ...
      cmpModifiersManager.AddModifiers(
        "AI Bonus",
        {
          "ResourceGatherer/BaseSpeed": [
            { affects: ["Unit", "Structure"], multiply: rate[AIDiff] },
          ],
          "Trader/GainMultiplier": [
            { affects: ["Unit", "Structure"], multiply: rate[AIDiff] },
          ],
          "Cost/BuildTime": [
            { affects: ["Unit", "Structure"], multiply: time[AIDiff] },
          ],
        },
        cmpPlayer.entity
      );
  ...
  if (settings.WorldPopulationCap)
    Engine.QueryInterface(
      SYSTEM_ENTITY,
      IID_PlayerManager
    ).SetMaxWorldPopulation(settings.WorldPopulationCap);

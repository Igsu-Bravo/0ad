...
class ObstructionSnap {
  getValidEdges(allEdges, position, maxSide) {
    ...
    for (let edge of allEdges) {
      let signedDistance =
        Vector2D.dot(edge.normal, position) -
      ...
      if (offsetDistance > this.MinimalDistanceToSnap + maxSide) continue;
  ...
  compareEdges(a, b) {
    ...
    if (Math.abs(scoreA - scoreB) < this.EPS) {
      ...
      if (!behindA) return a.offsetDistance - b.offsetDistance;
  ...
  getNearestSizeAlongNormal(width, depth, angle, normal) {
    ...
    if (Math.abs(dot) > threshold) return [depth, width];
    return [width, depth];
  }
  ...
  getPosition(data, template) {
    ...
    for (let edge of edges)
      if (this.compareEdges(edge, baseEdge) < 0) baseEdge = edge;
    ...
    let [sizeToBaseEdge, sizeToPairedEdge] = this.getNearestSizeAlongNormal(
      width,
      depth,
      templateAngle,
      baseEdge.normal
    ...
    let distance =
      Vector2D.dot(baseEdge.normal, templatePos) -
      Vector2D.dot(baseEdge.normal, baseEdge.begin);
    templatePos.sub(
      Vector2D.mult(
        baseEdge.normal,
        distance - sizeToBaseEdge - this.getPadding(baseEdge)
      )
    );
    ...
    if (edges.length > 1) {
      ...
      if (pairedEdges.length) {
        ...
        for (let edge of pairedEdges)
          if (this.compareEdges(edge, secondEdge) < 0) secondEdge = edge;
        let distance =
          Vector2D.dot(secondEdge.normal, templatePos) -
          Vector2D.dot(secondEdge.normal, secondEdge.begin);
        templatePos.sub(
          Vector2D.mult(
            secondEdge.normal,
            distance - sizeToPairedEdge - this.getPadding(secondEdge)
          )
        );
      }
    }

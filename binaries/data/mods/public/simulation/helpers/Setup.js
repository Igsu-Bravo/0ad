...
function LoadMapSettings(settings) {
  ...
  if (settings.RevealMap) {
    ...
    if (cmpRangeManager) cmpRangeManager.SetLosRevealAll(-1, true);
  }
  ...
  if (cmpRangeManager) cmpRangeManager.SetLosCircular(!!settings.CircularMap);

  const cmpObstructionManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
  ...
  if (settings.TriggerDifficulty !== undefined)
    Engine.QueryInterface(SYSTEM_ENTITY, IID_Trigger).SetDifficulty(
      settings.TriggerDifficulty
    );
  else if (settings.SupportedTriggerDifficulties) {
    // used by Atlas and autostart games
    let difficulties = Engine.ReadJSONFile(
      "simulation/data/settings/trigger_difficulties.json"
    ).Data;
    let defaultDiff;
    if (settings.SupportedTriggerDifficulties.Default)
      defaultDiff = difficulties.find(
        (d) => d.Name == settings.SupportedTriggerDifficulties.Default
      ).Difficulty;
    else defaultDiff = difficulties.find((d) => d.Default).Difficulty;
    Engine.QueryInterface(SYSTEM_ENTITY, IID_Trigger).SetDifficulty(
      defaultDiff
    );
  }

  const cmpEndGameManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_EndGameManager
  );
  ...
  cmpEndGameManager.SetAlliedVictory(
    settings.LockTeams || !settings.LastManStanding
  );
  if (settings.LockTeams && settings.LastManStanding)
    warn("Last man standing is only available in games with unlocked teams!");

  const cmpCeasefireManager = Engine.QueryInterface(
    SYSTEM_ENTITY,

...
function ProcessCommand(player, cmd) {
  ...
  if (cmd.entities)
    data.entities = FilterEntityList(
      cmd.entities,
      player,
      data.controlAllUnits
    );
  ...
  if (cmd.type == "delete-entities") {
    let cmpPosition =
      cmd.entities[0] && Engine.QueryInterface(cmd.entities[0], IID_Position);
    commandData.position =
      cmpPosition && cmpPosition.IsInWorld() && cmpPosition.GetPosition2D();
  }
  ...
  } else error("Invalid command: unknown command type: " + uneval(cmd));
}

var g_Commands = {
  aichat: function (player, cmd, data) {
    var cmpGuiInterface = Engine.QueryInterface(
      SYSTEM_ENTITY,
      IID_GuiInterface
    );
    var notification = { players: [player] };
    for (var key in cmd) notification[key] = cmd[key];
  ...
  "collect-treasure": function (player, cmd, data) {
    GetFormationUnitAIs(data.entities, player, cmd, data.formation).forEach(
      (cmpUnitAI) => {
  ...
  diplomacy: function (player, cmd, data) {
    ...
    switch (cmd.to) {
      ...
      default:
        warn(
          "Invalid command: Could not set " +
            player +
            " diplomacy status of player " +
            cmd.player +
            " to " +
            cmd.to
        );
  ...
  "control-all": function (player, cmd, data) {
    if (!data.cmpPlayer.GetCheatsEnabled()) return;
  ...
  attack: function (player, cmd, data) {
    ...
    if (
      ...
      !allowCapture &&
      !(
        IsOwnedByEnemyOfPlayer(player, cmd.target) ||
        IsOwnedByNeutralOfPlayer(player, cmd.target)
      )
    )
      warn(
        "Invalid command: attack target is not owned by enemy of player " +
          player +
          ": " +
          uneval(cmd)
      );
  ...
  heal: function (player, cmd, data) {
    if (
      g_DebugCommands &&
      !(
        IsOwnedByPlayer(player, cmd.target) ||
        IsOwnedByAllyOfPlayer(player, cmd.target)
      )
    )
      warn(
        "Invalid command: heal target is not owned by player " +
          player +
          " or their ally: " +
          uneval(cmd)
      );
  ...
  repair: function (player, cmd, data) {
    ...
    if (g_DebugCommands && !IsOwnedByAllyOfPlayer(player, cmd.target))
      warn(
        "Invalid command: repair target is not owned by ally of player " +
          player +
          ": " +
          uneval(cmd)
      );
  ...
  gather: function (player, cmd, data) {
    if (
      g_DebugCommands &&
      !(IsOwnedByPlayer(player, cmd.target) || IsOwnedByGaia(cmd.target))
    )
      warn(
        "Invalid command: resource is not owned by gaia or player " +
          player +
          ": " +
          uneval(cmd)
      );
  ...
  "gather-near-position": function (player, cmd, data) {
    GetFormationUnitAIs(data.entities, player, cmd, data.formation).forEach(
      (cmpUnitAI) => {
        cmpUnitAI.GatherNearPosition(
          cmd.x,
          cmd.z,
          cmd.resourceType,
          cmd.resourceTemplate,
          cmd.queued,
          cmd.pushFront
        );
      }
  ...
  returnresource: function (player, cmd, data) {
    if (g_DebugCommands && !IsOwnedByPlayer(player, cmd.target))
      warn(
        "Invalid command: dropsite is not owned by player " +
          player +
          ": " +
          uneval(cmd)
      );
  ...
  "remove-guard": function (player, cmd, data) {
    for (let ent of data.entities) {
      var cmpUnitAI = Engine.QueryInterface(ent, IID_UnitAI);
      if (cmpUnitAI) cmpUnitAI.RemoveGuard();
    }
  },
  ...
  train: function (player, cmd, data) {
    ...
    // Check entity limits
    var template = Engine.QueryInterface(
      SYSTEM_ENTITY,
      IID_TemplateManager
    ...
    if (data.entities.length <= 0) {
      if (g_DebugCommands)
        warn(
          "Invalid command: training building(s) cannot be controlled by player " +
            player +
            ": " +
            uneval(cmd)
        );
    ...
    for (let ent of data.entities) {
      if (unitCategory) {
        ...
        if (
          ...
          !cmpPlayerEntityLimits.AllowedToTrain(
            unitCategory,
            cmd.count,
            cmd.template,
            template.TrainingRestrictions.MatchLimit
          )
        ) {
          if (g_DebugCommands)
      ...
      var cmpTechnologyManager = QueryOwnerInterface(
        ent,
        IID_TechnologyManager
      );
      if (
        cmpTechnologyManager &&
        !cmpTechnologyManager.CanProduce(cmd.template)
      ) {
        if (g_DebugCommands)
          warn(
            "Invalid command: training requires unresearched technology: " +
              uneval(cmd)
          );
        continue;
      }
      ...
      if (queue && data.cmpPlayer.IsAI()) {
        ...
        if (list.indexOf(cmd.template) === -1 && cmd.promoted) {
          for (var promoted of cmd.promoted) {
            if (list.indexOf(promoted) === -1) continue;
      ...
      if (queue && queue.GetEntitiesList().indexOf(cmd.template) != -1)
        ...
        else queue.AddItem(cmd.template, "unit", +cmd.count);
    }
  },
  ...
  research: function (player, cmd, data) {
    ...
    if (
      cmpTechnologyManager &&
      !cmpTechnologyManager.CanResearch(cmd.template)
    ) {
      if (g_DebugCommands)
        warn(
          "Invalid command: Requirements to research technology are not met: " +
            uneval(cmd)
        );
      return;
    }
    ...
    if (queue) queue.AddItem(cmd.template, "technology");
  },
  ...
  "stop-production": function (player, cmd, data) {
    ...
    if (cmpProductionQueue) cmpProductionQueue.RemoveItem(cmd.id);
  },
  ...
  "delete-entities": function (player, cmd, data) {
    for (let ent of data.entities) {
      if (!data.controlAllUnits) {
        ...
        if (cmpIdentity && cmpIdentity.IsUndeletable()) continue;

        let cmpCapturable = QueryMiragedInterface(ent, IID_Capturable);
        if (
          cmpCapturable &&
          cmpCapturable.GetCapturePoints()[player] <
            cmpCapturable.GetMaxCapturePoints() / 2
        )
          continue;
      ...
      if (cmpMirage) {
        let cmpMiragedHealth = Engine.QueryInterface(
          cmpMirage.parent,
          IID_Health
        );
        if (cmpMiragedHealth) cmpMiragedHealth.Kill();
        else Engine.DestroyEntity(cmpMirage.parent);
      ...
      if (cmpHealth) cmpHealth.Kill();
      else Engine.DestroyEntity(ent);
    }
  },
  ...
  "set-rallypoint": function (player, cmd, data) {
    for (let ent of data.entities) {
      ...
      if (cmpRallyPoint) {
        if (!cmd.queued) cmpRallyPoint.Unset();
  ...
  "unset-rallypoint": function (player, cmd, data) {
    for (let ent of data.entities) {
      var cmpRallyPoint = Engine.QueryInterface(ent, IID_RallyPoint);
      if (cmpRallyPoint) cmpRallyPoint.Reset();
    }
  },

  resign: function (player, cmd, data) {
    data.cmpPlayer.SetState(
      "defeated",
      markForTranslation("%(player)s has resigned.")
    );
  },
  ...
  garrison: function (player, cmd, data) {
    if (!CanPlayerOrAllyControlUnit(cmd.target, player, data.controlAllUnits)) {
      if (g_DebugCommands)
        warn(
          "Invalid command: garrison target cannot be controlled by player " +
            player +
            " (or ally): " +
            uneval(cmd)
        );
  ...
  guard: function (player, cmd, data) {
    if (
      !IsOwnedByPlayerOrMutualAlly(cmd.target, player, data.controlAllUnits)
    ) {
      if (g_DebugCommands)
        warn(
          "Invalid command: Guard/escort target is not owned by player " +
            player +
            " or ally thereof: " +
            uneval(cmd)
        );
  ...
  "leave-turret": function (player, cmd, data) {
    ...
    for (let ent of data.entities) {
      ...
      if (!cmpTurretable || !cmpTurretable.LeaveTurret()) ++notUnloaded;
    }

    if (notUnloaded) notifyUnloadFailure(player);
  },
  ...
  unload: function (player, cmd, data) {
    if (
      ...
      if (g_DebugCommands)
        warn(
          "Invalid command: unload target cannot be controlled by player " +
            player +
            " (or ally): " +
            uneval(cmd)
        );
    ...
    var cmpGarrisonHolder = Engine.QueryInterface(
      cmd.garrisonHolder,
      IID_GarrisonHolder
    );
    ...
    if (notUngarrisoned != 0) notifyUnloadFailure(player, cmd.garrisonHolder);
  },

  "unload-template": function (player, cmd, data) {
    var entities = FilterEntityListWithAllies(
      cmd.garrisonHolders,
      player,
      data.controlAllUnits
    );
    for (let garrisonHolder of entities) {
      ...
      if (cmpGarrisonHolder) {
        ...
        if (
          !IsOwnedByPlayer(player, garrisonHolder) &&
          !data.controlAllUnits &&
          player != +cmd.owner
        )
          continue;
  ...
  "alert-raise": function (player, cmd, data) {
    for (let ent of data.entities) {
      ...
      if (cmpAlertRaiser) cmpAlertRaiser.RaiseAlert();
    }
  },
  ...
  "alert-end": function (player, cmd, data) {
    for (let ent of data.entities) {
      var cmpAlertRaiser = Engine.QueryInterface(ent, IID_AlertRaiser);
      if (cmpAlertRaiser) cmpAlertRaiser.EndOfAlert();
    }
  },
    ...
    for (let ent of cmd.entities) {
      ...
      if (cmpPromotion)
        cmpPromotion.IncreaseXp(
          cmpPromotion.GetRequiredXp() - cmpPromotion.GetCurrentXp()
        );
    }
  },
  ...
  "lock-gate": function (player, cmd, data) {
    for (let ent of data.entities) {
      ...
      if (cmd.lock) cmpGate.LockGate();
      else cmpGate.UnlockGate();
    }
  },
  ...
  pack: function (player, cmd, data) {
    for (let ent of data.entities) {
      ...
      if (cmd.pack) cmpUnitAI.Pack(cmd.queued, cmd.pushFront);
      else cmpUnitAI.Unpack(cmd.queued, cmd.pushFront);
    }
  },
  ...
  "cancel-pack": function (player, cmd, data) {
    for (let ent of data.entities) {
      ...
      if (cmd.pack) cmpUnitAI.CancelPack(cmd.queued, cmd.pushFront);
      else cmpUnitAI.CancelUnpack(cmd.queued, cmd.pushFront);
    }
  },
  ...
  upgrade: function (player, cmd, data) {
    for (let ent of data.entities) {
      ...
      if (!cmpUpgrade || !cmpUpgrade.CanUpgradeTo(cmd.template)) continue;

      if (
        cmpUpgrade.WillCheckPlacementRestrictions(cmd.template) &&
        ObstructionsBlockingTemplateChange(ent, cmd.template)
      ) {
        var cmpGUIInterface = Engine.QueryInterface(
          SYSTEM_ENTITY,
          IID_GuiInterface
        ...
        cmpGUIInterface.PushNotification({
          ...
          message: markForTranslation(
            "Cannot upgrade as distance requirements are not verified or terrain is obstructed."
          ),
        });
        continue;
      }
      ...
      if (
        cmpEntityLimits &&
        !cmpEntityLimits.AllowedToReplace(ent, cmd.template)
      ) {
        if (g_DebugCommands)
          warn(
            "Invalid command: build limits check failed for player " +
              player +
              ": " +
              uneval(cmd)
          );
      ...
      let cmpTechnologyManager = QueryOwnerInterface(
        ent,
        IID_TechnologyManager
      );
      ...
      if (
        requiredTechnology &&
        (!cmpTechnologyManager ||
          !cmpTechnologyManager.IsTechnologyResearched(requiredTechnology))
      ) {
        if (g_DebugCommands)
          warn(
            "Invalid command: upgrading is not possible for this player or requires unresearched technology: " +
              uneval(cmd)
          );
        continue;
      }
  ...
  "cancel-upgrade": function (player, cmd, data) {
    for (let ent of data.entities) {
      ...
      if (cmpUpgrade) cmpUpgrade.CancelUpgrade(player);
    }
  },
  ...
  "attack-request": function (player, cmd, data) {
    ...
    cmpGuiInterface.PushNotification({
      ...
      message:
        "/allies " +
        markForTranslation("Attack against %(_player_)s requested."),
    ...
    if (cmpAIInterface) cmpAIInterface.PushEvent("AttackRequest", cmd);
  },
  ...
  "spy-request": function (player, cmd, data) {
    ...
    let ent = pickRandom(
      cmpRangeManager.GetEntitiesByPlayer(cmd.player).filter((ent) => {
        let cmpVisionSharing = Engine.QueryInterface(ent, IID_VisionSharing);
        return (
          cmpVisionSharing &&
          cmpVisionSharing.IsBribable() &&
          !cmpVisionSharing.ShareVisionWith(player)
        );
      })
    );
    ...
    if (ent) Engine.QueryInterface(ent, IID_VisionSharing).AddSpy(cmd.source);
    else {
      let template = Engine.QueryInterface(
        SYSTEM_ENTITY,
        IID_TemplateManager
      ).GetTemplate("special/spy");
      IncurBribeCost(template, player, cmd.player, true);
      // update statistics for failed bribes
      let cmpBribesStatisticsTracker = QueryPlayerIDInterface(
        player,
  ...
  "diplomacy-request": function (player, cmd, data) {
    let cmpAIInterface = Engine.QueryInterface(SYSTEM_ENTITY, IID_AIInterface);
    if (cmpAIInterface) cmpAIInterface.PushEvent("DiplomacyRequest", cmd);
  },

  "tribute-request": function (player, cmd, data) {
    let cmpAIInterface = Engine.QueryInterface(SYSTEM_ENTITY, IID_AIInterface);
    if (cmpAIInterface) cmpAIInterface.PushEvent("TributeRequest", cmd);
  },
  ...
  "autoqueue-on": function (player, cmd, data) {
    for (let ent of data.entities) {
      ...
      if (cmpProductionQueue) cmpProductionQueue.EnableAutoQueue();
    }
  },
  ...
  "autoqueue-off": function (player, cmd, data) {
    for (let ent of data.entities) {
      let cmpProductionQueue = Engine.QueryInterface(ent, IID_ProductionQueue);
      if (cmpProductionQueue) cmpProductionQueue.DisableAutoQueue();
    }
  },
};
...
function ExtractFormations(ents) {
  ...
  for (let ent of ents) {
    ...
    if (fid == INVALID_ENTITY) continue;
...
function GetDockAngle(template, x, z) {
  ...
  if (!cmpTerrain || !cmpWaterManager) return undefined;
  ...
  if (template.Footprint.Square)
    halfSize =
      Math.max(
        template.Footprint.Square["@depth"],
        template.Footprint.Square["@width"]
      ) / 2;
  ...
  for (var dist = 0; dist < 4; ++dist) {
    ...
    for (var i = 0; i < numPoints; ++i) {
      ...
      if (
        cmpTerrain.GetGroundLevel(nx, nz) <
        cmpWaterManager.GetWaterLevel(nx, nz)
      )
        waterPoints.push(i);
    }
    var consec = [];
    ...
    for (var i = 0; i < length; ++i) {
      ...
      for (let j = 0; j < length - 1; ++j) {
        if (
          (waterPoints[(i + j) % length] + 1) % numPoints ==
          waterPoints[(i + j + 1) % length]
        )
          ++count;
        else break;
      }
    ...
    if (count != numPoints - 1)
      return (
        (-((waterPoints[start] + consec[start] / 2) % numPoints) / numPoints) *
        2 *
        Math.PI
      );
  }
...
function TryConstructBuilding(player, cmpPlayer, controlAllUnits, cmd) {
  ...
  if (template.BuildRestrictions.PlacementType === "shore") {
    ...
    if (angleDock !== undefined) angle = angleDock;
  }
  ...
  if (cmd.obstructionControlGroup || cmd.obstructionControlGroup2) {
    ...
    if (cmd.obstructionControlGroup) {
      if (cmd.obstructionControlGroup <= 0)
        warn(
          "[TryConstructBuilding] Invalid primary obstruction control group " +
            cmd.obstructionControlGroup +
            " received; must be > 0"
        );
  ...
  if (cmpBuildRestrictions) {
    ...
    if (!ret.success) {
      if (g_DebugCommands)
        warn(
          "Invalid command: build restrictions check failed with '" +
            ret.message +
            "' for player " +
            player +
            ": " +
  ...
  } else error("cmpBuildRestrictions not defined");

  // Check entity limits
  var cmpEntityLimits = QueryPlayerIDInterface(player, IID_EntityLimits);
  if (
    cmpEntityLimits &&
    !cmpEntityLimits.AllowedToBuild(cmpBuildRestrictions.GetCategory())
  ) {
    if (g_DebugCommands)
      warn(
        "Invalid command: build limits check failed for player " +
  ...
  var cmpTechnologyManager = QueryPlayerIDInterface(
    player,
    IID_TechnologyManager
  );
  if (cmpTechnologyManager && !cmpTechnologyManager.CanProduce(cmd.template)) {
    if (g_DebugCommands)
      warn(
        "Invalid command: required technology check failed for player " +
          player +
          ": " +
          uneval(cmd)
      );
    ...
    cmpGuiInterface.PushNotification({
      ...
      players: [player],
      message: markForTranslation(
        "The building's technology requirements are not met."
      ),
      translateMessage: true,
    });
  ...
  if (!cmpPlayer.TrySubtractResources(costs)) {
    if (g_DebugCommands)
      warn(
        "Invalid command: building cost check failed for player " +
          player +
          ": " +
          uneval(cmd)
      );
  ...
  if (cmd.metadata)
    Engine.PostMessage(ent, MT_AIMetadata, {
      id: ent,
...
function TryConstructWall(player, cmpPlayer, controlAllUnits, cmd) {
  ...
  if (cmd.pieces.length <= 0) return;

  if (
    cmd.startSnappedEntity &&
    cmd.pieces[0].template == cmd.wallSet.templates.tower
  ) {
    error(
      "[TryConstructWall] Starting wall piece cannot be a tower (" +
        cmd.wallSet.templates.tower +
        ") when snapping at the starting side"
    );
    return;
  }
  ...
  if (
    ...
    cmd.pieces[cmd.pieces.length - 1].template == cmd.wallSet.templates.tower
  ) {
    error(
      "[TryConstructWall] Ending wall piece cannot be a tower (" +
        cmd.wallSet.templates.tower +
        ") when snapping at the ending side"
    );
    return;
  }
  ...
  if (cmd.startSnappedEntity) {
    var cmpSnappedStartObstruction = Engine.QueryInterface(
      cmd.startSnappedEntity,
      IID_Obstruction
    );
    if (!cmpSnappedStartObstruction) {
      error(
        "[TryConstructWall] Snapped entity on starting side does not have an obstruction component"
      );
      return;
    }
  ...
  for (; i < pieces.length; ++i) {
    ...
    if (
      lastTowerControlGroup === null ||
      lastTowerControlGroup == INVALID_ENTITY
    ) {
      if (
        !(
          i == 0 &&
          piece.template == cmd.wallSet.templates.tower &&
          !cmd.startSnappedEntity
        )
      ) {
        error(
          "[TryConstructWall] Expected last tower control group to be available, none found (1st pass, iteration " +
            i +
            ")"
        );
        break;
    ...
    if (i == pieces.length - 1 && cmd.endSnappedEntity) {
      var cmpEndSnappedObstruction = Engine.QueryInterface(
        cmd.endSnappedEntity,
        IID_Obstruction
      );
      if (cmpEndSnappedObstruction)
        constructPieceCmd.obstructionControlGroup2 =
          cmpEndSnappedObstruction.GetControlGroup();
    }

    var pieceEntityId = TryConstructBuilding(
      player,
      cmpPlayer,
      controlAllUnits,
      constructPieceCmd
    ...
    if (pieceEntityId) {
      ...
      if (piece.template == cmd.wallSet.templates.tower) {
        var cmpTowerObstruction = Engine.QueryInterface(
          pieceEntityId,
          IID_Obstruction
        );
        ...
        if (i > 0) {
          ...
          var cmpPreviousObstruction = Engine.QueryInterface(
            pieces[i - 1].ent,
            IID_Obstruction
          );
  ...
  if (cmd.endSnappedEntity && wallComplete) {
    var cmpSnappedEndObstruction = Engine.QueryInterface(
      cmd.endSnappedEntity,
      IID_Obstruction
    );
    if (!cmpSnappedEndObstruction) {
      error(
        "[TryConstructWall] Snapped entity on ending side does not have an obstruction component"
      );
      return;
    }
  ...
  for (var j = lastBuiltPieceIndex; j >= 0; --j) {
    ...
    if (!piece.ent) {
      error(
        "[TryConstructWall] No entity ID set for constructed entity of template '" +
          piece.template +
          "'"
      );
      continue;
    ...
    if (!cmpPieceObstruction) {
      error(
        "[TryConstructWall] Wall piece of template '" +
          piece.template +
          "' has no Obstruction component"
      );
      continue;
    }
    ...
    if (piece.template == cmd.wallSet.templates.tower) {
      ...
      var existingSecondaryControlGroup =
      ...
      if (existingSecondaryControlGroup == INVALID_ENTITY) {
        ...
        error(
          "[TryConstructWall] Existing secondary control group of non-tower entity does not match expected value (2nd pass, iteration " +
            j +
            ")"
        );
        break;
...
function RemoveFromFormation(ents) {
  ...
  for (let fid in formation.members) {
    ...
    if (cmpFormation) cmpFormation.RemoveMembers(formation.members[fid]);
  }
}
...
function GetFormationUnitAIs(
  ...
  if (formationTemplate != NULL_FORMATION) {
    ...
    if (formationIds.length == 1) {
      ...
      if (
        cmpFormation &&
        cmpFormation.GetMemberCount() == formation.members[fid].length &&
        ...
        if (!forceTemplate || formationTemplate == formation.templates[fid]) {
          ...
          formationTemplate &&
          CanMoveEntsIntoFormation(formation.entities, formationTemplate)
        )
      ...
      if (template && !forceTemplate) formationTemplate = template;
    }
  }
  ...
  for (let ent of ents) {
    ...
    if (!cmpUnitAI || !cmpPosition || !cmpPosition.IsInWorld()) continue;
    ...
    // for the other cases.
    let nullFormation =
      (formationTemplate || cmpUnitAI.GetFormationTemplate()) == NULL_FORMATION;
    if (
      nullFormation ||
      !cmpIdentity ||
      !cmpIdentity.CanUseFormation(formationTemplate || NULL_FORMATION)
    ) {
      if (nullFormation && cmpUnitAI.GetFormationController())
        cmpUnitAI.LeaveFormation(cmd.queued || false);
      nonformedUnitAIs.push(cmpUnitAI);
    } else formedUnits.push(ent);
  }
  ...
  if (!formationUnitAIs.length) {
    ...
    for (let cluster of clusters) {
      ...
      if (
        !formationTemplate ||
        !CanMoveEntsIntoFormation(cluster, formationTemplate)
      ) {
      ...
      for (let ent of formationEnts) cmpFormation.RegisterTwinFormation(ent);
...
function ClusterEntities(ents, separationDistance) {
  let clusters = [];
  if (!ents.length) return clusters;
  ...
  while (clusters.length > 1) {
    ...
    for (let i = matrix.length - 1; i >= 0 && !closeClusters; --i)
      for (let j = i - 1; j >= 0 && !closeClusters; --j)
        if (matrix[i][j] < distSq) closeClusters = [i, j];

    // if no more close clusters found, just return all found clusters so far
    if (!closeClusters) return clusters;

    // make a new cluster with the entities from the two found clusters
    let newCluster = clusters[closeClusters[0]].concat(
      clusters[closeClusters[1]]
    );
...
function GetFormationRequirements(formationTemplate) {
  var template = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_TemplateManager
  ).GetTemplate(formationTemplate);
  if (!template.Formation) return false;
...
function CanControlUnit(entity, player, controlAll) {
  let cmpIdentity = Engine.QueryInterface(entity, IID_Identity);
  let canBeControlled =
    (IsOwnedByPlayer(player, entity) &&
      (!cmpIdentity || cmpIdentity.IsControllable())) ||
    controlAll;

  if (!canBeControlled) notifyOrderFailure(entity, player);
...
function IsOwnedByPlayerOrMutualAlly(entity, player) {
  return (
    IsOwnedByPlayer(player, entity) ||
    IsOwnedByMutualAllyOfPlayer(player, entity)
  );
}
...
function CanPlayerOrAllyControlUnit(entity, player, controlAll) {
  return (
    CanControlUnit(player, entity, controlAll) ||
    (IsOwnedByMutualAllyOfPlayer(player, entity) &&
...
function FilterEntityListWithAllies(entities, player, controlAll) {
  return entities.filter((ent) =>
    CanPlayerOrAllyControlUnit(ent, player, controlAll)
  );
}

...
AlertRaiser.prototype.UnitFilter = function (unit) {
  ...
  return (
    cmpIdentity &&
    MatchesClassList(cmpIdentity.GetClassesList(), this.template.List._string)
  );
};

AlertRaiser.prototype.RaiseAlert = function () {
  let cmpTimer = Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer);
  if (cmpTimer.GetTime() == this.lastTime) return;
  ...
  let units = cmpRangeManager
    .ExecuteQuery(
      this.entity,
      0,
      +this.template.RaiseAlertRange,
      [owner],
      IID_UnitAI,
      true
    )
    .filter((ent) => this.UnitFilter(ent));
  for (let unit of units) {
    ...
    let holder = cmpRangeManager
      .ExecuteQuery(
        unit,
        0,
        +this.template.SearchRange,
        mutualAllies,
        IID_GarrisonHolder,
        true
      )
      .find((ent) => {
        ...
        if (Engine.QueryInterface(ent, IID_UnitAI)) return false;

        // Ensure that the garrison holder is within range of the alert raiser
        if (
          +this.template.EndOfAlertRange > 0 &&
          PositionHelper.DistanceBetweenEntities(this.entity, ent) >
            +this.template.EndOfAlertRange
        )
          return false;

        if (!cmpUnitAI.CheckTargetVisible(ent)) return false;

        let cmpGarrisonHolder = Engine.QueryInterface(ent, IID_GarrisonHolder);
        if (!reserved.has(ent))
          reserved.set(
            ent,
            cmpGarrisonHolder.GetCapacity() - cmpGarrisonHolder.OccupiedSlots()
          );

        return (
          cmpGarrisonHolder.IsAllowedToGarrison(unit) &&
          reserved.get(ent) >= size
        );
      });
    ...
    else cmpUnitAI.ReplaceOrder("Stop", { force: true });
  }
};
...
AlertRaiser.prototype.EndOfAlert = function () {
  ...
  let holders = cmpRangeManager.ExecuteQuery(
    this.entity,
    0,
    +this.template.EndOfAlertRange,
    mutualAllies,
  ...
  for (let holder of holders) {
    if (Engine.QueryInterface(holder, IID_UnitAI)) continue;
    ...
    for (let unit of units)
      if (cmpGarrisonHolder.Unload(unit)) {
        let cmpUnitAI = Engine.QueryInterface(unit, IID_UnitAI);
        if (cmpUnitAI.HasWorkOrders()) cmpUnitAI.BackToWork();

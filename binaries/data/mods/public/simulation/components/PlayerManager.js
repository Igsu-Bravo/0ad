...
PlayerManager.prototype.AddPlayer = function (ent) {
  ...
  var newDiplo = [];
  for (var i = 0; i < id; i++) {
    var cmpOtherPlayer = Engine.QueryInterface(
      this.GetPlayerByID(i),
      IID_Player
    );
...
PlayerManager.prototype.ReplacePlayer = function (id, ent) {
  ...
  for (let e of entities) {
    ...
    if (cmpOwnership) cmpOwnership.SetOwner(INVALID_PLAYER);
  }
  ...
  for (let e of entities) {
    let cmpOwnership = Engine.QueryInterface(e, IID_Ownership);
    if (cmpOwnership) cmpOwnership.SetOwner(id);
  }
...
PlayerManager.prototype.GetPlayerByID = function (id) {
  if (id in this.playerEntities) return this.playerEntities[id];

  // Observers don't have player data.
  if (id == INVALID_PLAYER) return INVALID_ENTITY;
...
PlayerManager.prototype.GetAllPlayers = function () {
  let players = [];
  for (let i = 0; i < this.playerEntities.length; ++i) players.push(i);
  return players;
};
...
PlayerManager.prototype.GetNonGaiaPlayers = function () {
  let players = [];
  for (let i = 1; i < this.playerEntities.length; ++i) players.push(i);
  return players;
};
...
PlayerManager.prototype.GetActivePlayers = function () {
  return this.GetNonGaiaPlayers().filter(

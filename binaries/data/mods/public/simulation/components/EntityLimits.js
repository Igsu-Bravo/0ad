  ...
  for (var category in this.template.Limits) {
    ...
    if (category in this.template.LimitRemovers) {
      ...
      for (var c in this.template.LimitRemovers[category]) {
        this.removers[category][c] =
          this.template.LimitRemovers[category][c]._string.split(/\s+/);
        if (c === "RequiredClasses")
          for (var cls of this.removers[category][c]) this.classCount[cls] = 0;
      }
    }
  }
};
...
EntityLimits.prototype.ChangeCount = function (category, value) {
  if (this.count[category] !== undefined) this.count[category] += value;
};

EntityLimits.prototype.ChangeMatchCount = function (template, value) {
  if (!this.matchTemplateCount[template]) this.matchTemplateCount[template] = 0;
...
EntityLimits.prototype.UpdateLimitsFromTech = function (tech) {
  for (var category in this.removers)
    if (
      "RequiredTechs" in this.removers[category] &&
      this.removers[category].RequiredTechs.indexOf(tech) !== -1
    )
      this.removers[category].RequiredTechs.splice(
        this.removers[category].RequiredTechs.indexOf(tech),
        1
      );
...
EntityLimits.prototype.AllowedToCreate = function (
  limitType,
  category,
  count,
  templateName,
  ...
  if (
    this.matchTemplateCount[templateName] !== undefined &&
...
EntityLimits.prototype.NotifyLimit = function (limitType, category, limit) {
  ...
  if (limitType == BUILD)
    notification.message = markForTranslation(
      "%(category)s build limit of %(limit)s reached"
    );
  else if (limitType == TRAINING)
    notification.message = markForTranslation(
      "%(category)s training limit of %(limit)s reached"
    );
  else {
    warn("EntityLimits.js: Unknown LimitType " + limitType);
    notification.message = markForTranslation(
      "%(category)s limit of %(limit)s reached"
    );
  }
...
EntityLimits.prototype.AllowedToTrain = function (
  category,
  count,
  templateName,
  matchLimit
...
EntityLimits.prototype.AllowedToReplace = function (ent, template) {
  ...
  let templateFrom = cmpTemplateManager.GetTemplate(
  ...
  if (templateTo.TrainingRestrictions) {
    ...
    return this.AllowedToCreate(
      ...
      templateFrom.TrainingRestrictions &&
        templateFrom.TrainingRestrictions.Category == category
        ? 0
        : 1
    );
  }
  ...
  if (templateTo.BuildRestrictions) {
    ...
    return this.AllowedToCreate(
      BUILD,
      category,
      templateFrom.BuildRestrictions &&
        templateFrom.BuildRestrictions.Category == category
        ? 0
        : 1
    );
  }
...
EntityLimits.prototype.OnGlobalOwnershipChanged = function (msg) {
  ...
  if (msg.from == cmpPlayer.GetPlayerID()) var modifier = -1;
  else if (msg.to == cmpPlayer.GetPlayerID()) var modifier = 1;
  else return;
  ...
  if (cmpBuildRestrictions) category = cmpBuildRestrictions.GetCategory();
  var cmpTrainingRestrictions = Engine.QueryInterface(
    msg.entity,
    IID_TrainingRestrictions
  );
  if (cmpTrainingRestrictions) category = cmpTrainingRestrictions.GetCategory();
  if (category) this.ChangeCount(category, modifier);
    ...
    for (var c in this.changers[category])
      if (classes.indexOf(c) >= 0) {
        ...
        if (this.removedLimit[category] != undefined)
          // update removed limits in case we want to restore it
  ...
  for (var category in this.removers)
    if ("RequiredClasses" in this.removers[category])
      for (var cls of this.removers[category].RequiredClasses)
        if (classes.indexOf(cls) !== -1) this.classCount[cls] += modifier;

...
TechnologyManager.prototype.Schema = "<empty/>";
...
TechnologyManager.prototype.UpdateAutoResearch = function () {
  for (let key of this.unresearchedAutoResearchTechs) {
    ...
    if (
      (tech.autoResearch && this.CanResearch(key)) ||
      (tech.top &&
        (this.IsTechnologyResearched(tech.top) ||
          this.IsTechnologyResearched(tech.bottom)))
    ) {
...
TechnologyManager.prototype.CanProduce = function (templateName) {
  var cmpTempManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_TemplateManager
  );
...
TechnologyManager.prototype.CanResearch = function (tech) {
  ...
  if (
    (template.top && this.IsInProgress(template.top)) ||
    (template.bottom && this.IsInProgress(template.bottom))
  )
    return false;

  if (template.pair && !this.CanResearch(template.pair)) return false;

  if (this.IsInProgress(tech)) return false;

  if (this.IsTechnologyResearched(tech)) return false;

  return this.CheckTechnologyRequirements(
    DeriveTechnologyRequirements(
      template,
      Engine.QueryInterface(this.entity, IID_Player).GetCiv()
    )
  );
};
 ...
 */
TechnologyManager.prototype.CheckTechnologyRequirements = function (
  reqs,
  civonly = false
) {
  ...
  if (civonly || !reqs.length) return true;
...
TechnologyManager.prototype.DoesEntitySpecPass = function (entity) {
  switch (entity.check) {
    case "count":
      if (
        !this.classCounts[entity.class] ||
        this.classCounts[entity.class] < entity.number
      )
        return false;
      break;
    ...
    case "variants":
      if (
        !this.typeCountsByClass[entity.class] ||
        Object.keys(this.typeCountsByClass[entity.class]).length < entity.number
      )
        return false;
      break;
  }
  ...
  if (msg.to == playerID) {
    ...
    if (!Engine.QueryInterface(msg.entity, IID_Foundation)) {
      for (let cls of classes) {
        ...
        this.typeCountsByClass[cls][template] =
          this.typeCountsByClass[cls][template] || 0;
  ...
  if (msg.from == playerID) {
    ...
    if (!Engine.QueryInterface(msg.entity, IID_Foundation)) {
      ...
      if (cmpIdentity) {
        ...
        for (let cls of classes) {
          this.classCounts[cls] -= 1;
          if (this.classCounts[cls] <= 0) delete this.classCounts[cls];
...
TechnologyManager.prototype.ResearchTechnology = function (tech) {
  ...
  if (template.modifications) {
    ...
    cmpModifiersManager.AddModifiers(
      "tech/" + tech,
      DeriveModificationsFromTech(template),
      this.entity
    );
  }
  ...
  if (template.replaces && template.replaces.length > 0) {
    for (let i of template.replaces) {
      if (!i || this.IsTechnologyResearched(i)) continue;
  ...
  let cmpPlayer = Engine.QueryInterface(this.entity, IID_Player);
  if (!cmpPlayer || cmpPlayer.GetPlayerID() === undefined) return;
  let playerID = cmpPlayer.GetPlayerID();
  ...
  if (cmpPlayerEntityLimits) cmpPlayerEntityLimits.UpdateLimitsFromTech(tech);

  // Always send research finished message.
  Engine.PostMessage(this.entity, MT_ResearchFinished, {
    player: playerID,
    tech: tech,
  });
...
TechnologyManager.prototype.StoppedResearch = function (tech, notification) {
  if (
    notification &&
    tech.startsWith("phase") &&
    this.researchStarted.has(tech)
  ) {
    let cmpPlayer = Engine.QueryInterface(this.entity, IID_Player);
    let cmpGUIInterface = Engine.QueryInterface(
      SYSTEM_ENTITY,

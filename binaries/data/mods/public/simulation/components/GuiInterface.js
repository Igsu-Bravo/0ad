...
GuiInterface.prototype.GetSimulationState = function () {
  ...
  for (let i = 0; i < numPlayers; ++i) {
    ...
    ret.players.push({
      ...
      entityCounts: cmpPlayerEntityLimits
        ...
        : null,
      matchEntityCounts: cmpPlayerEntityLimits
        ? cmpPlayerEntityLimits.GetMatchCounts()
        : null,
  ...
  for (let i = 0; i < numPlayers; ++i) {
    ...
    if (cmpPlayerStatisticsTracker)
      ret.players[i].statistics =
        cmpPlayerStatisticsTracker.GetBasicStatistics();
  }
...
GuiInterface.prototype.GetEntityState = function (player, ent) {
    ...
    for (let type of types) {
      ...
      if (ret.attack[type].splash)
        Object.assign(
          ...
          cmpAttack.GetAttackEffectsData(type, true)
        );
      ...
      if (cmpPosition && cmpPosition.IsInWorld())
        ...
        ret.attack[type].elevationAdaptedRange =
          cmpRangeManager.GetElevationAdaptedRange(
            cmpPosition.GetPosition(),
            cmpPosition.GetRotation(),
            range.max,
            range.elevationBonus,
            cmpUnitAI ? 0 : 2 * Math.PI
          );
      // Not in world, set a default?
      else ret.attack[type].elevationAdaptedRange = ret.attack.maxRange;
    }
  }

  let cmpResistance = QueryMiragedInterface(ent, IID_Resistance);
  if (cmpResistance)
    ret.resistance = cmpResistance.GetResistanceOfForm(
      cmpFoundation ? "Foundation" : "Entity"
    );
  ...
  if (cmpTreasureCollector) ret.treasureCollector = true;
...
GuiInterface.prototype.GetTemplateData = function (player, data) {
  ...
  for (let name of auraNames) {
    ...
    else aurasTemplate[name] = auraTemplate;
  }
...
GuiInterface.prototype.GetNeededResources = function (player, data) {
  let cmpPlayer = QueryPlayerIDInterface(
    data.player !== undefined ? data.player : player
  );
...
GuiInterface.prototype.GetAvailableFormations = function (
  player,
  wantedPlayer
) {
...
GuiInterface.prototype.DisplayRallyPoint = function (player, cmd) {
  ...
  for (let ent of cmd.entities) {
    ...
    else pos = cmpRallyPoint.GetPositions()[0];
    ...
    if (pos) {
      ...
      if ("queued" in cmd) {
        ...
        else cmpRallyPointRenderer.SetPosition(new Vector2D(pos.x, pos.z));
...
GuiInterface.prototype.AddTargetMarker = function (player, cmd) {
  ...
  if (!ent) return;
  let cmpOwnership = Engine.QueryInterface(ent, IID_Ownership);
  if (cmpOwnership) cmpOwnership.SetOwner(cmd.owner);
  let cmpPosition = Engine.QueryInterface(ent, IID_Position);
...
GuiInterface.prototype.SetWallPlacementPreview = function (player, cmd) {
  ...
  for (let i = 0; i < previewEntities.length; ++i) {
    ...
    } else ent = entPool.entities[entPool.numUsed];
...
GuiInterface.prototype.PlaySoundForPlayer = function (player, data) {
  let playerEntityID = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_PlayerManager
  ...
  if (soundGroup)
    Engine.QueryInterface(
      SYSTEM_ENTITY,
      IID_SoundManager
    ).PlaySoundGroupForPlayer(soundGroup, player);
};
...
GuiInterface.prototype.IdleUnitFilter = function (
  ...
  if (!cmpUnitAI || !cmpUnitAI.IsIdle()) return { idle: false };

  let cmpGarrisonable = Engine.QueryInterface(unit, IID_Garrisonable);
  if (cmpGarrisonable && cmpGarrisonable.IsGarrisoned()) return { idle: false };
...
GuiInterface.prototype.GetTradingRouteGain = function (player, data) {
  ...
  return (
    cmpMarket && cmpMarket.CalculateTraderGain(data.secondMarket, data.template)
  );
};

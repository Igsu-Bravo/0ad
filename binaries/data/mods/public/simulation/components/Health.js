...
Health.prototype.ExecuteRegeneration = function () {
  ...
  if (this.GetIdleRegenRate() != 0) {
    ...
    if (cmpUnitAI && cmpUnitAI.IsIdle()) regen += this.GetIdleRegenRate();
  }
...
Health.prototype.TakeDamage = function (amount, attacker, attackerOwner) {
  if (!amount || !this.hitpoints) return { healthChange: 0 };
  ...
  if (cmpLoot && cmpLoot.GetXp() > 0 && change.healthChange < 0)
    change.xp =
      (cmpLoot.GetXp() * -change.healthChange) / this.GetMaxHitpoints();

  if (!this.hitpoints) this.KilledBy(attacker, attackerOwner);
...
Health.prototype.KilledBy = function (attacker, attackerOwner) {
  ...
  let cmpKillerPlayerStatisticsTracker = QueryPlayerIDInterface(
    attackerOwner,
  ...
  let cmpTargetPlayerStatisticsTracker = QueryOwnerInterface(
    this.entity,
    IID_StatisticsTracker
  );
  ...
  if (cmpLooter) cmpLooter.Collect(this.entity);
};
...
Health.prototype.Increase = function (amount) {
  ...
  if (!this.IsInjured()) return { old: this.hitpoints, new: this.hitpoints };
...
Health.prototype.CreateCorpse = function () {
  ...
  let templateName = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_TemplateManager
  ).GetCurrentTemplateName(this.entity);
  ...
  if (resource) entCorpse = Engine.AddEntity("resource|" + templateName);
  else entCorpse = Engine.AddLocalEntity("corpse|" + templateName);
  ...
  if (cmpVisualCorpse) {
    ...
    if (cmpVisual) cmpVisualCorpse.SetActorSeed(cmpVisual.GetActorSeed());
...
Health.prototype.OnValueModification = function (msg) {
  if (msg.component == "Health") this.RecalculateValues();
};

Health.prototype.OnOwnershipChanged = function (msg) {
  if (msg.to != INVALID_PLAYER) this.RecalculateValues();
};
...
HealthMirage.prototype.GetMaxHitpoints = function () {
  return this.maxHitpoints;
};
HealthMirage.prototype.GetHitpoints = function () {
  return this.hitpoints;
};
HealthMirage.prototype.IsRepairable = function () {
  return this.repairable;
};
HealthMirage.prototype.IsInjured = function () {
  return this.injured;
};
HealthMirage.prototype.IsUnhealable = function () {

...
FormationAttack.prototype.Schema =
  "<element name='CanAttackAsFormation'>" + "<text/>" + "</element>";
...
FormationAttack.prototype.GetRange = function (target) {
  ...
  for (var ent of members) {
    ...
    if (this.canAttackAsFormation) {
      if (range.max < result.max || result.max < 0) result.max = range.max;
    } else {
      if (range.max > result.max || range.max < 0) result.max = range.max;
    }
  ...
  if (result.max >= 0) result.max += extraRange;

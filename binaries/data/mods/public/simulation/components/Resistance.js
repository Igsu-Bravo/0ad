...
Resistance.prototype.SetInvulnerability = function (invulnerability) {
  ...
  Engine.PostMessage(this.entity, MT_InvulnerabilityChanged, {
    entity: this.entity,
...
Resistance.prototype.GetEffectiveResistanceAgainst = function (effectType) {
  ...
  let template = this.GetResistanceOfForm(
    Engine.QueryInterface(this.entity, IID_Foundation) ? "Foundation" : "Entity"
  );
  if (template[effectType]) ret[effectType] = template[effectType];
...
Resistance.prototype.GetResistanceOfForm = function (entityForm) {
  ...
  if (template.Damage) {
    ...
    for (let damageType in template.Damage)
      ret.Damage[damageType] = ApplyValueModificationsToEntity(
        "Resistance/" + entityForm + "/Damage/" + damageType,
        +this.template[entityForm].Damage[damageType],
        this.entity
      );
  }
    ...
    for (let effect in template.ApplyStatus)
      ret.ApplyStatus[effect] = {
        duration: ApplyValueModificationsToEntity(
          "Resistance/" + entityForm + "/ApplyStatus/" + effect + "/Duration",
          +(template.ApplyStatus[effect].Duration || 1),
          this.entity
        ),
        blockChance: ApplyValueModificationsToEntity(
          "Resistance/" +
            ...
            effect +
            "/BlockChance",
          +(template.ApplyStatus[effect].BlockChance || 0),
          this.entity
        ),
      };
  }
...
Resistance.prototype.OnOwnershipChanged = function (msg) {
  if (msg.to === INVALID_PLAYER)
    for (let attacker of this.attackers)
      Engine.QueryInterface(attacker, IID_Attack)?.StopAttacking(
        "TargetInvalidated"
      );
};
...
ResistanceMirage.prototype.Init = function (cmpResistance) {
  ...
  this.isFoundation = !!Engine.QueryInterface(
    cmpResistance.entity,
    IID_Foundation
  );
  this.resistanceOfForm = {};
  for (const entityForm in cmpResistance.template)
    this.resistanceOfForm[entityForm] =
      cmpResistance.GetResistanceOfForm(entityForm);
};

ResistanceMirage.prototype.IsInvulnerable = Resistance.prototype.IsInvulnerable;

ResistanceMirage.prototype.GetEffectiveResistanceAgainst = function (
  entityForm
) {

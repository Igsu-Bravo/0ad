...
Pack.prototype.Pack = function () {
  if (this.IsPacked() || this.IsPacking()) return;
  ...
  if (cmpVisual) cmpVisual.SelectAnimation("packing", true, 1.0);
};
...
Pack.prototype.SetElapsedTime = function (time) {
  ...
  Engine.PostMessage(this.entity, MT_PackProgressUpdate, {
    progress: this.elapsedTime,
  });
};
...
Pack.prototype.PackProgress = function (data, lateness) {
  ...
  if (newEntity) PlaySound(this.packed ? "packed" : "unpacked", newEntity);
};

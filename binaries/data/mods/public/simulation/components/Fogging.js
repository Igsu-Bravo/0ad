...
Fogging.prototype.Activate = function () {
  ...
  if (mustUpdate) {
    ...
    for (let player = 0; player < numPlayers; ++player)
      if (
        this.seen[player] &&
        cmpRangeManager.GetLosVisibility(this.entity, player) != "visible"
      )
...
Fogging.prototype.LoadMirage = function (player) {
  ...
  if (this.mirages[player] == INVALID_ENTITY)
    this.mirages[player] = Engine.AddEntity(
      "mirage|" +
  ...
  let cmpMirageOwnership = Engine.QueryInterface(
    this.mirages[player],
    IID_Ownership
  );
  if (!cmpParentOwnership || !cmpMirageOwnership) {
    error(
      "Failed to copy the ownership data of the fogged entity " + this.entity
    );
    return;
  }
  ...
  let cmpMiragePosition = Engine.QueryInterface(
    this.mirages[player],
    IID_Position
  );
  if (!cmpParentPosition || !cmpMiragePosition) {
    error(
      "Failed to copy the position data of the fogged entity " + this.entity
    );
    return;
  }
  if (!cmpParentPosition.IsInWorld()) return;
  ...
  let cmpMirageVisualActor = Engine.QueryInterface(
    this.mirages[player],
    IID_Visual
  );
  ...
  Engine.QueryInterface(SYSTEM_ENTITY, IID_GuiInterface).AddMiragedEntity(
    player,
    this.entity,
    this.mirages[player]
  );
  ...
  Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_RangeManager
  ).RequestVisibilityUpdate(this.entity);
};
...
Fogging.prototype.IsMiraged = function (player) {
  if (player < 0 || player >= this.mirages.length) return false;

  return this.miraged[player];
};

Fogging.prototype.GetMirage = function (player) {
  if (player < 0 || player >= this.mirages.length) return INVALID_ENTITY;

  return this.mirages[player];
};

Fogging.prototype.WasSeen = function (player) {
  if (player < 0 || player >= this.seen.length) return false;
...
Fogging.prototype.OnOwnershipChanged = function (msg) {
  ...
  if (msg.to > 0) this.Activate();

  if (msg.to != -1) return;

  let cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);
  for (let player = 0; player < this.mirages.length; ++player) {
    if (this.mirages[player] == INVALID_ENTITY) continue;
    ...
    if (
      cmpRangeManager.GetLosVisibility(this.mirages[player], player) == "hidden"
    ) {
    ...
    let cmpMirage = Engine.QueryInterface(this.mirages[player], IID_Mirage);
    if (cmpMirage) cmpMirage.SetParent(INVALID_ENTITY);
  }
};

Fogging.prototype.OnVisibilityChanged = function (msg) {
  if (msg.player < 0 || msg.player >= this.mirages.length) return;

...
Promotion.prototype.IncreaseXp = function (amount) {
  ...
  if (this.promotedUnitEntity) {
    let cmpPromotion = Engine.QueryInterface(
      this.promotedUnitEntity,
      IID_Promotion
    );
    if (cmpPromotion) cmpPromotion.IncreaseXp(amount);
    return;
  }
  ...
  if (this.currentXp >= requiredXp) {
    ...
    while (true) {
      ...
      if (!template.Promotion) break;
      requiredXp = ApplyValueModificationsToTemplate(
        "Promotion/RequiredXp",
...
Promotion.prototype.ComputeTrickleRate = function () {
  this.trickleRate = ApplyValueModificationsToEntity(
    "Promotion/TrickleRate",
    +(this.template.TrickleRate || 0),
    this.entity
  );
...
Promotion.prototype.CheckTrickleTimer = function () {
  ...
  let cmpTimer = Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer);
  this.trickleTimer = cmpTimer.SetInterval(
    this.entity,
    IID_Promotion,
    "TrickleTick",
    1000,
...
Promotion.prototype.OnValueModification = function (msg) {
  if (msg.component != "Promotion") return;

...
Mirage.prototype.CopyComponent = function (iid) {
  ...
  if (cmp) this.miragedIids.set(iid, cmp.Mirage(this.entity, this.player));
};
...
Mirage.prototype.OnVisibilityChanged = function (msg) {
  ...
  if (msg.player != this.player || msg.newVisibility != VIS_HIDDEN) return;

  if (this.miragedIids.has(IID_Market))
    this.miragedIids.get(IID_Market).UpdateTraders(msg);

  if (this.parent == INVALID_ENTITY) Engine.DestroyEntity(this.entity);
  else

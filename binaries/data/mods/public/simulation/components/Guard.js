...
Guard.prototype.GetRange = function (entity) {
  ...
  if (cmpFootprint) {
    ...
    if (shape.type == "square")
      range +=
        (Math.sqrt(shape.depth * shape.depth + shape.width * shape.width) * 2) /
        3;
    else if (shape.type == "circle") range += (shape.radius * 3) / 2;
  }
  return range;
};
...
Guard.prototype.AddGuard = function (ent) {
  if (this.entities.indexOf(ent) != -1) return;
...
Guard.prototype.RenameGuard = function (oldent, newent) {
  ...
  if (index != -1) this.entities[index] = newent;
};

Guard.prototype.OnAttacked = function (msg) {
  for (let ent of this.entities)
    Engine.PostMessage(ent, MT_GuardedAttacked, {
      guarded: this.entity,
      data: msg,
    });
};
  ...
  for (let ent of entities) {
    if (force || !IsOwnedByMutualAllyOfEntity(this.entity, ent)) {
      ...
      if (
        cmpUnitAI &&
        cmpUnitAI.IsGuardOf() &&
        cmpUnitAI.IsGuardOf() == this.entity
      )
        cmpUnitAI.RemoveGuard();
      else this.RemoveGuard(ent);
    }
  }
};

...
UnitMotionFlying.prototype.Init = function () {
  ...
  this.passabilityClass = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_Pathfinder
  ).GetPassabilityClass(this.template.PassabilityClass);
};
...
UnitMotionFlying.prototype.OnUpdate = function (msg) {
  ...
  let ground = Math.max(
    cmpTerrain.GetGroundLevel(pos.x, pos.z),
    cmpWaterManager.GetWaterLevel(pos.x, pos.z)
  );
  let newangle = angle;
  let canTurn = true;
  let distanceToTargetSquared = Math.euclidDistance2DSquared(
    pos.x,
    pos.z,
    this.targetX,
    this.targetZ
  ...
  if (this.landing) {
    if (this.speed > 0 && this.onGround) {
      if (
        pos.y <= cmpWaterManager.GetWaterLevel(pos.x, pos.z) &&
        this.template.DiesInWater == "true"
      )
      ...
      if (this.waterDeath)
        this.speed = Math.max(
          0,
          this.speed - turnLength * this.template.BrakingRate * 10
        );
      else
        this.speed = Math.max(
          0,
          this.speed - turnLength * this.template.BrakingRate
        );
      canTurn = false;
      ...
      if (this.waterDeath && cmpHealth) cmpHealth.Kill();
      else {
        this.pitch = 0;
        // We've stopped.
        ...
        if (cmpRangeManager.GetLosCircular()) {
          ...
          if (div < 1) {
            ...
            newangle += Math.PI;
            distanceToTargetSquared = Math.euclidDistance2DSquared(
              pos.x,
              pos.z,
              this.targetX,
              this.targetZ
      ...
      this.speed = Math.max(
        this.template.LandingSpeed,
        this.speed - turnLength * this.template.SlowingRate
      );
      canTurn = false;
      ...
      else this.pitch = Math.PI / 18;
      let descentRate =
        (((pos.y - targetHeight) / this.template.FlyingHeight) *
          this.template.ClimbRate +
          SHORT_FINAL) *
        SHORT_FINAL;
      ...
      if (targetHeight == pos.y) {
        this.onGround = true;
        if (
          targetHeight == cmpWaterManager.GetWaterLevel(pos.x, pos.z) &&
          this.template.DiesInWater
        )
          this.waterDeath = true;
      }
    }
    ...
    if (
      this.template.StationaryDistance &&
      distanceToTargetSquared <=
        +this.template.StationaryDistance * +this.template.StationaryDistance
    ) {
      ...
      cmpPosition.TurnTo(
        Math.atan2(this.targetX - pos.x, this.targetZ - pos.z)
      );
      Engine.PostMessage(this.entity, MT_MotionUpdate, {
        updateString: "likelySuccess",
      });
      return;
    }
    ...
    if (this.speed < this.template.TakeoffSpeed && this.onGround) {
      ...
      // Accelerate forwards.
      this.speed = Math.min(
        this.template.MaxSpeed,
        this.speed + turnLength * this.template.AccelRate
      );
      canTurn = false;
      // Clamp to ground if below it, or descend if above.
      if (pos.y < ground) pos.y = ground;
      ...
      if (pos.y < targetHeight)
        pos.y = Math.min(
          targetHeight,
          pos.y + turnLength * this.template.ClimbRate
        );
      else if (pos.y > targetHeight) {
        pos.y = Math.max(
          targetHeight,
  ...
  let isBehindTarget =
    (this.targetX - pos.x) * Math.sin(angle) +
      (this.targetZ - pos.z) * Math.cos(angle) <
    0;
  ...
  if (canTurn) {
    ...
    delta -= Math.PI;
    // Clamp to max rate.
    let deltaClamped = Math.min(
      Math.max(delta, -this.template.TurnRate * turnLength),
      this.template.TurnRate * turnLength
    );
    // Calculate new orientation, in a peculiar way in order to make sure the
    // result gets close to targetAngle (rather than being n*2*pi out).
    newangle = targetAngle + deltaClamped - delta;
    if (newangle - angle > Math.PI / 18) this.roll = Math.PI / 9;
    else if (newangle - angle < -Math.PI / 18) this.roll = -Math.PI / 9;
    else this.roll = newangle - angle;
  } else this.roll = 0;
...
UnitMotionFlying.prototype.MoveToPointRange = function (
  x,
  z,
  minRange,
  maxRange
...
UnitMotionFlying.prototype.MoveToTargetRange = function (
  target,
  minRange,
  maxRange
) {
  let cmpTargetPosition = Engine.QueryInterface(target, IID_Position);
  if (!cmpTargetPosition || !cmpTargetPosition.IsInWorld()) return false;
...
UnitMotionFlying.prototype.EstimateFuturePosition = function (dt) {
  let cmpPosition = Engine.QueryInterface(this.entity, IID_Position);
  if (!cmpPosition || !cmpPosition.IsInWorld()) return Vector2D();
  let position = cmpPosition.GetPosition2D();

  return Vector2D.add(
    position,
    Vector2D.sub(position, cmpPosition.GetPreviousPosition2D()).mult(
      dt / Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer).GetLatestTurnLength()
    )
  );
};
...
UnitMotionFlying.prototype.StopMoving = function () {
  // Invert.
  if (!this.waterDeath) this.landing = !this.landing;
};

...
AttackDetection.prototype.Init = function () {
  ...
  this.suppressionTransferRangeSquared =
    +this.template.SuppressionTransferRange *
    +this.template.SuppressionTransferRange;
  this.suppressionRangeSquared =
    +this.template.SuppressionRange * +this.template.SuppressionRange;
  this.suppressedList = [];
};
...
AttackDetection.prototype.OnGlobalAttacked = function (msg) {
  ...
  if (cmpOwnership.GetOwner() != cmpPlayer.GetPlayerID()) return;
...
AttackDetection.prototype.AttackAlert = function (
  target,
  attacker,
  type,
  attackerOwner
  ...
  let atkOwner =
    cmpAttackerOwnership && cmpAttackerOwnership.GetOwner() != INVALID_PLAYER
      ? cmpAttackerOwnership.GetOwner()
      : attackerOwner;
  ...
  var targetIsDomesticAnimal =
    cmpTargetIdentity &&
    cmpTargetIdentity.HasClass("Animal") &&
    cmpTargetIdentity.HasClass("Domestic");
  ...
  for (var i = 0; i < this.suppressedList.length; ++i) {
    ...
    if (dist >= this.suppressionRangeSquared) continue;

    isPriorityIncreased =
      element.targetIsDomesticAnimal && !targetIsDomesticAnimal;
    var isPriorityDescreased =
      !element.targetIsDomesticAnimal && targetIsDomesticAnimal;
  ...
  if (!isPriorityIncreased) this.AddSuppression(event);

  Engine.PostMessage(this.entity, MT_AttackDetected, {
    player: playerID,
    event: event,
  });

...
Upkeep.prototype.ComputeRates = function () {
  ...
  for (let resource in this.template.Rates) {
    let rate = ApplyValueModificationsToEntity(
      "Upkeep/Rates/" + resource,
      +this.template.Rates[resource],
      this.entity
    );
    if (rate) {
...
Upkeep.prototype.Pay = function (data, lateness) {
  ...
  else this.HandleSufficientUpkeep();
};
...
Upkeep.prototype.HandleInsufficientUpkeep = function () {
  if (this.unpayed) return;

  let cmpIdentity = Engine.QueryInterface(this.entity, IID_Identity);
  if (cmpIdentity) cmpIdentity.SetControllable(false);
  this.unpayed = true;
};
...
Upkeep.prototype.HandleSufficientUpkeep = function () {
  ...
  let cmpIdentity = Engine.QueryInterface(this.entity, IID_Identity);
  if (cmpIdentity) cmpIdentity.SetControllable(true);
  delete this.unpayed;
};

Upkeep.prototype.OnValueModification = function (msg) {
  if (msg.component != "Upkeep") return;
...
Upkeep.prototype.CheckTimer = function () {
  ...
  this.upkeepInterval = ApplyValueModificationsToEntity(
    "Upkeep/Interval",
  ...
  if (this.timer) {
    if (this.upkeepInterval == oldUpkeepInterval) return;

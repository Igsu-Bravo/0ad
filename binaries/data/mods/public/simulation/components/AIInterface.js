...
AIInterface.prototype.Schema = "<a:component type='system'/><empty/>";
...
AIInterface.prototype.Init = function () {
  this.events = {};
  for (let name of this.EventNames) this.events[name] = [];
...
AIInterface.prototype.GetRepresentation = function () {
  ...
  for (let id in this.changedEntities) {
    ...
    if (cmpAIProxy) state.entities[id] = cmpAIProxy.GetRepresentation();
  }
  this.changedEntities = {};
  Engine.ProfileStop();
...
AIInterface.prototype.GetFullRepresentation = function (flushEvents) {
  ...
  if (flushEvents) for (let name of this.EventNames) state.events[name] = [];
  ...
  for (let id of Engine.GetEntitiesWithInterface(IID_AIProxy))
    state.entities[id] = Engine.QueryInterface(
      id,
      IID_AIProxy
...
AIInterface.prototype.PushEvent = function (type, msg) {
  if (this.events[type] === undefined)
    warn(
      "Tried to push unknown event type " +
        type +
        ", please add it to AIInterface.js"
    );
...
AIInterface.prototype.OnTemplateModification = function (msg) {
  ...
  if (!this.templates) {
    ...
    for (let templateName of cmpTemplateManager.FindAllTemplates(false)) {
      ...
      if (
        templateName.startsWith("campaigns/") ||
        templateName.startsWith("rubble/") ||
        templateName.startsWith("skirmish/")
      )
        continue;
      let template =
        cmpTemplateManager.GetTemplateWithoutValidation(templateName);
      if (!template || !template.Identity || !template.Identity.Civ) continue;
  ...
  for (let name of this.templates) {
    ...
    if (!template || !template[msg.component]) continue;
    for (let valName of msg.valueNames) {
      ...
      for (let str of strings) {
        if (item !== undefined && item[str] !== undefined) item = item[str];
        else ended = false;
      }
      if (!ended) continue;
      // item now contains the template value for this.
      let oldValue = +item == item ? +item : item;
      let newValue = ApplyValueModificationsToTemplate(
        valName,
        oldValue,
        msg.player,
        template
      ...
      if (
        valName === "Player/MaxPopulation" ||
        valName === "Cost/Population" ||
      ...
      if (!this.changedTemplateInfo[msg.player][name])
        this.changedTemplateInfo[msg.player][name] = [
          { variable: valName, value: newValue },
        ];
      else
        this.changedTemplateInfo[msg.player][name].push({
          variable: valName,
          value: newValue,
        });
    }
...
AIInterface.prototype.OnGlobalValueModification = function (msg) {
  ...
  for (let ent of msg.entities) {
    ...
    if (!templateName || !templateName.length) continue;
    let template =
      cmpTemplateManager.GetTemplateWithoutValidation(templateName);
    if (!template || !template[msg.component]) continue;
    for (let valName of msg.valueNames) {
      ...
      if (!this.changedEntityTemplateInfo[ent])
        this.changedEntityTemplateInfo[ent] = [
          { variable: valName, value: newValue },
        ];
      else
        this.changedEntityTemplateInfo[ent].push({
          variable: valName,
          value: newValue,
        });
    }

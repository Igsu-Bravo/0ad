...
Player.prototype.Deserialize = function (state) {
  for (let prop in state) this[prop] = state[prop];
};
...
Player.prototype.AddResources = function (amounts) {
  for (let type in amounts) this.resourceCount[type] += +amounts[type];
};
...
Player.prototype.OnGlobalOwnershipChanged = function (msg) {
  ...
  if (msg.from == this.playerID) {
    ...
    if (panelIndex >= 0) this.panelEntities.splice(panelIndex, 1);

    let barterIndex = this.barterEntities.indexOf(msg.entity);
    if (barterIndex >= 0) this.barterEntities.splice(barterIndex, 1);
  }
...
Player.prototype.TributeResource = function (player, amounts) {
  ...
  for (let resCode in amounts)
    if (
      resTribCodes.indexOf(resCode) == -1 ||
      !Number.isInteger(amounts[resCode]) ||
      amounts[resCode] < 0
    ) {
      warn(
        "Invalid tribute amounts: " + uneval(resCode) + ": " + uneval(amounts)
      );
      return;
    }
  ...
  let total = Object.keys(amounts).reduce(
    (sum, type) => sum + amounts[type],
    0
  );
  let cmpOurStatisticsTracker = QueryPlayerIDInterface(
    this.playerID,
    IID_StatisticsTracker
  );
  if (cmpOurStatisticsTracker)
    cmpOurStatisticsTracker.IncreaseTributesSentCounter(total);
  let cmpTheirStatisticsTracker = QueryPlayerIDInterface(
    player,
    IID_StatisticsTracker
  );
...
Player.prototype.OnGlobalPlayerDefeated = function (msg) {
  ...
  let soundGroup = cmpSound.GetSoundGroup(
    this.playerID === msg.playerId
      ? "defeated"
      : this.IsAlly(msg.playerId)
      ? "defeated_ally"
      : this.state === "won"
      ? "won"
      : "defeated_enemy"
  ...
  if (soundGroup)
    Engine.QueryInterface(
      SYSTEM_ENTITY,
      IID_SoundManager
    ).PlaySoundGroupForPlayer(soundGroup, this.playerID);
};

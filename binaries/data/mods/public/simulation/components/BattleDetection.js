...
BattleDetection.prototype.SetState = function (state) {
  ...
  Engine.PostMessage(this.entity, MT_BattleStateChanged, {
...
BattleDetection.prototype.TimerHandler = function (data, lateness) {
  ...
  if (!this.alertness || this.recordControl++ == this.recordLength - 1) {
    var recordDamage = this.damageRecord.reduce(function (a, b) {
      return a + b;
    }, 0); // Sum up all values in the damage record.
    ...
    // Increment alertness up to 'alertnessMax'.
    else this.alertness = Math.max(0, this.alertness - 1); // Decrement alertness down to zero.

    // Stop the damage rate timer if we're no longer alert.
    if (!this.alertness) this.StopTimer();
  ...
  if (this.recordControl > this.recordLength - 1) this.recordControl = 0;
};
...
BattleDetection.prototype.OnGlobalAttacked = function (msg) {
  ...
  if (
    ...
    cmpAttackerOwnership.GetOwner() != cmpPlayer.GetPlayerID()
  )
    return;
  ...
  if (
    !cmpTargetOwnership ||
    cmpTargetOwnership.GetOwner() <= 0 ||
    cmpTargetOwnership.GetOwner() == cmpPlayer.GetPlayerID()
  )
    return;

  // If the damage rate timer isn't already started, start it now.
  if (!this.timer) this.StartTimer(0, this.interval);
  // Add damage of this attack to the damage counter.
  if (msg.damage) this.damage += msg.damage;
};

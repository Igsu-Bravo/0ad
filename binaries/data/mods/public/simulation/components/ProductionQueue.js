...
ProductionQueue.prototype.CalculateEntitiesMap = function () {
  ...
  if (!this.entitiesMap) this.entitiesMap = new Map();
  if (!this.template.Entities) return;

  let string = this.template.Entities._string;
  // Tokens can be added -> process an empty list to get them.
  let addedTokens = ApplyValueModificationsToEntity(
    "ProductionQueue/Entities/_string",
    "",
    this.entity
  );
  if (!addedTokens && !string) return;
  ...
  for (let tok of addedTokens) toks.push(tok);

  let addedDict = addedTokens.reduce((out, token) => {
    out[token] = true;
    return out;
  }, {});
  this.entitiesMap = toks.reduce((entMap, token) => {
    ...
    if (!(token in addedDict)) {
      ...
      token = ApplyValueModificationsToEntity(
        "ProductionQueue/Entities/_string",
        token,
        this.entity
      );
    ...
    // Replace the "{civ}" and "{native}" codes with the owner's civ ID and entity's civ ID.
    if (cmpIdentity) token = token.replace(/\{native\}/g, cmpIdentity.GetCiv());
    if (cmpPlayer) token = token.replace(/\{civ\}/g, cmpPlayer.GetCiv());
...
ProductionQueue.prototype.GetUpgradedTemplate = function (templateName) {
  let cmpPlayer = QueryOwnerInterface(this.entity);
  if (!cmpPlayer) return templateName;
...
ProductionQueue.prototype.GetTechnologiesList = function () {
  if (!this.template.Technologies) return [];

  let string = this.template.Technologies._string;
  string = ApplyValueModificationsToEntity(
...
ProductionQueue.prototype.IsTechnologyResearchedOrInProgress = function (tech) {
  ...
  if (template.top)
    return (
      cmpTechnologyManager.IsTechnologyResearched(template.top) ||
  ...
  return (
    ...
    cmpTechnologyManager.IsInProgress(tech)
  );
};
 ...
 */
ProductionQueue.prototype.AddItem = function (
  templateName,
  type,
  count,
  metadata
    ...
    if (cmpUpgrade && cmpUpgrade.IsUpgrading()) {
      ...
      cmpGUIInterface.PushNotification({
        ...
        message: markForTranslation(
          "Entity is being upgraded. Cannot start production."
        ),
  ...
  if (type == "unit") {
    ...
    let template = cmpTemplateManager.GetTemplate(
      this.GetUpgradedTemplate(templateName)
    );
    ...
    if (!cmpPlayer.TrySubtractResources(totalCosts)) return false;

    if (template.TrainingRestrictions) {
      let unitCategory = template.TrainingRestrictions.Category;
      let cmpPlayerEntityLimits = QueryPlayerIDInterface(
        player,
        IID_EntityLimits
      );
    ...
    if (!TechnologyTemplates.Has(templateName)) return false;

    if (
      !this.GetTechnologiesList().some(
        (tech) =>
          tech &&
          (tech == templateName ||
            (tech.pair &&
    ...
    if (template.cost)
      for (let res in template.cost)
        cost[res] = Math.floor(
          (techCostMultiplier[res] || 1) * template.cost[res]
        );

    // TrySubtractResources should report error to player (they ran out of resources).
    if (!cmpPlayer.TrySubtractResources(cost)) return false;
    ...
    warn(
      'Tried to add invalid item of type "' +
        type +
        '" and template "' +
        templateName +
...
ProductionQueue.prototype.RemoveItem = function (id) {
  ...
  if (item.entityCache && item.entityCache.length) {
    for (let ent of item.entityCache) Engine.DestroyEntity(ent);
  ...
  if (item.unitTemplate) {
    ...
    if (template.TrainingRestrictions) {
      ...
      if (cmpPlayerEntityLimits)
        cmpPlayerEntityLimits.ChangeCount(
          template.TrainingRestrictions.Category,
          -item.count
        );
      if (template.TrainingRestrictions.MatchLimit)
  ...
  let totalCosts = {};
  let cmpStatisticsTracker = QueryPlayerIDInterface(
    item.player,
  ...
  for (let resource in item.resources) {
    ...
    if (cmpStatisticsTracker)
      cmpStatisticsTracker.IncreaseResourceUsedCounter(
        resource,
        -totalCosts[resource]
      );
  }
  ...
  if (cmpPlayer) {
    ...
    if (itemIndex == 0) cmpPlayer.UnBlockTraining();
  }
  ...
  if (!this.queue.length) this.StopTimer();
};

ProductionQueue.prototype.SetAnimation = function (name) {
  let cmpVisual = Engine.QueryInterface(this.entity, IID_Visual);
  if (cmpVisual) cmpVisual.SelectAnimation(name, false, 1);
};
...
ProductionQueue.prototype.ResetQueue = function () {
  while (this.queue.length) this.RemoveItem(this.queue[0].id);
};
...
ProductionQueue.prototype.SpawnUnits = function (item) {
  ...
  while (item.entityCache.length) {
    ...
    if (cmpPlayerEntityLimits) {
      let cmpTrainingRestrictions = Engine.QueryInterface(
        ent,
        IID_TrainingRestrictions
      );
      if (cmpTrainingRestrictions)
        cmpPlayerEntityLimits.ChangeCount(
          cmpTrainingRestrictions.GetCategory(),
          -1
        );
    }
  ...
  while (this.queue.length) {
    ...
    if (!item.productionStarted) {
      if (item.unitTemplate) {
        ...
        item.neededSlots = cmpPlayer.TryReservePopulationSlots(
          item.population * item.count
        );
        if (item.neededSlots) {
        ...
        Engine.PostMessage(this.entity, MT_TrainingStarted, {
          entity: this.entity,
        });
      }
      if (item.technologyTemplate) {
        ...
        if (cmpTechnologyManager)
          ...
          warn(
            "Failed to start researching " +
              item.technologyTemplate +
              ": No TechnologyManager available."
          );
      ...
      if (numSpawned == item.count) {
        ...
        if (!this.spawnNotified) {
          ...
          cmpGUIInterface.PushNotification({
            ...
            message: markForTranslation(
              "Can't find free space to spawn trained units"
            ),
            translateMessage: true,
          });
    ...
    if (item.technologyTemplate) {
      ...
      if (template && template.soundComplete) {
        let cmpSoundManager = Engine.QueryInterface(
          SYSTEM_ENTITY,
          IID_SoundManager
        );
        if (cmpSoundManager)
    ...
    if (this.autoqueuing && item.unitTemplate) {
      if (!this.AddItem(item.unitTemplate, "unit", item.count, item.metadata)) {
        ...
        const cmpGUIInterface = Engine.QueryInterface(
          SYSTEM_ENTITY,
          IID_GuiInterface
        );
        cmpGUIInterface.PushNotification({
          players: [cmpPlayer.GetPlayerID()],
          message: markForTranslation(
            "Could not auto-queue unit, de-activating."
          ),
          translateMessage: true,
        });
      }
...
ProductionQueue.prototype.OnValueModification = function (msg) {
  ...
  if (
    ...
    (msg.component != "ProductionQueue" ||
      !msg.valueNames.some((val) =>
        val.startsWith("ProductionQueue/Entities/")
      ))
  )
    return;
  ...
  let cmpPlayer = QueryOwnerInterface(this.entity);
  if (cmpPlayer)
    Engine.QueryInterface(SYSTEM_ENTITY, IID_GuiInterface).SetSelectionDirty(
      cmpPlayer.GetPlayerID()
    );
};
...
ProductionQueue.prototype.OnGarrisonedStateChanged = function (msg) {
  if (msg.holderID != INVALID_ENTITY) this.PauseProduction();
  else this.UnpauseProduction();
};

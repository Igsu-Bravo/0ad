...
Auras.prototype.Init = function () {
  ...
  for (let name of this.GetAuraNames()) this.affectedPlayers[name] = [];
...
Auras.prototype.GetModifierIdentifier = function (name) {
  if (AuraTemplates.Get(name).stackable) return "aura/" + name + this.entity;
  return "aura/" + name;
};
...
Auras.prototype.GetRange = function (name) {
  if (this.IsRangeAura(name)) return +AuraTemplates.Get(name).radius;
  return undefined;
};
...
Auras.prototype.GetRangeOverlays = function () {
  ...
  for (let name of this.GetAuraNames()) {
    if (!this.IsRangeAura(name) || !this[name].isApplied) continue;
...
Auras.prototype.CalculateAffectedPlayers = function (name) {
  ...
  if (!cmpPlayer) cmpPlayer = QueryOwnerInterface(this.entity);

  if (!cmpPlayer || cmpPlayer.GetState() == "defeated") return;
  ...
  for (let i of cmpPlayerManager.GetAllPlayers()) {
    ...
    if (
      affectedPlayers.some((p) =>
        p == "Player" ? cmpPlayer.GetPlayerID() == i : cmpPlayer["Is" + p](i)
      )
    )
...
Auras.prototype.CanApply = function (name) {
  if (!AuraTemplates.Get(name).requiredTechnology) return true;
  ...
  return cmpTechnologyManager.IsTechnologyResearched(
    AuraTemplates.Get(name).requiredTechnology
  );
};
...
Auras.prototype.Clean = function () {
  ...
  for (let name of auraNames) {
    ...
    if (this.IsRangeAura(name)) needVisualizationUpdate = true;

    if (this[name].targetUnits)
      targetUnitsClone[name] = this[name].targetUnits.slice();

    if (this.IsGlobalAura(name)) this.RemoveTemplateAura(name);
  ...
  for (let name of auraNames) {
    ...
    if (!affectedPlayers.length) continue;
    ...
    if (this.IsPlayerAura(name)) {
      ...
      this.ApplyAura(
        name,
        affectedPlayers.map((p) => cmpPlayerManager.GetPlayerByID(p))
      );
      continue;
    }
    ...
    if (
      this[name].isApplied &&
      (this.IsRangeAura(name) ||
        (this.IsGlobalAura(name) && !!this.GetOverlayIcon(name)))
    ) {
...
Auras.prototype.OnRangeUpdate = function (msg) {
  for (let name of this.GetAuraNames().filter(
    (n) => this[n] && msg.tag == this[n].rangeQuery
  )) {
...
Auras.prototype.OnGarrisonedUnitsChanged = function (msg) {
  for (let name of this.GetAuraNames().filter((n) =>
    this.IsGarrisonedUnitsAura(n)
  )) {
    this.ApplyAura(name, msg.added);
    this.RemoveAura(name, msg.removed);
  }
...
Auras.prototype.ApplyAura = function (name, ents) {
  ...
  if (this.GetOverlayIcon(name))
    for (let ent of validEnts) {
      ...
      if (cmpStatusBars) cmpStatusBars.AddAuraSource(this.entity, name);
    }
...
Auras.prototype.RemoveAura = function (name, ents, skipModifications = false) {
  ...
  this[name].targetUnits = this[name].targetUnits.filter(
    (v) => validEnts.indexOf(v) == -1
  );
  ...
  if (this.GetOverlayIcon(name))
    for (let ent of validEnts) {
      let cmpStatusBars = Engine.QueryInterface(ent, IID_StatusBars);
      if (cmpStatusBars) cmpStatusBars.RemoveAuraSource(this.entity, name);
    }
...
Auras.prototype.OnDiplomacyChanged = function (msg) {
  ...
  if (
    (cmpPlayer &&
      (cmpPlayer.GetPlayerID() == msg.player ||
        cmpPlayer.GetPlayerID() == msg.otherPlayer)) ||
...
Auras.prototype.OnGlobalResearchFinished = function (msg) {
  var cmpPlayer = Engine.QueryInterface(this.entity, IID_Player);
  if (
    (!cmpPlayer || cmpPlayer.GetPlayerID() != msg.player) &&
    !IsOwnedByPlayer(msg.player, this.entity)
  )
    return;
  for (let name of this.GetAuraNames()) {
...
Auras.prototype.OnGlobalPlayerDefeated = function (msg) {
  ...
  if (
    (cmpPlayer && cmpPlayer.GetPlayerID() == msg.playerId) ||
    this.GetAuraNames().some(
      (name) => this.GetAffectedPlayers(name).indexOf(msg.playerId) != -1
    )
  )
    this.Clean();
};
...
Auras.prototype.OnGarrisonedStateChanged = function (msg) {
  if (!this.HasGarrisonAura()) return;

  if (msg.holderID != INVALID_ENTITY) this.ApplyGarrisonAura(msg.holderID);
  if (msg.oldHolder != INVALID_ENTITY) this.RemoveGarrisonAura(msg.oldHolder);
};

...
BuildRestrictions.prototype.CheckPlacement = function () {
  ...
  var result = {
    ...
    message: markForTranslation(
      "%(name)s cannot be built due to unknown error"
    ),
    parameters: {
      name: name,
    },
  ...
  if (!cmpPlayer) return result; // Fail
  ...
  if (!cmpPlayer.IsAI()) {
    ...
    if (!cmpRangeManager || !cmpOwnership) return result; // Fail

    var explored =
      cmpRangeManager.GetLosVisibility(this.entity, cmpOwnership.GetOwner()) !=
      "hidden";
    if (!explored) {
  ...
  if (!cmpObstruction) return result; // Fail
  ...
  if (ret != "success") {
    switch (ret) {
      ...
      case "fail_obstructs_foundation":
        result.message = markForTranslation(
          "%(name)s cannot be built on another building or resource"
        );
        break;
      case "fail_terrain_class":
        // TODO: be more specific and/or list valid terrain?
        result.message = markForTranslation(
          "%(name)s cannot be built on invalid terrain"
        );
    }
    return result; // Fail
  }
  ...
  if (isOwn) {
    if (!this.HasTerritory("own"))
      ...
      // Translation: territoryType being displayed in a translated sentence in the form: "House cannot be built in %(territoryType)s territory.".
      invalidTerritory = markForTranslationWithContext(
        "Territory type",
        "unconnected own"
      );
  ...
  if (invalidTerritory) {
    result.message = markForTranslation(
      "%(name)s cannot be built in %(territoryType)s territory. Valid territories: %(validTerritories)s"
    );
    result.translateParameters.push("territoryType");
    result.translateParameters.push("validTerritories");
    result.parameters.territoryType = {
      context: "Territory type",
      message: invalidTerritory,
    };
    // gui code will join this array to a string
    result.parameters.validTerritories = {
      context: "Territory type list",
  ...
  if (this.template.PlacementType == "shore") {
    if (!cmpObstruction.CheckShorePlacement()) {
      result.message = markForTranslation(
        "%(name)s must be built on a valid shoreline"
      );
      return result; // Fail
    }
  }
  ...
  let template = cmpTemplateManager.GetTemplate(
    removeFiltersFromTemplateName(templateName)
  );
  ...
  if (this.template.Distance) {
    ...
    if (this.template.Distance.MinDistance !== undefined) {
      let minDistance = ApplyValueModificationsToTemplate(
    ...
    if (this.template.Distance.MaxDistance !== undefined) {
      let maxDistance = ApplyValueModificationsToTemplate(

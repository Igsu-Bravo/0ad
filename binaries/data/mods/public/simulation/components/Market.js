...
Market.prototype.GetInternationalBonus = function () {
  return ApplyValueModificationsToEntity(
    ...
    +this.template.InternationalBonus,
    this.entity
  );
};
...
Market.prototype.UpdateTraders = function (onDestruction) {
  for (let trader of this.traders) {
    ...
    if (
      !cmpTrader.HasMarket(this.entity) ||
      (!onDestruction && cmpTrader.CanTrade(this.entity))
    )
      continue;
...
MarketMirage.prototype.Init = function (cmpMarket, entity, parent, player) {
  ...
  for (let trader of cmpMarket.GetTraders()) {
    ...
    if (this.player != cmpOwnership.GetOwner()) continue;
...
MarketMirage.prototype.HasType = function (type) {
  return this.marketType.has(type);
};
MarketMirage.prototype.GetInternationalBonus = function () {
  return this.internationalBonus;
};
MarketMirage.prototype.AddTrader = function (trader) {
  this.traders.add(trader);
};
MarketMirage.prototype.RemoveTrader = function (trader) {
...
MarketMirage.prototype.UpdateTraders = function (msg) {
  ...
  if (!cmpMarket) {
    ...
    for (let trader of this.traders) {
      let cmpTrader = Engine.QueryInterface(trader, IID_Trader);
      if (cmpTrader) cmpTrader.RemoveMarket(this.entity);
    }
    return;
  }
...
MarketMirage.prototype.CalculateTraderGain =
  Market.prototype.CalculateTraderGain;

...
Heal.prototype.CanHeal = function (target) {
  ...
  return (
    !MatchesClassList(targetClasses, this.GetUnhealableClasses()) &&
...
Heal.prototype.StartHealing = function (target, callerIID) {
  if (this.target) this.StopHealing();

  if (!this.CanHeal(target)) return false;
...
Heal.prototype.PerformHeal = function (data, lateness) {
  ...
  if (targetState !== undefined && cmpLoot && cmpPromotion)
    ...
    cmpPromotion.IncreaseXp(
      ((targetState.new - targetState.old) / cmpHealth.GetMaxHitpoints()) *
        cmpLoot.GetXp()
    );

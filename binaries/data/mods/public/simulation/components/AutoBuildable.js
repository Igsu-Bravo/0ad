class AutoBuildable {
  ...
  UpdateRate() {
    this.rate = ApplyValueModificationsToEntity(
      "AutoBuildable/Rate",
      +this.template.Rate,
      this.entity
    );
    if (this.rate) this.StartTimer();
  }
  ...
  CancelTimer() {
    ...
    let cmpFoundation = Engine.QueryInterface(this.entity, IID_Foundation);
    if (cmpFoundation) cmpFoundation.RemoveBuilder(this.entity);
  ...
  OnValueModification(msg) {
    if (msg.component != "AutoBuildable") return;

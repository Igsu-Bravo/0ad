...
RangeOverlayManager.prototype.UpdateRangeOverlays = function (componentName) {
  ...
  if (cmp) this.rangeVisualizations.set(componentName, cmp.GetRangeOverlays());
};

RangeOverlayManager.prototype.SetEnabled = function (
  enabled,
  enabledRangeTypes,
  forceUpdate
) {
...
RangeOverlayManager.prototype.RegenerateRangeOverlays = function (forceUpdate) {
  let cmpRangeOverlayRenderer = Engine.QueryInterface(
    this.entity,
    IID_RangeOverlayRenderer
  );
  if (!cmpRangeOverlayRenderer) return;

  cmpRangeOverlayRenderer.ResetRangeOverlays();

  if (!this.enabled && !forceUpdate) return;
...
RangeOverlayManager.prototype.OnOwnershipChanged = function (msg) {
  ...
  for (let type in this.enabledRangeTypes) this.UpdateRangeOverlays(type);
...
RangeOverlayManager.prototype.OnValueModification = function (msg) {
  if (
    msg.valueNames.indexOf("Heal/Range") == -1 &&

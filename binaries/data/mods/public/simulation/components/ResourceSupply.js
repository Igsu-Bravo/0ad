...
ResourceSupply.prototype.GetDiminishingReturns = function () {
  if (!this.template.DiminishingReturns) return null;
  ...
  if (!diminishingReturns) return null;
...
ResourceSupply.prototype.TakeResources = function (amount) {
  if (this.IsInfinite()) return { amount: amount, exhausted: false };
...
ResourceSupply.prototype.AddGatherer = function (gathererID) {
  ...
  if (this.IsGatheringUs(gathererID)) return true;
...
ResourceSupply.prototype.AddActiveGatherer = function (entity) {
  if (!this.AddGatherer(entity)) return false;
...
ResourceSupply.prototype.CheckTimers = function () {
  if (!this.template.Change || this.IsInfinite()) return;
  ...
  for (let changeKey in this.template.Change) {
    ...
    if (
      this.amount < +(template.LowerLimit || -1) ||
    ...
    if (!this.timers[changeKey]) this.StartTimer(changeKey);
  }
};
...
ResourceSupply.prototype.CheckState = function (changeKey) {
  let template = this.template.Change[changeKey];
  if (!template.State) return true;

  let states = template.State;
  let cmpHealth = Engine.QueryInterface(this.entity, IID_Health);
  if (
    (states.indexOf("alive") != -1 &&
      !cmpHealth &&
      states.indexOf("dead") == -1) ||
  ...
  if (
    (states.indexOf("gathered") != -1 &&
      activeGatherers == 0 &&
      states.indexOf("notGathered") == -1) ||
    (states.indexOf("notGathered") != -1 &&
      activeGatherers > 0 &&
      states.indexOf("gathered") == -1)
  )
...
ResourceSupply.prototype.StartTimer = function (changeKey) {
  if (this.timers[changeKey]) return;

  let cmpTimer = Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer);
  let interval = ApplyValueModificationsToEntity(
    "ResourceSupply/Change/" + changeKey + "/Interval",
    +(this.template.Change[changeKey].Interval || 1000),
    this.entity
  );
  this.timers[changeKey] = cmpTimer.SetInterval(
    this.entity,
    IID_ResourceSupply,
    "TimerTick",
    interval,
...
ResourceSupply.prototype.RecalculateValues = function () {
  this.maxAmount = ApplyValueModificationsToEntity(
    "ResourceSupply/Max",
    +this.template.Max,
    this.entity
  );
  ...
  for (let changeKey in this.template.Change)
    this.cachedChanges[changeKey] = ApplyValueModificationsToEntity(
      "ResourceSupply/Change/" + changeKey + "/Value",
      +this.template.Change[changeKey].Value,
      this.entity
    );
...
ResourceSupply.prototype.OnValueModification = function (msg) {
  if (msg.component != "ResourceSupply") return;
...
ResourceSupply.prototype.OnEntityRenamed = function (msg) {
  let cmpResourceSupplyNew = Engine.QueryInterface(
    msg.newentity,
    IID_ResourceSupply
  );
...
ResourceSupplyMirage.prototype.GetMaxAmount = function () {
  return this.maxAmount;
};
ResourceSupplyMirage.prototype.GetCurrentAmount = function () {
  return this.amount;
};
ResourceSupplyMirage.prototype.GetType = function () {
  return this.type;
};
ResourceSupplyMirage.prototype.IsInfinite = function () {
  return this.isInfinite;
};
ResourceSupplyMirage.prototype.GetKillBeforeGather = function () {
  return this.killBeforeGather;
};
ResourceSupplyMirage.prototype.GetMaxGatherers = function () {
  return this.maxGatherers;
};
ResourceSupplyMirage.prototype.GetNumGatherers = function () {
  return this.numGatherers;
};
...
ResourceSupplyMirage.prototype.GetDiminishingReturns = function () {
  return null;
};

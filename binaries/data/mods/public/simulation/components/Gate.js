...
Gate.prototype.OnOwnershipChanged = function (msg) {
  if (msg.to != INVALID_PLAYER) {
    ...
    if (!this.locked) this.UnlockGate(true);
  }
};
...
Gate.prototype.OnDestroy = function () {
  ...
  if (this.unitsQuery) cmpRangeManager.DestroyActiveQuery(this.unitsQuery);
...
Gate.prototype.SetupRangeQuery = function (owner) {
  ...
  if (range > 0) {
    ...
    this.unitsQuery = cmpRangeManager.CreateActiveQuery(
      this.entity,
      0,
      range,
      players,
...
Gate.prototype.OnRangeUpdate = function (msg) {
  if (msg.tag != this.unitsQuery) return;
  ...
  if (msg.added.length > 0)
    for (let entity of msg.added) {
      ...
      if (!unitAI || !unitAI.AbleToMove()) this.ignoreList.push(entity);
  ...
  if (msg.removed.length > 0)
    for (let entity of msg.removed) {
      ...
      if (index !== -1) this.ignoreList.splice(index, 1);
...
Gate.prototype.OnGlobalUnitAbleToMoveChanged = function (msg) {
  if (this.allies.indexOf(msg.entity) === -1) return;

  let index = this.ignoreList.indexOf(msg.entity);
  if (msg.ableToMove && index !== -1) this.ignoreList.splice(index, 1);
  else if (!msg.ableToMove && index === -1) this.ignoreList.push(msg.entity);
...
Gate.prototype.OperateGate = function () {
  ...
  if (this.opened && (this.locked || !this.ShouldOpen())) this.CloseGate();
  else if (!this.opened && this.ShouldOpen()) this.OpenGate();
};
...
Gate.prototype.UnlockGate = function (quiet) {
  ...
  // TODO: Possibly move the lock/unlock sounds to UI? Needs testing
  if (!quiet) PlaySound("gate_unlocked", this.entity);

  // If the gate is closed, open it if necessary
  if (!this.opened) this.OperateGate();
};
...
Gate.prototype.OpenGate = function () {
  ...
  if (cmpVisual) cmpVisual.SelectAnimation("gate_opening", true, 1.0);
};
...
Gate.prototype.CloseGate = function () {
  ...
  else cmpObstruction.SetDisableBlockMovementPathfinding(false, true, 0);
  this.opened = false;

  PlaySound("gate_closing", this.entity);
  let cmpVisual = Engine.QueryInterface(this.entity, IID_Visual);
  if (cmpVisual) cmpVisual.SelectAnimation("gate_closing", true, 1.0);
};

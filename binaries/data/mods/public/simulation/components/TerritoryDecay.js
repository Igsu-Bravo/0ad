...
TerritoryDecay.prototype.Init = function () {
  this.decaying = false;
  this.connectedNeighbours = new Array(
    Engine.QueryInterface(SYSTEM_ENTITY, IID_PlayerManager).GetNumPlayers()
  ).fill(0);
...
TerritoryDecay.prototype.IsConnected = function () {
  ...
  if (!cmpPlayer) return true; // something without ownership can't decay

  const decayTerritory = ApplyValueModificationsToEntity(
    "TerritoryDecay/Territory",
    this.template.Territory,
    this.entity
  );

  var cmpTerritoryManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
  ...
  if (tileOwner == 0) {
    ...
    return (
      cmpPlayer.GetPlayerID() == 0 || decayTerritory.indexOf("neutral") === -1
    );
  }
  ...
  this.connectedNeighbours = cmpTerritoryManager.GetNeighbours(
    pos.x,
    pos.y,
    true
  );
...
TerritoryDecay.prototype.UpdateDecayState = function () {
  ...
  if (decaying === this.decaying) return;
  this.decaying = decaying;
  Engine.PostMessage(this.entity, MT_TerritoryDecayChanged, {
    entity: this.entity,
    to: decaying,
    rate: this.GetDecayRate(),
  });
};
...
TerritoryDecay.prototype.UpdateOwner = function () {
  ...
  if (!cmpOwnership || !cmpPosition || !cmpPosition.IsInWorld()) return;
  ...
  if (tileOwner != cmpOwnership.GetOwner()) cmpOwnership.SetOwner(tileOwner);
};

TerritoryDecay.prototype.OnTerritoriesChanged = function (msg) {
  if (this.territoryOwnership) this.UpdateOwner();
  else this.UpdateDecayState();
};
...
TerritoryDecay.prototype.OnDiplomacyChanged = function (msg) {
  ...
  if (!this.territoryOwnership) this.UpdateDecayState();
};
...
TerritoryDecay.prototype.OnOwnershipChanged = function (msg) {
  ...
  if (msg.from == INVALID_PLAYER || msg.to == INVALID_PLAYER) {
    let cmpTerritoryDecayManager = Engine.QueryInterface(
      SYSTEM_ENTITY,
      IID_TerritoryDecayManager
    );
    if (msg.from == INVALID_PLAYER) cmpTerritoryDecayManager.Add(this.entity);
    else cmpTerritoryDecayManager.Remove(this.entity);
  }

  // if it influences the territory, wait until we get a TerritoriesChanged message
  if (
    !this.territoryOwnership &&
    !Engine.QueryInterface(this.entity, IID_TerritoryInfluence)
  )

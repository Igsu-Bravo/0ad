...
ResourceGatherer.prototype.RecalculateGatherRates = function () {
  this.baseSpeed = ApplyValueModificationsToEntity(
...
ResourceGatherer.prototype.RecalculateCapacity = function (type) {
  if (type in this.capacities)
    this.capacities[type] = ApplyValueModificationsToEntity(
      "ResourceGatherer/Capacities/" + type,
      +this.template.Capacities[type],
      this.entity
    );
};
...
ResourceGatherer.prototype.StartGathering = function (target, callerIID) {
  if (this.target) this.StopGathering();
  ...
  if (this.IsCarryingAnythingExcept(resourceType.generic)) this.DropResources();
...
ResourceGatherer.prototype.StopGathering = function (reason) {
  ...
  let cmpResourceSupply = Engine.QueryInterface(
    this.target,
    IID_ResourceSupply
  );
  if (cmpResourceSupply) cmpResourceSupply.RemoveGatherer(this.entity);
...
ResourceGatherer.prototype.PerformGather = function (data, lateness) {
  ...
  let status = cmpResourceSupply.TakeResources(
    Math.min(this.GATHER_AMOUNT, maxGathered)
  );
  ...
  if (!this.CanCarryMore(type.generic)) this.StopGathering("InventoryFilled");
  else if (status.exhausted) this.StopGathering("TargetInvalidated");
};
...
ResourceGatherer.prototype.GetTargetGatherRate = function (target) {
  ...
  if (!cmpResourceSupply || cmpResourceSupply.GetCurrentAmount() <= 0) return 0;
...
ResourceGatherer.prototype.CanReturnResource = function (
  ...
  return (
    cmpPlayer &&
    cmpPlayer.HasSharedDropsites() &&
    cmpResourceDropsite.IsShared() &&
    cmpOwnership &&
    IsOwnedByMutualAllyOfPlayer(cmpOwnership.GetOwner(), target)
  );
};
...
ResourceGatherer.prototype.CommitResources = function (target) {
  ...
  for (let type in change) {
    ...
    if (this.carrying[type] == 0) delete this.carrying[type];
    changed = true;
  }
...
ResourceGatherer.prototype.AddToPlayerCounter = function (type) {
  ...
  if (this.lastGathered) return;

  let cmpPlayer = QueryOwnerInterface(this.entity, IID_Player);
  if (cmpPlayer) cmpPlayer.AddResourceGatherer(type);
...
ResourceGatherer.prototype.RemoveFromPlayerCounter = function (playerid) {
  ...
  if (cmpPlayer) cmpPlayer.RemoveResourceGatherer(this.lastGathered);
...
ResourceGatherer.prototype.OnValueModification = function (msg) {
  ...
  if (msg.valueNames.length === 1) {
    ...
    else this.RecalculateGatherRates();
  } else {

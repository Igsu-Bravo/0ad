...
GarrisonHolder.prototype.Init = function () {
  this.entities = [];
  this.allowedClasses = ApplyValueModificationsToEntity(
    "GarrisonHolder/List/_string",
    this.template.List._string,
    this.entity
  );
};
...
GarrisonHolder.prototype.CanPickup = function (ent) {
  if (!this.template.Pickup || this.IsFull()) return false;
...
GarrisonHolder.prototype.AllowGarrisoning = function (allow, callerID) {
  if (!this.allowGarrisoning) this.allowGarrisoning = new Map();
...
GarrisonHolder.prototype.OccupiedSlots = function () {
  ...
  for (let ent of this.entities) {
    ...
    if (cmpGarrisonable) count += cmpGarrisonable.TotalSize();
  }
  return count;
};

GarrisonHolder.prototype.IsAllowedToGarrison = function (entity) {
  if (!this.IsGarrisoningAllowed()) return false;

  let cmpGarrisonable = Engine.QueryInterface(entity, IID_Garrisonable);
  if (
    !cmpGarrisonable ||
    this.OccupiedSlots() + cmpGarrisonable.TotalSize() > this.GetCapacity()
  )
    return false;
...
GarrisonHolder.prototype.IsAllowedToBeGarrisoned = function (entity) {
  if (!IsOwnedByMutualAllyOfEntity(entity, this.entity)) return false;

  let cmpIdentity = Engine.QueryInterface(entity, IID_Identity);
  return (
    cmpIdentity &&
    MatchesClassList(cmpIdentity.GetClassesList(), this.allowedClasses)
  );
};
...
GarrisonHolder.prototype.Garrison = function (entity) {
  if (!this.IsAllowedToGarrison(entity)) return false;

  if (!this.HasEnoughHealth()) return false;

  if (!this.timer && this.GetHealRate()) this.StartTimer();
...
GarrisonHolder.prototype.Eject = function (entity, forced) {
  if (!this.IsGarrisoningAllowed() && !forced) return false;
...
GarrisonHolder.prototype.UnloadEntities = function (entities) {
  let success = true;
  for (let entity of entities) if (!this.Unload(entity)) success = false;
  return success;
};
...
GarrisonHolder.prototype.UnloadTemplate = function (template, owner, all) {
  ...
  for (let entity of this.entities) {
    ...
    let name =
      cmpIdentity.GetSelectionGroupName() ||
      cmpTemplateManager.GetCurrentTemplateName(entity);
    if (
      name != template ||
      owner != Engine.QueryInterface(entity, IID_Ownership).GetOwner()
    )
      continue;
...
GarrisonHolder.prototype.HasEnoughHealth = function () {
  ...
  if (this.template.EjectHealth === undefined) return true;

  let cmpHealth = Engine.QueryInterface(this.entity, IID_Health);
  return (
    !cmpHealth ||
    cmpHealth.GetHitpoints() >
      Math.floor(+this.template.EjectHealth * cmpHealth.GetMaxHitpoints())
  );
};
...
GarrisonHolder.prototype.HealTimeout = function (data, lateness) {
  ...
  for (let entity of this.entities) {
    ...
    if (cmpHealth && !cmpHealth.IsUnhealable()) cmpHealth.Increase(healRate);
  }
};
...
GarrisonHolder.prototype.UpdateGarrisonFlag = function () {
  ...
  cmpVisual.SetVariant(
    "garrison",
    this.entities.length ? "garrisoned" : "ungarrisoned"
  );
};
...
GarrisonHolder.prototype.OnGlobalOwnershipChanged = function (msg) {
  ...
  if (this.entity == msg.entity) {
    let entities = this.entities.filter(
      (ent) =>
        ...
        !IsOwnedByMutualAllyOfEntity(this.entity, ent)
    );

    if (entities.length) this.EjectOrKill(entities);
  ...
  if (
    entityIndex != -1 &&
    (msg.to == INVALID_PLAYER ||
      !IsOwnedByMutualAllyOfEntity(this.entity, msg.entity))
  )
...
GarrisonHolder.prototype.OnGlobalSkirmishReplacerReplaced = function (msg) {
  ...
  if (msg.entity == this.entity) {
    ...
    if (cmpGarrisonHolder) cmpGarrisonHolder.initGarrison = this.initGarrison;
  } else {
    let entityIndex = this.initGarrison.indexOf(msg.entity);
    if (entityIndex != -1) this.initGarrison[entityIndex] = msg.newentity;
  }
};
...
GarrisonHolder.prototype.OnDiplomacyChanged = function () {
  this.EjectOrKill(
    this.entities.filter(
      (ent) => !IsOwnedByMutualAllyOfEntity(this.entity, ent)
    )
  );
};
...
GarrisonHolder.prototype.EjectOrKill = function (entities) {
  ...
  if (cmpPosition && cmpPosition.IsInWorld()) {
    ...
    if (ejectables.length) this.UnloadEntities(ejectables);
  }
  ...
  for (let entity of entities) {
    ...
    else Engine.DestroyEntity(entity);
    this.entities.splice(entityIndex, 1);
...
GarrisonHolder.prototype.IsEjectable = function (entity) {
  if (!this.entities.find((ent) => ent == entity)) return false;

  let ejectableClasses = this.template.EjectClassesOnDestroy._string;
  let entityClasses = Engine.QueryInterface(
    entity,
    IID_Identity
  ).GetClassesList();
...
GarrisonHolder.prototype.OnValueModification = function (msg) {
  if (msg.component != "GarrisonHolder") return;
  ...
  if (msg.valueNames.indexOf("GarrisonHolder/List/_string") !== -1) {
    ...
    this.EjectOrKill(
      this.entities.filter((entity) => !this.IsAllowedToBeGarrisoned(entity))
    );
  }

  if (msg.valueNames.indexOf("GarrisonHolder/BuffHeal") === -1) return;

  if (this.timer && !this.GetHealRate()) this.StopTimer();
  else if (!this.timer && this.GetHealRate()) this.StartTimer();
};

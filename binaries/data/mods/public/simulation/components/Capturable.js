...
Capturable.prototype.Capture = function (amount, captor, captorOwner) {
  if (captorOwner == INVALID_PLAYER || !this.CanCapture(captorOwner)) return {};
...
Capturable.prototype.Reduce = function (amount, playerID) {
  ...
  if (!cmpOwnership || cmpOwnership.GetOwner() == INVALID_PLAYER) return 0;

  let cmpPlayerSource = QueryPlayerIDInterface(playerID);
  if (!cmpPlayerSource) return 0;

  // Before changing the value, activate Fogging if necessary to hide changes.
  let cmpFogging = Engine.QueryInterface(this.entity, IID_Fogging);
  if (cmpFogging) cmpFogging.Activate();

  let numberOfEnemies = this.capturePoints.filter(
    (v, i) => v > 0 && cmpPlayerSource.IsEnemy(i)
  ).length;
  ...
  while (distributedAmount > 0.0001) {
    ...
    for (let i in this.capturePoints) {
      if (!this.capturePoints[i] || !cmpPlayerSource.IsEnemy(i)) continue;
    ...
    distributedAmount = numberOfEnemies
      ? (amount - removedAmount) / numberOfEnemies
      : 0;
  }

  // Give all capture points taken to the player.
  let takenCapturePoints =
    this.maxCapturePoints - this.capturePoints.reduce((a, b) => a + b);
...
Capturable.prototype.RegisterCapturePointsChanged = function () {
  ...
  Engine.PostMessage(this.entity, MT_CapturePointsChanged, {
    capturePoints: this.capturePoints,
  });

  let owner = cmpOwnership.GetOwner();
  if (owner == INVALID_PLAYER || this.capturePoints[owner] > 0) return;

  // If all capture points have been taken from the owner, convert it to player with the most capture points.
  let cmpLostPlayerStatisticsTracker = QueryOwnerInterface(
    this.entity,
  ...
  cmpOwnership.SetOwner(
    this.capturePoints.reduce(
      (bestPlayer, playerCapturePoints, player, capturePoints) =>
        playerCapturePoints > capturePoints[bestPlayer] ? player : bestPlayer,
      0
    )
  );

  let cmpCapturedPlayerStatisticsTracker = QueryOwnerInterface(
    this.entity,
    IID_StatisticsTracker
  );
...
Capturable.prototype.GetRegenRate = function () {
  ...
  if (!cmpGarrisonHolder) return this.regenRate;

  return (
    this.regenRate +
    this.GetGarrisonRegenRate() * cmpGarrisonHolder.GetEntities().length
  );
};

Capturable.prototype.TimerTick = function () {
  let cmpOwnership = Engine.QueryInterface(this.entity, IID_Ownership);
  if (!cmpOwnership || cmpOwnership.GetOwner() == INVALID_PLAYER) return;
  ...
  let cmpTerritoryDecay = Engine.QueryInterface(
    this.entity,
    IID_TerritoryDecay
  );
  if (cmpTerritoryDecay && cmpTerritoryDecay.IsDecaying()) {
    ...
    else this.capturePoints[0] += decay;
  ...
  if (regenRate < 0) modifiedCapturePoints += this.Reduce(-regenRate, 0);
  else if (regenRate > 0)
    modifiedCapturePoints += this.Reduce(regenRate, owner);

  if (modifiedCapturePoints) return;
...
Capturable.prototype.UpdateCachedValues = function () {
  this.garrisonRegenRate = ApplyValueModificationsToEntity(
    "Capturable/GarrisonRegenRate",
    +this.template.GarrisonRegenRate,
    this.entity
  );
  this.regenRate = ApplyValueModificationsToEntity(
    "Capturable/RegenRate",
    +this.template.RegenRate,
    this.entity
  );
  this.maxCapturePoints = ApplyValueModificationsToEntity(
    "Capturable/CapturePoints",
    +this.template.CapturePoints,
    this.entity
  );
};
 ...
 */
Capturable.prototype.UpdateCachedValuesAndNotify = function (
  sendMessage = true
) {
  ...
  if (oldMaxCapturePoints != this.maxCapturePoints) {
    ...
    for (let i in this.capturePoints) this.capturePoints[i] *= scale;
    if (sendMessage)
      Engine.PostMessage(this.entity, MT_CapturePointsChanged, {
        capturePoints: this.capturePoints,
      });
  ...
  if (
    oldGarrisonRegenRate != this.garrisonRegenRate ||
    oldRegenRate != this.regenRate
  )
    this.CheckTimer();
};
...
Capturable.prototype.OnValueModification = function (msg) {
  if (msg.component == "Capturable") this.UpdateCachedValuesAndNotify();
};
...
Capturable.prototype.OnOwnershipChanged = function (msg) {
  ...
  if (!this.capturePoints.length) {
    ...
    for (let i = 0; i < numPlayers; ++i) {
      if (i == msg.to) this.capturePoints[i] = this.maxCapturePoints;
      else this.capturePoints[i] = 0;
    }
    this.CheckTimer();
    return;
  }
...
Capturable.prototype.OnGlobalPlayerDefeated = function (msg) {
  if (!this.capturePoints[msg.playerId]) return;
  let cmpOwnership = Engine.QueryInterface(this.entity, IID_Ownership);
  if (
    cmpOwnership &&
    (cmpOwnership.GetOwner() == INVALID_PLAYER ||
...
CapturableMirage.prototype.GetCapturePoints = function () {
  return this.capturePoints;
};
CapturableMirage.prototype.GetMaxCapturePoints = function () {
  return this.maxCapturePoints;
};

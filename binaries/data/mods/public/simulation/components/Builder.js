...
Builder.prototype.GetEntitiesList = function () {
  ...
  if (cmpIdentity) string = string.replace(/\{native\}/g, cmpIdentity.GetCiv());
  ...
  return entities.filter(
    (ent) => !disabledTemplates[ent] && cmpTemplateManager.TemplateExists(ent)
  );
};
...
Builder.prototype.GetRange = function () {
  ...
  if (cmpObstruction) max += cmpObstruction.GetSize();
...
Builder.prototype.StartRepairing = function (target, callerIID) {
  if (this.target) this.StopRepairing();

  if (!this.CanRepair(target)) return false;

  let cmpBuilderList = QueryBuilderListInterface(target);
  if (cmpBuilderList) cmpBuilderList.AddBuilder(this.entity);

  let cmpVisual = Engine.QueryInterface(this.entity, IID_Visual);
  if (cmpVisual) cmpVisual.SelectAnimation("build", false, 1.0);
...
Builder.prototype.StopRepairing = function (reason) {
  ...
  if (cmpBuilderList) cmpBuilderList.RemoveBuilder(this.entity);
...
Builder.prototype.OnValueModification = function (msg) {
  if (
    msg.component != "Builder" ||
    !msg.valueNames.some((name) => name.endsWith("_string"))
  )
    return;

...
Upgrade.prototype.Init = function () {
  ...
  for (let choice in this.template) {
    ...
    if (cmpIdentity) name = name.replace(/\{civ\}/g, cmpIdentity.GetCiv());
    if (this.upgradeTemplates.name)
      warn(
        "Upgrade Component: entity " +
          this.entity +
          " has two upgrades to the same entity, only the last will be used."
      );
...
Upgrade.prototype.OnOwnershipChanged = function (msg) {
  if (!this.completed) this.CancelUpgrade(msg.from);

  if (msg.to != INVALID_PLAYER) this.owner = msg.to;
};
...
Upgrade.prototype.ChangeUpgradedEntityCount = function (amount) {
  ...
  if (categoryTo == categoryFrom) return;

  let cmpEntityLimits = QueryPlayerIDInterface(this.owner, IID_EntityLimits);
  if (cmpEntityLimits) cmpEntityLimits.ChangeCount(categoryTo, amount);
};
...
Upgrade.prototype.GetUpgrades = function () {
  ...
  for (let option in this.template) {
    ...
    let templateName = cmpIdentity
      ? choice.Entity.replace(/\{civ\}/g, cmpIdentity.GetCiv())
      : choice.Entity;

    let cost = {};
    if (choice.Cost) cost = this.GetResourceCosts(templateName);
    if (choice.Time) cost.time = this.GetUpgradeTime(templateName);
...
Upgrade.prototype.WillCheckPlacementRestrictions = function (template) {
  if (!this.upgradeTemplates[template]) return undefined;

  // is undefined by default so use X in Y
  return (
    "CheckPlacementRestrictions" in
    this.template[this.upgradeTemplates[template]]
  );
};
...
Upgrade.prototype.GetRequiredTechnology = function (templateArg) {
  ...
  if (!("RequiredTechnology" in this.template[choice])) return undefined;
  ...
  if (cmpIdentity) entType = entType.replace(/\{civ\}/g, cmpIdentity.GetCiv());
...
Upgrade.prototype.GetResourceCosts = function (template) {
  ...
  if (!this.template[choice].Cost) return {};

  let costs = {};
  for (let r in this.template[choice].Cost)
    costs[r] = ApplyValueModificationsToEntity(
      "Upgrade/Cost/" + r,
      +this.template[choice].Cost[r],
      this.entity
    );
...
Upgrade.prototype.Upgrade = function (template) {
  if (this.IsUpgrading() || !this.upgradeTemplates[template]) return false;
  ...
  if (cmpProductionQueue && cmpProductionQueue.HasQueuedProduction()) {
    ...
    cmpGUIInterface.PushNotification({
      ...
      message: markForTranslation(
        "Entity is producing. Cannot start upgrading."
      ),
  ...
  if (this.GetUpgradeTime(template) !== 0) {
    ...
    this.timer = cmpTimer.SetInterval(
      ...
      "UpgradeProgress",
      0,
      UPGRADING_PROGRESS_INTERVAL,
      { upgrading: template }
    );
  } else this.UpgradeProgress();
...
Upgrade.prototype.CancelUpgrade = function (owner) {
  ...
  if (cmpPlayer) cmpPlayer.AddResources(this.expendedResources);
...
Upgrade.prototype.GetProgress = function () {
  if (!this.IsUpgrading()) return undefined;
  return this.GetUpgradeTime() == 0
    ? 1
    : Math.min(this.elapsedTime / 1000.0 / this.GetUpgradeTime(), 1.0);
};
...
Upgrade.prototype.SetUpgradeAnimationVariant = function () {
  ...
  if (!choice || !this.template[choice].Variant) return;
...
Upgrade.prototype.UpgradeProgress = function (data, lateness) {
  if (this.elapsedTime / 1000.0 < this.GetUpgradeTime()) {
    this.SetElapsedTime(
      this.GetElapsedTime() + UPGRADING_PROGRESS_INTERVAL + lateness
    );
    return;
  }
  ...
  if (newEntity) PlaySound("upgraded", newEntity);
};

...
Attack.prototype.GetAttackTypes = function (wantedTypes) {
  ...
  if (!wantedTypes) return types;

  let wantedTypesReal = wantedTypes.filter((wtype) => wtype.indexOf("!") != 0);
  return types.filter(
    (type) =>
      wantedTypes.indexOf("!" + type) == -1 &&
      (!wantedTypesReal ||
        !wantedTypesReal.length ||
        wantedTypesReal.indexOf(type) != -1)
  );
};
...
Attack.prototype.GetPreferredClasses = function (type) {
  if (
    this.template[type] &&
    this.template[type].PreferredClasses &&
...
Attack.prototype.GetRestrictedClasses = function (type) {
  if (
    this.template[type] &&
    this.template[type].RestrictedClasses &&
...
Attack.prototype.CanAttack = function (target, wantedTypes) {
  ...
  const heightDiff = Math.abs(
    cmpThisPosition.GetHeightOffset() - cmpTargetPosition.GetHeightOffset()
  );

  for (const type of types) {
    if (
      type != "Capture" &&
      (!cmpEntityPlayer.IsEnemy(targetOwner) ||
        !cmpHealth ||
        !cmpHealth.GetHitpoints())
    )
      continue;
    ...
    if (heightDiff > this.GetRange(type).max) continue;

    const restrictedClasses = this.GetRestrictedClasses(type);
    if (!restrictedClasses.length) return true;

    if (!MatchesClassList(targetClasses, restrictedClasses)) return true;
  }
...
Attack.prototype.GetPreference = function (target) {
  ...
  if (!cmpIdentity) return undefined;
  ...
  for (let type of this.GetAttackTypes()) {
    ...
    for (let pref = 0; pref < preferredClasses.length; ++pref) {
      if (MatchesClassList(targetClasses, preferredClasses[pref])) {
        ...
        if (minPref === undefined || minPref > pref) minPref = pref;
      }
    }
  }
  return minPref;
...
Attack.prototype.GetAttackEffectsData = function (type, splash) {
  ...
  if (splash) template = template.Splash;
  return AttackHelper.GetAttackEffectsData(
    "Attack/" + type + (splash ? "/Splash" : ""),
    template,
    this.entity
  );
};
...
Attack.prototype.GetBestAttackAgainst = function (target, allowCapture) {
  ...
  let types = this.GetAttackTypes().filter((type) =>
    this.CanAttack(target, [type])
  );

  // Check whether the target is capturable and prefer that when it is allowed.
  let captureIndex = types.indexOf("Capture");
  if (captureIndex != -1) {
    if (allowCapture) return "Capture";
    types.splice(captureIndex, 1);
  }

  let targetClasses = cmpIdentity.GetClassesList();
  let isPreferred = (attackType) =>
    MatchesClassList(targetClasses, this.GetPreferredClasses(attackType));

  return types
    .sort(
      (a, b) =>
        types.indexOf(a) +
        (isPreferred(a) ? types.length : 0) -
        (types.indexOf(b) + (isPreferred(b) ? types.length : 0))
    )
    .pop();
};
...
Attack.prototype.GetAttackName = function (type) {
  return {
    name:
      this.template[type].AttackName._string || this.template[type].AttackName,
...
Attack.prototype.GetTimers = function (type) {
  return {
    prepare: ApplyValueModificationsToEntity(
      "Attack/" + type + "/PrepareTime",
      +(this.template[type].PrepareTime || 0),
      this.entity
    ),
...
Attack.prototype.GetSplashData = function (type) {
  if (!this.template[type].Splash) return undefined;

  return {
    attackData: this.GetAttackEffectsData(type, true),
    friendlyFire: this.template[type].Splash.FriendlyFire == "true",
    radius: ApplyValueModificationsToEntity(
      "Attack/" + type + "/Splash/Range",
      +this.template[type].Splash.Range,
      this.entity
    ),
...
Attack.prototype.StartAttacking = function (target, type, callerIID) {
  if (this.target) this.StopAttacking();

  if (!this.CanAttack(target, [type])) return false;

  let cmpResistance = Engine.QueryInterface(target, IID_Resistance);
  if (!cmpResistance || !cmpResistance.AddAttacker(this.entity)) return false;
...
Attack.prototype.StopAttacking = function (reason) {
  ...
  if (cmpResistance) cmpResistance.RemoveAttacker(this.entity);

  delete this.target;

  let cmpVisual = Engine.QueryInterface(this.entity, IID_Visual);
  if (cmpVisual) cmpVisual.SelectAnimation("idle", false, 1.0);
  ...
  if (reason && callerIID) {
    ...
    if (component) component.ProcessMessage(reason, null);
  }
};
...
Attack.prototype.Attack = function (type, lateness) {
  ...
  Engine.QueryInterface(this.entity, IID_UnitAI)?.FaceTowardsTarget(
    this.target
  );
...
Attack.prototype.PerformAttack = function (type, target) {
  ...
  if (this.template[type].Projectile) {
    ...
    let targetVelocity = Vector3D.sub(
      targetPosition,
      cmpTargetPosition.GetPreviousPosition()
    ).div(turnLength);

    let timeToTarget = PositionHelper.PredictTimeToTarget(
      selfPosition,
      horizSpeed,
      targetPosition,
      targetVelocity
    ...
    let predictedHeight = cmpTargetPosition.GetHeightAt(
      predictedPosition.x,
      predictedPosition.z
    );

    // Add inaccuracy based on spread.
    let distanceModifiedSpread =
      (ApplyValueModificationsToEntity(
        "Attack/" + type + "/Spread",
        +this.template[type].Projectile.Spread,
        this.entity
      ) *
        predictedPosition.horizDistanceTo(selfPosition)) /
      100;
    ...
    data.position = new Vector3D(
      predictedPosition.x + offsetX,
      predictedHeight,
      predictedPosition.z + offsetZ
    );
    ...
    data.direction = Vector3D.sub(data.position, selfPosition).div(
      realHorizDistance
    );

    let actorName = this.template[type].Projectile.ActorName || "";
    let impactActorName = this.template[type].Projectile.ImpactActorName || "";
    let impactAnimationLifetime =
      this.template[type].Projectile.ImpactAnimationLifetime || 0;

    // TODO: Use unit rotation to implement x/z offsets.
    let deltaLaunchPoint = new Vector3D(
      0,
      +this.template[type].Projectile.LaunchPoint["@y"],
      0
    );
    ...
    if (cmpVisual) {
      ...
      if (!actorName) actorName = cmpVisual.GetProjectileActor();
    ...
    data.projectileId = cmpProjectileManager.LaunchProjectileAtPoint(
      launchPoint,
      data.position,
      horizSpeed,
      gravity,
    ...
    data.attackImpactSound = cmpSound
      ? cmpSound.GetSoundGroup("attack_impact_" + type.toLowerCase())
      : "";
  ...
  } else Engine.QueryInterface(SYSTEM_ENTITY, IID_DelayedDamage).Hit(data, 0);
};
...
Attack.prototype.IsTargetInRange = function (target, type) {
  ...
  if (type == "Ranged") {
    ...
    if (!cmpPositionTarget || !cmpPositionTarget.IsInWorld()) return false;

    let cmpPositionSelf = Engine.QueryInterface(this.entity, IID_Position);
    if (!cmpPositionSelf || !cmpPositionSelf.IsInWorld()) return false;

    let positionSelf = cmpPositionSelf.GetPosition();
    let positionTarget = cmpPositionTarget.GetPosition();

    let heightDifference =
      positionSelf.y + range.elevationBonus - positionTarget.y;
    range.max = Math.sqrt(
      Math.square(range.max) + 2 * range.max * heightDifference
    );
...
Attack.prototype.OnValueModification = function (msg) {
  ...
  if (
    this.GetAttackTypes().some(
      (type) => msg.valueNames.indexOf("Attack/" + type + "/MaxRange") != -1
    )
  )
...
Attack.prototype.GetRangeOverlays = function (type = "Ranged") {
  if (!this.template[type] || !this.template[type].RangeOverlay) return [];

...
ResourceTrickle.prototype.ComputeRates = function () {
  ...
  for (let resource in this.template.Rates) {
    let rate = ApplyValueModificationsToEntity(
      "ResourceTrickle/Rates/" + resource,
...
ResourceTrickle.prototype.Trickle = function (data, lateness) {
  ...
  let cmpPlayer =
    QueryOwnerInterface(this.entity) ||
    Engine.QueryInterface(this.entity, IID_Player);
  if (!cmpPlayer) return;
...
ResourceTrickle.prototype.OnValueModification = function (msg) {
  if (msg.component != "ResourceTrickle") return;
...
ResourceTrickle.prototype.CheckTimer = function () {
  ...
  this.trickleInterval = ApplyValueModificationsToEntity(
  ...
  if (this.timer) {
    if (this.trickleInterval == oldTrickleInterval) return;

...
Trigger.prototype.Schema = "<a:component type='system'/><empty/>";
...
Trigger.prototype.Init = function () {
  ...
  this.triggers = {};
  for (const eventName of this.eventNames) this.triggers[eventName] = {};
};

Trigger.prototype.RegisterTriggerPoint = function (ref, ent) {
  if (!this.triggerPoints[ref]) this.triggerPoints[ref] = [];
...
Trigger.prototype.RemoveRegisteredTriggerPoint = function (ref, ent) {
  ...
  if (i == -1) {
    warn(
      "entity " + ent + " wasn't found under the trigger points with ref " + ref
    );
    return;
  }
 ...
 */
Trigger.prototype.RegisterTrigger = function (
  event,
  name,
  triggerData,
  ...
  if (this.triggers[event][name]) {
    warn(
      'Trigger.js: Trigger "' +
        name +
        '" has been registered before. Aborting...'
    );
    return;
  }
  ...
  if (!triggerData.action) triggerData.action = name;

  this.triggers[event][name] = {
  ...
  if (event == "OnRange") {
    ...
    for (const ent of triggerData.entities) {
      ...
      triggerData.queries.push(
        cmpTriggerPoint.RegisterRangeTrigger(name, triggerData)
      );
    }
  }

  if (triggerData.enabled) this.EnableTrigger(event, name);
};
...
Trigger.prototype.DisableTrigger = function (event, name) {
  ...
  if (event == "OnInterval") {
    if (!triggerData.timer)
      // don't disable it a second time
      return;
    ...
    if (!triggerData.queries) {
      warn(
        "Trigger.js: Range query wasn't set up before trying to disable it."
      );
      return;
    }
    const cmpRangeManager = Engine.QueryInterface(
      SYSTEM_ENTITY,
      IID_RangeManager
    );
...
Trigger.prototype.EnableTrigger = function (event, name) {
  ...
  if (event == "OnInterval") {
    if (triggerData.timer)
      // don't enable it a second time
      return;
    if (!triggerData.interval) {
      warn(
        "Trigger.js: An interval trigger should have an intervel in its data"
      );
      return;
    }
    const cmpTimer = Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer);
    triggerData.timer = cmpTimer.SetInterval(
      this.entity,
      IID_Trigger,
      "DoAction",
      triggerData.delay || 0,
      triggerData.interval,
      { name: name }
    );
  } else if (event == "OnRange") {
    if (!triggerData.queries) {
...
Trigger.prototype.OnGlobalConstructionFinished = function (msg) {
  this.CallEvent("OnStructureBuilt", {
    building: msg.newentity,
    foundation: msg.entity,
  });
};
 ...
 */
Trigger.prototype.CallTrigger = function (
  event,
  name,
  eventData,
  evenIfDisabled = true
  ...
  if (!this?.[event]?.[name]) {
    warn(
      `Trigger.js: called a trigger '${name}' for event '${event}' that wasn't found`
    );
    return;
  }
...
Trigger.prototype.DoAction = function (msg) {
  if (this[msg.action])
    ...
    warn(
      "Trigger.js: called a trigger action '" +
        msg.action +
        "' that wasn't found"
    );
};

...
BuildingAI.prototype.SetupRangeQuery = function () {
  ...
  // Remove gaia.
  if (enemies.length && enemies[0] == 0) enemies.shift();
...
BuildingAI.prototype.SetupGaiaRangeQuery = function () {
  ...
  var cmpPlayer = QueryOwnerInterface(this.entity);
  if (!cmpPlayer || !cmpPlayer.IsEnemy(0)) return;
...
BuildingAI.prototype.OnRangeUpdate = function (msg) {
  ...
  for (let entity of msg.added)
    if (cmpAttack.CanAttack(entity)) this.targetUnits.push(entity);
  ...
  if (this.targetUnits.length) this.StartTimer();
};
...
BuildingAI.prototype.StartTimer = function () {
  ...
  this.timer = cmpTimer.SetInterval(
    ...
    attackTimers.repeat / roundCount,
    null
  );
};
...
BuildingAI.prototype.FireArrows = function () {
  ...
  if (this.currentRound > roundCount - 1) this.currentRound = 0;

  if (this.currentRound == 0) this.arrowsLeft = this.GetArrowCount();

  let arrowsToFire = 0;
  if (this.currentRound == roundCount - 1) arrowsToFire = this.arrowsLeft;
  else
  ...
  for (let target of this.targetUnits) addTarget(target);
  ...
  if (!thisCmpPosition.IsInWorld()) return;
  ...
  while (firedArrows < arrowsToFire && targets.length()) {
    ...
    if (
      targetCmpPosition &&
      targetCmpPosition.IsInWorld() &&
      this.CheckTargetVisible(selectedTarget)
    ) {

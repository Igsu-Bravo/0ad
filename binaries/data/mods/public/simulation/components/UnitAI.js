...
UnitAI.prototype.UnitFsmSpec = {
  ...
  FormationLeave: function (msg) {
    ...
    if (this.order && this.order.type === "LeaveFormation") this.FinishOrder();
  },
  ...
  "Order.FormationWalk": function (msg) {
    ...
    if (this.CanPack()) {
      ...
      let cmpControllerAI = Engine.QueryInterface(
        this.GetFormationController(),
        IID_UnitAI
      );
      if (cmpControllerAI.IsIdle()) return this.FinishOrder();
      this.PushOrderFront("Pack", { force: true });
    } else this.SetNextState("FORMATIONMEMBER.WALKING");
    return ACCEPT_ORDER;
  },
  ...
  "Order.LeaveFormation": function () {
    if (!this.IsFormationMember()) return this.FinishOrder();

    let cmpFormation = Engine.QueryInterface(
  ...
  "Order.WalkToTarget": function (msg) {
    ...
    if (this.CheckRange(msg.data)) return this.FinishOrder();
  ...
  "Order.PickupUnit": function (msg) {
    ...
    if (!cmpHolder || cmpHolder.IsFull()) return this.FinishOrder();
    ...
    let cmpPassengerMotion = Engine.QueryInterface(
      msg.data.target,
      IID_UnitMotion
    );
    if (
      cmpPassengerMotion &&
      cmpPassengerMotion.IsTargetRangeReachable(
        this.entity,
        range.min,
        range.max
      ) &&
    ...
    else return this.FinishOrder();
    return ACCEPT_ORDER;
  },

  "Order.Guard": function (msg) {
    if (!this.AddGuard(msg.data.target)) return this.FinishOrder();

    if (this.CheckTargetRangeExplicit(this.isGuardOf, 0, this.guardRange))
      this.SetNextState("INDIVIDUAL.GUARD.GUARDING");
    else if (this.AbleToMove()) this.SetNextState("INDIVIDUAL.GUARD.ESCORTING");
    else return this.FinishOrder();
    return ACCEPT_ORDER;
  },
  ...
  "Order.Heal": function (msg) {
    if (!this.TargetIsAlive(msg.data.target)) return this.FinishOrder();

    // Healers can't heal themselves.
    if (msg.data.target == this.entity) return this.FinishOrder();
  ...
  "Order.Gather": function (msg) {
    let cmpResourceGatherer = Engine.QueryInterface(
      this.entity,
      IID_ResourceGatherer
    );
    if (!cmpResourceGatherer) return this.FinishOrder();
    ...
    this.RememberTargetPosition();
    if (!msg.data.initPos) msg.data.initPos = msg.data.lastPos;
  ...
  "Order.Trade": function (msg) {
    ...
    if (!cmpTrader || !cmpTrader.HasBothMarkets()) return this.FinishOrder();
  ...
  "Order.Garrison": function (msg) {
    ...
    if (
      this.CheckTargetRange(
        msg.data.target,
        msg.data.garrison ? IID_Garrisonable : IID_Turretable
      )
    )
      this.SetNextState("INDIVIDUAL.GARRISON.GARRISONING");
    else this.SetNextState("INDIVIDUAL.GARRISON.APPROACHING");
    return ACCEPT_ORDER;
  },
  ...
  "Order.Pack": function (msg) {
    if (!this.CanPack()) return this.FinishOrder();
    this.SetNextState("INDIVIDUAL.PACKING");
    return ACCEPT_ORDER;
  },

  "Order.Unpack": function (msg) {
    if (!this.CanUnpack()) return this.FinishOrder();
  ...
  FORMATIONCONTROLLER: {
    ...
    "Order.Garrison": function (msg) {
      ...
      } else this.SetNextState("GARRISON.GARRISONING");
      return ACCEPT_ORDER;
    },
    ...
    "Order.Gather": function (msg) {
      ...
      if (!this.CheckTargetRangeExplicit(msg.data.target, 0, 10)) {
        if (
          !this.CanGather(msg.data.target) ||
        ...
        if (!msg.data.secondTry) {
          msg.data.secondTry = true;
          this.PushOrderFront("WalkToTargetRange", {
            target: msg.data.target,
            min: 0,
            max: 10,
          });
    ...
    "Order.GatherNearPosition": function (msg) {
      ...
      if (!this.CheckPointRangeExplicit(msg.data.x, msg.data.z, 0, 20)) {
        ...
        this.PushOrderFront("WalkToPointRange", {
          x: msg.data.x,
          z: msg.data.z,
          min: 0,
          max: 20,
      ...
      this.CallMemberFunction("GatherNearPosition", [
        msg.data.x,
        msg.data.z,
        msg.data.type,
        msg.data.template,
    ...
    "Order.Heal": function (msg) {
      ...
      if (!this.CheckTargetRangeExplicit(msg.data.target, 0, 10)) {
        if (
          !this.TargetIsAlive(msg.data.target) ||
          !this.CheckTargetVisible(msg.data.target)
        )
          return this.FinishOrder();
    ...
    "Order.Repair": function (msg) {
      ...
      this.CallMemberFunction("Repair", [
        msg.data.target,
        msg.data.autocontinue,
        false,
      ]);
    ...
    PATROL: {
      enter: function () {
        ...
        if (!this.patrolStartPosOrder) {
          ...
          this.patrolStartPosOrder.targetClasses =
            this.order.data.targetClasses;
      ...
      PATROLLING: {
        ...
        MovementUpdate: function (msg) {
          if (
            !msg.likelyFailure &&
            !msg.likelySuccess &&
            !this.RelaxedMaxRangeCheck(
              this.order.data,
              this.DefaultRelaxedMaxRange
            )
          )
            return;
      ...
      CHECKINGWAYPOINT: {
        ...
        Timer: function (msg) {
          ...
          if (!this.FindWalkAndFightTargets()) ++this.stopSurveying;
        },
      },
    },
    ...
    GARRISON: {
      APPROACHING: {
        enter: function () {
          if (
            !this.MoveToTargetRange(
              this.order.data.target,
              this.order.data.garrison ? IID_Garrisonable : IID_Turretable
            )
          ) {
            this.FinishOrder();
            return true;
          ...
          let cmpHolder = Engine.QueryInterface(
            this.order.data.target,
            this.order.data.garrison ? IID_GarrisonHolder : IID_TurretHolder
          );
          if (cmpHolder && cmpHolder.CanPickup(this.entity)) {
            this.pickup = this.order.data.target; // temporary, deleted in "leave"
            Engine.PostMessage(this.pickup, MT_PickupRequested, {
              entity: this.entity,
              iid: this.order.data.garrison
                ? IID_GarrisonHolder
                : IID_TurretHolder,
            });
          }
        ...
        leave: function () {
          this.StopMoving();
          if (this.pickup) {
            Engine.PostMessage(this.pickup, MT_PickupCanceled, {
              entity: this.entity,
            });
            delete this.pickup;
          }
      ...
      GARRISONING: {
        enter: function () {
          this.CallMemberFunction(
            this.order.data.garrison ? "Garrison" : "OccupyTurret",
            [this.order.data.target, false]
          );
          // We might have been disbanded due to the lack of members.
          if (
            Engine.QueryInterface(this.entity, IID_Formation).GetMemberCount()
          )
            this.SetNextState("MEMBER");
          return true;
        },
    ...
    FORMING: {
      ...
      MovementUpdate: function (msg) {
        if (!msg.likelyFailure && !this.CheckRange(this.order.data)) return;
    ...
    COMBAT: {
      APPROACHING: {
        ...
        MovementUpdate: function (msg) {
          ...
          if (cmpTargetUnitAI && cmpTargetUnitAI.IsFormationMember())
            target = cmpTargetUnitAI.GetFormationController();
          let cmpAttack = Engine.QueryInterface(this.entity, IID_Attack);
          this.CallMemberFunction("Attack", [
            target,
      ...
      ATTACKING: {
        ...
        leave: function (msg) {
          ...
          if (cmpFormation) cmpFormation.SetRearrange(true);
        },
      },
    },
    ...
    MEMBER: {
      ...
      enter: function (msg) {
        ...
        // what they're doing.
        let cmpFormation = Engine.QueryInterface(this.entity, IID_Formation);
        if (cmpFormation) cmpFormation.SetRearrange(false);
      ...
      Timer: function (msg) {
        let cmpFormation = Engine.QueryInterface(this.entity, IID_Formation);
        if (cmpFormation && !cmpFormation.AreAllMembersWaiting()) return;

        if (this.FinishOrder()) {
          if (this.IsWalkingAndFighting()) this.FindWalkAndFightTargets();
          return;
        }
        return;
      },
      ...
      leave: function (msg) {
        ...
        let cmpFormation = Engine.QueryInterface(this.entity, IID_Formation);
        if (cmpFormation) cmpFormation.MoveMembersIntoFormation(true);
  ...
  FORMATIONMEMBER: {
    ...
    enter: function () {
      ...
      if (cmpFormation) {
        this.formationAnimationVariant =
          cmpFormation.GetFormationAnimationVariant(this.entity);
        if (this.formationAnimationVariant)
          this.SetAnimationVariant(this.formationAnimationVariant);
        else this.SetDefaultAnimationVariant();
      }
      return false;
    },
    ...
    WALKING: {
      enter: function () {
        ...
        cmpUnitMotion.MoveToFormationOffset(
          this.order.data.target,
          this.order.data.x,
          this.order.data.z
        );
    ...
    WALKINGTOPOINT: {
      ...
      MovementUpdate: function () {
        if (!this.CheckRange(this.order.data)) return;
        this.FinishOrder();
      },
    },
  },
  ...
  INDIVIDUAL: {
    ...
    GuardedAttacked: function (msg) {
      ...
      for (var i = 0; i < this.orderQueue.length; ++i) {
        if (this.orderQueue[i].type == "Guard") break;
        if (this.orderQueue[i].data && this.orderQueue[i].data.force) return;
      }
      // if we already are targeting another unit still alive, finish with it first
      if (
        this.order &&
        (this.order.type == "WalkAndFight" || this.order.type == "Attack")
      )
        if (
          this.order.data.target != msg.data.attacker &&
          this.CanAttack(msg.data.attacker)
        )
          return;
      ...
      if (
        ...
        if (this.CanHeal(this.isGuardOf))
          ...
          this.PushOrderFront("Repair", {
            target: this.isGuardOf,
            autocontinue: false,
            force: false,
          });
        return;
      ...
      var cmpBuildingAI = Engine.QueryInterface(
        msg.data.attacker,
        IID_BuildingAI
      );
      ...
      if (this.CheckTargetVisible(msg.data.attacker))
        this.PushOrderFront("Attack", {
          target: msg.data.attacker,
          force: false,
          allowCapture: true,
        });
      else {
        var cmpPosition = Engine.QueryInterface(
          msg.data.attacker,
          IID_Position
        );
        if (!cmpPosition || !cmpPosition.IsInWorld()) return;
        var pos = cmpPosition.GetPosition();
        this.PushOrderFront("WalkAndFight", {
          x: pos.x,
          z: pos.z,
          target: msg.data.attacker,
          force: false,
        ...
        if (this.orderQueue[1] && this.orderQueue[1].type == "WalkAndFight") {
          ...
          Engine.PostMessage(this.entity, MT_UnitAIOrderDataChanged, {
            to: this.GetOrderData(),
          });
        }
      }
    },
    ...
    IDLE: {
      "Order.Cheer": function () {
        ...
        if (!this.cheeringTime || !this.isIdle) return this.FinishOrder();
      ...
      leave: function () {
        let cmpRangeManager = Engine.QueryInterface(
          SYSTEM_ENTITY,
          IID_RangeManager
        );
        ...
        if (this.isIdle) {
          this.isIdle = false;
          Engine.PostMessage(this.entity, MT_UnitIdleChanged, {
            idle: this.isIdle,
          });
        }
      },
      ...
      LosAttackRangeUpdate: function (msg) {
        if (
          ...
          msg.data.added &&
          msg.data.added.length &&
          this.GetStance().targetVisibleEnemies
        )
      ...
      Timer: function (msg) {
        ...
        if (this.IsHealer() && this.FindNewHealTargets()) return;

        // If we entered the idle state we must have nothing better to do,
        // so immediately check whether there's anybody nearby to attack.
        // (If anyone approaches later, it'll be handled via LosAttackRangeUpdate.)
        if (this.FindNewTargets()) return;

        if (this.FindSightedEnemies()) return;

        if (!this.isIdle) {
          // Move back to the held position if we drifted away.
          // (only if not a formation member).
          if (
            !this.IsFormationMember() &&
            this.GetStance().respondHoldGround &&
            this.heldPosition &&
            !this.CheckPointRangeExplicit(
              this.heldPosition.x,
              this.heldPosition.z,
              0,
              10
          ...
          if (this.IsFormationMember()) {
            let cmpFormationAI = Engine.QueryInterface(
              this.formationController,
              IID_UnitAI
            );
            if (!cmpFormationAI || !cmpFormationAI.IsIdle()) return;
          }
        ...
        if (this.template.RoamDistance) this.SetNextState("LINGERING");
      },
      ...
      ROAMING: {
        enter: function () {
          ...
          this.StartTimer(
            randIntInclusive(
              +this.template.RoamTimeMin,
              +this.template.RoamTimeMax
            )
          );
          return false;
        },
      ...
      LINGERING: {
        enter: function () {
          ...
          this.StartTimer(
            randIntInclusive(
              +this.template.FeedTimeMin,
              +this.template.FeedTimeMax
            )
          );
          return false;
        },
    ...
    GUARD: {
      ...
      ESCORTING: {
        ...
        Timer: function (msg) {
          ...
          if (
            cmpObstructionManager.IsInTargetRange(
              this.entity,
              this.isGuardOf,
              0,
              3 * this.guardRange,
        ...
        MovementUpdate: function (msg) {
          if (
            msg.likelyFailure ||
            this.CheckTargetRangeExplicit(this.isGuardOf, 0, this.guardRange)
          )
    ...
    FLEEING: {
      enter: function () {
        ...
        this.order.data.distanceToFlee =
          PositionHelper.DistanceBetweenEntities(
            this.entity,
            this.order.data.target
          ) + +this.template.FleeDistance;
        let cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
        ...
        if (
          ...
          !cmpUnitMotion ||
          !cmpUnitMotion.MoveToTargetRange(
            this.order.data.target,
            this.order.data.distanceToFlee,
            -1
          )
      ...
      Attacked: function (msg) {
        if (msg.data.attacker == this.order.data.target) return;
        ...
        if (
          cmpObstructionManager.DistanceToTarget(this.entity, msg.data.target) >
          cmpObstructionManager.DistanceToTarget(
            this.entity,
            this.order.data.target
          )
        )
    ...
    COMBAT: {
      ...
      leave: function () {
        if (!this.formationAnimationVariant) this.SetDefaultAnimationVariant();
      },

      APPROACHING: {
        enter: function () {
          if (
            !this.MoveToTargetAttackRange(
              this.order.data.target,
              this.order.data.attackType
            )
          ) {
            this.FinishOrder();
            return true;
        ...
        Timer: function (msg) {
          if (
            ...
            if (this.GetStance().respondHoldGround) this.WalkToHeldPosition();
          } else {
            this.RememberTargetPosition();
            if (
              this.order.data.hunting &&
      ...
      ATTACKING: {
        enter: function () {
          ...
          let cmpBuildingAI = Engine.QueryInterface(
            this.entity,
            IID_BuildingAI
          );
          if (cmpBuildingAI) {
          ...
          let cmpUnitAI = Engine.QueryInterface(
            this.order.data.target,
            IID_UnitAI
          );
          ...
          this.shouldCheer =
            cmpUnitAI &&
            (!cmpUnitAI.IsAnimal() || cmpUnitAI.IsDangerousAnimal()) &&
            this.cheeringTime > 0;
        ...
        leave: function () {
          ...
          if (cmpBuildingAI) cmpBuildingAI.SetUnitAITarget(0);
          let cmpAttack = Engine.QueryInterface(this.entity, IID_Attack);
          if (cmpAttack) cmpAttack.StopAttacking();
        },

        OutOfRange: function () {
          if (
            this.ShouldChaseTargetedEntity(
              this.order.data.target,
              this.order.data.force
            )
          ) {
        ...
        Attacked: function (msg) {
          if (
            this.order.data.attackType == "Capture" &&
            (this.GetStance().targetAttackersAlways ||
              !this.order.data.force) &&
            this.order.data.target != msg.data.attacker &&
      ...
      FINDINGNEWTARGET: {
        "Order.Cheer": function () {
          if (!this.cheeringTime) return this.FinishOrder();
        ...
        enter: function () {
          ...
          if (!cmpFormation)
            cmpFormation = Engine.QueryInterface(
              this.order.data.formationTarget || INVALID_ENTITY,
              IID_Formation
            );
          ...
          if (this.FindNewTargets()) return true;

          if (this.GetStance().respondHoldGround) this.WalkToHeldPosition();

          if (this.shouldCheer) {
            this.Cheer();
            this.CallPlayerOwnedEntitiesFunctionInRange(
              "Cheer",
              [],
              this.notifyToCheerInRange
            );
          }
      ...
      CHASING: {
        ...
        enter: function () {
          ...
          var cmpUnitAI = Engine.QueryInterface(
            this.order.data.target,
            IID_UnitAI
          );
          ...
          MovementUpdate: function (msg) {
            ...
            if (
              ...
              (msg.obstructed &&
                this.RelaxedMaxRangeCheck(
                  this.order.data,
                  this.order.data.max + this.DefaultRelaxedMaxRange
                )) ||
    ...
    GATHER: {
      ...
      leave: function () {
        ...
        if (cmpResourceGatherer) cmpResourceGatherer.RemoveFromPlayerCounter();
      ...
      APPROACHING: {
        enter: function () {
          ...
          let cmpSupply = Engine.QueryInterface(
            this.gatheringTarget,
            IID_ResourceSupply
          );
          let cmpMirage = Engine.QueryInterface(
            this.gatheringTarget,
            IID_Mirage
          );
          if (
            ((!cmpMirage || !cmpMirage.Mirages(IID_ResourceSupply)) &&
            ...
            if (
              ...
              lastPos &&
              !this.CheckPositionVisible(lastPos.x, lastPos.z)
            ) {
        ...
        MovementUpdate: function (msg) {
          ...
          if (msg.likelyFailure) this.SetNextState("FINDINGNEWTARGET");
        ...
        leave: function () {
          ...
          if (cmpSupply) cmpSupply.RemoveGatherer(this.entity);
      ...
      GATHERING: {
        enter: function () {
          ...
          if (
            !this.CheckTargetRange(this.order.data.target, IID_ResourceGatherer)
          ) {
            this.ProcessMessage("OutOfRange");
            return true;
          }
          ...
          if (
            !cmpResourceGatherer.StartGathering(
              this.order.data.target,
              IID_UnitAI
            )
          ) {
        ...
        leave: function () {
          ...
          if (cmpResourceGatherer) cmpResourceGatherer.StopGathering();
        },
        ...
        OutOfRange: function (msg) {
          if (
            this.MoveToTargetRange(this.order.data.target, IID_ResourceGatherer)
          )
            this.SetNextState("APPROACHING");
          // Our target is no longer visible - go to its last known position first
          // and then hopefully it will become visible.
          else if (
            !this.CheckTargetVisible(this.order.data.target) &&
            this.order.data.lastPos
          )
            this.PushOrderFront("Walk", {
          ...
          else this.SetNextState("FINDINGNEWTARGET");
        },
      ...
      FINDINGNEWTARGET: {
        enter: function () {
          ...
          if (
            cmpResourceGatherer.IsCarrying(resourceType.generic) &&
            this.orderQueue.length > 1 &&
            this.orderQueue[1] !== "ReturnResource" &&
            (this.orderQueue[1].type !== "Gather" ||
              this.orderQueue[1].data.type.generic !== resourceType.generic)
          ) {
            let nearestDropsite = this.FindNearestDropsite(
              resourceType.generic
            );
            if (nearestDropsite)
              this.orderQueue.splice(1, 0, {
                type: "ReturnResource",
                data: { target: nearestDropsite, force: false },
              });
          }
          ...
          if (this.FinishOrder()) return true;

          // No remaining orders - pick a useful default behaviour

          let cmpPosition = Engine.QueryInterface(this.entity, IID_Position);
          if (!cmpPosition || !cmpPosition.IsInWorld()) return true;
          ...
          let nearbyResource;
          if (!previousForced)
            nearbyResource = this.FindNearbyResource(
              Vector2D.from3D(pos),
              filter
            );
          ...
          if (!initPos) initPos = { x: pos.X, z: pos.Z };
      ...
      RETURNINGRESOURCE: {
        ...
        DROPPINGRESOURCES: {
          enter: function () {
            ...
            } else this.SetNextState("RETURNINGRESOURCE");
    ...
    HEAL: {
      ...
      HEALING: {
        ...
        leave: function () {
          let cmpHeal = Engine.QueryInterface(this.entity, IID_Heal);
          if (cmpHeal) cmpHeal.StopHealing();
        },
        ...
        OutOfRange: function (msg) {
          if (
            ...
            if (this.CanPack()) this.PushOrderFront("Pack", { force: true });
            else this.SetNextState("APPROACHING");
          } else this.SetNextState("FINDINGNEWTARGET");
        },
      ...
      FINDINGNEWTARGET: {
        enter: function () {
          ...
          if (this.FindNewHealTargets()) return true;
    ...
    RETURNRESOURCE: {
      ...
      DROPPINGRESOURCES: {
        enter: function () {
          ...
          let nearby = this.FindNearestDropsite(
            cmpResourceGatherer.GetMainCarryingType()
          );
          this.FinishOrder();
          if (nearby)
            this.PushOrderFront("ReturnResource", {
    ...
    COLLECTTREASURE: {
      ...
      APPROACHING: {
        ...
        MovementUpdate: function (msg) {
          if (
            this.CheckTargetRange(this.order.data.target, IID_TreasureCollector)
          )
            this.SetNextState("COLLECTING");
          else if (msg.likelyFailure) this.SetNextState("FINDINGNEWTARGET");
        },
      },

      COLLECTING: {
        enter: function () {
          let cmpTreasureCollector = Engine.QueryInterface(
            this.entity,
            IID_TreasureCollector
          );
          if (
            !cmpTreasureCollector.StartCollecting(
              this.order.data.target,
              IID_UnitAI
            )
          ) {
        ...
        leave: function () {
          ...
          if (cmpTreasureCollector) cmpTreasureCollector.StopCollecting();
        },
      ...
      FINDINGNEWTARGET: {
        enter: function () {
          ...
          let nearbyTreasure = this.FindNearbyTreasure(
            this.TargetPosOrEntPos(oldTarget)
          );
          if (nearbyTreasure) this.CollectTreasure(nearbyTreasure, true);
    ...
    TRADE: {
      ...
      leave: function () {
        let cmpTrader = Engine.QueryInterface(this.entity, IID_Trader);
        if (cmpTrader) cmpTrader.StopTrading();
      },
      ...
      APPROACHINGMARKET: {
        ...
        MovementUpdate: function (msg) {
          if (
            !msg.likelyFailure &&
            !this.CheckRange(this.order.data.nextTarget, IID_Trader)
          )
            return;
          if (this.waypoints && this.waypoints.length) {
            if (!this.MoveToMarket(this.order.data.target)) this.FinishOrder();
          } else this.SetNextState("TRADING");
        },
      },
      ...
      TradingCanceled: function (msg) {
        if (msg.market != this.order.data.target) return;
        let cmpTrader = Engine.QueryInterface(this.entity, IID_Trader);
        let otherMarket = cmpTrader && cmpTrader.GetFirstMarket();
        if (otherMarket) this.WalkToTarget(otherMarket);
    ...
    REPAIR: {
      ...
      REPAIRING: {
        enter: function () {
          ...
          // If this order was forced, the player probably gave it, but now we've reached the target
          //	switch to an unforced order (can be interrupted by attacks)
          if (this.order.data.force) this.order.data.autoharvest = true;
          ...
          let cmpHealth = Engine.QueryInterface(
            this.order.data.target,
            IID_Health
          );
          if (
            ...
            this.ConstructionFinished({
              entity: this.order.data.target,
              newentity: this.order.data.target,
            });
            return true;
          }
        ...
        leave: function () {
          let cmpBuilder = Engine.QueryInterface(this.entity, IID_Builder);
          if (cmpBuilder) cmpBuilder.StopRepairing();
        },
      ...
      ConstructionFinished: function (msg) {
        if (msg.data.entity != this.order.data.target) return; // ignore other buildings
        ...
        let canReturnResources = this.CanReturnResource(
          msg.data.newentity,
          true,
          cmpResourceGatherer
        );
        if (
          this.CheckTargetRange(msg.data.newentity, IID_Builder) &&
          canReturnResources
        ) {
        ...
        if (
          ...
          let cmpResourceDropsite = Engine.QueryInterface(
            msg.data.newentity,
          ...
          let nearby = this.FindNearbyResource(
            this.TargetPosOrEntPos(msg.data.newentity),
        ...
        if (nearbyFoundation) {
          this.AddOrder(
            "Repair",
            {
              target: nearbyFoundation,
              autocontinue: oldData.autocontinue,
              force: false,
            },
            true
          );
    ...
    GARRISON: {
      APPROACHING: {
        enter: function () {
          if (
            this.order.data.garrison
              ? !this.CanGarrison(this.order.data.target)
        ...
        MovementUpdate: function (msg) {
          if (!msg.likelyFailure && !msg.likelySuccess) return;
          ...
          if (
            ...
            if (this.pickup) {
              ...
              if (
                !cmpUnitAI ||
                (!cmpUnitAI.HasPickupOrder(this.entity) && !cmpUnitAI.IsIdle())
              )
                this.FinishOrder();
            }
          }
      ...
      GARRISONING: {
        enter: function () {
          ...
          if (this.order.data.garrison) {
            let cmpGarrisonable = Engine.QueryInterface(
              this.entity,
              IID_Garrisonable
            );
            ...
            let cmpTurretable = Engine.QueryInterface(
              this.entity,
              IID_Turretable
            );
...
UnitAI.prototype.IsTurret = function () {
  if (!this.isGarrisoned) return false;
...
UnitAI.prototype.IsDangerousAnimal = function () {
  return (
    this.IsAnimal() &&
    this.GetStance().targetVisibleEnemies &&
    !!Engine.QueryInterface(this.entity, IID_Attack)
  );
};
...
UnitAI.prototype.ResetIdle = function () {
  ...
  if (this.isIdle == shouldBeIdle) return;
...
UnitAI.prototype.AbleToMove = function (cmpUnitMotion) {
  if (this.isImmobile || this.IsTurret()) return false;
...
UnitAI.prototype.IsWalkingAndFighting = function () {
  if (this.IsFormationMember()) return false;

  return (
    this.orderQueue.length > 0 &&
    (this.orderQueue[0].type == "WalkAndFight" ||
      this.orderQueue[0].type == "Patrol")
  );
};
...
UnitAI.prototype.OnCreate = function () {
  ...
  else this.UnitFsm.Init(this, "INDIVIDUAL.IDLE");
  this.isIdle = true;
};
...
UnitAI.prototype.OnDiplomacyChanged = function (msg) {
  ...
  if (
    this.isGuardOf &&
    !IsOwnedByMutualAllyOfEntity(this.entity, this.isGuardOf)
  )
    this.RemoveGuard();
};
...
UnitAI.prototype.OnOwnershipChanged = function (msg) {
  ...
  if (msg.to != INVALID_PLAYER && msg.from != INVALID_PLAYER) {
    ...
    if (this.IsPacking()) {
      ...
      if (index != -1)
        this.UnitFsm.SwitchToNextState(
          this,
    ...
    this.SetStance(this.template.DefaultStance);
    if (this.IsTurret()) this.SetTurretStance();
  }
};
...
UnitAI.prototype.OnVisionRangeChanged = function (msg) {
  if (this.entity == msg.entity) this.SetupRangeQueries();
};

UnitAI.prototype.HasPickupOrder = function (entity) {
  return this.orderQueue.some(
    (order) => order.type == "PickupUnit" && order.data.target == entity
  );
};

UnitAI.prototype.OnPickupRequested = function (msg) {
  if (this.HasPickupOrder(msg.entity)) return;
...
UnitAI.prototype.OnPickupCanceled = function (msg) {
  for (let i = 0; i < this.orderQueue.length; ++i) {
    if (
      this.orderQueue[i].type != "PickupUnit" ||
      this.orderQueue[i].data.target != msg.entity
    )
      continue;
    if (i == 0)
      this.UnitFsm.ProcessMessage(this, { type: "PickupCanceled", data: msg });
    else this.orderQueue.splice(i, 1);
    Engine.PostMessage(this.entity, MT_UnitAIOrderDataChanged, {
      to: this.GetOrderData(),
    });
    break;
...
UnitAI.prototype.SetupRangeQueries = function () {
  if (this.GetStance().respondFleeOnSight) this.SetupLOSRangeQuery();

  if (this.IsHealer()) this.SetupHealRangeQuery();
...
UnitAI.prototype.UpdateRangeQueries = function () {
  ...
  if (this.losRangeQuery)
    this.SetupLOSRangeQuery(
      cmpRangeManager.IsActiveQueryEnabled(this.losRangeQuery)
    );

  if (this.losHealRangeQuery)
    this.SetupHealRangeQuery(
      cmpRangeManager.IsActiveQueryEnabled(this.losHealRangeQuery)
    );

  if (this.losAttackRangeQuery)
    this.SetupAttackRangeQuery(
      cmpRangeManager.IsActiveQueryEnabled(this.losAttackRangeQuery)
    );
};
...
UnitAI.prototype.SetupLOSRangeQuery = function (enable = true) {
  ...
  if (enable) cmpRangeManager.EnableActiveQuery(this.losRangeQuery);
};
...
UnitAI.prototype.SetupHealRangeQuery = function (enable = true) {
  ...
  // Do not compensate for entity sizes: LOS doesn't, and UnitAI relies on that.
  this.losHealRangeQuery = cmpRangeManager.CreateActiveQuery(
    this.entity,
    range.min,
    range.max,
    players,
    IID_Health,
    cmpRangeManager.GetEntityFlagMask("injured"),
    false
  );

  if (enable) cmpRangeManager.EnableActiveQuery(this.losHealRangeQuery);
};
...
UnitAI.prototype.SetupAttackRangeQuery = function (enable = true) {
  ...
  // Do not compensate for entity sizes: LOS doesn't, and UnitAI relies on that.
  this.losAttackRangeQuery = cmpRangeManager.CreateActiveQuery(
    this.entity,
    range.min,
    range.max,
    players,
  ...
  if (enable) cmpRangeManager.EnableActiveQuery(this.losAttackRangeQuery);
};
...
UnitAI.prototype.FinishOrder = function () {
  if (!this.orderQueue.length) {
    let stack = new Error().stack.trimRight().replace(/^/gm, "  "); // indent each line
    ...
    error(
      "FinishOrder called for entity " +
        this.entity +
        " (" +
        template +
        ") when order queue is empty\n" +
        stack
    );
  }
  ...
  if (this.IsFormationMember()) {
    ...
    if (cmpUnitAI) {
      ...
      if (cmpUnitAI.GetOrders().length > 1) return true;
    }
  }
  return false;
};
...
UnitAI.prototype.PushOrderAfterForced = function (type, data) {
  if (
    !this.order ||
    ((!this.order.data || !this.order.data.force) && this.order.type != type)
  )
    this.PushOrderFront(type, data);
  else {
    for (let i = 1; i < this.orderQueue.length; ++i) {
      if (this.orderQueue[i].data && this.orderQueue[i].data.force) continue;
      if (this.orderQueue[i].type == type) continue;
...
UnitAI.prototype.WillMoveFromFoundation = function (
  target,
  checkPacking = true
) {
  let cmpUnitAI = Engine.QueryInterface(target, IID_UnitAI);
  if (
    (!IsOwnedByAllyOfEntity(this.entity, target) &&
      cmpUnitAI &&
      !cmpUnitAI.IsAnimal() &&
      !Engine.QueryInterface(
        SYSTEM_ENTITY,
...
UnitAI.prototype.ReplaceOrder = function (type, data) {
  ...
  if (data && data.force) {
    ...
    else this.UpdateWorkOrders(type);
  }
  ...
  if (
    ...
    if (type == "Attack") {
      ...
      packingOrder.type == "Unpack" &&
      g_OrdersCancelUnpacking.has(type)
    ) {
      ...
      this.orderQueue = [];
      this.PushOrder(type, data);
    } else this.orderQueue = [packingOrder, order];
    ...
    if (idx === -1) {
      this.orderQueue = [];
      this.order = undefined;
    } else this.orderQueue.splice(0, idx);
    this.PushOrderFront(type, data);
  } else {
    this.orderQueue = [];
    this.PushOrder(type, data);
...
UnitAI.prototype.GetOrderData = function () {
  ...
  for (let order of this.orderQueue)
    if (order.data) orders.push(clone(order.data));
...
UnitAI.prototype.BackToWork = function () {
  if (this.workOrders.length == 0) return false;

  if (this.isGarrisoned) {
    if (this.IsTurret()) {
      let cmpTurretable = Engine.QueryInterface(this.entity, IID_Turretable);
      if (!cmpTurretable || !cmpTurretable.LeaveTurret()) return false;
    }
    let cmpGarrisonable = Engine.QueryInterface(this.entity, IID_Garrisonable);
    if (!cmpGarrisonable || !cmpGarrisonable.UnGarrison(false)) return false;
  }
  ...
  if (this.IsFormationMember()) {
    var cmpFormation = Engine.QueryInterface(
      this.formationController,
      IID_Formation
    );
    if (cmpFormation) cmpFormation.RemoveMembers([this.entity]);
  }
...
UnitAI.prototype.TimerHandler = function (data, lateness) {
  // Reset the timer
  if (data.timerRepeat === undefined) this.timer = undefined;
...
UnitAI.prototype.OnMotionUpdate = function (msg) {
  if (msg.veryObstructed) msg.obstructed = true;
  this.UnitFsm.ProcessMessage(
    this,
    Object.assign({ type: "MovementUpdate" }, msg)
  );
};
...
UnitAI.prototype.OnGlobalEntityRenamed = function (msg) {
  ...
  for (let i = 0; i < this.orderQueue.length; ++i) {
    ...
    if (order.data && order.data.target && order.data.target == msg.entity) {
      changed = true;
      if (i == 0) currentOrderChanged = true;
      order.data.target = msg.newentity;
    }
    if (
      order.data &&
      order.data.formationTarget &&
      order.data.formationTarget == msg.entity
    ) {
      changed = true;
      if (i == 0) currentOrderChanged = true;
...
UnitAI.prototype.OnAttacked = function (msg) {
  if (msg.fromStatusEffect) return;
...
UnitAI.prototype.OnRangeUpdate = function (msg) {
  if (msg.tag == this.losRangeQuery)
    ...
    this.UnitFsm.ProcessMessage(this, {
      type: "LosHealRangeUpdate",
      data: msg,
    });
  else if (msg.tag == this.losAttackRangeQuery)
    this.UnitFsm.ProcessMessage(this, {
      type: "LosAttackRangeUpdate",
      data: msg,
    });
};
...
UnitAI.prototype.MustKillGatherTarget = function (ent) {
  ...
  if (!cmpResourceSupply) return false;

  if (!cmpResourceSupply.GetKillBeforeGather()) return false;
...
UnitAI.prototype.FindNearbyResource = function (position, filter) {
  ...
  let players = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_PlayerManager
  ).GetAllPlayers();
  ...
  let nearby = cmpRangeManager.ExecuteQueryAroundPos(
    position,
    0,
    range,
    players,
  ...
  return nearby.find((ent) => {
    if (!this.CanGather(ent) || !this.CheckTargetVisible(ent)) return false;

    let template = cmpTemplateManager.GetCurrentTemplateName(ent);
    if (template.indexOf("resource|") != -1) template = template.slice(9);

    let cmpResourceSupply = Engine.QueryInterface(ent, IID_ResourceSupply);
    let type = cmpResourceSupply.GetType();
    return (
      cmpResourceSupply.IsAvailableTo(this.entity) &&
      filter(ent, type, template)
    );
  });
};
...
UnitAI.prototype.FindNearestDropsite = function (genericType) {
  ...
  let players =
    cmpPlayer && cmpPlayer.HasSharedDropsites()
      ...
      : [owner];
  let nearestDropsites = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_RangeManager
  ).ExecuteQuery(this.entity, 0, -1, players, IID_ResourceDropsite, false);

  let isShip = Engine.QueryInterface(this.entity, IID_Identity).HasClass(
    "Ship"
  );
  let cmpObstructionManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
  ...
  for (let dropsite of nearestDropsites) {
    ...
    if (
      isShip &&
      !Engine.QueryInterface(dropsite, IID_Identity).HasClass("Naval")
    )
      continue;
    ...
    if (
      !cmpResourceDropsite.AcceptsType(genericType) ||
      !this.CheckTargetVisible(dropsite)
    )
      continue;
    if (
      Engine.QueryInterface(dropsite, IID_Ownership).GetOwner() != owner &&
...
UnitAI.prototype.FindNearbyFoundation = function (position) {
  ...
  return nearby.find(
    (ent) => !Engine.QueryInterface(ent, IID_Foundation).IsFinished()
  );
};
...
UnitAI.prototype.FindNearbyTreasure = function (position) {
  ...
  if (!cmpTreasureCollector) return undefined;
...
UnitAI.prototype.PlaySound = function (name) {
  if (this.IsFormationController()) {
    ...
    if (member) PlaySound(name, member);
  } else {
...
UnitAI.prototype.SetDefaultAnimationVariant = function () {
  ...
  if (cmpResourceGatherer) {
    ...
    if (type) {
      ...
      if (type.specific == "meat") typename = "carry_" + type.specific;
...
UnitAI.prototype.StopMoving = function () {
  let cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
  if (cmpUnitMotion) cmpUnitMotion.StopMoving();
};
...
UnitAI.prototype.MoveTo = function (data, iid, type) {
  if (data.target) {
    if (data.min || data.max)
      return this.MoveToTargetRangeExplicit(
        data.target,
        data.min || -1,
        data.max || -1
      );
    else if (!iid) return this.MoveToTarget(data.target);

    return this.MoveToTargetRange(data.target, iid, type);
  } else if (data.min || data.max)
    return this.MoveToPointRange(
      data.x,
      data.z,
      data.min || -1,
      data.max || -1
...
UnitAI.prototype.MoveToPoint = function (x, z) {
  let cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
  return (
    this.AbleToMove(cmpUnitMotion) && cmpUnitMotion.MoveToPointRange(x, z, 0, 0)
  ); // For point goals, allow a max range of 0.
};

UnitAI.prototype.MoveToPointRange = function (x, z, rangeMin, rangeMax) {
  let cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
  return (
    this.AbleToMove(cmpUnitMotion) &&
    cmpUnitMotion.MoveToPointRange(x, z, rangeMin, rangeMax)
  );
};

UnitAI.prototype.MoveToTarget = function (target) {
  if (!this.CheckTargetVisible(target)) return false;

  let cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
  return (
    this.AbleToMove(cmpUnitMotion) &&
    cmpUnitMotion.MoveToTargetRange(target, 0, 1)
  );
};
...
UnitAI.prototype.MoveToTargetAttackRange = function (target, type) {
  ...
  if (this.IsFormationMember()) {
    let cmpFormationUnitAI = Engine.QueryInterface(
      this.formationController,
      IID_UnitAI
    );
  ...
  let cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
  if (!this.AbleToMove(cmpUnitMotion)) return false;

  let cmpFormation = Engine.QueryInterface(target, IID_Formation);
  if (cmpFormation) target = cmpFormation.GetClosestMember(this.entity);

  if (type != "Ranged") return this.MoveToTargetRange(target, IID_Attack, type);
  ...
  if (!thisCmpPosition.IsInWorld()) return false;
  let s = thisCmpPosition.GetPosition();

  let targetCmpPosition = Engine.QueryInterface(target, IID_Position);
  if (!targetCmpPosition || !targetCmpPosition.IsInWorld()) return false;
  ...
  let guessedMaxRange =
    parabolicMaxRange > range.max
      ? (range.max + parabolicMaxRange) / 2
      : parabolicMaxRange;

  return (
    cmpUnitMotion &&
    cmpUnitMotion.MoveToTargetRange(target, range.min, guessedMaxRange)
  );
};
...
UnitAI.prototype.MoveToTargetRangeExplicit = function (target, min, max) {
  ...
  return (
    this.AbleToMove(cmpUnitMotion) &&
    cmpUnitMotion.MoveToTargetRange(target, min, max)
  );
};
...
UnitAI.prototype.MoveFormationToTargetAttackRange = function (target) {
  ...
  if (!cmpFormationAttack) return false;
...
UnitAI.prototype.CheckRange = function (data, iid, type) {
  if (data.target) {
    ...
    else if (!iid) return this.CheckTargetRangeExplicit(data.target, 0, 1);

    return this.CheckTargetRange(data.target, iid, type);
  } else if (data.min || data.max)
    return this.CheckPointRangeExplicit(
      data.x,
      data.z,
      data.min || -1,
      data.max || -1
...
UnitAI.prototype.CheckTargetAttackRange = function (target, type) {
  ...
  if (this.IsFormationMember()) {
    ...
    if (
      cmpFormationUnitAI &&
      cmpFormationUnitAI.IsAttackingAsFormation() &&
...
UnitAI.prototype.CheckTargetVisible = function (target) {
  ...
  if (!cmpRangeManager) return false;

  // Entities that are hidden and miraged are considered visible
  var cmpFogging = Engine.QueryInterface(target, IID_Fogging);
  if (cmpFogging && cmpFogging.IsMiraged(cmpOwnership.GetOwner())) return true;

  if (
    cmpRangeManager.GetLosVisibility(target, cmpOwnership.GetOwner()) ==
    "hidden"
  )
    return false;
...
UnitAI.prototype.CheckPositionVisible = function (x, z) {
  ...
  return (
    cmpRangeManager.GetLosVisibilityPosition(x, z, cmpOwnership.GetOwner()) ==
    "visible"
  );
};
...
UnitAI.prototype.FaceTowardsTarget = function (target) {
  let cmpTargetPosition = Engine.QueryInterface(target, IID_Position);
  if (!cmpTargetPosition || !cmpTargetPosition.IsInWorld()) return;
...
UnitAI.prototype.CheckTargetDistanceFromHeldPosition = function (
  target,
  iid,
  type
) {
  ...
  if (!cmpPosition || !cmpPosition.IsInWorld()) return false;

  let cmpVision = Engine.QueryInterface(this.entity, IID_Vision);
  if (!cmpVision) return false;
  let halfvision = cmpVision.GetRange() / 2;

  let pos = cmpPosition.GetPosition();
  let heldPosition = this.heldPosition;
  if (heldPosition === undefined) heldPosition = { x: pos.x, z: pos.z };

  return (
    Math.euclidDistance2D(pos.x, pos.z, heldPosition.x, heldPosition.z) <
    halfvision + range.max
  );
};
...
UnitAI.prototype.AttackEntityInZone = function (ents) {
  var target = ents.find(
    (target) =>
      this.CanAttack(target) &&
      this.CheckTargetDistanceFromHeldPosition(
        target,
        IID_Attack,
        this.GetBestAttackAgainst(target, true)
      ) &&
      (this.GetStance().respondChaseBeyondVision ||
        this.CheckTargetIsInVisionRange(target))
  );
  if (!target) return false;
...
UnitAI.prototype.RespondToTargetedEntities = function (ents) {
  ...
  if (this.GetStance().respondChase) return this.AttackVisibleEntity(ents);

  if (this.GetStance().respondStandGround)
    return this.AttackVisibleEntity(ents);

  if (this.GetStance().respondHoldGround) return this.AttackEntityInZone(ents);

  if (this.GetStance().respondFlee) {
    if (this.order && this.order.type == "Flee") this.orderQueue.shift();
...
UnitAI.prototype.ShouldAbandonChase = function (target, force, iid, type) {
  if (!this.CheckTargetVisible(target)) return true;
  ...
  if (this.isGuardOf) {
    ...
    if (
      ...
      cmpAttack
        .GetAttackTypes()
        .some((type) => cmpUnitAI.CheckTargetAttackRange(this.isGuardOf, type))
    )
      return false;
  }
  ...
  if (!this.GetStance().respondChaseBeyondVision)
    if (!this.CheckTargetIsInVisionRange(target)) return true;
...
UnitAI.prototype.ShouldChaseTargetedEntity = function (target, force) {
  if (!this.AbleToMove()) return false;

  if (this.GetStance().respondChase) return true;
...
UnitAI.prototype.LeaveFormation = function (queued = true) {
  ...
  if (!queued && !this.IsFormationMember()) return;

  if (queued) this.AddOrder("LeaveFormation", { force: true }, queued);
  else this.PushOrderFront("LeaveFormation", { force: true });
};
...
UnitAI.prototype.SetFormationController = function (ent) {
  ...
  if (cmpObstruction) {
    if (ent == INVALID_ENTITY) cmpObstruction.SetControlGroup(this.entity);
    else cmpObstruction.SetControlGroup(ent);
  }

  const cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
  if (cmpUnitMotion) cmpUnitMotion.SetMemberOfFormation(ent);
...
UnitAI.prototype.GetFormationTemplate = function () {
  return (
    Engine.QueryInterface(
      SYSTEM_ENTITY,
      IID_TemplateManager
    ).GetCurrentTemplateName(this.formationController) || NULL_FORMATION
  );
};
...
UnitAI.prototype.GetTargetPositions = function () {
  ...
  for (var i = 0; i < this.orderQueue.length; ++i) {
    ...
    switch (order.type) {
      ...
      case "Repair":
      case "Garrison":
        var cmpTargetPosition = Engine.QueryInterface(
          order.data.target,
          IID_Position
        );
        if (!cmpTargetPosition || !cmpTargetPosition.IsInWorld())
      ...
      default:
        error(
          "GetTargetPositions: Unrecognised order type '" + order.type + "'"
        );
        return [];
    }
  }
...
UnitAI.prototype.ComputeWalkingDistance = function () {
  ...
  var cmpPosition = Engine.QueryInterface(this.entity, IID_Position);
  if (!cmpPosition || !cmpPosition.IsInWorld()) return 0;
...
UnitAI.prototype.AddOrder = function (type, data, queued, pushFront) {
  if (this.expectedRoute) this.expectedRoute = undefined;

  if (pushFront) this.PushOrderFront(type, data);
  else if (queued) this.PushOrder(type, data);
  else this.ReplaceOrder(type, data);
};
...
UnitAI.prototype.RemoveGuard = function () {
  if (!this.isGuardOf) return;

  let cmpGuard = Engine.QueryInterface(this.isGuardOf, IID_Guard);
  if (cmpGuard) cmpGuard.RemoveGuard(this.entity);
  ...
  if (this.order.type == "Guard")
    ...
    for (let i = 1; i < this.orderQueue.length; ++i)
      if (this.orderQueue[i].type == "Guard") this.orderQueue.splice(i, 1);
  Engine.PostMessage(this.entity, MT_UnitAIOrderDataChanged, {
    to: this.GetOrderData(),
  });
};
...
UnitAI.prototype.CanGuard = function () {
  ...
  if (this.IsFormationController()) return true;
...
UnitAI.prototype.Walk = function (x, z, queued, pushFront) {
  ...
  else this.AddOrder("Walk", { x: x, z: z, force: true }, queued, pushFront);
};

/**
 * Adds walk to point range order to queue, forced by the player.
 */
UnitAI.prototype.WalkToPointRange = function (
  x,
  z,
  min,
  max,
  ...
  this.AddOrder(
    "Walk",
    { x: x, z: z, min: min, max: max, force: true },
    queued,
    pushFront
  );
};
 ...
 */
UnitAI.prototype.WalkAndFight = function (
  x,
  z,
  targetClasses,
  allowCapture = true,
...
UnitAI.prototype.Patrol = function (
  x,
  z,
  targetClasses,
  allowCapture = true,
...
UnitAI.prototype.LeaveFoundation = function (target) {
  ...
  if (
    this.order &&
    (this.order.type == "LeaveFoundation" ||
      (this.order.type == "Flee" && this.order.data.target == target))
  )
    return;

  if (
    this.orderQueue.length &&
    this.orderQueue[0].type == "Unpack" &&
    this.WillMoveFromFoundation(target, false)
  ) {
    let cmpPack = Engine.QueryInterface(this.entity, IID_Pack);
    if (cmpPack) cmpPack.CancelPack();
  }
 ...
 */
UnitAI.prototype.Attack = function (
  target,
  allowCapture = true,
  queued = false,
  pushFront = false
  ...
  if (!this.CanAttack(target)) {
    ...
    if (this.IsHealer()) this.MoveToTargetRange(target, IID_Heal);
    else this.WalkToTarget(target, queued, pushFront);
    return;
  }
  ...
  if (
    ...
    if (order.force) this.orderQueue = [this.order];
    return;
  }
...
UnitAI.prototype.Garrison = function (target, queued, pushFront) {
  ...
  this.AddOrder(
    "Garrison",
    { target: target, force: true, garrison: true },
    queued,
    pushFront
  );
};
...
UnitAI.prototype.OccupyTurret = function (target, queued, pushFront) {
  ...
  this.AddOrder(
    "Garrison",
    { target: target, force: true, garrison: false },
    queued,
    pushFront
  );
};
 ...
 */
UnitAI.prototype.PerformGather = function (
  target,
  queued,
  force,
  pushFront = false
  ...
  if (cmpResourceSupply) type = cmpResourceSupply.GetType();
  else error("CanGather allowed gathering from invalid entity");
  ...
  if (
    ...
    if (order.force) {
      ...
      else this.orderQueue = [this.order];
    }
    return;
  }
 ...
 */
UnitAI.prototype.GatherNearPosition = function (
  x,
  z,
  type,
  template,
  ...
  if (
    this.IsFormationController() ||
    Engine.QueryInterface(this.entity, IID_ResourceGatherer)
  )
    this.AddOrder(
      "GatherNearPosition",
      { type: type, template: template, x: x, z: z, force: false },
      queued,
      pushFront
    );
  else this.AddOrder("Walk", { x: x, z: z, force: false }, queued, pushFront);
};
 ...
 * Adds order to collect a treasure to queue, forced by the player.
 */
UnitAI.prototype.CollectTreasureNearPosition = function (
  posX,
  posZ,
  queued,
  pushFront
 ...
 */
UnitAI.prototype.SetupTradeRoute = function (
  target,
  source,
  route,
  queued,
  ...
  if (
    ...
    this.workOrders[0].type == "Trade"
  ) {
    let cmpTrader = Engine.QueryInterface(this.entity, IID_Trader);
    if (
      cmpTrader.HasBothMarkets() &&
      ((cmpTrader.GetFirstMarket() == target &&
        cmpTrader.GetSecondMarket() == source) ||
        (cmpTrader.GetFirstMarket() == source &&
          cmpTrader.GetSecondMarket() == target))
    ) {
      this.BackToWork();
      return;
    }
  ...
  if (cmpTrader.HasBothMarkets()) {
    ...
    if (this.IsFormationController()) {
      ...
      let cmpFormation = Engine.QueryInterface(this.entity, IID_Formation);
      if (cmpFormation) cmpFormation.Disband();
    } else this.AddOrder("Trade", data, queued, pushFront);
  } else {
    if (this.IsFormationController())
      this.CallMemberFunction("WalkToTarget", [
        cmpTrader.GetFirstMarket(),
        queued,
        pushFront,
      ]);
    else this.WalkToTarget(cmpTrader.GetFirstMarket(), queued, pushFront);
    this.expectedRoute = [];
  }
};
...
UnitAI.prototype.SwitchMarketOrder = function (oldMarket, newMarket) {
  if (
    ...
    this.order.data &&
    this.order.data.target &&
    this.order.data.target == oldMarket
  )
...
UnitAI.prototype.MoveToMarket = function (targetMarket) {
  ...
  else nextTarget = { target: targetMarket };
...
UnitAI.prototype.MarketRemoved = function (market) {
  if (
    ...
    this.order.data &&
    this.order.data.target &&
    this.order.data.target == market
  )
    this.UnitFsm.ProcessMessage(this, {
      type: "TradingCanceled",
      market: market,
    });
};
...
UnitAI.prototype.Repair = function (target, autocontinue, queued, pushFront) {
  ...
  this.AddOrder(
    "Repair",
    { target: target, autocontinue: autocontinue, force: true },
    queued,
    pushFront
  );
};
...
UnitAI.prototype.Pack = function (queued, pushFront) {
  if (this.CanPack()) this.AddOrder("Pack", { force: true }, queued, pushFront);
};
...
UnitAI.prototype.SetStance = function (stance) {
  ...
  } else error("UnitAI: Setting to invalid stance '" + stance + "'");
};
...
UnitAI.prototype.SetTurretStance = function () {
  ...
  if (this.GetStance().respondStandGround) return;
  for (let stance in g_Stances) {
    if (!g_Stances[stance].respondStandGround) continue;
...
UnitAI.prototype.FindSightedEnemies = function () {
  if (!this.losRangeQuery) return false;

  let cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);
  return this.RespondToSightedEntities(
    cmpRangeManager.ResetActiveQuery(this.losRangeQuery)
  );
};
...
UnitAI.prototype.FindNewHealTargets = function () {
  if (!this.losHealRangeQuery) return false;

  let cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);
  return this.RespondToHealableEntities(
    cmpRangeManager.ResetActiveQuery(this.losHealRangeQuery)
  );
};
...
UnitAI.prototype.FindNewTargets = function () {
  if (!this.losAttackRangeQuery) return false;

  if (!this.GetStance().targetVisibleEnemies) return false;

  let cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);
  return this.AttackEntitiesByPreference(
    cmpRangeManager.ResetActiveQuery(this.losAttackRangeQuery)
  );
};
...
UnitAI.prototype.FindWalkAndFightTargets = function () {
  if (this.IsFormationController()) {
    ...
    for (let ent of cmpFormation.members) {
      if (!(cmpUnitAI = Engine.QueryInterface(ent, IID_UnitAI))) continue;
      if (cmpUnitAI.FindWalkAndFightTargets()) return true;
    }
    return false;
  }
  ...
  let attackfilter = (e) => {
    ...
    if (cmpOwnership && cmpOwnership.GetOwner() > 0) return true;
  ...
  for (let targ of targets) {
    if (prefs[targ] !== bestPref) continue;
    this.PushOrderFront("Attack", {
      target: targ,
      force: false,
      allowCapture: this?.order?.data?.allowCapture,
  ...
  if (this.IsHealer()) return this.FindNewHealTargets();
...
UnitAI.prototype.GetQueryRange = function (iid) {
  ...
  } else if (this.GetStance().respondChase) ret.max = visionRange;
  ...
  else if (iid === IID_Heal) ret.max = visionRange;
...
UnitAI.prototype.ResetSpeedMultiplier = function () {
  let cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
  if (cmpUnitMotion) cmpUnitMotion.SetSpeedMultiplier(1);
};

UnitAI.prototype.SetSpeedMultiplier = function (speed) {
  let cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
  if (cmpUnitMotion) cmpUnitMotion.SetSpeedMultiplier(speed);
};
...
UnitAI.prototype.RememberTargetPosition = function (orderData) {
  if (!orderData) orderData = this.order.data;
...
UnitAI.prototype.WalkToHeldPosition = function () {
  if (this.heldPosition) {
    this.AddOrder(
      "Walk",
      { x: this.heldPosition.x, z: this.heldPosition.z, force: false },
      false,
      false
    );
    return true;
 ...
 */
UnitAI.prototype.CanReturnResource = function (
  target,
  checkCarriedResource,
  cmpResourceGatherer = undefined
) {
  // Formation controllers should always respond to commands
  // (then the individual units can make up their own minds).
  if (this.IsFormationController()) return true;
  ...
  if (!cmpResourceGatherer)
    cmpResourceGatherer = Engine.QueryInterface(
      this.entity,
      IID_ResourceGatherer
    );
  ...
  return (
    cmpResourceGatherer &&
    cmpResourceGatherer.CanReturnResource(target, checkCarriedResource)
  );
};
...
UnitAI.prototype.IsAttackingAsFormation = function () {
  ...
  return (
    cmpAttack &&
    cmpAttack.CanAttackAsFormation() &&
...
UnitAI.prototype.MoveRandomly = function (distance) {
  ...
  let cmpPosition = Engine.QueryInterface(this.entity, IID_Position);
  let cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
  if (!cmpPosition || !cmpPosition.IsInWorld() || !cmpUnitMotion) return;
  ...
  if (!this.roamAngle) {
    ...
    Math.abs(((ang - this.startAngle + Math.PI) % (2 * Math.PI)) - Math.PI) <
    Math.abs(this.roamAngle / 2)
  )
  ...
  ang += halfDelta;
  cmpUnitMotion.FaceTowardsPoint(
    pos.x + 0.5 * Math.sin(ang),
    pos.z + 0.5 * Math.cos(ang)
  );
...
UnitAI.prototype.SetFacePointAfterMove = function (val) {
  ...
  if (cmpMotion) cmpMotion.SetFacePointAfterMove(val);
};
...
UnitAI.prototype.AttackEntitiesByPreference = function (ents) {
  ...
  let cmpAttack = Engine.QueryInterface(this.entity, IID_Attack);
  if (!cmpAttack) return false;

  let attackfilter = function (e) {
    if (!cmpAttack.CanAttack(e)) return false;
  ...
  let entsWithoutPref = [];
  for (let ent of ents) {
    if (!attackfilter(ent)) continue;
    let pref = cmpAttack.GetPreference(ent);
    if (pref === null || pref === undefined) entsWithoutPref.push(ent);
    else if (!entsByPreferences[pref]) {
      preferences.push(pref);
      entsByPreferences[pref] = [ent];
    } else entsByPreferences[pref].push(ent);
  }

  if (preferences.length) {
    preferences.sort((a, b) => a - b);
    for (let pref of preferences)
      if (this.RespondToTargetedEntities(entsByPreferences[pref])) return true;
  }
...
UnitAI.prototype.CallMemberFunction = function (
  funcname,
  args,
  resetWaitingEntities = true
) {
  var cmpFormation = Engine.QueryInterface(this.entity, IID_Formation);
  if (!cmpFormation) return;

  if (resetWaitingEntities) cmpFormation.ResetWaitingEntities();
 ...
 */
UnitAI.prototype.CallPlayerOwnedEntitiesFunctionInRange = function (
  funcname,
  args,
  range
) {
  let cmpOwnership = Engine.QueryInterface(this.entity, IID_Ownership);
  if (!cmpOwnership) return;
  let owner = cmpOwnership.GetOwner();
  if (owner == INVALID_PLAYER) return;
  let cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);
  let nearby = cmpRangeManager.ExecuteQuery(
    this.entity,
    0,
    range,
    [owner],
...
UnitAI.prototype.TestAllMemberFunction = function (funcname, args) {
  ...
  return (
    cmpFormation &&
    cmpFormation.GetMembers().every((ent) => {
      let cmpUnitAI = Engine.QueryInterface(ent, IID_UnitAI);

...
Turretable.prototype.Schema = "<empty/>";
 ...
 */
Turretable.prototype.OccupyTurret = function (
  target,
  turretPointName = "",
  ejectable = true
) {
  if (!this.CanOccupy(target)) return false;

  let cmpTurretHolder = Engine.QueryInterface(target, IID_TurretHolder);
  if (
    !cmpTurretHolder ||
    !cmpTurretHolder.OccupyNamedTurretPoint(this.entity, turretPointName)
  )
    return false;
  ...
  if (cmpUnitMotion) cmpUnitMotion.SetFacePointAfterMove(false);

  // Remove the unit's obstruction to avoid interfering with pathing.
  let cmpObstruction = Engine.QueryInterface(this.entity, IID_Obstruction);
  if (cmpObstruction) cmpObstruction.SetActive(false);
...
Turretable.prototype.LeaveTurret = function (forced = false) {
  ...
  if (!this.ejectable && !forced) return false;
  ...
  if (
    !cmpTurretHolder ||
    !cmpTurretHolder.LeaveTurretPoint(this.entity, forced)
  )
    return false;

  let cmpUnitMotionEntity = Engine.QueryInterface(this.entity, IID_UnitMotion);
  if (cmpUnitMotionEntity) cmpUnitMotionEntity.SetFacePointAfterMove(true);
  ...
  if (cmpPosition) {
    ...
    if (cmpHolderPosition)
      cmpPosition.SetYRotation(
        cmpHolderPosition.GetPosition().horizAngleTo(pos)
      );
  }
  ...
  let cmpObstruction = Engine.QueryInterface(this.entity, IID_Obstruction);
  if (cmpObstruction) cmpObstruction.SetActive(true);
...
Turretable.prototype.OnOwnershipChanged = function (msg) {
  ...
  if (msg.to == INVALID_PLAYER) this.LeaveTurret(true);

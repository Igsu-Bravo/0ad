...
Garrisonable.prototype.GetRange = function (type, target) {
  ...
  return cmpGarrisonHolder
    ? cmpGarrisonHolder.LoadingRange()
    : { min: 0, max: 1 };
};
...
Garrisonable.prototype.TotalSize = function () {
  ...
  if (cmpGarrisonHolder) size += cmpGarrisonHolder.OccupiedSlots();
  return size;
};
...
Garrisonable.prototype.CanGarrison = function (target) {
  ...
  let cmpGarrisonHolder = Engine.QueryInterface(target, IID_GarrisonHolder);
  return (
    cmpGarrisonHolder && cmpGarrisonHolder.IsAllowedToGarrison(this.entity)
  );
};
...
Garrisonable.prototype.Garrison = function (target) {
  if (!this.CanGarrison(target)) return false;
  ...
  if (cmpUnitAI) cmpUnitAI.SetGarrisoned();

  let cmpPosition = Engine.QueryInterface(this.entity, IID_Position);
  if (cmpPosition) cmpPosition.MoveOutOfWorld();
...
Garrisonable.prototype.UnGarrison = function (forced = false) {
  ...
  if (cmpRallyPoint) cmpRallyPoint.OrderToRallyPoint(this.entity, ["garrison"]);

...
StatusBars.prototype.AddExperienceBar = function (cmpOverlayRenderer, yoffset) {
  if (!this.enabled || !this.showExperience) return 0;

  let cmpPromotion = Engine.QueryInterface(this.entity, IID_Promotion);
  if (
    !cmpPromotion ||
    !cmpPromotion.GetCurrentXp() ||
    !cmpPromotion.GetRequiredXp()
  )
    return 0;
  ...
  return this.AddBar(
    ...
    yoffset,
    "pack",
    cmpPromotion.GetCurrentXp() / cmpPromotion.GetRequiredXp(),
    2 / 3
  );
};
...
StatusBars.prototype.AddUpgradeBar = function (cmpOverlayRenderer, yoffset) {
  ...
  if (!cmpUpgrade || !cmpUpgrade.IsUpgrading()) return 0;

  return this.AddBar(
    cmpOverlayRenderer,
    yoffset,
    "upgrade",
    cmpUpgrade.GetProgress()
  );
};
...
StatusBars.prototype.AddCaptureBar = function (cmpOverlayRenderer, yoffset) {
  ...
  let setCaptureBarPart = function (playerID, startSize) {
    ...
    let size =
      (width * capturePoints[playerID]) / cmpCapturable.GetMaxCapturePoints();
  ...
  for (let i in capturePoints)
    if (i != owner && capturePoints[i] > 0) size = setCaptureBarPart(i, size);

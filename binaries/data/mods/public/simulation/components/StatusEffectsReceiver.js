 ...
 */
StatusEffectsReceiver.prototype.ApplyStatus = function (
  effectData,
  attacker,
  attackerOwner
) {
 ...
 */
StatusEffectsReceiver.prototype.AddStatus = function (
  baseCode,
  data,
  ...
  if (!status.Duration && !status.Interval) return;
  ...
  if (!status.Interval) status._interval = this.DefaultInterval;
  ...
  status._timer = cmpTimer.SetInterval(
    this.entity,
    IID_StatusEffectsReceiver,
    "ExecuteEffect",
    0,
    +(status.Interval || status._interval),
    statusCode
  );
};
 ...
 */
StatusEffectsReceiver.prototype.ExecuteEffect = function (
  statusCode,
  lateness
) {
  let status = this.activeStatusEffects[statusCode];
  ...
  if (status._timeElapsed >= +status.Duration) this.RemoveStatus(statusCode);
};

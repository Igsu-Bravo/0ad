...
TS_ASSERT_UNEVAL_EQUALS(cmpBarter.priceDifferences, {
  wood: 0,
  stone: 0,
  metal: 0,
});
...
TS_ASSERT_UNEVAL_EQUALS(cmpBarter.priceDifferences, {
  wood: -cmpBarter.DIFFERENCE_PER_DEAL,
  stone: cmpBarter.DIFFERENCE_PER_DEAL,
  metal: 0,
});
...
TS_ASSERT_EQUALS(
  bought,
  Math.round(
    (100 * (100 - cmpBarter.CONSTANT_DIFFERENCE + 0)) /
      (100 + cmpBarter.CONSTANT_DIFFERENCE + 0)
  )
);
...
cmpBarter.priceDifferences = {
  wood: 0,
  stone: 99 - cmpBarter.CONSTANT_DIFFERENCE,
  metal: 0,
};
cmpBarter.ExchangeResources(playerID, "wood", "stone", 100);
TS_ASSERT_UNEVAL_EQUALS(cmpBarter.priceDifferences, {
...
TS_ASSERT_EQUALS(
  ...
  Math.round(
    (100 * (100 - cmpBarter.CONSTANT_DIFFERENCE + 0)) /
      (100 + cmpBarter.CONSTANT_DIFFERENCE + 99 - cmpBarter.CONSTANT_DIFFERENCE)
  )
);

cmpBarter.priceDifferences = {
  wood: -99 + cmpBarter.CONSTANT_DIFFERENCE,
  stone: 0,
  metal: 0,
};
cmpBarter.ExchangeResources(playerID, "wood", "stone", 100);
...
cmpBarter.priceDifferences = {
  wood: -cmpBarter.DIFFERENCE_RESTORE,
  stone: -1 - cmpBarter.DIFFERENCE_RESTORE,
  metal: cmpBarter.DIFFERENCE_RESTORE,
};
...
cmpBarter.priceDifferences = {
  wood: -cmpBarter.DIFFERENCE_RESTORE,
  stone: cmpBarter.DIFFERENCE_RESTORE / 2,
  metal: cmpBarter.DIFFERENCE_RESTORE,
};
cmpBarter.ProgressTimeout();
TS_ASSERT_EQUALS(cmpBarter.restoreTimer, undefined);

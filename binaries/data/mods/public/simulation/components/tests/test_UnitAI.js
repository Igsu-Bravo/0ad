...
TestTargetEntityRenaming(
  "INDIVIDUAL.GARRISON.APPROACHING",
  "INDIVIDUAL.IDLE",
...
function TestFormationExiting(mode) {
  ...
  AddMock(SYSTEM_ENTITY, IID_TemplateManager, {
    GetCurrentTemplateName: function (ent) {
      return "special/formations/line_closed";
    },
  });
  ...
  var unitAI = ConstructComponent(unit, "UnitAI", {
    FormationController: "false",
    DefaultStance: "aggressive",
  });
  ...
  AddMock(unit, IID_Attack, {
    ...
    GetFullAttackRange: function () {
      return { max: 40, min: 0 };
    },
    GetBestAttackAgainst: function (t) {
      return "melee";
    },
    GetPreference: function (t) {
      return 0;
    },
    GetTimers: function () {
      return { prepare: 500, repeat: 1000 };
    },
    CanAttack: function (v) {
      return true;
    },
    CompareEntitiesByPreference: function (a, b) {
      return 0;
    },
  ...
  if (mode == 0) TS_ASSERT_EQUALS(unitAI.fsmStateName, "INDIVIDUAL.IDLE");
  else if (mode == 1)
    TS_ASSERT_EQUALS(unitAI.fsmStateName, "INDIVIDUAL.COMBAT.ATTACKING");
  else if (mode == 2) TS_ASSERT_EQUALS(unitAI.fsmStateName, "INDIVIDUAL.IDLE");
  else TS_FAIL("invalid mode");
}
...
function TestMoveIntoFormationWhileAttacking() {
  ...
  for (var i = 0; i < unitCount; i++) {
    ...
    var unitAI = ConstructComponent(unit + i, "UnitAI", {
      FormationController: "false",
      DefaultStance: "aggressive",
    });
  ...
  for (let ent of unitAIs) controllerFormation.SetWaitingOnController(ent);
...
function TestWalkAndFightTargets() {
  ...
  unitAI.PushOrderFront = function (type, order) {
    if (type === "Attack" && order?.target) result = order.target;
  };

  // Create some targets.
  AddMock(ent + 1, IID_UnitAI, {
    IsAnimal: () => true,
    IsDangerousAnimal: () => false,
  });
  ...
  runTest(
    [ent + 1, ent + 2, ent + 7, ent + 6, ent + 3, ent + 4, ent + 5],
    ent + 4
  );

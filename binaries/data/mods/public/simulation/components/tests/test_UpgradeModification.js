...
AddMock(SYSTEM_ENTITY, IID_ModifiersManager, {
  ApplyTemplateModifiers: (valueName, curValue, template, player) => {
    ...
    let mods = isResearched
      ? templateTechModifications.with
      : templateTechModifications.without;
  ...
  ApplyModifiers: (valueName, curValue, ent) => {
    ...
    let mods = isResearched
      ? entityTechModifications.with
      : entityTechModifications.without;
...
TS_ASSERT_UNEVAL_EQUALS(parsed_template.upgrades[0].cost, {
  stone: 100,
  wood: 50,
  time: 100,
});
...
TS_ASSERT_UNEVAL_EQUALS(cmpUpgrade.GetUpgrades()[0].cost, {
  stone: 100,
  wood: 50,
  time: 100,
});

...
function testFoundation(...mocks) {
  ...
  for (let mock of mocks) AddMock(...mock);
  ...
  TS_ASSERT_EQUALS(
    cmpFoundation.CalculateBuildMultiplier(2),
    twoBuilderMultiplier
  );
  TS_ASSERT_EQUALS(
    cmpFoundation.CalculateBuildMultiplier(3),
    threeBuilderMultiplier
  );
  ...
  TS_ASSERT_EQUALS(
    foundationHP,
    1 + work * cmpFoundation.GetBuildRate() * cmpFoundation.buildMultiplier
  );
...
const cmpAutoBuildingFoundation = ConstructComponent(
  foundationEnt2,
  "Foundation",
  {}
);
...
for (let i = 0; i < 10; ++i) {
  ...
  if (i >= 8) TS_ASSERT_EQUALS(currentPercentage, newPercentage);
  // Rate * Max Health / Cost.
  else TS_ASSERT_EQUALS(currentPercentage + 2, newPercentage);
}

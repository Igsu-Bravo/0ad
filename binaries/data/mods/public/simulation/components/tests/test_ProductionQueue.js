...
function testEntitiesList() {
  ...
  let cmpProductionQueue = ConstructComponent(
    productionQueueId,
    "ProductionQueue",
    {
      Entities: {
      ...
      Technologies: {
        _string:
          "gather_fishing_net " + "phase_town_{civ} " + "phase_city_{civ}",
      },
    }
  );
  ...
  TS_ASSERT_UNEVAL_EQUALS(cmpProductionQueue.GetEntitiesList(), [
    "units/iber/cavalry_javelineer_b",
    "units/iber/infantry_swordsman_b",
    "units/iber/support_female_citizen",
  ]);
  TS_ASSERT_UNEVAL_EQUALS(cmpProductionQueue.GetTechnologiesList(), [
    "gather_fishing_net",
    "phase_town_generic",
    "phase_city_generic",
  ]);
...
function test_token_changes() {
  ...
  let cmpProductionQueue = ConstructComponent(10, "ProductionQueue", {
    Entities: { _string: "units/{civ}/a " + "units/{civ}/b" },
    Technologies: { _string: "a " + "b_{civ} " + "c_{civ}" },
    BatchTimeModifier: 1,
  });
  cmpProductionQueue.GetUpgradedTemplate = (template) => template;
  ...
  TS_ASSERT_EQUALS(
    cmpProductionQueue.GetQueue()[1].unitTemplate,
    "units/test/b"
  );
  ...
  Engine.RegisterGlobal("ApplyValueModificationsToEntity", (_, val) => {
    return HandleTokens(
      val,
      "units/{civ}/a>units/{civ}/c units/{civ}/d -units/{civ}/b"
    );
  });

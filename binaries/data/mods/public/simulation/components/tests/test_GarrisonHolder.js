...
let testGarrisonAllowed = function () {
  ...
  TS_ASSERT_EQUALS(
    cmpGarrisonHolder.UnloadTemplate("spart_infantry_archer_a", 2, false),
    true
  );
...
AddMock(siegeEngineId, IID_Garrisonable, {
  UnitSize: () => 1,
  TotalSize: () => 1,
  Garrison: (entity, renamed) =>
    cmpGarrisonHolder.Garrison(siegeEngineId, renamed),
  UnGarrison: () => true,
});
let cavalryId = 46;
...
Engine.RegisterGlobal(
  ...
  (valueName, currentValue, entity) => {
    if (valueName !== "GarrisonHolder/List/_string") return valueName;
...
TS_ASSERT_UNEVAL_EQUALS(cmpGarrisonHolder.GetAllowedClasses().split(/\s+/), [
  "Infantry+Ranged",
  "Cavalry",
  "Trader",
]);
...
Engine.RegisterGlobal(
  "ApplyValueModificationsToEntity",
  oldApplyValueModificationsToEntity
);
TS_ASSERT_UNEVAL_EQUALS(cmpGarrisonHolder.GetEntities(), []);
TS_ASSERT(!cmpGarrisonHolder.Garrison(siegeEngineId));

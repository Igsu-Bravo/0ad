...
function testCapturable(testData, test_function) {
  ...
  TS_ASSERT_EQUALS(
    cmpCapturable.GetRegenRate(),
    testData.regenRate +
      testData.garrisonRegenRate * testData.garrisonedEntities.length
  );
...
testCapturable(testData, (cmpCapturable) => {
  Engine.PostMessage = function (ent, iid, message) {
    TS_ASSERT_UNEVAL_EQUALS(message, {
      regenerating: true,
      regenRate: cmpCapturable.GetRegenRate(),
      territoryDecay: 0,
    });
  };
  cmpCapturable.OnOwnershipChanged({
    from: INVALID_PLAYER,
    to: testData.playerID,
  });
...
testCapturable(testData, (cmpCapturable) => {
  ...
  Engine.PostMessage = function (ent, iid, message) {
    if (iid == MT_CapturePointsChanged)
      TS_ASSERT_UNEVAL_EQUALS(message, {
        capturePoints: [0, 2000 - 100, 0, 1000 + 100],
      });
  ...
  TS_ASSERT_UNEVAL_EQUALS(cmpCapturable.GetCapturePoints(), [
    0,
    2000 - 100,
    0,
    1000 + 100,

...
let cmpModifiersManager = ConstructComponent(
  SYSTEM_ENTITY,
  "ModifiersManager",
  {}
);
...
function SerializationCycle() {
  ...
  cmpModifiersManager = ConstructComponent(
    SYSTEM_ENTITY,
    "ModifiersManager",
    {}
  );
...
cmpModifiersManager.OnGlobalPlayerEntityChanged({
  player: PLAYER_ID_FOR_TEST,
  from: -1,
  to: PLAYER_ENTITY_ID,
});
...
TS_ASSERT_EQUALS(
  ApplyValueModificationsToEntity("Test_A", 5, PLAYER_ENTITY_ID),
  5
);
cmpModifiersManager.AddModifier(
  "Test_A",
  "Test_A_Player",
  [{ affects: ["Player"], add: 3 }],
  PLAYER_ENTITY_ID
);
TS_ASSERT_EQUALS(
  ApplyValueModificationsToEntity("Test_A", 5, PLAYER_ENTITY_ID),
...
TS_ASSERT(cmpModifiersManager.HasAnyModifier("Test_C_3", PLAYER_ENTITY_ID));
TS_ASSERT(
  cmpModifiersManager.HasModifier("Test_C", "Test_C_3", PLAYER_ENTITY_ID)
);
...
AddMock(SYSTEM_ENTITY, IID_PlayerManager, {
  GetPlayerByID: (a) =>
    a == PLAYER_ID_FOR_TEST ? PLAYER_ENTITY_ID : PLAYER_ENTITY_ID + 1,
});
...
cmpModifiersManager.AddModifier(
  "Test_D",
  "Test_D_1",
  [{ affects: ["Structure"], add: 1 }],
  PLAYER_ENTITY_ID + 1
);
cmpModifiersManager.AddModifier(
  "Test_D",
  "Test_D_2",
  [{ affects: ["Structure"], add: 5 }],
  5
);
...
cmpModifiersManager.OnGlobalPlayerEntityChanged({
  player: PLAYER_ID_FOR_TEST + 1,
  from: -1,
  to: PLAYER_ENTITY_ID + 1,
});

TS_ASSERT_EQUALS(ApplyValueModificationsToEntity("Test_D", 10, 5), 25);
cmpModifiersManager.OnGlobalOwnershipChanged({
  entity: 5,
  from: PLAYER_ID_FOR_TEST,
  to: PLAYER_ID_FOR_TEST + 1,
});

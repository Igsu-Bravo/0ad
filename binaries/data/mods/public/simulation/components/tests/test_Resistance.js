...
class testResistance {
  ...
  Reset(schema = {}) {
    this.cmpResistance = ConstructComponent(
      this.EntityID,
      "Resistance",
      schema
    );
  ...
  TestStatusEffectsResistancesApplies() {
    ...
    let cmpStatusEffectsReceiver = AddMock(
      this.EntityID,
      IID_StatusEffectsReceiver,
      {
        ApplyStatus: (effectData, __, ___) => {
          TS_ASSERT_EQUALS(
            effectData[statusName].Duration,
            duration * durationFactor
          );
    ...
    cmpStatusEffectsReceiver = AddMock(
      this.EntityID,
      IID_StatusEffectsReceiver,
      {
        ApplyStatus: (effectData, __, ___) => {
    ...
    cmpStatusEffectsReceiver = AddMock(
      ...
      IID_StatusEffectsReceiver,
      {
        ApplyStatus: (effectData, __, ___) => {
          TS_ASSERT_EQUALS(
            effectData[reducedStatusName].Duration,
            duration * durationFactor
          );
  ...
  TestResistanceAndBonus() {
    ...
    let cmpHealth = AddMock(this.EntityID, IID_Health, {
      TakeDamage: (amount, __, ___) => {
        TS_ASSERT_EQUALS(
          amount,
          damage * bonus * Math.pow(0.9, resistanceValue)
        );
        return { healthChange: -amount };
      },
    });
  ...
  TestMultipleEffects() {
    ...
    let cmpCapturable = AddMock(this.EntityID, IID_Capturable, {
      Capture: (amount, __, ___) => {
        TS_ASSERT_EQUALS(
          amount,
          damage * bonus * Math.pow(0.9, captureResistanceValue)
        );
        return { captureChange: amount };
      },
    });

...
attackComponentTest(undefined, true, (attacker, cmpAttack, defender) => {
  TS_ASSERT_UNEVAL_EQUALS(cmpAttack.GetAttackTypes(), [
    "Melee",
    "Ranged",
    "Capture",
  ]);
  ...
  TS_ASSERT_UNEVAL_EQUALS(
    cmpAttack.GetAttackTypes(["Melee", "Ranged", "Capture"]),
    ["Melee", "Ranged", "Capture"]
  );
  TS_ASSERT_UNEVAL_EQUALS(cmpAttack.GetAttackTypes(["Melee", "Ranged"]), [
    "Melee",
    "Ranged",
  ]);
  ...
  TS_ASSERT_UNEVAL_EQUALS(cmpAttack.GetAttackTypes(["!Melee"]), [
    "Ranged",
    "Capture",
  ]);
  TS_ASSERT_UNEVAL_EQUALS(cmpAttack.GetAttackTypes(["!Melee", "!Ranged"]), [
  ...
  TS_ASSERT_UNEVAL_EQUALS(cmpAttack.GetAttackTypes(["Capture", "!Ranged"]), [
    "Capture",
  ]);
  TS_ASSERT_UNEVAL_EQUALS(
    cmpAttack.GetAttackTypes(["Capture", "Melee", "!Ranged"]),
    ["Melee", "Capture"]
  );

  TS_ASSERT_UNEVAL_EQUALS(cmpAttack.GetPreferredClasses("Melee"), [
    "FemaleCitizen",
  ]);
  TS_ASSERT_UNEVAL_EQUALS(cmpAttack.GetRestrictedClasses("Melee"), [
    "Elephant",
    "Archer",
  ]);
  TS_ASSERT_UNEVAL_EQUALS(cmpAttack.GetFullAttackRange(), { min: 0, max: 80 });
  TS_ASSERT_UNEVAL_EQUALS(cmpAttack.GetAttackEffectsData("Capture"), {
    Capture: 8,
  });
...
for (let className of ["Infantry", "Cavalry"])
  attackComponentTest(className, true, (attacker, cmpAttack, defender) => {
    TS_ASSERT_EQUALS(
      cmpAttack.GetAttackEffectsData("Melee").Bonuses.BonusCav.Multiplier,
      2
    );

    TS_ASSERT_EQUALS(
      cmpAttack.GetAttackEffectsData("Capture").Bonuses || null,
      null
    );

    let getAttackBonus = (s, t, e, splash) =>
      AttackHelper.GetAttackBonus(
        s,
        e,
        t,
        cmpAttack.GetAttackEffectsData(t, splash).Bonuses || null
      );
    TS_ASSERT_UNEVAL_EQUALS(
      getAttackBonus(attacker, "Melee", defender),
      className == "Cavalry" ? 2 : 1
    );
    TS_ASSERT_UNEVAL_EQUALS(getAttackBonus(attacker, "Ranged", defender), 1);
    TS_ASSERT_UNEVAL_EQUALS(
      getAttackBonus(attacker, "Ranged", defender, true),
      className == "Cavalry" ? 3 : 1
    );
...
function testGetBestAttackAgainst(
  defenderClass,
  bestAttack,
  bestAllyAttack,
  isBuilding = false
  ...
  attackComponentTest(defenderClass, true, (attacker, cmpAttack, defender) => {
    ...
    TS_ASSERT_EQUALS(
      cmpAttack.CanAttack(defender, ["Melee", "Capture"]),
      defenderClass != "Archer"
    );
    TS_ASSERT_EQUALS(
      cmpAttack.CanAttack(defender, ["Ranged", "Capture"]),
      true
    );
    TS_ASSERT_EQUALS(
      cmpAttack.CanAttack(defender, ["!Ranged", "!Melee"]),
      isBuilding || defenderClass == "Domestic"
    );
    TS_ASSERT_EQUALS(cmpAttack.CanAttack(defender, ["Melee", "!Melee"]), false);

    let allowCapturing = [true];
    if (!isBuilding) allowCapturing.push(false);

    for (let ac of allowCapturing)
      TS_ASSERT_EQUALS(
        cmpAttack.GetBestAttackAgainst(defender, ac),
        bestAttack
      );
  });
  ...
  attackComponentTest(defenderClass, false, (attacker, cmpAttack, defender) => {
    ...
    TS_ASSERT_EQUALS(
      cmpAttack.CanAttack(defender, ["!Melee"]),
      isBuilding || defenderClass == "Domestic"
    );
    TS_ASSERT_EQUALS(cmpAttack.CanAttack(defender, ["Capture"]), isBuilding);
    TS_ASSERT_EQUALS(

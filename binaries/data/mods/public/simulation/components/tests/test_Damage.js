...
function Test_Generic() {
  ...
  cmpAttack.GetAttackStrengths = (attackType) => ({
    Hack: 0,
    Pierce: 0,
    Crush: damage,
  });
  ...
  TS_ASSERT_UNEVAL_EQUALS(
    AttackHelper.GetPlayersToDamage(atkPlayerEntity, true),
    [0, 1, 2, 3, 4]
  );
  TS_ASSERT_UNEVAL_EQUALS(
    AttackHelper.GetPlayersToDamage(atkPlayerEntity, false),
    [2, 3]
  );
}
...
function TestLinearSplashDamage() {
  ...
  let fallOff = function (x, y) {
    return (
      (1 - (x * x) / (data.radius * data.radius)) *
      (1 - (25 * y * y) / (data.radius * data.radius))
    );
  };

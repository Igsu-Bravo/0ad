...
AIProxy.prototype.Schema = "<empty/>";
...
AIProxy.prototype.GetRepresentation = function () {
  ...
  let ret;
  if (this.needsFullGet) ret = this.GetFullRepresentation();
  else ret = this.changes;
...
AIProxy.prototype.NotifyChange = function () {
  if (this.needsFullGet) {
    ...
    if (!cmpOwnership || cmpOwnership.GetOwner() < 0) return false;
  }
...
AIProxy.prototype.OnGarrisonedUnitsChanged = function (msg) {
  ...
  for (let ent of msg.added)
    this.cmpAIInterface.PushEvent("Garrison", {
      entity: ent,
      holder: this.entity,
    });
...
AIProxy.prototype.GetFullRepresentation = function () {
  ...
  if (cmpResistance) ret.invulnerability = cmpResistance.IsInvulnerable();
  ...
  if (cmpTerritoryDecay) ret.decaying = cmpTerritoryDecay.IsDecaying();

  let cmpCapturable = Engine.QueryInterface(this.entity, IID_Capturable);
  if (cmpCapturable) ret.capturePoints = cmpCapturable.GetCapturePoints();

...
ModifiersManager.prototype.Schema = "<empty/>";
...
ModifiersManager.prototype.Init = function () {
  ...
  this.modifiersStorage._OnItemModified = (prim, sec, itemID) =>
    this.ModifiersChanged.apply(this, [prim, sec, itemID]);
};
...
ModifiersManager.prototype.Deserialize = function (data) {
  ...
  data.players.forEach((player) =>
    this.playerEntitiesCached.set(player, new Map())
  );
};
...
ModifiersManager.prototype.ModifiersChanged = function (propertyName, entity) {
  ...
  if (playerCache) {
    ...
    Engine.PostMessage(entity, MT_ValueModification, {
      entities: [entity],
      component: propertyName.split("/")[0],
      valueNames: [propertyName],
    });
};

ModifiersManager.prototype.SendPlayerModifierMessages = function (
  propertyName,
  player
) {
  ...
  Engine.BroadcastMessage(MT_ValueModification, {
    entities: ents,
    component: propertyName.split("/")[0],
    valueNames: [propertyName],
  });
...
ModifiersManager.prototype.InvalidatePlayerEntCache = function (
  valueCache,
  propertyName,
  entsMap
) {
...
ModifiersManager.prototype.InvalidateCache = function (
  propertyName,
  entity,
  playerCache
) {
  let valueCache = this.cachedValues.get(propertyName);
  if (!valueCache) return;
 ...
 */
ModifiersManager.prototype.FetchModifiedProperty = function (
  classesList,
  propertyName,
  originalValue,
  target
  ...
  if (!modifs.length) return originalValue;
  // Flatten the list of modifications
  let modifications = [];
  modifs.forEach((item) => {
    modifications = modifications.concat(item.value);
  });
 ...
 * @returns originalValue after modifiers
 */
ModifiersManager.prototype.Cache = function (
  classesList,
  propertyName,
  originalValue,
  entity
  ...
  if (!cache2) cache2 = cache.set(entity, new Map()).get(entity);

  let value = this.FetchModifiedProperty(
    classesList,
    propertyName,
    originalValue,
    entity
 ...
 */
ModifiersManager.prototype.ApplyModifiers = function (
  propertyName,
  originalValue,
  entity
) {
  ...
  if (ownerEntity == entity) ownerEntity = null;

  newValue = originalValue;

  let cmpIdentity = QueryMiragedInterface(entity, IID_Identity);
  if (!cmpIdentity) return originalValue;
  ...
  if (ownerEntity) {
    ...
    if (!pc)
      pc = this.playerEntitiesCached
        .get(ownerEntity)
        .set(propertyName, new Set())
        .get(propertyName);
    pc.add(entity);
    newValue = this.FetchModifiedProperty(
      classesList,
      propertyName,
      newValue,
      ownerEntity
 ...
 */
ModifiersManager.prototype.ApplyTemplateModifiers = function (
  propertyName,
  originalValue,
  template,
  player
) {
  if (!template || !template.Identity) return originalValue;

  let cmpPlayerManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_PlayerManager
  );
  return this.FetchModifiedProperty(
    GetIdentityClasses(template.Identity),
    propertyName,
    originalValue,
    cmpPlayerManager.GetPlayerByID(player)
  );
};
...
ModifiersManager.prototype.OnGlobalPlayerEntityChanged = function (msg) {
  ...
  if (msg.from != INVALID_PLAYER && this.playerEntitiesCached.has(msg.from)) {
    this.playerEntitiesCached
      .get(msg.from)
      .forEach((propName) => this.InvalidateCache(propName, msg.from));
...
ModifiersManager.prototype.OnGlobalOwnershipChanged = function (msg) {
  if (msg.from == INVALID_PLAYER || msg.to == INVALID_PLAYER) return;
  ...
  for (let propertyName in playerModifs) {
    ...
    playerModifs[propertyName].forEach((item) =>
      item.value.forEach((modif) => {
        if (!DoesModificationApply(modif, classes)) return;
        if (!modifiedComponents[component]) modifiedComponents[component] = [];
  ...
  for (let component in modifiedComponents)
    Engine.PostMessage(msg.entity, MT_ValueModification, {
      entities: [msg.entity],
      component: component,
      valueNames: modifiedComponents[component],
    });
};
 ...
 */
ModifiersManager.prototype.AddModifier = function (
  propName,
  ModifID,
  Modif,
  entity,
  ...
  return this.modifiersStorage.AddItem(
    propName,
    ModifID,
    Modif,
    entity,
...
ModifiersManager.prototype.AddModifiers = function (
  ModifID,
  Modifs,
  entity,
  stackable = false
...
ModifiersManager.prototype.RemoveModifier = function (
  propName,
  ModifID,
  entity,
  stackable = false
...
ModifiersManager.prototype.RemoveAllModifiers = function (
  ModifID,
  entity,
  stackable = false
) {
...
ModifiersManager.prototype.GetModifiers = function (
  propName,
  entity,
  stackable = false
) {
...
ModifiersManager.prototype.GetAllModifiers = function (
  entity,
  stackable = false
) {

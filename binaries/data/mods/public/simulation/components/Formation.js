...
Formation.prototype.Init = function (deserialized = false) {
  ...
  if (this.template.AnimationVariants) {
    ...
    let differentAnimationVariants =
      this.template.AnimationVariants.split(/\s*;\s*/);
    ...
    for (let rectAnimationVariant of differentAnimationVariants) {
      ...
      [rect, replacementAnimationVariant] =
        rectAnimationVariant.split(/\s*:\s*/);
      let rows, columns;
...
Formation.prototype.Serialize = function () {
  let result = {};
  for (let key of this.variablesToSerialize) result[key] = this[key];
...
Formation.prototype.GetClosestMember = function (ent, filter) {
  ...
  if (!cmpEntPosition || !cmpEntPosition.IsInWorld()) return INVALID_ENTITY;
...
Formation.prototype.GetFormationAnimationVariant = function (entity) {
  if (
    !this.animationvariants ||
    !this.animationvariants.length ||
    this.columnar ||
    !this.memberPositions[entity]
  )
    return undefined;
...
Formation.prototype.SetWaitingOnController = function (ent) {
  ...
  if (
    cmpEntPosition &&
    cmpEntPosition.IsInWorld() &&
    cmpPosition &&
    cmpPosition.IsInWorld()
  )
    cmpEntPosition.TurnTo(cmpPosition.GetRotation().y);

  if (this.waitingOnController.indexOf(ent) !== -1) return;
...
Formation.prototype.UnsetWaitingOnController = function (ent) {
  ...
  if (ind !== -1) this.waitingOnController.splice(ind, 1);
};
...
Formation.prototype.RemoveMembers = function (ents, renamed = false) {
  ...
  this.waitingOnController = this.waitingOnController.filter(
    (ent) => ents.indexOf(ent) === -1
  );
  ...
  for (let ent of this.formationMembersWithAura) {
    ...
    if (ents.indexOf(ent) !== -1) cmpAuras.RemoveFormationAura(this.members);
  }

  this.formationMembersWithAura = this.formationMembersWithAura.filter(
    function (e) {
      return ents.indexOf(e) == -1;
    }
  );
...
Formation.prototype.MoveMembersIntoFormation = function (
  ...
  if (moveCenter || (cmpPosition && !cmpPosition.IsInWorld()))
    this.SetupPositionAndHandleRotation(
      avgpos.x,
      avgpos.y,
...
Formation.prototype.GetAvgFootprint = function (active) {
  ...
  for (let ent of active) {
    ...
    if (cmpFootprint) footprints.push(cmpFootprint.GetShape());
  }
  if (!footprints.length) return { width: 1, depth: 1 };
...
Formation.prototype.ComputeFormationOffsets = function (active, positions) {
  ...
  let sortingClasses;
  if (this.columnar) sortingClasses = ["Cavalry", "Infantry"];
  else sortingClasses = this.sortingClasses.slice();
  ...
  let types = {};
  for (let i = 0; i < sortingClasses.length; ++i) types[sortingClasses[i]] = [];
  ...
  for (let i in active) {
    ...
    if (!done) types.Unknown.push({ ent: active[i], pos: positions[i] });
  }
  ...
  if (this.columnar) {
    ...
    if (this.maxRows && depth > this.maxRows) depth = this.maxRows;
    cols = Math.ceil(count / Math.ceil(depth) + (this.shiftRows ? 0.5 : 0));
    if (cols < this.minColumns) cols = Math.min(count, this.minColumns);
  ...
  if (shape != "special") {
    ...
    while (left > 0) {
      ...
      if (!shiftRows && n > left) n = left;
      for (let c = 0; c < n && left > 0; ++c) {
        // Switch sides for the next entity.
        side *= -1;
        let x;
        if (n % 2 == 0) x = side * (Math.floor(c / 2) + 0.5) * separation.width;
        else x = side * Math.ceil(c / 2) * separation.width;
        if (centerGap) {
  ...
  if (sortingOrder == "fillFromTheSides")
    ...
    offsets.sort(function (o1, o2) {
      return (
        Math.max(Math.abs(o1.x), Math.abs(o1.y)) <
        Math.max(Math.abs(o2.x), Math.abs(o2.y))
      );
    });
  ...
  for (let i = sortingClasses.length; i; --i) {
    ...
    for (let entPos of t) {
      let closestOffsetId = this.TakeClosestOffset(
        entPos,
        usedRealPositions,
        usedOffsets
      );
 ...
 */
Formation.prototype.TakeClosestOffset = function (
  entPos,
  realPositions,
  offsets
) {
  ...
  this.memberPositions[entPos.ent] = {
    row: offsets[closestOffsetId].row,
    column: offsets[closestOffsetId].column,
  };
...
Formation.prototype.GetRealOffsetPositions = function (offsets, pos) {
  ...
  for (let o of offsets)
    offsetPositions.push(
      new Vector2D(pos.x + o.y * sin + o.x * cos, pos.y + o.y * cos - o.x * sin)
    );
...
Formation.prototype.ShapeUpdate = function () {
  ...
  for (let i = this.twinFormations.length - 1; i >= 0; --i) {
    ...
    if (this.twinFormations[i] <= this.entity) continue;
    let cmpPosition = Engine.QueryInterface(this.entity, IID_Position);
    let cmpOtherPosition = Engine.QueryInterface(
      this.twinFormations[i],
      IID_Position
    );
    let cmpOtherFormation = Engine.QueryInterface(
      this.twinFormations[i],
      IID_Formation
    );
    if (
    ...
    let minDist =
      Math.max(thisSize.width / 2, thisSize.depth / 2) +
...
Formation.prototype.OnGlobalOwnershipChanged = function (msg) {
  ...
  if (this.members.indexOf(msg.entity) != -1) this.RemoveMembers([msg.entity]);
};

Formation.prototype.OnGlobalEntityRenamed = function (msg) {
  if (this.members.indexOf(msg.entity) === -1) return;
...
Formation.prototype.DeleteTwinFormations = function () {
  for (let ent of this.twinFormations) {
    ...
    if (cmpFormation)
      cmpFormation.twinFormations.splice(
        cmpFormation.twinFormations.indexOf(this.entity),
        1
      );
  }
  this.twinFormations = [];
...
Formation.prototype.LoadFormation = function (newTemplate) {
  ...
  if (orders.length) cmpNewUnitAI.AddOrders(orders);
  else cmpNewUnitAI.MoveIntoFormation();

  Engine.PostMessage(this.entity, MT_EntityRenamed, {
    entity: this.entity,
    newentity: newFormation,
  });

...
CeasefireManager.prototype.StartCeasefire = function (ceasefireTime) {
  ...
  if (this.stopCeasefireTimer) cmpTimer.CancelTimer(this.stopCeasefireTimer);
  ...
  if (!this.ceasefireIsActive) {
    ...
    for (let i = 1; i < numPlayers; ++i)
      this.diplomacyBeforeCeasefire.push(
        QueryPlayerIDInterface(i).GetDiplomacy()
      );
  ...
  this.stopCeasefireTimer = cmpTimer.SetTimeout(
    SYSTEM_ENTITY,
    IID_CeasefireManager,
    "StopCeasefire",
    this.ceasefireTime
  );
  this.ceasefireCountdownMessageTimer = cmpTimer.SetTimeout(
    SYSTEM_ENTITY,
    IID_CeasefireManager,
    "ShowCeasefireCountdownMessage",
...
CeasefireManager.prototype.StopCeasefire = function () {
  ...
  for (let i = 1; i < numPlayers; ++i)
    QueryPlayerIDInterface(i).SetDiplomacy(
      this.diplomacyBeforeCeasefire[i - 1]
    );

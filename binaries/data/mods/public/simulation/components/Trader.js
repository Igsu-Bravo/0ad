...
Trader.prototype.CalculateGain = function (currentMarket, nextMarket) {
  ...
  let gain =
    cmpMarket &&
    cmpMarket.CalculateTraderGain(nextMarket, this.template, this.entity);
  if (!gain)
    // One of our markets must have been destroyed
    return null;
  ...
  if (garrisonGainMultiplier === undefined) return gain;

  let cmpGarrisonHolder = Engine.QueryInterface(
    this.entity,
    IID_GarrisonHolder
  );
  if (!cmpGarrisonHolder) return gain;
  ...
  for (let entity of cmpGarrisonHolder.GetEntities()) {
    ...
    if (cmpGarrisonedUnitTrader) ++garrisonedTradersCount;
  }
...
Trader.prototype.RemoveTargetMarket = function (target) {
  if (this.markets.length != 1 || this.markets[0] != target) return false;
  let cmpTargetMarket = QueryMiragedInterface(target, IID_Market);
  if (!cmpTargetMarket) return false;
...
Trader.prototype.SetTargetMarket = function (target, source) {
  ...
  if (source) {
    ...
    if (!cmpSourceMarket) return false;
    this.markets = [source];
  }
  if (this.markets.length >= 2) {
    ...
    for (let market of this.markets) {
      ...
      if (cmpMarket) cmpMarket.RemoveTrader(this.entity);
    }
    this.index = 0;
    this.markets = [target];
    ...
    if (target == this.markets[0]) return false;
...
Trader.prototype.GetGarrisonGainMultiplier = function () {
  if (this.template.GarrisonGainMultiplier === undefined) return undefined;
  return ApplyValueModificationsToEntity(
    "Trader/GarrisonGainMultiplier",
    +this.template.GarrisonGainMultiplier,
    this.entity
  );
};
...
Trader.prototype.CanTrade = function (target) {
  ...
  if (cmpTargetFoundation) return false;

  if (
    !(
      cmpTraderIdentity.HasClass("Organic") && cmpTargetMarket.HasType("land")
    ) &&
  ...
  return (
    cmpTraderPlayer &&
    cmpTargetPlayer &&
    !cmpTraderPlayer.IsEnemy(cmpTargetPlayer.GetPlayerID())
  );
};

Trader.prototype.AddResources = function (ent, gain) {
  let cmpPlayer = QueryOwnerInterface(ent);
  if (cmpPlayer) cmpPlayer.AddResource(this.goods.type, gain);
...
Trader.prototype.PerformTrade = function (currentMarket) {
  ...
  if (!cmpPlayer) return INVALID_ENTITY;
...
Trader.prototype.RemoveMarket = function (market) {
  ...
  if (cmpUnitAI) cmpUnitAI.MarketRemoved(market);
};
...
Trader.prototype.SwitchMarket = function (oldMarket, newMarket) {
  ...
  let cmpUnitAI = Engine.QueryInterface(this.entity, IID_UnitAI);
  if (cmpUnitAI) cmpUnitAI.SwitchMarketOrder(oldMarket, newMarket);
};
...
Trader.prototype.GetRange = function () {
  ...
  let max = 1;
  if (cmpObstruction) max += cmpObstruction.GetSize() * 1.5;

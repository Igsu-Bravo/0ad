...
class TurretHolder {
    ...
    for (let point in points)
      this.turretPoints.push({
        ...
        angle: points[point].Angle
          ? (+points[point].Angle * Math.PI) / 180
          : null,
        entity: null,
        template: points[point].Template,
        ejectable:
          "Ejectable" in points[point]
            ? points[point].Ejectable == "true"
            : true,
      });
  }
  ...
  CreateSubunit(turretPointName) {
    ...
    return (
      cmpTurretable?.OccupyTurret(
        this.entity,
        turretPoint.name,
        turretPoint.ejectable
  ...
  SetReservedTurretPoint(name) {
    if (!this.reservedTurrets) this.reservedTurrets = new Set();
  ...
  AllowedToOccupyTurretPoint(entity, turretPoint) {
    if (!turretPoint || turretPoint.entity) return false;

    if (!IsOwnedByMutualAllyOfEntity(entity, this.entity)) return false;

    if (!turretPoint.allowedClasses) return true;

    let cmpIdentity = Engine.QueryInterface(entity, IID_Identity);
    return (
      cmpIdentity &&
      MatchesClassList(cmpIdentity.GetClassesList(), turretPoint.allowedClasses)
    );
  }
   ...
   */
  CanOccupy(entity) {
    return !!this.turretPoints.find((turretPoint) =>
      this.AllowedToOccupyTurretPoint(entity, turretPoint)
    );
  }
  ...
  OccupyTurretPoint(entity, requestedTurretPoint) {
    ...
    if (!cmpPositionOccupant) return false;

    let cmpPositionSelf = Engine.QueryInterface(this.entity, IID_Position);
    if (!cmpPositionSelf) return false;

    if (this.OccupiesTurretPoint(entity)) return false;
    ...
    if (requestedTurretPoint) {
      ...
      turretPoint = this.turretPoints.find(
        (turret) =>
          !turret.entity && this.AllowedToOccupyTurretPoint(entity, turret)
      );

    if (!turretPoint) return false;
    ...
    if (!turretPoint && turretPoint.angle != null)
      cmpPositionOccupant.SetYRotation(
        cmpPositionSelf.GetRotation().y + turretPoint.angle
      );
    else if (!turretPoint && !cmpPosition.IsInWorld())
      cmpPositionOccupant.SetYRotation(
        cmpPositionSelf.GetRotation().y + Math.PI
      );
  ...
  LeaveTurretPoint(entity, forced, requestedTurretPoint) {
    ...
    if (requestedTurretPoint) {
      if (requestedTurretPoint.entity == entity)
        turretPoint = requestedTurretPoint;
    } else turretPoint = this.GetOccupiedTurretPoint(entity);

    if (!turretPoint || (!turretPoint.ejectable && !forced)) return false;
  ...
  OccupiesTurretPoint(entity, requestedTurretPoint) {
    return requestedTurretPoint
      ? requestedTurretPoint.entity == entity
  ...
  GetOccupiedTurretPoint(entity) {
    return this.turretPoints.find(
      (turretPoint) => turretPoint.entity == entity
    );
  }
  ...
  GetEntities() {
    ...
    for (let turretPoint of this.turretPoints)
      if (turretPoint.entity) entities.push(turretPoint.entity);
    return entities;
  }
  ...
  IsFull() {
    return !!this.turretPoints.find(
      (turretPoint) => turretPoint.entity == null
    );
  }
  ...
  SetInitEntity(turretName, entity) {
    if (!this.initTurrets) this.initTurrets = new Map();
  ...
  OnGlobalSkirmishReplacerReplaced(msg) {
    ...
    if (msg.entity == this.entity) {
      ...
      if (cmpTurretHolder) cmpTurretHolder.initTurrets = this.initTurrets;
    } else {
      let entityIndex = this.initTurrets.indexOf(msg.entity);
      if (entityIndex != -1) this.initTurrets[entityIndex] = msg.newentity;
    }
  }
    ...
    for (let [turretPointName, entity] of this.initTurrets) {
      ...
      if (
        ...
        !cmpTurretable.OccupyTurret(
          this.entity,
          turretPointName,
          this.TurretPointByName(turretPointName).ejectable
        )
      )
        warn(
          "Entity " +
            entity +
            " could not occupy the turret point " +
            turretPointName +
            " of turret holder " +
            this.entity +
            "."
  ...
  OnOwnershipChanged(msg) {
    ...
    for (let point of this.turretPoints) {
      ...
      if (!point.entity) continue;
      if (!point.ejectable) {
        let cmpTurretOwnership = Engine.QueryInterface(
          point.entity,
          IID_Ownership
        );
        if (cmpTurretOwnership) cmpTurretOwnership.SetOwner(msg.to);

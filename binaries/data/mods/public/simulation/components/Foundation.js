...
Foundation.prototype.InitialiseConstruction = function (template) {
  ...
  if (!cmpCost)
    error(
      "A foundation, from " +
        template +
        ", must have a cost component to know the build time"
    );
...
Foundation.prototype.OnHealthChanged = function (msg) {
  ...
  if (cmpPosition) cmpPosition.SetConstructionProgress(this.GetBuildProgress());

  Engine.PostMessage(this.entity, MT_FoundationProgressChanged, {
    to: this.GetBuildPercentage(),
  });
};
...
Foundation.prototype.OnOwnershipChanged = function (msg) {
  if (msg.to != INVALID_PLAYER && this.previewEntity != INVALID_ENTITY) {
    let cmpPreviewOwnership = Engine.QueryInterface(
      this.previewEntity,
      IID_Ownership
    );
    if (cmpPreviewOwnership) cmpPreviewOwnership.SetOwner(msg.to);
    return;
  }

  if (msg.to != INVALID_PLAYER || !this.initialised) return;
  ...
  for (let r in this.costs) {
    ...
    if (scaled) {
      if (cmpPlayer) cmpPlayer.AddResource(r, scaled);
...
Foundation.prototype.AddBuilders = function (builders) {
  ...
  if (changed) this.HandleBuildersChanged();
};
...
Foundation.prototype.AddBuilderHelper = function (builderEnt) {
  if (this.builders.has(builderEnt)) return false;

  let cmpBuilder =
    Engine.QueryInterface(builderEnt, IID_Builder) ||
    Engine.QueryInterface(this.entity, IID_AutoBuildable);
  if (!cmpBuilder) return false;
...
Foundation.prototype.RemoveBuilder = function (builderEnt) {
  if (!this.builders.has(builderEnt)) return;
...
Foundation.prototype.HandleBuildersChanged = function () {
  ...
  if (cmpVisual) cmpVisual.SetVariable("numbuilders", this.GetNumBuilders());

  Engine.PostMessage(this.entity, MT_FoundationBuildersChanged, {
    to: this.GetBuilders(),
  });
};
...
Foundation.prototype.GetBuildTime = function () {
  let timeLeft =
    (1 - this.GetBuildProgress()) *
    Engine.QueryInterface(this.entity, IID_Cost).GetBuildTime();
  let rate = this.totalBuilderRate * this.buildMultiplier;
  let rateNew =
    (this.totalBuilderRate + 1) *
    this.CalculateBuildMultiplier(this.GetNumBuilders() + 1);
  return {
    // Avoid division by zero, in particular 0/0 = NaN which isn't reliably serialized
  ...
  if (cmpObstruction && cmpObstruction.GetBlockMovementFlag(true)) {
    ...
    if (collisions.length) {
      for (let ent of collisions) {
        ...
        if (cmpUnitAI) cmpUnitAI.LeaveFoundation(this.entity);
...
Foundation.prototype.Build = function (builderEnt, work) {
  ...
  if (!this.committed && !this.Commit()) return;
  ...
  if (deltaHP > 0) cmpHealth.Increase(deltaHP);
  ...
  if (this.maxProgress >= 1.0) {
    let cmpPlayerStatisticsTracker = QueryOwnerInterface(
      this.entity,
      IID_StatisticsTracker
    );
    ...
    for (let builder of this.GetBuilders()) {
      ...
      if (cmpUnitAIBuilder)
        cmpUnitAIBuilder.ConstructionFinished({
          entity: this.entity,
          newentity: building,
        });
    }
...
Foundation.prototype.CreateConstructionPreview = function () {
  ...
  this.previewEntity = Engine.AddLocalEntity(
    "construction|" + this.finalTemplateName
  );
  let cmpFoundationOwnership = Engine.QueryInterface(
    this.entity,
    IID_Ownership
  );
  let cmpPreviewOwnership = Engine.QueryInterface(
  ...
  let cmpPreviewPosition = Engine.QueryInterface(
    this.previewEntity,
    IID_Position
  );
...
Foundation.prototype.OnEntityRenamed = function (msg) {
  ...
  if (cmpFoundationNew) cmpFoundationNew.AddBuilders(this.GetBuilders());
};
...
FoundationMirage.prototype.GetNumBuilders = function () {
  return this.numBuilders;
};
FoundationMirage.prototype.GetBuildTime = function () {

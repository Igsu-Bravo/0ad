 ...
 */
StatisticsTracker.prototype.CounterIncrement = function (
  cmpIdentity,
  counter,
  type
) {
  ...
  if (classes.indexOf(type) != -1) ++this[counter][type];
};
...
StatisticsTracker.prototype.IncreaseTrainedUnitsCounter = function (
  ...
  if (!cmpUnitEntityIdentity) return;
  ...
  if (!cmpUnitEntityIdentity.HasClass("Domestic")) ++this.unitsTrained.total;
...
StatisticsTracker.prototype.IncreaseConstructedBuildingsCounter = function (
  ...
  var cmpBuildingEntityIdentity = Engine.QueryInterface(
    constructedBuilding,
    IID_Identity
  );

  if (!cmpBuildingEntityIdentity) return;
...
StatisticsTracker.prototype.KilledEntity = function (targetEntity) {
  var cmpTargetEntityIdentity = Engine.QueryInterface(
    targetEntity,
    IID_Identity
  );
  if (!cmpTargetEntityIdentity) return;
  ...
  if (
    cmpTargetEntityIdentity.HasClass("Unit") &&
    !cmpTargetEntityIdentity.HasClass("Animal")
  ) {
    for (let type of this.unitsClasses)
    ...
    if (costs)
      for (let type in costs) this.enemyUnitsKilledValue += costs[type];
  }
  ...
  if (cmpTargetEntityIdentity.HasClass("Structure") && !cmpFoundation) {
    ...
    if (costs)
      for (let type in costs) this.enemyBuildingsDestroyedValue += costs[type];
  }
};
...
StatisticsTracker.prototype.LostEntity = function (lostEntity) {
  ...
  if (!cmpLostEntityIdentity) return;
  ...
  if (
    ...
    !cmpLostEntityIdentity.HasClass("Domestic")
  ) {
    for (let type of this.unitsClasses)
    ...
    if (costs) for (let type in costs) this.unitsLostValue += costs[type];
  }
  ...
  if (cmpLostEntityIdentity.HasClass("Structure") && !cmpFoundation) {
    ...
    if (costs) for (let type in costs) this.buildingsLostValue += costs[type];
  }
};

StatisticsTracker.prototype.CapturedEntity = function (capturedEntity) {
  let cmpCapturedEntityIdentity = Engine.QueryInterface(
    capturedEntity,
    IID_Identity
  );
  if (!cmpCapturedEntityIdentity) return;
  ...
  if (cmpCapturedEntityIdentity.HasClass("Unit")) {
    ...
    if (costs) for (let type in costs) this.unitsCapturedValue += costs[type];
  }

  if (cmpCapturedEntityIdentity.HasClass("Structure")) {
    for (let type of this.buildingsClasses)
      this.CounterIncrement(
        cmpCapturedEntityIdentity,
        "buildingsCaptured",
        type
      );
    ...
    if (costs)
      for (let type in costs) this.buildingsCapturedValue += costs[type];
  }
};
 ...
 */
StatisticsTracker.prototype.IncreaseResourceGatheredCounter = function (
  type,
  amount,
  specificType
) {
 ...
 */
StatisticsTracker.prototype.IncreaseResourceUsedCounter = function (
  type,
  amount
) {
...
StatisticsTracker.prototype.IncreaseResourcesSoldCounter = function (
  type,
  amount
) {
...
StatisticsTracker.prototype.IncreaseResourcesBoughtCounter = function (
  type,
  amount
) {
...
StatisticsTracker.prototype.IncreaseTributesReceivedCounter = function (
  amount
) {
...
StatisticsTracker.prototype.GetPercentMapExplored = function () {
  let cmpPlayer = Engine.QueryInterface(this.entity, IID_Player);
  if (!cmpPlayer) return 0;

  return Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_RangeManager
  ).GetPercentMapExplored(cmpPlayer.GetPlayerID());
};
...
StatisticsTracker.prototype.GetTeamPercentMapExplored = function () {
  ...
  for (let i = 1; i < numPlayers; ++i) {
    ...
    if (cmpOtherPlayer && cmpOtherPlayer.GetTeam() == team) teamPlayers.push(i);
  }
...
StatisticsTracker.prototype.GetPercentMapControlled = function () {
  ...
  ).GetTerritoryPercentage(cmpPlayer.GetPlayerID());
};
...
StatisticsTracker.prototype.UpdatePeakPercentages = function () {
  this.peakPercentMapControlled = Math.max(
...
StatisticsTracker.prototype.PushValue = function (fromData, toData) {
  if (typeof fromData == "object")
    for (let prop in fromData) {
      if (typeof toData[prop] != "object") toData[prop] = [fromData[prop]];
      else this.PushValue(fromData[prop], toData[prop]);
    }
  else toData.push(fromData);
};

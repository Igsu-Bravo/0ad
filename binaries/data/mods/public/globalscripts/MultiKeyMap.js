...
MultiKeyMap.prototype.Deserialize = function (data) {
  for (let primary in data) {
    ...
    for (let secondary in data[primary][1])
      this.items
        .get(data[primary][0])
        .set(data[primary][1][secondary][0], data[primary][1][secondary][1]);
  }
};
 ...
 */
MultiKeyMap.prototype.AddItem = function (
  primaryKey,
  itemID,
  item,
  secondaryKey,
 ...
 * @returns true if the items list changed in such a way that cached values are possibly invalidated.
 */
MultiKeyMap.prototype.AddItems = function (
  itemID,
  items,
  secondaryKey,
  stackable = false
 ...
 * @returns true if the items list changed in such a way that cached values are possibly invalidated.
 */
MultiKeyMap.prototype.RemoveItem = function (
  primaryKey,
  itemID,
  secondaryKey,
  stackable = false
 ...
 * @returns true if the items list changed in such a way that cached values are possibly invalidated.
 */
MultiKeyMap.prototype.RemoveAllItems = function (
  itemID,
  secondaryKey,
  stackable = false
) {
  ...
  for (let primaryKey of this.items.keys())
    modified =
      this.RemoveItem(primaryKey, itemID, secondaryKey, stackable) || modified;
  return modified;
};
...
MultiKeyMap.prototype.HasItem = function (primaryKey, itemID, secondaryKey) {
  ...
  return this._getItems(primaryKey, secondaryKey).some(
...
MultiKeyMap.prototype.HasAnyItem = function (itemID, secondaryKey) {
  ...
  for (let primaryKey of this.items.keys())
    if (this.HasItem(primaryKey, itemID, secondaryKey)) return true;
  return false;
};
...
MultiKeyMap.prototype.GetAllItems = function (secondaryKey) {
  ...
  for (let primaryKey of this.items.keys()) {
    if (!this.items.get(primaryKey).has(secondaryKey)) continue;
...
MultiKeyMap.prototype._getItems = function (primaryKey, secondaryKey) {
  ...
  if (cache) cache = cache.get(secondaryKey);
  return cache ? cache : [];
};
...
MultiKeyMap.prototype._getItemsOrInit = function (primaryKey, secondaryKey) {
  let cache = this.items.get(primaryKey);
  if (!cache) cache = this.items.set(primaryKey, new Map()).get(primaryKey);

  let cache2 = cache.get(secondaryKey);
  if (!cache2) cache2 = cache.set(secondaryKey, []).get(secondaryKey);
  return cache2;
};
 ...
 * @returns true if the items list changed in such a way that cached values are possibly invalidated.
 */
MultiKeyMap.prototype._AddItem = function (
  primaryKey,
  itemID,
  item,
  secondaryKey,
 ...
 * @returns true if the items list changed in such a way that cached values are possibly invalidated.
 */
MultiKeyMap.prototype._RemoveItem = function (
  primaryKey,
  itemID,
  secondaryKey,
  stackable
  ...
  let existingItem = items.filter((item) => {
    return item._ID == itemID;
  });
  if (!existingItem.length) return false;

  if (--existingItem[0]._count > 0) return stackable;
  ...
  if (!stilValidItems.length) {
    ...
    if (!this.items.get(primaryKey).size) this.items.delete(primaryKey);
    return true;
  }
 ...
 */
MultiKeyMap.prototype._OnItemModified = function (
  primaryKey,
  secondaryKey,
  itemID
) {};

...
function LoadModificationTemplates() {
  ...
  global.TechnologyTemplates = new ModificationTemplates(
...
function DeriveModificationsFromTechnologies(techsDataArray) {
  if (!techsDataArray.length) return {};

  let derivedModifiers = {};
  for (let technology of techsDataArray) {
    // Auras don't have a "reqs" property
    if ("reqs" in technology && !technology.reqs) continue;

    let modifiers = DeriveModificationsFromTech(technology);
    for (let modPath in modifiers) {
      if (!derivedModifiers[modPath]) derivedModifiers[modPath] = [];
      derivedModifiers[modPath] = derivedModifiers[modPath].concat(
        modifiers[modPath]
      );
    }
  }
...
function DeriveModificationFromXMLTemplate(template) {
  let effect = {};
  if (template.Add) effect.add = +template.Add;
  if (template.Multiply) effect.multiply = +template.Multiply;
  if (template.Replace) effect.replace = template.Replace;

...
function GetIdentityClasses(template) {
  ...
  if (template.Rank) classString += " " + template.Rank;
...
function GetVisibleIdentityClasses(template) {
  return template.VisibleClasses && template.VisibleClasses._string
...
function GetTemplateDataHelper(
    ...
    if (template.Resistance.Entity) {
      if (template.Resistance.Entity.Damage) {
        ...
        for (let damageType in template.Resistance.Entity.Damage)
          ret.resistance.Damage[damageType] = getEntityValue(
      ...
      if (template.Resistance.Entity.ApplyStatus) {
        ...
        for (let statusEffect in template.Resistance.Entity.ApplyStatus)
          ret.resistance.ApplyStatus[statusEffect] = {
            blockChance: getEntityValue(
              "Resistance/Entity/ApplyStatus/" + statusEffect + "/BlockChance"
            ),
            duration: getEntityValue(
              "Resistance/Entity/ApplyStatus/" + statusEffect + "/Duration"
            ),
          };
      }
    }
  ...
  let getAttackEffects = (temp, path) => {
    let effects = {};
    if (temp.Capture) effects.Capture = getEntityValue(path + "/Capture");

    if (temp.Damage) {
      effects.Damage = {};
      for (let damageType in temp.Damage)
        effects.Damage[damageType] = getEntityValue(
          path + "/Damage/" + damageType
        );
    }

    if (temp.ApplyStatus) effects.ApplyStatus = temp.ApplyStatus;
  ...
  if (template.Attack) {
    ...
    for (let type in template.Attack) {
      ...
      ret.attack[type] = {
        attackName: {
          name:
            template.Attack[type].AttackName._string ||
            template.Attack[type].AttackName,
      ...
      if (template.Attack[type].Projectile)
        ret.attack[type].friendlyFire =
          template.Attack[type].Projectile.FriendlyFire == "true";

      Object.assign(
        ret.attack[type],
        getAttackEffects(template.Attack[type], "Attack/" + type)
      );
  ...
  if (template.DeathDamage) {
    ...
    Object.assign(
      ret.deathDamage,
      getAttackEffects(template.DeathDamage, "DeathDamage")
    );
  }
  ...
  if (template.TrainingRestrictions) {
    ...
    if (template.TrainingRestrictions.MatchLimit)
      ret.trainingRestrictions.matchLimit =
  ...
  if (template.Treasure) {
    ...
    for (let resource in template.Treasure.Resources)
      ret.treasure.resources[resource] = getEntityValue(
  ...
  if (template.WallSet) {
    ret.wallSet = {
      templates: {
        ...
        fort:
          ...
          "structures/" + template.Identity.Civ + "/fortress",
    ...
    if (template.WallSet.Templates.WallCurves)
      ret.wallSet.templates.curves =
        template.WallSet.Templates.WallCurves.split(/\s+/);
  }

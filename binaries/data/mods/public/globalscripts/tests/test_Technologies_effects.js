...
function test_numeric() {
  ...
  let add_mul_add = [
    { add: 10, affects: "Unit" },
    { multiply: 2, affects: "Unit" },
    { add: 5, affects: "Unit" },
  ];

  let add_replace = [
    { add: 10, affects: "Unit" },
    { replace: 10, affects: "Unit" },
  ];
...
function test_non_numeric() {
  ...
  TS_ASSERT_EQUALS(
    GetTechModifiedProperty(replace_nonnum, "Unit", "beta"),
    "alpha"
  );
  TS_ASSERT_EQUALS(
    GetTechModifiedProperty(replace_nonnum, "Structure", "beta"),
    "beta"
  );

  let replace_tokens = [
    { tokens: "-beta alpha gamma -delta", affects: "Unit" },
  ];
  TS_ASSERT_EQUALS(
    GetTechModifiedProperty(replace_tokens, "Unit", "beta"),
    "alpha gamma"
  );
  TS_ASSERT_EQUALS(
    GetTechModifiedProperty(replace_tokens, "Structure", "beta"),
    "beta"
  );

  let replace_tokens_2 = [{ tokens: "beta>gamma -delta", affects: "Unit" }];
  TS_ASSERT_EQUALS(
    GetTechModifiedProperty(replace_tokens_2, "Unit", "beta"),
    "gamma"
  );
  TS_ASSERT_EQUALS(
    GetTechModifiedProperty(replace_tokens_2, "Structure", "beta"),
    "beta"
  );
  ...
  TS_ASSERT_EQUALS(
    GetTechModifiedProperty(replace_tokens_3, "Unit", "beta"),
    "zeta delta"
  );
  TS_ASSERT_EQUALS(
    GetTechModifiedProperty(replace_tokens_3, "Structure", "beta"),
    "beta"
  );
  ...
  TS_ASSERT_EQUALS(
    GetTechModifiedProperty(replace_tokens_4, "Unit", "beta"),
    "alpha delta gamma"
  );
}

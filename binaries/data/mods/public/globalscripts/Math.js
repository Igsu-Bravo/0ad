              ...
              a *
                ...
                (1 / 7 -
                  a *
                    a *
                    (1 / 9 -
                      a * a * (1 / 11 - a * a * (1 / 13 - (a * a) / 15)))))));

  // shift the result back where necessary
  r += tanPiBy6Shift;
  if (inverted) r = Math.PI / 2 - r;
  return sign * r;
};
...
Math.atan2 = function (y, x) {
  ...
  if (uy === 0) r = 0;
    ...
    if (uy === Infinity) {
      if (ux === Infinity) r = Math.PI / 4;
      else r = Math.PI / 2;
    } else {
      if (ux === Infinity) r = 0;
      else r = Math.atan(uy / ux);
    }
  }
  ...
  if (x < 0 || 1 / x === -Infinity) {
    if (y < 0 || 1 / y === -Infinity) return -Math.PI + r;
    else return Math.PI - r;
  } else {
    if (y < 0 || 1 / y === -Infinity) return -r;
    else return r;
  }
};
 ...
 */
Math.exp = function (x) {
  if (x < 0) var iPart = 1 / Math.intPow(Math.E, -Math.floor(x));
  else var iPart = Math.intPow(Math.E, Math.floor(x));
  ...
  for (var i = 22; i > 0; i--) dPart = 1 + (x * dPart) / i;
...
Math.intPow = function (x, y) {
  if (Math.abs(y) === Infinity) {
    ...
    if ((Math.abs(x) < 1 && y > 0) || (Math.abs(x) > 1 && y < 0)) return 0;

...
class ProgressBar {
  constructor() {
    ...
    this.progressbar_right_width =
      this.progressbar_right.size.right - this.progressbar_right.size.left;
    ...
    this.progressBarSize =
      this.progressbar.size.right -
      this.progressbar.size.left -
      this.progressbar_right_width / 2;

    this.progressText = Engine.GetGUIObjectByName("progressText");
    this.showDescription =
      Engine.ConfigDB_GetValue("user", this.ConfigKey) == "true";
  ...
  onGameLoadProgress(progression, description) {
    ...
    if (this.showDescription) this.progressText.caption = description;
    else {
...
ProgressBar.prototype.CaptionFormat = translateWithContext(
  "loading screen progress",
  "%(percentage)s%%"
);

ProgressBar.prototype.ConfigKey = "gui.loadingscreen.progressdescription";

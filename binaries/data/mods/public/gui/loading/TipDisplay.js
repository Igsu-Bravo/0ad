...
class TipDisplay {
  ...
  displayRandomTip() {
    ...
    if (tipFile) this.displayTip(tipFile);
    else error("Failed to find any matching tips for the loading screen.");
  }

  displayTip(tipFile) {
    this.tipImage.sprite = "stretched:" + this.ImagePath + tipFile + ".png";

    let tipText = Engine.TranslateLines(
      Engine.ReadFile(this.TextPath + tipFile + ".txt")
    ).split("\n");
    ...
    titleSize.bottom =
      titleSize.top + this.tipTitle.getTextSize().height + margin;
    ...
    this.tipText.caption = tipText
      .map((text) => text && sprintf(this.BulletFormat, { tiptext: text }))
      .join("\n\n");
  }
}

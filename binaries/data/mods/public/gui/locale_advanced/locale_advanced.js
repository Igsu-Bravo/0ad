function init(initData) {
  ...
  if (languageList.list_data.indexOf(currentLocaleLanguage) != -1)
    languageList.selected = languageList.list_data.indexOf(
...
function updateResultingLocale() {
  ...
  if (
    countryList.selected != -1 &&
    countryList.list_data[countryList.selected] !=
      translateWithContext("localeCountry", "None")
  )
    resultingLocaleTmp =
      resultingLocaleTmp + "_" + countryList.list_data[countryList.selected];
  ...
  if (Engine.ValidateLocale(resultingLocaleTmp)) {
    ...
    let dictionaryFileList = Engine.GetDictionariesForLocale(
      Engine.GetDictionaryLocale(resultingLocaleTmp)
    );
    let dictionaryFileString = "";
    dictionaryFileList.forEach((entry) => {
      dictionaryFileString = dictionaryFileString + entry + "\n";
    });
...
function autoDetectLocale() {
  ...
  languageList.selected = languageList.list_data.indexOf(
    Engine.GetLocaleLanguage(locale)
  );
  countryList.selected = countryList.list_data.indexOf(
    Engine.GetLocaleCountry(locale)
  );

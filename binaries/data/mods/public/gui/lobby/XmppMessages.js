...
class XmppMessages {
  ...
  onTick() {
    ...
    if (Engine.LobbyGuiPollHasPlayerListUpdate())
      for (let handler of this.playerListUpdateHandlers) handler();
  }
  ...
  handleMessages(getMessages) {
    ...
    for (let message of messages) {
      if (!this.xmppMessageHandlers[message.type])
        ...
        for (let handler of this.xmppMessageHandlers[message.type][
          message.level
        ])
          handler(message);
    }

    for (let handler of this.messageBatchProcessedHandlers) handler();
  }
}
...
XmppMessages.prototype.MessageTypes = {
  system: ["registered", "connected", "disconnected", "error"],
  chat: [
    "subject",
    "join",
    "leave",
  ...
  game: ["gamelist", "profile", "leaderboard", "ratinglist"],
};

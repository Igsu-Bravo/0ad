...
class BuddyButton {
  ...
  rebuild() {
    ...
    this.toggleBuddyButton.enabled =
      Engine.IsXmppClientConnected() &&
      !!this.playerName &&
      this.playerName != g_Nickname;
  }
  ...
  onPress() {
    if (
      !this.playerName ||
      this.playerName == g_Nickname ||
      this.playerName.indexOf(g_BuddyListDelimiter) != -1
    )
      return;

    let index = g_Buddies.indexOf(this.playerName);
    if (index != -1) g_Buddies.splice(index, 1);
    else g_Buddies.push(this.playerName);

    Engine.ConfigDB_CreateAndWriteValueToFile(
      "user",
      "lobby.buddies",
      g_Buddies.filter((nick) => nick).join(g_BuddyListDelimiter) ||
    ...
    for (let handler of this.buddyChangedHandlers) handler();
  }
}

...
class GameDetails {
  constructor(dialog, gameList, mapCache) {
    ...
    gameList.registerSelectionChangeHandler(
      this.onGameListSelectionChange.bind(this)
    );
  ...
  resize(dialog) {
    let bottom =
      Engine.GetGUIObjectByName(dialog ? "leaveButton" : "joinButton").size
        .top - 5;
  ...
  onGameListSelectionChange(game) {
    ...
    if (
      stanza.mapType != this.lastGame.mapType ||
      stanza.mapName != this.lastGame.mapName
    ) {
      this.sgMapName.caption = displayData.mapName;
      if (this.mapCache.checkIfExists(stanza.mapType, stanza.mapName))
        this.sgMapPreview.sprite = this.mapCache.getMapPreview(
          stanza.mapType,
          stanza.mapName
        );
      else
...

    {
      let txt =
        ...
        (stanza.victoryConditions
          ? stanza.victoryConditions
              .split(",")
              .map(translateVictoryCondition)
              .join(this.Comma)
          : translateWithContext("victory condition", "Endless Game"));

      txt +=
        "\n" +
        setStringTags(this.MapTypeFormat, this.CaptionTags) +
        " " +
        displayData.mapType +
        "\n" +
        setStringTags(this.MapSizeFormat, this.CaptionTags) +
        " " +
        displayData.mapSize +
        "\n" +
...

    {
      ...
      if (stanza.startTime) {
        this.gameStartArgs.time = Engine.FormatMillisecondsIntoDateStringLocal(
          +stanza.startTime * 1000,
          this.TimeFormat
        );
...

    {
      ...
      if (!game.isCompatible)
        txt += setStringTags(
          coloredText(
            setStringTags(this.IncompatibleModsFormat, this.CaptionTags),
            "red"
          ),
          {
            tooltip: sprintf(
              translate("You have some incompatible mods:\n%(details)s"),
              {
                details: comparedModsString(
                  game.mods,
                  Engine.GetEngineInfo().mods
                ),
              }
            ),
          }
        );
      else txt += setStringTags(this.ModsFormat, this.CaptionTags);

      const sortedMods = game.mods;
      sortedMods.sort(
        (a, b) => a.ignoreInCompatibilityChecks - b.ignoreInCompatibilityChecks
      );
...
GameDetails.prototype.PlayerCountFormat = translate(
  "Players: %(current)s/%(total)s"
);

GameDetails.prototype.VictoryConditionsFormat = translate(
  "Victory Conditions:"
);
...
GameDetails.prototype.IncompatibleModsFormat = translate(
  "Mods (incompatible):"
);

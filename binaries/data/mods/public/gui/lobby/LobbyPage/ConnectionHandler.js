...
class ConnectionHandler {
  ...
  onNickChange(message) {
    if (message.oldnick == g_Nickname) g_Nickname = message.newnick;
  }

  onClientLeave(message) {
    if (message.nick == g_Nickname) Engine.DisconnectXmppClient();
  }

  onClientKicked(banned, message) {
    if (message.nick != g_Nickname) return;
  ...
  askReconnect() {
    ...
    if (Engine.IsXmppClientConnected() || this.askingReconnect) return;

...
class ChatCommandHandler {
  ...
  handleChatCommand(text) {
    ...
    if (
      commandObj.moderatorOnly &&
      Engine.LobbyGetPlayerRole(g_Nickname) != "moderator"
    ) {
      this.chatMessagesPanel.addText(
        Date.now() / 1000,
        this.systemMessageFormat.format(
  ...
  argumentCount(commandName, args) {
    ...
    this.chatMessagesPanel.addText(
      ...
      this.systemMessageFormat.format(
        sprintf(
          translate("The command '%(cmd)s' requires at least one argument."),
          {
...
ChatCommandHandler.prototype.ChatCommands = {
  ...
  kick: {
    description: translate(
      "Kick a specified user from the lobby. Usage: /kick nick reason"
    ),
    handler: function (args) {
      let index = args.indexOf(" ");
      if (index == -1) Engine.LobbyKick(args, "");
      else Engine.LobbyKick(args.substr(0, index), args.substr(index + 1));
      return true;
    },
    moderatorOnly: true,
  },
  ban: {
    description: translate(
      "Ban a specified user from the lobby. Usage: /ban nick reason"
    ),
    handler: function (args) {
      let index = args.indexOf(" ");
      if (index == -1) Engine.LobbyBan(args, "");
      else Engine.LobbyBan(args.substr(0, index), args.substr(index + 1));
      return true;
    },
    moderatorOnly: true,
  },
  ...
  me: {
    description: translate(
      "Send a chat message about yourself. Example: /me goes swimming."
    ),
    handler: function (args) {

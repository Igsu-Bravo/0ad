...
ChatMessageEvents.ClientEvents = class {
  constructor(xmppMessages, chatMessagesPanel, statusMessageFormat) {
    ...
    xmppMessages.registerXmppMessageHandler(
      "chat",
      "join",
    ...
    xmppMessages.registerXmppMessageHandler(
      "chat",
      "kicked",
      this.onClientKicked.bind(this, false)
    );
    xmppMessages.registerXmppMessageHandler(
      "chat",
      "banned",
      this.onClientKicked.bind(this, true)
    );
  }
...
ChatMessageEvents.ClientEvents.prototype.FormatJoin = translate(
  "%(nick)s has joined."
);
ChatMessageEvents.ClientEvents.prototype.FormatLeave =
  translate("%(nick)s has left.");
...
ChatMessageEvents.Nick.prototype.Format = translate(
  "%(oldnick)s is now known as %(newnick)s."
);
...
ChatMessageEvents.Role = class {
  constructor(xmppMessages, chatMessagesPanel, statusMessageFormat) {
    ...
    xmppMessages.registerXmppMessageHandler(
      "chat",
      "role",
      this.onRoleChange.bind(this)
    );
  }

  onRoleChange(message) {
    let roleType = this.RoleStrings.find(
      (type) =>
    ...
    let txt;
    if (message.nick == g_Nickname) txt = roleType.you;
    else {
...
ChatMessageEvents.Subject.prototype.FormatChange = translate(
  "%(nick)s changed the lobby subject to:"
);
ChatMessageEvents.Subject.prototype.FormatDelete = translate(
  "%(nick)s deleted the lobby subject."
);

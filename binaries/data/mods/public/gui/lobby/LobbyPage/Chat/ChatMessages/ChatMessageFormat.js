...
class ChatMessageFormat {
  ...
  format(message) {
    ...
    if (g_Nickname != message.from) {
      ...
      if (
        !message.historic &&
        text.toLowerCase().indexOf(g_Nickname.toLowerCase()) != -1
      )
        soundNotification("nick");
    }

    let sender = PlayerColor.ColorPlayerName(
      message.from,
      undefined,
      Engine.LobbyGetPlayerRole(message.from)
    );
    ...
    if (text.startsWith("/") && index != -1) {
      ...
      switch (command) {
        case "me": {
          formattedMessage = this.chatMessageFormatMe.format(
            sender,
            commandText
          );
          break;
        ...
        case "say": {
          formattedMessage = this.chatMessageFormatSay.format(
            sender,
            commandText
          );
          break;
    ...
    } else formattedMessage = this.chatMessageFormatSay.format(sender, text);

    if (message.level == "private-message")
      formattedMessage =
        this.chatMessagePrivateWrapper.format(formattedMessage);

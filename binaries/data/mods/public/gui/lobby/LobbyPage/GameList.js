...
class GameList {
  constructor(xmppMessages, buddyButton, mapCache) {
    ...
    let ratingColumn =
      Engine.ConfigDB_GetValue("user", "lobby.columns.gamerating") == "true";
    ...
    for (let name in GameListFilters)
      this.filters.push(
        new GameListFilters[name](this.onFilterChange.bind(this))
      );

    xmppMessages.registerXmppMessageHandler(
      "game",
      "gamelist",
      this.rebuildGameList.bind(this)
    );
    xmppMessages.registerXmppMessageHandler(
      "system",
      "disconnected",
  ...
  onBuddyChange() {
    for (let name in this.games) this.games[name].onBuddyChange();
  ...
  onSelectionChange() {
    ...
    for (let handler of this.selectionChangeHandlers) handler(game);
  }
  ...
  rebuildGameList() {
...

    {
      ...
      for (let stanza of gameListData) {
        ...
        let exists = !!game;
        if (!exists) game = new Game(this.mapCache);
...

    {
      ...
      this.gameList.forEach((game, i) => {
        ...
        if (
          selectedGame &&
          game.stanza.hostJID == selectedGame.stanza.hostJID &&
          game.stanza.name == selectedGame.stanza.name
        )
          selectedGameIndex = i;
      });
      Engine.ProfileStop();
  ...
  selectGameFromPlayername(playerName) {
    ...
    for (let i = 0; i < this.gameList.length; ++i)
      for (let player of this.gameList[i].players) {
        if (playerName != splitRatingFromNick(player.Name).nick) continue;
        ...
        if (!foundAsObserver) this.gamesBox.selected = i;
      }
  }
}

...
class Game {
  constructor(mapCache) {
    ...
    this.stanza = {};
    for (let name of this.StanzaKeys) this.stanza[name] = "";
  ...
  update(newStanza, sortKey) {
    ...
    if (oldStanza.niceMapName != newStanza.niceMapName) {
      ...
      if (this.mapCache.checkIfExists(newStanza.mapType, newStanza.mapName)) {
        displayData.mapName = escapeText(
          this.mapCache.translateMapName(newStanza.niceMapName)
        );
        displayData.mapDescription = this.mapCache.getTranslatedMapDescription(
          newStanza.mapType,
          newStanza.mapName
        );
      } else {
    ...
    if (oldStanza.mapType != newStanza.mapType) {
      ...
      displayData.mapType =
        g_MapTypes.Title[g_MapTypes.Name.indexOf(newStanza.mapType)] || "";
  ...
  updatePlayers(newStanza) {
...

    {
      ...
      for (let player of players) {
        ...
        if (player.Team == "observer") ++observerCount;
        else
      ...
      let displayData = this.displayData;
      let sortValues = this.sortValues;
      displayData.buddy = this.hasBuddies
        ? setStringTags(g_BuddySymbol, displayData.tags)
        : "";
  ...
  updateMods(newStanza) {
...

    {
      ...
      let isCompatible =
        this.mods && hasSameMods(this.mods, Engine.GetEngineInfo().mods);
  ...
  updateGameTags(newStanza) {
    let displayData = this.displayData;
    displayData.tags = this.isCompatible
      ? this.StateTags[newStanza.state]
  ...
  updateGameName(newStanza) {
    let displayData = this.displayData;
    displayData.gameName = setStringTags(
      escapeText(newStanza.name),
      displayData.tags
    );

    let sortValues = this.sortValues;
    sortValues.gameName =
      sortValues.compatibility + sortValues.state + sortValues.gameName;
    sortValues.buddy = sortValues.hasBuddyString + sortValues.gameName;
  }

  getTranslatedPlayerCount(newStanza) {
    let playerCountArgs = this.playerCountArgs;
    playerCountArgs.current = setStringTags(
      escapeText(newStanza.nbp),
      this.PlayerCountTags.CurrentPlayers
    );
    playerCountArgs.max = setStringTags(
      escapeText(newStanza.maxnbp),
      this.PlayerCountTags.MaxPlayers
    );

    let txt;
    if (this.observerCount) {
      playerCountArgs.observercount = setStringTags(
        this.observerCount,
        this.PlayerCountTags.Observers
      );
      txt = this.PlayerCountObservers;
    } else txt = this.PlayerCountNoObservers;
 ...
 */
Game.prototype.GameStatusOrder = ["init", "waiting", "running"];

// Translation: The number of players and observers in this game
Game.prototype.PlayerCountObservers = translate(
  "%(current)s/%(max)s +%(observercount)s"
);

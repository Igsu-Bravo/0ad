GameListFilters.PlayerCount = class {
  constructor(onFilterChange) {
    ...
    let playersArray = Array(g_MaxPlayers)
      .fill(0)
      .map((v, i) => i + 1); // 1, 2, ... MaxPlayers
    this.playersNumberFilter = Engine.GetGUIObjectByName("playersNumberFilter");
    this.playersNumberFilter.list = [
    ...
    this.playersNumberFilter.onSelectionChange =
      this.onSelectionChange.bind(this);
  }

  onSelectionChange() {
    this.selected =
      this.playersNumberFilter.list_data[this.playersNumberFilter.selected];
    this.onFilterChange();
  }

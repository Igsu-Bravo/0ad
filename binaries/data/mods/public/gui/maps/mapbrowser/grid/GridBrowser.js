...
class GridBrowser {
  ...
  goToPage(pageNumber) {
    ...
    for (let handler of this.pageChangeHandlers) handler();
  }
  ...
  nextPage(wrapAround = true) {
    ...
    else this.goToPage((this.currentPage + 1) % numberPages);
  }

  previousPage(wrapAround = true) {
    let numberPages = Math.max(1, this.pageCount);
    if (!wrapAround) this.goToPage(Math.max(this.currentPage - 1, 0));
    else this.goToPage((this.currentPage + numberPages - 1) % numberPages);
  }
  ...
  goToPageOfSelected() {
    this.goToPage(
      Math.max(
        Math.min(
          Math.floor(this.selected / this.itemsPerRow) -
            Math.floor(this.rowCount / 2),
          this.pageCount - 1
        ),
        0
      )
  ...
  increaseColumnCount(diff) {
    ...
    if (isSelectedInPage) this.goToPageOfSelected();
    else
      this.goToPage(
        Math.min(this.currentPage, Math.max(0, this.pageCount - 1))
      );
  }
  ...
  resizeGrid() {
    ...
    this.columnCount = Math.min(
      this.maxColumns,
      Math.max(this.minColumns, this.columnCount)
    );
    ...
    this.itemsPerPage = Math.min(
      this.columnCount * this.rowCount,
      this.items.length
    );
    // NB: pages only change by one row, so items are in several pages.
    this.pageCount =
      Math.ceil(this.itemCount / this.itemsPerRow) - this.rowCount + 1;

    for (let handler of this.gridResizeHandlers) handler();
  }
}

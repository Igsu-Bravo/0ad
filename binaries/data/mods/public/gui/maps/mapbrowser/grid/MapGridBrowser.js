class MapGridBrowser extends GridBrowser {
  constructor(mapBrowserPage, setupWindow) {
    ...
    this.mapBrowserPage.mapBrowserPageDialog.onMouseWheelUp = () =>
      this.previousPage(false);
    this.mapBrowserPage.mapBrowserPageDialog.onMouseWheelDown = () =>
      this.nextPage(false);
  }
  ...
  updateMapList() {
    ...
    const filterText =
      this.mapBrowserPage.controls.MapFiltering.getSearchText();
    const randomMap = {
      file: "random",
    ...
    if (mapType === "random") mapList.unshift(randomMap);
    ...
    this.setSelectedIndex(
      this.mapList.findIndex((map) => map.file == selectedMap)
    );
  }
}

class MapBrowser {
  constructor(mapCache, mapFilters, setupWindow = undefined) {
    ...
    this.mapBrowserPageDialog = Engine.GetGUIObjectByName(
      "mapBrowserPageDialog"
    );
  ...
  submitMapSelection() {
    ...
    this.onSubmitMapSelection(file, type, filter);
    this.closePage();
  }
  ...
  openPage() {
    if (this.open) return;
    for (let handler of this.openPageHandlers) handler();
    this.open = true;
  }

MapBrowserPageControls.prototype.MapDescription = class {
  constructor(mapBrowserPage, gridBrowser) {
    ...
    this.mapBrowserSelectedName = Engine.GetGUIObjectByName(
      "mapBrowserSelectedName"
    );
    this.mapBrowserSelectedPreview = Engine.GetGUIObjectByName(
      "mapBrowserSelectedPreview"
    );
    this.mapBrowserSelectedDescription = Engine.GetGUIObjectByName(
      "mapBrowserSelectedDescription"
    );

    let computedSize = this.mapBrowserSelectedPreview.getComputedSize();
    let top = this.mapBrowserSelectedName.size.bottom;
    let height = Math.floor(
      (computedSize.right - computedSize.left) / this.ImageRatio
    );
    ...
    gridBrowser.registerSelectionChangeHandler(
  ...
  onSelectionChange() {
    ...
    this.mapBrowserSelectedPreview.sprite = this.mapCache.getMapPreview(

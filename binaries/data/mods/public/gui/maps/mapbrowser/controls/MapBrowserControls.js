class MapBrowserPageControls {
  ...
  setupButtons() {
    ...
    if (!g_IsController) this.pickRandom.hidden = true;
    ...
    if (g_SetupWindow)
      ...
      this.close.tooltip = colorizeHotkey(
        translate("%(hotkey)s: Close map browser."),
        "cancel"
      );
    }
    ...
    if (!g_IsController) this.close.size = this.select.size;

    this.gridBrowser.registerSelectionChangeHandler(() =>

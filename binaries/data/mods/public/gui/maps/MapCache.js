...
class MapCache {
  ...
  checkIfExists(mapType, mapPath) {
    if (!mapPath || mapPath == "random") return true;

    return g_Settings.MapTypes.find(
      (type) => type.Name == mapType
    ).CheckIfExists(mapPath);
  }

  getMapData(mapType, mapPath) {
    if (!mapPath || mapPath == "random") return undefined;
    if (!this.cache[mapPath]) {
      let mapData = g_Settings.MapTypes.find(
        (type) => type.Name == mapType
      ).GetData(mapPath);
  ...
  getTranslatableMapName(mapType, mapPath) {
    if (mapPath == "random") return "random";
  ...
  getTranslatedMapDescription(mapType, mapPath) {
    if (mapPath == "random") return translate("A randomly selected map.");

    let mapData = this.getMapData(mapType, mapPath);
    return (
      (mapData &&
        mapData.settings &&
        translate(mapData.settings.Description)) ||
      ""
    );
  }

  previewExists(filename) {
    return Engine.TextureExists(
      this.TexturesPath + this.PreviewsPath + filename
    );
  }
...
MapCache.prototype.TexturesPath = "art/textures/ui/";

MapCache.prototype.PreviewsPath = "session/icons/mappreview/";

MapCache.prototype.DefaultPreview = "nopreview.png";

MapCache.prototype.PreviewWidth = 400 / 512;

MapCache.prototype.PreviewHeight = 300 / 512;

class MapFilters {
  ...
  getFilteredMaps(mapTypeName, filterName, existence) {
    if (!mapTypeName || !filterName) return existence ? false : [];
    ...
    for (let filename of listFiles(
      mapTypePath,
      g_MapTypes.Suffix[index],
      false
    )) {
      if (filename.startsWith(this.HiddenFilesPrefix)) continue;
      ...
      if (!mapData || !mapData.settings) continue;
...
MapFilters.prototype.Filters = [
  ...
  },
  {
    ...
    Description: translate(
      "These maps are not playable but for demonstration purposes only."
    ),
    Match: ["demo"],
  },
  {
    ...
    Description: translate(
      "Maps that are brand new in this release of the game."
    ),
    Match: ["new"],
  },
  {
  ...
  },
  {
    ...
    Description: translate(
      "Maps that come with scripted events and potentially spawn enemy units."
    ),
    Match: ["trigger"],
  },
  {

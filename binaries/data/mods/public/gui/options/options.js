...
var g_OptionType = {
    ...
    sanitizeValue: (value, control, option) => {
      ...
      if (control) {
        control.sprite =
          sanitized == value
            ? "ModernDarkBoxWhite"
            : "ModernDarkBoxWhiteInvalid";
        control.children[1].sprite =
          sanitized == value ? "color:" + value : "color:" + g_InsaneColor;
      }
      return sanitized;
    },
  ...
  number: {
    ...
    guiSetter: "onTextEdit",
    sanitizeValue: (value, control, option) => {
      let sanitized = Math.min(
        option.max !== undefined ? option.max : +Infinity,
        Math.max(
          option.min !== undefined ? option.min : -Infinity,
          isNaN(+value) ? 0 : value
        )
      );
    ...
    tooltip: (value, option) =>
      sprintf(
        translateWithContext(
          "slider number",
          "Value: %(val)s (min: %(min)s, max: %(max)s)"
        ),
        {
          val: value.toFixed(2),
...
function displayOptions() {
  ...
  for (let body of Engine.GetGUIObjectByName("option_controls").children) {
    body.hidden = true;
    for (let control of body.children) control.hidden = true;
  }
  ...
  for (let i = 0; i < g_Options[g_TabCategorySelected].options.length; ++i) {
    ...
    bodySize.top =
      g_OptionControlOffset + i * (g_OptionControlHeight + g_OptionControlDist);
    ...
    let value = optionType.configToValue(
      Engine.ConfigDB_GetValue("user", option.config)
    );

    // Setup control
    let control = Engine.GetGUIObjectByName(
      "option_control_" + option.type + "[" + i + "]"
    );
    control.tooltip =
      option.tooltip +
    ...
    if (optionType.initGUI) optionType.initGUI(option, control);
    ...
    control[optionType.guiSetter] = function () {
      ...
      if (option.function) Engine[option.function](value);
...
function enableButtons() {
  g_Options[g_TabCategorySelected].options.forEach((option, i) => {
    let enabled =
      !option.dependencies ||
      option.dependencies.every(
        (config) => Engine.ConfigDB_GetValue("user", config) == "true"
      );
...
function setDefaults() {
  messageBox(
    ...
    200,
    translate(
      "Resetting the options will erase your saved settings. Do you want to continue?"
    ),
    translate("Warning"),
...
function saveChanges() {
  for (let category in g_Options)
    for (let i = 0; i < g_Options[category].options.length; ++i) {
      ...
      let optionType = g_OptionType[option.type];
      if (!optionType.sanitizeValue) continue;

      let value = optionType.configToValue(
        Engine.ConfigDB_GetValue("user", option.config)
      );
      if (value == optionType.sanitizeValue(value, undefined, option)) continue;
      ...
      messageBox(
        ...
        200,
        translate(
          "Some setting values are invalid! Are you sure you want to save them?"
        ),
        translate("Warning"),
        [translate("No"), translate("Yes")],
...
function closePage() {
  ...
  else closePageWithoutConfirmation();
}

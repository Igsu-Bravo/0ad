...
class ReadyController {
  constructor(
    ...
    gameSettingsController.registerSettingsChangeHandler(
      this.onSettingsChange.bind(this)
    );
    playerAssignmentsController.registerClientJoinHandler(
      this.onClientJoin.bind(this)
    );
    playerAssignmentsController.registerClientLeaveHandler(
  ...
  onClientJoin(newGUID, newAssignments) {
    if (newAssignments[newGUID].player != -1) this.resetReady();
  }

  onClientLeave(guid) {
    if (g_PlayerAssignments[guid].player != -1) this.resetReady();
  }
  ...
  onPlayerAssignmentsChange() {
    ...
    for (let guid in g_PlayerAssignments)
      if (
        this.previousAssignments[guid] &&
        this.previousAssignments[guid].player !=
  ...
  setReady(ready, sendMessage) {
    ...
    if (sendMessage) Engine.SendNetworkReady(ready);
  ...
  resetReady() {
    ...
    if (!g_IsNetworked || this.gameSettingsController.gameStarted) return;

    for (let handler of this.resetReadyHandlers) handler();

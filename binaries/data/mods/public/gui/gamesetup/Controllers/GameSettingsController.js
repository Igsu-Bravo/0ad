...
class GameSettingsController {
  constructor(setupWindow, netMessages, playerAssignmentsController, mapCache) {
    ...
    if (g_IsNetworked) {
      if (g_IsController)
        playerAssignmentsController.registerClientJoinHandler(
      ...
      // clients switch when they receive the appropriate message.
        ...
        netMessages.registerNetMessageHandler(
          "start",
          this.switchToLoadingPage.bind(this)
        );
      netMessages.registerNetMessageHandler(
        "gamesetup",
        this.onGamesetupMessage.bind(this)
      );
    }
  }
  ...
  onLoad(initData, hotloadData) {
    if (hotloadData) this.parseSettings(hotloadData.initAttributes);
    else if (
      g_IsController &&
      (initData?.gameSettings || this.persistentMatchSettings.enabled)
    ) {
      // Allow opting-in to persistence when sending initial data (though default off)
      if (initData?.gameSettings)
        this.persistentMatchSettings.enabled =
          !!initData.gameSettings?.usePersistence;
      const settings =
        initData?.gameSettings || this.persistentMatchSettings.loadFile();
    ...
    for (const guid in g_PlayerAssignments)
      if (
        ...
        g_GameSettings.playerAI.set(
          g_PlayerAssignments[guid].player - 1,
          undefined
        );

    for (const handler of this.settingsLoadedHandlers) handler();

    this.updateLayout();
    this.setNetworkInitAttributes();

    // If we are the controller, we are done loading.
    if (hotloadData || !g_IsNetworked || g_IsController) this.setLoading(false);
  }
  ...
  onGamesetupMessage(message) {
    ...
    if (!message.data || g_IsController) return;
    ...
    for (let handler of this.settingsChangeHandlers) handler();
  }
  ...
  parseSettings(settings) {
    if (settings.guiData) this.guiData.Deserialize(settings.guiData);
  ...
  setLoading(loading) {
    ...
    for (let handler of this.loadingChangeHandlers) handler(loading);
  }
  ...
  updateLayout() {
    ...
    this.layoutTimer = setTimeout(() => {
      for (let handler of this.updateLayoutHandlers) handler();
  ...
  setNetworkInitAttributes() {
    ...
    if (g_IsNetworked && this.timer === undefined)
      this.timer = setTimeout(
        this.setNetworkInitAttributesImmediately.bind(this),
        this.Timeout
      );
  }

...
class PlayerAssignmentsController {
  constructor(setupWindow, netMessages) {
    ...
    g_GameSettings.playerCount.watch(
      () => this.unassignInvalidPlayers(),
      ["nbPlayers"]
    );

    setupWindow.registerLoadHandler(this.onLoad.bind(this));
    setupWindow.registerGetHotloadDataHandler(this.onGetHotloadData.bind(this));
    netMessages.registerNetMessageHandler(
      "players",
      this.onPlayerAssignmentMessage.bind(this)
    );
  ...
  onClientJoin(newGUID, newAssignments) {
    if (!g_IsController || newAssignments[newGUID].player != -1) return;

    // Assign the client (or only buddies if prefered) to a free slot
    if (newGUID != Engine.GetPlayerGUID()) {
      const assignOption = Engine.ConfigDB_GetValue(
        "user",
        this.ConfigAssignPlayers
      );
      if (
        assignOption == "disabled" ||
        (assignOption == "buddies" &&
          g_Buddies.indexOf(
            splitRatingFromNick(newAssignments[newGUID].name).nick
          ) == -1)
      )
        return;
    }
    ...
    const possibleSlots = [
      ...Array(g_GameSettings.playerCount.nbPlayers).keys(),
    ].map((i) => i + 1);
    ...
    if (
      ...
      if (free) slot = this.lastAssigned[newName];
    }
    if (!slot)
      slot = possibleSlots.find((i) => {
        for (const guid in newAssignments)
          if (newAssignments[guid].player == i) return false;
        return true;
      });
  ...
  updatePlayerAssignments() {
    ...
    for (const guid in g_PlayerAssignments)
      this.lastAssigned[g_PlayerAssignments[guid].name] =
        g_PlayerAssignments[guid].player;
    for (const handler of this.playerAssignmentsChangeHandlers) handler();
    Engine.ProfileStop();
  }
  ...
  onPlayerAssignmentMessage(message) {
    ...
    for (let guid in g_PlayerAssignments)
      if (!newAssignments[guid])
        for (let handler of this.clientLeaveHandlers) handler(guid);
  ...
  assignClient(guid, playerIndex) {
    if (g_IsNetworked) Engine.AssignNetworkPlayer(playerIndex, guid);
    else {
  ...
  unassignClient(playerID) {
    if (g_IsNetworked) Engine.AssignNetworkPlayer(playerID, "");
  ...
  unassignInvalidPlayers() {
    if (g_IsNetworked) {
      for (let guid in g_PlayerAssignments)
        if (
          g_PlayerAssignments[guid].player >
          g_GameSettings.playerCount.nbPlayers
        )
          Engine.AssignNetworkPlayer(g_PlayerAssignments[guid].player, "");
    } else if (
      g_PlayerAssignments.local.player > g_GameSettings.playerCount.nbPlayers
    ) {
      g_PlayerAssignments.local.player = -1;
      this.updatePlayerAssignments();
    }

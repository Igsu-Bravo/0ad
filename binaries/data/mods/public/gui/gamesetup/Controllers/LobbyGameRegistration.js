...
class LobbyGameRegistrationController {
  constructor(
    ...
    playerAssignmentsController.registerPlayerAssignmentsChangeHandler(
      this.sendImmediately.bind(this)
    );

    g_GameSettings.map.watch(() => this.onSettingsChange(), ["map", "type"]);
    g_GameSettings.mapSize.watch(() => this.onSettingsChange(), ["size"]);
    g_GameSettings.victoryConditions.watch(
  ...
  onSettingsChange() {
    if (this.lastStanza) this.sendDelayed();
    else this.sendImmediately();
  }
  ...
  sendImmediately() {
    ...
    if (!g_GameSettings.map.map) return;
    ...
    let stanza = {
      ...
      niceMapName: this.mapCache.getTranslatableMapName(
        g_GameSettings.map.type,
        g_GameSettings.map.map
      ),
      mapSize:
        g_GameSettings.map.type == "random"
          ? g_GameSettings.mapSize.size
          : "Default",
      mapType: g_GameSettings.map.type,
      victoryConditions: Array.from(
        g_GameSettings.victoryConditions.active
      ).join(","),
    ...
    if (
      this.lastStanza &&
      Object.keys(stanza).every((prop) => this.lastStanza[prop] == stanza[prop])
    )
      return;
  ...
  formatClientsForStanza() {
    ...
    for (let guid in g_PlayerAssignments) {
      ...
      if (
        g_PlayerAssignments[guid].player <= g_GameSettings.playerCount.nbPlayers
      )
        ++connectedPlayers;
      else pData.Team = "observer";

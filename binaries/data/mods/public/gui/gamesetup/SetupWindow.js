...
class SetupWindow {
  constructor(initData, hotloadData) {
    ...
    if (initData?.backPage) this.backPage = initData.backPage;
    ...
    let playerAssignmentsController = new PlayerAssignmentsController(
      this,
      netMessages
    );
    let gameSettingsController = new GameSettingsController(
      this,
      netMessages,
      playerAssignmentsController,
      mapCache
    );
    let readyController = new ReadyController(
      netMessages,
      gameSettingsController,
      playerAssignmentsController
    );
    let lobbyGameRegistrationController =
    ...
    for (let handler of this.loadHandlers) handler(initData, hotloadData);
  ...
  getHotloadData() {
    let object = {};
    for (let handler of this.getHotloadDataHandlers) handler(object);
    return object;
  }

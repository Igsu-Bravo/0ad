...
SetupWindowPages.AIConfigPage = class {
  constructor(setupWindow) {
    ...
    for (let name of this.AIGameSettingControlOrder)
      this.AIGameSettingControls[name] = new AIGameSettingControls[name](
        this,
        undefined,
        undefined,
        setupWindow
    ...
    Engine.GetGUIObjectByName("aiConfigOkButton").onPress =
  ...
  openPage(playerIndex) {
    ...
    for (let handler of this.openPageHandlers) handler(playerIndex);

    this.aiConfigPage.hidden = false;
  }

  maybeClose() {
    if (!g_GameSettings.playerAI.get(this.playerIndex)) this.closePage();
  }

...
class GameSettingControl {
  /* extends Profilable /* Uncomment to profile controls without hassle. */
  constructor(gameSettingControlManager, category, playerIndex, setupWindow) {
    ...
    if (this.setControl) this.setControl(gameSettingControlManager);

    // This variable also used for autocompleting chat.
    this.autocompleteTitle = undefined;

    if (this.title && this.TitleCaption) this.setTitle(this.TitleCaption);
    ...
    if (this.onSettingsLoaded)
      this.gameSettingsController.registerSettingsLoadedHandler(
        this.onSettingsLoaded.bind(this)
      );

    if (this.onPlayerAssignmentsChange)
      this.playerAssignmentsController.registerPlayerAssignmentsChangeHandler(
        this.onPlayerAssignmentsChange.bind(this)
      );
  }
  ...
  setTooltip(tooltip) {
    if (this.title) this.title.tooltip = tooltip;

    if (this.label) this.label.tooltip = tooltip;

    if (this.setControlTooltip) this.setControlTooltip(tooltip);
  }
  ...
  updateVisibility() {
    ...
    if (this.frame) this.frame.hidden = hidden;

    if (hidden) return;

    let enabled = g_IsController && this.enabled;

    if (this.setControlHidden) this.setControlHidden(!enabled);

    if (this.label) this.label.hidden = !!enabled;
  }
  ...
  addAutocompleteEntries(name, autocomplete) {
    if (this.autocompleteTitle) autocomplete[0].push(this.autocompleteTitle);

    if (!Number.isInteger(this.AutocompleteOrder)) return;

    if (!this.getAutocompleteEntries) {
      error(
        name +
          " specifies AutocompleteOrder but didn't implement getAutocompleteEntries"
      );
      return;
    }
...
GameSettingControl.prototype.TitleCaptionFormat = translateWithContext(
  "Title for specific setting",
  "%(setting)s:"
);

class GameSettingTabs {
  constructor(setupWindow, lobbyButton) {
    ...
    this.settingsTabButtonsFrame = Engine.GetGUIObjectByName(
    ...
    for (let tab in g_GameSettingsLayout)
      g_GameSettingsLayout[tab].tooltip =
        ...
        colorizeHotkey(
          "\n" + this.HotkeyNextTooltip,
          this.ConfigNameHotkeyNext
        ) +
        colorizeHotkey(
          "\n" + this.HotkeyPreviousTooltip,
          this.ConfigNameHotkeyPrevious
        );
  ...
  resize() {
    ...
    size.bottom =
      size.top +
      g_GameSettingsLayout.length *
        (this.TabButtonHeight + this.TabButtonMargin);

    if (!this.lobbyButton.lobbyButton.hidden) {
      let lobbyButtonSize = this.lobbyButton.lobbyButton.parent.size;
      size.right -=
        lobbyButtonSize.right - lobbyButtonSize.left + this.LobbyButtonMargin;
    }
  ...
  onTabSelect() {
    for (let handler of this.tabSelectHandlers) handler();
  }
}

GameSettingTabs.prototype.ToggleTooltip = translate(
  "Click to toggle the %(name)s settings tab."
);

GameSettingTabs.prototype.HotkeyNextTooltip = translate(
  "Press %(hotkey)s to move to the next settings tab."
);

GameSettingTabs.prototype.HotkeyPreviousTooltip = translate(
  "Press %(hotkey)s to move to the previous settings tab."
);

GameSettingTabs.prototype.ConfigNameHotkeyNext = "tab.next";

GameSettingTabs.prototype.ConfigNameHotkeyPrevious = "tab.prev";

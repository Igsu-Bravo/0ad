GameSettingControls.TeamPlacement = class TeamPlacement extends (
  ...
  constructor(...args) {
    ...
    g_GameSettings.teamPlacement.watch(
      () => this.render(),
      ["value", "available"]
    );
    this.render();
  ...
  render() {
    ...
    if (!g_GameSettings.teamPlacement.value) return;
    ...
    for (let pattern of g_GameSettings.teamPlacement.available)
      patterns.push(
        g_GameSettings.teamPlacement.StartingPositions.find(
          (pObj) => pObj.Id == pattern
        )
      );
...
GameSettingControls.TeamPlacement.prototype.Tooltip = translate(
  "Select one of the starting position patterns of this map."
);

GameSettingControls.TeamPlacement.prototype.RandomItem = {
  Id: "random",
  Name: translateWithContext("team placement", "Random"),
  Description: translateWithContext(
    "team placement",
    "Select a random team placement pattern when starting the game."
  ),
};

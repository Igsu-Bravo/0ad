PlayerSettingControls.PlayerColor = class PlayerColor extends (
  ...
  setControl() {
    this.dropdown = Engine.GetGUIObjectByName(
      "playerColor[" + this.playerIndex + "]"
    );
    this.playerBackgroundColor = Engine.GetGUIObjectByName(
      "playerBackgroundColor[" + this.playerIndex + "]"
    );
    this.playerColorHeading = Engine.GetGUIObjectByName("playerColorHeading");
  }

  render() {
    if (g_GameSettings.playerCount.nbPlayers < this.playerIndex + 1) return;
    ...
    this.dropdown.list = this.values.map((color) =>
      coloredText(this.ColorIcon, rgbToGuiColor(color))
    );

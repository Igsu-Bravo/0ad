GameSettingControls.LockedTeams = class LockedTeams extends (
  GameSettingControlCheckbox
) {
  constructor(...args) {
    super(...args);
    g_GameSettings.map.watch(() => this.render(), ["type"]);
    g_GameSettings.lockedTeams.watch(
      () => this.render(),
      ["available", "enabled"]
    );
    this.render();
  ...
  render() {
    this.setEnabled(
      g_GameSettings.map.type != "scenario" &&
        g_GameSettings.lockedTeams.available
    );
...
GameSettingControls.LockedTeams.prototype.Tooltip = translate(
  "Toggle locked teams."
);

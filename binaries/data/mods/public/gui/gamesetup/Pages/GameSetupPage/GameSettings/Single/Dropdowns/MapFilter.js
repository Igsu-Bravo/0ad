GameSettingControls.MapFilter = class MapFilter extends (
  ...
  checkMapTypeChange() {
    ...
    let values = prepareForDropdown(
      this.mapFilters.getAvailableMapFilters(g_GameSettings.map.type)
    );
    ...
    if (values.Name.length) {
      ...
      this.values = values;
    } else this.values = undefined;

    if (
      this.values &&
      this.values.Name.indexOf(
        this.gameSettingsController.guiData.mapFilter.filter
      ) === -1
    ) {
      this.gameSettingsController.guiData.mapFilter.filter =
        this.values.Name[this.values.Default];
  ...
  onSelectionChange(itemIdx) {
    this.gameSettingsController.guiData.mapFilter.filter =
      this.values.Name[itemIdx];
    this.gameSettingsController.setNetworkInitAttributes();
  }
};

GameSettingControls.MapFilter.prototype.TitleCaption = translate("Map Filter");

GameSettingControls.MapFilter.prototype.Tooltip = translate(
  "Select a map filter."
);

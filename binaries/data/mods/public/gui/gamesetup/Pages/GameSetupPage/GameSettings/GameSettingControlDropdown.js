/**
 * This class is implemented by game settings that are controlled by a dropdown.
 */
class GameSettingControlDropdown extends GameSettingControl {
  ...
  setSelectedValue(value) {
    ...
    if (this.label)
      this.label.caption =
        index == -1 ? this.UnknownValue : this.dropdown.list[index];
  }

  onSelectionChangeSuper() {
    if (!this.isInGuiUpdate) this.onSelectionChange(this.dropdown.selected);
  }
}
...
GameSettingControlDropdown.prototype.UnknownValue = translateWithContext(
  "settings value",
  "Unknown"
);

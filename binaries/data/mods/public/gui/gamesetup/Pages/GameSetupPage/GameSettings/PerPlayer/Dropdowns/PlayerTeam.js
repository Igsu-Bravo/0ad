PlayerSettingControls.PlayerTeam = class PlayerTeam extends (
  ...
  constructor(...args) {
    ...
    this.values = prepareForDropdown([
      ...
      ...Array.from(new Array(g_MaxTeams), (v, i) => ({
        label: i + 1,
        id: i,
      })),
    ]);
  ...
  setControl() {
    this.label = Engine.GetGUIObjectByName(
      "playerTeamText[" + this.playerIndex + "]"
    );
    this.dropdown = Engine.GetGUIObjectByName(
      "playerTeam[" + this.playerIndex + "]"
    );
  }
...
PlayerSettingControls.PlayerTeam.prototype.Tooltip = translate(
  "Select player's team."
);

PlayerSettingControls.PlayerTeam.prototype.NoTeam = translateWithContext(
  "team",
  "None"
);

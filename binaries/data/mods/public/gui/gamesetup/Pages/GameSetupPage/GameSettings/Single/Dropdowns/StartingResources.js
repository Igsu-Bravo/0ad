GameSettingControls.StartingResources = class StartingResources extends (
  ...
  constructor(...args) {
    ...
    g_GameSettings.startingResources.watch(
      () => this.render(),
      ["resources", "perPlayer"]
    );
  ...
  onHoverChange() {
    ...
    if (this.dropdown.hovered != -1) {
      this.sprintfArgs.resources =
        g_StartingResources.Resources[this.dropdown.hovered];
  ...
  render() {
    this.setEnabled(
      g_GameSettings.map.type != "scenario" &&
        !g_GameSettings.startingResources.perPlayer
    );
    if (g_GameSettings.startingResources.perPlayer)
      this.label.caption = this.PerPlayerCaption;
    else this.setSelectedValue(g_GameSettings.startingResources.resources);
  }
  ...
  onSelectionChange(itemIdx) {
    g_GameSettings.startingResources.setResources(
      g_StartingResources.Resources[itemIdx]
    );
...
GameSettingControls.StartingResources.prototype.Tooltip = translate(
  "Select the game's starting resources."
);

GameSettingControls.StartingResources.prototype.HoverTooltip = translate(
  "Initial amount of each resource: %(resources)s."
);

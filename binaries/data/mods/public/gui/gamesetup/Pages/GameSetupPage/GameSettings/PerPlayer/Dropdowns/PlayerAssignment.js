 ...
 */
PlayerSettingControls.PlayerAssignment = class PlayerAssignment extends (
  ...
  constructor(...args) {
    ...
    this.aiItems = g_Settings.AIDescriptions.filter(
      (ai) => !ai.data.hidden
    ).map(this.aiItemFactory.createItem.bind(this.aiItemFactory));
    ...
    g_GameSettings.playerCount.watch(
      (_, oldNb) => this.OnPlayerNbChange(oldNb),
      ["nbPlayers"]
    );
  }

  setControl() {
    this.dropdown = Engine.GetGUIObjectByName(
      "playerAssignment[" + this.playerIndex + "]"
    );
    this.label = Engine.GetGUIObjectByName(
      "playerAssignmentText[" + this.playerIndex + "]"
    );
  }

  OnPlayerNbChange(oldNb) {
    let isPlayerSlot = Object.values(g_PlayerAssignments).some(
      (x) => x.player === this.playerIndex + 1
    );
    if (
      !isPlayerSlot &&
      !g_GameSettings.playerAI.get(this.playerIndex) &&
      this.playerIndex >= oldNb &&
  ...
  onPlayerAssignmentsChange() {
    ...
    if (this.assignedGUID === newGUID) return;
...

{
  PlayerAssignmentItem.Client = class {
    ...
    onSelectionChange(
      ...
      if (sourcePlayer >= 0) {
        ...
        if (ai) g_GameSettings.playerAI.swap(sourcePlayer, playerIndex);
  ...
  PlayerAssignmentItem.Client.prototype.PlayerTags = { color: "white" };

  PlayerAssignmentItem.Client.prototype.ObserverTags = { color: "170 170 250" };
}

{
  ...
  PlayerAssignmentItem.AI.prototype.Label = translate("AI: %(ai)s");

  PlayerAssignmentItem.AI.prototype.Tags = { color: "70 150 70" };
}

{
  ...
  PlayerAssignmentItem.Unassigned.prototype.Label = translate("Unassigned");

  PlayerAssignmentItem.Unassigned.prototype.Tags = { color: "140 140 140" };
}

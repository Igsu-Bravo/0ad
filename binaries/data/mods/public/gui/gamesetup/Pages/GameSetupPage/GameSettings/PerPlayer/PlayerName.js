...
PlayerSettingControls.PlayerName = class PlayerName extends GameSettingControl {
  constructor(...args) {
    ...
    this.playerName = Engine.GetGUIObjectByName(
      "playerName[" + this.playerIndex + "]"
    );
  ...
  render() {
    ...
    if (g_IsNetworked) {
      let status = this.guid
        ? g_PlayerAssignments[this.guid].status
        : this.ReadyTags.length - 1;
      name = setStringTags(name, this.ReadyTags[status]);
    } else if (name && !this.guid) {
      name = translate(name);
    }
    if (name) this.playerName.caption = name;
  }
};

...
class GameSettingControlSlider extends GameSettingControl {
  constructor(...args) {
    ...
    if (this.MinValue !== undefined) this.slider.min_value = this.MinValue;

    if (this.MaxValue !== undefined) this.slider.max_value = this.MaxValue;
  }
  ...
  setControl(gameSettingControlManager) {
    ...
    this.slider = Engine.GetGUIObjectByName(
      "sliderSettingControl[" + row + "]"
    );
    this.valueLabel = Engine.GetGUIObjectByName(
      "sliderSettingLabel[" + row + "]"
    );
...
GameSettingControlSlider.prototype.UnknownValue = translateWithContext(
  "settings value",
  "Unknown"
);

ChatMessageEvents.ClientKicked = class {
  constructor(setupWindow, chatMessagesPanel) {
    ...
    setupWindow.controls.netMessages.registerNetMessageHandler(
      "kicked",
      this.onClientKicked.bind(this)
    );
  }

  onClientKicked(message) {
    this.messageArgs.username = message.username;
    this.chatMessagesPanel.addStatusMessage(
      sprintf(
...
ChatMessageEvents.ClientKicked.prototype.KickedMessage = translate(
  "%(username)s has been kicked"
);

ChatMessageEvents.ClientKicked.prototype.BannedMessage = translate(
  "%(username)s has been banned"
);

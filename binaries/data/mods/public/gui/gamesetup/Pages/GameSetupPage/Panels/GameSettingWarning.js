class GameSettingWarning {
  constructor(setupWindow, cancelButton) {
    if (!g_IsNetworked) return;

    this.gameSettingWarning = Engine.GetGUIObjectByName("gameSettingWarning");

    cancelButton.registerCancelButtonResizeHandler(
      this.onCancelButtonResize.bind(this)
    );
  ...
  onSettingsChange() {
    let caption = g_GameSettings.cheats.enabled
      ? this.CheatsEnabled
...
GameSettingWarning.prototype.CheatsEnabled = translate("Cheats enabled.");

GameSettingWarning.prototype.RatingEnabled = translate("Rated game.");

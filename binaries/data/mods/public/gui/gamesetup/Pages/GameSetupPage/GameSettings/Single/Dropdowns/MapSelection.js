GameSettingControls.MapSelection = class MapSelection extends (
  ...
  onSettingsLoaded() {
    if (this.gameSettingsController.guiData.lockSettings?.map) {
      ...
      this.gameSettingsController.guiData.mapFilter.watch(
        () => this.updateMapList(),
        ["filter"]
      );
  ...
  onHoverChange() {
    this.dropdown.tooltip =
      this.values.description[this.dropdown.hovered] || this.Tooltip;
  }

  render() {
    if (!this.enabled) {
      const mapData = this.mapCache.getMapData(
        g_GameSettings.map.mapType,
        g_GameSettings.map.map
      );
      this.label.caption =
        g_GameSettings.mapName.value || mapData.settings.Name;
      return;
    }
  ...
  updateMapList() {
    ...
    if (!g_GameSettings.map.type) return;

    {
      const values = this.mapFilters.getFilteredMaps(
      ...
      if (g_GameSettings.map.type == "random") values.unshift(this.randomItem);
  ...
  onSelectionChange(itemIdx) {
    ...
    if (this.reRenderTimeout) setNewTimerFunction(this.reRenderTimeout, call);
    else this.reRenderTimeout = setTimeout(call, 0);
  }
...
GameSettingControls.MapSelection.prototype.Tooltip = translate(
  "Select a map to play on."
);

GameSettingControls.MapSelection.prototype.RandomMapId = "random";

GameSettingControls.MapSelection.prototype.RandomMapCaption =
  translateWithContext("map selection", "Random");

GameSettingControls.MapSelection.prototype.RandomMapDescription = translate(
  "Pick any of the given maps at random."
);

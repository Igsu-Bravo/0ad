ChatMessageEvents.ClientChat = class {
  ...
  onClientChat(message) {
    ...
    this.messageArgs.username = setStringTags(
      sprintf(this.SenderFormat, this.usernameArgs),
      this.SenderTags
    );
    this.messageArgs.message = escapeText(message.text);
    this.chatMessagesPanel.addText(
      sprintf(this.MessageFormat, this.messageArgs)
    );
  }
  ...
  colorizePlayernameByGUID(guid) {
    ...
    let username = g_PlayerAssignments[guid]
      ? escapeText(g_PlayerAssignments[guid].name)
      : translate("Unknown Player");
...
ChatMessageEvents.ClientChat.prototype.MessageFormat = translate(

...
class GameSettings {
  init(mapCache) {
    if (!mapCache) mapCache = new MapCache();
    ...
    for (let comp in this) this[comp].init();
  ...
  toInitAttributes() {
    ...
    for (let comp in this)
      if (this[comp].toInitAttributes) this[comp].toInitAttributes(attribs);
  ...
  fromInitAttributes(attribs) {
    ...
    for (let comp in this)
      if (
        this[comp].fromInitAttributes &&
        comp !== "map" &&
        comp !== "playerCount" &&
        comp !== "victoryConditions"
      )
  ...
  pickRandomItems() {
    ...
    while (components.length && i < 100) {
      ...
      components = components.filter((comp) =>
        this[comp].pickRandomItems ? !!this[comp].pickRandomItems() : false
      );
      ++i;
    }
    if (i === 100) {
      throw new Error(
        "Infinite loop picking random items, remains : " + uneval(components)
      );
    }
  }
  ...
  launchGame(playerAssignments) {
    ...
    for (let guid in playerAssignments)
      if (playerAssignments[guid].player !== -1)
        this.playerName.values[playerAssignments[guid].player - 1] =
          playerAssignments[guid].name;

    // NB: for multiplayer support, the clients must be listening to "start" net messages.
    if (this.isNetworked) Engine.StartNetworkGame(this.toInitAttributes());
    else

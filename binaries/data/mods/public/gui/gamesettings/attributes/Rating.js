GameSettings.prototype.Attributes.Rating = class Rating extends GameSetting {
  ...
  toInitAttributes(attribs) {
    if (this.available) attribs.settings.RatingEnabled = this.enabled;
  }

  fromInitAttributes(attribs) {
    if (this.getLegacySetting(attribs, "RatingEnabled") !== undefined) {
      this.available =
        this.hasXmppClient && this.settings.playerCount.nbPlayers === 2;
      this.enabled =
        this.available && !!this.getLegacySetting(attribs, "RatingEnabled");
    }
  }

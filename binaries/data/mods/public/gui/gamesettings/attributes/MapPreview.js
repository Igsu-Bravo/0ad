 ...
 */
GameSettings.prototype.Attributes.MapPreview = class MapPreview extends (
  GameSetting
) {
  init() {
    this.isDefault = true;
  ...
  toInitAttributes(attribs) {
    ...
    if (this.value) attribs.mapPreview = this.value;
  }
  ...
  getPreviewForSubtype(basepath, subtype) {
    ...
    if (this.settings.mapCache.previewExists(path))
      return this.settings.mapCache.getMapPreview(
        this.settings.map.type,
  ...
  updatePreview() {
    ...
    this.value =
      this.getPreviewForSubtype(mapPath, this.settings.biome.biome) ||
      this.getLandscapePreview() ||
      this.getPreviewForSubtype(mapPath, this.settings.daytime.value) ||
      this.settings.mapCache.getMapPreview(
        this.settings.map.type,
        this.settings.map.map
      );
  }

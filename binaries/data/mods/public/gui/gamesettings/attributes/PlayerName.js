 ...
 */
GameSettings.prototype.Attributes.PlayerName = class PlayerName extends (
  GameSetting
) {
  init() {
  ...
  toInitAttributes(attribs) {
    ...
    for (let i in this.values)
      if (this.values[i]) attribs.settings.PlayerData[i].Name = this.values[i];
  }
  ...
  pickRandomItems() {
    ...
    for (let i in this.values) {
      ...
      if (!civ || civ == "random") continue;

      picked = true;
      // Pick one of the available botnames for the chosen civ
      // Determine botnames
      let chosenName = pickRandom(this.settings.civData[civ].AINames);
      if (!this.settings.isNetworked) chosenName = translate(chosenName);

      // Count how many players use the chosenName
      let usedName = this.values.filter(
        (oName) => oName && oName.indexOf(chosenName) !== -1
      ).length;

      this.values[i] = usedName
  ...
  _set(playerIndex) {
    this.values[playerIndex] =
      this._getMapData(playerIndex) ||
      (g_Settings && g_Settings.PlayerDefaults[playerIndex + 1].Name) ||
      "";
  }
};

GameSettings.prototype.Attributes.PlayerName.prototype.RomanLabel = translate(
  "%(playerName)s %(romanNumber)s"
);

GameSettings.prototype.Attributes.Biome = class Biome extends GameSetting {
  init() {
    ...
    this.biomeData = {};
    for (const biome of this.biomes) this.biomeData[biome.Id] = biome;
  ...
  fromInitAttributes(attribs) {
    if (!this.getLegacySetting(attribs, "Biome")) this.setBiome(undefined);
    else this.setBiome(this.getLegacySetting(attribs, "Biome"));
  }
  ...
  onMapChange() {
    ...
    if (
      mapData &&
      mapData.settings &&
      mapData.settings.SupportedBiomes !== undefined
    ) {
      if (mapData.settings.SupportedBiomes === this.cachedMapData) return;
      this.cachedMapData = mapData.settings.SupportedBiomes;
      this.available = new Set(
        this.biomes
          .filter(this.filterBiome(mapData.settings.SupportedBiomes))
          .map((biome) => biome.Id)
      );
      this.biome = "random";
  ...
  setBiome(biome) {
    ...
    if (this.available.size) this.biome = biome || "random";
    else this.biome = undefined;
  }

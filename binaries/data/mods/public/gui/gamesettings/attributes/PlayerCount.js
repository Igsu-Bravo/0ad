GameSettings.prototype.Attributes.PlayerCount = class PlayerCount extends (
  GameSetting
) {
  init() {
    this.nbPlayers = 1;
  ...
  fromInitAttributes(attribs) {
    ...
    if (this.nbPlayers !== pData.length) this.nbPlayers = pData.length;
  }
  ...
  onMapChange() {
    if (this.settings.map.type == "random") return;
    if (
      !this.settings.map.data ||
      !this.settings.map.data.settings ||

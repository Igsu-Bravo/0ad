 ...
 */
GameSettings.prototype.Attributes.PlayerColor = class PlayerColor extends (
  GameSetting
) {
  init() {
    this.defaultColors = g_Settings.PlayerDefaults.slice(1).map(
      (pData) => pData.Color
    );
  ...
  toInitAttributes(attribs) {
    ...
    for (let i in this.values)
      if (this.values[i]) attribs.settings.PlayerData[i].Color = this.values[i];
  }

  fromInitAttributes(attribs) {
    if (!this.getLegacySetting(attribs, "PlayerData")) return;
    let pData = this.getLegacySetting(attribs, "PlayerData");
    if (this.values.length < pData.length) this._resize(pData.length);
    for (let i in pData)
      if (pData[i] && pData[i].Color) this.setColor(i, pData[i].Color);
  }
  ...
  onMapChange() {
    ...
    this.trigger("locked");
    if (this.settings.map.type === "scenario") this._resize(0);
    this._updateAvailable();
    this.maybeUpdate();
  }
  ...
  _set(playerIndex, color) {
    let inUse = this.values.findIndex(
      (otherColor, i) => color && otherColor && sameColor(color, otherColor)
    );
    ...
    } else this.values[playerIndex] = color;
  }

  get(playerIndex) {
    if (playerIndex >= this.values.length) return undefined;
  ...
  swap(sourceIndex, targetIndex) {
    [this.values[sourceIndex], this.values[targetIndex]] = [
      this.values[targetIndex],
      this.values[sourceIndex],
    ];
    [this.locked[sourceIndex], this.locked[targetIndex]] = [
      this.locked[targetIndex],
      this.locked[sourceIndex],
    ];
    this.trigger("values");
  }
  ...
  _getMapData(i) {
    ...
    if (!data || !data.settings || !data.settings.PlayerData) return undefined;
    if (data.settings.PlayerData.length <= i) return undefined;
  ...
  _updateAvailable() {
    ...
    for (let i = 0; i < g_MaxPlayers; ++i) {
      ...
      if (col) mapColors = true;
      if (mapColors) values.push(col || this._findFarthestUnusedColor(values));
      else values.push(this.defaultColors[i]);
    }
  ...
  _getUnusedColor() {
    return this.available.find((color) => {
      return this.values.every(
        (otherColor) => !otherColor || !sameColor(color, otherColor)
      );
    });
  }
};

 ...
 */
GameSettings.prototype.Attributes.PlayerAI = class PlayerAI extends (
  GameSetting
) {
  init() {
  ...
  toInitAttributes(attribs) {
    ...
    for (let i = 0; i < this.values.length; ++i)
      ...
      } else attribs.settings.PlayerData[i].AI = false;
  }
  ...
  fromInitAttributes(attribs) {
    ...
    for (let i in pData) {
      ...
      this.set(+i, {
        ...
        difficulty:
          pData[i].AIDiff ||
          +Engine.ConfigDB_GetValue("user", "gui.gamesetup.aidifficulty"),
        behavior:
          pData[i].AIBehavior ||
          Engine.ConfigDB_GetValue("user", "gui.gamesetup.aibehavior"),
      });
    }
  }

  _resize(nb) {
    while (this.values.length > nb) this.values.pop();
    while (this.values.length < nb) this.values.push(undefined);
  }

  maybeUpdate() {
    if (this.values.length === this.settings.playerCount.nbPlayers) return;
  ...
  setAI(playerIndex, ai) {
    let old = this.values[playerIndex]
      ? this.values[playerIndex].bot
      : undefined;
    if (!ai) this.values[playerIndex] = undefined;
    else this.values[playerIndex].bot = ai;
    if (
      old !==
      (this.values[playerIndex] ? this.values[playerIndex].bot : undefined)
    )
      this.trigger("values");
  }

  setBehavior(playerIndex, value) {
    if (!this.values[playerIndex]) return;

GameSettings.prototype.Attributes.SeaLevelRise = class SeaLevelRise extends (
  GameSetting
) {
  init() {
    this.min = undefined;
  ...
  toInitAttributes(attribs) {
    if (this.value) attribs.settings.SeaLevelRiseTime = this.value;
  }
  ...
  setValue(val) {
    if (!this.getMapSetting("SeaLevelRise")) this.value = undefined;
    else this.value = Math.max(this.min, Math.min(this.max, Math.round(val)));
  }
};

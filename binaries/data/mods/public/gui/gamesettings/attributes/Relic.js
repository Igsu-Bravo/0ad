GameSettings.prototype.Attributes.Relic = class Relic extends GameSetting {
  ...
  onMapChange() {
    ...
    if (!this.getMapSetting("RelicCount")) this.available = false;
    else
      this._set(
        +this.getMapSetting("RelicCount"),
        +this.getMapSetting("RelicDuration")
      );
  }

  _set(count, duration) {
    this.available =
      this.settings.victoryConditions.active.has("capture_the_relic");

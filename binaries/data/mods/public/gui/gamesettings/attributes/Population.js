 ...
 */
GameSettings.prototype.Attributes.Population = class Population extends (
  GameSetting
) {
  init() {
    this.popDefault =
      this.getDefaultValue("PopulationCapacities", "Population") || 200;
    this.worldPopDefault =
      this.getDefaultValue("WorldPopulationCapacities", "Population") || 800;
  ...
  toInitAttributes(attribs) {
    ...
    } else attribs.settings.PopulationCap = this.cap;
  }
  ...
  fromInitAttributes(attribs) {
    if (!!this.getLegacySetting(attribs, "WorldPopulation"))
      this.setPopCap(
        true,
        this.getLegacySetting(attribs, "WorldPopulationCap")
      );
  ...
  onMapChange() {
    ...
    if (this.getMapSetting("PlayerData")?.some((data) => data.PopulationLimit))
      this.perPlayer = this.getMapSetting("PlayerData").map(
        (data) => data.PopulationLimit || undefined
      );
    else if (this.getMapSetting("WorldPopulation"))
      this.setPopCap(true, +this.getMapSetting("WorldPopulationCap"));
    else this.setPopCap(false, +this.getMapSetting("PopulationCap"));
  }

  setPopCap(worldPop, cap = undefined) {
    if (worldPop != this.useWorldPop) this.cap = undefined;

    this.useWorldPop = worldPop;

    if (!!cap) this.cap = cap;
    else if (!this.cap && !this.useWorldPop) this.cap = this.popDefault;

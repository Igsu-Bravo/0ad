...
GameSettings.prototype.Attributes.Map = class Map extends GameSetting {
  ...
  toInitAttributes(attribs) {
    ...
    if (this.script) attribs.script = this.script;
  }

  fromInitAttributes(attribs) {
    if (attribs.mapType) this.setType(attribs.mapType);

    if (!attribs.map) return;
  ...
  updateMapMetadata() {
    ...
    else this.script = undefined;
  }

  pickRandomItems() {
    if (this.map !== "random") return false;

 ...
 */
GameSettings.prototype.Attributes.PlayerCiv = class PlayerCiv extends (
  GameSetting
) {
  init() {
  ...
  toInitAttributes(attribs) {
    ...
    for (let i in this.values)
      if (this.values[i]) attribs.settings.PlayerData[i].Civ = this.values[i];
  }
  ...
  fromInitAttributes(attribs) {
    ...
    for (let i in pData)
      if (pData[i] && pData[i].Civ) this.setValue(i, pData[i].Civ);
  }
  ...
  onMapChange() {
    // Reset.
    if (
      this.settings.map.type == "scenario" ||
  ...
  pickRandomItems() {
    ...
    let cultures = Object.keys(this.settings.civData)
      .filter((civ) => this.settings.civData[civ].SelectableInGameSetup)
      .map((civ) => this.settings.civData[civ].Culture);
    cultures = cultures.filter(
      (culture, index) => cultures.indexOf(culture) === index
    );

    let picked = false;
    for (let i in this.values) {
      if (this.values[i] != "random") continue;
      picked = true;

      // Pick a random civ of a random culture
      let culture = pickRandom(cultures);
      this.values[i] = pickRandom(
        Object.keys(this.settings.civData).filter(
          (civ) =>
            this.settings.civData[civ].Culture == culture &&
            this.settings.civData[civ].SelectableInGameSetup
        )
      );
    }
    if (picked) this.trigger("values");

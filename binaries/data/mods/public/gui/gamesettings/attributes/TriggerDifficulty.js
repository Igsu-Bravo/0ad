GameSettings.prototype.Attributes.TriggerDifficulty = class TriggerDifficulty extends (
  GameSetting
) {
  init() {
  ...
  toInitAttributes(attribs) {
    if (this.available) attribs.settings.TriggerDifficulty = this.value;
  }
  ...
  getAvailableSettings() {
    return this.difficulties.filter(
      (x) => this.available.indexOf(x.Name) !== -1
    );
  }
  ...
  onMapChange() {
    ...
    this.value = this.difficulties.find(
      (x) => x.Default && this.available.indexOf(x.Name) !== -1
    ).Difficulty;
  }

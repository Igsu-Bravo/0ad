...
class HotkeyMetadata {
  ...
  parseSpec() {
    ...
    for (let file of files) {
      ...
      if (data.categories)
        for (let cat in data.categories) categories[cat] = data.categories[cat];
      if (data.hotkeys)
        for (let hotkey in data.hotkeys) {
          this.hotkeys[hotkey] = data.hotkeys[hotkey];
          this.hotkeys[hotkey].order = hotkey_i++;
          this.hotkeys[hotkey].categories = data.hotkeys[hotkey].categories || [
            this.DEFAULT_CATEGORY,
          ];
        }
        ...
        for (let cat in data.mapped_hotkeys)
          for (let hotkey in data.mapped_hotkeys[cat]) {
            if (data.mapped_hotkeys[cat][hotkey].categories)
              warn(
                "Categories will be overwritten for mapped hotkey " + hotkey
              );
...
HotkeyMetadata.prototype.DefaultCategoryString =
  markForTranslation("Other Hotkeys");

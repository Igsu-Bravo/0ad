...
class HotkeyPicker {
  constructor(metadata, onClose, name, combinations) {
    ...
    if (this.metadata.hotkeys[name]) {
      Engine.GetGUIObjectByName("hotkeyPickerTitle").caption =
        translateWithContext(
          "hotkey metadata",
          this.metadata.hotkeys[name].name
        );
      Engine.GetGUIObjectByName("hotkeyPickerDescHotkey").caption =
        translateWithContext(
          "hotkey metadata",
          this.metadata.hotkeys[name].desc
        );
    } else {
  ...
  setupCombinations() {
    for (let i = 0; i < 4; ++i) {
      ...
      Engine.GetGUIObjectByName("combNb[" + i + "]").caption = sprintf(
        translate("#%i"),
        i
      );

      if (i == this.combinations.length) this.combinations.push([]);
      ...
      Engine.GetGUIObjectByName("deleteComb[" + i + "]").onPress = (
        (j) => () => {
          this.combinations[j] = [];
          this.render();
        }
  ...
  render() {
    for (let i = 0; i < 4; ++i) {
      ...
      if (i == this.enteringInput)
        ...
        input.caption =
          formatHotkeyCombination(this.combinations[i]) ||
          translateWithContext("Unassigned hotkey", "(unused)");
      Engine.GetGUIObjectByName("conflicts[" + i + "]").caption = "";

      Engine.GetGUIObjectByName("deleteComb[" + i + "]").hidden =
        !this.combinations[i].length;
      ...
      if (conflicts.length)
        Engine.GetGUIObjectByName("conflicts[" + i + "]").caption =
          coloredText(translate("May conflict with: "), "255 153 0") +
          conflicts.join(", ");
    }
    // Gray out buttons when entering an input so it's obvious clicking on them will do nothing.
    Engine.GetGUIObjectByName("hotkeyPickerReset").enabled =
      this.enteringInput == -1;
    Engine.GetGUIObjectByName("hotkeyPickerCancel").enabled =
      this.enteringInput == -1;
    Engine.GetGUIObjectByName("hotkeyPickerSave").enabled =
      this.enteringInput == -1;
  }
}

class HotkeysPage {
  constructor(metadata) {
    ...
    Engine.GetGUIObjectByName("hotkeyList").onHoverChange = () =>
      this.onHotkeyHover();

    Engine.GetGUIObjectByName("hotkeyFilter").onSelectionChange = () =>
      this.setupHotkeyList();
    Engine.GetGUIObjectByName("hotkeyFilter").onHoverChange = () =>
      this.onFilterHover();

    Engine.GetGUIObjectByName("hotkeyTextFilter").onTextEdit = () =>
      this.setupHotkeyList();

    this.saveButton = Engine.GetGUIObjectByName("hotkeySave");
    this.saveButton.enabled = false;

    Engine.GetGUIObjectByName("hotkeyClose").onPress = () =>
      Engine.PopGuiPage();
    Engine.GetGUIObjectByName("hotkeyReset").onPress = () =>
      this.resetUserHotkeys();
  ...
  setupHotkeyData() {
    ...
    for (let name in categories) categories[name].hotkeys = [];
    // Add hotkeys defined in the metadata but not in the C++ map.
    for (let hotkeyName in this.metadata.hotkeys)
      if (!this.hotkeys[hotkeyName]) this.hotkeys[hotkeyName] = [];
    for (let hotkeyName in this.hotkeys) {
      ...
      else categories[this.metadata.DEFAULT_CATEGORY].hotkeys.push(hotkeyName);
    }
    ...
    for (let cat in categories)
      if (categories[cat].hotkeys.length === 0) delete categories[cat];
  ...
  setupFilters() {
    ...
    for (let cat in this.categories)
      names.push(
        translateWithContext("hotkey metadata", this.categories[cat].name)
      );
  ...
  setupHotkeyList() {
    ...
    let textFilter =
      Engine.GetGUIObjectByName("hotkeyTextFilter").caption.toLowerCase();
    ...
    if (dropdown.selected && dropdown.selected !== 0)
      ...
      hotkeys = Object.values(this.categories)
        .map((x) => x.hotkeys)
        .flat();
    hotkeys = hotkeys.filter((x) => {
      return (
        x.indexOf(textFilter) !== -1 ||
        translateWithContext(
          "hotkey metadata",
          this.metadata.hotkeys[x]?.name || x
        )
          .toLowerCase()
    ...
    hotkeyList.list_name = hotkeys.map((x) =>
      translateWithContext(
        "hotkey metadata",
        this.metadata.hotkeys[x]?.name || x
      )
    );
    hotkeyList.list_mapping = hotkeys.map((x) =>
      formatHotkeyCombinations(this.hotkeys[x])
    );
  ...
  onFilterHover() {
    let dropdown = Engine.GetGUIObjectByName("hotkeyFilter");
    if (dropdown.hovered === -1) dropdown.tooltip = "";
    else if (dropdown.hovered === 0)
      dropdown.tooltip = translate("All available hotkeys.");
    else
      dropdown.tooltip = translateWithContext(
        "hotkey metadata",
        this.categories[dropdown.list_data[dropdown.hovered]].desc
      );
  }

  onHotkeyHover() {
    let hotkeyList = Engine.GetGUIObjectByName("hotkeyList");
    if (hotkeyList.hovered === -1) hotkeyList.tooltip = "";
    else {
      let hotkey = hotkeyList.list_data[hotkeyList.hovered];
      hotkeyList.tooltip = this.metadata.hotkeys[hotkey]?.desc
        ? translateWithContext(
            "hotkey metadata",
            this.metadata.hotkeys[hotkey]?.desc
          )
  ...
  onHotkeyPicked(picker, success) {
    ...
    for (let cat in this.categories) {
      let idx = this.categories[cat].hotkeys.findIndex(
        ([name, _]) => name == picker.name
      );
      if (idx === -1) continue;
  ...
  saveUserHotkeys() {
    ...
    for (let hotkey in this.hotkeys) {
      ...
      if (
        keymap.join("") !==
        formatHotkeyCombinations(defaultData[hotkey], false).join("")
      )
...
HotkeysPage.prototype.UnavailableTooltipString = markForTranslation(

class TrainerSection {
  constructor(page) {
    ...
    this.TrainerSectionHeading = Engine.GetGUIObjectByName(
      "trainerSectionHeading"
    );
  ...
  draw(units, civCode) {
    ...
    this.width =
      Engine.GetTextWidth(caption.font, caption.caption) +
      (caption.size.left + caption.buffer_zone) * 2;
    let count = 0;

    for (let unitCode of units.keys()) {
      let unitTemplate = this.page.TemplateParser.getEntity(unitCode, civCode);
      if (
        !unitTemplate.production.units.length &&
        !unitTemplate.production.techs.length &&
        !unitTemplate.upgrades
      )
        continue;
      ...
      if (count > this.trainerBoxes.length) {
        error(
          '"' +
            this.activeCiv +
            '" has more unit trainers than can be supported by the current GUI layout'
        );
        break;
      }
...
TrainerSection.prototype.Caption = translate("Trainer Units");

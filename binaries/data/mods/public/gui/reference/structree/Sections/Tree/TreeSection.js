class TreeSection {
  constructor(page) {
    ...
    page.TrainerSection.registerWidthChangedHandler(
      this.onTrainerSectionWidthChange.bind(this)
    );
  }
  ...
  draw(structures, civCode) {
    if (structures.size > this.structureBoxes.length)
      error(
        '"' +
          this.activeCiv +
          '" has more structures than can be supported by the current GUI layout'
      );
    ...
    for (let idx = 0; idx < count; ++idx)
      this.structureBoxes[idx].draw(
        structureIterator.next().value,
        civCode,
        runningWidths
      );
    ...
    this.PhaseIdents.draw(
      phaseList,
      civCode,
      runningWidths,
      this.Structures.size.left
    );

    this.width =
      this.Structures.size.left +
      Math.max(...runningWidths) +
      EntityBox.prototype.HMargin;
    this.height = this.constructor.getPositionOffset(
      phaseList.length,
      this.page.TemplateParser
    );
  }
  ...
  drawPhaseIcon(phaseIcon, phaseIndex, civCode) {
    ...
    let prodPhaseTemplate =
      this.page.TemplateParser.getTechnology(
        phaseName + "_" + civCode,
        civCode
      ) || this.page.TemplateParser.getTechnology(phaseName, civCode);

    phaseIcon.sprite =
      "stretched:" + this.page.IconPath + prodPhaseTemplate.icon;

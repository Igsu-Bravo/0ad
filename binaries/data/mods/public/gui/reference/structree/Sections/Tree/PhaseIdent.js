class PhaseIdent {
  constructor(page, phaseIdx) {
    ...
    this.Ident = Engine.GetGUIObjectByName(
      "phase[" + this.phaseIdx + "]_ident"
    );
  ...
  draw(phaseList, barLength, civCode) {
    ...
    identSize.top = TreeSection.getPositionOffset(
      this.phaseIdx,
      this.page.TemplateParser
    );
    identSize.bottom = TreeSection.getPositionOffset(
      this.phaseIdx + 1,
      this.page.TemplateParser
    );

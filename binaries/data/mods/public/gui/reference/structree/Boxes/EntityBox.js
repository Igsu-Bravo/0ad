...
class EntityBox {
  ...
  draw(templateName, civCode) {
    ...
    this.constructor.setViewerOnPress(
      icon,
      this.template.name.internal,
      civCode
    );
  }
  ...
  static compileTooltip(template) {
    return (
      ReferencePage.buildText(template, this.prototype.TooltipFunctions) +
      "\n" +
  ...
  static IconAndCaptionHeight() {
    let height =
      Engine.GetGUIObjectByName("structure[0]_icon").size.bottom +
      this.prototype.IconPadding;

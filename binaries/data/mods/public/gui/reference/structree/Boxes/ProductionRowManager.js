  ...
  constructor(page, guiName, sortByPhase) {
    ...
    for (let row of Engine.GetGUIObjectByName(guiName).children)
      this.productionRows.push(
        new ProductionRow(this.page, row, this.productionRows.length)
      );
  }
  ...
  draw(template, civCode, phaseIdx = 0) {
    ...
    if (this.sortProductionsByPhase)
      for (let r = 0; r < this.page.TemplateParser.phaseList.length; ++r)
        this.productionRows[r].startDraw(
          this.page.TemplateParser.phaseList.length - phaseIdx
        );
    else this.productionRows[0].startDraw(1);
    ...
    for (let prodType of Object.keys(template.production).reverse())
      for (let prod of template.production[prodType]) {
        ...
        switch (prodType) {
          ...
          case "techs":
            pIdx = this.page.TemplateParser.phaseList.indexOf(
              this.page.TemplateParser.getPhaseOfTechnology(prod, civCode)
            );
        ...
        let rowIdx = this.sortProductionsByPhase
          ? Math.max(0, pIdx - phaseIdx)
          : 0;
    ...
    else this.width = this.productionRows[r++].finishDraw();

    // Hide any remaining phase rows
    for (; r < this.productionRows.length; ++r) this.productionRows[r].hide();
  }
}

...
class StructureBox extends EntityBox {
  constructor(page, structureIdx) {
    ...
    this.ProductionRows = new ProductionRowManager(
      this.page,
      "structure[" + structureIdx + "]_productionRows",
      true
    );
  }
  ...
  draw(templateName, civCode, runningWidths) {
    ...
    this.phaseIdx = this.page.TemplateParser.phaseList.indexOf(
      this.template.phase
    );
    ...
    size.top = TreeSection.getPositionOffset(
      this.phaseIdx,
      this.page.TemplateParser
    );
    size.bottom =
      TreeSection.getPositionOffset(
        this.phaseIdx + 1,
        this.page.TemplateParser
      ) - this.VMargin;
    this.gui.size = size;

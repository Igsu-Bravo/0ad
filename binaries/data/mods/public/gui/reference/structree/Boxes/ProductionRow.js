class ProductionRow {
  constructor(page, guiObject, rowIndex) {
    ...
    horizontallySpaceObjects(
      this.productionRow.name,
      ProductionIcon.Size().hMargin
    );
  ...
  drawIcon(template, civCode) {
    if (this.productionIconsDrawn == this.productionIcons.length) {
      error(
        "The currently displayed civ has more production options " +
  ...
  finishDraw() {
    ...
    let rowWidth =
      this.productionIconsDrawn * IconSize.rowWidth + IconSize.hMargin;

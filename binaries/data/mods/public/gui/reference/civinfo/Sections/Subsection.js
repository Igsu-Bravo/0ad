class Subsection {
  ...
  getEntityCaptions(entityList, classList, civCode) {
    ...
    for (let entityCode of entityList) {
      ...
      let template = this.page.TemplateLoader.loadEntityTemplate(
        entityCode,
        civCode
      );
      let classListFull = GetIdentityClasses(template.Identity);
      if (!MatchesClassList(classListFull, classList)) continue;
  ...
  getTechnologyCaptions(technologyList, civCode) {
    ...
    for (let techCode of technologyList) {
      let technology = this.page.TemplateParser.getTechnology(
        techCode,
        civCode
      );
      ...
      let genericReqs = this.page.TemplateParser.getTechnology(
        techCode,
        "anyciv"
      ).reqs;

      if (!technology.reqs || genericReqs) continue;

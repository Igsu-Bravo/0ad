class CivInfoPage extends ReferencePage {
  ...
  selectCiv(civCode) {
    ...
    if (!civInfo)
      error(
        sprintf('Error loading civ data for "%(code)s"', { code: civCode })
      );
  ...
  bigFirstLetter(text, size) {
    return (
      setStringTags(text[0], { font: "sans-bold-" + (size + 6) }) +
      text.substring(1)
    );
  }
  ...
  formatEntry(name, tooltip, description) {
    ...
    if (tooltip)
      tooltip_icon =
        '[icon="iconInfo" tooltip="' +
        escapeQuotation(tooltip) +
...
CivInfoPage.prototype.CloseButtonTooltip = translate(
  "%(hotkey)s: Close Civilization Overview."
);

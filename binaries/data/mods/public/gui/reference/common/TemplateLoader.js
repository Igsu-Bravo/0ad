...
class TemplateLoader {
  ...
  loadEntityTemplate(templateName, civCode) {
    if (!(templateName in this.templateData)) {
      ...
      if (
        data.Identity.Civ != this.DefaultCiv &&
        civCode != this.DefaultCiv &&
        data.Identity.Civ != civCode
      )
        warn(
          'The "' +
            templateName +
            '" template has a defined civ of "' +
            data.Identity.Civ +
            '". ' +
            'This does not match the currently selected civ "' +
            civCode +
            '".'
        );
  ...
  loadTechnologyTemplate(templateName) {
    if (!(templateName in this.technologyData)) {
      let data = Engine.ReadJSONFile(
        this.TechnologyPath + templateName + ".json"
      );
      ...
      if (typeof data.specificName === "object")
        ...
        warn(
          "specificName should be an object of civ->name mappings in " +
            templateName +
            ".json"
        );
  ...
  deriveProductionQueue(template, civCode) {
    ...
    if (!template.ProductionQueue) return production;

    if (
      template.ProductionQueue.Entities &&
      template.ProductionQueue.Entities._string
    )
    ...
    if (
      template.ProductionQueue.Technologies &&
      ...
      for (let technologyName of template.ProductionQueue.Technologies._string.split(
        ...
        if (technologyName.indexOf("{civ}") != -1) {
          ...
          technologyName = TechnologyTemplateExists(civTechName)
            ? civTechName
            : technologyName.replace("{civ}", "generic");
        }

        if (this.isPairTech(technologyName)) {
          let technologyPair = this.loadTechnologyPairTemplate(
            technologyName,
            civCode
          );
        ...
        } else appendTechnology(technologyName);
      }
  ...
  deriveBuildQueue(template, civCode) {
    ...
    for (let build of template.Builder.Entities._string.split(" ")) {
      ...
      if (Engine.TemplateExists(build)) buildQueue.push(build);
    }
    ...
    for (const techName of this.autoResearchTechList)
      modificationData.push(
        GetTechnologyBasicDataHelper(
          this.loadTechnologyTemplate(techName),
          civCode
        )
      );
  ...
  buildPlayerTemplateName(civCode) {
    ...
    if (Engine.TemplateExists(templateName)) return templateName;
  ...
  findAllAutoResearchedTechs() {
    ...
    for (let templateName of listFiles(this.TechnologyPath, ".json", true)) {
      ...
      if (data && data.autoResearch) techList.push(templateName);
    }
    return techList;
  }
  ...
  getVariantBaseAndType(templateName, civCode) {
    if (!templateName || !Engine.TemplateExists(templateName)) return undefined;
    ...
    if (parentTemplate.Identity) {
      if (
        parentTemplate.Identity.Civ &&
        parentTemplate.Identity.Civ != template.Identity.Civ
      )
        return [templateName, TemplateVariant.base];

      if (
        parentTemplate.Identity.Rank &&
        parentTemplate.Identity.Rank != template.Identity.Rank
      )
        return [
          inheritedVariance[0],
          TemplateVariant.promotion,
          template.Identity.Rank.toLowerCase(),
        ];
    }
    ...
    if (template.Identity.RequiredTechnology)
      return [
        inheritedVariance[0],
        TemplateVariant.unlockedByTechnology,
...
TemplateLoader.prototype.EntityTranslateKeys = [
  "GenericName",
  "SpecificName",
  "Tooltip",
  "History",
];
TemplateLoader.prototype.TechnologyTranslateKeys = [
  "genericName",
  "tooltip",
  "description",
];

...
class TemplateLister {
  ...
  compileTemplateLists(civCode, civData) {
    if (this.hasTemplateLists(civCode)) return this.templateLists.get(civCode);

    let templatesToParse = civData[civCode].StartEntities.map(
    ...
    do {
      ...
      for (let templateBeingParsed of templatesThisIteration) {
        let baseOfTemplateBeingParsed =
          this.TemplateLoader.getVariantBaseAndType(
            templateBeingParsed,
            civCode
          )[0];
        let list = this.deriveTemplateListsFromTemplate(
          templateBeingParsed,
          civCode
        );
        for (let type in list)
          for (let templateName of list[type]) {
            ...
            if (!templateLists[type].has(templateName)) {
              templateLists[type].set(templateName, [
                baseOfTemplateBeingParsed,
              ]);
              if (type != "techs") templatesToParse.push(templateName);
            } else if (
              templateLists[type]
                .get(templateName)
                .indexOf(baseOfTemplateBeingParsed) == -1
            )
              templateLists[type]
                .get(templateName)
                .push(baseOfTemplateBeingParsed);
          }
      }
    ...
    for (let [techCode, researcherList] of templateLists.techs) {
      if (!this.TemplateLoader.isPairTech(techCode)) continue;

      for (let subTech of this.TemplateLoader.loadTechnologyPairTemplate(
        techCode,
        civCode
      ).techs)
  ...
  getTemplateLists(civCode) {
    ...
    error(
      'Template lists of "' +
        civCode +
        '" requested, but this civ has not been loaded.'
    );
    return {};
  }
  ...
  deriveTemplateListsFromTemplate(templateName, civCode) {
    if (!templateName || !Engine.TemplateExists(templateName)) return {};
    ...
    let templateLists = this.TemplateLoader.deriveProductionQueue(
      template,
      civCode
    );
    templateLists.structures = this.TemplateLoader.deriveBuildQueue(
      template,
      civCode
    );
    ...
    if (template.WallSet) {
      ...
      for (let segment in template.WallSet.Templates) {
        segment = template.WallSet.Templates[segment].replace(
          /\{(civ|native)\}/g,
          civCode
        );

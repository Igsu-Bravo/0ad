...
class TemplateParser {
  ...
  getAura(auraName) {
    if (auraName in this.auras) return this.auras[auraName];

    if (!AuraTemplateExists(auraName)) return null;

    let template = this.TemplateLoader.loadAuraTemplate(auraName);
    let parsed = GetAuraDataHelper(template);

    if (template.civ) parsed.civ = template.civ;

    let affectedPlayers =
      template.affectedPlayers || this.AuraAffectedPlayerDefault;
    parsed.affectsTeam = this.AuraTeamIndicators.some((indicator) =>
      affectedPlayers.includes(indicator)
    );
    parsed.affectsSelf = this.AuraSelfIndicators.some((indicator) =>
      affectedPlayers.includes(indicator)
    );
  ...
  getEntity(templateName, civCode) {
    if (!(civCode in this.entities)) this.entities[civCode] = {};
    else if (templateName in this.entities[civCode])
      return this.entities[civCode][templateName];

    if (!Engine.TemplateExists(templateName)) return null;

    let template = this.TemplateLoader.loadEntityTemplate(
      templateName,
      civCode
    );
    let parsed = GetTemplateDataHelper(
      template,
      null,
      this.TemplateLoader.auraData,
      this.modifiers[civCode] || {}
    );
    parsed.name.internal = templateName;

    parsed.history = template.Identity.History;

    parsed.production = this.TemplateLoader.deriveProductionQueue(
      template,
      civCode
    );
    if (template.Builder)
    ...
    if (!parsed.requiredTechnology) parsed.phase = this.phaseList[0];
      ...
      parsed.phase = this.getPhaseOfTechnology(
        parsed.requiredTechnology,
        civCode
      );
    ...
    if (parsed.wallSet) {
      ...
      for (let wSegm in parsed.wallSet.templates) {
        if (wSegm == "fort" || wSegm == "curves") continue;
        ...
        if (["gate", "tower"].indexOf(wSegm) != -1) continue;
      ...
      if (health.min == health.max) parsed.health = health.min;
      else
  ...
  getTechnology(technologyName, civCode) {
    if (!TechnologyTemplateExists(technologyName)) return null;

    if (
      this.TemplateLoader.isPhaseTech(technologyName) &&
      technologyName in this.phases
    )
      return this.phases[technologyName];

    if (!(civCode in this.techs)) this.techs[civCode] = {};
    ...
    if (template.pair !== undefined) {
      ...
      tech.reqs = this.mergeRequirements(
        tech.reqs,
        this.TemplateLoader.loadTechnologyPairTemplate(template.pair).reqs
      );
    }

    if (this.TemplateLoader.isPhaseTech(technologyName)) {
      tech.actualPhase = technologyName;
      if (tech.replaces !== undefined) tech.actualPhase = tech.replaces[0];
      this.phases[technologyName] = tech;
    } else this.techs[civCode][technologyName] = tech;
    return tech;
  }
   ...
   */
  getPlayer(civCode) {
    if (civCode in this.players) return this.players[civCode];
  ...
  getActualUpgradeData(upgradesInfo, civCode) {
    ...
    for (let upgrade of upgradesInfo) {
      ...
      let data = GetTemplateDataHelper(
        this.TemplateLoader.loadEntityTemplate(upgrade.entity, civCode),
        null,
        this.TemplateLoader.auraData,
        this.modifiers[civCode] || {}
      );
      ...
      data.requiredTechnology =
        upgrade.requiredTechnology || data.requiredTechnology;

      if (!data.requiredTechnology) data.phase = this.phaseList[0];
      else if (this.TemplateLoader.isPhaseTech(data.requiredTechnology))
        data.phase = this.getActualPhase(data.requiredTechnology);
      else
        data.phase = this.getPhaseOfTechnology(
          data.requiredTechnology,
          civCode
        );
  ...
  getPhaseOfTechnology(techName, civCode) {
    ...
    if (basename(techName).startsWith("phase")) {
      if (!this.phases[techName].reqs) return false;

      phaseIdx = this.phaseList.indexOf(this.getActualPhase(techName));
      if (phaseIdx > 0) return this.phaseList[phaseIdx - 1];
    }

    let techReqs = this.getTechnology(techName, civCode).reqs;
    if (!techReqs) return false;

    for (let option of techReqs)
      if (option.techs)
        for (let tech of option.techs) {
          if (basename(tech).startsWith("phase")) return tech;
          if (basename(tech).startsWith("pair")) continue;
          phaseIdx = Math.max(
            phaseIdx,
            this.phaseList.indexOf(this.getPhaseOfTechnology(tech, civCode))
          );
        }
  ...
  getActualPhase(phaseName) {
    if (this.phases[phaseName]) return this.phases[phaseName].actualPhase;
  ...
  deriveModifications(civCode) {
    ...
    for (const bonusname of player.teambonuses)
      if (this.getAura(bonusname).affectsSelf) auraList.push(bonusname);

    this.modifiers[civCode] = this.TemplateLoader.deriveModifications(
      civCode,
      auraList
    );
  }
    ...
    for (let option of reqsB)
      for (let type in option)
        for (let opt in finalReqs) {
          if (!finalReqs[opt][type]) finalReqs[opt][type] = [];
...
TemplateParser.prototype.AuraAffectedPlayerDefault = ["Player"];
...
TemplateParser.prototype.AuraSelfIndicators = ["Player", "Ally", "MutualAlly"];

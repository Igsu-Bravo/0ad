...
class ViewerPage extends ReferencePage {
  ...
  selectTemplate(data) {
    ...
    if (!isTech)
      this.setActiveCiv(
        this.TemplateLoader.loadEntityTemplate(
          templateName,
          this.TemplateLoader.DefaultCiv
        ).Identity.Civ
      );
    ...
    this.currentTemplate = isTech
      ? this.TemplateParser.getTechnology(templateName, this.activeCiv)
      : this.TemplateParser.getEntity(templateName, this.activeCiv);
    if (!this.currentTemplate) {
      error(
        "Viewer: unable to recognize or load template (" + templateName + ")"
      );
      this.closePage();
      return;
    }
    ...
    if (this.activeCiv != this.TemplateLoader.DefaultCiv) {
      ...
      if (!isTech) {
        ...
        let templateVariance = this.TemplateLoader.getVariantBaseAndType(
          this.currentTemplate.name.internal,
          this.TemplateLoader.DefaultCiv
        );
      ...
      if (builders && builders.length)
        this.currentTemplate.builtByListOfNames = builders.map((builder) =>
          getEntityNames(this.TemplateParser.getEntity(builder, this.activeCiv))
        );

      let trainers = templateLists.units.get(currentTemplateName);
      if (trainers && trainers.length)
        this.currentTemplate.trainedByListOfNames = trainers.map((trainer) =>
          getEntityNames(this.TemplateParser.getEntity(trainer, this.activeCiv))
        );

      let researchers = templateLists.techs.get(currentTemplateName);
      if (researchers && researchers.length)
        this.currentTemplate.researchedByListOfNames = researchers.map(
          (researcher) =>
            getEntityNames(
              this.TemplateParser.getEntity(researcher, this.activeCiv)
            )
        );
    }

    if (this.currentTemplate.builder && this.currentTemplate.builder.length)
      this.currentTemplate.buildListOfNames = this.currentTemplate.builder.map(
        (prod) =>
          getEntityNames(this.TemplateParser.getEntity(prod, this.activeCiv))
      );

    if (this.currentTemplate.production) {
      if (
        this.currentTemplate.production.units &&
        this.currentTemplate.production.units.length
      )
        this.currentTemplate.trainListOfNames =
          this.currentTemplate.production.units.map((prod) =>
            getEntityNames(this.TemplateParser.getEntity(prod, this.activeCiv))
          );

      if (
        this.currentTemplate.production.techs &&
        this.currentTemplate.production.techs.length
      ) {
        this.currentTemplate.researchListOfNames = [];
        for (let tech of this.currentTemplate.production.techs) {
          let techTemplate = this.TemplateParser.getTechnology(
            tech,
            this.activeCiv
          );
          if (techTemplate.reqs)
            this.currentTemplate.researchListOfNames.push(
              getEntityNames(techTemplate)
            );
        }
      }
    }
    ...
    if (this.currentTemplate.upgrades)
      this.currentTemplate.upgradeListOfNames =
        this.currentTemplate.upgrades.map((upgrade) =>
          getEntityNames(
            upgrade.name
              ? upgrade
              : this.TemplateParser.getEntity(upgrade.entity, this.activeCiv)
          )
        );
   ...
   */
  draw() {
    this.guiElements.entityName.caption = getEntityNamesFormatted(
      this.currentTemplate
    );

    let entityIcon = this.guiElements.entityIcon;
    entityIcon.sprite =
      "stretched:" + this.IconPath + this.currentTemplate.icon;

    let entityStats = this.guiElements.entityStats;
    entityStats.caption = this.constructor.buildText(
      this.currentTemplate,
      this.StatsFunctions
    );
    ...
    infoSize.top = Math.max(
      entityIcon.size.bottom + 8,
      entityStats.size.top + entityStats.getTextSize().height
    );
    this.guiElements.entityInfo.size = infoSize;

    this.guiElements.entityInfo.caption = this.constructor.buildText(
      this.currentTemplate,
      this.InfoFunctions,
      "\n\n"
    );
    ...
    if (this.currentTemplate.promotion)
      this.guiElements.entityRankGlyph.sprite =
        "stretched:" +
        this.RankIconPath +
        this.currentTemplate.promotion.current_rank +
        ".png";
 ...
 * on the page.
 */
ViewerPage.prototype.StatsFunctions = [getEntityCostTooltip].concat(
...
ViewerPage.prototype.CloseButtonTooltip = translate(
  "%(hotkey)s: Close Template Viewer"
);

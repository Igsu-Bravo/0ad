function checkUsername(register) {
  ...
  if (!username) return translate("Please enter your username");
...
function checkPasswordConfirmation() {
  ...
  if (!password1) return translate("Please enter your password again");

  let password2 = Engine.GetGUIObjectByName("passwordRepeat").caption;
  if (password1 != password2) return translate("Passwords do not match");
...
function toggleRememberPassword() {
  ...
  if (
    ...
    messageBox(
      ...
      160,
      translate(
        "Are you sure you want to delete the password after connecting?"
      ),
...
function getEncryptedPassword() {
  ...
  let storedEncryptedPassword = Engine.ConfigDB_GetValue(
    "user",
    "lobby.password"
  );

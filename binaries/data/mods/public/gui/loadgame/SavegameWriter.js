...
class SavegameWriter {
  ...
  saveGame(gameID, label) {
    ...
    messageBox(
      ...
      [
        null,
        () => {
          this.reallySaveGame(name, desc, false);
        },
      ]
    );
  }
  ...
  reallySaveGame(name, desc, nameIsPrefix) {
    ...
    this.savedGameData.states = simulationState.players.map(
      (pState) => pState.state
    );

    if (nameIsPrefix) Engine.SaveGamePrefix(name, desc, this.savedGameData);
    else Engine.SaveGame(name, desc, this.savedGameData);

...
class CampaignMenu extends AutoWatcher {
  constructor(campaignRun) {
    ...
    this.levelSelection.onSelectionChange = () => {
      this.selectedLevel = this.levelSelection.selected;
    };
  ...
  startScenario() {
    ...
    if (!meetsRequirements(this.run, level)) return;
    ...
    const assignments = {
      local: {
        player: 1,
        name:
          Engine.ConfigDB_GetValue("user", "playername.singleplayer") ||
    ...
    if (level.Preview)
      gameSettings.mapPreview.setCustom(
        "cropped:" + 400 / 512 + "," + 300 / 512 + ":" + level.Preview
      );
    ...
    if (level.useGameSetup) {
      ...
      for (let i = 1; i < gameSettings.playerCount.nbPlayers; ++i)
        gameSettings.playerAI.set(i, {
          ...
          difficulty: +Engine.ConfigDB_GetValue(
            "user",
            "gui.gamesetup.aidifficulty"
          ),
          behavior: Engine.ConfigDB_GetValue(
            "user",
  ...
  getSelectedLevelData() {
    if (this.selectedLevel === -1) return undefined;
    return this.run.template.Levels[
      this.levelSelection.list_data[this.selectedLevel]
    ];
  }

  shouldShowLevel(levelData) {
    if (this.run.template.ShowUnavailable) return true;
  ...
  getLevelName(levelData) {
    ...
    return translate(
      this.mapCache.getTranslatableMapName(
        levelData.MapType,
        "maps/" + levelData.Map
      )
    );
  ...
  getLevelDescription(levelData) {
    ...
    return this.mapCache.getTranslatedMapDescription(
      levelData.MapType,
      "maps/" + levelData.Map
    );
  }
  ...
  getLevelPreview(levelData) {
    ...
    return this.mapCache.getMapPreview(
      levelData.MapType,
      "maps/" + levelData.Map
    );
  }
  ...
  displayLevelsList() {
    ...
    for (let key in this.run.template.Levels) {
      ...
      if (!this.shouldShowLevel(level)) continue;
      ...
      if (isCompleted(this.run, key))
        ...
        status = coloredText(
          translateWithContext("campaign status", "Available"),
          "green"
        );
      else name = coloredText(name, "gray");
    ...
    list.sort(
      (a, b) =>
        this.run.template.Order.indexOf(a.ID) -
        this.run.template.Order.indexOf(b.ID)
    );
  ...
  displayLevelDetails() {
    ...
    Engine.GetGUIObjectByName("scenarioName").caption =
      this.getLevelName(level);
    Engine.GetGUIObjectByName("scenarioDesc").caption =
      this.getLevelDescription(level);
    Engine.GetGUIObjectByName("levelPreviewBox").sprite =
      this.getLevelPreview(level);

    Engine.GetGUIObjectByName("startButton").enabled = meetsRequirements(
      this.run,
      level
    );
    Engine.GetGUIObjectByName("startButton").hidden = false;
...
function init(initData) {
  ...
  try {
    ...
    error(
      sprintf(
        translate("Error loading campaign run %s: %s."),

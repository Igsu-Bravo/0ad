...
function markLevelComplete(run, levelID) {
  if (!isCompleted(run, levelID)) {
    if (!run.data.completedLevels) run.data.completedLevels = [];
...
function isCompleted(run, levelID) {
  return (
    run.data.completedLevels && run.data.completedLevels.indexOf(levelID) !== -1
  );
}

function meetsRequirements(run, levelData) {
  if (!levelData.Requires) return true;

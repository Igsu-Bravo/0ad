...
class BrokenRun {
  ...
  getLabel(forList) {
    if (!forList) return this.filename + ".0adcampaign";
    return coloredText(
      sprintf("%(filename)s (%(error)s)", {
...
class LoadModal extends AutoWatcher {
  constructor(campaignTemplate) {
    ...
    Engine.GetGUIObjectByName("deleteGameButton").onPress = () =>
    ...
    this.runSelection.onSelectionChange = () => {
      ...
      if (this.selectedRun === -1)
        ...
        Engine.GetGUIObjectByName("runDescription").caption =
          this.currentRuns[this.selectedRun].getLabel();
    };
  ...
  getRuns() {
    let out = [];
    let files = Engine.ListDirectoryFiles(
      "saves/campaigns/",
      "*.0adcampaign",
      false
    );
  ...
  loadCampaign() {
    ...
    let run = new CampaignRun(filename).load().setCurrent();
  ...
  deleteSelectedRun() {
    ...
    messageBox(
      ...
      sprintf(
        translate(
          "Are you sure you want to delete run %s? This cannot be undone."
        ),
        run.getLabel()
      ),
  ...
  render() {
    ...
    Engine.GetGUIObjectByName("deleteGameButton").enabled =
      this.selectedRun !== -1;
    Engine.GetGUIObjectByName("startButton").enabled =
      this.selectedRun !== -1 &&
      this.currentRuns[this.selectedRun] instanceof CampaignRun;

...
function loadReplays(replaySelectionData, compareFiles) {
  ...
  for (let replay of g_Replays) {
    ...
    // Extract map names
    if (
      g_MapNames.indexOf(replay.attribs.settings.Name) == -1 &&
    ...
    for (let playerData of replay.attribs.settings.PlayerData) {
      if (!playerData || playerData.AI) continue;

      // Remove rating from nick
      let playername = playerData.Name;
      let ratingStart = playername.indexOf(" (");
      if (ratingStart != -1) playername = playername.substr(0, ratingStart);
    ...
    if (replay.attribs.campaignData) replay.isCampaign = true;
...
function sanitizeInitAttributes(attribs) {
  if (!attribs.settings) attribs.settings = {};

  if (!attribs.settings.Size) attribs.settings.Size = -1;

  if (!attribs.settings.Name) attribs.settings.Name = "";

  if (!attribs.settings.PlayerData) attribs.settings.PlayerData = [];

  if (!attribs.settings.PopulationCap) attribs.settings.PopulationCap = 300;

  if (!attribs.mapType) attribs.mapType = "skirmish";

  // Remove gaia
  if (
    attribs.settings.PlayerData.length &&
    attribs.settings.PlayerData[0] == null
  )
...
function initHotkeyTooltips() {
  ...
  if (deleteTooltip)
    deleteTooltip += colorizeHotkey(
      "\n" +
        translate(
          "Hold %(hotkey)s to skip the confirmation dialog while deleting."
        ),
...
function displayReplayList() {
  ...
  if (replaySelection.selected != -1)
    g_SelectedReplayDirectory =
      g_ReplaysFiltered[replaySelection.selected].directory;
  ...
  if (list.length) list = prepareForDropdown(list);
  ...
  replaySelection.selected = replaySelection.list.findIndex(
    (directory) => directory == g_SelectedReplayDirectory
  );
...
function displayReplayDetails() {
  ...
  Engine.GetGUIObjectByName("sgMapName").caption = translate(
    replay.attribs.settings.Name
  );
  Engine.GetGUIObjectByName("sgMapSize").caption = translateMapSize(
    replay.attribs.settings.Size
  );
  Engine.GetGUIObjectByName("sgMapType").caption = translateMapType(
    replay.attribs.mapType
  );
  Engine.GetGUIObjectByName("sgVictory").caption =
    replay.attribs.settings.VictoryConditions.map((victoryConditionName) =>
      translateVictoryCondition(victoryConditionName)
    ).join(translate(", "));
  Engine.GetGUIObjectByName("sgNbPlayers").caption = sprintf(
    translate("Players: %(numberOfPlayers)s"),
    { numberOfPlayers: replay.attribs.settings.PlayerData.length }
  );
  Engine.GetGUIObjectByName("replayFilename").caption =
    Engine.GetReplayDirectoryName(replay.directory);

  let metadata = Engine.GetReplayMetadata(replay.directory);
  Engine.GetGUIObjectByName("sgPlayersNames").caption = formatPlayerInfo(
  ...
  Engine.GetGUIObjectByName("sgMapPreview").sprite = g_MapCache.getMapPreview(
    replay.attribs.mapType,
    replay.attribs.map,
    replay.attribs?.mapPreview
  );
  Engine.GetGUIObjectByName("sgMapDescription").caption =
    g_MapCache.getTranslatedMapDescription(
      replay.attribs.mapType,
      replay.attribs.map
    );

  Engine.GetGUIObjectByName("summaryButton").hidden = !Engine.HasReplayMetadata(
    replay.directory
  );
}
...
function getReplayDateTime(replay) {
  return Engine.FormatMillisecondsIntoDateStringLocal(
    replay.attribs.timestamp * 1000,
    translate("yyyy-MM-dd HH:mm")
  );
}
...
function isReplayCompatible(replay) {
  return (
    replayHasSameEngineVersion(replay) &&
    hasSameMods(replay.attribs.mods, g_EngineInfo.mods)
  );
}
...
function replayHasSameEngineVersion(replay) {
  return (
    replay.attribs.engine_version &&
    replay.attribs.engine_version == g_EngineInfo.engine_version
  );
}

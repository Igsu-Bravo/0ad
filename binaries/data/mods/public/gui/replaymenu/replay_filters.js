...
function initFilters(filters) {
  if (filters && filters.compatibility !== undefined)
    Engine.GetGUIObjectByName("compatibilityFilter").checked =
...
function initDateFilter(filters) {
  ...
  months = months
    .filter((month, index) => months.indexOf(month) == index)
    .sort();

  var dateTimeFilter = Engine.GetGUIObjectByName("dateTimeFilter");
  dateTimeFilter.list = [translateWithContext("datetime", "Any")].concat(
    months
  );
  ...
  if (
    dateTimeFilter.selected == -1 ||
    dateTimeFilter.selected >= dateTimeFilter.list.length
  )
...
function initMapSizeFilter(filters) {
  ...
  mapSizeFilter.list = [translateWithContext("map size", "Any")].concat(
    g_MapSizes.Name
  );
  ...
  if (
    mapSizeFilter.selected == -1 ||
    mapSizeFilter.selected >= mapSizeFilter.list.length
  )
...
function initMapNameFilter(filters) {
  ...
  mapNameFilter.list = [translateWithContext("map name", "Any")].concat(
    g_MapNames.map((name) => translate(name))
  );
  ...
  if (
    mapNameFilter.selected == -1 ||
    mapNameFilter.selected >= mapNameFilter.list.length
  )
...
function initPopCapFilter(filters) {
  ...
  populationFilter.list = [
    translateWithContext("population capacity", "Any"),
  ].concat(g_PopulationCapacities.Title);
  populationFilter.list_data = [""].concat(g_PopulationCapacities.Population);

  if (filters && filters.popCap)
    populationFilter.selected = populationFilter.list_data.indexOf(
      filters.popCap
    );

  if (
    populationFilter.selected == -1 ||
    populationFilter.selected >= populationFilter.list.length
  )
...
function initDurationFilter(filters) {
  ...
  durationFilter.list = g_DurationFilterIntervals.map((interval, index) => {
    if (index == 0) return translateWithContext("duration", "Any");
    ...
    return sprintf(
      translateWithContext("duration filter", "%(min)s - %(max)s min"),
      interval
    );
  });
  durationFilter.list_data = g_DurationFilterIntervals.map(
    (interval, index) => index
  );

  if (filters && filters.duration)
    durationFilter.selected = durationFilter.list_data.indexOf(
      filters.duration
    );

  if (
    durationFilter.selected == -1 ||
    durationFilter.selected >= g_DurationFilterIntervals.length
  )
...
function initSingleplayerFilter(filters) {
  ...
  if (filters && filters.singleplayer)
    singleplayerFilter.selected = singleplayerFilter.list_data.indexOf(
      filters.singleplayer
    );

  if (
    singleplayerFilter.selected < 0 ||
    singleplayerFilter.selected >= singleplayerFilter.list.length
  )
    singleplayerFilter.selected = 0;
}

function initVictoryConditionFilter(filters) {
  let victoryConditionFilter = Engine.GetGUIObjectByName(
    "victoryConditionFilter"
  );
  victoryConditionFilter.list = [translate("Any victory condition")].concat(
    g_VictoryConditions.map((victoryCondition) =>
      translateVictoryCondition(victoryCondition.Name)
    )
  );
  victoryConditionFilter.list_data = [""].concat(
    g_VictoryConditions.map((victoryCondition) => victoryCondition.Name)
  );

  if (filters && filters.victoryCondition)
    victoryConditionFilter.selected = victoryConditionFilter.list_data.indexOf(
      filters.victoryCondition
    );

  if (
    victoryConditionFilter.selected < 0 ||
    victoryConditionFilter.selected >= victoryConditionFilter.list.length
  )
    victoryConditionFilter.selected = 0;
}
...
function initRatedGamesFilter(filters) {
  ...
  ratedGamesFilter.list = [
    translate("Rated and unrated games"),
    translate("Rated games"),
    translate("Unrated games"),
  ];
  ratedGamesFilter.list_data = ["", "rated", "not rated"];

  if (filters && filters.ratedGames)
    ratedGamesFilter.selected = ratedGamesFilter.list_data.indexOf(
      filters.ratedGames
    );

  if (
    ratedGamesFilter.selected < 0 ||
    ratedGamesFilter.selected >= ratedGamesFilter.list.length
  )
    ratedGamesFilter.selected = 0;
}
...
function filterReplays() {
  ...
  let sortOrder =
    Engine.GetGUIObjectByName("replaySelection").selected_column_order;

  g_ReplaysFiltered = g_Replays
    .filter((replay) => filterReplay(replay))
    .sort((a, b) => {
      let cmpA, cmpB;
      switch (sortKey) {
        case "months":
      ...
      if (cmpA < cmpB) return -sortOrder;
      else if (cmpA > cmpB) return +sortOrder;
...
function filterReplay(replay) {
  ...
  if (compatibilityFilter.checked && !isReplayCompatible(replay)) return false;

  // Filter by single-player or multiplayer.
  let singleplayerFilter = Engine.GetGUIObjectByName("singleplayerFilter");
  let selectedSingleplayerFilter =
    singleplayerFilter.list_data[singleplayerFilter.selected] || "";
  if (
    (selectedSingleplayerFilter == "Campaigns" && !replay.isCampaign) ||
    (selectedSingleplayerFilter == "Single-player" &&
      (replay.isMultiplayer || replay.isCampaign)) ||
    (selectedSingleplayerFilter == "Multiplayer" &&
      (!replay.isMultiplayer || replay.isCampaign))
  )
    return false;
  ...
  if (
    victoryConditionFilter.selected > 0 &&
    replay.attribs.settings.VictoryConditions.indexOf(
      victoryConditionFilter.list_data[victoryConditionFilter.selected]
    ) == -1
  )
    return false;
  ...
  // Filter by rating
  let ratedGamesFilter = Engine.GetGUIObjectByName("ratedGamesFilter");
  let selectedRatedGamesFilter =
    ratedGamesFilter.list_data[ratedGamesFilter.selected] || "";
  if (
    (selectedRatedGamesFilter == "rated" && !replay.isRated) ||
  ...
  if (
    dateTimeFilter.selected > 0 &&
    getReplayMonth(replay) != dateTimeFilter.list_data[dateTimeFilter.selected]
  )
    return false;
  ...
  if (keywords.length) {
    ...
    let playerList = replay.attribs.settings.PlayerData.map((player) =>
      player ? player.Name : ""
    )
      .join(" ")
      .toLowerCase();
  ...
  if (
    mapNameFilter.selected > 0 &&
    replay.attribs.settings.Name !=
      mapNameFilter.list_data[mapNameFilter.selected]
  )
    return false;
  ...
  if (
    mapSizeFilter.selected > 0 &&
    replay.attribs.settings.Size !=
      mapSizeFilter.list_data[mapSizeFilter.selected]
  )
    return false;
  ...
  if (
    populationFilter.selected > 0 &&
    replay.attribs.settings.PopulationCap !=
  ...
  if (durationFilter.selected > 0) {
    ...
    if (
      (interval.min > -1 && replay.duration < interval.min * 60) ||

...
function startReplay() {
  ...
  if (isReplayCompatible(replay)) reallyStartVisualReplay(replay.directory);
  ...
  if (!Engine.StartVisualReplay(replayDirectory)) {
    warn(
      'Replay "' +
        ...
        '" not found! Please click on reload cache.'
    );
    return;
  }
...
function displayReplayCompatibilityError(replay) {
  ...
  if (replayHasSameEngineVersion(replay)) {
    ...
    errMsg =
      translate("This replay needs a different sequence of mods:") +
      "\n" +
      comparedModsString(gameMods, g_EngineInfo.mods);
  } else {
    errMsg =
      translate(
        "This replay is not compatible with your version of the game!"
      ) + "\n";
    errMsg +=
      sprintf(translate("Your version: %(version)s"), {
        version: g_EngineInfo.engine_version,
      }) + "\n";
    errMsg += sprintf(translate("Required version: %(version)s"), {
      version: replay.attribs.engine_version,
    });
  }
...
function showReplaySummary() {
  ...
  if (isReplayCompatible(replay)) reallyShowReplaySummary(replay.directory);
  else displayReplayCompatibilityError(replay);
}
...
function reallyShowReplaySummary(directory) {
  ...
  if (!simData) {
    messageBox(
      500,
      200,
      translate("No summary data available."),
      translate("Error")
    );
    return;
  }
...
function reloadCache() {
  ...
  loadReplays(
    selected > -1
      ? createReplaySelectionData(g_ReplaysFiltered[selected].directory)
      : "",
    true
  );
}
...
function deleteReplayButtonPressed() {
  if (!Engine.GetGUIObjectByName("deleteReplayButton").enabled) return;
...
function deleteReplay() {
  ...
  messageBox(
    ...
    [
      null,
      function () {
        reallyDeleteReplay(replay.directory);
      },
    ]
  );
}
...
function deleteReplayWithoutConfirmation() {
  var selected = Engine.GetGUIObjectByName("replaySelection").selected;
  if (selected > -1) reallyDeleteReplay(g_ReplaysFiltered[selected].directory);
}
...
function reallyDeleteReplay(replayDirectory) {
  ...
  if (!Engine.DeleteReplay(replayDirectory)) error("Could not delete replay!");

  // Refresh replay list
  init();

  replaySelection.selected = Math.min(
    selectedIndex,
    g_ReplaysFiltered.length - 1
  );
}

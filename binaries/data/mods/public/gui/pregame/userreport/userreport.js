var g_TermsUserReport = {
  TermsAndConditions: {
    ...
    instruction: translate(
      "Please read and accept the UserReporter Terms and Conditions."
    ),
    file: "gui/userreport/Terms_and_Conditions.txt",
    termsURL: Engine.ConfigDB_GetValue("user", "userreport.url_terms"),
    sprintf: {
      logPath: setStringTags(Engine.GetUserReportLogPath(), {
        font: "sans-bold-12",
      }),
      configPath: setStringTags(Engine.GetUserReportConfigPath(), {
        font: "sans-bold-12",
      }),
    },
...
var g_UserReportStatusFormat = {
  ...
  sending: (data) =>
    sprintf(translate("uploading (%f%%)"), Math.floor(100 * data[1])),
  completed: (data) =>
    data[1] == 200
  ...
  failed: (data) =>
    sprintf(translate("upload failed (%(errorMessage)s)"), {
...
function updateUserReportButtons() {
  ...
  let userReportEnableButton = Engine.GetGUIObjectByName(
    "userReportEnableButton"
  );
  userReportEnableButton.caption = Engine.IsUserReportEnabled()
    ? translate("Disable Feedback")
  ...
  let userReportTermsButton = Engine.GetGUIObjectByName(
    "userReportTermsButton"
  );
...
function updateUserReportStatus() {
  ...
  Engine.GetGUIObjectByName("userReportText").caption =
    Engine.IsUserReportEnabled()
      ? setStringTags(translate("Thank you for helping improve 0 A.D.!"), {
          font: "sans-bold-16",
        }) +
        "\n\n" +
        ...
        sprintf(translate("Status: %(status)s."), {
          status: g_UserReportStatusFormat[statusData[0]]
            ? g_UserReportStatusFormat[statusData[0]](statusData)
            : translate("unknown"),
        })
      : setStringTags(translate("Help improve 0 A.D.!"), {
          font: "sans-bold-16",
        }) +
        "\n\n" +
        translate(

...
class MainMenuItemHandler {
  constructor(menuItems) {
    ...
    this.MainMenuPanelRightBorderTop = Engine.GetGUIObjectByName(
      "MainMenuPanelRightBorderTop"
    );
    this.MainMenuPanelRightBorderBottom = Engine.GetGUIObjectByName(
      "MainMenuPanelRightBorderBottom"
    );
  ...
  setupMenuButtons(buttons, menuItems) {
    buttons.forEach((button, i) => {
      ...
      button.size = new GUISize(
        0,
        (this.ButtonHeight + this.Margin) * i,
        0,
        (this.ButtonHeight + this.Margin) * i + this.ButtonHeight,
        0,
        0,
        100,
        0
  ...
  pressButton(item, i) {
    if (this.submenu.hidden) {
      ...
      if (this.lastOpenItem && this.lastOpenItem != item)
        this.performButtonAction(item, i);
      else this.lastOpenItem = undefined;
    }
  }
  ...
  setupHotkeys(menuItems) {
    for (let i in menuItems) {
      ...
      if (item.submenu) this.setupHotkeys(item.submenu);
    }
  }

  openSubmenu(i) {
    this.setupMenuButtons(
      this.submenuButtons.children,
      this.menuItems[i].submenu
    );

    let top =
      this.mainMenuButtons.size.top + this.mainMenuButtons.children[i].size.top;

    this.submenu.size = new GUISize(
      this.submenu.size.left,
      top - this.Margin,
      this.submenu.size.right,
      top + (this.ButtonHeight + this.Margin) * this.menuItems[i].submenu.length
    );
  ...
  onTick() {
    let now = Date.now();
    if (now == this.lastTickTime) return;

    let maxOffset = this.mainMenu.size.right - this.submenu.size.left;
    let offset = Math.min(
      this.MenuSpeed * (now - this.lastTickTime),
      maxOffset
    );

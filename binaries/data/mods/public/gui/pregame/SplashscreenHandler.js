class SplashScreenHandler {
  constructor(initData, hotloadData) {
    this.showSplashScreen = hotloadData
      ? hotloadData.showSplashScreen
      : initData && initData.isStartup;
  ...
  onFirstTick() {
    if (this.showSplashScreen) this.openPage();
  ...
  openPage() {
    ...
    if (
      ...
      Engine.ConfigDB_GetValue("user", "gui.splashscreen.version") <
        Engine.GetFileMTime("gui/splashscreen/splashscreen.txt")
    )

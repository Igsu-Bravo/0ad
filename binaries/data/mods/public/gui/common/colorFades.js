...
function fadeColorTick(name) {
  if (!isColorFadeRunning(name)) return;
  ...
  if (
    !data.stopFade &&
    (data.justStopAtExternCall ||
      data.duration - data.tickInterval * data.tickCounter > 0)
  ) {
...
function stopColorFade(name, hideOverlay = true) {
  ...
  if (!isColorFadeRunning(name)) return false;
...
function colorFade_attackUnit(data) {
  ...
  // init color
  if (data.tickCounter == 0) rgb.r = 175;

  // blinking
  if (data.tickCounter < g_FadeAttackUnit.blinkingTicks) {
    // slow that process down
    if (data.tickCounter % g_FadeAttackUnit.blinkingChangeInterval != 0) return;
  ...
  // wait a short time and then color fade from red to grey to nothing
    ...
    data.tickCounter >=
    g_FadeAttackUnit.blinkingTicks + g_FadeAttackUnit.blinkingChangeInterval
  ) {
    rgb.g += Math.round(
      g_FadeAttackUnit.gbcolorChangeRate *
        Math.sqrt(data.tickCounter - g_FadeAttackUnit.blinkingTicks)
    );
    ...
    if (rgb.g > g_FadeAttackUnit.fadeOutStart)
      rgb.o =
        rgb.o > g_FadeAttackUnit.opacityChangeRate
          ? (rgb.o -= g_FadeAttackUnit.opacityChangeRate)
          : 0;
    // check for end
    if (rgb.o == 0) data.stopFade = true;
  }
  rgb.b = rgb.g;
}

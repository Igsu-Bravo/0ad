...
function setNewTimerFunction(id, func) {
  if (id in g_Timers) g_Timers[id][1] = func;
}
...
function updateTimers() {
  ...
  var run = [];
  for (let id in g_Timers) if (g_Timers[id][0] <= g_Time) run.push(id);

  for (let id of run) {
    let t = g_Timers[id];
    if (!t) continue; // an earlier timer might have cancelled this one, so skip it

    try {
      t[1]();
    } catch (e) {
      var stack = e.stack.trimRight().replace(/^/gm, "  "); // indent the stack trace
      error(
        sprintf("Error in timer: %(error)s", { error: e }) + "\n" + stack + "\n"
      );
    }
    delete g_Timers[id];
  }

...
function colorizeHotkey(text, hotkey) {
  ...
  else key = formatHotkeyCombinations(key);
...
function colorizeAutocompleteHotkey(string) {
  return sprintf(
    string || translate("Press %(hotkey)s to autocomplete player names."),
    {
      hotkey: setStringTags(

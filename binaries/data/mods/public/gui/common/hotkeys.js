...
function hotkeySort(a, b) {
  ...
  if (specialKeys.indexOf(a) !== -1) a = " " + a;
  if (specialKeys.indexOf(b) !== -1) b = " " + b;
  return a.localeCompare(b, Engine.GetCurrentLocale().substr(0, 2), {
    numeric: true,
  });
}

function formatHotkeyCombination(comb, translateScancodes = true) {
  if (!translateScancodes) return comb.sort(hotkeySort).join("+");

  if (!g_ScancodesMap) g_ScancodesMap = Engine.GetScancodeKeyNames();

  return comb
    .sort(hotkeySort)
    .map((hk) => g_ScancodesMap[hk])
    .join("+");
}
...
function formatHotkeyCombinations(combinations, translateScancodes = true) {
  ...
  let combs = combinations.map((x) =>
    formatHotkeyCombination(x, translateScancodes)
  );

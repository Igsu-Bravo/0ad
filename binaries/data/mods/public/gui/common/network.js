  ...
  switch (id) {
    case 0:
      return wasConnected
        ? ""
        : translate(
            "This is often caused by UDP port 20595 not being forwarded on the host side, by a firewall, or anti-virus software."
          );
    case 1:
      return translate("The host has ended the game.");
    case 2:
      return translate("Incorrect network protocol version.");
    case 3:
      return translate("Game is loading, please try again later.");
    case 4:
      return translate("Game has already started, no observers allowed.");
    case 5:
      return translate("You have been kicked.");
    case 6:
      return translate("You have been banned.");
    case 7:
      return translate(
        "Player name in use. If you were disconnected, retry in few seconds."
      );
    case 8:
      return translate("Server full.");
    case 9:
      return translate("Secure lobby authentication failed. Join via lobby.");
    case 10:
      return translate(
        "Error: Server failed to allocate a unique client identifier."
      );
    case 11:
      return translate(
        "Error: Client commands were ready for an unexpected game turn."
      );
    case 12:
      return translate("Error: Client simulated an unexpected game turn.");
    case 13:
      return translate("Password is invalid.");
    case 14:
      return translate(
        "Could not find an unused port for the enet STUN client."
      );
    case 15:
      return translate("Could not find the STUN endpoint.");
    default:
...
function kickPlayer(username, ban) {
  if (g_IsController) Engine.KickPlayer(username, ban);
  else kickError();
}
...
function getUsernameList() {
  let usernames = sortGUIDsByPlayerID().map((guid) =>
...
function executeNetworkCommand(input) {
  ...
  if (g_NetworkCommands[command]) g_NetworkCommands[command](argument);
...
function addNetworkWarning(msg) {
  ...
  if (Engine.ConfigDB_GetValue("user", "overlay.netwarnings") != "true") return;
...
function getNetworkWarnings() {
  ...
  for (let guid in g_NetworkWarnings)
    if (
      Date.now() > g_NetworkWarnings[guid].added + g_NetworkWarningTimeout ||
  ...
  for (let guid of guids) {
    ...
    // Add formatted text
    messages.push(
      g_NetworkWarningTexts[msg.warntype](msg, colorizePlayernameByGUID(guid))
    );

    // Add width of unformatted text
    let username = guid != "server" && g_PlayerAssignments[guid].name;
    let textWidth = Engine.GetTextWidth(
      font,
      g_NetworkWarningTexts[msg.warntype](msg, username)
    );

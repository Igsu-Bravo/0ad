...
var g_SoundNotifications = {
  ...
  "gamesetup.join": {
    soundfile: "audio/interface/ui/gamesetup_join.ogg",
    threshold: 0,
  },
};
...
function fireConfigChangeHandlers(changes) {
  for (let handler of g_ConfigChangeHandlers) handler(changes);
}
...
function loadCivData(selectableOnly, gaia) {
  ...
  if (gaia) civData.gaia = { Code: "gaia", Name: translate("Gaia") };
...
function playerDataToStringifiedTeamList(playerData) {
  ...
  for (let pData of playerData) {
    ...
    if (!teamList[team]) teamList[team] = [];
...
function removeDupes(array) {
  ...
  let i = array.length;
  while (i--) if (array.indexOf(array[i]) != i) array.splice(i, 1);
}
...
function multiplayerName() {
  return (
    Engine.ConfigDB_GetValue("user", "playername.multiplayer") ||
    Engine.GetSystemUsername()
  );
}
...
function tryAutoComplete(text, autoCompleteList) {
  ...
  if (!wordSplit.length) return text;

  var lastWord = wordSplit.pop();
  if (!lastWord.length) return text;

  for (var word of autoCompleteList) {
    if (word.toLowerCase().indexOf(lastWord.toLowerCase()) != 0) continue;
...
function autoCompleteText(guiObject, words) {
  ...
  guiObject.buffer_position =
    bufferPosition + (newText.length - textTillBufferPosition.length);
}
...
function soundNotification(type) {
  ...
  if (
    !notificationType.lastInteractionTime ||
    timeNow > notificationType.lastInteractionTime + notificationType.threshold
  )
...
function hideRemaining(parentName, start = 0) {
  ...
  for (let i = start; i < objects.length; ++i) objects[i].hidden = true;
}

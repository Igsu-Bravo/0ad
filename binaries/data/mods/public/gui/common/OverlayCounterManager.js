...
class OverlayCounterManager {
  constructor(dataCounter) {
    ...
    for (let name of this.availableCounterNames()) {
      ...
      if (counterType.IsAvailable && !counterType.IsAvailable()) continue;
  ...
  setCounterEnabled(counter, enabled) {
    if (enabled)
      this.enabledCounters = this.counters.filter(
        (count) => this.enabledCounters.indexOf(count) != -1 || count == counter
      );
    else
      this.enabledCounters = this.enabledCounters.filter(
        (count) => count != counter
      );
  ...
  onTick() {
    ...
    let now = Date.now();
    if (now < this.lastTick + this.Delay) return;
    ...
    if (this.lastHeight != height) {
      ...
      for (let handler of this.resizeHandlers) handler(height);
    }
  }
}

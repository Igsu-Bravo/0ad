function initMusic() {
  ...
  if (!global.music) global.music = new Music();
}
...
Music.prototype.updateState = function () {
  if (this.currentState != this.oldState) {
    ...
    switch (this.currentState) {
      ...
      default:
        warn(
          sprintf("%(functionName)s: Unknown music state: %(state)s", {
            functionName: "Music.updateState()",
...
Music.prototype.storeTracks = function (civMusic) {
  ...
  for (let music of civMusic) {
    ...
    if (type === undefined) {
      warn(
        sprintf("%(functionName)s: Unrecognized music type: %(musicType)s", {
          functionName: "Music.storeTracks()",

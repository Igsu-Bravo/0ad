...
var g_SpecificNamesPrimary =
  Engine.ConfigDB_GetValue("user", "gui.session.howtoshownames") == 0 ||
  Engine.ConfigDB_GetValue("user", "gui.session.howtoshownames") == 2;
var g_ShowSecondaryNames =
  Engine.ConfigDB_GetValue("user", "gui.session.howtoshownames") == 0 ||
  Engine.ConfigDB_GetValue("user", "gui.session.howtoshownames") == 1;

function initDisplayedNames() {
  registerConfigChangeHandler((changes) => {
    if (changes.has("gui.session.howtoshownames")) updateDisplayedNames();
  });
}
...
function getCurrentCaptureTooltip(entState, label) {
  if (!entState.maxCapturePoints) return "";
...
function getResistanceTooltip(template) {
  ...
  if (template.resistance.ApplyStatus)
    details.push(
      getStatusEffectsResistanceTooltip(template.resistance.ApplyStatus)
    );
...
function getDamageResistanceTooltip(resistanceTypeTemplate) {
  if (!resistanceTypeTemplate) return "";

  return sprintf(translate("%(label)s %(details)s"), {
    label: headerFont(translate("Damage:")),
    details: g_DamageTypesMetadata
      .sort(Object.keys(resistanceTypeTemplate))
      .map((dmgType) =>
        sprintf(
          translate("%(damage)s %(damageType)s %(resistancePercentage)s"),
          {
            damage: resistanceTypeTemplate[dmgType].toFixed(1),
            damageType: unitFont(
              translateWithContext(
                "damage type",
                g_DamageTypesMetadata.getName(dmgType)
              )
            ),
            resistancePercentage:
              '[font="sans-10"]' +
              sprintf(translate("(%(resistancePercentage)s)"), {
                resistancePercentage: resistanceLevelToPercentageString(
                  resistanceTypeTemplate[dmgType]
                ),
              }) +
              "[/font]",
          }
...
function getStatusEffectsResistanceTooltip(resistanceTypeTemplate) {
  ...
  return sprintf(translate("%(label)s %(details)s"), {
    ...
    details: Object.keys(resistanceTypeTemplate)
      .map((statusEffect) => {
        if (resistanceTypeTemplate[statusEffect].blockChance == 1)
          return sprintf(translate("Blocks %(name)s"), {
            name: unitFont(
              translateWithContext(
                "status effect",
                g_StatusEffectsMetadata.getName(statusEffect)
              )
            ),
          });
        ...
        if (resistanceTypeTemplate[statusEffect].blockChance == 0)
          return sprintf(translate("%(name)s %(details)s"), {
            ...
            details: sprintf(
              translate("Duration reduction: %(durationReduction)s%%"),
              {
                durationReduction:
                  100 - resistanceTypeTemplate[statusEffect].duration * 100,
              }
            ),
          });
        ...
        if (resistanceTypeTemplate[statusEffect].duration == 1)
          return sprintf(translate("%(name)s %(details)s"), {
            ...
            details: sprintf(translate("Blocks: %(blockPercentage)s%%"), {
              blockPercentage:
                resistanceTypeTemplate[statusEffect].blockChance * 100,
            }),
          });
...
function attackRateDetails(interval, projectiles) {
  ...
  if (projectiles === 0) return translate("Garrison to fire arrows");
  ...
  if (projectiles && +projectiles > 1) {
    ...
    let projectileString = sprintf(
      translatePlural(
        "%(projectileCount)s %(projectileName)s",
        "%(projectileCount)s %(projectileName)s",
        projectiles
      ),
      {
        projectileCount: projectiles,
        projectileName: unitFont(
          translatePlural("arrow", "arrows", projectiles)
        ),
      }
    );

    attackRateString = sprintf(
      translate("%(projectileString)s / %(attackRateString)s"),
      {
...
function rangeDetails(attackTypeTemplate) {
  if (!attackTypeTemplate.maxRange) return "";
  ...
  let rangeTooltipString = {
    relative: {
      ...
      "no-minRange": translate(
        "%(rangeLabel)s %(maxRange)s (%(relativeRange)s) %(rangeUnit)s"
      ),
    },
    "non-relative": {
      // Translation: For example: Range: 2 to 10 meters
      minRange: translate(
        "%(rangeLabel)s %(minRange)s to %(maxRange)s %(rangeUnit)s"
      ),
  ...
  return sprintf(
    rangeTooltipString[relativeRange ? "relative" : "non-relative"][
...
function damageDetails(damageTemplate) {
  if (!damageTemplate) return "";

  return g_DamageTypesMetadata
    .sort(
      Object.keys(damageTemplate).filter((dmgType) => damageTemplate[dmgType])
    )
    .map((dmgType) =>
  ...
  if (g_AlwaysDisplayFriendlyFire || splashTemplate.friendlyFire)
    splashDamageTooltip +=
      ...
      sprintf(translate("Friendly Fire: %(enabled)s"), {
        enabled: splashTemplate.friendlyFire
          ? translate("Yes")
          : translate("No"),
      });
...
function applyStatusDetails(applyStatusTemplate) {
  if (!applyStatusTemplate) return "";

  return sprintf(translate("gives %(name)s"), {
    name: Object.keys(applyStatusTemplate)
      .map((x) =>
        unitFont(
          translateWithContext(
            "status effect",
            g_StatusEffectsMetadata.getName(x)
          )
        )
      )
      .join(commaFont(translate(", "))),
...
function getAttackTooltip(template) {
  ...
  for (let attackType in template.attack) {
    ...
    if (attackType == "Slaughter") continue;

    let attackTypeTemplate = template.attack[attackType];
    let attackLabel = sprintf(headerFont(translate("%(attackType)s")), {
      attackType: translateWithContext(
        attackTypeTemplate.attackName.context ||
          "Name of an attack, usually the weapon.",
        attackTypeTemplate.attackName.name
      ),
    });
      ...
      for (let status in attackTypeTemplate.ApplyStatus)
        statusEffectsDetails.push(
          "\n" +
            g_Indent +
            g_Indent +
            getStatusEffectsTooltip(
              status,
              attackTypeTemplate.ApplyStatus[status],
              true
            )
        );
...
function getStatusEffectsTooltip(statusCode, template, applier) {
  ...
  if (applier && statusData.applierTooltip)
    tooltipAttributes.push(
      translateWithContext("status effect", statusData.applierTooltip)
    );
  else if (!applier && statusData.receiverTooltip)
    tooltipAttributes.push(
      translateWithContext("status effect", statusData.receiverTooltip)
    );

  if (applier)
    return sprintf(
      translate("%(statusName)s: %(statusInfo)s %(stackability)s"),
      {
        statusName: headerFont(
          translateWithContext("status effect", statusData.statusName)
        ),
...
function getStatusEffectDurationTooltip(template) {
  ...
  return sprintf(translate("%(durName)s: %(duration)s"), {
    ...
    duration: getSecondsString(
      (template._timeElapsed
      ...
      tooltips.push(
        sprintf(
          translate("%(healRateLabel)s %(value)s %(health)s / %(second)s"),
          {
            ...
            health: unitFont(
              translateWithContext("garrison tooltip", "Health")
            ),
...
function getTurretsTooltip(template) {
  if (!template.turretHolder) return "";
  ...
  if (entState.repairable.numBuilders) {
    result.push(
      sprintf(translate("%(label)s %(details)s"), {
        ...
        details: getSecondsString(
          Math.floor(entState.repairable.buildTime.timeRemaining)
        ),
      })
    );
    let timeReduction = Math.round(
  ...
  if (entState.foundation.numBuilders) {
    result.push(
      sprintf(translate("%(label)s %(details)s"), {
        ...
        details: getSecondsString(
          Math.floor(entState.foundation.buildTime.timeRemaining)
        ),
      })
    );
    let timeReduction = Math.round(
...
function getGatherTooltip(template) {
  ...
  for (let resource of g_ResourceData.GetResources()) {
    ...
    for (let subtype in resource.subtypes) {
      ...
      if (rate > 0) rates[resource.code + "_" + subtype] = rate;
    }
  }
...
function getTreasureTooltip(template) {
  ...
  return sprintf(translate("%(label)s %(details)s"), {
    ...
    details: resourceNames
      .map((type) =>
        sprintf(translate("%(resourceIcon)s %(reward)s"), {
          resourceIcon: resourceIcon(type),
...
function getUpkeepTooltip(template) {
  if (!template.upkeep) return "";

  let resCodes = g_ResourceData
    .GetCodes()
    .filter((res) => !!template.upkeep.rates[res]);
  if (!resCodes.length) return "";
...
function getEntityCostTooltip(
  ...
  if (template.wallSet) {
    ...
    let templateMedium = GetTemplateData(
      template.wallSet.templates.medium,
      player
    );
    let templateShort = GetTemplateData(
      template.wallSet.templates.short,
      player
    );
    let templateTower = GetTemplateData(
...
function getPopulationBonusTooltip(template) {
  if (!template.population || !template.population.bonus) return "";
...
function getEntityNames(template) {
  ...
  let primaryName = g_SpecificNamesPrimary
    ? template.name.specific
    : template.name.generic;
  let secondaryName;
  if (g_ShowSecondaryNames)
    secondaryName = g_SpecificNamesPrimary
      ? template.name.generic
      : template.name.specific;
...
function getEntityNamesFormatted(template) {
  if (!template.name.specific)
    return setStringTags(
      template.name.generic,
      g_TooltipTextFormats.namePrimaryBig
    );
  ...
  if (!secondaryName || primaryName == secondaryName)
    return sprintf(translate("%(primaryName)s"), {
      primaryName:
        ...
        setStringTags(
          primaryName.slice(1).toUpperCase(),
          g_TooltipTextFormats.namePrimarySmall
        ),
    });
  ...
  return sprintf(translate("%(primaryName)s (%(secondaryName)s)"), {
    ...
    secondaryName: setStringTags(
      secondaryName,
      g_TooltipTextFormats.nameSecondary
    ),
  });
}
...
function getEntityPrimaryNameFormatted(template) {
  ...
  if (!primaryName)
    return setStringTags(
      g_SpecificNamesPrimary ? template.name.generic : template.name.specific,
      g_TooltipTextFormats.namePrimaryBig
    );
...
function getResourceDropsiteTooltip(template) {
  ...
  return sprintf(translate("%(label)s %(icons)s"), {
    ...
    icons: template.resourceDropsite.types
      .map((type) => resourceIcon(type))
      .join("  "),
  });
}
...
function abbreviateLargeNumbers(number) {
  if (number >= 1e6)
    return (
      ...
      translateWithContext("One letter abbreviation for million", "M")
    );

  if (number >= 1e5)
    return (
      Math.floor(number / 1e3) +
      translateWithContext("One letter abbreviation for thousand", "k")
    );

  if (number >= 1e4)
    return (
      (number / 1e3).toFixed(1).replace(/\.0$/, "") +
      translateWithContext("One letter abbreviation for thousand", "k")
    );

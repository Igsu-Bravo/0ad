...
class CampaignTemplate {
  ...
  static getAvailableTemplates() {
    if (g_CachedTemplates) return g_CachedTemplates;
    ...
    for (let filename of campaigns)
      g_CachedTemplates.push(
        new CampaignTemplate(
          filename.slice("campaigns/".length, -".json".length)
        )
      );
  ...
  static getTemplate(identifier) {
    if (!g_CachedTemplates) CampaignTemplate.getAvailableTemplates();
  ...
  constructor(identifier) {
    Object.assign(
      this,
      Engine.ReadJSONFile("campaigns/" + identifier + ".json")
    );

    this.identifier = identifier;

    if (this.Interface) this.interface = this.Interface;
    else this.interface = "default_menu";

    if (!this.isValid())
      throw (
        "Campaign template " +
        this.identifier +
        ".json is not a valid campaign template."
      );
  }

...
function splitRatingFromNick(playerName) {
  ...
  return {
    nick: result ? result[1] : playerName,
    rating: result ? +result[2] : "",
  };
}
 ...
 */
var g_Buddies = Engine.ConfigDB_GetValue("user", "lobby.buddies").split(
...
function formatPlayerInfo(playerDataArray, playerStates) {
  ...
  for (let playerData of playerDataArray) {
    ...
    let playerState =
      (playerStates && playerStates[playerIdx]) || playerData.State;
    ...
    if (isAI) {
      if (playerData.Civ) {
        if (isActive)
          ...
          playerDescription = translate(
            "%(playerName)s (%(civ)s, %(AIdescription)s)"
          );
        // Translation: Describe a player in a selected game, f.e. in the replay- or savegame menu
        else
          playerDescription = translate(
            "%(playerName)s (%(civ)s, %(AIdescription)s, %(state)s)"
          );
      } else {
        if (isActive)
          // Translation: Describe a player in a selected game, f.e. in the replay- or savegame menu
        ...
        // Translation: Describe a player in a selected game, f.e. in the replay- or savegame menu
          ...
          playerDescription = translate(
            "%(playerName)s (%(AIdescription)s, %(state)s)"
          );
      }
    } else {
      if (playerData.Offline) {
        ...
        if (playerData.Civ)
          ...
          // Translation: Describe a player in a selected game, f.e. in the replay- or savegame menu
            ...
            playerDescription = translate(
              "%(playerName)s (%(civ)s, %(state)s)"
            );
        else if (isActive)
          // Translation: Describe a player in a selected game, f.e. in the replay- or savegame menu
          playerDescription = translate("%(playerName)s");
        // Translation: Describe a player in a selected game, f.e. in the replay- or savegame menu
        else playerDescription = translate("%(playerName)s (%(state)s)");
      }
    }

    // Sort player descriptions by team
    if (!playerDescriptions[teamIdx]) playerDescriptions[teamIdx] = [];

    let playerNick = splitRatingFromNick(playerData.Name).nick;
    playerDescriptions[teamIdx].push(
      sprintf(playerDescription, {
        playerName: coloredText(
          (g_Buddies.indexOf(playerNick) != -1 ? g_BuddySymbol + " " : "") +
            escapeText(playerData.Name),
          typeof getPlayerColor == "function"
            ? isAI
              ? "white"
              : getPlayerColor(playerNick)
            : rgbToGuiColor(
                playerData.Color || g_Settings.PlayerDefaults[playerIdx].Color
              )
        ),
        ...
        civ: !playerData.Civ
          ...
          : g_CivData &&
            g_CivData[playerData.Civ] &&
            g_CivData[playerData.Civ].Name
  ...
  if (playerDescriptions.observer)
    teamDescription.push(
      sprintf(translate("%(team)s:\n%(playerDescriptions)s"), {
        team:
          '[font="sans-bold-14"]' +
          translatePlural(
            "Observer",
            "Observers",
            playerDescriptions.observer.length
          ) +
          "[/font]",
...
function getGameDescription(initAttributes, mapCache) {
  ...
  if (!initAttributes.settings.VictoryConditions.length)
    titles.push({
      ...
      value: translate(
        "No winner will be determined, even if everyone is defeated."
      ),
    });

  for (let victoryCondition of g_VictoryConditions) {
    if (
      initAttributes.settings.VictoryConditions.indexOf(
    ...
    if (victoryCondition.Name == "regicide")
      if (initAttributes.settings.RegicideGarrison)
        ...
        titles.push({
          ...
          value: translate(
            "Heroes cannot be garrisoned and they are vulnerable to raids."
          ),
        });
  }
  ...
  if (initAttributes.settings.LastManStanding)
    titles.push({
      ...
      value: translate(
        "Only one player can win the game. If the remaining players are allies, the game continues until only one remains."
      ),
    });
  else
    titles.push({
      ...
      value: translate(
        "If one player wins, his or her allies win too. If one group of allies remains, they win."
      ),
    });
  ...
  if (initAttributes.map == "random")
    ...
    titles.push({
      ...
      value: mapCache.translateMapName(
        mapCache.getTranslatableMapName(
          initAttributes.mapType,
          initAttributes.map,
          initAttributes
        )
      ),
    ...
    titles.push({
      ...
      value: mapCache.getTranslatedMapDescription(
        initAttributes.mapType,
        initAttributes.map
      ),
    });
  ...
  if (initAttributes.mapType == "random") {
    let mapSize =
      g_MapSizes.Name[g_MapSizes.Tiles.indexOf(initAttributes.settings.Size)];
    if (mapSize)
      titles.push({
  ...
  if (initAttributes.settings.Biome) {
    let biome = g_Settings.Biomes.find(
      (b) => b.Id == initAttributes.settings.Biome
    );
    titles.push({
      label: biome
        ? biome.Title
        : translateWithContext("biome", "Random Biome"),
      value: biome
        ? biome.Description
        : translate("Randomly select a biome from the list."),
    });
  }

  if (initAttributes.settings.TriggerDifficulty !== undefined) {
    let triggerDifficulty = g_Settings.TriggerDifficulties.find(
      (difficulty) =>
        difficulty.Difficulty == initAttributes.settings.TriggerDifficulty
    );
    titles.push({
  ...
  if (initAttributes.settings.Nomad !== undefined)
    titles.push({
      label: initAttributes.settings.Nomad
        ? translate("Nomad Mode")
        : translate("Civic Centers"),
      value: initAttributes.settings.Nomad
        ? translate(
            "Players start with only few units and have to find a suitable place to build their city."
          )
  ...
  if (initAttributes.settings.StartingResources !== undefined)
    titles.push({
      ...
      value:
        ...
        initAttributes.settings.PlayerData.some(
          (pData) => pData && pData.Resources !== undefined
        )
  ...
  if (initAttributes.settings.PopulationCap !== undefined)
    titles.push({
      ...
      value:
        initAttributes.settings.PlayerData &&
        initAttributes.settings.PlayerData.some(
          (pData) => pData && pData.PopulationLimit !== undefined
        )
  ...
  if (
    ...
    titles.push({
      label: translate("Rated game"),
      value:
        initAttributes.settings.RatingEnabled === true
          ? ratingDescription
          : translateWithContext("game setup option", "disabled"),
    });
  ...
  return titles
    .map((title) =>
      sprintf(translate("%(label)s %(details)s"), {
        ...
        details:
          title.value === true
            ? translateWithContext("game setup option", "enabled")
            : title.value ||
              translateWithContext("game setup option", "disabled"),
      })
    )

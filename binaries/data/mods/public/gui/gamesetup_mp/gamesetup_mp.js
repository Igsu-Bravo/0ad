  ...
  switch (attribs.multiplayerGameType) {
    ...
    case "host": {
      ...
      if (hasXmppClient) {
        ...
        Engine.GetGUIObjectByName("hostServerName").caption = sprintf(
          translate("%(name)s's game"),
          { name: attribs.name }
        );

        Engine.GetGUIObjectByName("useSTUN").checked =
          Engine.ConfigDB_GetValue("user", "lobby.stun.enabled") == "true";
      }
    ...
    default:
      error(
        "Unrecognised multiplayer game type: " + attribs.multiplayerGameType
      );
      break;
  }
}
...
function cancelSetup() {
  if (g_IsConnecting) Engine.DisconnectNetworkGame();
  ...
  if (g_GameType == "client") switchSetupPage("pageJoin");
  else if (g_GameType == "server") switchSetupPage("pageHost");
  else error("cancelSetup: Unrecognised multiplayer game type: " + g_GameType);
}

function confirmPassword() {
  if (Engine.GetGUIObjectByName("pagePassword").hidden) return;
  if (
    startJoinFromLobby(
      g_ServerName,
      g_ServerId,
      Engine.GetGUIObjectByName("clientPassword").caption
    )
  )
    switchSetupPage("pageConnecting");
}
...
function confirmSetup() {
  if (!Engine.GetGUIObjectByName("pageJoin").hidden) {
    ...
    if (!hostServerName) {
      Engine.GetGUIObjectByName("hostFeedback").caption = translate(
...
function startConnectionStatus(type) {
  ...
  g_IsConnecting = true;
  g_IsRejoining = false;
  Engine.GetGUIObjectByName("connectionStatus").caption = translate(
...
function getConnectionFailReason(reason) {
  switch (reason) {
    ...
    case "local_ip_failed":
      return translate(
        "Failed to get local IP of the server (it was assumed to be on the same network)."
      );
    default:
      warn("Unknown connection failure reason: " + reason);
      return sprintf(translate("\\[Invalid value %(reason)s]"), {
        reason: reason,
      });
  }
}
...
function pollAndHandleNetworkClient() {
  while (true) {
    ...
    log(
      sprintf(translate("Net message: %(message)s"), {
        message: uneval(message),
      })
    );
...
function switchSetupPage(newPage) {
  ...
  for (let page of multiplayerPages.children)
    if (page.name.startsWith("page")) page.hidden = true;

  if (newPage == "pageJoin" || newPage == "pageHost") {
    let pageSize = multiplayerPages.size;
    let halfHeight =
      newPage == "pageJoin" ? 145 : Engine.HasXmppClient() ? 140 : 125;
  ...
  Engine.GetGUIObjectByName("hostPlayerNameWrapper").hidden =
    Engine.HasXmppClient();
  Engine.GetGUIObjectByName("hostServerNameWrapper").hidden =
    !Engine.HasXmppClient();

  Engine.GetGUIObjectByName("continueButton").hidden =
    newPage == "pageConnecting" || newPage == "pagePassword";
}
...
function startHost(playername, servername, port, password) {
  ...
  let useSTUN =
    Engine.HasXmppClient() && Engine.GetGUIObjectByName("useSTUN").checked;

  try {
    Engine.StartNetworkHost(
    ...
    messageBox(
      400,
      200,
      sprintf(translate("Cannot host game: %(message)s."), {
        message: e.message,
      }),
      translate("Error")
    );
  ...
  if (Engine.HasXmppClient()) Engine.LobbySetPlayerPresence("playing");
...
function startJoin(playername, ip, port) {
  try {
    ...
    messageBox(
      400,
      200,
      sprintf(translate("Cannot join game: %(message)s."), {
        message: e.message,
      }),
      translate("Error")
    );

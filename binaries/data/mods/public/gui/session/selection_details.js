...
function displaySingle(entState) {
  ...
  if (template.pack && template.pack.state == "packed") {
    if (secondaryName && g_ShowSecondaryNames)
      secondaryName = sprintf(translate("%(secondaryName)s — Packed"), {
        secondaryName: secondaryName,
      });
    else secondaryName = sprintf(translate("Packed"));
  }
  ...
  if (entState.statusEffects) {
    ...
    for (let effectCode in entState.statusEffects) {
      ...
      statusIcons[i].sprite =
        "stretched:session/icons/status_effects/" +
        g_StatusEffectsMetadata.getIcon(effect.baseCode) +
        ".png";
      statusIcons[i].tooltip = getStatusEffectsTooltip(
        effect.baseCode,
        effect,
        false
      );
      ...
      if (++i >= statusIcons.length) break;
    }
    for (; i < statusIcons.length; ++i) statusIcons[i].hidden = true;
  } else Engine.GetGUIObjectByName("statusEffectsIcons").hidden = true;
  ...
  } else if (showCapture) captureSection.size = sectionPosTop.size;
  ...
  if (entState.capturePoints) {
    let setCaptureBarPart = function (playerID, startSize) {
      ...
      unitCaptureBar.sprite =
        "color:" + g_DiplomacyColors.getPlayerColor(playerID, 128);
    ...
    experienceSize.rtop =
      ...
      100 *
        Math.max(
          ...
          Math.min(
            1,
            (1.0 * +entState.promotion.curr) / +entState.promotion.req
          )
        );
  ...
  // And for number of workers
    ...
    resourceCarryingText.caption = entState.foundation.numBuilders
      ? sprintf(translate("(%(number)s)\n%(time)s"), {
          number: entState.foundation.numBuilders,
          time: Engine.FormatMillisecondsIntoDateStringGMT(
  ...
  Engine.GetGUIObjectByName("secondary").caption =
  ...
  Engine.GetGUIObjectByName("playerCivIcon").sprite = isGaia
    ? ""
    : "cropped:1.0, 0.15625 center:grayscale:" + civEmblem;
  ...
  if (detailedTooltip) {
    ...
    Engine.GetGUIObjectByName("attackAndResistanceStats").tooltip =
      detailedTooltip;
  } else Engine.GetGUIObjectByName("attackAndResistanceStats").hidden = true;

  let iconTooltips = [];

  iconTooltips.push(
    setStringTags(primaryName, g_TooltipTextFormats.namePrimaryBig)
  );
...
function displayMultiple(entStates) {
  ...
  for (let entState of entStates) {
    ...
    if (entState.garrisonable) garrisonSize += entState.garrisonable.size;
  ...
  if (averageHealth > 0) {
    ...
    Engine.GetGUIObjectByName("healthMultiple").tooltip =
  ...
  if (maxCapturePoints > 0) {
    let setCaptureBarPart = function (pID, startSize) {
      ...
      unitCaptureBar.size = sizeObj;
      unitCaptureBar.sprite =
        "color:" + g_DiplomacyColors.getPlayerColor(pID, 128);
      unitCaptureBar.hidden = false;
      return startSize + size;
    };
    ...
    Engine.GetGUIObjectByName("captureMultiple").tooltip =
      getCurrentHealthTooltip(
        {

...
class MiniMap {
  constructor() {
    ...
    registerConfigChangeHandler((changes) => {
      if (changes.has("gui.session.flarelifetime")) this.updateFlareLifetime();
    });
  }

  updateFlareLifetime() {
    this.miniMap.flare_lifetime_seconds = Math.max(
      0,
      +Engine.ConfigDB_GetValue("user", "gui.session.flarelifetime")
    );
  }
  ...
  onWorldClick(target, button) {
    ...
    if (button == SDL_BUTTON_LEFT) {
      ...
      if (inputState == INPUT_PRESELECTEDACTION) {
        ...
        inputState = INPUT_NORMAL;
        return true;
      } else if (inputState != INPUT_NORMAL) return false;
    } else return false;
  ...
  flare(target, playerID) {
    return this.miniMap.flare(
      ...
      g_DiplomacyColors.getPlayerColor(playerID)
    );
  }
}

  ...
  insertValues(report, playerStates) {
    ...
    for (let action of this.Actions)
      for (let resCode of g_ResourceData.GetCodes())
        report[resCode + action] =
          playerStates
            .map(
              (playerState) =>
                playerState.sequences["resources" + action][resCode][time]
            )
            .join(",") + ",";

    report.vegetarianFoodGathered =
      playerStates
        .map(
          (playerState) =>
            playerState.sequences.resourcesGathered.vegetarianFood[time]
        )
        .join(",") + ",";
  }
};

...
LobbyRatingReport.prototype.Units = class {
  insertValues(report, playerStates) {
    ...
    for (let unitClass in playerStates[0].sequences.unitsTrained)
      report[lower(unitClass) + "UnitsTrained"] =
        playerStates
          .map(
            (playerState) => playerState.sequences.unitsTrained[unitClass][time]
          )
          .join(",") + ",";

    for (let unitClass in playerStates[0].sequences.unitsLost)
      report[lower(unitClass) + "UnitsLost"] =
        playerStates
          .map(
            (playerState) => playerState.sequences.unitsLost[unitClass][time]
          )
          .join(",") + ",";

    for (let unitClass in playerStates[0].sequences.enemyUnitsKilled)
      report["enemy" + unitClass + "UnitsKilled"] =
        playerStates
          .map(
            (playerState) =>
              playerState.sequences.enemyUnitsKilled[unitClass][time]
          )
          .join(",") + ",";
  }
};

...
class LobbyGamelistReporter {
  ...
  getPlayers() {
    ...
    for (
      let playerID = 1;
      playerID < g_InitAttributes.settings.PlayerData.length;
      ++playerID
    ) {
      ...
      if (g_InitAttributes.settings.LockTeams) player.Team = pData.Team;
      ...
      if (g_Players[playerID].offline) player.Offline = true;

      // Whether the player has won or was defeated
      let state = g_Players[playerID].state;
      if (state != "active") player.State = state;
  ...
  countConnectedPlayers() {
    ...
    for (let guid in g_PlayerAssignments)
      if (g_PlayerAssignments[guid].player != -1) ++connectedPlayers;

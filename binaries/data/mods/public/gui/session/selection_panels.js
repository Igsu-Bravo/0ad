...
g_SelectionPanels.Barter = {
  ...
  getItems: function (unitEntStates) {
    ...
    if (
      ...
      g_ResourceData.GetBarterableCodes().length > this.rowLength
    )
      return [];
...
g_SelectionPanels.Formation = {
  ...
  getItems: function (unitEntStates) {
    ...
    if (
      unitEntStates.every(
        (state) => !state.identity || !state.identity.hasSomeFormation
      )
    )
      return [];
    ...
    return g_AvailableFormations
      .get(unitEntStates[0].player)
      .filter((formation) =>
        unitEntStates.some(
          (state) =>
            !!state.identity &&
            state.identity.formations.indexOf(formation) != -1
        )
      );
  },
  setupButton: function (data) {
    ...
    if (data.item === NULL_FORMATION)
      ...
      tooltip +=
        ...
        (isDefaultFormation
          ? translate(
              "This is the default formation, used for movement orders."
            )
...
g_SelectionPanels.Garrison = {
  ...
  setupButton: function (data) {
    ...
    let canUngarrison =
      controlsPlayer(data.player) || controlsPlayer(entState.player);
    ...
    data.guiSelection.sprite =
      "color:" + g_DiplomacyColors.getPlayerColor(entState.player, 160);
...
g_SelectionPanels.Queue = {
  ...
  getItems: function (unitEntStates) {
      ...
      for (const state of unitEntStates) {
        ...
        queue.push({
          ...
          autoqueue:
            ...
            state.production.queue[i].unitTemplate,
        });
        foundNew = true;
      }
    }
    ...
    if (repeat.length)
      for (
        let i = 0;
        queue.length < g_SelectionPanels.Queue.getMaxNumberOfItems();
        ++i
      )
  ...
  resizePanel: function (numberOfItems, rowLength) {
    ...
    let size = panel.size;
    let buttonSize =
      Engine.GetGUIObjectByName("unitQueueButton[0]").size.bottom;
    let margin = 4;
  ...
  setupButton: function (data) {
    ...
    if (data.item.ghost)
      tooltips.push(
        translate("The auto-queue will try to train this item later.")
      );
    ...
    if (data.item.ghost) {
      ...
      const guiObject = Engine.GetGUIObjectByName(
        "unitQueueProgressSlider[" + data.i + "]"
      );
    ...
    if (template.icon)
      data.icon.sprite =
        (data.item.ghost ? "grayscale:" : "") +
        "stretched:session/portraits/" +
        template.icon;

    const showTemplateFunc = () => {
      showTemplateDetails(
        data.item.queuedItem.unitTemplate ||
          data.item.queuedItem.technologyTemplate,
        data.playerState.civ
      );
    };
    data.button.onPressRight = showTemplateFunc;
    data.button.onPressRightDisabled = showTemplateFunc;
    ...
    if (unitEntStates.length == 1)
      return !unitEntStates[0].production ||
        ...
        : unitEntStates[0].production.technologies.map((tech) => ({
            ...
            isUpgrading:
              !!unitEntStates[0].upgrade &&
              unitEntStates[0].upgrade.isUpgrading,
          }));

    let sortedEntStates = unitEntStates.sort(
      (a, b) =>
        (!b.upgrade || !b.upgrade.isUpgrading) -
          (!a.upgrade || !a.upgrade.isUpgrading) ||
        (!a.production ? 0 : a.production.queue.length) -
          (!b.production ? 0 : b.production.queue.length)
    );
...
g_SelectionPanels.Selection = {
  ...
  setupButton: function (data) {
    ...
    data.guiSelection.sprite =
      "color:" + g_DiplomacyColors.getPlayerColor(unitOwner, 160);
...
g_SelectionPanels.Training = {
  ...
  setupButton: function (data) {
    ...
    if (
      data.unitEntStates.every(
        (state) => state.upgrade && state.upgrade.isUpgrading
      )
    ) {
      data.button.enabled = false;
      modifier = "color:0 0 0 127:grayscale:";
...
g_SelectionPanels.Upgrade = {
  ...
  setupButton: function (data) {
    ...
    let upgradingEntStates = data.unitEntStates.filter(
      (state) => state.upgrade.template == data.item.entity
    );

    let upgradableEntStates = data.unitEntStates.filter(
      (state) =>
        !state.upgrade.progress &&
        (!state.production ||
          !state.production.queue ||
          !state.production.queue.length)
    );
    ...
    if (!upgradingEntStates.length && upgradableEntStates.length) {
      ...
      if (g_ShowSecondaryNames) {
        if (data.item.tooltip)
          tooltips.push(
            sprintf(
              translate(
                "Upgrade to a %(primaryName)s (%(secondaryName)s). %(tooltip)s"
              ),
              {
          ...
          tooltips.push(
            sprintf(
              translate("Upgrade to a %(primaryName)s (%(secondaryName)s)."),
              {
                primaryName: primaryName,
                secondaryName: secondaryName,
              }
        ...
        if (data.item.tooltip)
          tooltips.push(
            sprintf(translate("Upgrade to a %(primaryName)s. %(tooltip)s"), {
              primaryName: primaryName,
              tooltip: translate(data.item.tooltip),
            })
          );
          ...
          tooltips.push(
            sprintf(translate("Upgrade to a %(primaryName)s."), {
              primaryName: primaryName,
            })
          );
      }
      ...
      if (
        ...
        (limits.canBeAddedCount == 0 &&
          !upgradableEntStates.some((state) =>
            hasSameRestrictionCategory(data.item.entity, state.template)
          ))
      ) {
        data.button.enabled = false;
        modifier = "color:0 0 0 127:grayscale:";
      ...
      data.countDisplay.caption =
        upgradableEntStates.length > 1 ? upgradableEntStates.length : "";
      ...
      data.countDisplay.caption =
        upgradingEntStates.length > 1 ? upgradingEntStates.length : "";
      ...
      progressOverlaySize.top =
        progressOverlaySize.bottom +
        Math.round(
          (1 - progress) *
            (progressOverlaySize.left - progressOverlaySize.right)
        );
...
function initSelectionPanels() {
  ...
  if (BarterButtonManager.IsAvailable(unitBarterPanel))
    g_SelectionPanelBarterButtonManager = new BarterButtonManager(
      unitBarterPanel
    );
}
...
function showTemplateDetails(templateName, civCode) {
  if (inputState != INPUT_NORMAL) return;

...
EntitySelection.prototype.update = function () {
  ...
  const removeOwnerChanges =
    !g_IsObserver && !controlsAll && this.selected.size > 1;
...
EntitySelection.prototype.addList = function (ents, quiet, force = false) {
  ...
  const firstEntState =
    this.selected.size == 1 && GetEntityState(this.getFirstSelected());
  if (firstEntState && firstEntState.player != g_ViewedPlayer && !force) return;

  let added = [];

  for (const ent of ents) {
    if (this.selected.size >= g_MaxSelectionSize) break;

    if (this.selected.has(ent)) continue;
...
EntitySelection.prototype.rebuildSelection = function (renamed) {
  const toAdd = [];
  for (const ent of this.selected) toAdd.push(renamed[ent] || ent);
  this.reset();
...
EntitySelection.prototype.getFirstSelected = function () {
  for (const ent of this.selected) return ent;
  return undefined;
};
...
EntitySelection.prototype.find = function (condition) {
  for (const ent of this.selected) if (condition(ent)) return ent;
  return null;
};
...
EntitySelection.prototype.filter = function (condition) {
  const result = [];
  for (const ent of this.selected) if (condition(ent)) result.push(ent);
  return result;
};

EntitySelection.prototype.setHighlightList = function (ents) {
  const highlighted = new Set();
  for (const ent of ents) highlighted.add(ent);
  ...
  for (const ent of this.highlighted)
    if (!highlighted.has(ent) && !this.selected.has(ent)) removed.push(ent);

  // Add new highlighting for units that aren't already highlighted.
  for (const ent of ents)
    if (!this.highlighted.has(ent) && !this.selected.has(ent)) added.push(ent);
...
EntityGroupsContainer.prototype.addEntities = function (groupName, ents) {
  if (
    Engine.ConfigDB_GetValue("user", "gui.session.disjointcontrolgroups") ==
    "true"
  )
    for (let ent of ents)
      for (let group of this.groups)
        if (ent in group.ents) group.removeEnt(ent);

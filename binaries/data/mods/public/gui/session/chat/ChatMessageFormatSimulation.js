...
ChatMessageFormatSimulation.attack = class {
  parse(msg) {
    if (msg.player != g_ViewedPlayer) return "";
...
ChatMessageFormatSimulation.barter = class {
  parse(msg) {
    if (
      !g_IsObserver ||
      Engine.ConfigDB_GetValue("user", "gui.session.notifications.barter") !=
        "true"
    )
      return "";
    ...
    return {
      text: sprintf(
        translate("%(player)s bartered %(amountGiven)s for %(amountGained)s."),
        {
...
ChatMessageFormatSimulation.diplomacy = class {
  parse(msg) {
    ...
    if (g_IsObserver) messageType = "observer";
    else if (Engine.GetPlayerID() == msg.sourcePlayer) messageType = "active";
    else if (Engine.GetPlayerID() == msg.targetPlayer) messageType = "passive";
    else return "";

    return {
      text: sprintf(translate(this.strings[messageType][msg.status]), {
        player: colorizePlayernameByID(
          messageType == "active" ? msg.targetPlayer : msg.sourcePlayer
        ),
        player2: colorizePlayernameByID(
          messageType == "active" ? msg.sourcePlayer : msg.targetPlayer
        ),
      }),
    };
  }
...
ChatMessageFormatSimulation.phase = class {
  parse(msg) {
    let notifyPhase = Engine.ConfigDB_GetValue(
      "user",
      "gui.session.notifications.phase"
    );
    if (
      notifyPhase == "none" ||
      (msg.player != g_ViewedPlayer &&
        !g_IsObserver &&
        !g_Players[msg.player].isMutualAlly[g_ViewedPlayer])
    )
      return "";
    ...
    if (notifyPhase == "all") {
      if (msg.phaseState == "started")
        ...
        message = translate(
          "The %(phaseName)s of %(player)s has been aborted."
        );
    }
    ...
    return {
      text: sprintf(message, {
        player: colorizePlayernameByID(msg.player),
        phaseName: getEntityNames(
          GetTechnologyData(msg.phaseName, g_Players[msg.player].civ)
        ),
      }),
    };
  }
...
ChatMessageFormatSimulation.playerstate = class {
  parse(msg) {
    ...
    let mPlayers = msg.players.map((playerID) =>
      colorizePlayernameByID(playerID)
    );
    ...
    return {
      text: sprintf(
        translatePlural(
          msg.message.message,
          msg.message.pluralMessage,
          msg.message.pluralCount
        ),
        {
...
ChatMessageFormatSimulation.tribute = class {
  parse(msg) {
    ...
    if (msg.targetPlayer == Engine.GetPlayerID())
      ...
      Engine.ConfigDB_GetValue("user", "gui.session.notifications.tribute") ==
        "true" &&
      (g_IsObserver ||
        (g_InitAttributes.settings.LockTeams &&

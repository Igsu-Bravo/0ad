...
class ChatMessageHandler {
  ...
  registerMessageFormat(type, handler) {
    if (!this.messageFormats[type]) this.messageFormats[type] = [];
  ...
  handleMessage(msg) {
    ...
    for (let handler of this.messageHandlers) handler(msg, formatted);
  }
  ...
  parseMessage(msg) {
    ...
    for (let messageFormat of this.messageFormats[msg.type]) {
      ...
      if (formatted && formatted.text) return formatted;
    }

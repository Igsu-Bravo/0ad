...
class ChatOverlay {
  constructor() {
    ...
    this.chatLinesNumber = Math.min(
      this.chatLinesNumber,
      this.chatLines.length
    );
  }
  ...
  onChatMessage(msg, chatMessage) {
    ...
    this.chatTimers.push(
      setTimeout(this.removeOldChatMessage.bind(this), this.chatTimeout * 1000)
    );

    if (this.chatMessages.length > this.chatLinesNumber)
      this.removeOldChatMessage();
    else this.displayChatMessages();
  }
  ...
  clearChatMessages() {
    ...
    for (let timer of this.chatTimers) clearTimeout(timer);

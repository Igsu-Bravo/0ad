...
class ChatMessageFormatPlayer {
  ...
  parse(msg) {
    ...
    if (!isMe && !this.parseMessageAddressee(msg)) return "";

    isMe = msg.text.startsWith("/me ");
    if (isMe) msg.text = msg.text.substr("/me ".length);
    ...
    if (msg.translate) {
      ...
      if (
        userName != g_PlayerAssignments[msg.guid].name &&
        msg.text
          .toLowerCase()
          .indexOf(splitRatingFromNick(userName).nick.toLowerCase()) != -1
      )
    ...
    let coloredUsername =
      msg.guid != -1
        ? colorizePlayernameByGUID(msg.guid)
        : colorizePlayernameByID(msg.player);

    return {
      text: sprintf(
        translate(
          this.strings[isMe ? "me" : "regular"][
        ...
        ),
        {
          message: msg.text,
          context: msg.context
            ? translateWithContext("chat message context", msg.context)
            : "",
  ...
  parseMessageAddressee(msg) {
    ...
    let senderID = msg.player
      ? msg.player
      : (g_PlayerAssignments[msg.guid] || msg).player;
    ...
    if (isPM) {
      ...
      if (!addressee) {
        if (isSender) warn("Couldn't match username: " + msg.text);
        return false;
      }

      // Prohibit PM if addressee and sender are identical
      if (isSender && addresseeGUID == Engine.GetPlayerGUID()) return false;
    ...
    // Set context string
    let addresseeType = this.AddresseeTypes.find(
      (type) => type.command == msg.cmd
    );
    if (!addresseeType) {
      if (isSender) warn("Unknown chat command: " + msg.cmd);
      return false;
    }
    ...
    if (
      isPlayerObserver(senderID) &&
      ((isPM && !isPlayerObserver(addresseeIndex)) ||
        (!isPM && msg.cmd != "/observers"))
    )
      return false;

    let visible =
      isSender || addresseeType.isAddressee(senderID, addresseeGUID);

...
class ChatAddressees {
  ...
  onSelectionChange() {
    ...
    for (let handler of this.selectionChangeHandlers) handler(selection);
  }
  ...
  onUpdatePlayers() {
    ...
    selectedName =
      selectedName.startsWith("/msg ") && selectedName.substr("/msg ".length);

    let addressees = this.AddresseeTypes.filter((addresseeType) =>
      addresseeType.isSelectable()
    ).map((addresseeType) => ({
    ...
    for (let guid of guids) {
      if (guid == Engine.GetPlayerGUID()) continue;

      let playerID = g_PlayerAssignments[guid].player;

      // Don't provide option for PM from observer to player
      if (g_IsObserver && !isPlayerObserver(playerID)) continue;

      let colorBox = isPlayerObserver(playerID)
        ? ""
        : colorizePlayernameHelper("■", playerID) + " ";
    ...
    if (
      selectedName &&
      guids.every((guid) => g_PlayerAssignments[guid].name != selectedName)
    )
      addressees.push({
        cmd: "/msg " + selectedName,
        label: sprintf(translate("\\[OFFLINE] %(player)s"), {
          player: selectedName,
        }),
      });
    ...
    this.chatAddressee.selected = Math.max(
      0,
      this.chatAddressee.list_data.indexOf(oldChatAddressee)
    );
  }
}
...
ChatAddressees.prototype.AddresseeTypes = [
  ...
  },
  {
    ...
    isAddressee: (senderID, addresseeGUID) =>

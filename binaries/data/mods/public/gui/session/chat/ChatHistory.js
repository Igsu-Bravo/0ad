...
class ChatHistory {
  constructor() {
    ...
    let filters = prepareForDropdown(
      this.Filters.filter((chatFilter) => !chatFilter.hidden)
    );
    this.chatHistoryFilter.list = filters.text.map((text) =>
      translateWithContext("chat history filter", text)
    );
    this.chatHistoryFilter.list_data = filters.key;
    this.chatHistoryFilter.selected = 0;
    this.chatHistoryFilter.onSelectionChange =
  ...
  onSelectionChange() {
    ...
    for (let handler of this.selectionChangeHandlers) handler();
  }

  displayChatHistory() {
    let selected =
      this.chatHistoryFilter.list_data[this.chatHistoryFilter.selected];

    this.chatHistoryText.caption = this.chatMessages
  ...
  onChatMessage(msg, formatted) {
    ...
    let historical = {
      ...
      timePrefix: sprintf(translate("\\[%(time)s]"), {
        time: Engine.FormatMillisecondsIntoDateStringLocal(
          Date.now(),
          translate("HH:mm")
        ),
      }),
    ...
    let senderID =
      msg.guid && g_PlayerAssignments[msg.guid]
        ? g_PlayerAssignments[msg.guid].player
        : 0;
...
ChatHistory.prototype.Filters = [
  {
    key: "all",
    text: markForTranslationWithContext(
      "chat history filter",

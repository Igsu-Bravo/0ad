...
class PanelEntity {
  constructor(selection, entityID, buttonID, orderKey) {
    ...
    this.panelEntityHealthBar = Engine.GetGUIObjectByName(
      "panelEntityHealthBar[" + buttonID + "]"
    );
    this.panelEntityCaptureBar = Engine.GetGUIObjectByName(
      "panelEntityCapture[" + buttonID + "]"
    );
    this.panelEntButton = Engine.GetGUIObjectByName(
      "panelEntityButton[" + buttonID + "]"
    );
    ...
    this.nameTooltip =
      setStringTags(
        g_SpecificNamesPrimary ? template.name.specific : template.name.generic,
        this.NameTags
      ) + "\n";

    Engine.GetGUIObjectByName(
      "panelEntityHealthSection[" + buttonID + "]"
    ).hidden = !entityState.hitpoints;

    let captureSection = Engine.GetGUIObjectByName(
      "panelEntityCaptureSection[" + buttonID + "]"
    );
    ...
    if (entityState.capturePoints) {
      ...
      if (!entityState.hitpoints)
        captureSection.size = Engine.GetGUIObjectByName(
          "panelEntitySectionPosTop[" + buttonID + "]"
        ).size;
    }
  ...
  update(i, reposition) {
    ...
    if (reposition) setPanelObjectPosition(this.panelEntButton, i, Infinity);
    ...
    this.panelEntButton.tooltip =
      this.nameTooltip +
      this.Tooltips.map((tooltip) => tooltip(entityState))
        .filter((tip) => tip)
        .join("\n");
  }

  updateHitpointsBar(entityState) {
    if (!entityState.hitpoints) return;
    ...
    if (entityState.hitpoints < this.hitpoints) this.onAttacked();
    this.hitpoints = entityState.hitpoints;
  }

  updateCapturePointsBar(entityState) {
    if (!entityState.capturePoints) return;
      ...
      size.rright =
        ...
        100 *
          Math.max(
            ...
            Math.min(
              1,
              entityState.capturePoints[player] / entityState.maxCapturePoints
            )
          );
      captureBar.size = size;
      captureBar.sprite =
        "color:" + g_DiplomacyColors.getPlayerColor(player, 128);
    ...
    for (let i in entityState.capturePoints)
      if (i != entityState.player) size = setCaptureBarPart(i, size);

    if (
      entityState.capturePoints[entityState.player] <
      this.capturePoints[entityState.player]
    )
      this.onAttacked();
  ...
  onPress() {
    if (!Engine.HotkeyIsPressed("selection.add")) this.selection.reset();

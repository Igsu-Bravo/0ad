...
DiplomacyDialogPlayerControl.prototype.TributeButtonManager = class {
  constructor(playerID) {
    ...
    let buttonCount = Engine.GetGUIObjectByName("diplomacyPlayer[0]_tribute")
      .children.length;

    Engine.GetGUIObjectByName("diplomacyHeaderTribute").hidden =
      !resCodes.length;
  ...
  update(playerInactive) {
    for (let button of this.buttons) button.update(playerInactive);
  }
};

DiplomacyDialogPlayerControl.prototype.TributeButtonManager.getWidthOffset =
  function () {
    let tributeButtonSize = Engine.GetGUIObjectByName(
      "diplomacyPlayer[0]_tribute[0]"
    ).size;
    return (
      g_ResourceData.GetTributableCodes().length *
      (tributeButtonSize.right - tributeButtonSize.left)
    );
  };
...
DiplomacyDialogPlayerControl.prototype.TributeButtonManager.prototype.TributeButton = class {
  constructor(playerID, resCode, resIndex) {
    ...
    let name =
      "diplomacyPlayer[" + (playerID - 1) + "]_tribute[" + resIndex + "]";
    ...
    Engine.GetGUIObjectByName(name + "_hotkey").onRelease =
      this.onMassTributeRelease.bind(this);
    Engine.GetGUIObjectByName(name + "_image").sprite =
      "stretched:" + this.ResourceIconPath + resCode + ".png";
  ...
  onPress() {
    ...
    else this.performTribute();
  }

  onMassTributeRelease() {
    if (this.amount >= this.MassAmount) this.performTribute();
  }

  setAmount(amount) {
    this.amount = amount;
    this.button.tooltip = sprintf(translate(this.Tooltip), {

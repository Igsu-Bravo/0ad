...
DiplomacyDialogPlayerControl.prototype.StanceButtonManager = class {
  constructor(playerID) {
    this.buttons = this.Stances.map(
      (stance) => new this.StanceButton(playerID, stance)
    );
  }
  ...
  update(playerInactive) {
    ...
    for (let button of this.buttons) button.update(hidden);
  }
};

DiplomacyDialogPlayerControl.prototype.StanceButtonManager.prototype.Stances = [
  "Ally",
  "Neutral",
  "Enemy",
];
...
DiplomacyDialogPlayerControl.prototype.StanceButtonManager.prototype.StanceButton = class {
  constructor(playerID, stance) {
    ...
    this.stance = stance;
    this.button = Engine.GetGUIObjectByName(
      "diplomacyPlayer" + stance + "[" + (playerID - 1) + "]"
    );
  ...
  update(hidden) {
    ...
    let isCurrentStance =
      g_Players[g_ViewedPlayer]["is" + this.stance][this.playerID];

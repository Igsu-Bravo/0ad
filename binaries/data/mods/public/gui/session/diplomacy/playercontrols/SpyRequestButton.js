...
DiplomacyDialogPlayerControl.prototype.SpyRequestButton = class {
  constructor(playerID) {
    ...
    let id = "[" + (playerID - 1) + "]";
    this.diplomacySpyRequest = Engine.GetGUIObjectByName(
      "diplomacySpyRequest" + id
    );
    this.diplomacySpyRequestImage = Engine.GetGUIObjectByName(
      "diplomacySpyRequestImage" + id
    );
  ...
  onSpyResponse(notification, player, playerInactive) {
    ...
    if (notification.target == this.playerID) {
      ...
      if (player == g_ViewedPlayer) this.update(false);
    }
  }
  ...
  update(playerInactive) {
    ...
    let hidden =
      playerInactive ||
      !template ||
      !!GetSimState().players[g_ViewedPlayer].disabledTemplates[
        this.TemplateName
      ] ||
    ...
    if (
      ...
      this.diplomacySpyRequestImage.sprite =
        this.SpriteModifierDisabled + this.Sprite;
      return;
    }
    ...
    if (template.cost) {
      ...
      for (let res in template.cost)
        modifiedTemplate.cost[res] = Math.floor(
          GetSimState().players[this.playerID].spyCostMultiplier *
            template.cost[res]
        );
      ...
      let costRatio = Engine.GetTemplate(this.TemplateName).VisionSharing
        .FailureCostRatio;
      if (costRatio) {
    ...
    this.diplomacySpyRequest.enabled =
      enabled && controlsPlayer(g_ViewedPlayer);
    this.diplomacySpyRequest.tooltip = tooltip;
    this.diplomacySpyRequestImage.sprite =
      (enabled ? "" : this.SpriteModifierDisabled) + this.Sprite;
  }
};

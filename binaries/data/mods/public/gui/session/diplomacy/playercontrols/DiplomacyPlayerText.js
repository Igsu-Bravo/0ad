...
DiplomacyDialogPlayerControl.prototype.DiplomacyPlayerText = class {
  constructor(playerID) {
    ...
    this.diplomacyPlayerCiv = Engine.GetGUIObjectByName(
      "diplomacyPlayerCiv" + id
    );
    this.diplomacyPlayerName = Engine.GetGUIObjectByName(
      "diplomacyPlayerName" + id
    );
    this.diplomacyPlayerTeam = Engine.GetGUIObjectByName(
      "diplomacyPlayerTeam" + id
    );
    this.diplomacyPlayerTheirs = Engine.GetGUIObjectByName(
      "diplomacyPlayerTheirs" + id
    );
    this.diplomacyPlayerOutcome = Engine.GetGUIObjectByName(
      "diplomacyPlayerOutcome" + id
    );
  ...
  init() {
    ...
    if (Engine.IsAtlasRunning()) return;

    this.diplomacyPlayerCiv.caption =
      g_CivData[g_Players[this.playerID].civ].Name;
    this.diplomacyPlayerName.tooltip = translateAISettings(
      g_InitAttributes.settings.PlayerData[this.playerID]
    );

    // Apply offset
    let rowSize =
      DiplomacyDialogPlayerControl.prototype.DiplomacyPlayerText.getRowHeight();
  ...
  update() {
    ...
    this.diplomacyPlayer.sprite =
      "color:" + g_DiplomacyColors.getPlayerColor(this.playerID, 32);
...
DiplomacyDialogPlayerControl.prototype.DiplomacyPlayerText.getHeightOffset =
  function () {
    return (
      (g_Players.length - 1) *
      DiplomacyDialogPlayerControl.prototype.DiplomacyPlayerText.getRowHeight()

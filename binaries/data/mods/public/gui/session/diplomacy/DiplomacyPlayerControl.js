...
class DiplomacyDialogPlayerControlManager {
  constructor() {
    ...
    for (let name in DiplomacyDialogPlayerControl.prototype) {
      ...
      for (let playerID = 1; playerID < g_Players.length; ++playerID)
        this.controls[name][playerID] =
          new DiplomacyDialogPlayerControl.prototype[name](playerID);
    }
  }

  isInactive(playerID) {
    return (
      playerID == g_ViewedPlayer ||
  ...
  onSpyResponse(notification, player) {
    for (let name in this.controls)
      for (let playerID = 1; playerID < g_Players.length; ++playerID) {
        if (!this.controls[name][playerID].onSpyResponse) break;

...
class PanelEntityManager {
  constructor(playerViewControl, selection, entityOrder) {
    this.panelEntityButtons =
      Engine.GetGUIObjectByName("panelEntityButtons").children;
  ...
  update() {
    ...
    for (let entityID of entityIDs)
      if (this.handlers.every((handler) => entityID != handler.entityID))
        if (this.insertIfRelevant(entityID)) reposition = true;
  ...
  insertIfRelevant(entityID) {
    ...
    let orderKey = this.entityOrder.findIndex(
      (entClass) => entityState.identity.classes.indexOf(entClass) != -1
    );

    // Sort depending on given order
    let insertPos = this.handlers.reduce((pos, handler) => {
      if (handler.orderKey <= orderKey) ++pos;
      return pos;
    }, 0);

    // Don't insert past the button limit
    if (insertPos >= this.panelEntityButtons.length) return false;

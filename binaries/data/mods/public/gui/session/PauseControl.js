...
class PauseControl {
  constructor() {
    ...
    registerNetworkStatusChangeHandler(
      this.onNetworkStatusChangeHandler.bind(this)
    );
  }
  ...
  callPauseHandlers() {
    for (let handler of this.pauseHandlers) handler();
  }
  ...
  canPause(explicit) {
    ...
    return (
      !g_IsNetworked ||
      (explicit && g_IsNetworkedActive && (!g_IsObserver || g_IsController))
    );
  }

  setPaused(pause, explicit) {
    if (!this.canPause(explicit)) return;

    if (explicit) this.explicitPause = pause;
  ...
  setClientPauseState(guid, paused) {
    ...
    if (paused && index == -1) this.pausingClients.push(guid);
    else if (!paused && index != -1) this.pausingClients.splice(index, 1);

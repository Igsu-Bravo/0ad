class ObjectivesDialog {
  constructor(playerViewControl, mapCache) {
    ...
    this.objectivesPlayerstate = Engine.GetGUIObjectByName(
    ...
    if (!Engine.IsAtlasRunning())
      Engine.GetGUIObjectByName("gameDescriptionText").caption =
        getGameDescription(g_InitAttributes, mapCache);

    Engine.GetGUIObjectByName("closeObjectives").onPress =
  ...
  rebuild() {
    ...
    this.objectivesPlayerstate.caption =
      g_PlayerStateMessages[playerState] || "";

    let size = this.gameDescription.size;
    size.top = (
      isActive ? this.objectivesTitle : this.objectivesPlayerstate
    ).size.bottom;

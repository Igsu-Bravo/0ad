...
class TraderStatusText {
  constructor() {
    ...
    this.components = Object.keys(this.Components.prototype).map(
      (name) => new this.Components.prototype[name]()
    );
  }

  update() {
    let traderNumber = Engine.GuiInterfaceCall(
      "GetTraderNumber",
      g_ViewedPlayer
    );
    this.traderCountText.caption = this.components
      .reduce(
        (caption, component) =>
          (caption +=
            component.getText(traderNumber, this.IdleTraderTextTags) + "\n\n"),
        ""
      )
      .trim();
  }

...
class BarterButtonManager {
  constructor(panel) {
    ...
    this.buttons = resourceCodes.map(
      (resourceCode, i) => new BarterButton(this, resourceCode, i, panel)
    );
  ...
  getSelectedButton() {
    for (let button of this.buttons)
      if (
        ...
        this.selectedResource == button.resourceCode
      )
        return button;
  }
  ...
  update() {
    if (this.viewedPlayer >= 1)
      for (let button of this.buttons) button.update(this.viewedPlayer);
  }
}

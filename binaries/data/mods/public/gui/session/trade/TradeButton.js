...
class TradeButton {
  constructor(tradeButtonManager, resourceCode, i) {
    ...
    this.tradeResourceText = Engine.GetGUIObjectByName(
      "tradeResourceText" + id
    );
    this.tradeResourceButton = Engine.GetGUIObjectByName(
      "tradeResourceButton" + id
    );
    this.tradeResourceSelection = Engine.GetGUIObjectByName(
      "tradeResourceSelection" + id
    );

    Engine.GetGUIObjectByName("tradeResourceIcon" + id).sprite =
      "stretched:" + this.ResourceIconPath + resourceCode + ".png";

    this.tradeResourceButton.onPress = () => {
      tradeButtonManager.selectResource(resourceCode);
    };

    this.tradeArrowUp.onPress = () => {
      tradeButtonManager.changeResourceAmount(
        resourceCode,
        +Math.min(
          this.AmountStep,
          tradeButtonManager.tradingGoods[tradeButtonManager.selectedResource]
        )
      );
    };
    ...
    this.tradeArrowDn.onPress = () => {
      tradeButtonManager.changeResourceAmount(
        ...
        -Math.min(
          this.AmountStep,
          tradeButtonManager.tradingGoods[resourceCode]
        )
      );
    };
  ...
  update(enabled) {
    let isSelected =
      this.tradeButtonManager.selectedResource == this.resourceCode;
    let currentAmount = this.tradeButtonManager.tradingGoods[this.resourceCode];
    let selectedAmount =
      this.tradeButtonManager.tradingGoods[
        this.tradeButtonManager.selectedResource
      ];

    this.tradeResourceText.caption = sprintf(
      translateWithContext("trading good ratio", this.AmountRatioCaption),
      {
    ...
    this.tradeArrowUp.hidden =
      !enabled || isSelected || currentAmount == 100 || selectedAmount == 0;
    this.tradeArrowDn.hidden =
      !enabled || isSelected || currentAmount == 0 || selectedAmount == 100;
  }
}

TradeButton.prototype.AmountRatioCaption = markForTranslationWithContext(
  "trading good ratio",
  "%(amount)s%%"
);

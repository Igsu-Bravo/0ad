...
class TradeButtonManager {
  constructor() {
    ...
    this.buttons = resourceCodes.map(
      (resCode, i) => new TradeButton(this, resCode, i)
    );
  ...
  update() {
    ...
    this.tradeHelp.tooltip = colorizeHotkey(
      translate(this.TradeSwapTooltip),
  ...
  fullTradeSwap(resourceCode) {
    for (let resCode in this.tradingGoods) this.tradingGoods[resCode] = 0;
...
TradeButtonManager.IsAvailable = function () {
  ...
  return (
    resourceCount &&
    resourceCount <= Engine.GetGUIObjectByName("tradeResources").children.length
  );
};

TradeButtonManager.prototype.TradeSwapTooltip = markForTranslation(

...
class BarterButton {
  constructor(barterButtonManager, resourceCode, i, panel) {
    ...
    this.sellButton.tooltip = sprintf(
      translate(this.SellTooltip),
      resourceName
    );
    this.sellButton.onPress = () => {
      barterButtonManager.setSelectedResource(this.resourceCode);
    };
    this.sellButton.hidden = false;

    this.buyButton.tooltip = sprintf(translate(this.BuyTooltip), resourceName);
    this.buyButton.onPress = () => {
      this.buy();
    };
  ...
  update(viewedPlayer) {
    ...
    let isSelected =
      this.resourceCode == this.barterButtonManager.selectedResource;
    let icon =
      "stretched:" + (isSelected ? this.SelectedModifier : "") + this.iconPath;
    this.sellIcon.sprite =
      (neededResourcesSell ? this.DisabledModifier : "") + icon;
    this.buyIcon.sprite =
      (neededResourcesBuy ? this.DisabledModifier : "") + icon;

    this.sellAmount.caption = sprintf(
      translateWithContext("sell action", this.SellCaption),
      {
        amount: this.amountToSell,
      }
    );
    ...
    this.buyAmount.caption = sprintf(
      translateWithContext("buy action", this.BuyCaption),
      {
        amount: Math.round(
          (prices.sell[this.barterButtonManager.selectedResource] /
            prices.buy[this.resourceCode]) *
            this.amountToSell
        ),
      }
    );
...
BarterButton.prototype.BuyCaption = markForTranslationWithContext(
  "buy action",
  "+%(amount)s"
);
BarterButton.prototype.SellCaption = markForTranslationWithContext(
  "sell action",
  "-%(amount)s"
);

...
function canMoveSelectionIntoFormation(formationTemplate) {
  if (formationTemplate == NULL_FORMATION) return true;
  if (!(formationTemplate in g_canMoveIntoFormation))
    g_canMoveIntoFormation[formationTemplate] = Engine.GuiInterfaceCall(
      "CanMoveEntsIntoFormation",
      {
...
function hasSameRestrictionCategory(templateName1, templateName2) {
  ...
  if (template1.trainingRestrictions && template2.trainingRestrictions)
    return (
      template1.trainingRestrictions.category ==
      template2.trainingRestrictions.category
    );

  if (template1.buildRestrictions && template2.buildRestrictions)
    return (
      template1.buildRestrictions.category ==
      template2.buildRestrictions.category
    );
...
function resourcesToAlphaMask(neededResources) {
  let totalCost = 0;
  for (let resource in neededResources) totalCost += +neededResources[resource];
  ...
  switch (name) {
    case "violent":
      return translateWithContext(
        "stance",
        "Attack nearby opponents, focus on attackers and chase while visible"
      );
    case "aggressive":
    ...
    case "defensive":
      return translateWithContext(
        "stance",
        "Attack nearby opponents, chase a short distance and return to the original location"
      );
    case "passive":
 ...
 */
function formatLimitString(
  trainEntLimit,
  trainEntCount,
  trainEntLimitChangers
) {
  if (trainEntLimit == undefined) return "";
  ...
  if (trainEntCount >= trainEntLimit) text = coloredText(text, "red");

  for (var c in trainEntLimitChangers) {
    if (!trainEntLimitChangers[c]) continue;

    let string =
      trainEntLimitChangers[c] > 0
...
function formatMatchLimitString(matchEntLimit, matchEntCount, type) {
  if (matchEntLimit == undefined) return "";
  ...
  if (type == "build") {
    if (passedLimit)
      ...
      text = sprintf(
        translatePlural(
          "Can be constructed %(count)s more time.",
          "Can be constructed %(count)s more times.",
          count
        ),
        {
          count: count,
    ...
    if (passedLimit)
      text = sprintf(
        translatePlural(
          ...
          "Could only be trained %(limit)s times.",
          matchEntLimit
        ),
        {
          limit: matchEntLimit,
    ...
    else if (matchEntLimit == 1) text = translate("Can be trained only once.");
    else
      text = sprintf(
        translatePlural(
          "Can be trained %(count)s more time.",
          "Can be trained %(count)s more times.",
          count
        ),
        {
          count: count,
    ...
    if (passedLimit)
      text = sprintf(
        translatePlural(
          ...
          "Could only be created %(limit)s times.",
          matchEntLimit
        ),
        {
          limit: matchEntLimit,
    ...
    else if (matchEntLimit == 1) text = translate("Can be created only once.");
    else
      text = sprintf(
        translatePlural(
          "Can be created %(count)s more time.",
          "Can be created %(count)s more times.",
          count
        ),
        {
          count: count,
 ...
 */
function formatBatchTrainingString(
  ...
  if (totalBatchTrainingCount < 2) return "";
  ...
  if (
    ...
    if (remainderBatch > 0)
      batchString = translate(
        "%(action)s to train %(number)s (%(fullBatch)s + %(remainderBatch)s)."
      );
    else
      batchString = translate(
        "%(action)s to train %(number)s (%(fullBatch)s)."
      );
  else batchString = translate("%(action)s to train %(number)s.");
...
function jumpCamera(index) {
  ...
  let threshold = Engine.ConfigDB_GetValue(
    "user",
    "gui.session.camerajump.threshold"
  );
...
function changePrimarySelectionGroup(templateName, deselectGroup) {
  g_Selection.makePrimarySelection(
    templateName,
...
function setCameraFollow(entity) {
  ...
  if (entState && hasClass(entState, "Unit")) Engine.CameraFollow(entity);
  else Engine.CameraFollow(0);
}
...
function unloadAll() {
  ...
  if (!garrisonHolders.length) return;

  let ownEnts = [];
  let otherEnts = [];

  for (let ent of garrisonHolders) {
    if (controlsPlayer(GetEntityState(ent).player)) ownEnts.push(ent);
...
function unloadAllTurrets() {
  ...
  if (!turretHolders.length) return;

  let ownedHolders = [];
  let ejectables = [];
  for (let ent of turretHolders) {
    let turretHolderState = GetEntityState(ent);
    if (controlsPlayer(turretHolderState.player)) ownedHolders.push(ent);
    else {
      for (let turret of turretHolderState.turretHolder.turretPoints.map(
        (tp) => tp.entity
      ))
...
function turnAutoQueueOn() {
  Engine.PostNetworkCommand({
    ...
    entities: g_Selection.filter((ent) => {
      let state = GetEntityState(ent);
      return (
        !!state?.production?.entities.length && !state.production.autoqueue
      );
    }),
  });
}
...
function turnAutoQueueOff() {
  Engine.PostNetworkCommand({
    ...
    entities: g_Selection.filter((ent) => {
      let state = GetEntityState(ent);
      return !!state?.production?.entities.length && state.production.autoqueue;
    }),
  });
}

...
class CounterPopulation {
  ...
  rebuild(playerState, getAllyStatTooltip) {
    ...
    this.stats.caption = total
      ? coloredText(total, this.DefaultTotalGatherersColor)
      : 0;
    ...
    this.panel.tooltip =
      setStringTags(
        translate(this.PopulationTooltip),
        CounterManager.ResourceTitleTags
      ) +
  ...
  onTick() {
    ...
    let newColor =
      this.isTrainingBlocked && Date.now() % 1000 < 500
...
CounterPopulation.prototype.CounterCaption = markForTranslation(
  "%(popCount)s/%(popLimit)s (%(popMax)s)"
);

CounterPopulation.prototype.PopulationTooltip = markForTranslation(
  "Population: current/limit (max)"
);

CounterPopulation.prototype.AllyPopulationTooltip = markForTranslation(
  "%(popCount)s/%(popLimit)s (%(popMax)s)"
);
 ...
 */
CounterPopulation.prototype.CurrentGatherersTooltip = setStringTags(
  translate("Gatherers: current"),
  { font: "sans-bold-16" }
);
/**

...
class CounterResource {
  ...
  rebuild(playerState, getAllyStatTooltip) {
    this.count.caption = abbreviateLargeNumbers(
      Math.floor(playerState.resourceCounts[this.resCode])
    );

    let gatherers = playerState.resourceGatherers[this.resCode];
    this.stats.caption = gatherers
      ? coloredText(gatherers, this.DefaultResourceGatherersColor)
      : 0;
    ...
    this.panel.tooltip =
      setStringTags(
        resourceNameFirstWord(this.resCode),

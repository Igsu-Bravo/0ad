...
class CounterManager {
  ...
  addCounter(resCode, type) {
    ...
    if (this.counters.length + 1 > panelCount)
      throw (
        ...
        " resource counters to display, but only " +
        panelCount +
        " panel items!"
      );
  ...
  init() {
    ...
    for (let counter of this.counters) {
      counter.icon.sprite =
        "stretched:session/icons/resources/" + counter.resCode + ".png";
  ...
  onPress() {
    Engine.ConfigDB_CreateAndWriteValueToFile(
      ...
      String(
        ((+Engine.ConfigDB_GetValue("user", "gui.session.respoptooltipsort") +
          2) %
          3) -
          1
      ),
  ...
  rebuild() {
    ...
    this.selectedOrder = +Engine.ConfigDB_GetValue(
      "user",
      "gui.session.respoptooltipsort"
    );
  ...
  getOrderTooltip() {
    if (!Object.keys(this.allyPlayerStates).length) return "";
  ...
  getAllyStatTooltip(getTooltipData) {
    ...
    for (let playerID in this.allyPlayerStates) {
      let playername =
        ...
        " " +
        g_Players[playerID].name;
      tooltipData.push(
        getTooltipData(this.allyPlayerStates[playerID], playername)
      );
    }

    if (this.selectedOrder)
      tooltipData.sort(
        (a, b) => this.selectedOrder * (b.orderValue - a.orderValue)
      );
...
CounterManager.prototype.AllyStatTooltip = markForTranslation(

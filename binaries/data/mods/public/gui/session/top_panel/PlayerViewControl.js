...
class PlayerViewControl {
  ...
  rebuild() {
    ...
    this.viewPlayer.list = [translate(this.ObserverTitle)].concat(
      g_Players.map(
  ...
  onPlayersInit() {
    ...
    this.selectViewPlayer(
      playerState && playerState.state == "defeated"
        ? 0
  ...
  onSelectionChange() {
    ...
    for (let handler of this.preViewedPlayerChangeHandlers) handler();
    ...
    if (this.changePerspective)
      for (let handler of this.playerIDChangeHandlers) handler();

    for (let handler of this.viewedPlayerChangeHandlers) handler();
  }
}

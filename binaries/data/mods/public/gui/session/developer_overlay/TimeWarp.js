...
class TimeWarp {
  ...
  setEnabled(enabled) {
    ...
    if (enabled) new TimeWarpMessageBox().display();

    Engine.EnableTimeWarpRecording(enabled ? this.NumberTurns : 0);
  }

  rewind() {
    if (this.enabled) Engine.RewindTimeWarp();
  }

  fastForward() {
    if (this.enabled) Engine.SetSimRate(this.FastForwardSpeed);
  }

  resetSpeed() {
    if (this.enabled) Engine.SetSimRate(1);
  }
}

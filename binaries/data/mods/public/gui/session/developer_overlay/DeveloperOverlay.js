...
class DeveloperOverlay {
  constructor(playerViewControl, selection) {
    ...
    this.checkBoxes = this.getCheckboxNames().map(
      (name, i) =>
        new DeveloperOverlayCheckbox(
          new DeveloperOverlayCheckboxes.prototype[name](
            playerViewControl,
            selection
          ),
          i
  ...
  toggle() {
    if (g_IsNetworked && !g_InitAttributes.settings.CheatsEnabled) return;
  ...
  sendNotification() {
    let message = this.devCommandsOverlay.hidden
      ? this.CloseNotification
      : this.OpenNotification;

    // Only players can send the simulation chat command
    if (Engine.GetPlayerID() == -1) g_Chat.submitChat(message);
    else
  ...
  resize() {
    ...
    size.bottom =
      ...
      this.checkBoxes.reduce(
        (height, checkbox) => height + checkbox.getHeight(),
        0
      );
...
DeveloperOverlay.prototype.OpenNotification = markForTranslation(
  "The Developer Overlay was opened."
);
DeveloperOverlay.prototype.CloseNotification = markForTranslation(
  "The Developer Overlay was closed."
);

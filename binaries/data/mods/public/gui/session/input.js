...
function updateCursorAndTooltip() {
  ...
  if (
    ...
    (inputState == INPUT_NORMAL &&
      Engine.HotkeyIsPressed("session.flare") &&
      !g_IsObserver)
  ) {
    ...
    g_MiniMapPanel.isMouseOverMiniMap()
  ) {
...
function updateBuildingPlacementPreview() {
  ...
  if (placementSupport.mode === "building") {
    ...
    placementSupport.wallSet &&
    placementSupport.position
  ) {
...
function tryPlaceBuilding(queued, pushFront) {
  ...
  let selection =
    (Engine.HotkeyIsPressed("session.orderone") &&
      popOneFromSelection({ type: "construct", target: placementSupport })) ||
    g_Selection.toList();
    ...
    case INPUT_BANDBOXING:
      ...
      switch (ev.type) {
        case "mousemotion": {
          ...
          let ents = Engine.PickPlayerEntitiesInRect(
            rect[0],
            rect[1],
            rect[2],
            rect[3],
      ...
      switch (ev.type) {
        ...
        case "mousebuttonup":
          if (ev.button == SDL_BUTTON_LEFT) {
            ...
            } else inputState = INPUT_BUILDING_PLACEMENT;
            return true;
          }
          break;
      ...
      switch (ev.type) {
        case "mousemotion":
          ...
          if (result && result.cost) {
            let neededResources = Engine.GuiInterfaceCall(
              "GetNeededResources",
              { cost: result.cost }
            );
            placementSupport.tooltipMessage = [
    ...
    case INPUT_BUILDING_DRAG:
      switch (ev.type) {
        case "mousemotion":
          ...
          else placementSupport.SetDefaultAngle();
          ...
          let snapData = Engine.GuiInterfaceCall("GetFoundationSnapData", {
            ...
            angle: placementSupport.angle,
            snapToEdges:
              isSnapToEdgesEnabled() &&
              Engine.GetEdgesOfStaticObstructionsOnScreenNearTo(
                placementSupport.position.x,
                placementSupport.position.z
              ),
          });
          if (snapData) {
      ...
      switch (ev.type) {
        ...
        case "mousebuttondown":
          ...
          if (ev.button == SDL_BUTTON_LEFT) {
            ...
            if (!controlsPlayer(g_ViewedPlayer)) break;
            g_DragStart = new Vector2D(ev.x, ev.y);
        ...
        case "mousebuttonup":
          if (ev.button == SDL_BUTTON_LEFT) {
            ...
            if (ev.clicks == 1) ents = [clickedEntity];
              ...
              if (ev.clicks == 2) {
                templateToMatch =
                  GetEntityState(clickedEntity).identity.selectionGroupName;
                if (templateToMatch) matchRank = false;
                // No selection group name defined, so fall back to exact match.
                else templateToMatch = GetEntityState(clickedEntity).template;
              }
              // Triple click
        ...
        case "mousebuttondown":
          if (ev.button == SDL_BUTTON_LEFT) {
            if (placementSupport.mode === "wall") {
              ...
              if (isSnapToEdgesEnabled()) {
                let snapData = Engine.GuiInterfaceCall(
...
function popOneFromSelection(action) {
  ...
  let unit =
    action.firstAbleEntity ||
    g_Selection.find((entity) =>
...
function triggerFlareAction(target) {
  let now = Date.now();
  if (g_LastFlareTime && now < g_LastFlareTime + g_FlareCooldown) return;
...
function getEntityLimitAndCount(playerState, entType) {
  ...
  if (matchLimit) {
    ...
    ret.canBeAddedCount = Math.min(
      Math.max(ret.entLimit - ret.entCount, 0),
      Math.max(ret.matchLimit - ret.matchCount, 0)
    );
  }
  return ret;
}
...
function OnTrainMouseWheel(dir) {
  if (!Engine.HotkeyIsPressed("session.batchtrain")) return;
...
function initBatchTrain() {
  registerConfigChangeHandler((changes) => {
    if (changes.has("gui.session.batchtrainingsize")) updateDefaultBatchSize();
  });
}
...
function addTrainingToQueue(selection, trainEntType, playerState) {
  ...
  let canBeAddedCount = getEntityLimitAndCount(
    playerState,
    trainEntType
  ).canBeAddedCount;
  ...
  if (
    ...
    if (inputState == INPUT_BATCHTRAINING) {
      ...
      if (
        ...
        return;
      } else if (!decrement) flushTrainingBatch();
    }
...
function getTrainingStatus(selection, trainEntType, playerState) {
  ...
  if (
    ...
    canBeAddedCount = getEntityLimitAndCount(
      playerState,
      trainEntType
    ).canBeAddedCount;
...
function performGroup(action, groupId) {
  switch (action) {
    ...
    case "add":
      let toSelect = [];
      g_Groups.update();
      for (let ent in g_Groups.groups[groupId].ents) toSelect.push(+ent);
...
function findIdleUnit(classes) {
  ...
  let indexChange = data.idleClasses.findIndex((elem) =>

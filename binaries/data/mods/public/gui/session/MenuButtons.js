...
MenuButtons.prototype.Summary = class {
  ...
  rebuild() {
    this.button.tooltip = sprintf(
      translate("Press %(hotkey)s to open the summary screen."),
      {
    ...
    Engine.PushGuiPage(
      "page_summary.xml",
      {
        sim: {
          ...
          playerStates: simState.players.filter(
            (state, player) =>
              g_IsObserver ||
              g_ViewedPlayer == 0 ||
              player == 0 ||
              player == g_ViewedPlayer ||
              (simState.players[g_ViewedPlayer].hasSharedLos &&
...
MenuButtons.prototype.Options = class {
  ...
  onPress() {
    ...
    Engine.PushGuiPage("page_options.xml", {}, (changes) => {
...
MenuButtons.prototype.Pause = class {
  ...
  rebuild() {
    ...
    this.button.caption = this.pauseControl.explicitPause
      ? translate("Resume")
      : translate("Pause");
    this.button.tooltip = sprintf(
      translate("Press %(hotkey)s to pause or resume the game."),
      {
        hotkey: colorizeHotkey("%(hotkey)s", this.button.hotkey),
      }
    );
...
MenuButtons.prototype.Exit = class {
  ...
  onPress() {
    for (let name in QuitConfirmationMenu.prototype) {
      ...
      if (quitConfirmation.enabled()) quitConfirmation.display();
    }
  }
};

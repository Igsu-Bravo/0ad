...
class Menu {
  constructor(pauseControl, playerViewControl, chat) {
    ...
    this.buttons = handlerNames.map((handlerName, i) => {
      let handler = new MenuButtons.prototype[handlerName](
        menuButtons[i],
        pauseControl,
        playerViewControl,
        chat
    ...
    this.endPosition =
      this.margin + this.buttonHeight * (1 + handlerNames.length);
  ...
  rebuild() {
    this.menuButton.tooltip = sprintf(
      translate("Press %(hotkey)s to toggle this menu."),
      {
  ...
  onTick() {
    ...
    let offset =
      Math.min(this.Speed * tickLength, maxOffset) * (this.isOpen ? +1 : -1);
    let size = this.menuButtonPanel.size;
    size.top += offset;
    size.bottom += offset;
    this.menuButtonPanel.size = size;

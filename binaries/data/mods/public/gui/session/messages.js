...
var g_NetMessageTypes = {
  ...
  "out-of-sync": (msg) => {
    for (let handler of g_NetworkOutOfSyncHandlers) handler(msg);
  },
  players: (msg) => {
  ...
  "clients-loading": (msg) => {
    for (let handler of g_ClientsLoadingHandlers) handler(msg.guids);
  },
  rejoined: (msg) => {
    addChatMessage({
      type: "rejoined",
...
var g_NotificationsTypes = {
  ...
  "ceasefire-ended": function (notification, player) {
    updatePlayerData();
    for (let handler of g_CeasefireEndedHandlers) handler();
  },
  ...
  attack: function (notification, player) {
    if (player != g_ViewedPlayer) return;
    ...
    if (g_FollowPlayer) {
      ...
      g_Selection.reset();
      if (notification.target) g_Selection.addList([notification.target]);
    }
    ...
    if (
      Engine.ConfigDB_GetValue("user", "gui.session.notifications.attack") !==
      "true"
    )
      return;
  ...
  playercommand: function (notification, player) {
    ...
    if (!g_FollowPlayer || player != g_ViewedPlayer) return;

    let cmd = notification.cmd;

    // Ignore rallypoint commands of trained animals
    let entState =
      cmd.entities && cmd.entities[0] && GetEntityState(cmd.entities[0]);
    if (
      g_ViewedPlayer != 0 &&
      entState &&
      entState.identity &&
    ...
    let selection = [];
    if (cmd.entities) selection = cmd.entities;
    if (cmd.target) selection.push(cmd.target);
  ...
  "play-tracks": function (notification, player) {
    if (notification.lock) {
      global.music.storeTracks(
        notification.tracks.map((track) => ({ Type: "custom", File: track }))
      );
  ...
  "map-flare": function (notification, player) {
    ...
    if (
      ...
      g_Players[player].isMutualAlly[Engine.GetPlayerID()]
    ) {
      let now = Date.now();
      if (g_FlareRateLimitLastTimes.length) {
        g_FlareRateLimitLastTimes = g_FlareRateLimitLastTimes.filter(
          (t) => now - t < g_FlareRateLimitScope * 1000
        );
        if (g_FlareRateLimitLastTimes.length >= g_FlareRateLimitMaximumFlares) {
          warn(
            "Received too many flares. Dropping a flare request by '" +
              g_Players[player].name +
              "'."
          );
          return;
        }
...
function findGuidForPlayerID(playerID) {
  return Object.keys(g_PlayerAssignments).find(
    (guid) => g_PlayerAssignments[guid].player == playerID
  );
}
...
function handleNotifications() {
  for (let notification of Engine.GuiInterfaceCall("GetNotifications")) {
    if (
      ...
      !notification.type ||
      !g_NotificationsTypes[notification.type]
    ) {
...
function focusAttack(attack) {
  ...
  if (entState && hasClass(entState, "Unit")) setCameraFollow(attack.target);
  else {
...
function updateTutorial(notification) {
  ...
  if (notification.warning) {
    Engine.GetGUIObjectByName("tutorialWarning").caption = coloredText(
      translate(notification.warning),
      "orange"
    );
    return;
  }

  let notificationText = notification.instructions.reduce(
    (instructions, item) =>
      instructions +
      (typeof item == "string"
        ? translate(item)
        : colorizeHotkey(translate(item.text), item.hotkey)),
    ""
  );

  Engine.GetGUIObjectByName("tutorialText").caption = g_TutorialMessages
    .concat(setStringTags(notificationText, g_TutorialNewMessageTags))
    .join("\n");
  ...
  if (notification.readyButton) {
    ...
    if (notification.leave) {
      Engine.GetGUIObjectByName("tutorialWarning").caption = translate(
        "Click to quit this tutorial."
      );
      Engine.GetGUIObjectByName("tutorialReady").caption = translate("Quit");
      Engine.GetGUIObjectByName("tutorialReady").onPress = endGame;
    } else
      Engine.GetGUIObjectByName("tutorialWarning").caption =
        translate("Click when ready.");
  } else {
    Engine.GetGUIObjectByName("tutorialWarning").caption = translate(
...
function handleNetMessages() {
  while (true) {
    ...
    if (g_NetMessageTypes[msg.type]) g_NetMessageTypes[msg.type](msg);
    else error("Unrecognised net message type '" + msg.type + "'");
  }
}
...
function handleNetStatusMessage(message) {
  ...
  for (let handler of g_NetworkStatusChangeHandlers) handler(message);
}

function handlePlayerAssignmentsMessage(message) {
  for (let guid in g_PlayerAssignments)
    if (!message.newAssignments[guid]) onClientLeave(guid);

  let joins = Object.keys(message.newAssignments).filter(
    (guid) => !g_PlayerAssignments[guid]
  );
  ...
  for (let handler of g_PlayerAssignmentsChangeHandlers) handler();
...
function onClientLeave(guid) {
  ...
  for (let id in g_Players)
    if (g_Players[id].guid == guid) g_Players[id].offline = true;
...
function colorizePlayernameByGUID(guid) {
  let username = g_PlayerAssignments[guid]
    ? g_PlayerAssignments[guid].name
    : "";
  let playerID = g_PlayerAssignments[guid]
    ? g_PlayerAssignments[guid].player
    : -1;
  return colorizePlayernameHelper(username, playerID);
}
...
function colorizePlayernameHelper(username, playerID) {
  let playerColor =
    playerID > -1 ? g_DiplomacyColors.getPlayerColor(playerID) : "white";
  ...
  for (let objName in data) {
    ...
    for (let key in data[objName]) {
      ...
      if (typeof n == "object" && n.message) {
        ...
        if (n.translateMessage) message = translate(message);

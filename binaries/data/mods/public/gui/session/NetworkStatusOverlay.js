...
class NetworkStatusOverlay {
  ...
  onClientsLoadingMessage(guids) {
    this.loadingClientsText.caption = guids
...
NetworkStatusOverlay.prototype.StatusCaption = {
  authenticated: (msg) =>
    translate("Connection to the server has been authenticated."),
  connected: (msg) => translate("Connected to the server."),
  disconnected: (msg) =>
    translate("Connection to the server has been lost.") +
    "\n" +
    getDisconnectReason(msg.reason, true),
  waiting_for_players: (msg) => translate("Waiting for players to connect:"),
  join_syncing: (msg) =>
    translate("Synchronizing gameplay with other players…"),
  active: (msg) => "",
};

NetworkStatusOverlay.prototype.Comma = translateWithContext(
  "Separator for a list of client loading messages",
  ", "
);

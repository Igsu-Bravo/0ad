...
class ResearchProgress {
  constructor(playerViewControl, selection) {
    ...
    this.buttonHandlers = this.buttons.map(
      (button, i) => new ResearchProgressButton(selection, i)
    );
  ...
  updateResearchProgress() {
    ...
    for (let techName in researchStarted) {
      if (i == this.buttons.length) break;

      this.buttonHandlers[i++].onResearchedProgress(
        this.topOffset,
        techName,
        researchStarted[techName]
      );
    }

    while (i < this.buttons.length) this.buttons[i++].hidden = true;
  }
}
...
class ResearchProgressButton {
  constructor(selection, i) {
    ...
    this.progress = Engine.GetGUIObjectByName(
      "researchStartedProgressSlider[" + i + "]"
    );
    this.timeRemaining = Engine.GetGUIObjectByName(
      "researchStartedTimeRemaining[" + i + "]"
    );
  ...
  onResearchedProgress(offset, techName, researchStatus) {
    ...
    this.timeRemaining.caption = Engine.FormatMillisecondsIntoDateStringGMT(
 ...
 */
ResearchProgressButton.prototype.CountdownFormat =

 ...
 */
const g_InitAttributes = deepfreeze(
  Engine.GuiInterfaceCall("GetInitAttributes")
);
...
function GetTemplateData(templateName, player) {
  if (!(templateName in g_TemplateData)) {
    let template = Engine.GuiInterfaceCall("GetTemplateData", {
...
function init(initData, hotloadData) {
  ...
  g_PlayerViewControl.registerViewedPlayerChangeHandler(
    g_DiplomacyColors.updateDisplayedPlayerColors.bind(g_DiplomacyColors)
  );
  g_DiplomacyColors.registerDiplomacyColorsChangeHandler(
    g_PlayerViewControl.rebuild.bind(g_PlayerViewControl)
  );
  g_DiplomacyColors.registerDiplomacyColorsChangeHandler(updateGUIObjects);
  g_PauseControl = new PauseControl();
  g_PlayerViewControl.registerPreViewedPlayerChangeHandler(
  ...
  g_PanelEntityManager = new PanelEntityManager(
    g_PlayerViewControl,
    g_Selection,
    g_PanelEntityOrder
  );
  ...
  for (let handler of g_PlayersInitHandlers) handler();

  for (let handler of g_HotkeyChangeHandlers) handler();
...
function playersFinished(players, victoryString, won) {
  ...
  for (let handler of g_PlayerFinishedHandlers) handler(players, won);
...
function onTick() {
  ...
  if (g_Selection.dirty) {
    ...
    GetMultipleEntityStates(
      g_Selection.filter((entId) => !g_EntityStates[entId])
    );

    for (let handler of g_EntitySelectionChangeHandlers) handler();
  ...
  if (isPlayingCinemaPath) updateCinemaOverlay();
}
...
function onSimulationUpdate() {
  ...
  for (let handler of g_SimulationUpdateHandlers) handler();
...
function recalculateStatusBarDisplay(remove = false) {
  ...
  if (g_ShowAllStatusBars && !remove)
    ...
    for (const ent of g_Selection.highlighted) selected.push(ent);
  ...
  Engine.GuiInterfaceCall("SetStatusBars", {
    ...
    showRank:
      ...
      "true",
    showExperience:
      Engine.ConfigDB_GetValue("user", "gui.session.experiencestatusbar") ==
      "true",
  });
}
...
function updateAdditionalHighlight() {
  ...
  for (const ent of g_Selection.highlighted) highlighted.push(ent);

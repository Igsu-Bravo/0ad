...
class PauseOverlay {
  ...
  rebuild() {
    let hidden =
      (!this.pauseControl.explicitPause &&
        !this.pauseControl.pausingClients.length) ||
      g_Disconnected;
    ...
    this.pausedByText.caption = sprintf(translate(this.PausedByCaption), {
      players: this.pauseControl.pausingClients
...
PauseOverlay.prototype.PausedByCaption = markForTranslation(
  "Paused by %(players)s"
);

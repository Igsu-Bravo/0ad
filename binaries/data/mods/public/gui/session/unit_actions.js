...
var g_UnitActions = {
  move: {
    ...
    getActionInfo: function (entState, targetState) {
      if (!entState.unitAI) return false;
  ...
  "attack-move": {
    execute: function (target, action, selection, queued, pushFront) {
      ...
      else targetClasses = { attack: ["Unit", "Structure"] };
    ...
    hotkeyActionCheck: function (target, selection) {
      return isAttackMovePressed() && this.actionCheck(target, selection);
    },
    actionCheck: function (target, selection) {
  ...
  attack: {
    ...
    hotkeyActionCheck: function (target, selection) {
      return (
        Engine.HotkeyIsPressed("session.attack") &&
        this.actionCheck(target, selection)
      );
    },
    actionCheck: function (target, selection) {
  ...
  patrol: {
    ...
    getActionInfo: function (entState, targetState) {
      if (!entState.unitAI || !entState.unitAI.canPatrol) return false;
  ...
  heal: {
    ...
    getActionInfo: function (entState, targetState) {
      if (
        !entState.heal ||
        !targetState ||
        !hasClass(targetState, "Unit") ||
        !targetState.needsHeal ||
  ...
  construct: {
    preSelectedActionCheck: function (target, selection) {
      ...
      if (
        ...
        target &&
        target.constructor &&
        target.constructor.name == "PlacementSupport"
      )
  ...
  repair: {
    ...
    hotkeyActionCheck: function (target, selection) {
      return (
        Engine.HotkeyIsPressed("session.repair") &&
        this.actionCheck(target, selection)
      );
    },
    actionCheck: function (target, selection) {
  ...
  gather: {
    ...
    getActionInfo: function (entState, targetState) {
      if (
        !entState.resourceGatherRates ||
        !targetState ||
        !targetState.resourceSupply
      )
        return false;
      ...
      if (
        entState.resourceGatherRates[
          targetState.resourceSupply.type.generic +
            "." +
            targetState.resourceSupply.type.specific
        ]
      )
        resource = targetState.resourceSupply.type.specific;
      else if (
        entState.resourceGatherRates[targetState.resourceSupply.type.generic]
      )
  ...
  returnresource: {
    ...
    getActionInfo: function (entState, targetState) {
      if (!targetState || !targetState.resourceDropsite) return false;

      let playerState = GetSimState().players[entState.player];
      if (
        playerState.hasSharedDropsites &&
        targetState.resourceDropsite.shared
      ) {
        if (!playerCheck(entState, targetState, ["Player", "MutualAlly"]))
          return false;
      } else if (!playerCheck(entState, targetState, ["Player"])) return false;
  ...
  "cancel-setup-trade-route": {
    ...
    getActionInfo: function (entState, targetState) {
      ...
      if (!tradingDetails || !tradingDetails.type) return false;
  ...
  "setup-trade-route": {
    ...
    getActionInfo: function (entState, targetState) {
      ...
      if (!tradingDetails) return false;
      ...
      switch (tradingDetails.type) {
        ...
        case "is second":
          tooltip =
            translate("Destination trade market.") +
            "\n" +
          ...
          if (tradingDetails.gain.traderGain == 0)
            return {
              ...
              tooltip: setStringTags(
                translate("This market is too close to the origin market."),
                g_DisabledTags
              ),
              disabled: true,
            };
  ...
  "occupy-turret": {
    ...
    getActionInfo: function (entState, targetState) {
      if (
        !entState.turretable ||
        !targetState ||
        !targetState.turretHolder ||
        !playerCheck(entState, targetState, ["Player", "MutualAlly"])
      )
        return false;

      if (
        !targetState.turretHolder.turretPoints.find(
          (point) =>
            !point.allowedClasses ||
            MatchesClassList(entState.identity.classes, point.allowedClasses)
        )
      )
        return false;

      let occupiedTurrets = targetState.turretHolder.turretPoints.filter(
        (point) => point.entity != null
      );
      let tooltip = sprintf(
        translate("Current turrets: %(occupied)s/%(capacity)s"),
        {
      ...
      if (
        occupiedTurrets.length == targetState.turretHolder.turretPoints.length
      )
    ...
    hotkeyActionCheck: function (target, selection) {
      return (
        Engine.HotkeyIsPressed("session.occupyturret") &&
        this.actionCheck(target, selection)
      );
    },
    actionCheck: function (target, selection) {
  ...
  garrison: {
    ...
    getActionInfo: function (entState, targetState) {
      ...
      let tooltip = sprintf(
        translate("Current garrison: %(garrisoned)s/%(capacity)s"),
        {
      ...
      if (
        targetState.garrisonHolder.occupiedSlots + extraCount >
      ...
      if (
        !MatchesClassList(
          ...
          targetState.garrisonHolder.allowedClasses
        )
      )
        return false;
  ...
  guard: {
    ...
    hotkeyActionCheck: function (target, selection) {
      return (
        Engine.HotkeyIsPressed("session.guard") &&
        this.actionCheck(target, selection)
      );
    },
    actionCheck: function (target, selection) {
  ...
  "collect-treasure": {
    ...
    getActionInfo: function (entState, targetState) {
      if (!entState.treasureCollector || !targetState || !targetState.treasure)
        return false;
  ...
  "remove-guard": {
    ...
    getActionInfo: function (entState, targetState) {
      if (!entState.unitAI || !entState.unitAI.isGuarding) return false;
      return { possible: true };
    },
    hotkeyActionCheck: function (target, selection) {
      return (
  ...
  "set-rallypoint": {
    execute: function (target, action, selection, queued, pushFront) {
      ...
      if (action.position) target = action.position;
    ...
    getActionInfo: function (entState, targetState) {
      if (!entState.rallyPoint) return false;
      ...
      if (
        ...
        for (const ent of g_Selection.toList())
          if (targetState.id == ent) return false;
      ...
      if (
        ...
        if (
          targetState.garrisonHolder.occupiedSlots >=
          targetState.garrisonHolder.capacity
        )
          tooltip = coloredText(tooltip, "orange");
      } else if (
        ...
        if (entState.production && entState.production.entities.length)
          for (let i = 0; i < entState.production.entities.length; ++i)
            if (
              (trader = GetTemplateData(entState.production.entities[i]).trader)
            )
              break;
        ...
        if (gain) {
          ...
          if (gain.traderGain)
            tooltip =
              translate(
                "Right-click to establish a default route for new traders."
              ) +
              "\n" +
              sprintf(
                trader
    ...
    actionCheck: function (target, selection) {
      ...
      if (
        ...
        selection.some((ent) => {
          let entState = GetEntityState(ent);
          return (
            entState && entState.identity && entState.identity.controllable
          );
        })
      )
        return false;
      ...
      return {
        type: "none",
        cursor: "cursor-no",
        tooltip: translatePlural(
          "This entity cannot be controlled.",
          "These entities cannot be controlled.",
          selection.length
        ),
      };
    },
...
var g_UnitActionsSortedKeys = Object.keys(g_UnitActions).sort(
...
var g_EntityCommands = {
  "unload-all": {
    getInfo: function (entStates) {
      ...
      for (let entState of entStates) {
        if (!entState.garrisonHolder) continue;
      ...
      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "session.unload") +
  ...
  "unload-all-turrets": {
    getInfo: function (entStates) {
      let count = 0;
      for (let entState of entStates) {
        if (!entState.turretHolder) continue;

        if (allowedPlayersCheck([entState], ["Player"]))
          count += entState.turretHolder.turretPoints.filter(
            (turretPoint) => turretPoint.entity && turretPoint.ejectable
          ).length;
        else
          for (let turretPoint of entState.turretHolder.turretPoints)
            if (
              turretPoint.entity &&
              allowedPlayersCheck(
                [GetEntityState(turretPoint.entity)],
                ["Player"]
              )
            )
              ++count;
      ...
      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "session.unloadturrets") +
  ...
  delete: {
    getInfo: function (entStates) {
      return entStates.some((entState) => !isUndeletable(entState))
        ...
        : {
            ...
            tooltip: entStates
              .map((entState) => isUndeletable(entState))
              .filter(
                (reason, pos, self) => self.indexOf(reason) == pos && reason
              )
              .join("\n"),
    ...
    execute: function (entStates) {
      let entityIDs = entStates.reduce((ids, entState) => {
        if (!isUndeletable(entState)) ids.push(entState.id);
        return ids;
      }, []);
      ...
      if (Engine.HotkeyIsPressed("session.noconfirmation")) deleteSelection();
      else new DeleteSelectionConfirmation(deleteSelection).display();
    },
  ...
  stop: {
    getInfo: function (entStates) {
      if (entStates.every((entState) => !entState.unitAI)) return false;

      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "session.stop") +
    ...
    execute: function (entStates) {
      if (entStates.length) stopUnits(entStates.map((entState) => entState.id));
    },
    allowedPlayers: ["Player"],
  },
  ...
  garrison: {
    getInfo: function (entStates) {
      ...
      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "session.garrison") +
          translate(
            "Order the selected units to garrison in a structure or unit."
          ),
        icon: "garrison.png",
        enabled: true,
      };
  ...
  "occupy-turret": {
    getInfo: function (entStates) {
      if (
        entStates.every(
          (entState) =>
            !entState.turretable || entState.turretable.holder != INVALID_ENTITY
        )
      )
        return false;
      ...
      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "session.occupyturret") +
  ...
  repair: {
    getInfo: function (entStates) {
      if (entStates.every((entState) => !entState.builder)) return false;

      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "session.repair") +
          translate(
            "Order the selected units to repair a structure, ship, or siege engine."
          ),
        icon: "repair.png",
        enabled: true,
      };
  ...
  "focus-rally": {
    getInfo: function (entStates) {
      if (entStates.every((entState) => !entState.rallyPoint)) return false;

      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "camera.rallypointfocus") +
    ...
    execute: function (entStates) {
      ...
      if (focusTarget) Engine.CameraMoveTo(focusTarget.x, focusTarget.z);
    },
  ...
  "back-to-work": {
    getInfo: function (entStates) {
      if (
        entStates.every(
          (entState) => !entState.unitAI || !entState.unitAI.hasWorkOrders
        )
      )
        return false;
      ...
      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "session.backtowork") +
  ...
  "add-guard": {
    getInfo: function (entStates) {
      ...
      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "session.guard") +
  ...
  "remove-guard": {
    getInfo: function (entStates) {
      if (
        entStates.every(
          (entState) => !entState.unitAI || !entState.unitAI.isGuarding
        )
      )
        return false;
  ...
  "select-trading-goods": {
    getInfo: function (entStates) {
      if (entStates.every((entState) => !entState.market)) return false;
  ...
  patrol: {
    getInfo: function (entStates) {
      if (
        !entStates.some(
          (entState) => entState.unitAI && entState.unitAI.canPatrol
        )
      )
        return false;
      ...
      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "session.patrol") +
          translate("Patrol") +
          "\n" +
          translate(
            "Attack all encountered enemy units while avoiding structures."
          ),
        icon: "patrol.png",
        enabled: true,
      };
  ...
  "share-dropsite": {
    getInfo: function (entStates) {
      ...
      if (!sharableEntities.length) return false;

      // Returns if none of the entities belong to a player with a mutual ally.
      if (
        entStates.every(
          (entState) =>
            !GetSimState().players[entState.player].isMutualAlly.some(
      ...
      return sharableEntities.some(
        (entState) => !entState.resourceDropsite.shared
      )
        ? {
        ...
        : {
            tooltip: translate(
              "Press to prevent allies from using this dropsite"
            ),
  ...
  "is-dropsite-shared": {
    getInfo: function (entStates) {
      ...
      if (!shareableEntities.length) return false;

      let player = Engine.GetPlayerID();
      let simState = GetSimState();
      if (
        (!g_IsObserver && !simState.players[player].hasSharedDropsites) ||
        shareableEntities.every((entState) => controlsPlayer(entState.player))
      )
        return false;

      if (
        !shareableEntities.every((entState) => entState.resourceDropsite.shared)
      )
        return {
      ...
      return {
        tooltip: g_IsObserver
          ? translate("Allies are allowed to use this dropsite.")
  ...
  "autoqueue-on": {
    getInfo: function (entStates) {
      if (
        entStates.every(
          (entState) =>
            !entState.production ||
            !entState.production.entities.length ||
            entState.production.autoqueue
        )
      )
        return false;
      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "session.queueunit.autoqueueon") +
    ...
    execute: function (entStates) {
      if (entStates.length) turnAutoQueueOn();
    },
    allowedPlayers: ["Player"],
  },
  ...
  "autoqueue-off": {
    getInfo: function (entStates) {
      ...
      return {
        tooltip:
          colorizeHotkey("%(hotkey)s" + " ", "session.queueunit.autoqueueoff") +
    ...
    execute: function (entStates) {
      if (entStates.length) turnAutoQueueOff();
    },
    allowedPlayers: ["Player"],
  },
};
...
function playerCheck(entState, targetState, validPlayers) {
  ...
  for (let player of validPlayers)
    if (
      (player == "Gaia" && targetState.player == 0) ||
      (player == "Player" && targetState.player == entState.player) ||
      (playerState["is" + player] &&
        playerState["is" + player][targetState.player])
    )
      return true;
...
function hasClass(entState, className) {
  ...
  return (
    entState.identity && entState.identity.classes.indexOf(className) != -1
  );
}
...
function isUndeletable(entState) {
  ...
  if (playerState && playerState.controlsAll) return false;

  if (entState.resourceSupply && entState.resourceSupply.killBeforeGather)
    return translate(
      "The entity has to be killed before it can be gathered from"
    );
...
function getActionInfo(action, target, selection) {
  ...
  for (let entityID of selection) {
    ...
    if (
      ...
      !entState.identity.controllable
    )
      continue;

    if (g_UnitActions[action] && g_UnitActions[action].getActionInfo) {
      let r = g_UnitActions[action].getActionInfo(
        entState,
        targetState,
        simState
      );

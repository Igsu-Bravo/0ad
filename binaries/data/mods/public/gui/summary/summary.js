...
function initGUIWindow() {
  ...
  summaryWindow.sprite = g_GameData.gui.dialog
    ? "ModernDialog"
    : "ModernWindow";
  summaryWindow.size = g_GameData.gui.dialog
    ? "16 24 100%-16 100%-24"
    : "0 0 100% 100%";
  Engine.GetGUIObjectByName("summaryWindowTitle").size = g_GameData.gui.dialog
    ? "50%-128 -16 50%+128 16"
    : "50%-128 4 50%+128 36";
}
...
function selectPanelGUI(panel) {
  ...
  if (chartsHidden) updatePanelData(g_ScorePanelsData[panel]);
  else [0, 1].forEach(updateCategoryDropdown);
}

function constructPlayersWithColor(color, playerListing) {
  return sprintf(
    translateWithContext(
      "Player listing with color indicator",
      "%(colorIndicator)s %(playerListing)s"
    ),
    {
      colorIndicator: setStringTags(
        translateWithContext("Charts player color indicator", "■"),
        { color: color }
      ),
  ...
  for (let i = 0; i < 2; ++i)
    Engine.GetGUIObjectByName("chart[" + i + "]").series_color =
      Engine.GetGUIObjectByName("toggleTeamBox").checked
        ? g_Teams
            .filter((el) => el !== null)
            .map((players) => playerColors[players[0] - 1])
        : playerColors;
  ...
  chartLegend.caption = (
    Engine.GetGUIObjectByName("toggleTeamBox").checked
      ? g_Teams
          .filter((el) => el !== null)
          .map((players) =>
            constructPlayersWithColor(
              playerColors[players[0] - 1],
              players
                .map((player) => g_GameData.sim.playerStates[player].name)
                .join(translateWithContext("Player listing", ", "))
            )
          )
      : g_GameData.sim.playerStates
          .slice(1)
          .map((state, index) =>
...
function resizeDropdown(dropdown) {
  ...
  size.bottom =
    ...
    dropdown.size.right - dropdown.size.left - 32
      ? 42
      : 27);
  dropdown.size = size;
}
...
function updateCategoryDropdown(number) {
  let chartCategory = Engine.GetGUIObjectByName(
    "chart[" + number + "]CategorySelection"
  );
  ...
  chartCategory.onSelectionChange = function () {
    if (!this.list_data[this.selected]) return;
...
function updateValueDropdown(number, category) {
  let chartValue = Engine.GetGUIObjectByName(
    "chart[" + number + "]ValueSelection"
  );
  let list = g_ScorePanelsData[category].headings.map(
    (heading) => heading.caption
  );
  list.shift();
  chartValue.list = list;
  let list_data = g_ScorePanelsData[category].headings.map(
    (heading) => heading.identifier
  );
  list_data.shift();
...
function updateTypeDropdown(number, category, item, itemNumber) {
  let testValue = g_ScorePanelsData[category].counters[itemNumber].fn(
    g_GameData.sim.playerStates[1],
    0,
    item
  );
  let hide =
    !g_ScorePanelsData[category].counters[itemNumber].fn ||
    typeof testValue != "object" ||
    Object.keys(testValue).length < 2;
  Engine.GetGUIObjectByName("chart[" + number + "]TypeLabel").hidden = hide;
  let chartType = Engine.GetGUIObjectByName(
    "chart[" + number + "]TypeSelection"
  );
  ...
  if (hide) {
    updateChart(
      ...
      item,
      itemNumber,
      Object.keys(testValue)[0] || undefined
    );
    return;
  }

  chartType.list = Object.keys(testValue).map(
    (type) => g_SummaryTypes[type].caption
  );
...
function updateChart(number, category, item, itemNumber, type) {
  if (!g_ScorePanelsData[category].counters[itemNumber].fn) return;
  let chart = Engine.GetGUIObjectByName("chart[" + number + "]");
  chart.format_y =
    g_ScorePanelsData[category].headings[itemNumber + 1].format || "INTEGER";
  Engine.GetGUIObjectByName("chart[" + number + "]XAxisLabel").caption =
    translate("Time elapsed");
  ...
  if (Engine.GetGUIObjectByName("toggleTeamBox").checked)
    for (let team in g_Teams) {
      ...
      for (let index in g_GameData.sim.playerStates[1].sequences.time) {
        let value = g_ScorePanelsData[category].teamCounterFn(
          team,
          index,
          item,
          g_ScorePanelsData[category].counters,
          g_ScorePanelsData[category].headings
        );
        if (type) value = value[type];
        data.push({
          x: g_GameData.sim.playerStates[1].sequences.time[index],
          y: value,
        });
      }
      series.push(data);
    ...
    for (let j = 1; j <= g_PlayerCount; ++j) {
      ...
      for (let index in playerState.sequences.time) {
        let value = g_ScorePanelsData[category].counters[itemNumber].fn(
          playerState,
          index,
          item
        );
...
function updatePanelData(panelInfo) {
  ...
  for (let i = 0; i < g_PlayerCount; ++i) {
    ...
    else playerBoxesCounts[playerState.team + 1] += 1;
    ...
    if (playerState.team != -1) {
      rowPlayer =
        "playerBoxt[" + playerState.team + "][" + positionObject + "]";
      playerOutcome =
        "playerOutcomet[" + playerState.team + "][" + positionObject + "]";
      playerNameColumn =
        "playerNamet[" + playerState.team + "][" + positionObject + "]";
      playerCivicBoxColumn =
        "civIcont[" + playerState.team + "][" + positionObject + "]";
      playerCounterValue =
        "valueDataTeam[" + playerState.team + "][" + positionObject + "]";
    }
    ...
    playerNameColumn.tooltip = translateAISettings(
      g_GameData.sim.mapSettings.PlayerData[i + 1]
    );
...
function continueButton() {
  ...
  else if (g_GameData.gui.dialog) Engine.PopGuiPage();
  ...
  else Engine.SwitchGuiPage("page_pregame.xml");
}
...
function initGUILabels() {
  let assignedState =
    g_GameData.sim.playerStates[g_GameData.gui.assignedPlayer || -1];

  Engine.GetGUIObjectByName("summaryText").caption = g_GameData.gui.isInGame
  ...
  let mapType = g_Settings.MapTypes.find(
    (type) => type.Name == g_GameData.sim.mapSettings.mapType
  );
  let mapSize = g_Settings.MapSizes.find(
    (size) => size.Tiles == g_GameData.sim.mapSettings.Size || 0
  );
...
function initGUIButtons() {
  ...
  replayButton.hidden =
    g_GameData.gui.isInGame || !g_GameData.gui.replayDirectory;

  let lobbyButton = Engine.GetGUIObjectByName("lobbyButton");
  lobbyButton.tooltip = colorizeHotkey(
    translate("%(hotkey)s: Toggle the multiplayer lobby in a dialog window."),
    "lobby"
  );
  ...
  lobbyButtonSize.right =
    (replayButton.hidden
      ? Engine.GetGUIObjectByName("continueButton").size.left
      : replayButton.size.left) - 10;
  ...
  for (let tab in allPanelsData)
    allPanelsData[tab].tooltip =
      sprintf(translate("Toggle the %(name)s summary tab."), {
        name: allPanelsData[tab].label,
      }) +
      colorizeHotkey(
        "\n" + translate("Use %(hotkey)s to move a summary tab right."),
        "tab.next"
      ) +
      colorizeHotkey(
        "\n" + translate("Use %(hotkey)s to move a summary tab left."),
        "tab.prev"
      );
...
function initTeamData() {
  ...
  if (g_GameData.sim.mapSettings.LockTeams) {
    ...
    for (let player = 1; player <= g_PlayerCount; ++player) {
      ...
      if (!g_Teams[playerTeam]) g_Teams[playerTeam] = [];
      g_Teams[playerTeam].push(player);
    }

    if (g_Teams.every((team) => team && team.length < 2)) g_Teams = false; // Each player has his own team. Displaying teams makes no sense.
  } else g_Teams = false;

...
function getPlayerValuesPerTeam(team, index, type, counters, headings) {
  let fn =
    counters[headings.map((heading) => heading.identifier).indexOf(type) - 1]
      .fn;
  return g_Teams[team].map((player) =>
    fn(g_GameData.sim.playerStates[player], index, type)
  );
}

function updateCountersPlayer(
  playerState,
  allCounters,
  allHeadings,
  idGUI,
  ...
  for (let n in counters) {
    ...
    Engine.GetGUIObjectByName(idGUI + "[" + n + "]").caption =
      formatSummaryValue(
        fn && fn(playerState, index, headings[+n + 1].identifier)
      );
  }
}
  ...
  for (let team in g_Teams) {
    ...
    for (let n in counters)
      Engine.GetGUIObjectByName(
        "valueDataTeam[" + team + "][" + n + "]"
      ).caption = formatSummaryValue(
        teamFn(team, index, headings[+n + 1].identifier, counters, headings)
      );
  }
}
...
function summaryAddObject(obj1, obj2) {
  for (let p in obj1) obj1[p] += obj2[p];
}
...
function summaryArraySum(array) {
  return array.reduce((sum, val) => {
    if (typeof sum != "object") return sum + val;
  ...
  for (let i = 0; i < g_PlayerCount; ++i) {
    ...
    if (!g_TeamHelperData[playerState.team]) {
      ...
      for (let value of [
        ...
        g_TeamHelperData[playerState.team][value] = new Array(
          playerState.sequences.time.length
        ).fill(0);
    }

    summaryAddObject(
      g_TeamHelperData[playerState.team].food,
      playerState.sequences.resourcesGathered.food
    );
    summaryAddObject(
      g_TeamHelperData[playerState.team].vegetarianFood,
      playerState.sequences.resourcesGathered.vegetarianFood
    );

    summaryAddObject(
      g_TeamHelperData[playerState.team].femaleCitizen,
      playerState.sequences.unitsTrained.FemaleCitizen
    );
    summaryAddObject(
      g_TeamHelperData[playerState.team].worker,
      playerState.sequences.unitsTrained.Worker
    );

    summaryAddObject(
      g_TeamHelperData[playerState.team].enemyUnitsKilled,
      playerState.sequences.enemyUnitsKilled.total
    );
    summaryAddObject(
      g_TeamHelperData[playerState.team].unitsLost,
      playerState.sequences.unitsLost.total
    );

    g_TeamHelperData[playerState.team].mapControl =
      playerState.sequences.teamPercentMapControlled;
    g_TeamHelperData[playerState.team].mapControlPeak =
      playerState.sequences.teamPeakPercentMapControlled;
    ...
    for (let type in playerState.sequences.resourcesBought)
      summaryAddObject(
        g_TeamHelperData[playerState.team].totalBought,
        playerState.sequences.resourcesBought[type]
      );

    for (let type in playerState.sequences.resourcesSold)
      summaryAddObject(
        g_TeamHelperData[playerState.team].totalSold,
        playerState.sequences.resourcesSold[type]
      );
  }
}
...
function calculateScoreTeam(team, index, type, counters, headings) {
  ...
  if (type == "totalScore")
    return ["economyScore", "militaryScore", "explorationScore"]
      .map((t) => calculateScoreTeam(team, index, t, counters, headings))
      .reduce((sum, value) => sum + value);

  return summaryArraySum(
    getPlayerValuesPerTeam(team, index, type, counters, headings)
  );
}
...
function calculateResources(playerState, index, type) {
  return {
    ...
    used:
      playerState.sequences.resourcesUsed[type][index] -
      playerState.sequences.resourcesSold[type][index],
  };
}
...
function calculateMarketTeam(team, index, type, counters, headings) {
  if (type == "barterEfficency")
    return calculatePercent(
      g_TeamHelperData[team].totalBought[index],
      g_TeamHelperData[team].totalSold[index]
    );
...
function calculateMiscellaneousTeam(team, index, type, counters, headings) {
  if (type == "vegetarianRatio")
    return calculatePercent(
      g_TeamHelperData[team].vegetarianFood[index],
      g_TeamHelperData[team].food[index]
    );

  if (type == "feminization")
    return calculatePercent(
      g_TeamHelperData[team].femaleCitizen[index],
      g_TeamHelperData[team].worker[index]
    );

  if (type == "killDeath")
    return calculateRatio(
      g_TeamHelperData[team].enemyUnitsKilled[index],
      g_TeamHelperData[team].unitsLost[index]
    );
